#!/bin/sh
# Perform install/upgrade/uninstall for runtime package
...
install_run() {
    ...
    if [ "$1" = "install" ]; then
        ...
        if [ $? -eq 0 ]; then
            update_version_info_version
            if [ "$hetero_arch" = "y" ]; then
                create_acllib_softlink "${runtime_install_path_param}"
                create_modeldeployer_softlink "${runtime_install_path_param}"
            else
                create_acllib_softlink "${runtime_install_path_param}/${pkg_version_dir}"
                create_modeldeployer_softlink "${runtime_install_path_param}/${pkg_version_dir}"
            fi
...
uninstall_run() {
    ...
    if [ "$1" = "uninstall" ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            if [ "$hetero_arch" = "y" ]; then
                remove_acllib_softlink "${runtime_install_path_param}"
                remove_modeldeployer_softlink "${runtime_install_path_param}"
            else
                local version_dir="$([ -n "$pkg_version_dir" ] && basename $(dirname $upgrade_default_dir))"
                remove_acllib_softlink "${runtime_install_path_param}/${version_dir}"
                remove_modeldeployer_softlink "${runtime_install_path_param}/${version_dir}"
            fi
...
if [ "$pkg_is_multi_version" = "true" ] && [ "$hetero_arch" != "y" ]; then
    default_dir="${pkg_install_path}/$pkg_version_dir/runtime"
else
    ...
    # 升级场景
        ...
        if [ -n "$pkg_version_dir" ]; then
            if [ "$hetero_arch" = "y" ]; then
                ...
                get_package_upgrade_install_info "upgrade_install_info" "$pkg_install_path" "runtime"
            fi
