...
set hetero_arch=`grep -i "Compiler_Hetero_Arch_Flag=" "$install_info" | cut --only-delimited -d"=" -f2`
if ( "$2" == "multi_version" ) then
    if ( "$hetero_arch" == "y" ) then
        set INSTALL_DIR="`realpath ${CURPATH}/../../../../../latest`/compiler"
    else
        set INSTALL_DIR="`realpath ${CURPATH}/../../../latest`/compiler"
    endif
else
...
if ( -d "${INSTALL_DIR}/ccec_compiler/bin" && -d "${INSTALL_DIR}/bin" ) then
    set p_path="${INSTALL_DIR}/ccec_compiler/bin:${INSTALL_DIR}/bin"
    set path_tmp="$PATH"
    set num=`echo ":${path_tmp}:" | grep ":${p_path}:" | wc -l`
    if ( "$num" == 0 ) then
        if ( "-${path_tmp}" == "-" ) then
            setenv PATH "${p_path}"
        else
            setenv PATH "${p_path}:${path_tmp}"
        endif
    endif
endif
