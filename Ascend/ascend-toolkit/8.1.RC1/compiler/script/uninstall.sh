...
log_operation() {
    ...
    echo "${RUN_CMD_TYPE} ${LEVEL} ${RUN_USERNAME} ${cur_date} ${HOST} compiler $2 installmode=${RUN_CMD}; cmdlist=--${RUN_CMD}" >> "${OPERATION_LOG_FILE}"
}
...
user_auth() {
    ...
    if [ "${run_user_id}" -ne 0 ]; then
        if [ "${run_user_id}" -ne "${dir_user_id}" ]; then
            err_no0x0093 "Current user is not supported to ${RUN_CMD} the compiler package"
        fi
    fi
}
...
hetero_arch=$(get_install_param "Compiler_Hetero_Arch_Flag" "${ASCEND_INSTALL_INFO_FILE}")
export hetero_arch
...
uninstall_run() {
    ...
    local compiler_install_path_param="$(get_pkg_install_path_param)"
    local install_top_path="$(get_install_top_path)"

    if [ -f "$ASCEND_INSTALL_INFO_FILE" ]; then
        local compiler_install_type=$(get_install_param "Compiler_Install_Type" "${ASCEND_INSTALL_INFO_FILE}")
    elif [ -f "${ASCEND_INSTALL_INFO_OLD_FILE}" ]; then
        num=$(grep -c -i compiler_install_path_param "${ASCEND_INSTALL_INFO_OLD_FILE}")
        if [ "${num}" != "0" ]; then
            local compiler_install_type="$(grep -iw compiler_install_type "$ASCEND_INSTALL_INFO_OLD_FILE" | cut -d"=" -f2-)"
        fi
    else
        err_no0x0080 "please complete ${ASCEND_INSTALL_INFO_FILE} or ${ASCEND_INSTALL_INFO_OLD_FILE}"
    fi
    if [ $? -eq 0 ]; then
        log "INFO" "${RUN_CMD} ${compiler_install_path_param} ${compiler_install_type}"
        bash "${UNINSTALL_SHELL}" "${RUN_CMD}" "${compiler_install_path_param}" "${compiler_install_type}" "${IS_QUIET}" "n" "" "y"
        if [ $? -eq 0 ]; then
            ...
            if [ $? -eq 0 ] && [ -f "${ASCEND_INSTALL_INFO_OLD_FILE}" ] && [ -w "${ASCEND_INSTALL_INFO_OLD_FILE}" ] && [ "${num}" != "0" ]; then
                sed -i '/compiler_install_path_param=/Id' "${ASCEND_INSTALL_INFO_OLD_FILE}"
                sed -i '/compiler_install_type=/Id' "${ASCEND_INSTALL_INFO_OLD_FILE}"
            fi
            if [ "$hetero_arch" = "y" ]; then
...
if [ "$hetero_arch" != "y" ]; then
    arch_path="$(dirname "$install_path_param")/$arch_scripts_path_hetero/compiler/script/uninstall.sh"
    ret=0
    if [ -f "$arch_path" ]; then
        sh "$arch_path"
        ret=$?
    elif [ "$in_hetero_arch" = "y" ]; then
        log "WARNING" "no hetero arch compiler package installed!"
    fi
