#!/bin/bash
# Perform install for compiler package
...
##########################################################################
log "INFO" "step into run_compiler_install.sh ......"
...
new_install() {
    if [ ! -d "${sourcedir}" ]; then
        log "INFO" "no need to install compiler files."
        return 0
    fi
    output_progress 10
    ...
    sh "$curpath/install_common_parser.sh" --package="compiler" --install --username="$username" --usergroup="$usergroup" --set-cann-uninstall \
