...
SOURCE_INSTALL_COMMON_PARSER_FILE="${common_parse_dir}/compiler/script/install_common_parser.sh"
SOURCE_FILELIST_FILE="${common_parse_dir}/compiler/script/filelist.csv"
...
install_info="${common_parse_dir}/compiler/ascend_install.info"
if [ -f "$install_info" ]; then
    hetero_arch=$(get_install_param "Compiler_Hetero_Arch_Flag" "${install_info}")
fi
...
##########################################################################
log "INFO" "step into run_compiler_uninstall.sh ......"
log "INFO" "uninstall target dir $common_parse_dir, type $common_parse_type."

if [ ! -d "$common_parse_dir/compiler" ]; then
    log "ERROR" "ERR_NO:0x0001;ERR_DES:path $common_parse_dir/compiler is not exist."
    exit 1
fi

new_uninstall() {
    if [ -f "${common_parse_dir}/compiler/data/version.info" ]; then
        log "INFO" "need to uninstall costmodel files."
        bash "${common_parse_dir}/compiler/data/script/install.sh" -- -- --uninstall --install-path="${common_parse_dir}"
    fi

    if [ ! -d "${common_parse_dir}/compiler" ]; then
        log "INFO" "no need to uninstall compiler files."
        return 0
    fi
    ...
    sh "$SOURCE_INSTALL_COMMON_PARSER_FILE" --package="compiler" --uninstall --username="$username" --usergroup="$usergroup" ${recreate_softlink_option} \
