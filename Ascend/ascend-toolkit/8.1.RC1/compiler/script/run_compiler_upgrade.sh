#!/bin/bash
# Perform upgrade for compiler package
...
if [ -f "$install_info" ]; then
    chip_type=$(get_install_param "Compiler_Chip_Type" "${install_info}")
    feature_type=$(get_install_param "Compiler_Feature_Type" "${install_info}")
...
##########################################################################
log "INFO" "step into run_compiler_upgrade.sh ......"
...
new_upgrade() {
    if [ ! -d "${sourcedir}" ]; then
        log "INFO" "no need to upgrade compiler files."
        return 0
    fi
    output_progress 10
    ...
    sh "$curpath/install_common_parser.sh" --package="compiler" --install --username="$username" --usergroup="$usergroup" --set-cann-uninstall --upgrade \
