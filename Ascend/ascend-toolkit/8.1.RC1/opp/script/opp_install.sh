...
# minios opp
PKG_FILE="${version_install_dir}/opp/built-in/Ascend-opp_rt-minios.aarch64.tar.gz"
INSTALL_MINIOS_DIR="${version_install_dir}/temp_minios/"
if [ -f ${PKG_FILE} ]; then
    ...
    NEW_PKG_FILE="${version_install_dir}/temp_minios/Ascend-opp_rt_without_head-minios.aarch64.tar.gz"
    ...
    if [ $? -ne 0 ]; then
        ...
        FOLDING_DIR="${version_install_dir}/temp_minios/opp/built-in/op_impl/host_cpu/"
        ...
        cp -rfL ${version_install_dir}/temp_minios/opp ${version_install_dir}/
        if [ $? -eq 0 ]; then
...
[ export ASCEND_OPP_PATH=${_TARGET_INSTALL_PATH}/latest/${opp_platform_dir} ] to set the environment path."
fi
