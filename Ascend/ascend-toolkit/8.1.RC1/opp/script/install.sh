...
opp_old_platform_dir=opp_$platform_data-linux
...
OPP_COMPATIBILITY_CEHCK_ERR_DES="Opp compatibility check error."
PERM_DENIED="0x0093"
...
startlog() {
    echo "[Opp] [$(getdate)] [INFO]: Start Time: $(getdate)"
}

exitlog() {
    echo "[Opp] [$(getdate)] [INFO]: End Time: $(getdate)"
}
...
module, are you sure to keep installing opp module in it? y"
            ...
            if [ ! -f  "${_opp_sub_dir}""/ascend_install.info" ]; then
                ...
                logandprint "[INFO]: Opp package has been installed on the path $(getinstalledinfo "${KEY_INSTALLED_PATH}"), \
...
check_docker_path(){
    docker_path="$1"
    if [ "${docker_path}" != "/"* ]; then
        echo "[Opp] [ERROR]: ERR_NO:${PARAM_INVALID};ERR_DES:Parameter --docker-root \
must with absolute path that which is start with root directory /. Such as --docker-root=/${docker_path}"
        exitlog
        exit 1
    fi
    if [ ! -d "${docker_path}" ]; then
        echo "[Opp] [ERROR]: ERR_NO:${FILE_NOT_EXIST}; The directory:${docker_path} not exist, please create this directory."
        exitlog
        exit 1
    fi
}
...
judgment_path() {
    ...
    if [ $? -ne 0 ]; then
        echo "[Opp][ERROR]: The opp install path ${1} is invalid, only characters in [a-z,A-Z,0-9,-,_] are supported!"
        exitlog
        exit 1
    fi
}
...
check_install_path(){
    ...
    if [ "x${in_install_path_param}" = "x" ]; then
        echo "[Opp] [ERROR]: ERR_NO:${PARAM_INVALID};ERR_DES:Parameter ${param_name} \
...
not support that the install path contains space character."
    ...
    if [ x"${prefix}" = "x" ]; then
        ...
        if [ x"${prefix}" = "x" ]; then
            ...
            echo "[Opp] [ERROR]: ERR_NO:${PARAM_INVALID};ERR_DES: Run package path is invalid: $run_path"
            exitlog
            exit 1
        fi
    fi
...
switchchip(){
    if [ "$tmp_chip_type" = "Ascend910" ];then
       ...
       echo "[Opp] [ERROR]: ERR_NO:${PARAM_INVALID};ERR_DES:Parameter --chip \
not support that $tmp_chipp_type"
        exitlog
        exit 1
    fi
}
...
repairaicpu(){
    ...
    if [ "$in_feature" = "All" ];then
        if [ "$is_for_all" = "y" ];then
            ...
            chmod 444 $1/${opp_platform_dir}/Ascend310RC/aicpu/*.info
            chmod 444 $1/${opp_platform_dir}/Ascend310/aicpu/*.info
            chmod 444 $1/${opp_platform_dir}/Ascend310P/aicpu/*.info
            chmod 444 $1/${opp_platform_dir}/Ascend910/aicpu/*.info
            if [ -d "$1/${opp_platform_dir}/Ascend/" ]; then
                chmod 444 $1/${opp_platform_dir}/Ascend/aicpu/*.info
            fi
        fi
        if [ $(id -u) -ne 0 ];then
...
aicpuinfofile(){
    if [ "$1" = "add" ];then
        if [ "$in_feature" = "All" ];then
            \cp ${target_dir}/${opp_platform_dir}/version.info ${target_dir}/${opp_platform_dir}/Ascend310/aicpu/  > /dev/null 2>&1
            versioninfoadd "${target_dir}/${opp_platform_dir}/Ascend310/aicpu/version.info"
            \cp ${target_dir}/${opp_platform_dir}/version.info ${target_dir}/${opp_platform_dir}/Ascend310RC/aicpu/   > /dev/null 2>&1
            versioninfoadd "${target_dir}/${opp_platform_dir}/Ascend310RC/aicpu/version.info"
            \cp ${target_dir}/${opp_platform_dir}/version.info ${target_dir}/${opp_platform_dir}/Ascend910/aicpu/  > /dev/null 2>&1
            versioninfoadd "${target_dir}/${opp_platform_dir}/Ascend910/aicpu/version.info"
            \cp ${target_dir}/${opp_platform_dir}/version.info ${target_dir}/${opp_platform_dir}/Ascend310P/aicpu/   > /dev/null 2>&1
            versioninfoadd "${target_dir}/${opp_platform_dir}/Ascend310P/aicpu/version.info"
            if [ -d "${target_dir}/${opp_platform_dir}/Ascend/" ]; then
                \cp ${target_dir}/${opp_platform_dir}/version.info ${target_dir}/${opp_platform_dir}/Ascend/aicpu/  > /dev/null 2>&1
                versioninfoadd "${target_dir}/${opp_platform_dir}/Ascend/aicpu/version.info"
            fi
            \cp ${target_dir}/${opp_platform_dir}/scene.info ${target_dir}/${opp_platform_dir}/Ascend310/aicpu/  > /dev/null 2>&1
            \cp ${target_dir}/${opp_platform_dir}/scene.info ${target_dir}/${opp_platform_dir}/Ascend310RC/aicpu/   > /dev/null 2>&1
            \cp ${target_dir}/${opp_platform_dir}/scene.info ${target_dir}/${opp_platform_dir}/Ascend910/aicpu/  > /dev/null 2>&1
            \cp ${target_dir}/${opp_platform_dir}/scene.info ${target_dir}/${opp_platform_dir}/Ascend310P/aicpu/   > /dev/null 2>&1
            if [ -d "${target_dir}/${opp_platform_dir}/Ascend/" ]; then
                \cp ${target_dir}/${opp_platform_dir}/scene.info ${target_dir}/${opp_platform_dir}/Ascend/aicpu/  > /dev/null 2>&1
            fi
            if [ $(id -u) -eq 0 ];then
               chmod 755 ${target_dir}/${opp_platform_dir}/Ascend310P > /dev/null 2>&1
               chmod 755 ${target_dir}/${opp_platform_dir}/Ascend910 > /dev/null 2>&1
               if [ -d "${target_dir}/${opp_platform_dir}/Ascend/" ]; then
                chmod 755 ${target_dir}/${opp_platform_dir}/Ascend > /dev/null 2>&1
               fi
               chmod 755 ${target_dir}/${opp_platform_dir}/Ascend310 > /dev/null 2>&1
               chmod 755 ${target_dir}/${opp_platform_dir}/Ascend310RC > /dev/null 2>&1
               chmod 755 ${target_dir}/${opp_platform_dir}/Ascend310P/aicpu > /dev/null 2>&1
               chmod 755 ${target_dir}/${opp_platform_dir}/Ascend910/aicpu > /dev/null 2>&1
               if [ -d "${target_dir}/${opp_platform_dir}/Ascend/" ]; then
                chmod 755 ${target_dir}/${opp_platform_dir}/Ascend/aicpu > /dev/null 2>&1
               fi
               chmod 755 ${target_dir}/${opp_platform_dir}/Ascend310/aicpu > /dev/null 2>&1
               chmod 755 ${target_dir}/${opp_platform_dir}/Ascend310RC/aicpu  > /dev/null 2>&1
               chown  "${_DEFAULT_USERNAME}":"${_DEFAULT_USERGROUP}" ${target_dir}/${opp_platform_dir}/Ascend310/aicpu/version.info > /dev/null 2>&1
               chown  "${_DEFAULT_USERNAME}":"${_DEFAULT_USERGROUP}" ${target_dir}/${opp_platform_dir}/Ascend310RC/aicpu/version.info > /dev/null 2>&1
               chown  "${_DEFAULT_USERNAME}":"${_DEFAULT_USERGROUP}" ${target_dir}/${opp_platform_dir}/Ascend310P/aicpu/version.info > /dev/null 2>&1
               chown  "${_DEFAULT_USERNAME}":"${_DEFAULT_USERGROUP}" ${target_dir}/${opp_platform_dir}/Ascend910/aicpu/version.info > /dev/null 2>&1
               if [ -d "${target_dir}/opp/Ascend/" ]; then
                chown  "${_DEFAULT_USERNAME}":"${_DEFAULT_USERGROUP}" ${target_dir}/${opp_platform_dir}/Ascend/aicpu/version.info > /dev/null 2>&1
               fi
               chown  "${_DEFAULT_USERNAME}":"${_DEFAULT_USERGROUP}" ${target_dir}/${opp_platform_dir}/Ascend310/aicpu/scene.info > /dev/null 2>&1
               chown "${_DEFAULT_USERNAME}":"${_DEFAULT_USERGROUP}" ${target_dir}/${opp_platform_dir}/Ascend310RC/aicpu/scene.info > /dev/null 2>&1
               chown  "${_DEFAULT_USERNAME}":"${_DEFAULT_USERGROUP}" ${target_dir}/${opp_platform_dir}/Ascend310P/aicpu/scene.info > /dev/null 2>&1
               chown  "${_DEFAULT_USERNAME}":"${_DEFAULT_USERGROUP}" ${target_dir}/${opp_platform_dir}/Ascend910/aicpu/scene.info > /dev/null 2>&1
               if [ -d "${target_dir}/opp/Ascend/" ]; then
                chown  "${_DEFAULT_USERNAME}":"${_DEFAULT_USERGROUP}" ${target_dir}/${opp_platform_dir}/Ascend/aicpu/scene.info > /dev/null 2>&1
               fi
            else
                chmod 755 ${target_dir}/${opp_platform_dir}/Ascend310P > /dev/null 2>&1
                chmod 755 ${target_dir}/${opp_platform_dir}/Ascend910 > /dev/null 2>&1
            ...
            \cp ${target_dir}/${opp_platform_dir}/version.info ${target_dir}/${opp_platform_dir}/$in_feature/aicpu/  > /dev/null 2>&1
            versioninfoadd "${target_dir}/${opp_platform_dir}/$in_feature/aicpu/version.info"
            \cp ${target_dir}/${opp_platform_dir}/scene.info ${target_dir}/${opp_platform_dir}/$in_feature/aicpu/  > /dev/null 2>&1
            if [ $(id -u) -eq 0 ];then
                chmod 755 ${target_dir}/${opp_platform_dir}/$in_feature
                chmod 755 ${target_dir}/${opp_platform_dir}/$in_feature/aicpu
                chown "${_DEFAULT_USERNAME}":"${_DEFAULT_USERGROUP}" ${target_dir}/${opp_platform_dir}/$in_feature/aicpu/scene.info > /dev/null 2>&1
                chown "${_DEFAULT_USERNAME}":"${_DEFAULT_USERGROUP}" ${target_dir}/${opp_platform_dir}/$in_feature/aicpu/version.info > /dev/null 2>&1
            else
                chmod 750 ${target_dir}/${opp_platform_dir}/$in_feature
                chmod 750 ${target_dir}/${opp_platform_dir}/$in_feature/aicpu
            fi
        fi
        num=550
        if [ "$is_for_all" = "y" ];then
        ...
        uname -m | grep x86 > /dev/null 2>&1
        if [ $? -eq 0 ];then
            chmod 755 ${target_dir}/${opp_platform_dir}/built-in/op_impl/aicpu/aicpu_kernel/lib  > /dev/null 2>&1
            chmod -R 755 ${target_dir}/${opp_platform_dir}/built-in/op_impl/aicpu/aicpu_kernel/lib/aarch64  > /dev/null 2>&1
            rm -rf ${target_dir}/${opp_platform_dir}/built-in/op_impl/aicpu/aicpu_kernel/lib/aarch64 > /dev/null 2>&1
        fi
        uname -m | grep aarch64 > /dev/null 2>&1
        if [ $? -eq 0 ];then
            chmod 755 ${target_dir}/${opp_platform_dir}/built-in/op_impl/aicpu/aicpu_kernel/lib  > /dev/null 2>&1
            chmod -R 755 ${target_dir}/${opp_platform_dir}/built-in/op_impl/aicpu/aicpu_kernel/lib/x86  > /dev/null 2>&1
            rm -rf ${target_dir}/${opp_platform_dir}/built-in/op_impl/aicpu/aicpu_kernel/lib/x86 > /dev/null 2>&1
        fi
        chmod $num ${target_dir}/${opp_platform_dir}/built-in/op_impl/aicpu/aicpu_kernel/lib  > /dev/null 2>&1
    else
        if [ "$in_feature" = "All" ];then
            if [ $(id -u) -ne 0 ];then
                chmod 750 ${target_dir}/${opp_platform_dir}/Ascend310P > /dev/null 2>&1
                chmod 750 ${target_dir}/${opp_platform_dir}/Ascend910 > /dev/null 2>&1
                if [ -d "${target_dir}/${opp_platform_dir}/Ascend/" ]; then
                    chmod 750 ${target_dir}/${opp_platform_dir}/Ascend > /dev/null 2>&1
                fi
                chmod 750 ${target_dir}/${opp_platform_dir}/Ascend310 > /dev/null 2>&1
                chmod 750 ${target_dir}/${opp_platform_dir}/Ascend310RC > /dev/null 2>&1
                chmod 750 ${target_dir}/${opp_platform_dir}/Ascend310P/aicpu > /dev/null 2>&1
                chmod 750 ${target_dir}/${opp_platform_dir}/Ascend910/aicpu > /dev/null 2>&1
                if [ -d "${target_dir}/${opp_platform_dir}/Ascend/" ]; then
                    chmod 750 ${target_dir}/${opp_platform_dir}/Ascend/aicpu > /dev/null 2>&1
                fi
                chmod 750 ${target_dir}/${opp_platform_dir}/Ascend310/aicpu > /dev/null 2>&1
                chmod 750 ${target_dir}/${opp_platform_dir}/Ascend310RC/aicpu > /dev/null 2>&1
            fi
            rm -rf  ${target_dir}/${opp_platform_dir}/Ascend310/aicpu/version.info  > /dev/null 2>&1
            rm -rf  ${target_dir}/${opp_platform_dir}/Ascend310RC/aicpu/version.info   > /dev/null 2>&1
            rm -rf  ${target_dir}/${opp_platform_dir}/Ascend910/aicpu/version.info  > /dev/null 2>&1
            if [ -d "${target_dir}/${opp_platform_dir}/Ascend/" ]; then
                rm -rf  ${target_dir}/${opp_platform_dir}/Ascend/aicpu/version.info  > /dev/null 2>&1
            fi
            rm -rf  ${target_dir}/${opp_platform_dir}/Ascend310P/aicpu/version.info   > /dev/null 2>&1
            rm -rf  ${target_dir}/${opp_platform_dir}/Ascend310/aicpu/scene.info  > /dev/null 2>&1
            rm -rf  ${target_dir}/${opp_platform_dir}/Ascend310RC/aicpu/scene.info   > /dev/null 2>&1
            rm -rf  ${target_dir}/${opp_platform_dir}/Ascend910/aicpu/scene.info  > /dev/null 2>&1
            if [ -d "${target_dir}/${opp_platform_dir}/Ascend/" ]; then
                rm -rf  ${target_dir}/${opp_platform_dir}/Ascend/aicpu/scene.info  > /dev/null 2>&1
            fi
            rm -rf  ${target_dir}/${opp_platform_dir}/Ascend310P/aicpu/scene.info   > /dev/null 2>&1
            rm -rf  ${target_dir}/${opp_platform_dir}/Ascend310/aicpu/ascend_install.info  > /dev/null 2>&1
            rm -rf  ${target_dir}/${opp_platform_dir}/Ascend310RC/aicpu/ascend_install.info   > /dev/null 2>&1
            rm -rf  ${target_dir}/${opp_platform_dir}/Ascend910/aicpu/ascend_install.info  > /dev/null 2>&1
            if [ -d "${target_dir}/${opp_platform_dir}/Ascend/" ]; then
                rm -rf  ${target_dir}/${opp_platform_dir}/Ascend/aicpu/ascend_install.info  > /dev/null 2>&1
            fi
            rm -rf  ${target_dir}/${opp_platform_dir}/Ascend310P/aicpu/ascend_install.info   > /dev/null 2>&1
        elif [ "$in_feature" = "exec_mode" ];then
            logandprint "[INFO]: Install only files required at runtime (not compile time), include [libopsproto.so] and [liboptiling.so]"
        else
            if [ $(id -u) -ne 0 ];then
                chmod 750 ${target_dir}/${opp_platform_dir}/$in_feature > /dev/null 2>&1
                chmod 750 ${target_dir}/${opp_platform_dir}/$in_feature/aicpu > /dev/null 2>&1
            fi
            rm -rf ${target_dir}/${opp_platform_dir}/$in_feature/aicpu/version.info > /dev/null 2>&1
            rm -rf ${target_dir}/${opp_platform_dir}/$in_feature/aicpu/scene.info  > /dev/null 2>&1
            rm -rf ${target_dir}/${opp_platform_dir}/$in_feature/aicpu/ascend_install.info  > /dev/null 2>&1
        fi
        uname -m | grep x86 > /dev/null 2>&1
        if [ $? -eq 0 ];then
            chmod 755 ${target_dir}/${opp_platform_dir}/built-in/op_impl/aicpu/aicpu_kernel/lib  > /dev/null 2>&1
...
(${_VERSION_INFO_FILE}) not exists or without execute permission."
    ...
    --chip=*)
        ...
        echo "[Opp] [INFO]: You are installing the ${tmp_chip_type} content. Please check whether current installing chip is \
consistent with soc_version. If the content is inconsistent, the execution may fail."
        if test -z "$tmp_chip_type"; then
            echo "[Opp] [ERROR]: ERR_NO:${PARAM_INVALID};ERR_DES:Paramter --chip cannot be null."
            exitlog
            exit 1
        fi
        if [ "${is_uninstall}" = "y" ]; then
            echo "[Opp] [ERROR]: ERR_NO:${PARAM_INVALID};ERR_DES:Paramter --chip is not supported to used by this way. please use with \
'--full', '--devel', '--run', '--upgrade'."
    ...
    --feature=*)
        ...
        if test -z "$feature_choice"; then
            echo "[Opp] [ERROR]: ERR_NO:${PARAM_INVALID};ERR_DES:Paramter --feature cannot be null."
            exitlog
            exit 1
        fi
        contain_feature "ret" "$feature_choice" "${_FILELIST_FILE}"
        if [ "$ret" = "false" ]; then
            log "WARNING" "Opp package doesn't contain features $feature_choice, skip installation."
            exit 0
        fi
        is_feature=y
        shift
	...
	docker_root=$(echo $1 | cut -d"=" -f2 )
        ...
        ;;
    -*)
        echo "[Opp] [ERROR]: ERR_NO:${PARAM_INVALID};ERR_DES:Unsupported parameters [$1], \
...
if [ "${iter_i}" != 1 ]; then
    echo "[Opp] [ERROR]: ERR_NO:${PARAM_INVALID};ERR_DES:only support one type: full/run/devel/upgrade/uninstall/check, operation execute failed! \
...
if [ "$?" = "0" ]; then
    echo "[Opp] [$(getdate)] [INFO]: Uninstall the version before multi version of opp successfully!"
else
    echo "[Opp] [$(getdate)] [ERROR]: Uninstall the version before multi version of opp failed!"
    exit 1
fi
if [ "${is_input_path}" = y ]; then
...
if [ "${is_install}" = "y" ];then
    ...
    if [ "${in_install_type}" != "devel" ]; then
        ...
        if [ "$?" != 0 ]; then
            logandprint "[ERROR]: ERR_NO:${INSTALL_FAILED};ERR_DES:Installation of opp\
...
user group (${_DEFAULT_USERGROUP}) for devel mode? [y/n]"
    ...
    # call opp_install.sh
    if [ -d "${target_dir}/opp" ] && [ ! -L "${target_dir}/opp" ] && [ -f "${target_dir}/opp/script/uninstall.sh" ]; then
        bash "${target_dir}/opp/script/uninstall.sh"
    fi
