...
private:
    ...
    __aicore__ inline void PointBilinearXInLocal(int32_t nIdx, int32_t hwIdx, int32_t calHWElems,
                                                 LocalTensor<int32_t> coordinatesUb, LocalTensor<float> weightUb,
                                                 LocalTensor<uint8_t> weightMaskUb, LocalTensor<float> outValueUb,
                                                 bool isAutomicAdd, LocalTensor<T> xLocal);
...
template <typename T>
__aicore__ inline void GridSampler2DFP16SlideWindow<T>::Init(GM_ADDR x, GM_ADDR gird, GM_ADDR y, GM_ADDR workspace,
                                              const GridSampleTilingData* tilingData)
{
...
template <typename T>
__aicore__ inline void GridSampler2DFP16SlideWindow<T>::ComputeWeightSub(LocalTensor<float> w1Ub, LocalTensor<float> w2Ub,
 ...
 */
template <typename T>
__aicore__ inline void GridSampler2DFP16SlideWindow<T>::ClipCoordinates(LocalTensor<float> iXFpUb, LocalTensor<float> iYFpUb,
                                                         LocalTensor<int32_t> iXIntUb, LocalTensor<int32_t> iYIntUb,
 ...
 */
template <typename T>
__aicore__ inline void GridSampler2DFP16SlideWindow<T>::ClipCoordinatesXInLocal(LocalTensor<float> iXFpUb, LocalTensor<float> iYFpUb,
                                                         LocalTensor<int32_t> iXIntUb, LocalTensor<int32_t> iYIntUb,
                                                         LocalTensor<int32_t> coorUb, LocalTensor<uint8_t> wMaskUb,
                                                         int32_t xMin, int32_t xMax, int32_t yMin, int32_t yMax)
{
    ...
    // 坐标一维化
    Muls(inputYIntTmpUb, inputYIntTmpUb, 
 ...
 */
template <typename T>
__aicore__ inline void GridSampler2DFP16SlideWindow<T>::CoordinatesFrameRange(LocalTensor<int32_t> iIntUb, int32_t upBound)
{
 ...
 */
template <typename T>
__aicore__ inline void GridSampler2DFP16SlideWindow<T>::CoordinatesGetMaskWithRange(
...
template <typename T>
__aicore__ inline void GridSampler2DFP16SlideWindow<T>::CoordinatesSelectTensor(LocalTensor<float> src0,
 ...
 */
template <typename T>
__aicore__ inline void GridSampler2DFP16SlideWindow<T>::ReflectClip(LocalTensor<float> iXFpUb, LocalTensor<float> iYFpUb) {
...
template <typename T>
__aicore__ inline void GridSampler2DFP16SlideWindow<T>::ReflectCoordinatesGeneral(
 ...
 */
template <typename T>
__aicore__ inline void GridSampler2DFP16SlideWindow<T>::MTE2ForNCHW(int32_t nIdx, int32_t cIdx, int32_t calCElems, int32_t channelAlign,
                                                     ...
                                                     LocalTensor<int32_t> coorUb, LocalTensor<T> xLocal)
{
    for (int32_t i = 0; i < loopElems; i++) {
        ...
        int64_t baseLocation = (int64_t)nIdx * inputC_ * inputH_ * inputW_ + coordVal + cIdx * CHANNEL_BLOCK * inputH_ * inputW_;
 ...
 */
template <typename T>
__aicore__ inline void GridSampler2DFP16SlideWindow<T>::MTE2ForNHWC(int32_t nIdx, int32_t cIdx, int32_t calCElems, int32_t channelAlign,
                                                     int32_t loopOffset, int32_t loopElems,
                                                     LocalTensor<int32_t> coorUb, LocalTensor<T> xLocal)
{
...
template <typename T>
__aicore__ inline void GridSampler2DFP16SlideWindow<T>::OutTranspose(int32_t channelAlign, LocalTensor<T> xLocal, LocalTensor<T> outValueUb)
{
...
template <typename T>
__aicore__ inline void GridSampler2DFP16SlideWindow<T>::MTE3ForNCHW(int32_t nIdx, int32_t cIdx, int32_t calCElems, int32_t channelAlign,
                                                     int32_t hwIdx, int32_t loopOffset, int32_t loopElems,
                                                     int64_t outBaseOffset, LocalTensor<float> weightUb,
                                                     LocalTensor<float> outValueUb, bool isAutomicAdd)
{
    ...
    for (int32_t loop_idx = 0; loop_idx < trans_loop; loop_idx++) {
        ...
        for (int32_t cIdx = 0; cIdx < channelLoop_; cIdx++) {
            ...
            if (calCElems >= 16) {
                ...
                pipe_barrier(PIPE_V);
                Select(outValueUb, dupUbU32, outValueUb, 0.0f, SELMODE::VSEL_TENSOR_SCALAR_MODE, 64, calCElems * (TRANSE_REP_STRIDE/64), repParams);
            } else {
 ...
 */
template <typename T>
__aicore__ inline void GridSampler2DFP16SlideWindow<T>::PointBilinearXInLocal(int32_t nIdx, int32_t hwIdx, int32_t calHWElems,
                                                       LocalTensor<int32_t> coordinatesUb, LocalTensor<float> weightUb,
                                                       LocalTensor<uint8_t> weightMaskUb, LocalTensor<float> outValueUb,
                                                       bool isAutomicAdd, LocalTensor<T> xLocal)
{
    ...
    for (size_t i = 0; i < inputC_; i++) {
        ...
        Select(outValueFp16Ub[ubOffset], weightMaskUbTmp, outValueFp16Ub[ubOffset], (half)0.0, SELMODE::VSEL_TENSOR_SCALAR_MODE, CAL_H_W_BLOCK);
    }
    pipe_barrier(PIPE_V);
    ...
    for (int32_t loop_idx = 0; loop_idx < trans_loop; loop_idx++) {
        ...
        Mul(outValueUb[loop_offset], outValueUb[loop_offset], weightUb[loop_offset], B32_MASK, inputC_, {1, 1, 1, repeatStride, repeatStride, 0});
    }
    event_t eventIdVToMte3 = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::V_MTE3));
    SetFlag<HardEvent::V_MTE3>(eventIdVToMte3);
    WaitFlag<HardEvent::V_MTE3>(eventIdVToMte3);
...
__aicore__ inline void GridSampler2DFP16SlideWindow<T>::PerLoopCompute(int32_t nIdx, int32_t hwIdx, int32_t calHWElems)
{
    ...
    if (!noSlideWindow) {
        ...
        // 滑框总面积超过UB大小
        noSlideWindow = noSlideWindow || Ceil(xMax - xMin + 1,
            B16_ALIGN_FACTOR) * B16_ALIGN_FACTOR * (yMax - yMin + 1) * inputC_ * sizeof(T) > X_UB_SIZE;
        // 滑窗只支持channel last场景
    ...
    SetFlag<HardEvent::MTE2_V>(eventIdMTE2_V);
    WaitFlag<HardEvent::MTE2_V>(eventIdMTE2_V);
    ClipCoordinatesXInLocal(inputXWFpLocal, inputYWFpLocal, inputXWIntLocal, inputYWIntLocal, coordinatesLocal, weightMaskUb, xMin, xMax, yMin, yMax);
    PointBilinearXInLocal(nIdx, hwIdx, calHWElems, coordinatesLocal, nwWeightLocal, weightMaskUb, outValueLocal, false, xLocal);
    ClipCoordinatesXInLocal(inputXEFpLocal, inputYWFpLocal, inputXEIntLocal, inputYWIntLocal, coordinatesLocal, weightMaskUb, xMin, xMax, yMin, yMax);
    PointBilinearXInLocal(nIdx, hwIdx, calHWElems, coordinatesLocal, neWeightLocal, weightMaskUb, outValueLocal, true, xLocal);
    ClipCoordinatesXInLocal(inputXWFpLocal, inputYEFpLocal, inputXWIntLocal, inputYEIntLocal, coordinatesLocal, weightMaskUb, xMin, xMax, yMin, yMax);
    PointBilinearXInLocal(nIdx, hwIdx, calHWElems, coordinatesLocal, swWeightLocal, weightMaskUb, outValueLocal, true, xLocal);
    ClipCoordinatesXInLocal(inputXEFpLocal, inputYEFpLocal, inputXEIntLocal, inputYEIntLocal, coordinatesLocal, weightMaskUb, xMin, xMax, yMin, yMax);
    PointBilinearXInLocal(nIdx, hwIdx, calHWElems, coordinatesLocal, seWeightLocal, weightMaskUb, outValueLocal, true, xLocal);
    pipe_barrier(PIPE_ALL);
    CopyOut(nIdx, hwIdx, calHWElems);
    pipe_barrier(PIPE_ALL);
    gridQueue_.FreeTensor(gridLocal);
