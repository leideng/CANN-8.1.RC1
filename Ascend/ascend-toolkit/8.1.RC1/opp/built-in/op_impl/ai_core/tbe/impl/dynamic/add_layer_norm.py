...
def _build_args(x1_in__, x2_in__, gamma_in__, beta_in__, bias_in__, y_out_, mean_out_, rstd_out_, x_out_, epsilon, additional_output):
    __inputs__ = []
    for arg in [x1_in__, x2_in__, gamma_in__, beta_in__, bias_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [y_out_, mean_out_, rstd_out_, x_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("AddLayerNorm", trans_bool_to_s8=False)
...
def add_layer_norm(x1_in__, x2_in__, gamma_in__, beta_in__, bias_in__=None, y_out_=None, mean_out_=None, rstd_out_=None, x_out_=None, epsilon=1e-05, additional_output=False, kernel_name="add_layer_norm", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, gamma_in__, beta_in__, bias_in__, y_out_, mean_out_, rstd_out_, x_out_, epsilon, additional_output)
    ...
    origin_func_name = "add_layer_norm"
    ascendc_src_dir_ex = "add_layer_norm"
    ascendc_src_dir = "add_layer_norm"
    ascendc_src_file = "add_layer_norm.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator AddLayerNorm, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "AddLayerNorm"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x1_in__, x2_in__, gamma_in__, beta_in__, bias_in__], origin_outputs = [y_out_, mean_out_, rstd_out_, x_out_],\
...
def op_select_format(x1_in__, x2_in__, gamma_in__, beta_in__, bias_in__=None, y_out_=None, mean_out_=None, rstd_out_=None, x_out_=None, epsilon=1e-05, additional_output=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, gamma_in__, beta_in__, bias_in__, y_out_, mean_out_, rstd_out_, x_out_, epsilon, additional_output)
    result = check_op_cap("op_select_format", "AddLayerNorm", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x1_in__, x2_in__, gamma_in__, beta_in__, bias_in__=None, y_out_=None, mean_out_=None, rstd_out_=None, x_out_=None, epsilon=1e-05, additional_output=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, gamma_in__, beta_in__, bias_in__, y_out_, mean_out_, rstd_out_, x_out_, epsilon, additional_output)
    result = check_op_cap("get_op_specific_info", "AddLayerNorm", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
