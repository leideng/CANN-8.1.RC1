...
def scatter_mul_tik(var, indices, updates, var_out, kernel_name="ScatterMul"):
    obj = scatter_common.ScatterCommon(var, indices, updates, var_out, False, kernel_name, "vmul")
    return obj.scatter_common_operator()


def scatter_mul_dsl(var, indices, updates, var_out, kernel_name="ScatterMul"):
    """
    scatter_mul interface for dsl
    """
...
def scatter_mul(var, indices, updates, var_out, use_locking=False, kernel_name="scatter_mul"):
    ...
    if not tbe_platform.api_check_support("tbe.dsl.vexp", "float32"):
        scatter_mul_tik(var, indices, updates, var_out, kernel_name)
    else:
        scatter_mul_dsl(var, indices, updates, var_out, kernel_name)
