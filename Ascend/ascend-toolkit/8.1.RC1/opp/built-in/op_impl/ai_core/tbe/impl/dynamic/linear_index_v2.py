...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['indices_list', 'stride', 'value_size']
    output_names = ['index']
...
def _build_args(indices_list_in__, stride_in__, value_size_in__, index_out_):
    __inputs__ = []
    for arg in [indices_list_in__, stride_in__, value_size_in__]:
        if arg != None:
...
@tbe_register.register_operator("LinearIndexV2", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.DYNAMIC_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.KERNEL_NAME)
def linear_index_v2(indices_list_in__, stride_in__, value_size_in__, index_out_, kernel_name="linear_index_v2", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(indices_list_in__, stride_in__, value_size_in__, index_out_)
    ...
    origin_func_name = "linear_index_v2"
    ascendc_src_dir_ex = "linear_index_v2"
    ascendc_src_dir = "linear_index_v2"
    ascendc_src_file = "linear_index_v2.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator LinearIndexV2, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "LinearIndexV2"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[indices_list_in__, stride_in__, value_size_in__], origin_outputs = [index_out_],\
                param_type_dynamic = True, mc2_ctx = [], param_type_list = ['dynamic', 'required', 'required', 'required'], init_value_list = [None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(indices_list_in__, stride_in__, value_size_in__, index_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(indices_list_in__, stride_in__, value_size_in__, index_out_)
    result = check_op_cap("op_select_format", "LinearIndexV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(indices_list_in__, stride_in__, value_size_in__, index_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(indices_list_in__, stride_in__, value_size_in__, index_out_)
    result = check_op_cap("get_op_specific_info", "LinearIndexV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
