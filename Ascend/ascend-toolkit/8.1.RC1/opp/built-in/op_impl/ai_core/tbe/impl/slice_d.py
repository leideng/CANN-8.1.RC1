...
class Constant:
    ...
    BURST_LEN = 65535
    BLOCK_SIZE = 32
    UB_SIZE_B = cce.get_soc_spec(cce.UB_SIZE)
    AICORE_NUM = cce.get_soc_spec(cce.CORE_NUM)
...
class SliceLastDimCompute():
    ...
    def __init__(self, shape, begin, size, dtype, kernel_name):
        ...
        self.ele_size = get_bit_len(self.dtype) // 8
...
def slice_d_compute(x, y, begin, size, kernel_name="slice_d"):
    ...
    float_size = get_bit_len(x.dtype) // 8
...
def _mov_scalar_one(dst, data, begin):
    ...
    device_core_num = Constant.AICORE_NUM
    float_size = get_bit_len(data.dtype) // 8
...
def _check_scalar_one(shape, size, dtype):
    ...
    float_size = get_bit_len(dtype) // 8
    cp_align_len = cce_params.BLOCK_REDUCE_INT8 // float_size
    ub_bytes = Constant.UB_SIZE_B - 32
    ub_ele = (ub_bytes // 2 // float_size // cp_align_len) * cp_align_len
