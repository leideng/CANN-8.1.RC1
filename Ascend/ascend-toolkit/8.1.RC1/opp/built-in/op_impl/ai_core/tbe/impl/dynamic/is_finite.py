...
@tbe_register.register_operator("IsFinite", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.KERNEL_NAME)
def is_finite(x_in__, y_out_, kernel_name="is_finite", impl_mode=""):
    ...
    origin_func_name = "is_finite"
    ascendc_src_dir_ex = "is_finite"
    ascendc_src_dir = "is_finite"
    ascendc_src_file = "is_finite.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator IsFinite, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "IsFinite"
...
def op_select_format(x_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_)
    result = check_op_cap("op_select_format", "IsFinite", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_)
    result = check_op_cap("get_op_specific_info", "IsFinite", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
