...
def _build_args(rgb_in__, alpha_in__, dst_out_):
    __inputs__ = []
    for arg in [rgb_in__, alpha_in__]:
        if arg != None:
...
@tbe_register.register_operator("MrgbaCustom", trans_bool_to_s8=False)
...
def mrgba_custom(rgb_in__, alpha_in__, dst_out_, kernel_name="mrgba_custom", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(rgb_in__, alpha_in__, dst_out_)
    ...
    origin_func_name = "mrgba_custom"
    ascendc_src_dir_ex = "mrgba_custom"
    ascendc_src_dir = "mrgba_custom"
    ascendc_src_file = "mrgba_custom.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MrgbaCustom, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "MrgbaCustom"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[rgb_in__, alpha_in__], origin_outputs = [dst_out_],\
...
def op_select_format(rgb_in__, alpha_in__, dst_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(rgb_in__, alpha_in__, dst_out_)
    result = check_op_cap("op_select_format", "MrgbaCustom", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(rgb_in__, alpha_in__, dst_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(rgb_in__, alpha_in__, dst_out_)
    result = check_op_cap("get_op_specific_info", "MrgbaCustom", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
