...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'indices', 'var', 'var_scale', 'var_offset']
    output_names = ['var', 'var_scale', 'var_offset']
...
def _build_args(x_in__, indices_in__, var_in__, var_scale_in__, var_offset_in__, var_out_, var_scale_out_, var_offset_out_):
    __inputs__ = []
    for arg in [x_in__, indices_in__, var_in__, var_scale_in__, var_offset_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [var_out_, var_scale_out_, var_offset_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("DynamicQuantUpdateScatterV2", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.KERNEL_NAME)
def dynamic_quant_update_scatter_v2(x_in__, indices_in__, var_in__, var_scale_in__, var_offset_in__, var_out_, var_scale_out_, var_offset_out_, kernel_name="dynamic_quant_update_scatter_v2", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, indices_in__, var_in__, var_scale_in__, var_offset_in__, var_out_, var_scale_out_, var_offset_out_)
    ...
    origin_func_name = "dynamic_quant_update_scatter_v2"
    ascendc_src_dir_ex = "dynamic_quant_update_scatter_v2"
    ascendc_src_dir = "dynamic_quant_update_scatter_v2"
    ascendc_src_file = "dynamic_quant_update_scatter_v2.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator DynamicQuantUpdateScatterV2, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "DynamicQuantUpdateScatterV2"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, indices_in__, var_in__, var_scale_in__, var_offset_in__], origin_outputs = [var_out_, var_scale_out_, var_offset_out_],\
...
def op_select_format(x_in__, indices_in__, var_in__, var_scale_in__, var_offset_in__, var_out_, var_scale_out_, var_offset_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, indices_in__, var_in__, var_scale_in__, var_offset_in__, var_out_, var_scale_out_, var_offset_out_)
    result = check_op_cap("op_select_format", "DynamicQuantUpdateScatterV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, indices_in__, var_in__, var_scale_in__, var_offset_in__, var_out_, var_scale_out_, var_offset_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, indices_in__, var_in__, var_scale_in__, var_offset_in__, var_out_, var_scale_out_, var_offset_out_)
    result = check_op_cap("get_op_specific_info", "DynamicQuantUpdateScatterV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
