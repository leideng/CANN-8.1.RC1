...
/*!
 * \file moe_tutel_combine_gates_16b.h
 * \brief
 */
#ifndef MOE_TUTEL_COMBINE_GATES_16B_
#define MOE_TUTEL_COMBINE_GATES_16B_
...
template <typename T>
class MoeTutelCombineGates {
 public:
  __aicore__ inline MoeTutelCombineGates() {
  }
  __aicore__ inline void Init(GM_ADDR x, GM_ADDR y_grad, GM_ADDR indices, GM_ADDR locations, GM_ADDR gates_grad,
    ...
    pipe.InitBuffer(mulQueue, 1, hidden * sizeof(float));
    pipe.InitBuffer(sumQueue, 1, innerOffset * sizeof(float));
    pipe.InitBuffer(workQueue, 1, hidden * sizeof(float));

    pipe.InitBuffer(outQueueGates, 1, innerOffset * sizeof(T));
    pipe.InitBuffer(tmpQueue, 1, innerOffset * sizeof(T));

    pipe.InitBuffer(tmpXCastUb, hidden * sizeof(float));
    pipe.InitBuffer(tmpYCastUb, hidden * sizeof(float));
    pipe.InitBuffer(tmpZCastUb, innerOffset * sizeof(T));
  }
 ...
 private:
  __aicore__ inline void Compute() {
    ...
    LocalTensor<float> mulLocal = mulQueue.AllocTensor<float>();
    ...
    LocalTensor<T> zTmpLocal = tmpZCastUb.Get<T>();
    ...
    for (int32_t taskOffset = startOffset; taskOffset < endOffset; taskOffset += innerOffset) {
      ...
      for (int32_t i = 0; i < innerTask; i++) {
        ...
        if (idx1 < capacity && idx0 >= 0) {
          ...
          Cast(xFloatLocal, xLocal, RoundMode::CAST_NONE, hidden);
          Cast(yFloatLocal, yLocal, RoundMode::CAST_NONE, hidden);
          Mul(mulLocal, xFloatLocal, yFloatLocal, hidden);
          pipe_barrier(PIPE_ALL);
          ReduceSum<float>(tmpLocal, mulLocal, workLocal, hidden);
          sumLocal.SetValue(i, tmpLocal.GetValue(0));
          moveCounts += 1;
        }
      }
      if (moveCounts > 0) {
        Cast(zTmpLocal, sumLocal, RoundMode::CAST_ROUND, innerOffset);
        DataCopy(gatesLocal, zTmpLocal, innerOffset);
