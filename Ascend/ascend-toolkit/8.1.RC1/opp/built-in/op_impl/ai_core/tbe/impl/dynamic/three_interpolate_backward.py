...
def _build_args(grad_x_in__, idx_in__, weight_in__, grad_y_out_, m):
    __inputs__ = []
    for arg in [grad_x_in__, idx_in__, weight_in__]:
        if arg != None:
...
@tbe_register.register_operator("ThreeInterpolateBackward", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def three_interpolate_backward(grad_x_in__, idx_in__, weight_in__, grad_y_out_, m, kernel_name="three_interpolate_backward", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(grad_x_in__, idx_in__, weight_in__, grad_y_out_, m)
    ...
    origin_func_name = "three_interpolate_backward"
    ascendc_src_dir_ex = "three_interpolate_backward"
    ascendc_src_dir = "three_interpolate_backward"
    ascendc_src_file = "three_interpolate_backward.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator ThreeInterpolateBackward, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "ThreeInterpolateBackward"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[grad_x_in__, idx_in__, weight_in__], origin_outputs = [grad_y_out_],\
...
def op_select_format(grad_x_in__, idx_in__, weight_in__, grad_y_out_, m, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_x_in__, idx_in__, weight_in__, grad_y_out_, m)
    result = check_op_cap("op_select_format", "ThreeInterpolateBackward", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(grad_x_in__, idx_in__, weight_in__, grad_y_out_, m, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_x_in__, idx_in__, weight_in__, grad_y_out_, m)
    result = check_op_cap("get_op_specific_info", "ThreeInterpolateBackward", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
