...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['features', 'features_batch_cnt', 'indices', 'indices_batch_cnt']
    output_names = ['y']
...
def _build_args(features_in__, features_batch_cnt_in__, indices_in__, indices_batch_cnt_in__, y_out_):
    __inputs__ = []
    for arg in [features_in__, features_batch_cnt_in__, indices_in__, indices_batch_cnt_in__]:
        if arg != None:
...
@tbe_register.register_operator("StackGroupPoints", trans_bool_to_s8=False)
...
def stack_group_points(features_in__, features_batch_cnt_in__, indices_in__, indices_batch_cnt_in__, y_out_, kernel_name="stack_group_points", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(features_in__, features_batch_cnt_in__, indices_in__, indices_batch_cnt_in__, y_out_)
    ...
    origin_func_name = "stack_group_points"
    ascendc_src_dir_ex = "stack_group_points"
    ascendc_src_dir = "stack_group_points"
    ascendc_src_file = "stack_group_points.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator StackGroupPoints, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "StackGroupPoints"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[features_in__, features_batch_cnt_in__, indices_in__, indices_batch_cnt_in__], origin_outputs = [y_out_],\
...
def op_select_format(features_in__, features_batch_cnt_in__, indices_in__, indices_batch_cnt_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(features_in__, features_batch_cnt_in__, indices_in__, indices_batch_cnt_in__, y_out_)
    result = check_op_cap("op_select_format", "StackGroupPoints", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(features_in__, features_batch_cnt_in__, indices_in__, indices_batch_cnt_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(features_in__, features_batch_cnt_in__, indices_in__, indices_batch_cnt_in__, y_out_)
    result = check_op_cap("get_op_specific_info", "StackGroupPoints", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
