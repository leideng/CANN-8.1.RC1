...
/*!
 * \file LstmMmMergeFP32Seq.cpp
 * \brief
 */
#include "LstmMmMergeFP32Seq.h"

using namespace AscendC;

// -------- LstmMmMergeFP32Seq begin ---------
template <typename T>
__aicore__ inline void LstmMmMergeFP32Seq<T>::Init(LstmBean lstmBean, DynamicRNNTilingData* rnnTiling,
                                                   GM_ADDR workspace) {
  param = rnnTiling;
...
template <typename T>
__aicore__ inline void LstmMmMergeFP32Seq<T>::InitBuffers(LstmBean lstmBean, GM_ADDR workspace) {
  // 初始化所有Gm地址，偏移到当前核的起始地址
  ...
  pipe.InitBuffer(calcBuf, 6 * bufSize * sizeof(float));
  ubLocal2 = calcBuf.Get<float>(6 * bufSize);
  ubLocal3 = ubLocal2[bufSize];
  ubLocal4 = ubLocal3[bufSize];
  ubLocal5 = ubLocal4[bufSize];
  ubLocal6 = ubLocal5[bufSize];
  ubLocal7 = ubLocal6[bufSize];
}

template <typename T>
__aicore__ inline void LstmMmMergeFP32Seq<T>::InitInBuffers(LstmBean lstmBean, GM_ADDR workspace) {
...
template <typename T>
__aicore__ inline void LstmMmMergeFP32Seq<T>::InitOutBuffers(LstmBean lstmBean, GM_ADDR workspace) {
  ...
  if (param->isInithc != 0) {
    ...
    initCGm.SetGlobalBuffer(reinterpret_cast<__gm__ T*>(initC + hiddenOffsets.COffset * sizeof(T)),
                            param->timeStep * param->batch * param->hiddenSize);
    outTmp1.SetGlobalBuffer(reinterpret_cast<__gm__ T*>(initH + hiddenOffsets.COffset * sizeof(T)),
                            param->timeStep * param->batch * param->hiddenSize);
  } else {
    initHGm = outTmp;
    initCGm = outCGm;
...
template <typename T>
__aicore__ inline void LstmMmMergeFP32Seq<T>::CalcGMOffset(TCubeTiling& cubeParam, TRnnOffsets& offset,
                                                           int32_t kSize) {
...
template <typename T>
__aicore__ inline void LstmMmMergeFP32Seq<T>::InitVars() {
  // 计算常用的偏移变量，提高地址偏移效率
...
template <typename T>
__aicore__ inline void LstmMmMergeFP32Seq<T>::Process() {
  /*
...
template <typename T>
__aicore__ inline void LstmMmMergeFP32Seq<T>::CalcOneOffset(int64_t mIdx, int64_t nIdx, int64_t tIdx) {
...
template <typename T>
__aicore__ inline void LstmMmMergeFP32Seq<T>::ProcessInitalT() {
  /*
...
template <typename T>
__aicore__ inline void LstmMmMergeFP32Seq<T>::ProcessNormalT(int64_t startT) {
  // T > 0 场景
  for (auto tIdx = startT; tIdx < param->timeStep; tIdx++) {
    ...
    if (param->direction == 1) {
      ...
      outTmp1 = outHGm[(param->timeStep - 1 - tIdx) * hiddenMKAllSize];
    } else {
      initCGm = outCGm[tIdx * hiddenMKAllSize];
      initHGm = outHOriTmp[tIdx * hiddenMKAllSize + hiddenAOffset];
      outTmp1 = outHGm[tIdx * hiddenMKAllSize];
    }
  }
}

template <typename T>
__aicore__ inline void LstmMmMergeFP32Seq<T>::CalcInitBlock(int64_t mIdx, int64_t nIdx) {
  ...
  Mul(ubLocal2, ubLocal4, ubLocal3, bufSize);
  pipe_barrier(PIPE_V);
  CalOutHYWithSeq(ubLocal2);
  pipe_barrier(PIPE_V);
  CopyUB2OutYH(ubLocal2, 0);
}

template <typename T>
__aicore__ inline void LstmMmMergeFP32Seq<T>::CalcNormalBlock(int64_t mIdx, int64_t nIdx, int64_t tIdx) {
  CalcOneOffset(mIdx, nIdx, tIdx);
  pipe_barrier(PIPE_V);
  CopyInInitHC(ubLocal7, outTmp1);

  // stage 2
  pipe_barrier(PIPE_V);
  CopyInInitHC(ubLocal6, initCGm);

  // copy seq
  pipe_barrier(PIPE_V);
  CopyInSeqLength(tIdx);
  ...
  // stage 3
  pipe_barrier(PIPE_V);
  Mul(ubLocal5, ubLocal6, ubLocal3, bufSize);
  ...
  pipe_barrier(PIPE_V);
  CalcNormalOutCWithSeq(ubLocal3, ubLocal6, ubLocal4);
  ...
  pipe_barrier(PIPE_V);
  CalOutYWithSeq(ubLocal3);

  // stage 17 --copyout outputC
  pipe_barrier(PIPE_V);
  CopyUB2Out(outYGm, ubLocal3, tIdx);

  pipe_barrier(PIPE_V);
  CalNormalOutHYWithSeq(ubLocal3, ubLocal7, ubLocal4);

  // stage 17/18 --copyout outputH/Y
  pipe_barrier(PIPE_V);
  CopyUB2Out(outHGm, ubLocal3, tIdx);
}

template <typename T>
__aicore__ inline void LstmMmMergeFP32Seq<T>::MMWithSigmod(LocalTensor<float>& ubLocal, int64_t offset) {
...
template <typename T>
__aicore__ inline void LstmMmMergeFP32Seq<T>::MMWithSigmodAndForgetBias(LocalTensor<float>& ubLocal, int64_t offset) {
  LocalTensor<float> tmp = outQueue.AllocTensor<float>();
  CalcMM(tmp, offset);
  pipe_barrier(PIPE_V);
...
template <typename T>
__aicore__ inline void LstmMmMergeFP32Seq<T>::MMWithTanh(LocalTensor<float>& ubLocal, int64_t offset) {
  LocalTensor<float> tmp = outQueue.AllocTensor<float>();
  CalcMM(tmp, offset);
  pipe_barrier(PIPE_V);
...
template <typename T>
__aicore__ inline void LstmMmMergeFP32Seq<T>::CalcMM(LocalTensor<float>& ubLocal, int64_t offset) {
...
template <typename T>
__aicore__ inline void LstmMmMergeFP32Seq<T>::CalOutYWithSeq(LocalTensor<float>& ubLocal) {
  auto seqLocal = inQueue.DeQue<T>();
  pipe_barrier(PIPE_V);
  Mul(ubLocal, ubLocal, seqLocal, bufSize);
  inQueue.EnQue(seqLocal);
}

template <typename T>
__aicore__ inline void LstmMmMergeFP32Seq<T>::CalcMMAdd(LocalTensor<float>& ubLocal, int64_t tIdx, int64_t offset) {
...
template <typename T>
__aicore__ inline void LstmMmMergeFP32Seq<T>::CalcInitOutCWithSeq(LocalTensor<float>& ubLocal) {
  auto seqLocal = inQueue.DeQue<T>();
  pipe_barrier(PIPE_V);
  Mul(ubLocal, ubLocal, seqLocal, bufSize);
  inQueue.EnQue(seqLocal);
...
template <typename T>
__aicore__ inline void LstmMmMergeFP32Seq<T>::CalcNormalOutCWithSeq(LocalTensor<float>& ubLocal,
                                                                    LocalTensor<float>& ubLocal1,
                                                                    LocalTensor<float>& ubLocal2) {
  auto seqLocal = inQueue.DeQue<T>();
  pipe_barrier(PIPE_V);
  Sub(ubLocal2, ubLocal, ubLocal1, bufSize);
  pipe_barrier(PIPE_V);
  Mul(ubLocal2, ubLocal2, seqLocal, bufSize);
  pipe_barrier(PIPE_V);
  Add(ubLocal, ubLocal2, ubLocal1, bufSize);
  inQueue.EnQue(seqLocal);
}

template <typename T>
__aicore__ inline void LstmMmMergeFP32Seq<T>::CalNormalOutHYWithSeq(LocalTensor<float>& ubLocal,
                                                                    LocalTensor<float>& ubLocal1,
                                                                    LocalTensor<float>& ubLocal2) {
  auto seqLocal = inQueue.DeQue<T>();
  pipe_barrier(PIPE_V);
  ...
  pipe_barrier(PIPE_V);
  Add(ubLocal, ubLocal2, ubLocal1, bufSize);
  inQueue.FreeTensor(seqLocal);
}

template <typename T>
__aicore__ inline void LstmMmMergeFP32Seq<T>::CalOutHYWithSeq(LocalTensor<float>& ubLocal) {
  auto seqLocal = inQueue.DeQue<T>();
  pipe_barrier(PIPE_V);
  Mul(ubLocal, ubLocal, seqLocal, bufSize);
  inQueue.FreeTensor(seqLocal);
...
template <typename T>
__aicore__ inline void LstmMmMergeFP32Seq<T>::MoveOut(LocalTensor<float>& ubLocal) {
...
template <typename T>
__aicore__ inline void LstmMmMergeFP32Seq<T>::CopyInInitHC(LocalTensor<float>& dstUb, GlobalTensor<T>& initGlobal) {
  ...
  Nd2NzParams transParam = {1,
                            ...
                            2,
                            0};
  DataCopy(ubLocalIn, initGlobal[outCOffset], transParam);
...
template <typename T>
__aicore__ inline void LstmMmMergeFP32Seq<T>::CopyInSeqLength(int64_t tIdx) {
  LocalTensor<T> ubLocalIn = inQueue.AllocTensor<T>();  // 16进 需要的f32计算
  ...
  Nd2NzParams transParam = {1,
                            ...
                            2,
                            0};
  DataCopy(ubLocalIn, seqLengthGm[inputOffsets.COffset], transParam);
  inQueue.EnQue(ubLocalIn);
}

// 输出的接口
template <typename T>
__aicore__ inline void LstmMmMergeFP32Seq<T>::CopyOut(GlobalTensor<T>& outGlobal, int64_t tIdx) {
...
template <typename T>
__aicore__ inline void LstmMmMergeFP32Seq<T>::CopyUB2Out(GlobalTensor<T>& outGlobal, LocalTensor<float>& ubLocal,
                                                         int64_t tIdx) {
  // 将要输出的数据搬到outque
  MoveOut(ubLocal);
  // 搬到outC的输出地址
...
template <typename T>
__aicore__ inline void LstmMmMergeFP32Seq<T>::CopyUB2OutYH(LocalTensor<float>& ubLocal, int64_t tIdx) {
  // 将要输出的数据搬到outque
  MoveOut(ubLocal);
  // 搬到y和outH的输出地址
  Nz2NdParamsFull transParam = {1,
