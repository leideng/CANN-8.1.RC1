...
from impl.util.util_tik_comm_func import OpBase, ceil_div, tik_func_vconv, tik_func_vcomple
...
class Constant:
    ...
    RESERVED_UB_BYTES_FOR_VBI = 32 * 1024
    ...
    # sub tiling key
    TK_MOVE_4_POINTS_TO_UB = 1
    TK_MOVE_WHOLE_ROI_TO_UB = 2
    TK_MOVE_ROI_TO_UB_SLICE_ROW = 3
    # output pool width per loop max value
    OUTPUT_W_PER_LOOP_MAX = 127
    # C1 * C0 temporary tensors used in calculation
    TEMP_C1xC0_COUNT = 8
    # c1 max value per loop, which is decided by instruction repeat max value
    C1_PER_LOOP_MAX = 255
...
class ProcessorBase(metaclass=ABCMeta):
    def __init__(self, op: "RoiAlign") -> None:
        self.op = op
        self.tik_inst: tik.Tik = op.tik_instance
        if op.pool_mode == 'avg':
            self.out_init_val = 0.0
            self.pool_func = op.tik_instance.vadd
        else:
            self.out_init_val = Constant.FP32_MIN if op.dtype == 'float32' else Constant.FP16_MIN
            self.pool_func = op.tik_instance.vmax

        # whole feature map count
        self.feature_hwc0 = self.tik_inst.Scalar(dtype="int64", name="feature_hwc0",
                                                 init_value=op.feature_h * op.feature_w * Constant.C0)
        self.feature_c1hwc0 = self.tik_inst.Scalar(dtype="int64", name="feature_c1hwc0",
                                                   init_value=op.c1_per_core * self.feature_hwc0)

        # wether cache whole feature map in L1
        self.cache_all_fm_in_l1 = self.tik_inst.Scalar(dtype="int32", name="cache_all_fm_in_l1", init_value=0)
        if op.l1_size_bytes > 0:
            self.fm_in_l1 = self.tik_inst.Tensor(dtype=op.dtype, name="fm_in_l1", scope=tik.scope_cbuf,
                                                 shape=[op.l1_size_bytes // op.data_size])
            with self.tik_inst.if_scope(tik.all(self.feature_c1hwc0 <= op.l1_size_bytes // op.data_size,
                                                op.rois_per_core > 1)):
                self.cache_all_fm_in_l1.set_as(1)

        self.roi_in_ub = self.tik_inst.Tensor(dtype="float32", shape=(5, Constant.BATCH_SIZE),
                                              name="roi_in_ub", scope=tik.scope_ubuf)

        # x0, y0
        self.cur_roi_start_x = self.tik_inst.Scalar(dtype="float32", name="cur_roi_start_x")
        self.cur_roi_start_y = self.tik_inst.Scalar(dtype="float32", name="cur_roi_start_y")
        # roi width / height
        self.roi_wh = self.tik_inst.Tensor(dtype="float32", shape=(2, Constant.BATCH_SIZE),
                                           name="roi_wh", scope=tik.scope_ubuf)
        # grid width / height
        self.grid_wh = self.tik_inst.Tensor(dtype="float32", shape=(2, Constant.BATCH_SIZE),
                                            name="grid_wh", scope=tik.scope_ubuf)
        # current roi grid height / width
        self.cur_roi_grid_h = self.tik_inst.Scalar(dtype="float32", name="cur_roi_grid_h")
        self.cur_roi_grid_w = self.tik_inst.Scalar(dtype="float32", name="cur_roi_grid_w")
        # height / width sample count
        self.sample_wh = self.tik_inst.Tensor(dtype="int32", shape=(2, Constant.BATCH_SIZE),
                                              name="sample_wh", scope=tik.scope_ubuf)
        # current roi samples
        self.cur_roi_sample_h = self.tik_inst.Scalar(dtype="int32", name="cur_roi_sample_h")
        self.cur_roi_sample_w = self.tik_inst.Scalar(dtype="int32", name="cur_roi_sample_w")
        self.cur_roi_sample_h_vec = self.tik_inst.Tensor(dtype="float32", name="cur_roi_sample_h_vec",
                                                         scope=tik.scope_ubuf, shape=[8])
        self.cur_roi_sample_w_vec = self.tik_inst.Tensor(dtype="float32", name="cur_roi_sample_w_vec",
                                                         scope=tik.scope_ubuf, shape=[8])

        # feature map index specified in ROI
        self.fm_idx = self.tik_inst.Tensor(dtype="int32", shape=(Constant.BATCH_SIZE,),
                                           name="fm_idx", scope=tik.scope_ubuf)
        self.cur_fm_idx = self.tik_inst.Scalar(dtype="int32", name="cur_fm_idx")
        # output y index
        self.cur_out_n = self.tik_inst.Scalar(dtype="int32", name="cur_out_n")
        # pool width index to find elements in y
        self.pool_w_idx = self.tik_inst.Tensor(dtype="int32", shape=(Constant.BATCH_SIZE,),
                                               name="pool_w_idx", scope=tik.scope_ubuf)
        # 4 points around, x_low, x_high, y_low, y_high
        self.x_lh = self.tik_inst.Tensor(dtype="int32", shape=(2, Constant.BATCH_SIZE),
                                         name="x_lh", scope=tik.scope_ubuf)
        self.y_lh = self.tik_inst.Tensor(dtype="int32", shape=(2, Constant.BATCH_SIZE),
                                         name="y_lh", scope=tik.scope_ubuf)
        # length of 4 points, hx, lx, hy, ly
        self.hlx = self.tik_inst.Tensor(dtype="float32", shape=(2, Constant.BATCH_SIZE),
                                        name="hlx", scope=tik.scope_ubuf)
        self.hly = self.tik_inst.Tensor(dtype="float32", shape=(2, Constant.BATCH_SIZE),
                                        name="hly", scope=tik.scope_ubuf)
        # weight of 4 points, w1=hy*hx, w2=hy*lx, w3=ly*hx, w4=ly*lx
        self.w1234 = self.tik_inst.Tensor(dtype=op.dtype, shape=[4 * Constant.BATCH_SIZE],
                                          name="w1234", scope=tik.scope_ubuf)

        # output row loop scalars
        self.h_grid_remain = self.tik_inst.Scalar(dtype="int64", name="h_grid_remain", init_value=0)
        self.h_grid_idx_all = self.tik_inst.Scalar(dtype="int64", name="h_grid_idx_all", init_value=0)
        self.h_grid_h1 = self.tik_inst.Scalar(dtype="int64", name="h_grid_h1")
        self.h_grid_h2 = self.tik_inst.Scalar(dtype="int64", name="h_grid_h2")

        # cache row / column weights in scenario: sample * pooled <= Constant.BATCH_SIZE
        self.x_weight_cache_idx = self.tik_inst.Scalar("int64", "x_weight_cache_idx")
        self.y_weight_cache_idx = self.tik_inst.Scalar("int64", "y_weight_cache_idx")

        # whole feature map cached index
        self.cache_fm_idx = self.tik_inst.Scalar(dtype="int32", name="cache_fm_idx", init_value=-1)

        # help tensors
        # filled with 0.0 in tensor, total count: one block
        self.const_zero_fp32 = self.tik_inst.Tensor(dtype="float32", shape=(8,),
                                                    name="const_zero_fp32", scope=tik.scope_ubuf)
        # filled with 1.0 in tensor, total count: one block
        self.const_one_fp32 = self.tik_inst.Tensor(dtype="float32", shape=(8,),
                                                   name="const_one_fp32", scope=tik.scope_ubuf)
        # filled with 1 in tensor, total count: one block
        self.const_one_int = self.tik_inst.Tensor(dtype="int32", shape=(8,),
                                                  name="const_one_int", scope=tik.scope_ubuf)
        # filled 0...127 in tensor
        self.const_0_127_fp32 = self.tik_inst.Tensor(dtype="float32", shape=(Constant.BATCH_SIZE,),
                                                     name="const_0_127_fp32", scope=tik.scope_ubuf)
        # filled with -1.0 in tensor, total count: one block
        self.const_minus_1_fp32 = self.tik_inst.Tensor(dtype="float32", shape=(8,),
        ...
        self.const_fm_h_fp = self.tik_inst.Tensor(dtype="float32", shape=(8,),
                                                  name="const_fm_h_fp", scope=tik.scope_ubuf)
        self.const_fm_w_fp = self.tik_inst.Tensor(dtype="float32", shape=(8,),
                                                  name="const_fm_w_fp", scope=tik.scope_ubuf)
        # filled with height-1/weight-1 of feature_map shape in tensor, total count: 8
        self.const_fm_h_minus_1 = self.tik_inst.Tensor(dtype="int32", shape=(8,),
                                                       name="const_fm_h_minus_1", scope=tik.scope_ubuf)
        self.const_fm_w_minus_1 = self.tik_inst.Tensor(dtype="int32", shape=(8,),
        ...
        self.const_fm_h_minus_1_fp = self.tik_inst.Tensor(dtype="float32", shape=(8,),
                                                          name="const_fm_h_minus_1_fp", scope=tik.scope_ubuf)
        self.const_fm_w_minus_1_fp = self.tik_inst.Tensor(dtype="float32", shape=(8,),
                                                          name="const_fm_w_minus_1_fp", scope=tik.scope_ubuf)
        # temporary fp tensor
        self.tmp_fp32 = self.tik_inst.Tensor(dtype="float32", shape=(2, Constant.BATCH_SIZE),
                                             name="tmp_fp32", scope=tik.scope_ubuf)
        # memory left for vbi
        int32_bytes = 4
        self.vbi_memory = self.tik_inst.Scalar(dtype="int32", name="vbi_memory", init_value=0)
        self.vbi_mem_factor = self.tik_inst.Scalar(dtype="int32", name="vbi_mem_factor",
                                                   init_value=4 * int32_bytes + (4 + Constant.C0) * op.data_size)
        # memory left max for vbi
        self.memory_for_vbi_max = self.tik_inst.Scalar(dtype="int32", name="memory_for_vbi_max")
    ...
    def run(self) -> None:
        ...
        with self.tik_inst.for_range(0, roi_loops) as _roi_loop_idx:
            rois, roi_offset = self._calc_segment(self.op.core_rois, _roi_loop_idx, Constant.BATCH_SIZE)
            roi_offset.set_as(self.op.core_roi_offset + roi_offset)
            self.compute_roi_batch(roi_offset, rois)

    def compute_roi_batch(self, roi_offset: tik.Scalar, roi_num: tik.Scalar) -> None:
    ...
    def compute_one_roi(self, roi_offset: tik.Scalar, roi_idx: tik.Scalar,
        ...
        self.tik_inst.vec_dup(mask=8, dst=self.cur_roi_sample_w_vec, scalar=1.0 * self.cur_roi_sample_w,
                              repeat_times=1, dst_rep_stride=8)
        self.tik_inst.vec_dup(mask=8, dst=self.cur_roi_sample_h_vec, scalar=1.0 * self.cur_roi_sample_h,
                              repeat_times=1, dst_rep_stride=8)

        with self.tik_inst.if_scope(tik.all(self.cur_fm_idx >= 0, self.cur_fm_idx < self.op.feature_n)):
    ...
    def tiling_compute_output(self, fm_in_ub: tik.Tensor, c1_offset: tik.Scalar, c1_num: tik.Scalar) -> None:
        ...
        with self.tik_inst.for_range(0, out_w_loops) as _out_w_loop_idx:
            ...
            with self.tik_inst.for_range(0, out_c1_loops) as _out_c1_loop_idx:
                out_c1_num, relative_c1_offset = self._calc_segment(c1_num, _out_c1_loop_idx,
                                                                    self.op.out_c1_per_loop)
                out_c1_offset = self.tik_inst.Scalar(dtype="int64", name="out_c1_offset",
                                                     init_value=c1_offset + relative_c1_offset)
                out_y_ub = self.tik_inst.Tensor(dtype=self.op.dtype, name="out_y_ub", scope=tik.scope_ubuf,
    ...
    def compute_grid_batch_h(self, fm_in_ub: tik.Tensor,
        ...
        with self.tik_inst.if_scope(self.y_weight_cache_idx != grid_h_offset):
        ...
        tmp_unfinished_grid = self.tik_inst.Scalar(dtype="int64", name="tmp_unfinished_grid",
                                                   init_value=self.cur_roi_sample_h - self.h_grid_remain)
        self.tik_inst.scalar_min(dst=self.h_grid_h2, src0=tmp_unfinished_grid, src1=grid_h_num)
        pool_h_loops = ceil_div(grid_h_num, self.cur_roi_sample_h) + 1
        # loop each output row
        with self.tik_inst.for_range(0, pool_h_loops) as _pool_h_loop_idx:
            # loop grid width
            ...
            with self.tik_inst.if_scope(self.h_grid_idx_all % self.cur_roi_sample_h == 0):
                # move y out
            ...
            tmp_next_grid = self.tik_inst.Scalar(dtype="int64", name="tmp_next_grid",
                                                 init_value=self.h_grid_h1 + self.cur_roi_sample_h)
            self.tik_inst.scalar_min(dst=self.h_grid_h2, src0=tmp_next_grid, src1=grid_h_num)
            with self.tik_inst.if_scope(self.h_grid_h1 == self.h_grid_h2):
                self.tik_inst.tik_break()
    ...
    def compute_grid_batch_w(self, fm_in_ub: tik.Tensor, out_y_ub: tik.Tensor, relative_c1_offset: tik.Scalar,
        ...
        with self.tik_inst.if_scope(self.x_weight_cache_idx != grid_w_offset):
        ...
        with self.tik_inst.for_range(self.h_grid_h1, self.h_grid_h2) as _grid_h_idx:  # _grid_h_idx: [0, grid_h_num)
            y_low = self.tik_inst.Scalar(dtype="int32", name="y_low", init_value=self.y_lh[0, _grid_h_idx])
            y_high = self.tik_inst.Scalar(dtype="int32", name="y_high", init_value=self.y_lh[1, _grid_h_idx])
    ...
    def _prepare(self) -> None:
        ...
        with self.tik_inst.if_scope(self.op.samples > 0):
            # int32 vector_dup
            self.tik_inst.vector_dup(mask=64, dst=self.sample_wh, scalar=self.op.samples, repeat_times=2 * 2,
                                     dst_blk_stride=1, dst_rep_stride=8)
        # constant vectors prepare, all are one block only
        self.tik_inst.vector_dup(8, self.const_zero_fp32, 0, 1, 0, 0)
        self.tik_inst.vector_dup(8, self.const_one_fp32, 1, 1, 0, 0)  # fp32
        self.tik_inst.vector_dup(8, self.const_one_int, 1, 1, 0, 0)  # int32
        self.tik_inst.vector_dup(8, self.const_minus_1_fp32, -1.0, 1, 0, 0)
        self.tik_inst.vector_dup(8, self.const_fm_h_fp, self.op.feature_h + 0, 1, 0, 0)
        self.tik_inst.vector_dup(8, self.const_fm_w_fp, self.op.feature_w + 0, 1, 0, 0)
        self.tik_inst.vector_dup(8, self.const_fm_h_minus_1, self.op.feature_h - 1, 1, 0, 0)  # int32
        self.tik_inst.vector_dup(8, self.const_fm_w_minus_1, self.op.feature_w - 1, 1, 0, 0)  # int32
        self.tik_inst.vector_dup(8, self.const_fm_h_minus_1_fp, self.op.feature_h - 1, 1, 0, 0)
        self.tik_inst.vector_dup(8, self.const_fm_w_minus_1_fp, self.op.feature_w - 1, 1, 0, 0)
        # 0-127 const index
    ...
    def _clear_roi_in_ub(self) -> None:
        ...
        self.tik_inst.vector_dup(mask=mask, dst=self.roi_in_ub, scalar=0.0,
    ...
    def _clear_ub(self, ub_to_clear: tik.Tensor, clear_len: tik.Scalar, clear_value: float = 0):
        ...
        with self.tik_inst.if_scope(repeat > 0):
            with self.tik_inst.for_range(0, repeat) as index:
                ...
                self.tik_inst.vec_dup(mask=mask_max, dst=ub_to_clear[tmp_offset], scalar=clear_value,
        ...
        with self.tik_inst.if_scope(last_num > 0):
            ...
            self.tik_inst.vec_dup(mask=last_num, dst=ub_to_clear[tmp_offset], scalar=clear_value,
                                  repeat_times=1, dst_rep_stride=8)
    ...
    def _move_roi_data_to_ub(self, roi_num: tik.Scalar, roi_offset: tik.Scalar) -> None:
        ...
        with self.tik_inst.new_stmt_scope():
            ...
            rois_data_tmp = self.tik_inst.Tensor(self.op.dtype, [Constant.BATCH_SIZE * self.op.rois_row_size],
            ...
            if tbe_platform.api_check_support("tik.data_move_pad"):
                self.tik_inst.data_move_pad(dst=rois_data_tmp, src=self.op.rois_gm[roi_offset * self.op.rois_row_size],
                                            nburst=1, burst=roi_num * self.op.rois_row_size * self.op.data_size,
                                            dst_gap=0, src_gap=0)
            else:
            ...
            if self.op.dtype == "float32":
                ...
                roi_data_fp32_tmp = self.tik_inst.Tensor("float32", [Constant.BATCH_SIZE * self.op.rois_row_size],
                                                         name="roi_data_fp32_tmp", scope=tik.scope_ubuf)
                self.tik_inst.vconv(mask=64, round_mode='', dst=roi_data_fp32_tmp, src=rois_data_tmp,
    ...
    def _calc_segment(self, total_seg: tik.Scalar, seg_index: tik.Scalar, seg_len: tik.Scalar):
        ret_seg_len = self.tik_inst.Scalar(dtype="int64", name="ret_seg_len", init_value=seg_len)
        ret_offset = self.tik_inst.Scalar(dtype="int64", name="ret_offset", init_value=seg_index * seg_len)
        with self.tik_inst.if_scope(total_seg - seg_index * seg_len < seg_len):
    ...
    def _convert_fpx_to_int32(self, round_mode: str, dst: tik.Tensor, src: tik.Tensor,
        ...
        if src.dtype == "float32" and not tbe_platform.api_check_support("tik.vconv", "f322s32r"):
            tik_func_vconv(self.tik_inst, dst_ub=dst, src_ub=src, do_len=max_num, mode=round_mode)
        else:
            if src.dtype == "float32":
                self.tik_inst.vconv(mask=64, round_mode=round_mode, dst=dst, src=src,
    ...
    def _vdiv(self, dst: tik.Tensor, src0: tik.Tensor, src1: tik.Tensor,
        ...
        if not tbe_platform.api_check_support("tik.vdiv", dst.dtype):
            tik_func_vcomple(self.tik_inst, "vdiv", out_dst=dst, src0=src0, src1=src1, copy_num=max_num)
        else:
            mask = 64 if dst.dtype == "float32" else 128
            repeat = 2 if dst.dtype == "float32" else 1
            self.tik_inst.vdiv(mask=mask, dst=dst, src0=src0, src1=src1, repeat_times=repeat * instr_num,
    ...
    def _vdivs_fp32(self, dst: tik.Tensor, divided: tik.Tensor, fp_divisor: tik.Scalar,
        ...
        if tbe_platform.api_check_support("tik.vdiv", "float32"):
            if fp_divisor_tensor is None:
                fp_divisor_tensor = self.tik_inst.Tensor("float32", name="fp_divisor_tensor",
                                                         shape=[8], scope=tik.scope_ubuf)
                self.tik_inst.vector_dup(mask=8, dst=fp_divisor_tensor, scalar=fp_divisor,
                                         repeat_times=1, dst_blk_stride=1, dst_rep_stride=8)
            self.tik_inst.vdiv(mask=64, dst=dst, src0=divided, src1=fp_divisor_tensor,
            ...
            self.tik_inst.vmuls(mask=64, dst=dst, src=divided, scalar=1.0 / fp_divisor,
                                repeat_times=2 * instr_num, dst_blk_stride=1, src_blk_stride=1,
                                dst_rep_stride=8, src_rep_stride=8)
    ...
    def _calc_fm_start_end_coordinate(self) -> None:
        ...
        self.tik_inst.vmuls(mask=64, dst=self.roi_in_ub[1, 0], src=self.roi_in_ub[1, 0],
                            scalar=self.op.scale, repeat_times=2 * 4,
                            dst_blk_stride=1, src_blk_stride=1,
                            dst_rep_stride=8, src_rep_stride=8)
        with self.tik_inst.if_scope(self.op.roi_end_mode > 0):
            with self.tik_inst.if_scope(self.op.roi_end_mode == 1):
                # x1' = (x1 + 1) * scale, y1' = (y1 + 1) * scale
                self.tik_inst.vadds(mask=64, dst=self.roi_in_ub[3, 0], src=self.roi_in_ub[3, 0],
                                    scalar=self.op.scale, repeat_times=2 * 2,
                                    dst_blk_stride=1, src_blk_stride=1,
                                    dst_rep_stride=8, src_rep_stride=8)
            with self.tik_inst.else_scope():
                # x0' = x0 * scale - 0.5, y0' = y0 * scale - 0.5, x1' = x1 * scale - 0.5, y1' = y1 * scale - 0.5
                self.tik_inst.vadds(mask=64, dst=self.roi_in_ub[1, 0], src=self.roi_in_ub[1, 0], scalar=-0.5,
    ...
    def _calc_roi_size(self) -> None:
        ...
        self.tik_inst.vsub(mask=64, dst=self.roi_wh, src0=self.roi_in_ub[3, 0], src1=self.roi_in_ub[1, 0],
                           repeat_times=2 * 2, dst_blk_stride=1, src0_blk_stride=1, src1_blk_stride=1,
                           dst_rep_stride=8, src0_rep_stride=8, src1_rep_stride=8)
        with self.tik_inst.if_scope(self.op.roi_end_mode < 2):
            # `roi_w = max(roi_w, 1)`, `roi_h = max(roi_h, 1)`
            self.tik_inst.vmax(mask=64, dst=self.roi_wh, src0=self.roi_wh, src1=self.const_one_fp32,
    ...
    def _calc_grid_size(self) -> None:
        ...
        op = self.op
        # `bin_w = roi_w / pool_w`
        self.tik_inst.vmuls(mask=64, dst=self.grid_wh[0, 0], src=self.roi_wh[0, 0],
                            scalar=op.pool_w_reciprocal, repeat_times=2,
                            dst_blk_stride=1, src_blk_stride=1,
                            dst_rep_stride=8, src_rep_stride=8)
        # `bin_h = roi_h / pool_h`
        self.tik_inst.vmuls(mask=64, dst=self.grid_wh[1, 0], src=self.roi_wh[1, 0],
        ...
        with self.tik_inst.else_scope():
            ...
            # 2 instructions in one
            self.tik_inst.vmuls(mask=64, dst=self.grid_wh, src=self.grid_wh, scalar=op.samples_reciprocal,
    ...
    def _calc_pool_idx_of_grid(self, dst_pool_idx: tik.Tensor,
        ...
        self.tik_inst.vadds(mask=64, dst=self.tmp_fp32[0, 0], src=self.const_0_127_fp32, scalar=grid_offset * 1.0,
                            repeat_times=2, dst_blk_stride=1, src_blk_stride=1,
                            dst_rep_stride=8, src_rep_stride=8)
        # grid index / sample
        tmp = self.tik_inst.Scalar(dtype="float32", name="tmp", init_value=sample_int)
    ...
    def _calc_bilinear_interpolate_coordinate(self, grid_offset: tik.Scalar, is_width: bool = True) -> None:
        ...
        x = self.tik_inst.Tensor("float32", shape=(Constant.BATCH_SIZE,), name="x", scope=tik.scope_ubuf)
        # `grid_idx + grid_offset * 1.0 + 0.5`
        self.tik_inst.vadds(mask=64, dst=self.tmp_fp32[0, 0], src=self.const_0_127_fp32, scalar=grid_offset * 1.0 + 0.5,
                            repeat_times=2, dst_blk_stride=1, src_blk_stride=1,
                            dst_rep_stride=8, src_rep_stride=8)
        # `(grid_idx + grid_offset * 1.0 + 0.5) * grid_distance`
        self.tik_inst.vmuls(mask=64, dst=self.tmp_fp32[0, 0], src=self.tmp_fp32[0, 0], scalar=grid_distance,
                            repeat_times=2, dst_blk_stride=1, src_blk_stride=1,
                            dst_rep_stride=8, src_rep_stride=8)
        # `start_coordinate + grid_distance * (grid_offset + grid_idx + 0.5)`
        self.tik_inst.vadds(mask=64, dst=self.tmp_fp32[0, 0], src=self.tmp_fp32[0, 0], scalar=start_coordinate,
                            repeat_times=2, dst_blk_stride=1, src_blk_stride=1,
                            dst_rep_stride=8, src_rep_stride=8)
        # `if x <= 0: x = 0`
        self.tik_inst.vmax(mask=64, dst=x, src0=self.tmp_fp32[0, 0], src1=self.const_zero_fp32,
                           repeat_times=2, dst_blk_stride=1, src0_blk_stride=1, src1_blk_stride=0,
                           dst_rep_stride=8, src0_rep_stride=8, src1_rep_stride=0)
        # `x_low = floor(x)`
        self._convert_fpx_to_int32(round_mode='floor', dst=dst_x_low_high[0, 0], src=x,
                                   max_num=Constant.BATCH_SIZE)
        # `x_high = x_low + 1`. INT32
        self.tik_inst.vadd(mask=64, dst=dst_x_low_high[1, 0], src0=dst_x_low_high[0, 0], src1=self.const_one_int,
                           repeat_times=2, dst_blk_stride=1, src0_blk_stride=1, src1_blk_stride=0,
                           dst_rep_stride=8, src0_rep_stride=8, src1_rep_stride=0)
        # `if x_high < x_low: x_high = x_low` int overflow detection
        self.tik_inst.vmax(mask=64, dst=dst_x_low_high[1, 0], src0=dst_x_low_high[1, 0], src1=dst_x_low_high[0, 0],
                           repeat_times=2, dst_blk_stride=1, src0_blk_stride=1, src1_blk_stride=0,
                           dst_rep_stride=8, src0_rep_stride=8, src1_rep_stride=0)
        # `if x_low >= width - 1: x_low = width - 1;  if x_high >= width - 1: x_high = width - 1`. INT32
        self.tik_inst.vmin(mask=64, dst=dst_x_low_high, src0=dst_x_low_high, src1=fm_border_minus_1,
                           repeat_times=2 * 2, dst_blk_stride=1, src0_blk_stride=1, src1_blk_stride=0,
                           dst_rep_stride=8, src0_rep_stride=8, src1_rep_stride=0)
        # `if x >= width - 1: x = width - 1`
        self.tik_inst.vmin(mask=64, dst=x, src0=x, src1=fm_border_minus_1_fp,
                           repeat_times=2, dst_blk_stride=1, src0_blk_stride=1, src1_blk_stride=0,
                           dst_rep_stride=8, src0_rep_stride=8, src1_rep_stride=0)
        # `lx = x - x_low`
        self._convert_int32_to_fpx(round_mode='', dst=self.tmp_fp32[1, 0], src=dst_x_low_high[0, 0],
                                   max_num=Constant.BATCH_SIZE)
        self.tik_inst.vsub(mask=64, dst=dst_hx_lx[1, 0], src0=x, src1=self.tmp_fp32[1, 0],
                           repeat_times=2, dst_blk_stride=1, src0_blk_stride=1, src1_blk_stride=1,
                           dst_rep_stride=8, src0_rep_stride=8, src1_rep_stride=8)
        # `hx = 1 - lx`
        self.tik_inst.vsub(mask=64, dst=dst_hx_lx[0, 0], src0=self.const_one_fp32, src1=dst_hx_lx[1, 0],
        ...
        if tbe_platform.get_soc_spec(tbe_platform.SHORT_SOC_VERSION) in ("Ascend910",):
            ...
            with self.tik_inst.for_range(0, 2) as _repeat_idx:
                ...
                # 2 instructions in one
                self.tik_inst.vsel(mask=64, mode=0, dst=dst_hx_lx[_repeat_idx * num_per_loop], sel=cmpmask,
            ...
            self.tik_inst.vcmpv_lt(dst=tmp_cmp_mask, src0=self.tmp_fp32, src1=self.const_minus_1_fp32,
                                   repeat_times=2, src0_blk_stride=1, src1_blk_stride=0,
                                   src0_rep_stride=8, src1_rep_stride=0)
            self.tik_inst.vcmpv_gt(dst=tmp_cmp_mask[16], src0=self.tmp_fp32, src1=fm_border_fp,
                                   repeat_times=2, src0_blk_stride=1, src1_blk_stride=0,
                                   src0_rep_stride=8, src1_rep_stride=0)
            # 8: 128 bit together
            self.tik_inst.vor(mask=8, dst=tmp_cmp_mask, src0=tmp_cmp_mask[0], src1=tmp_cmp_mask[16],
                              repeat_times=1, dst_blk_stride=1, src0_blk_stride=1, src1_blk_stride=1,
                              dst_rep_stride=2, src0_rep_stride=2, src1_rep_stride=2)
            self.tik_inst.vsel(mask=64, mode=2, dst=dst_hx_lx[0, 0], sel=tmp_cmp_mask,
                               src0=self.const_zero_fp32, src1=dst_hx_lx[0, 0], repeat_times=2,
                               dst_blk_stride=1, src0_blk_stride=0, src1_blk_stride=1,
                               dst_rep_stride=8, src0_rep_stride=0, src1_rep_stride=8)
            self.tik_inst.vsel(mask=64, mode=2, dst=dst_hx_lx[1, 0], sel=tmp_cmp_mask,
                               src0=self.const_zero_fp32, src1=dst_hx_lx[1, 0], repeat_times=2,
                               dst_blk_stride=1, src0_blk_stride=0, src1_blk_stride=1,
                               dst_rep_stride=8, src0_rep_stride=0, src1_rep_stride=8)

        if self.op.pool_mode == 'avg':
            if is_width:
                tmp = self.tik_inst.Scalar(dtype="float32", name="tmp", init_value=self.cur_roi_sample_w)
                self._vdivs_fp32(dst=dst_hx_lx, divided=dst_hx_lx, fp_divisor=tmp,
                                 fp_divisor_tensor=self.cur_roi_sample_w_vec, instr_num=2)
            else:
                tmp = self.tik_inst.Scalar(dtype="float32", name="tmp", init_value=self.cur_roi_sample_h)
                self._vdivs_fp32(dst=dst_hx_lx, divided=dst_hx_lx, fp_divisor=tmp,
    ...
    def _calc_bilinear_interpolate_weight(self, grid_h_idx: tik.Scalar, disable_vbi: bool = False) -> None:
        ...
        hy = self.tik_inst.Scalar(dtype="float32", name="hy", init_value=self.hly[0, grid_h_idx])
        ly = self.tik_inst.Scalar(dtype="float32", name="ly", init_value=self.hly[1, grid_h_idx])
        if not self.op.support_vbi or disable_vbi:
            if self.op.dtype == "float32":
                ...
                with self.tik_inst.new_stmt_scope(disable_sync=True):
                    self.tik_inst.vmuls(mask=64, dst=self.w1234[0:], src=self.hlx, scalar=hy,
                                        repeat_times=2 * 2, dst_blk_stride=1, src_blk_stride=1,
                                        dst_rep_stride=8, src_rep_stride=8)
                    self.tik_inst.vmuls(mask=64, dst=self.w1234[2 * Constant.BATCH_SIZE:], src=self.hlx, scalar=ly,
                                        repeat_times=2 * 2, dst_blk_stride=1, src_blk_stride=1,
                                        dst_rep_stride=8, src_rep_stride=8)
            else:
                w_fp32 = self.tik_inst.Tensor(dtype="float32", shape=[4 * Constant.BATCH_SIZE],
                                              name="w1234_tmp", scope=tik.scope_ubuf)
                with self.tik_inst.new_stmt_scope(disable_sync=True):
                    self.tik_inst.vmuls(mask=64, dst=w_fp32[0:], src=self.hlx, scalar=hy,
                                        repeat_times=2 * 2, dst_blk_stride=1, src_blk_stride=1,
                                        dst_rep_stride=8, src_rep_stride=8)
                    self.tik_inst.vmuls(mask=64, dst=w_fp32[2 * Constant.BATCH_SIZE:], src=self.hlx, scalar=ly,
                                        repeat_times=2 * 2, dst_blk_stride=1, src_blk_stride=1,
                                        dst_rep_stride=8, src_rep_stride=8)
                # float32 -> float16
                self.tik_inst.vconv(mask=64, round_mode='', dst=self.w1234, src=w_fp32,
                                    repeat_times=2 * 4, dst_blk_stride=1, src_blk_stride=1,
                                    dst_rep_stride=4, src_rep_stride=8)
        elif self.op.dtype == 'float32':
            with self.tik_inst.new_stmt_scope(disable_sync=True):
                # 8*w1|8*w2|8*w3|8*w4|8*w1|8*w2|8*w3|8*w4|...
                self.tik_inst.vmuls(mask=8, dst=self.w1234[0:], src=self.hlx[0, 0], scalar=hy, repeat_times=16,
                                    dst_blk_stride=1, src_blk_stride=1, dst_rep_stride=4, src_rep_stride=1)
                self.tik_inst.vmuls(mask=8, dst=self.w1234[8:], src=self.hlx[1, 0], scalar=hy, repeat_times=16,
                                    dst_blk_stride=1, src_blk_stride=1, dst_rep_stride=4, src_rep_stride=1)
                self.tik_inst.vmuls(mask=8, dst=self.w1234[16:], src=self.hlx[0, 0], scalar=ly, repeat_times=16,
                                    dst_blk_stride=1, src_blk_stride=1, dst_rep_stride=4, src_rep_stride=1)
                self.tik_inst.vmuls(mask=8, dst=self.w1234[24:], src=self.hlx[1, 0], scalar=ly, repeat_times=16,
                                    dst_blk_stride=1, src_blk_stride=1, dst_rep_stride=4, src_rep_stride=1)
        else:  # float16
            w_fp32 = self.tik_inst.Tensor(dtype="float32", name="w_fp32", scope=tik.scope_ubuf,
                                          shape=[4 * Constant.BATCH_SIZE])
            with self.tik_inst.new_stmt_scope(disable_sync=True):
                self.tik_inst.vmuls(mask=8, dst=w_fp32[0:], src=self.hlx[0, 0], scalar=hy, repeat_times=16,
                                    dst_blk_stride=1, src_blk_stride=1, dst_rep_stride=4, src_rep_stride=1)
                self.tik_inst.vmuls(mask=8, dst=w_fp32[8:], src=self.hlx[1, 0], scalar=hy, repeat_times=16,
                                    dst_blk_stride=1, src_blk_stride=1, dst_rep_stride=4, src_rep_stride=1)
                self.tik_inst.vmuls(mask=8, dst=w_fp32[16:], src=self.hlx[0, 0], scalar=ly, repeat_times=16,
                                    dst_blk_stride=1, src_blk_stride=1, dst_rep_stride=4, src_rep_stride=1)
                self.tik_inst.vmuls(mask=8, dst=w_fp32[24:], src=self.hlx[1, 0], scalar=ly, repeat_times=16,
                                    dst_blk_stride=1, src_blk_stride=1, dst_rep_stride=4, src_rep_stride=1)
            # float32 -> float16
            self.tik_inst.vconv(mask=64, round_mode='', dst=self.w1234, src=w_fp32,
                                repeat_times=2 * 4, dst_blk_stride=1, src_blk_stride=1,
    ...
    def _gather_bilinear_interpolate_4_points(self, dst_ub: tik.Tensor, fm_tensor: tik.Tensor,
        ...
        def _move_one_row(dst_offset, src_offset):
            if check_src_burst_gap:
                with self.tik_inst.if_scope(burst_src_gap <= Constant.BURST_STRIDE_MAX):
                    self.tik_inst.data_move(dst=dst_ub[dst_offset:], src=fm_tensor[src_offset:],
                                            sid=0, nburst=c1_num, burst=burst_len,
                                            src_stride=burst_src_gap, dst_stride=burst_dst_gap)
                with self.tik_inst.else_scope():
                    with self.tik_inst.for_range(0, c1_num) as _c1_idx:
                        _dst_offset = dst_offset + _c1_idx * 4 * Constant.C0
                        _src_offset = src_offset + _c1_idx * fm_hwc0
                        self.tik_inst.data_move(dst=dst_ub[_dst_offset:], src=fm_tensor[_src_offset:],
        ...
        burst_len = self.tik_inst.Scalar("int32", "burst_len", init_value=2 * self.op.c0_blocks)
        with self.tik_inst.if_scope(x_low == x_high):
        ...
        with self.tik_inst.new_stmt_scope(disable_sync=True):
            ...
            with self.tik_inst.if_scope(y_low != y_high):
                _move_one_row(2 * Constant.C0, fm_offset + 1 * fm_row_elements)

    # 'pylint: disable=too-many-arguments
    def _bilinear_interpolate_without_vbi(self, grid_value: tik.Tensor, fm_4_points: tik.Tensor,
                                          c1_num: tik.Scalar, grid_w_idx: tik.Scalar,
                                          x_low: tik.Scalar, x_high: tik.Scalar,
                                          y_low: tik.Scalar, y_high: tik.Scalar) -> None:
        op = self.op
        with self.tik_inst.new_stmt_scope():
            w1 = self.tik_inst.Scalar(dtype=op.dtype, name="w1", init_value=self.w1234[grid_w_idx])
            w2 = self.tik_inst.Scalar(dtype=op.dtype, name="w2",
                                      init_value=self.w1234[Constant.BATCH_SIZE + grid_w_idx])
            w3 = self.tik_inst.Scalar(dtype=op.dtype, name="w3",
                                      init_value=self.w1234[2 * Constant.BATCH_SIZE + grid_w_idx])
            w4 = self.tik_inst.Scalar(dtype=op.dtype, name="w4",
                                      init_value=self.w1234[3 * Constant.BATCH_SIZE + grid_w_idx])

            tmp_result = self.tik_inst.Tensor(dtype=op.dtype, shape=[3, c1_num, Constant.C0],
                                              name="tmp_result", scope=tbe_platform.scope_ubuf)
            p2_offset = self.tik_inst.Scalar(dtype="int32", name="p2_offset", init_value=x_high - x_low)
            p3_offset = self.tik_inst.Scalar(dtype="int32", name="p3_offset", init_value=(y_high - y_low) * 2)
            p4_offset = self.tik_inst.Scalar(dtype="int32", name="p4_offset", init_value=p3_offset + p2_offset)
            ...
            with self.tik_inst.new_stmt_scope(disable_sync=True):
                # `v1=p1*w1`
                self.tik_inst.vmuls(mask=mask, dst=grid_value, src=fm_4_points[0:],
                ...
                # `v2=p2*w2`
                self.tik_inst.vmuls(mask=mask, dst=tmp_result[0, 0, 0], src=fm_4_points[p2_offset * Constant.C0:],
                                    scalar=w2, repeat_times=repeat, dst_blk_stride=1, src_blk_stride=1,
                                    dst_rep_stride=dst_rep_stride, src_rep_stride=src_rep_stride)
                # `v3=p3*w3`
                self.tik_inst.vmuls(mask=mask, dst=tmp_result[1, 0, 0], src=fm_4_points[p3_offset * Constant.C0:],
                                    scalar=w3, repeat_times=repeat, dst_blk_stride=1, src_blk_stride=1,
                                    dst_rep_stride=dst_rep_stride, src_rep_stride=src_rep_stride)
                # `v4=p4*w4`
                self.tik_inst.vmuls(mask=mask, dst=tmp_result[2, 0, 0], src=fm_4_points[p4_offset * Constant.C0:],
                                    scalar=w4, repeat_times=repeat, dst_blk_stride=1, src_blk_stride=1,
                                    dst_rep_stride=dst_rep_stride, src_rep_stride=src_rep_stride)
            with self.tik_inst.new_stmt_scope(disable_sync=True):
                # `v1 + v2`
                self.tik_inst.vadd(mask=mask, dst=grid_value,
                                   src0=grid_value, src1=tmp_result[0, 0, 0],
                ...
                # `v3 + v4`
                self.tik_inst.vadd(mask=mask, dst=tmp_result[1, 0, 0],
                                   src0=tmp_result[1, 0, 0], src1=tmp_result[2, 0, 0],
                                   repeat_times=repeat, dst_blk_stride=1, src0_blk_stride=1, src1_blk_stride=1,
                                   dst_rep_stride=op.c0_blocks, src0_rep_stride=op.c0_blocks,
                                   src1_rep_stride=op.c0_blocks)
            # `v1 + v2` + `v3 + v4`
            self.tik_inst.vadd(mask=mask, dst=grid_value, src0=grid_value, src1=tmp_result[1, 0, 0],
                               repeat_times=repeat, dst_blk_stride=1, src0_blk_stride=1, src1_blk_stride=1,
    ...
    def _bilinear_interpolate_with_vbi(self, out_y_ub: tik.Tensor, out_w_offset: tik.Scalar, out_w_num: tik.Scalar,
                                       fm_in_ub: tik.Tensor, fm_offset_base: tik.Scalar,
                                       fm_in_ub_h: tik.Scalar, fm_in_ub_w: tik.Scalar,
                                       c1_num: tik.Scalar, grid_w_num: tik.Scalar,
                                       y_low: tik.Scalar, y_high: tik.Scalar):
        op = self.op
        aligned_grid_w_num = ceil_div(grid_w_num, 8) * 8  # range: [8, 128]
        rep_times = aligned_grid_w_num // 8  # range: [1, 16]
        src0_offset_backup = self.tik_inst.Tensor(dtype="int32", name="src0_offset_backup", scope=tik.scope_ubuf,
                                                  shape=[4, aligned_grid_w_num])
        self._calc_fm_offset_used_in_vbi(dst_offset=src0_offset_backup, fm_offset_base=fm_offset_base,
                                         fm_in_ub_w=fm_in_ub_w, grid_w_num=grid_w_num,
                                         y_low=y_low, y_high=y_high)
        vbi_c1_per_loop = self.tik_inst.Scalar(dtype="int32", name="vbi_c1_per_loop",
                                               init_value=255 // rep_times)
        vbi_c1_limit = self.tik_inst.Scalar(dtype="int32", name="vbi_c1_limit",
                                            init_value=self.vbi_memory // (self.vbi_mem_factor * aligned_grid_w_num))
        with self.tik_inst.if_scope(vbi_c1_per_loop > vbi_c1_limit):
        ...
        with self.tik_inst.for_range(0, c1_loops) as _c1_loop_idx:
            ...
            src0_offset = self.tik_inst.Tensor(dtype="int32", name="src0_offset", scope=tik.scope_ubuf,
                                               shape=[c1, 4, aligned_grid_w_num])
            w1234_dup = self.tik_inst.Tensor(dtype=op.dtype, name="w1234_dup", scope=tik.scope_ubuf,
                                             shape=[c1, 4, aligned_grid_w_num])
            grid_value = self.tik_inst.Tensor(dtype=op.dtype, name="grid_value", scope=tik.scope_ubuf,
                                              shape=[c1, aligned_grid_w_num, Constant.C0])
            with self.tik_inst.for_range(0, c1) as _c1_idx:
                self.tik_inst.vadds(mask=32, dst=src0_offset[_c1_idx, 0, 0], src=src0_offset_backup,
                ...
                self.tik_inst.vadds(mask=32, dst=w1234_dup[_c1_idx, 0, 0], src=self.w1234,
                                    scalar=0.0,
                                    repeat_times=rep_times, dst_blk_stride=1, src_blk_stride=1,
            ...
            if op.dtype == 'float32':
                # `C0[0:8]`
                self.tik_inst.vbi(mask=64, dst=grid_value[0:],
    ...
    # 'pylint: disable=too-many-arguments
    def _calc_fm_offset_used_in_vbi(self, dst_offset: tik.Tensor,
                                    fm_offset_base: tik.Scalar, fm_in_ub_w: tik.Scalar,
        ...
        self.tik_inst.vadds(mask=8, dst=dst_offset[0:], src=self.x_lh[0, 0], scalar=fm_offset_base,
                            repeat_times=rep_times, dst_blk_stride=1, src_blk_stride=1,
                            dst_rep_stride=4, src_rep_stride=1)
        self.tik_inst.vadds(mask=8, dst=dst_offset[8:], src=self.x_lh[1, 0], scalar=fm_offset_base,
                            repeat_times=rep_times, dst_blk_stride=1, src_blk_stride=1,
                            dst_rep_stride=4, src_rep_stride=1)
        tmp_scalar = self.tik_inst.Scalar(dtype="int32", name="tmp_scalar", init_value=fm_offset_base)
        with self.tik_inst.if_scope(y_low != y_high):
            tmp_scalar.set_as(fm_offset_base + fm_in_ub_w)
        self.tik_inst.vadds(mask=8, dst=dst_offset[16:], src=self.x_lh[0, 0], scalar=tmp_scalar,
                            repeat_times=rep_times, dst_blk_stride=1, src_blk_stride=1,
                            dst_rep_stride=4, src_rep_stride=1)
        self.tik_inst.vadds(mask=8, dst=dst_offset[24:], src=self.x_lh[1, 0], scalar=tmp_scalar,
                            repeat_times=rep_times, dst_blk_stride=1, src_blk_stride=1,
                            dst_rep_stride=4, src_rep_stride=1)
        self.tik_inst.vmuls(mask=32, dst=dst_offset, src=dst_offset, scalar=Constant.C0 * op.data_size,
                            repeat_times=rep_times, dst_blk_stride=1, src_blk_stride=1,
                            dst_rep_stride=4, src_rep_stride=4)
        # reset tail to 0 in case of tensor overlap
        start_1, start_2 = grid_w_num // 8, grid_w_num % 8
        for idx in range(0, 4):
            with self.tik_inst.for_range(start_2, 8) as inner_idx:
    ...
    def _pool_output_value_without_vbi(self, out_y_ub: tik.Tensor, grid_value: tik.Tensor, c1_num: tik.Scalar,
                                       out_w_offset: tik.Scalar, out_w_num: tik.Scalar,
                                       grid_w_idx: tik.Scalar) -> None:
        relative_out_w = self.tik_inst.Scalar(dtype="int64", name="relative_out_w",
                                              init_value=self.pool_w_idx[grid_w_idx])
        relative_out_w.set_as(relative_out_w - out_w_offset)
        # in case of memory out of range due to coordinate calculation error
        relative_out_w_max = self.tik_inst.Scalar(dtype="int64", name="relative_out_w_max",
                                                  init_value=out_w_num - 1)
        self.tik_inst.scalar_max(relative_out_w, relative_out_w, 0)
        self.tik_inst.scalar_min(relative_out_w, relative_out_w, relative_out_w_max)
    ...
    # 'pylint: disable=too-many-arguments
    def _pool_output_value_with_vbi(self, out_y_ub: tik.Tensor, grid_value: tik.Tensor,
        ...
        with self.tik_inst.for_range(0, grid_w_num) as _grid_w_idx:
    ...
    def _move_out_y_ub_to_gm(self, out_y_ub: tik.Tensor,
                             c1_num: tik.Scalar, w_num: tik.Scalar, gm_offset: tik.Scalar) -> None:
        ...
        with self.tik_inst.new_stmt_scope(disable_sync=True):
            with self.tik_inst.if_scope(burst_dst_gap <= Constant.BURST_STRIDE_MAX):
                self.tik_inst.data_move(dst=op.y_gm[gm_offset:], src=out_y_ub,
                                        sid=0, nburst=burst_num, burst=burst_len,
                                        src_stride=0, dst_stride=burst_dst_gap)
            with self.tik_inst.else_scope():
                with self.tik_inst.for_range(0, burst_num) as _c1_idx:
                    dst_offset = gm_offset + _c1_idx * op.pool_h * op.pool_w * Constant.C0
                    src_offset = _c1_idx * w_num * Constant.C0
                    self.tik_inst.data_move(dst=op.y_gm[dst_offset:], src=out_y_ub[src_offset:],
                                            sid=0, nburst=1, burst=burst_len,
                                            src_stride=0, dst_stride=0)

    def _cache_whole_feature_map(self, cache_dst: tik.Tensor) -> None:
        """whole feature map GM -> UB/L1"""
        with self.tik_inst.if_scope(self.cache_fm_idx != self.cur_fm_idx):
            op = self.op
            # no need to check burst_len. max is 65535 blocks. which is almost 2MB, but UB is 100-200 kB
            ...
            self.tik_inst.data_move(dst=cache_dst,
    ...
    def _move_fm_to_ub(self, dst: tik.Tensor, src: tik.Tensor, src_offset: tik.Scalar, c1_num: tik.Scalar,
        ...
        with self.tik_inst.new_stmt_scope(disable_sync=True):
            if check_src_gap:
                with self.tik_inst.if_scope(burst_src_gap <= Constant.BURST_STRIDE_MAX):
                    self.tik_inst.data_move(dst=dst, src=src[src_offset:],
                                            sid=0, nburst=c1_num, burst=burst_len,
                                            src_stride=burst_src_gap, dst_stride=burst_dst_gap)
                with self.tik_inst.else_scope():
                    with self.tik_inst.for_range(0, c1_num) as _c1_idx:
                        ...
                        self.tik_inst.data_move(dst=dst[_dst_offset:], src=src[_src_offset:],
                                                sid=0, nburst=1, burst=burst_len,
    ...
    def _gather_4_points_from_gm(self, dst_ub: tik.Tensor, relative_c1_offset: tik.Scalar,
        ...
        if op.l1_size_bytes > 0:
            with self.tik_inst.if_scope(self.cache_all_fm_in_l1 == 1):
    ...
    def _calc_vbi_memory(self):
        ...
        aligned_grid_w_max = self.tik_inst.Scalar(dtype="int32", name="aligned_grid_w_max",
                                                  init_value=ceil_div(self.cur_roi_sample_w *
                                                                      op.out_w_per_loop, 8) * 8)
        with self.tik_inst.if_scope(aligned_grid_w_max > Constant.BATCH_SIZE):
            aligned_grid_w_max.set_as(Constant.BATCH_SIZE)
        # vbi vertical_repeat_times range: [1, 255]
        vbi_rep_times = aligned_grid_w_max // 8
        vbi_c1_per_loop = self.tik_inst.Scalar(dtype="int32", name="vbi_c1_per_loop", init_value=255 // vbi_rep_times)
        with self.tik_inst.if_scope(vbi_c1_per_loop > op.out_c1_per_loop):
            vbi_c1_per_loop.set_as(op.out_c1_per_loop)

        self.vbi_memory.set_as(vbi_c1_per_loop * aligned_grid_w_max * self.vbi_mem_factor)
        with self.tik_inst.if_scope(self.vbi_memory > self.memory_for_vbi_max):
...
class MoveWholeFeatureMapToUb(ProcessorBase):
    def __init__(self, op: "RoiAlign") -> None:
        super().__init__(op)
        self.fm_in_ub = self.tik_inst.Tensor(dtype=op.dtype, name="fm_in_ub", scope=tik.scope_ubuf,
    ...
    def bilinear_interpolate_one_grid_row(self, fm_in_ub: tik.Tensor, out_y_ub: tik.Tensor,
        ...
        if op.support_vbi:
            ...
            fm_4_points = self.tik_inst.Tensor(dtype=op.dtype, name="fm_4_points", scope=tik.scope_ubuf,
                                               shape=[c1_num, 2, 2, Constant.C0])
            grid_value = self.tik_inst.Tensor(dtype=op.dtype, name="grid_value", scope=tik.scope_ubuf,
                                              shape=[c1_num, Constant.C0])
            with self.tik_inst.for_range(0, grid_w_num) as _grid_w_idx:
                # calculate sample grid one by one
                x_low = self.tik_inst.Scalar(dtype="int32", name="x_low", init_value=self.x_lh[0, _grid_w_idx])
                x_high = self.tik_inst.Scalar(dtype="int32", name="x_high", init_value=self.x_lh[1, _grid_w_idx])
...
class MoveFeatureMapMultipleRows(ProcessorBase):
    def __init__(self, op: "RoiAlign") -> None:
        ...
        self.roi_rows_to_ub_batch = self.tik_inst.Scalar(dtype="int32", name="roi_rows_to_ub_batch")
        self.cache_c1 = self.tik_inst.Scalar(dtype="int64", name="cache_c1")
        self.cache_y0_low = self.tik_inst.Scalar(dtype="int32", name="cache_y0_low")
        self.cache_y1_high = self.tik_inst.Scalar(dtype="int32", name="cache_y1_high")
        if op.support_vbi:
            self.memory_for_vbi_max.set_as(op.max_ub_element * op.data_size + Constant.RESERVED_UB_BYTES_FOR_OUT_MIN -
                                           op.out_w_per_loop * op.out_c1_per_loop * Constant.C0 * op.data_size -
        ...
        if op.support_vbi:
            ...
            with self.tik_inst.for_range(0, grid_w_num) as _grid_w_idx:
                ...
                self._gather_bilinear_interpolate_4_points(dst_ub=fm_4_points, fm_tensor=fm_in_ub,
                                                           ...
                                                           c1_num=c1_num, x_low=x_low, x_high=x_high,
                                                           y_low=y_low, y_high=y_high, check_src_burst_gap=False)
                self._bilinear_interpolate_without_vbi(grid_value, fm_4_points,
                                                       c1_num, _grid_w_idx,
    ...
    def _cache_fm_rows_to_ub(self, dst_ub: tik.Tensor,
        ...
        """
        op = self.op
        with self.tik_inst.if_scope(tik.any(self.cache_c1 != absolute_c1_offset,
                                            self.cache_y1_high < y_high,
                                            self.cache_y0_low > y_low)):
            rows = self.tik_inst.Scalar(dtype="int64", name="rows", init_value=self.roi_rows_to_ub_batch)
            with self.tik_inst.if_scope(y_low + rows > op.feature_h):
            ...
            if op.l1_size_bytes > 0:
                with self.tik_inst.if_scope(self.cache_all_fm_in_l1 == 1):
                    # no need to check burst_src_gap vs 65535.
                    gm_offset = (((0 * op.c1 + relative_c1_offset) * op.feature_h + y_low
                                  ) * op.feature_w) * Constant.C0
                    self._move_fm_to_ub(dst_ub, self.fm_in_l1, gm_offset, c1_num, burst_len,
                                        src_hwc0, dst_hwc0, False)
                with self.tik_inst.else_scope():
                    # need to check burst_src_gap vs 65535.
                    gm_offset = (((self.cur_fm_idx * op.c1 + absolute_c1_offset) * op.feature_h + y_low
                                  ) * op.feature_w) * Constant.C0
                    self._move_fm_to_ub(dst_ub, op.features_gm, gm_offset, c1_num, burst_len,
                                        src_hwc0, dst_hwc0, True)
            else:
                # need to check burst_src_gap vs 65535.
                gm_offset = (((self.cur_fm_idx * op.c1 + absolute_c1_offset) * op.feature_h + y_low
                              ) * op.feature_w) * Constant.C0
...
class MoveFeatureMap4Points(ProcessorBase):
    ...
    def bilinear_interpolate_one_grid_row(self, fm_in_ub: tik.Tensor, out_y_ub: tik.Tensor,
        ...
        with self.tik_inst.for_range(0, grid_w_num) as _grid_w_idx:
            ...
            x_low = self.tik_inst.Scalar(dtype="int32", name="x_low", init_value=self.x_lh[0, _grid_w_idx])
            x_high = self.tik_inst.Scalar(dtype="int32", name="x_high", init_value=self.x_lh[1, _grid_w_idx])
            self._gather_4_points_from_gm(fm_4_points, relative_c1_offset, absolute_c1_offset, c1_num,
                                          x_low, x_high, y_low, y_high)
            self._bilinear_interpolate_without_vbi(grid_value, fm_4_points,
                                                   c1_num, _grid_w_idx,
                                                   x_low, x_high, y_low, y_high)
...
class RoiAlign(OpBase):
    """RoiAlign operator information only"""

    def __init__(self, features: dict, rois: dict, rois_n: dict, y: dict, pool_mode: str, kernel_name: str):
        OpBase.__init__(self, kernel_name)
        self.pool_mode = pool_mode
        # dtype of features
        self.dtype = features.get("dtype").lower()
        # use hardware vbi only (support float16 only). software vbi does not support scalar repeat yet...
        self.support_vbi = tbe_platform.api_check_support("tik.vbi", self.dtype) and \
                           not tbe_platform.api_check_support("tik.vbi", "float32")
        # reset ub available size for temporary variables
        self.ub_size_bytes = self.ub_size_bytes - Constant.RESERVED_UB_BYTES
        # get L1 available size
        ...
        inputs = [features, rois, rois_n] if rois_n else [features, rois]
        self.op_init_gm(inputs, [y], tiling_info=tiling_dict)
        self.features_gm, self.rois_gm = self.input_gm_list[0], self.input_gm_list[1]
        self.rois_n_gm = self.input_gm_list[2] if rois_n else None
        self.y_gm = self.output_gm_list[0]

        # data size in bytes
        self.data_size = common_util.get_data_size(self.dtype)
        # data count per block
        self.block_element = common_util.get_block_element(self.dtype)
        # max data could be put in ub
        self.max_ub_element = self.ub_size_bytes // self.data_size
        # applied blocks per C0. fp16 = 2, fp32 = 1
        self.c0_blocks = Constant.C0 // self.block_element

        # init tiling scalar
        self.tiling_key = self.tik_instance.Scalar("int64", name="tiling_key")
        self.rois_n = self.tik_instance.Scalar("int64", name="rois_n")
        self.rois_row_size = self.tik_instance.Scalar("int64", name="rois_row_size")
        self.feature_n = self.tik_instance.Scalar("int64", name="feature_n")
        self.c1 = self.tik_instance.Scalar("int64", name="c1")
        self.feature_h = self.tik_instance.Scalar("int64", name="feature_h")
        self.feature_w = self.tik_instance.Scalar("int64", name="feature_w")
        self.rois_per_core = self.tik_instance.Scalar("int64", name="rois_per_core")
        self.c1_per_core = self.tik_instance.Scalar("int64", name="c1_per_core")
        self.out_c1_per_loop = self.tik_instance.Scalar("int64", name="out_c1_per_loop")
        self.out_w_per_loop = self.tik_instance.Scalar("int64", name="out_w_per_loop")
        self.out_h_per_loop = self.tik_instance.Scalar("int64", name="out_h_per_loop")
        ...
        # parameters below will be responsible for coordinate. so we use fp32 to calculate even in fp16 scenario
        ...
        self.core_roi_offset = self.tik_instance.Scalar("int64", name="core_roi_offset")
        self.core_rois = self.tik_instance.Scalar("int64", name="core_rois")
        # c1 offset of current feature
        self.core_c1_offset = self.tik_instance.Scalar("int64", name="core_c1_offset")

        # processor dictionary for different tiling keys
        self._processors = {
            100: MoveWholeFeatureMapToUb,
            300: MoveFeatureMapMultipleRows,
    ...
    def tiling_args(self):
        with self.tik_instance.new_stmt_scope():
            ...
            self.feature_n.set_as(tiling_ub[4])
            self.c1.set_as(tiling_ub[5])
            self.feature_h.set_as(tiling_ub[6])
            self.feature_w.set_as(tiling_ub[7])
            self.rois_per_core.set_as(tiling_ub[8])
            self.c1_per_core.set_as(tiling_ub[9])
            self.out_c1_per_loop.set_as(tiling_ub[10])
            self.out_w_per_loop.set_as(tiling_ub[11])
            self.out_h_per_loop.set_as(tiling_ub[12])

            i32_start = 13 * 2
            tiling_ub_int32 = tiling_ub.reinterpret_cast_to("int32")
            self.pool_w.set_as(tiling_ub_int32[i32_start + 0])
            self.pool_h.set_as(tiling_ub_int32[i32_start + 1])
            self.samples.set_as(tiling_ub_int32[i32_start + 2])
            self.roi_end_mode.set_as(tiling_ub_int32[i32_start + 3])

            tiling_ub_fp32 = tiling_ub.reinterpret_cast_to("float32")
            self.scale.set_as(tiling_ub_fp32[i32_start + 4])
            self.pool_w_reciprocal.set_as(tiling_ub_fp32[i32_start + 5])
            self.pool_h_reciprocal.set_as(tiling_ub_fp32[i32_start + 6])
            self.samples_reciprocal.set_as(tiling_ub_fp32[i32_start + 7])

    def core_scedule_args(self, core_idx: tik.Scalar):
        """prepare some variables as per core_id"""
        with self.tik_instance.if_scope(self.c1_per_core < self.c1):
            # schedule core by roisn * c1
            self.core_roi_offset.set_as(core_idx // (self.c1 // self.c1_per_core))
            self.core_rois.set_as(1)
            self.core_c1_offset.set_as((core_idx * self.c1_per_core) % self.c1)
        with self.tik_instance.else_scope():
            # schedule core by roisn
            self.core_roi_offset.set_as(core_idx * self.rois_per_core)
            with self.tik_instance.if_scope(core_idx + 1 < self.running_core_num):
                self.core_rois.set_as(self.rois_per_core)
            with self.tik_instance.else_scope():
                self.core_rois.set_as(self.rois_n - core_idx * self.rois_per_core)
            self.core_c1_offset.set_as(0)
...
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT,
                            ...
                            para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT,
                            para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_STR,
                            para_check.KERNEL_NAME)
def roi_align(features: dict, rois: dict, rois_n: Optional[dict],  # inputs
              y: dict,  # outputs
              spatial_scale: float, pooled_height: int, pooled_width: int,
              sample_num: int = 2, roi_end_mode: int = 1, pool_mode: str = 'avg',
              kernel_name: str = "roi_align",
              impl_mode: str = "high_performance"):
    """
    ROIAlign is a pooling layer used over feature maps of non-uniform input sizes
    and outputs feature maps of fixed size.

    Parameters
    ----------
    features: dict
        5D input tensor of shape [N, C1, H, W, C0] with feature maps of float32 or float16. Required.
    rois: dict
        2D input tensor of shape [NUM_ROIS, 5] describing box consisting of 5 element tuples:
        [features_index, x_start, y_start, x_end, y_end] with dtype same with features. Required.
        It can also be a tensor of shape [NUM_ROIS, 8], in which the last 3 number is invalid.
    rois_n: dict
        1D input tensor of shape [NUM_ROIS] with features indices of type int. Optional.
    y: dict
        5D output tensor of shape [NUM_ROIS, C1, pooled_height, pooled_width, C0] with dtype same with features.
    spatial_scale: float
        a multiplicative spatial scale factor to translate ROI coordinates to input feature map.
    pooled_height: int
        height of the ROI output feature map
    pooled_width: int
        width of the ROI output feature map
    sample_num: int
        number of bins over height and width to use to calculate each output element.
        if 0 is specified, it would be ceil(roi_height / pooled_height) and ceil(roi_width / pooled_width)
    roi_end_mode: int
        aligned_mode specifies how to transform the coordinates in original tensor to resized tensor.
        candidates are 0 / 1 / 2 / 3
    pool_mode: str
        mode specify a method to perform pooling to produce output feature map elements.
        candidates are avg / max
    kernel_name: str
        kernel name
    impl_mode: str
        implement mode. works with float16 input.
        candidates are "high_performance" and "high_precision".
        RESERVED not supported yet.
    Returns
    -------
    tik_instance: tik_instance
    """
    para_check.check_dtype(features.get("dtype"), ("float32", "float16"), param_name="features")
    para_check.check_dtype(rois.get("dtype"), ("float32", "float16"), param_name="rois")
    if rois_n:
        para_check.check_dtype(rois_n.get("dtype"), ("int32",), param_name="rois_n")

    obj = RoiAlign(features, rois, rois_n, y, pool_mode, kernel_name)

    return obj.roi_align_compute()
