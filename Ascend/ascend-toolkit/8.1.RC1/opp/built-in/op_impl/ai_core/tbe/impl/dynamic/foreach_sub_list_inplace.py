...
@tbe_register.register_operator("ForeachSubListInplace", trans_bool_to_s8=False)
...
def foreach_sub_list_inplace(x1_in__, x2_in__, alpha_in__=None, kernel_name="foreach_sub_list_inplace", impl_mode=""):
    ...
    origin_func_name = "foreach_sub_list_inplace"
    ascendc_src_dir_ex = "foreach_sub_list_inplace"
    ascendc_src_dir = "foreach_sub_list_inplace"
    ascendc_src_file = "foreach_sub_list_inplace.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator ForeachSubListInplace, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "ForeachSubListInplace"
...
def op_select_format(x1_in__, x2_in__, alpha_in__=None, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, alpha_in__)
    result = check_op_cap("op_select_format", "ForeachSubListInplace", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x1_in__, x2_in__, alpha_in__=None, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, alpha_in__)
    result = check_op_cap("get_op_specific_info", "ForeachSubListInplace", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
