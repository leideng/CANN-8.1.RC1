...
def pad_d(input_x, output_x, paddings, kernel_name="pad_d"):
    ...
    if src_dtype not in ["float32", "float16", "bfloat16", "int32"]:
        error_detail = "Only support float, float16, bfloat16 and int32"
        error_manager_vector.raise_err_two_input_dtype_invalid("PadD", "src_dtype", "dst_dtype",
                                                               error_detail)
    src_dtype = "float16" if src_dtype == "bfloat16" else src_dtype
    tik_obj = tik.Tik()
