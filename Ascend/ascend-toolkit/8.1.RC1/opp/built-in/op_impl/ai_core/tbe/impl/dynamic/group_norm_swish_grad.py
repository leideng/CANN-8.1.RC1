...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['dy', 'mean', 'rstd', 'x', 'gamma', 'beta']
    output_names = ['dx', 'dgamma', 'dbeta']
...
def _build_args(dy_in__, mean_in__, rstd_in__, x_in__, gamma_in__, beta_in__, dx_out_, dgamma_out_, dbeta_out_, num_groups, data_format, swish_scale, dgamma_is_require, dbeta_is_require):
    __inputs__ = []
    for arg in [dy_in__, mean_in__, rstd_in__, x_in__, gamma_in__, beta_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [dx_out_, dgamma_out_, dbeta_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if swish_scale != None:
        attr = {}
        attr["name"] = "swish_scale"
        attr["dtype"] = "float"
        attr["value"] = swish_scale
        __attrs__.append(attr)
    if dgamma_is_require != None:
        attr = {}
        attr["name"] = "dgamma_is_require"
        attr["dtype"] = "bool"
        attr["value"] = dgamma_is_require
        __attrs__.append(attr)
    if dbeta_is_require != None:
        attr = {}
        attr["name"] = "dbeta_is_require"
        attr["dtype"] = "bool"
        attr["value"] = dbeta_is_require
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("GroupNormSwishGrad", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def group_norm_swish_grad(dy_in__, mean_in__, rstd_in__, x_in__, gamma_in__, beta_in__, dx_out_, dgamma_out_, dbeta_out_, num_groups, data_format="NCHW", swish_scale=1, dgamma_is_require=True, dbeta_is_require=True, kernel_name="group_norm_swish_grad", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(dy_in__, mean_in__, rstd_in__, x_in__, gamma_in__, beta_in__, dx_out_, dgamma_out_, dbeta_out_, num_groups, data_format, swish_scale, dgamma_is_require, dbeta_is_require)
    ...
    origin_func_name = "group_norm_swish_grad"
    ascendc_src_dir_ex = "group_norm_swish_grad"
    ascendc_src_dir = "group_norm_swish_grad"
    ascendc_src_file = "group_norm_swish_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator GroupNormSwishGrad, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "GroupNormSwishGrad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[dy_in__, mean_in__, rstd_in__, x_in__, gamma_in__, beta_in__], origin_outputs = [dx_out_, dgamma_out_, dbeta_out_],\
...
def op_select_format(dy_in__, mean_in__, rstd_in__, x_in__, gamma_in__, beta_in__, dx_out_, dgamma_out_, dbeta_out_, num_groups, data_format="NCHW", swish_scale=1, dgamma_is_require=True, dbeta_is_require=True, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(dy_in__, mean_in__, rstd_in__, x_in__, gamma_in__, beta_in__, dx_out_, dgamma_out_, dbeta_out_, num_groups, data_format, swish_scale, dgamma_is_require, dbeta_is_require)
    result = check_op_cap("op_select_format", "GroupNormSwishGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(dy_in__, mean_in__, rstd_in__, x_in__, gamma_in__, beta_in__, dx_out_, dgamma_out_, dbeta_out_, num_groups, data_format="NCHW", swish_scale=1, dgamma_is_require=True, dbeta_is_require=True, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(dy_in__, mean_in__, rstd_in__, x_in__, gamma_in__, beta_in__, dx_out_, dgamma_out_, dbeta_out_, num_groups, data_format, swish_scale, dgamma_is_require, dbeta_is_require)
    result = check_op_cap("get_op_specific_info", "GroupNormSwishGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
