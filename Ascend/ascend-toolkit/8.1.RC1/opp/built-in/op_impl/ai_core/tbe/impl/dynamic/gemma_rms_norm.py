...
def _build_args(x_in__, gamma_in__, y_out_, rstd_out_, epsilon):
    __inputs__ = []
    for arg in [x_in__, gamma_in__]:
        if arg != None:
...
@tbe_register.register_operator("GemmaRmsNorm", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def gemma_rms_norm(x_in__, gamma_in__, y_out_, rstd_out_, epsilon=1e-06, kernel_name="gemma_rms_norm", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, gamma_in__, y_out_, rstd_out_, epsilon)
    ...
    origin_func_name = "gemma_rms_norm"
    ascendc_src_dir_ex = "gemma_rms_norm"
    ascendc_src_dir = "gemma_rms_norm"
    ascendc_src_file = "gemma_rms_norm.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator GemmaRmsNorm, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "GemmaRmsNorm"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, gamma_in__], origin_outputs = [y_out_, rstd_out_],\
...
def op_select_format(x_in__, gamma_in__, y_out_, rstd_out_, epsilon=1e-06, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, gamma_in__, y_out_, rstd_out_, epsilon)
    result = check_op_cap("op_select_format", "GemmaRmsNorm", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, gamma_in__, y_out_, rstd_out_, epsilon=1e-06, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, gamma_in__, y_out_, rstd_out_, epsilon)
    result = check_op_cap("get_op_specific_info", "GemmaRmsNorm", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
