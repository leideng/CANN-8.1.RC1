...
def _build_args(x1_in__, x2_in__, bias_in__, x3_in__, y_out_, shifts):
    __inputs__ = []
    for arg in [x1_in__, x2_in__, bias_in__, x3_in__]:
        if arg != None:
...
@tbe_register.register_operator("SwinAttentionFFN", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_LIST_INT, para_check.KERNEL_NAME)
def swin_attention_ffn(x1_in__, x2_in__, bias_in__, x3_in__=None, y_out_=None, shifts=[], kernel_name="swin_attention_ffn", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, bias_in__, x3_in__, y_out_, shifts)
    ...
    origin_func_name = "swin_attention_ffn"
    ascendc_src_dir_ex = "swin_attention_ffn"
    ascendc_src_dir = "swin_attention_f_f_n"
    ascendc_src_file = "swin_attention_ffn.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator SwinAttentionFFN, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "SwinAttentionFFN"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x1_in__, x2_in__, bias_in__, x3_in__], origin_outputs = [y_out_],\
...
def op_select_format(x1_in__, x2_in__, bias_in__, x3_in__=None, y_out_=None, shifts=[], impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, bias_in__, x3_in__, y_out_, shifts)
    result = check_op_cap("op_select_format", "SwinAttentionFFN", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x1_in__, x2_in__, bias_in__, x3_in__=None, y_out_=None, shifts=[], impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, bias_in__, x3_in__, y_out_, shifts)
    result = check_op_cap("get_op_specific_info", "SwinAttentionFFN", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
