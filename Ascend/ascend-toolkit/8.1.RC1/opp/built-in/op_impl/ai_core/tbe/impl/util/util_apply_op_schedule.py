...
def _get_custom_max_ub_count(compute_width):
    ...
    total_size = CceProductParams().getParams("Unified_Buffer") // 2
...
def _cache_and_inline(sch, cache_list, inline_compute_list,
    ...
    buffer_read_list = []
    for k, v in cache_list['read'].items():
        buffer_read_list.append(sch.cache_read(k, cce_params.scope_ubuf, v))
...
def _get_emit_insn_map(emit_insn_intrin):
    ...
    emit_insn_map = {
        "elewise_single_round": "vector_conv_round",
...
def _get_info_from_gragh(outs):
    ...
    def get_read_and_inline(cur_buf):
        if isinstance(cur_buf.op, tvm.PlaceholderOp):
            if cur_buf in read_cache:
                read_cache[cur_buf].add(tmp_op["dst_buffer"])
            else:
                read_cache[cur_buf] = {tmp_op["dst_buffer"]}
        else:
...
def _execute_split_core(sch, out, shape, split_axis, rfactor):
    ...
    if out_extent > 1:
        ...
        max_core_num = get_soc_spec("CORE_NUM")
...
def _common_apply_op_schedule(outs, rewrite_back_map, config, ir_tensor_list=None):
    """
    Parameters
    ----------
    ...
    # tiling
    rfactor, split_axis = _tile_apply_ops(shape, out.dtype, config.args.width)

    # split axes
    axes = _execute_split_core(sch, out, shape, split_axis, rfactor)

    _execute_compute_at(sch, out, buffer_list, axes, config.name.scalar)
    if config.options.optype == "SGD":
        for output in outs:
            sch[output].compute_at(sch[out], axes[OUTER_AXIS_IDX])
    ...
    if config.options.optype == "SGD":
        for output in outs:
            sch[output].emit_insn(output.op.axis[0], "dma_copy")
        sch[out].emit_insn(axes[INNER_AXIS_IDX], "phony_insn")
    else:
    ...
    if config.options.optype == "SGD":
        for i, o in rewrite_back_map.items():
            sch[i].reused_by(o)
    else:
...
def common_apply_op_process(config, kernel_name, same_flag=True):
    ...
    if (not isinstance(attrs, collections.abc.Iterable)) or isinstance(attrs, str):
        attrs = [attrs]
    ...
    sch = _common_apply_op_schedule(outs, rewrite_back_map, config)
    ...
    if config.options.build is not None:
        config_dict = config.options.build
    config_dict['dummy_placeholder'] = False
    with build_config(**config_dict):
        build(sch, input_tensors + list(outs), "cce", name=kernel_name)
