...
def _build_args(indices_in__, shape_in__, index_out_, axis, combine):
    __inputs__ = []
    for arg in [indices_in__, shape_in__]:
        if arg != None:
...
@tbe_register.register_operator("LinearIndex", trans_bool_to_s8=False)
...
def linear_index(indices_in__, shape_in__, index_out_, axis=-1, combine=False, kernel_name="linear_index", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(indices_in__, shape_in__, index_out_, axis, combine)
    ...
    origin_func_name = "linear_index"
    ascendc_src_dir_ex = "linear_index"
    ascendc_src_dir = "linear_index"
    ascendc_src_file = "linear_index.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator LinearIndex, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "LinearIndex"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[indices_in__, shape_in__], origin_outputs = [index_out_],\
...
def op_select_format(indices_in__, shape_in__, index_out_, axis=-1, combine=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(indices_in__, shape_in__, index_out_, axis, combine)
    result = check_op_cap("op_select_format", "LinearIndex", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(indices_in__, shape_in__, index_out_, axis=-1, combine=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(indices_in__, shape_in__, index_out_, axis, combine)
    result = check_op_cap("get_op_specific_info", "LinearIndex", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
