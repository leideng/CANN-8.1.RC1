...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['y_grad', 'gates', 'indices', 'locations']
    output_names = ['x_grad']
...
def _build_args(y_grad_in__, gates_in__, indices_in__, locations_in__, x_grad_out_):
    __inputs__ = []
    for arg in [y_grad_in__, gates_in__, indices_in__, locations_in__]:
        if arg != None:
...
@tbe_register.register_operator("MoeTutelCombineX", trans_bool_to_s8=False)
...
def moe_tutel_combine_x(y_grad_in__, gates_in__, indices_in__, locations_in__, x_grad_out_, kernel_name="moe_tutel_combine_x", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(y_grad_in__, gates_in__, indices_in__, locations_in__, x_grad_out_)
    ...
    origin_func_name = "moe_tutel_combine_x"
    ascendc_src_dir_ex = "moe_tutel_combine_x"
    ascendc_src_dir = "moe_tutel_combine_x"
    ascendc_src_file = "moe_tutel_combine_x.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MoeTutelCombineX, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "MoeTutelCombineX"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[y_grad_in__, gates_in__, indices_in__, locations_in__], origin_outputs = [x_grad_out_],\
...
def op_select_format(y_grad_in__, gates_in__, indices_in__, locations_in__, x_grad_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(y_grad_in__, gates_in__, indices_in__, locations_in__, x_grad_out_)
    result = check_op_cap("op_select_format", "MoeTutelCombineX", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(y_grad_in__, gates_in__, indices_in__, locations_in__, x_grad_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(y_grad_in__, gates_in__, indices_in__, locations_in__, x_grad_out_)
    result = check_op_cap("get_op_specific_info", "MoeTutelCombineX", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
