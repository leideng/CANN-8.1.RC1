...
def _build_args(x_in__, cos_in__, sin_in__, y_out_):
    __inputs__ = []
...
@tbe_register.register_operator("InterleaveRope", trans_bool_to_s8=False)
...
def interleave_rope(x_in__, cos_in__, sin_in__, y_out_, kernel_name="interleave_rope", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, cos_in__, sin_in__, y_out_)
    ...
    origin_func_name = "interleave_rope"
    ascendc_src_dir_ex = "interleave_rope"
    ascendc_src_dir = "interleave_rope"
    ascendc_src_file = "interleave_rope.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator InterleaveRope, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "InterleaveRope"
...
def op_select_format(x_in__, cos_in__, sin_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, cos_in__, sin_in__, y_out_)
    result = check_op_cap("op_select_format", "InterleaveRope", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, cos_in__, sin_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, cos_in__, sin_in__, y_out_)
    result = check_op_cap("get_op_specific_info", "InterleaveRope", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
