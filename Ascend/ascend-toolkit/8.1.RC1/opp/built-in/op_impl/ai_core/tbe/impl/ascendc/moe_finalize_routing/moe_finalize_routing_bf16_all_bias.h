...
template <typename T>
__aicore__ inline void MoeFinalizeRoutingBF16AllBias<T>::Init(
    GM_ADDR expandedPermutedRows,
    GM_ADDR skip1,
    GM_ADDR skip2,
    GM_ADDR bias,
...
__aicore__ inline void MoeFinalizeRoutingBF16AllBias<T>::PrepareData()
{
    ...
    DataCopyExtParams copyParamsSrcToDstRow{static_cast<uint16_t>(K_), static_cast<uint32_t>(curCoreHandleNum_ *
                                         sizeof(int32_t)), static_cast<uint32_t>((BS_ - curCoreHandleNum_) * 
                                         sizeof(int32_t)), 0, 0};
...
__aicore__ inline void MoeFinalizeRoutingBF16AllBias<T>::CopyIn(int64_t nLoopIdx, int64_t curRepeatTimes)
{
    ...
    // ---------------------------- [Scales] -------------------------------
    DataCopyParams copyParamsScales{static_cast<uint16_t>(curRepeatTimes), static_cast<uint16_t>(K_ * sizeof(T)),
                                    0, 0};
...
__aicore__ inline void MoeFinalizeRoutingBF16AllBias<T>::Compute(int64_t nLoopIdx, int64_t curRepeatTimes)
{
    ...
    for (int64_t i = 0; i < curRepeatTimes; i++) {
        ...
        for (int64_t j = 0; j < K_ / PARALLEL_NUM; j++) {
            /*******************************乒***********************************************/
            wait_flag(PIPE_MTE2, PIPE_S, EVENT_ID0);
            int64_t expandedSrcToDstRowIndexDb0 = biasInLoop + i + PARALLEL_NUM * j * Int32AlignmentProcess(curCoreHandleNum_);
            ...
            int64_t expandedSrcToDstRowIndexDb1 = biasInLoop + i + (PARALLEL_NUM * j + 1) * Int32AlignmentProcess(curCoreHandleNum_);
