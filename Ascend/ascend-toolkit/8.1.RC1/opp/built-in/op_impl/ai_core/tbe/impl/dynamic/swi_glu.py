...
@tbe_register.register_operator("SwiGlu", trans_bool_to_s8=False)
...
def swi_glu(x_in__, y_out_, dim=-1, kernel_name="swi_glu", impl_mode=""):
    ...
    origin_func_name = "swi_glu"
    ascendc_src_dir_ex = "swi_glu"
    ascendc_src_dir = "swi_glu"
    ascendc_src_file = "swi_glu.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator SwiGlu, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "SwiGlu"
...
def op_select_format(x_in__, y_out_, dim=-1, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, dim)
    result = check_op_cap("op_select_format", "SwiGlu", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, y_out_, dim=-1, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, dim)
    result = check_op_cap("get_op_specific_info", "SwiGlu", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
