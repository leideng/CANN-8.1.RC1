...
def _apply_store_buffer(ib,
    ...
    if double_buffer is True:
        ib.scope_attr(buf_var.asobject(), "double_buffer_scope", 1)
...
class ResizeNearestNeighbor:
    ...
    def __init__(self,
        ...
        self.core_number = tbe_platform.get_soc_spec(
            tbe_platform.CORE_NUM)
        self.ub_size = tbe_platform.get_soc_spec(
            tbe_platform.UB_SIZE)
        self.l1_size = tbe_platform.get_soc_spec(
            tbe_platform.L1_SIZE)
...
def resize_nearest_neighbor_v2_d_compute(images,
    ...
    """
    dtype = images.dtype
    input_shape = shape_util.shape_to_list(images.shape)
    output_shape = shape_util.shape_to_list(y.shape)
