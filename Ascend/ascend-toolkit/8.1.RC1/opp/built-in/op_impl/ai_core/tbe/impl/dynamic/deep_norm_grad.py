...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['dy', 'x', 'gx', 'gamma', 'mean', 'rstd']
    output_names = ['dx', 'dgx', 'dbeta', 'dgamma']
...
def _build_args(dy_in__, x_in__, gx_in__, gamma_in__, mean_in__, rstd_in__, dx_out_, dgx_out_, dbeta_out_, dgamma_out_, alpha):
    __inputs__ = []
    for arg in [dy_in__, x_in__, gx_in__, gamma_in__, mean_in__, rstd_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [dx_out_, dgx_out_, dbeta_out_, dgamma_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("DeepNormGrad", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def deep_norm_grad(dy_in__, x_in__, gx_in__, gamma_in__, mean_in__, rstd_in__, dx_out_, dgx_out_, dbeta_out_, dgamma_out_, alpha=0.3, kernel_name="deep_norm_grad", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(dy_in__, x_in__, gx_in__, gamma_in__, mean_in__, rstd_in__, dx_out_, dgx_out_, dbeta_out_, dgamma_out_, alpha)
    ...
    origin_func_name = "deep_norm_grad"
    ascendc_src_dir_ex = "deep_norm_grad"
    ascendc_src_dir = "deep_norm_grad"
    ascendc_src_file = "deep_norm_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator DeepNormGrad, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "DeepNormGrad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[dy_in__, x_in__, gx_in__, gamma_in__, mean_in__, rstd_in__], origin_outputs = [dx_out_, dgx_out_, dbeta_out_, dgamma_out_],\
...
def op_select_format(dy_in__, x_in__, gx_in__, gamma_in__, mean_in__, rstd_in__, dx_out_, dgx_out_, dbeta_out_, dgamma_out_, alpha=0.3, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(dy_in__, x_in__, gx_in__, gamma_in__, mean_in__, rstd_in__, dx_out_, dgx_out_, dbeta_out_, dgamma_out_, alpha)
    result = check_op_cap("op_select_format", "DeepNormGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(dy_in__, x_in__, gx_in__, gamma_in__, mean_in__, rstd_in__, dx_out_, dgx_out_, dbeta_out_, dgamma_out_, alpha=0.3, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(dy_in__, x_in__, gx_in__, gamma_in__, mean_in__, rstd_in__, dx_out_, dgx_out_, dbeta_out_, dgamma_out_, alpha)
    result = check_op_cap("get_op_specific_info", "DeepNormGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
