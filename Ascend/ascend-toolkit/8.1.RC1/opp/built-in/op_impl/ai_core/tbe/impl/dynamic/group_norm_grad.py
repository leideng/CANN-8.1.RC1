...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['dy', 'mean', 'rstd', 'x', 'gamma']
...
def _build_args(dy_in__, mean_in__, rstd_in__, x_in__, gamma_in__, dx_out_, dgamma_out_, dbeta_out_, num_groups, data_format, dx_is_require, dgamma_is_require, dbeta_is_require):
    __inputs__ = []
    for arg in [dy_in__, mean_in__, rstd_in__, x_in__, gamma_in__]:
        if arg != None:
    ...
    if data_format != None:
        ...
        __attrs__.append(attr)
    if dx_is_require != None:
        attr = {}
        attr["name"] = "dx_is_require"
        attr["dtype"] = "bool"
        attr["value"] = dx_is_require
        __attrs__.append(attr)
...
@tbe_register.register_operator("GroupNormGrad", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def group_norm_grad(dy_in__, mean_in__, rstd_in__, x_in__, gamma_in__, dx_out_, dgamma_out_, dbeta_out_, num_groups, data_format="NCHW", dx_is_require=True, dgamma_is_require=True, dbeta_is_require=True, kernel_name="group_norm_grad", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(dy_in__, mean_in__, rstd_in__, x_in__, gamma_in__, dx_out_, dgamma_out_, dbeta_out_, num_groups, data_format, dx_is_require, dgamma_is_require, dbeta_is_require)
    ...
    origin_func_name = "group_norm_grad"
    ascendc_src_dir_ex = "group_norm_grad"
    ascendc_src_dir = "group_norm_grad"
    ascendc_src_file = "group_norm_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator GroupNormGrad, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[dy_in__, mean_in__, rstd_in__, x_in__, gamma_in__], origin_outputs = [dx_out_, dgamma_out_, dbeta_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required', 'required', 'required', 'required'], init_value_list = [None, None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(dy_in__, mean_in__, rstd_in__, x_in__, gamma_in__, dx_out_, dgamma_out_, dbeta_out_, num_groups, data_format="NCHW", dx_is_require=True, dgamma_is_require=True, dbeta_is_require=True, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(dy_in__, mean_in__, rstd_in__, x_in__, gamma_in__, dx_out_, dgamma_out_, dbeta_out_, num_groups, data_format, dx_is_require, dgamma_is_require, dbeta_is_require)
    result = check_op_cap("op_select_format", "GroupNormGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(dy_in__, mean_in__, rstd_in__, x_in__, gamma_in__, dx_out_, dgamma_out_, dbeta_out_, num_groups, data_format="NCHW", dx_is_require=True, dgamma_is_require=True, dbeta_is_require=True, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(dy_in__, mean_in__, rstd_in__, x_in__, gamma_in__, dx_out_, dgamma_out_, dbeta_out_, num_groups, data_format, dx_is_require, dgamma_is_require, dbeta_is_require)
    result = check_op_cap("get_op_specific_info", "GroupNormGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
