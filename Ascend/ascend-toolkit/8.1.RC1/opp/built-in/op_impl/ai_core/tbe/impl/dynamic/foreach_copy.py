...
@tbe_register.register_operator("ForeachCopy", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.DYNAMIC_INPUT, para_check.DYNAMIC_OUTPUT, para_check.KERNEL_NAME)
def foreach_copy(x_in__, y_out_, kernel_name="foreach_copy", impl_mode=""):
    ...
    origin_func_name = "foreach_copy"
    ascendc_src_dir_ex = "foreach_copy"
    ascendc_src_dir = "foreach_copy"
    ascendc_src_file = "foreach_copy.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator ForeachCopy, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "ForeachCopy"
...
def op_select_format(x_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_)
    result = check_op_cap("op_select_format", "ForeachCopy", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_)
    result = check_op_cap("get_op_specific_info", "ForeachCopy", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
