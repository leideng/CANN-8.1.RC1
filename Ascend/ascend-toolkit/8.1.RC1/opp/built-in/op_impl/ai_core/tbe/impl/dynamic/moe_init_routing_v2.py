...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'expert_idx']
    output_names = ['expanded_x', 'expanded_row_idx', 'expert_tokens_count_or_cumsum', 'expert_tokens_before_capacity']
...
def _build_args(x_in__, expert_idx_in__, expanded_x_out_, expanded_row_idx_out_, expert_tokens_count_or_cumsum_out_, expert_tokens_before_capacity_out_, active_num, expert_capacity, expert_num, drop_pad_mode, expert_tokens_count_or_cumsum_flag, expert_tokens_before_capacity_flag):
    __inputs__ = []
    for arg in [x_in__, expert_idx_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [expanded_x_out_, expanded_row_idx_out_, expert_tokens_count_or_cumsum_out_, expert_tokens_before_capacity_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if drop_pad_mode != None:
        ...
        __attrs__.append(attr)
    if expert_tokens_count_or_cumsum_flag != None:
        attr = {}
        attr["name"] = "expert_tokens_count_or_cumsum_flag"
        attr["dtype"] = "int"
        attr["value"] = expert_tokens_count_or_cumsum_flag
        __attrs__.append(attr)
    if expert_tokens_before_capacity_flag != None:
        attr = {}
        attr["name"] = "expert_tokens_before_capacity_flag"
        attr["dtype"] = "bool"
        attr["value"] = expert_tokens_before_capacity_flag
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("MoeInitRoutingV2", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_OUTPUT, para_check.OPTION_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def moe_init_routing_v2(x_in__, expert_idx_in__, expanded_x_out_, expanded_row_idx_out_, expert_tokens_count_or_cumsum_out_=None, expert_tokens_before_capacity_out_=None, active_num=0, expert_capacity=0, expert_num=0, drop_pad_mode=0, expert_tokens_count_or_cumsum_flag=0, expert_tokens_before_capacity_flag=False, kernel_name="moe_init_routing_v2", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, expert_idx_in__, expanded_x_out_, expanded_row_idx_out_, expert_tokens_count_or_cumsum_out_, expert_tokens_before_capacity_out_, active_num, expert_capacity, expert_num, drop_pad_mode, expert_tokens_count_or_cumsum_flag, expert_tokens_before_capacity_flag)
    ...
    custom_compile_options = {'__ALLSOC__': ['--cce-auto-sync', '-Wno-deprecated-declarations', '-Werror']},
    ...
    origin_func_name = "moe_init_routing_v2"
    ascendc_src_dir_ex = "moe_init_routing_v2"
    ascendc_src_dir = "moe_init_routing_v2"
    ascendc_src_file = "moe_init_routing_v2.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MoeInitRoutingV2, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "MoeInitRoutingV2"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, expert_idx_in__], origin_outputs = [expanded_x_out_, expanded_row_idx_out_, expert_tokens_count_or_cumsum_out_, expert_tokens_before_capacity_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'optional', 'optional'], init_value_list = [None, None, None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(x_in__, expert_idx_in__, expanded_x_out_, expanded_row_idx_out_, expert_tokens_count_or_cumsum_out_=None, expert_tokens_before_capacity_out_=None, active_num=0, expert_capacity=0, expert_num=0, drop_pad_mode=0, expert_tokens_count_or_cumsum_flag=0, expert_tokens_before_capacity_flag=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, expert_idx_in__, expanded_x_out_, expanded_row_idx_out_, expert_tokens_count_or_cumsum_out_, expert_tokens_before_capacity_out_, active_num, expert_capacity, expert_num, drop_pad_mode, expert_tokens_count_or_cumsum_flag, expert_tokens_before_capacity_flag)
    result = check_op_cap("op_select_format", "MoeInitRoutingV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, expert_idx_in__, expanded_x_out_, expanded_row_idx_out_, expert_tokens_count_or_cumsum_out_=None, expert_tokens_before_capacity_out_=None, active_num=0, expert_capacity=0, expert_num=0, drop_pad_mode=0, expert_tokens_count_or_cumsum_flag=0, expert_tokens_before_capacity_flag=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, expert_idx_in__, expanded_x_out_, expanded_row_idx_out_, expert_tokens_count_or_cumsum_out_, expert_tokens_before_capacity_out_, active_num, expert_capacity, expert_num, drop_pad_mode, expert_tokens_count_or_cumsum_flag, expert_tokens_before_capacity_flag)
    result = check_op_cap("get_op_specific_info", "MoeInitRoutingV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
