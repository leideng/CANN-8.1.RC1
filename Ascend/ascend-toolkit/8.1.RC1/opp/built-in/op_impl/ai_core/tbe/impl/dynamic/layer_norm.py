...
def _get_precision_mode(op_type):
    """
    get calculation mode, hign_performance or high_precision or keep_fp16
    """
    ...
    for op_info in op_infos:
        if op_info.op_type == op_type and op_info.precision_mode is not None and len(op_info.precision_mode) > 0:
...
def check_supported(input_x,
    ...
    ori_shape_x = shape_util.shape_to_list(input_x.get("ori_shape"))
    dtype_x = input_x.get("dtype").lower()
    format_x = input_x.get("format").upper()

    # dynamic inputs
    if is_unknown((input_x, input_gamma, input_beta)):
        return True, "go to dynamic operator"
    impl_mode = _get_precision_mode("LayerNorm")
    if impl_mode == "keep_fp16" and \
      get_soc_spec(SHORT_SOC_VERSION) in ("Ascend310P", "Ascend610", "BS9SX1A", 
                                          "Ascend610Lite", "BS9SX2A", "MC61AM21A"):
        return False, "keep_fp16 mode go to static operator"
    if format_x == "FRACTAL_NZ" and ori_shape_x[-1] % 16 != 0:
        return False, "NZ last dim not align go to static operator"
    begin_norm_axis = shape_util.axis_check(len(ori_shape_x), begin_norm_axis)
    if _check_vector_to_cube(dtype_x, format_x, ori_shape_x, shape_x, begin_norm_axis) and \
        impl_mode == "high_performance":
        return False, "go to static operator"

    return True, "go to dynamic operator"


def _check_vector_to_cube(dtype_x, format_x, ori_shape_x, shape_x, begin_norm_axis):
    """
    judge case using cube to handle reducesum
    only supported follow case in Ascend910 and Ascend310P:
        ori_shape:  ((batch), m, k(k>1024), "shape": ((batch), k//16, m//16, 16, 16), "dtype": fp16
    """
    if format_x != "FRACTAL_NZ":
        return False
    if dtype_x != "float16":
        return False
    if len(ori_shape_x) not in (2, 3) or ori_shape_x[-1] > 1024:
        return False
    if len(shape_x) not in (4, 5) or shape_x[-4] > 64:
        return False
    if get_soc_spec(SHORT_SOC_VERSION) not in ("Ascend910", "Ascend310P", "Ascend610",
                                               "BS9SX1A", "Ascend310B", "AS31XM1", 
                                               "Ascend610Lite", "BS9SX2A", "MC61AM21A"):
        return False
...
def layer_norm_compute_nz(input_x,
    ...
    if impl_mode != "keep_fp16":
        ...
        if is_cast and ori_dtype == "bfloat16":
            variance_16 = tbe.round(variance, ori_dtype)
            variance = tbe.cast_to(variance_16, "float32")

        if is_cast and ori_dtype == "float16":
            variance_16 = tbe.cast_to(variance, ori_dtype)
            variance = tbe.cast_to(variance_16, "float32")
    ...
    if impl_mode == "keep_fp16":
        ...
        if impl_mode == "high_performance" and is_support_vexp:
            ...
            normalize_exp = tbe.vexp(normalize_log_mul)
            normalize_exp_broadcast = _broadcast_nz(normalize_exp, shape_x)
            normalize_mul = tbe.vmul(normalize_sub, normalize_exp_broadcast)
        else:
            ...
            normalize_rsqrt = tbe.vdiv(tensor_one, normalize_sqrt)
            normalize_rsqrt_broadcast = _broadcast_nz(normalize_rsqrt, shape_x)
            normalize_mul = tbe.vmul(normalize_sub, normalize_rsqrt_broadcast)
...
def layer_norm_compute(input_x,
    ...
    is_support_vexp = tbe_platform.api_check_support("te.lang.cce.vexp", "float32")
    tbe_context.get_context().add_compile_info("is_support_vexp", is_support_vexp)
    is_mix_dtype = dtype == "float16" and gamma_dtype == "float32" and beta_dtype == "float32"
    is_fp16_promote = dtype == "float16" and impl_mode != "keep_fp16" and \
        ((is_support_vexp and impl_mode == "high_performance") or impl_mode == "high_precision")
    ...
    if impl_mode == "keep_fp16":
        ...
        if (impl_mode == "high_performance" and is_support_vexp) or cur_cce_product in (tbe_platform.ASCEND_910,):
            ...
            normalize_log = tbe.vlog(normalize_add)
            normalize_log_mul = tbe.vmuls(normalize_log, tvm.const(-0.5, dtype=cast_dtype))
            normalize_to_broadcast = tbe.vexp(normalize_log_mul)
        else:
            epsilon_scalar = get_attr_by_cls(epsilon, LayerNormAttrInfo.ATTR_EPSILON, cast_dtype_precision)
            normalize_add = tbe.vadds(variance, epsilon_scalar)
            normalize_sqrt = tbe.vsqrt(normalize_add)
            tensor_one = tbe.broadcast(tvm.const(1, cast_dtype_precision), normalize_sqrt.shape)
            normalize_to_broadcast = tbe.vdiv(tensor_one, normalize_sqrt)
        normalize_broadcast = tbe.broadcast(normalize_to_broadcast, input_x.shape)
        normalize_mul = tbe.vmul(normalize_sub, normalize_broadcast)
