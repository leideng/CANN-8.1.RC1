...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['grad_expanded_x', 'expanded_row_idx']
    output_names = ['grad_x']
...
def _build_args(grad_expanded_x_in__, expanded_row_idx_in__, grad_x_out_, top_k, drop_pad_mode, active_num):
    __inputs__ = []
    for arg in [grad_expanded_x_in__, expanded_row_idx_in__]:
        if arg != None:
...
@tbe_register.register_operator("MoeInitRoutingV2Grad", trans_bool_to_s8=False)
...
def moe_init_routing_v2_grad(grad_expanded_x_in__, expanded_row_idx_in__, grad_x_out_, top_k, drop_pad_mode=0, active_num=0, kernel_name="moe_init_routing_v2_grad", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(grad_expanded_x_in__, expanded_row_idx_in__, grad_x_out_, top_k, drop_pad_mode, active_num)
    ...
    origin_func_name = "moe_init_routing_v2_grad"
    ascendc_src_dir_ex = "moe_init_routing_v2_grad"
    ascendc_src_dir = "moe_init_routing_v2_grad"
    ascendc_src_file = "moe_init_routing_v2_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MoeInitRoutingV2Grad, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[grad_expanded_x_in__, expanded_row_idx_in__], origin_outputs = [grad_x_out_],\
...
def op_select_format(grad_expanded_x_in__, expanded_row_idx_in__, grad_x_out_, top_k, drop_pad_mode=0, active_num=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_expanded_x_in__, expanded_row_idx_in__, grad_x_out_, top_k, drop_pad_mode, active_num)
    result = check_op_cap("op_select_format", "MoeInitRoutingV2Grad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(grad_expanded_x_in__, expanded_row_idx_in__, grad_x_out_, top_k, drop_pad_mode=0, active_num=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_expanded_x_in__, expanded_row_idx_in__, grad_x_out_, top_k, drop_pad_mode, active_num)
    result = check_op_cap("get_op_specific_info", "MoeInitRoutingV2Grad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
