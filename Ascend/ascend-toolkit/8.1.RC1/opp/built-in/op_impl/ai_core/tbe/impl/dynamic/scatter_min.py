...
def scatter_min_tik(var, indices, updates, var_out, kernel_name="ScatterMin"):
    obj = scatter_common.ScatterCommon(var, indices, updates, var_out, False, kernel_name, "vmin")
    return obj.scatter_common_operator()


def scatter_min_dsl(var, indices, updates, var_out, kernel_name="ScatterMin"):
    """
    scatter_min interface for dsl
    """
...
def scatter_min(var, indices, updates, var_out, use_locking=False, kernel_name="scatter_min"):
    ...
    if not tbe_platform.api_check_support("tbe.dsl.vexp", "float32"):
        scatter_min_tik(var, indices, updates, var_out, kernel_name)
    else:
        scatter_min_dsl(var, indices, updates, var_out, kernel_name)
