...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['input_tokens', 'sampled_token_ids', 'input_positions', 'seq_lens', 'slot_mapping', 'block_tables']
    output_names = []
...
def _build_args(input_tokens_in__, sampled_token_ids_in__, input_positions_in__, seq_lens_in__, slot_mapping_in__, block_tables_in__, num_seqs, num_queries, block_size):
    __inputs__ = []
    for arg in [input_tokens_in__, sampled_token_ids_in__, input_positions_in__, seq_lens_in__, slot_mapping_in__, block_tables_in__]:
        if arg != None:
    ...
    if num_queries != None:
        attr = {}
        attr["name"] = "num_queries"
        attr["dtype"] = "int"
        attr["value"] = num_queries
        __attrs__.append(attr)
    if block_size != None:
        attr = {}
...
@tbe_register.register_operator("AdvanceStep", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def advance_step(input_tokens_in__, sampled_token_ids_in__, input_positions_in__, seq_lens_in__, slot_mapping_in__, block_tables_in__, num_seqs, num_queries, block_size, kernel_name="advance_step", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(input_tokens_in__, sampled_token_ids_in__, input_positions_in__, seq_lens_in__, slot_mapping_in__, block_tables_in__, num_seqs, num_queries, block_size)
    ...
    origin_func_name = "advance_step"
    ascendc_src_dir_ex = "advance_step"
    ascendc_src_dir = "advance_step"
    ascendc_src_file = "advance_step.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator AdvanceStep, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "AdvanceStep"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[input_tokens_in__, sampled_token_ids_in__, input_positions_in__, seq_lens_in__, slot_mapping_in__, block_tables_in__], origin_outputs = [],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required', 'required'], init_value_list = [],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{"auto_sync": false}')

def op_select_format(input_tokens_in__, sampled_token_ids_in__, input_positions_in__, seq_lens_in__, slot_mapping_in__, block_tables_in__, num_seqs, num_queries, block_size, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(input_tokens_in__, sampled_token_ids_in__, input_positions_in__, seq_lens_in__, slot_mapping_in__, block_tables_in__, num_seqs, num_queries, block_size)
    result = check_op_cap("op_select_format", "AdvanceStep", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(input_tokens_in__, sampled_token_ids_in__, input_positions_in__, seq_lens_in__, slot_mapping_in__, block_tables_in__, num_seqs, num_queries, block_size, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(input_tokens_in__, sampled_token_ids_in__, input_positions_in__, seq_lens_in__, slot_mapping_in__, block_tables_in__, num_seqs, num_queries, block_size)
    result = check_op_cap("get_op_specific_info", "AdvanceStep", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
