...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['positions', 'queryIn', 'keyIn', 'cosSinCache']
    output_names = ['queryOut', 'keyOut']
...
def _build_args(positions_in__, queryIn_in__, keyIn_in__, cosSinCache_in__, queryOut_out_, keyOut_out_, numQHeads, numKHeads, headSize, mropeSection, qStride, kStride, isNeoxStyle):
    __inputs__ = []
    for arg in [positions_in__, queryIn_in__, keyIn_in__, cosSinCache_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [queryOut_out_, keyOut_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if numQHeads != None:
        ...
        attr["name"] = "numQHeads"
        attr["dtype"] = "int"
        attr["value"] = numQHeads
        __attrs__.append(attr)
    if numKHeads != None:
        attr = {}
        attr["name"] = "numKHeads"
        attr["dtype"] = "int"
        attr["value"] = numKHeads
        __attrs__.append(attr)
    if headSize != None:
        attr = {}
        attr["name"] = "headSize"
        ...
        attr["value"] = headSize
        __attrs__.append(attr)
    if mropeSection != None:
        attr = {}
        attr["name"] = "mropeSection"
        attr["dtype"] = "list_int"
        attr["value"] = mropeSection
        __attrs__.append(attr)
    if qStride != None:
        attr = {}
        attr["name"] = "qStride"
    ...
    if isNeoxStyle != None:
        attr = {}
        attr["name"] = "isNeoxStyle"
        attr["dtype"] = "bool"
        attr["value"] = isNeoxStyle
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("RopeWithSinCosCache", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def rope_with_sin_cos_cache(positions_in__, queryIn_in__, keyIn_in__, cosSinCache_in__, queryOut_out_, keyOut_out_, numQHeads, numKHeads, headSize, mropeSection=[0,0,0], qStride=0, kStride=0, isNeoxStyle=True, kernel_name="rope_with_sin_cos_cache", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(positions_in__, queryIn_in__, keyIn_in__, cosSinCache_in__, queryOut_out_, keyOut_out_, numQHeads, numKHeads, headSize, mropeSection, qStride, kStride, isNeoxStyle)
    ...
    origin_func_name = "rope_with_sin_cos_cache"
    ascendc_src_dir_ex = "rope_with_sin_cos_cache"
    ascendc_src_dir = "rope_with_sin_cos_cache"
    ascendc_src_file = "rope_with_sin_cos_cache.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator RopeWithSinCosCache, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "RopeWithSinCosCache"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[positions_in__, queryIn_in__, keyIn_in__, cosSinCache_in__], origin_outputs = [queryOut_out_, keyOut_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required', 'required'], init_value_list = [None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(positions_in__, queryIn_in__, keyIn_in__, cosSinCache_in__, queryOut_out_, keyOut_out_, numQHeads, numKHeads, headSize, mropeSection=[0,0,0], qStride=0, kStride=0, isNeoxStyle=True, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(positions_in__, queryIn_in__, keyIn_in__, cosSinCache_in__, queryOut_out_, keyOut_out_, numQHeads, numKHeads, headSize, mropeSection, qStride, kStride, isNeoxStyle)
    result = check_op_cap("op_select_format", "RopeWithSinCosCache", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(positions_in__, queryIn_in__, keyIn_in__, cosSinCache_in__, queryOut_out_, keyOut_out_, numQHeads, numKHeads, headSize, mropeSection=[0,0,0], qStride=0, kStride=0, isNeoxStyle=True, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(positions_in__, queryIn_in__, keyIn_in__, cosSinCache_in__, queryOut_out_, keyOut_out_, numQHeads, numKHeads, headSize, mropeSection, qStride, kStride, isNeoxStyle)
    result = check_op_cap("get_op_specific_info", "RopeWithSinCosCache", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
