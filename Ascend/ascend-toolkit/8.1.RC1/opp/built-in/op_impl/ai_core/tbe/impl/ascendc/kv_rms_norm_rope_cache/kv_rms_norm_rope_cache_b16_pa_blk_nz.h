 ...
 public:
  ...
  __aicore__ inline void Process() {
    ...
    for (int64_t loopIdx = 0; loopIdx < ubLoop; ++loopIdx) {
      ...
      ScatterUpdateNZ<KV_DTYPE, isPagedAttention, ROPE_LENGTH, D1_SIZE4_PA_BLK_NZ, D0_SIZE16_PA_BLK_NZ>(kCacheGm, kCacheGmNd, outLocal, startIdx, ubFactor);
      ...
      ScatterUpdateNZ<KV_DTYPE, isPagedAttention, RMS_NORM_LENGTH, D1_SIZE32_PA_BLK_NZ, D0_SIZE16_PA_BLK_NZ>(vCacheGm, vCacheGmNd, outLocal, startIdx, ubFactor);
      outQueue.FreeTensor(outLocal);
    }
    if (ubTail > 0) {
      ...
      // Scatter Update kCache
      ScatterUpdateNZ<KV_DTYPE, isPagedAttention, ROPE_LENGTH, D1_SIZE4_PA_BLK_NZ, D0_SIZE16_PA_BLK_NZ>(kCacheGm, kCacheGmNd, outLocal, startIdx, ubTail);
      outQueue.FreeTensor(outLocal);
      ...
      // Scatter Update vCache
      ScatterUpdateNZ<KV_DTYPE, isPagedAttention, RMS_NORM_LENGTH, D1_SIZE32_PA_BLK_NZ, D0_SIZE16_PA_BLK_NZ>(vCacheGm, vCacheGmNd, outLocal, startIdx, ubTail);
      outQueue.FreeTensor(outLocal);
    }
  ...
  __aicore__ inline void ScatterUpdateNZ(const GlobalTensor<T>& dst, const GlobalTensor<T>& dstNd,
    ...
    DataCopyExtParams copyParamsNz{static_cast<uint16_t>(D1),
                                 ...
                                 0,
                                 static_cast<uint32_t>(tilingData_->blockSize * D0 * sizeof(T)) - static_cast<uint32_t>(D0 * sizeof(T)),
                                 0};
