...
template <typename OriT, typename IdxT>
__aicore__ inline int32_t MoeTokenUnpermuteGradBase<OriT, IdxT>::AlignUp(int32_t a, int32_t b) {
    if (unlikely(b == 0)) {
        return a;
    }
    return (a + b - 1) / b * b;
...
template <typename OriT, typename IdxT>
__aicore__ inline int32_t MoeTokenUnpermuteGradBase<OriT, IdxT>::CeilDiv(int32_t a, int32_t b) {
    if (unlikely(b == 0)) {
        return a;
    }
    return (a + b - 1) / b;
...
template <typename OriT, typename IdxT>
__aicore__ inline void MoeTokenUnpermuteGradBase<OriT, IdxT>::BinaryAddFunc(LocalTensor<float> tmpBuffer, int32_t hiddensizeLen, int32_t threshold, int32_t offset) {
...
template <typename OriT, typename IdxT>
__aicore__ inline void MoeTokenUnpermuteGradBase<OriT, IdxT>::ReduceSumFunc(LocalTensor<float> dstBuffer, LocalTensor<float> tmpBuffer, int32_t hiddensizeLen) {
...
template <typename OriT, typename IdxT>
__aicore__ inline void MoeTokenUnpermuteGradBase<OriT, IdxT>::Init(GM_ADDR permuted_tokens, GM_ADDR unpermuted_tokens_grad, GM_ADDR sorted_indices, GM_ADDR probs, GM_ADDR permuted_tokens_grad, GM_ADDR probs_grad,
                                                                 const MoeTokenUnpermuteGradTilingData& tilingData) {
