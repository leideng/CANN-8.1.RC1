...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'cos', 'sin', 'k_cache', 'v_cache', 'indices', 'scale_k', 'scale_v', 'offset_k', 'offset_v', 'weight_scale', 'activation_scale', 'bias']
    output_names = ['q', 'k', 'v', 'k_cache', 'v_cache']
...
def _build_args(x_in__, cos_in__, sin_in__, k_cache_in__, v_cache_in__, indices_in__, scale_k_in__, scale_v_in__, offset_k_in__, offset_v_in__, weight_scale_in__, activation_scale_in__, bias_in__, q_out_, k_out_, v_out_, k_cache_out_, v_cache_out_, size_splits, quant_mode, layout, kv_output, cache_mode):
    __inputs__ = []
    for arg in [x_in__, cos_in__, sin_in__, k_cache_in__, v_cache_in__, indices_in__, scale_k_in__, scale_v_in__, offset_k_in__, offset_v_in__, weight_scale_in__, activation_scale_in__, bias_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [q_out_, k_out_, v_out_, k_cache_out_, v_cache_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if size_splits != None:
        attr = {}
        attr["name"] = "size_splits"
        attr["dtype"] = "list_int"
        attr["value"] = size_splits
...
@tbe_register.register_operator("DequantRopeQuantKvcache", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_STR, para_check.KERNEL_NAME)
def dequant_rope_quant_kvcache(x_in__, cos_in__, sin_in__, k_cache_in__, v_cache_in__, indices_in__, scale_k_in__, scale_v_in__, offset_k_in__=None, offset_v_in__=None, weight_scale_in__=None, activation_scale_in__=None, bias_in__=None, q_out_=None, k_out_=None, v_out_=None, k_cache_out_=None, v_cache_out_=None, size_splits=[], quant_mode="static", layout="BSND", kv_output=False, cache_mode="contiguous", kernel_name="dequant_rope_quant_kvcache", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, cos_in__, sin_in__, k_cache_in__, v_cache_in__, indices_in__, scale_k_in__, scale_v_in__, offset_k_in__, offset_v_in__, weight_scale_in__, activation_scale_in__, bias_in__, q_out_, k_out_, v_out_, k_cache_out_, v_cache_out_, size_splits, quant_mode, layout, kv_output, cache_mode)
    ...
    origin_func_name = "dequant_rope_quant_kvcache"
    ascendc_src_dir_ex = "dequant_rope_quant_kvcache"
    ascendc_src_dir = "dequant_rope_quant_kvcache"
    ascendc_src_file = "dequant_rope_quant_kvcache.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator DequantRopeQuantKvcache, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "DequantRopeQuantKvcache"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, cos_in__, sin_in__, k_cache_in__, v_cache_in__, indices_in__, scale_k_in__, scale_v_in__, offset_k_in__, offset_v_in__, weight_scale_in__, activation_scale_in__, bias_in__], origin_outputs = [q_out_, k_out_, v_out_, k_cache_out_, v_cache_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required', 'required', 'required', 'required', 'optional', 'optional', 'optional', 'optional', 'optional', 'required', 'required', 'required', 'required', 'required'], init_value_list = [None, None, None, None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{"auto_sync": false}')

def op_select_format(x_in__, cos_in__, sin_in__, k_cache_in__, v_cache_in__, indices_in__, scale_k_in__, scale_v_in__, offset_k_in__=None, offset_v_in__=None, weight_scale_in__=None, activation_scale_in__=None, bias_in__=None, q_out_=None, k_out_=None, v_out_=None, k_cache_out_=None, v_cache_out_=None, size_splits=[], quant_mode="static", layout="BSND", kv_output=False, cache_mode="contiguous", impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, cos_in__, sin_in__, k_cache_in__, v_cache_in__, indices_in__, scale_k_in__, scale_v_in__, offset_k_in__, offset_v_in__, weight_scale_in__, activation_scale_in__, bias_in__, q_out_, k_out_, v_out_, k_cache_out_, v_cache_out_, size_splits, quant_mode, layout, kv_output, cache_mode)
    result = check_op_cap("op_select_format", "DequantRopeQuantKvcache", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, cos_in__, sin_in__, k_cache_in__, v_cache_in__, indices_in__, scale_k_in__, scale_v_in__, offset_k_in__=None, offset_v_in__=None, weight_scale_in__=None, activation_scale_in__=None, bias_in__=None, q_out_=None, k_out_=None, v_out_=None, k_cache_out_=None, v_cache_out_=None, size_splits=[], quant_mode="static", layout="BSND", kv_output=False, cache_mode="contiguous", impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, cos_in__, sin_in__, k_cache_in__, v_cache_in__, indices_in__, scale_k_in__, scale_v_in__, offset_k_in__, offset_v_in__, weight_scale_in__, activation_scale_in__, bias_in__, q_out_, k_out_, v_out_, k_cache_out_, v_cache_out_, size_splits, quant_mode, layout, kv_output, cache_mode)
    result = check_op_cap("get_op_specific_info", "DequantRopeQuantKvcache", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
