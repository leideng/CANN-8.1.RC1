...
@tbe_register.register_operator("ReflectionPad3dGrad", trans_bool_to_s8=False)
...
def reflection_pad3d_grad(x_in__, paddings_in__, y_out_, kernel_name="reflection_pad3d_grad", impl_mode=""):
    ...
    origin_func_name = "reflection_pad3d_grad"
    ascendc_src_dir_ex = "reflection_pad3d_grad"
    ascendc_src_dir = "reflection_pad3d_grad"
    ascendc_src_file = "reflection_pad3d_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator ReflectionPad3dGrad, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "ReflectionPad3dGrad"
...
def op_select_format(x_in__, paddings_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, paddings_in__, y_out_)
    result = check_op_cap("op_select_format", "ReflectionPad3dGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, paddings_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, paddings_in__, y_out_)
    result = check_op_cap("get_op_specific_info", "ReflectionPad3dGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
