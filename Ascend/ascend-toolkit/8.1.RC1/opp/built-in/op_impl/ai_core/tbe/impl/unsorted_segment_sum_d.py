...
def op_select_format(x, segment_ids, y, num_segments,
    ...
    atomic_add = tbe_platform.api_check_support("tik.set_atomic_add") and \
...
def check_supported(x,
    ...
    """
    unsorted_segment_sum_d is not support
    """
    return False, "unsorted_segment_sum_d is not support"
...
class SegmentParams():
    ...
    def __init__(self, ib_, src_dtype, in_shape, buffers):
        ...
        self.type_size_ids = tbe_platform.get_bit_len('int32') // 8
        self.cp_align_len_ids = \
            tbe_platform.BLOCK_REDUCE_INT8 // self.type_size_ids
        self.unified_buffer_len_ids = \
            tbe_platform.get_soc_spec(tbe_platform.UB_SIZE) // \
            self.type_size_ids
        self.vec_align_len_ids = \
            tbe_platform.VECTOR_INST_BLOCK_WIDTH // self.type_size_ids
        ...
        self.type_size = tbe_platform.get_bit_len(src_dtype) // 8
        self.cp_align_len = tbe_platform.BLOCK_REDUCE_INT8 // self.type_size
        self.cp_align_len_int8 = tbe_platform.BLOCK_REDUCE_INT8 // 1
        self.type_size_ratio = _ceil_div(self.type_size_ids, self.type_size)
        self.vec_align_len = \
            tbe_platform.VECTOR_INST_BLOCK_WIDTH // self.type_size
        self.vec_align_len_int8 = tbe_platform.VECTOR_INST_BLOCK_WIDTH // 1
        self.unified_buffer_len = \
            tbe_platform.get_soc_spec(tbe_platform.UB_SIZE) // self.type_size
    ...
    def get_mask(self, align, data_len):
        ...
        with self.ib_.if_scope(data_len <= self.half_mask):
            ...
            with self.ib_.for_range(0, data_len, kind="serial", name="i"):
                self.mask[0] = self.mask[0] * tvm.const(2, "uint64") + \
                               tvm.const(1, "uint64")
            with self.ib_.for_range(0, align, kind="serial", name="i"):
                self.mask[0] = self.mask[0] * tvm.const(2, "uint64")
...
def _is_32_byte_align(in_shape, dtype):
    byte_each_num = tbe_platform.get_bit_len(dtype) // 8
...
def _do_vadd(data_len, output_offset, input_offset, masks, params):
    ...
    with params.ib_.else_scope():
        ...
        with params.ib_.for_range(0, num_cycle, kind="serial",
                                  name="i") as i:
...
def _do_cp_ids(segment_ids_ub, segment_ids, gm_offset, data_len, params):
    ...
    args_str = tvm.call_cce_pure_intrin("int32", "tvm_cce_string_print",
                                    "PIPE_ALL")
...
def _all_in_fun(num_segments, input_buf, segment_ids, params):
    ...
    def _core_func(out_begin, out_end):
        ...
        if params.src_dtype == "int8" or params.src_dtype == "uint8":
            params.refresh_dtype("float16")
        with params.ib_.for_range(0, ids_len, kind="serial",
                                  name="index") as index:
...
def _ub_cp_part_ids(ids_block, data_block, index, _main_cp_fun, params):
    ...
    with params.ib_.for_range(0, ids_block_len, kind="serial",
                              name="j") as j:
        segment_id = params.segment_ids_ub.vload(j)
        with params.ib_.if_scope(segment_id == index):
            _main_cp_fun(data_len, 0,
...
def _new_alloc(tvm_ib, dtype, shape, name, scope, double_buffer=False):
    ...
    if double_buffer is True:
        tvm_ib.scope_attr(buf_var.asobject(), "double_buffer_scope", 1)
...
def _get_ub_space():
    ub_space_all = tbe_platform.get_soc_spec(tbe_platform.UB_SIZE)
    # open double buffer
...
def _get_data_type_byte_num(input_dtype):
    return tbe_platform.get_bit_len(input_dtype) // BIT_NUM_ONE_BYTE
...
def _compute_atomic_once_in_once_out(in_shape, num_segments, dtype, ins,
    ...
    tvm_ib = tvm.tir.ir_builder.create()
...
def _one_core_in_multi_ir_atomic(tvm_ib, in_shape, dtype, ins, output_buf,
    ...
    reg_for_idx = tvm_ib.allocate(dtype_segment_ids, (1, ),
                                  name="reg_buf_for_idx", scope=tbe_platform.scope_reg)
    reg_conf = tvm_ib.allocate("uint64", (2, ), scope=tbe_platform.scope_reg,
                               name="reg_conf")
    ...
    if mte2_loop_num > 1:
        with tvm_ib.for_range(0, mte2_loop_num - 1) as mte2_loop_index:
            ...
            input_ub = _new_alloc(tvm_ib,
                                  ...
                                  "input_ub",
                                  scope=tbe_platform.scope_ubuf, double_buffer=True)
            # copy data to ub
...
def _is_all_ids_in(in_shape, num_segments, dtype, segment_ids):
    ...
    dtype_segment_ids = segment_ids.dtype
    ele_num_segments_ids = shape_segment_ids[0]
    byte_num_segments_id = tbe_platform.get_bit_len(dtype_segment_ids) // 8
    byte_num_in_out = tbe_platform.get_bit_len(dtype) // 8
