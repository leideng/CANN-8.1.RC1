...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['orig_input_shape', 'grads']
    output_names = ['output']
...
def _build_args(orig_input_shape_in__, grads_in__, output_out_, ksize, strides, pads, ceil_mode, count_include_pad, divisor_override, data_format):
    __inputs__ = []
    for arg in [orig_input_shape_in__, grads_in__]:
        if arg != None:
    ...
    if ceil_mode != None:
        ...
        attr["name"] = "ceil_mode"
        attr["dtype"] = "bool"
        attr["value"] = ceil_mode
        __attrs__.append(attr)
    if count_include_pad != None:
        attr = {}
        attr["name"] = "count_include_pad"
        attr["dtype"] = "bool"
        attr["value"] = count_include_pad
        __attrs__.append(attr)
    if divisor_override != None:
        attr = {}
        attr["name"] = "divisor_override"
        attr["dtype"] = "int"
        attr["value"] = divisor_override
        __attrs__.append(attr)
    if data_format != None:
        attr = {}
...
@tbe_register.register_operator("AvgPool3DGrad", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_STR, para_check.KERNEL_NAME)
def avg_pool3_d_grad(orig_input_shape_in__, grads_in__, output_out_, ksize, strides, pads, ceil_mode=False, count_include_pad=True, divisor_override=0, data_format="NDHWC", kernel_name="avg_pool3_d_grad", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(orig_input_shape_in__, grads_in__, output_out_, ksize, strides, pads, ceil_mode, count_include_pad, divisor_override, data_format)
    ...
    origin_func_name = "avg_pool3_d_grad"
    ascendc_src_dir_ex = "avg_pool3_d_grad"
    ascendc_src_dir = "avg_pool3_d_grad"
    ascendc_src_file = "avg_pool3_d_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator AvgPool3DGrad, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[orig_input_shape_in__, grads_in__], origin_outputs = [output_out_],\
