...
def _build_args(input_in__, output_out_, output_size, align_corners, scales_d, scales_h, scales_w):
    __inputs__ = []
...
@tbe_register.register_operator("ResizeUpsampleTrilinear", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def resize_upsample_trilinear(input_in__, output_out_, output_size=[], align_corners=False, scales_d=0, scales_h=0, scales_w=0, kernel_name="resize_upsample_trilinear", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(input_in__, output_out_, output_size, align_corners, scales_d, scales_h, scales_w)
    ...
    origin_func_name = "resize_upsample_trilinear"
    ascendc_src_dir_ex = "resize_upsample_trilinear"
    ascendc_src_dir = "resize_upsample_trilinear"
    ascendc_src_file = "resize_upsample_trilinear.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator ResizeUpsampleTrilinear, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "ResizeUpsampleTrilinear"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[input_in__], origin_outputs = [output_out_],\
...
def op_select_format(input_in__, output_out_, output_size=[], align_corners=False, scales_d=0, scales_h=0, scales_w=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(input_in__, output_out_, output_size, align_corners, scales_d, scales_h, scales_w)
    result = check_op_cap("op_select_format", "ResizeUpsampleTrilinear", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(input_in__, output_out_, output_size=[], align_corners=False, scales_d=0, scales_h=0, scales_w=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(input_in__, output_out_, output_size, align_corners, scales_d, scales_h, scales_w)
    result = check_op_cap("get_op_specific_info", "ResizeUpsampleTrilinear", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
