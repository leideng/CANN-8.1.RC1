...
@tbe_register.register_operator("CircularPad", trans_bool_to_s8=False)
...
def circular_pad(x_in__, paddings_in__, y_out_, kernel_name="circular_pad", impl_mode=""):
    ...
    origin_func_name = "circular_pad"
    ascendc_src_dir_ex = "circular_pad"
    ascendc_src_dir = "circular_pad"
    ascendc_src_file = "circular_pad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator CircularPad, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "CircularPad"
...
def op_select_format(x_in__, paddings_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, paddings_in__, y_out_)
    result = check_op_cap("op_select_format", "CircularPad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, paddings_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, paddings_in__, y_out_)
    result = check_op_cap("get_op_specific_info", "CircularPad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
