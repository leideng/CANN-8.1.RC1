...
def _build_args(grad_output_in__, grad_input_out_, output_size, input_size, scales_h, scales_w):
    __inputs__ = []
...
@tbe_register.register_operator("UpsampleNearestExact2dGrad", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def upsample_nearest_exact2d_grad(grad_output_in__, grad_input_out_, output_size, input_size, scales_h=0, scales_w=0, kernel_name="upsample_nearest_exact2d_grad", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(grad_output_in__, grad_input_out_, output_size, input_size, scales_h, scales_w)
    ...
    origin_func_name = "upsample_nearest_exact2d_grad"
    ascendc_src_dir_ex = "upsample_nearest_exact2d_grad"
    ascendc_src_dir = "upsample_nearest_exact2d_grad"
    ascendc_src_file = "upsample_nearest_exact2d_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator UpsampleNearestExact2dGrad, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "UpsampleNearestExact2dGrad"
...
def op_select_format(grad_output_in__, grad_input_out_, output_size, input_size, scales_h=0, scales_w=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_output_in__, grad_input_out_, output_size, input_size, scales_h, scales_w)
    result = check_op_cap("op_select_format", "UpsampleNearestExact2dGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(grad_output_in__, grad_input_out_, output_size, input_size, scales_h=0, scales_w=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_output_in__, grad_input_out_, output_size, input_size, scales_h, scales_w)
    result = check_op_cap("get_op_specific_info", "UpsampleNearestExact2dGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
