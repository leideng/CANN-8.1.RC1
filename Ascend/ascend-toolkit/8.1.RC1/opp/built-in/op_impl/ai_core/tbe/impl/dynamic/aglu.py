...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'weight1', 'bias1', 'weight2', 'bias2']
    output_names = ['y']
...
def _build_args(x_in__, weight1_in__, bias1_in__, weight2_in__, bias2_in__, y_out_, activate_func, activate_left):
    __inputs__ = []
    for arg in [x_in__, weight1_in__, bias1_in__, weight2_in__, bias2_in__]:
        if arg != None:
    ...
    if activate_func != None:
        attr = {}
        attr["name"] = "activate_func"
        attr["dtype"] = "str"
        attr["value"] = activate_func
...
@tbe_register.register_operator("AGLU", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def aglu(x_in__, weight1_in__, bias1_in__=None, weight2_in__=None, bias2_in__=None, y_out_=None, activate_func="", activate_left=False, kernel_name="aglu", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight1_in__, bias1_in__, weight2_in__, bias2_in__, y_out_, activate_func, activate_left)
    ...
    origin_func_name = "aglu"
    ascendc_src_dir_ex = "aglu"
    ascendc_src_dir = "a_g_l_u"
    ascendc_src_file = "aglu.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator AGLU, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "AGLU"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, weight1_in__, bias1_in__, weight2_in__, bias2_in__], origin_outputs = [y_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'optional', 'optional', 'optional', 'required'], init_value_list = [None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{"auto_sync": false}')

def op_select_format(x_in__, weight1_in__, bias1_in__=None, weight2_in__=None, bias2_in__=None, y_out_=None, activate_func="", activate_left=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight1_in__, bias1_in__, weight2_in__, bias2_in__, y_out_, activate_func, activate_left)
    result = check_op_cap("op_select_format", "AGLU", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, weight1_in__, bias1_in__=None, weight2_in__=None, bias2_in__=None, y_out_=None, activate_func="", activate_left=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight1_in__, bias1_in__, weight2_in__, bias2_in__, y_out_, activate_func, activate_left)
    result = check_op_cap("get_op_specific_info", "AGLU", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
