...
class FusedBiasLeakyReluGradAttrInfo:
    """
    define attr info
    """
...
@register_operator_compute("FusedBiasLeakyReluGrad", op_mode="dynamic", support_fusion=True)
def fused_bias_leaky_relu_grad_compute(g, x, y, negative_slope=0.2,
                            scale=2**0.5, kernel_name="fused_bias_leaky_relu_grad"):
    """
    calculate the backpropagation of leaky_relu operation
    y = scale*gradients(x>0) or 
    scale*negative_slope*gradients(x<=0).
    ...
    if tbe_platform.api_check_support("tbe.dsl.vcmpsel", "float32"):
        ...
        x_ = tbe.broadcast(x, shape_list[2])
        x = tbe.vcmpsel(x_, x_, 'eq', x_, tvm.const(-1, x.dtype))

        negative_slope_attr = get_attr_by_cls(negative_slope, FusedBiasLeakyReluGradAttrInfo.ATTR_SLOPE, dtype)
        scale_attr = get_attr_by_cls(scale, FusedBiasLeakyReluGradAttrInfo.ATTR_SLOPE, dtype)

        mask_gt = tbe.vcmpsel(x, tvm.const(0, x.dtype), 'gt', g, tvm.const(0, x.dtype))
        res1 = tbe.vmuls(mask_gt, scale_attr)

        mask_le = tbe.vcmpsel(x, tvm.const(0, x.dtype), 'le', g, tvm.const(0, x.dtype))
        res2_ = tbe.vmuls(mask_le, scale_attr)
        res2 = tbe.vmuls(res2_, negative_slope_attr)
        ...
        scale_attr = get_attr_by_cls(scale, FusedBiasLeakyReluGradAttrInfo.ATTR_SLOPE, dtype)
        result_tmp = tbe.vmuls(result_tmp, scale_attr)
...
@register_operator("FusedBiasLeakyReluGrad")
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT,
                            para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def fused_bias_leaky_relu_grad(g, x, y, negative_slope=0.2, scale=2**0.5, kernel_name="fused_bias_leaky_relu_grad"):
    """
    calculate the backpropagation of fused_bias_leaky_relu operation
    y = scale*gradients(x>0) or scale*negative_slope*gradients(x<=0).
    support dtype:float16,float32

    Parameters
    ----------
    g : dict
        the backpropagated gradients to the corresponding fused_bias_leaky_relu operation
    x : dict
        the x passed as output of fused_bias_leaky_relu operation
    y : dict
    ...
    scale : float 
        allow non-zero slope for negative inputs to speed up optimization
    kernel_name : str
        kernel name, default value is "fused_bias_leaky_relu_grad"
    ...
    for (g, x) in ins:
        with tbe.compute():
            ...
            res = fused_bias_leaky_relu_grad_compute(tensor_g, tensor_x, y, negative_slope,
                                          scale, kernel_name)
