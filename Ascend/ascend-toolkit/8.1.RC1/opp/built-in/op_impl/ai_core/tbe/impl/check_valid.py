    ...
    def __move_job_bbox_to_ub(self, job_index=0, inverted=False):
        ...
        with self.tik_instance.else_scope():
            ...
            if self.job_num > 1:
                ...
                _burst_fp16_byte = self.last_job_row_aligned * 4 * self.bbox_dtype_size - self.padded_bytes
                if self.data_move_pad_support:
                    self.tik_instance.data_move_pad(self.bbox_tensor_ub,
                                                    self.bbox_tensor_gm[bbox_head], 1,
                                                    _burst_fp16_byte, 0, 0)
                else:
                    self.tik_instance.data_move(self.bbox_tensor_ub,
                                                self.bbox_tensor_gm[bbox_head], 0, 1,
                                                _burst_fp16, 0, 0)
    ...
    def __move_once_job_ret_to_gm(self, job_index=0, inverted=False):
        ...
        with self.tik_instance.else_scope():
            ...
            if self.job_num > 1:
                ...
                if self.bbox_shape[0] % self.__default_rows_per_job != 0:
                    _burst_int8_byte = self.bbox_shape[0] % self.__default_rows_per_job * self.valid_dtype_size
                else:
                    _burst_int8_byte = self.__default_rows_per_job * self.valid_dtype_size

                if self.data_move_pad_support:
                    self.tik_instance.data_move_pad(self.data_ret_int8_gm[valid_head],
