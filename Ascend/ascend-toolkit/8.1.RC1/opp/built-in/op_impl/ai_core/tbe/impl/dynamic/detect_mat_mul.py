...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x1', 'x2', 'bias', 'offset_w']
    output_names = ['y']
...
def _build_args(x1_in__, x2_in__, bias_in__, offset_w_in__, y_out_, transpose_x1, transpose_x2, offset_x, enable_hf32):
    __inputs__ = []
    for arg in [x1_in__, x2_in__, bias_in__, offset_w_in__]:
        if arg != None:
    ...
    if enable_hf32 != None:
        attr = {}
        attr["name"] = "enable_hf32"
        attr["dtype"] = "bool"
        attr["value"] = enable_hf32
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("DetectMatMul", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def detect_mat_mul(x1_in__, x2_in__, bias_in__=None, offset_w_in__=None, y_out_=None, transpose_x1=False, transpose_x2=False, offset_x=0, enable_hf32=False, kernel_name="detect_mat_mul", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, bias_in__, offset_w_in__, y_out_, transpose_x1, transpose_x2, offset_x, enable_hf32)
    ...
    origin_func_name = "detect_mat_mul"
    ascendc_src_dir_ex = "detect_mat_mul"
    ascendc_src_dir = "detect_mat_mul"
    ascendc_src_file = "detect_mat_mul.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator DetectMatMul, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "DetectMatMul"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x1_in__, x2_in__, bias_in__, offset_w_in__], origin_outputs = [y_out_],\
