...
class RoiAlign:
    ...
    def __init__(self, feature_map_dict,
        ...
        self.out_init_val = 0.0 if pool_mode == 'avg' else -65504  # this class only support fp16
                            ...
                            with self.tik_inst.else_scope():
                                ...
                                with self.tik_inst.for_range(0, grid_len) as loop_gh:
                                    ...
                                    with self.tik_inst.if_scope(gh_left == 0):
                                        self.tik_inst.vec_dup(128, output_ub, self.out_init_val, \
                                    ...
                                    with self.tik_inst.for_range(0, w_dim) as loop_gw:
                                        ...
                                        self.tik_inst.vec_muls(128, val_ub[len_c2], val_ub[len_c2], \
                                                               ly0, self.repeat, 8, 8)
                                        # `(m1*hx + m2*lx) * hy + (m3*hx + m4*lx) * ly`
                                        self.tik_inst.vec_add(128, val_ub, val_ub, val_ub[len_c2], \
                                                              self.repeat, 8, 8, 8)
                                        if self.pool_mode == 'avg':
                                            pool_func = self.tik_inst.vadd
                                        else:
                                            pool_func = self.tik_inst.vmax
                                        # add/max (m1*hx + m2*lx) * hy + (m3*hx + m4*lx) * ly to vout
                                        pool_func(128, output_ub[pw_c0], output_ub[pw_c0], \
                                                  val_ub, self.repeat, \
                                    ...
                                    t.set_as(grid_h - 1)
                                    if self.pool_mode == 'avg' and self.sample_ratio != 1:
...
def roi_align_cce(feature_map_dict, rois_dict, roisn_dict, output_dict, \
                  ...
                  roi_end_mode, pool_mode, kernel_name_val):
    """
    roi_align_cce
...
def _compute_roi_with_single_point(tik_instance, feature_shape, dtype,
                                   ...
                                   w_number_ub, feature_map_ub, pool_mode='avg'):
    """
    ...
    if pool_mode == 'avg':
        init_val = 0.0
        pool_func = tik_instance.vec_add
    else:
        init_val = -3.4028235e+38 if dtype == "float32" else -65504
        pool_func = tik_instance.vec_max
    ...
    with tik_instance.for_range(0, pool_h) as p_h:
        with tik_instance.for_range(0, pool_w, thread_num=thread_num) as p_w:
            ...
            for current_cb in range(c_number):
                ...
                tik_instance.vec_dup(16, val, init_val, c_block,
                                     n_bust)
                            ...
                            with tik_instance.for_range(0, grid_curr_w) as g_w:
                                ...
                                with tik_instance.if_scope(verify == 0):
                                    ...
                                    _compute_w1234(
                                        ...
                                        n_bust)
                                    pool_func(16, val, val, w1_lt, c_valid, n_bust, n_bust, n_bust)
                                with tik_instance.else_scope():
                                    if pool_mode != 'avg':
                                        tik_instance.vec_dup(mask=16, dst=w1_lt, scalar=0.0,
                                                             repeat_times=c_block, dst_rep_stride=n_bust)
                                        pool_func(16, val, val, w1_lt, c_valid, n_bust, n_bust, n_bust)
                        with tik_instance.else_scope():
                            if pool_mode != 'avg':
                                tik_instance.vec_dup(mask=16, dst=w1_lt, scalar=0.0,
...
def _bilinear_interpolate(tik_instance, x_lo_w, x_hi_w, y_lo_w, y_hi_w, x_lo,
    ...
    tik_instance.vec_dup(16, val, init_val, fm_c1 * pw_s, n_bust)
            ...
            with tik_instance.for_range(0, pw_int * sample_num_w) as grid_num_w:
                ...
                with tik_instance.if_scope(verify == 0):
                    ...
                    with tik_instance.for_range(0, fm_c1) as c_iter_i:
                        pool_func(16, val[c_iter_i, grid_num_w // sample_num_w, 0],
                                  val[c_iter_i, grid_num_w // sample_num_w, 0],
                                  w1_lt[c_iter_i, 0], 1, n_bust, n_bust, n_bust)
                with tik_instance.else_scope():
                    if pool_mode != 'avg':
                        w1_lt = tik_instance.Tensor(dtype, [fm_c1, 16], name="w1_lt", scope=tbe_platform.scope_ubuf)
                        tik_instance.vec_dup(mask=16, dst=w1_lt, scalar=0.0, repeat_times=fm_c1, dst_rep_stride=n_bust)
                        with tik_instance.for_range(0, fm_c1) as c_iter_i:
                            pool_func(16, val[c_iter_i, grid_num_w // sample_num_w, 0],
                                      val[c_iter_i, grid_num_w // sample_num_w, 0],
                                      w1_lt[c_iter_i, 0], 1, n_bust, n_bust, n_bust)
    ...
    with tik_instance.for_range(0, rois_valid_in_block) as curr_roi:
        ...
        with tik_instance.else_scope():
            _compute_roi_with_single_point(tik_instance, feature_shape, dtype,
                                           ...
                                           fm_to_ub, w_number_ub, feature_map_ub, pool_mode)
...
def roi_align_tik(feature_map_dict, rois_dict, roisn_dict, \
    ...
    if dtype == "float32":
        ...
        n_bust = 1
    # only v300 support vcopy
    l1_size = 0 if tbe_platform.api_check_support("tik.vcopy") else tbe_platform.get_soc_spec(tbe_platform.L1_SIZE)
    if roisn_dict:
            ...
            with tik_instance.for_range(
                ...
                if (cce_product in (tbe_platform.ASCEND_610, tbe_platform.ASCEND_310P)) and (dtype == "float16") \
                    ...
                    rois_ub_n5 = tik_instance.Tensor(
                        ...
                        scope=tbe_platform.scope_ubuf)
                    tik_instance.vector_dup(128, rois_ub_n5, 0.0, 5, 1, 8)

                    tik_instance.data_move(rois_ub_n5[0],
                                           rois[block_i * block_num + roi_128_number * 128, 0],
                                           0, 1,
                                           (rois_valid_in_block * 5 * 2 * n_bust + 31) // 32, 0, 0)
                    ...
                    if rois_shape[1] == 5:
                        ...
                        tik_instance.data_move(rois_ub_n5[0, 0],
                                               ...
                                               0, 1,
                                               (rois_valid_in_block * 5 * 2 * n_bust + 31) // 32, 0, 0)
                        _tf_n52n8(tik_instance, rois_ub, rois_ub_n5, rois_valid_in_block)
                    else:
                        tik_instance.data_move(rois_ub[0, 0],
                                               rois[block_i * block_num + roi_128_number * 128, 0],
                                               0, 1,
                                               (rois_valid_in_block * 8 * 2 * n_bust + 31) // 32, 0, 0)
...
def roi_align(feature_map_dict,
    ...
    if roi_end_mode > 1 and cce_product in (tbe_platform.ASCEND_910, "Ascend910B"):
        ...
        return roi_align_true(feature_map_dict, rois_dict, roisn_dict, output, spatial_scale, pooled_height,
                              pooled_width, sample_num, roi_end_mode, pool_mode, kernel_name)

    if roi_end_mode == 0 and cce_product == tbe_platform.ASCEND_310P and dtype == "float32":
        return roi_align_true(feature_map_dict, rois_dict, roisn_dict, output, spatial_scale, pooled_height,
                              pooled_width, sample_num, roi_end_mode, pool_mode, kernel_name)

    if ((cce_product in (tbe_platform.ASCEND_610, "BS9SX1A", "AS31XM1", 
                         "Ascend610Lite", "BS9SX2A", "MC61AM21A")) \
            and (dtype == "float16") and \
            (pooled_height == 7) and (pooled_width == 7) and (roi_end_mode == 1) \
    ...
    if (dtype == "float16") and (impl_mode == "high_precision"):
        return roi_align_tik(feature_map_dict, rois_dict, roisn_dict,
                             output, spatial_scale, pooled_height, pooled_width,
                             sample_num, roi_end_mode, pool_mode, kernel_name)
