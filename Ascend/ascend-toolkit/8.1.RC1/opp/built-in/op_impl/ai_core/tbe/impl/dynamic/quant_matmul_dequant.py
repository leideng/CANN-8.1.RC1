...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'quantized_weight', 'weight_scale', 'bias', 'x_scale', 'x_offset', 'smooth_scale']
    output_names = ['y']
...
def _build_args(x_in__, quantized_weight_in__, weight_scale_in__, bias_in__, x_scale_in__, x_offset_in__, smooth_scale_in__, y_out_, x_quant_mode, transpose_weight):
    __inputs__ = []
    for arg in [x_in__, quantized_weight_in__, weight_scale_in__, bias_in__, x_scale_in__, x_offset_in__, smooth_scale_in__]:
        if arg != None:
...
@tbe_register.register_operator("QuantMatmulDequant", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def quant_matmul_dequant(x_in__, quantized_weight_in__, weight_scale_in__, bias_in__=None, x_scale_in__=None, x_offset_in__=None, smooth_scale_in__=None, y_out_=None, x_quant_mode="pertoken", transpose_weight=True, kernel_name="quant_matmul_dequant", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, quantized_weight_in__, weight_scale_in__, bias_in__, x_scale_in__, x_offset_in__, smooth_scale_in__, y_out_, x_quant_mode, transpose_weight)
    ...
    origin_func_name = "quant_matmul_dequant"
    ascendc_src_dir_ex = "quant_matmul_dequant"
    ascendc_src_dir = "quant_matmul_dequant"
    ascendc_src_file = "quant_matmul_dequant.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator QuantMatmulDequant, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, quantized_weight_in__, weight_scale_in__, bias_in__, x_scale_in__, x_offset_in__, smooth_scale_in__], origin_outputs = [y_out_],\
...
def op_select_format(x_in__, quantized_weight_in__, weight_scale_in__, bias_in__=None, x_scale_in__=None, x_offset_in__=None, smooth_scale_in__=None, y_out_=None, x_quant_mode="pertoken", transpose_weight=True, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, quantized_weight_in__, weight_scale_in__, bias_in__, x_scale_in__, x_offset_in__, smooth_scale_in__, y_out_, x_quant_mode, transpose_weight)
    result = check_op_cap("op_select_format", "QuantMatmulDequant", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, quantized_weight_in__, weight_scale_in__, bias_in__=None, x_scale_in__=None, x_offset_in__=None, smooth_scale_in__=None, y_out_=None, x_quant_mode="pertoken", transpose_weight=True, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, quantized_weight_in__, weight_scale_in__, bias_in__, x_scale_in__, x_offset_in__, smooth_scale_in__, y_out_, x_quant_mode, transpose_weight)
    result = check_op_cap("get_op_specific_info", "QuantMatmulDequant", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
