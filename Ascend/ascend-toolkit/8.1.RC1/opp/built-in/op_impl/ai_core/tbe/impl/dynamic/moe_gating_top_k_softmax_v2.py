...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'finished']
    output_names = ['y', 'expert_idx', 'softmax_result']
...
def _build_args(x_in__, finished_in__, y_out_, expert_idx_out_, softmax_result_out_, k, renorm, output_softmax_result_flag):
    __inputs__ = []
    for arg in [x_in__, finished_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [y_out_, expert_idx_out_, softmax_result_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if renorm != None:
        ...
        attr["value"] = renorm
        __attrs__.append(attr)
    if output_softmax_result_flag != None:
        attr = {}
        attr["name"] = "output_softmax_result_flag"
        attr["dtype"] = "bool"
        attr["value"] = output_softmax_result_flag
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("MoeGatingTopKSoftmaxV2", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def moe_gating_top_k_softmax_v2(x_in__, finished_in__=None, y_out_=None, expert_idx_out_=None, softmax_result_out_=None, k=0, renorm=0, output_softmax_result_flag=False, kernel_name="moe_gating_top_k_softmax_v2", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, finished_in__, y_out_, expert_idx_out_, softmax_result_out_, k, renorm, output_softmax_result_flag)
    ...
    origin_func_name = "moe_gating_top_k_softmax_v2"
    ascendc_src_dir_ex = "moe_gating_top_k_softmax_v2"
    ascendc_src_dir = "moe_gating_top_k_softmax_v2"
    ascendc_src_file = "moe_gating_top_k_softmax_v2.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MoeGatingTopKSoftmaxV2, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, finished_in__], origin_outputs = [y_out_, expert_idx_out_, softmax_result_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'optional', 'required', 'required', 'optional'], init_value_list = [None, None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{"auto_sync": false}')

def op_select_format(x_in__, finished_in__=None, y_out_=None, expert_idx_out_=None, softmax_result_out_=None, k=0, renorm=0, output_softmax_result_flag=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, finished_in__, y_out_, expert_idx_out_, softmax_result_out_, k, renorm, output_softmax_result_flag)
    result = check_op_cap("op_select_format", "MoeGatingTopKSoftmaxV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, finished_in__=None, y_out_=None, expert_idx_out_=None, softmax_result_out_=None, k=0, renorm=0, output_softmax_result_flag=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, finished_in__, y_out_, expert_idx_out_, softmax_result_out_, k, renorm, output_softmax_result_flag)
    result = check_op_cap("get_op_specific_info", "MoeGatingTopKSoftmaxV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
