...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'filter', 'bias', 'offset_w']
    output_names = ['y']
...
def _build_args(x_in__, filter_in__, bias_in__, offset_w_in__, y_out_, strides, pads, dilations, groups, data_format, offset_x):
    __inputs__ = []
    for arg in [x_in__, filter_in__, bias_in__, offset_w_in__]:
        if arg != None:
...
@tbe_register.register_operator("Conv3DV2", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def conv3dv2(x_in__, filter_in__, bias_in__=None, offset_w_in__=None, y_out_=None, strides=[], pads=[], dilations=[1,1,1,1,1], groups=1, data_format="NCDHW", offset_x=0, kernel_name="conv3dv2", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, filter_in__, bias_in__, offset_w_in__, y_out_, strides, pads, dilations, groups, data_format, offset_x)
    ...
    origin_func_name = "conv3dv2"
    ascendc_src_dir_ex = "conv3_dv2"
    ascendc_src_dir = "conv3_d_v2"
    ascendc_src_file = "conv3dv2.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator Conv3DV2, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "Conv3DV2"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, filter_in__, bias_in__, offset_w_in__], origin_outputs = [y_out_],\
...
def op_select_format(x_in__, filter_in__, bias_in__=None, offset_w_in__=None, y_out_=None, strides=[], pads=[], dilations=[1,1,1,1,1], groups=1, data_format="NCDHW", offset_x=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, filter_in__, bias_in__, offset_w_in__, y_out_, strides, pads, dilations, groups, data_format, offset_x)
    result = check_op_cap("op_select_format", "Conv3DV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, filter_in__, bias_in__=None, offset_w_in__=None, y_out_=None, strides=[], pads=[], dilations=[1,1,1,1,1], groups=1, data_format="NCDHW", offset_x=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, filter_in__, bias_in__, offset_w_in__, y_out_, strides, pads, dilations, groups, data_format, offset_x)
    result = check_op_cap("get_op_specific_info", "Conv3DV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
