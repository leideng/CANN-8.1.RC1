...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['tensor_list']
    output_names = ['found_flag']
...
def _build_args(tensor_list_in__, found_flag_out_):
    __inputs__ = []
    for arg in [tensor_list_in__]:
        if arg != None:
...
@tbe_register.register_operator("NonFiniteCheck", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.DYNAMIC_INPUT, para_check.REQUIRED_OUTPUT, para_check.KERNEL_NAME)
def non_finite_check(tensor_list_in__, found_flag_out_, kernel_name="non_finite_check", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(tensor_list_in__, found_flag_out_)
    ...
    origin_func_name = "non_finite_check"
    ascendc_src_dir_ex = "non_finite_check"
    ascendc_src_dir = "non_finite_check"
    ascendc_src_file = "non_finite_check.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator NonFiniteCheck, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "NonFiniteCheck"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[tensor_list_in__], origin_outputs = [found_flag_out_],\
                param_type_dynamic = True, mc2_ctx = [], param_type_list = ['dynamic', 'required'], init_value_list = [None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{"auto_sync": false}')

def op_select_format(tensor_list_in__, found_flag_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(tensor_list_in__, found_flag_out_)
    result = check_op_cap("op_select_format", "NonFiniteCheck", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(tensor_list_in__, found_flag_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(tensor_list_in__, found_flag_out_)
    result = check_op_cap("get_op_specific_info", "NonFiniteCheck", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
