...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['grad', 'x', 'grid']
    output_names = ['dx', 'dgrid']
...
@tbe_register.register_operator("GridSampler2DGrad", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def grid_sampler2_d_grad(grad_in__, x_in__, grid_in__, dx_out_, dgrid_out_, interpolation_mode, padding_mode, align_corners, kernel_name="grid_sampler2_d_grad", impl_mode=""):
    ...
    origin_func_name = "grid_sampler2_d_grad"
    ascendc_src_dir_ex = "grid_sampler2_d_grad"
    ascendc_src_dir = "grid_sampler2_d_grad"
    ascendc_src_file = "grid_sampler2_d_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator GridSampler2DGrad, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "GridSampler2DGrad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[grad_in__, x_in__, grid_in__], origin_outputs = [dx_out_, dgrid_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required'], init_value_list = ['{ "is_list" : true, "bfloat16": { "type": "float16", "value": 0.000000 }, "float16": { "type": "float16", "value": 0.000000 }, "float32": { "type": "float32", "value": 0.000000 }}', None],\
...
def op_select_format(grad_in__, x_in__, grid_in__, dx_out_, dgrid_out_, interpolation_mode, padding_mode, align_corners, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_in__, x_in__, grid_in__, dx_out_, dgrid_out_, interpolation_mode, padding_mode, align_corners)
    result = check_op_cap("op_select_format", "GridSampler2DGrad", __inputs__, __outputs__, __attrs__)
...
def get_op_specific_info(grad_in__, x_in__, grid_in__, dx_out_, dgrid_out_, interpolation_mode, padding_mode, align_corners, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_in__, x_in__, grid_in__, dx_out_, dgrid_out_, interpolation_mode, padding_mode, align_corners)
    result = check_op_cap("get_op_specific_info", "GridSampler2DGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
