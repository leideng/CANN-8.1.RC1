...
public:
    __aicore__ inline MoeFinalizeRoutingV2FpDbAllBias(
...
)
{
    ...
    pipe_.InitBuffer(skip1Queue_, 1, tilingData_.normalCoreHandleNumPerLoop * AlignmentProcess(tilingData_.H) * sizeof(T));
    if (tilingData_.skip2IsNull == 0) {
        ...
        pipe_.InitBuffer(skip2Queue_, 1, tilingData_.normalCoreHandleNumPerLoop * AlignmentProcess(tilingData_.H) * sizeof(T));
    }
...
template <typename T, const bool ISBIASEXIST>
__aicore__ inline void MoeFinalizeRoutingV2FpDbAllBias<T, ISBIASEXIST>::CopyIn(int64_t nLoopIdx, 
    int64_t curRepeatTimes)
{
...
template <typename T, const bool ISBIASEXIST>
__aicore__ inline void MoeFinalizeRoutingV2FpDbAllBias<T, ISBIASEXIST>::Compute(int64_t nLoopIdx, 
    int64_t curRepeatTimes)
{
...
template <typename T, const bool ISBIASEXIST>
__aicore__ inline void MoeFinalizeRoutingV2FpDbAllBias<T, ISBIASEXIST>::CopyOut(int64_t nLoopIdx, 
    int64_t curRepeatTimes)
{
