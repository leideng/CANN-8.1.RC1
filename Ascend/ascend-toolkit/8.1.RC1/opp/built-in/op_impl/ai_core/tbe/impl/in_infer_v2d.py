...
def _output_data_y_compute(x, mean, scale, offset, y_sqrt):
    ...
    if scale is not None and offset is not None:
        ...
        scale_sqrt = tbe.vdiv(scale, y_sqrt)
        scale_broad = tbe.broadcast(scale_sqrt, shape_x)
        offset_broad = tbe.broadcast(offset, shape_x)
        res = tbe.vadd(tbe.vmul(scale_broad, var_sub),
                               offset_broad)
    else:
        sqrt_broadcast = tbe.broadcast(y_sqrt, shape_x)
        res = tbe.vdiv(var_sub, sqrt_broadcast)
    return res


# 'pylint: disable=locally-disabled,too-many-locals
@register_operator_compute("in_infer_v2d", op_mode="static", support_fusion=True)
