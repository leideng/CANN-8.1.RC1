...
@tbe_register.register_operator("ForeachSubScalar", trans_bool_to_s8=False)
...
def foreach_sub_scalar(x_in__, scalar_in__, y_out_, kernel_name="foreach_sub_scalar", impl_mode=""):
    ...
    origin_func_name = "foreach_sub_scalar"
    ascendc_src_dir_ex = "foreach_sub_scalar"
    ascendc_src_dir = "foreach_sub_scalar"
    ascendc_src_file = "foreach_sub_scalar.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator ForeachSubScalar, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "ForeachSubScalar"
...
def op_select_format(x_in__, scalar_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, scalar_in__, y_out_)
    result = check_op_cap("op_select_format", "ForeachSubScalar", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, scalar_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, scalar_in__, y_out_)
    result = check_op_cap("get_op_specific_info", "ForeachSubScalar", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
