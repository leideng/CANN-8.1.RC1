...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['grad', 'sum_grad_r', 'sum_grad_c', 'sum_grad_rc', 'r_in', 'c_in', 'beta2', 'sum_r', 'global_shape']
    output_names = ['r_out', 'c_out', 'u', 'sum_square_u']
...
def _build_args(grad_in__, sum_grad_r_in__, sum_grad_c_in__, sum_grad_rc_in__, r_in_in__, c_in_in__, beta2_in__, sum_r_in__, global_shape_in__, r_out_out_, c_out_out_, u_out_, sum_square_u_out_):
    __inputs__ = []
    for arg in [grad_in__, sum_grad_r_in__, sum_grad_c_in__, sum_grad_rc_in__, r_in_in__, c_in_in__, beta2_in__, sum_r_in__, global_shape_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [r_out_out_, c_out_out_, u_out_, sum_square_u_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("ApplyCamePart2", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.KERNEL_NAME)
def apply_came_part2(grad_in__, sum_grad_r_in__, sum_grad_c_in__, sum_grad_rc_in__, r_in_in__, c_in_in__, beta2_in__, sum_r_in__=None, global_shape_in__=None, r_out_out_=None, c_out_out_=None, u_out_=None, sum_square_u_out_=None, kernel_name="apply_came_part2", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(grad_in__, sum_grad_r_in__, sum_grad_c_in__, sum_grad_rc_in__, r_in_in__, c_in_in__, beta2_in__, sum_r_in__, global_shape_in__, r_out_out_, c_out_out_, u_out_, sum_square_u_out_)
    ...
    origin_func_name = "apply_came_part2"
    ascendc_src_dir_ex = "apply_came_part2"
    ascendc_src_dir = "apply_came_part2"
    ascendc_src_file = "apply_came_part2.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator ApplyCamePart2, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "ApplyCamePart2"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[grad_in__, sum_grad_r_in__, sum_grad_c_in__, sum_grad_rc_in__, r_in_in__, c_in_in__, beta2_in__, sum_r_in__, global_shape_in__], origin_outputs = [r_out_out_, c_out_out_, u_out_, sum_square_u_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required', 'required', 'required', 'optional', 'optional', 'required', 'required', 'required', 'required'], init_value_list = [None, None, None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(grad_in__, sum_grad_r_in__, sum_grad_c_in__, sum_grad_rc_in__, r_in_in__, c_in_in__, beta2_in__, sum_r_in__=None, global_shape_in__=None, r_out_out_=None, c_out_out_=None, u_out_=None, sum_square_u_out_=None, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_in__, sum_grad_r_in__, sum_grad_c_in__, sum_grad_rc_in__, r_in_in__, c_in_in__, beta2_in__, sum_r_in__, global_shape_in__, r_out_out_, c_out_out_, u_out_, sum_square_u_out_)
    result = check_op_cap("op_select_format", "ApplyCamePart2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(grad_in__, sum_grad_r_in__, sum_grad_c_in__, sum_grad_rc_in__, r_in_in__, c_in_in__, beta2_in__, sum_r_in__=None, global_shape_in__=None, r_out_out_=None, c_out_out_=None, u_out_=None, sum_square_u_out_=None, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_in__, sum_grad_r_in__, sum_grad_c_in__, sum_grad_rc_in__, r_in_in__, c_in_in__, beta2_in__, sum_r_in__, global_shape_in__, r_out_out_, c_out_out_, u_out_, sum_square_u_out_)
    result = check_op_cap("get_op_specific_info", "ApplyCamePart2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
