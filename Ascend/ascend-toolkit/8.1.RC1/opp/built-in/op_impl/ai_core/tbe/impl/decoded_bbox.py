...
class DecodeBbox:
    ...
    def __init__(self, tik_instance, input_data):
        ...
        if not self.is_rpn_supported:
            self.x1y1x2y2_gm = input_data[5]
            self.scores_gm = input_data[6]
    ...
    def get_decode_bbox(self, i):
        ...
        if self.is_rpn_supported:
            ...
            tik_instance.data_move(self.x1y1x2y2_gm[0, i * self.H * self.W],
                                   self.pred_box_left_up_x, 0, 1, (self.H * self.W)*self.size//32, 0, 0, 0)
            tik_instance.data_move(self.x1y1x2y2_gm[1, i * self.H * self.W],
                                   self.pred_box_left_up_y, 0, 1, (self.H * self.W)*self.size//32, 0, 0, 0)
            tik_instance.data_move(self.x1y1x2y2_gm[2, i * self.H * self.W],
                                   self.pred_box_right_down_x, 0, 1, (self.H * self.W)*self.size//32, 0, 0, 0)
            tik_instance.data_move(self.x1y1x2y2_gm[3, i * self.H * self.W],
                                   self.pred_box_right_down_y, 0, 1, (self.H * self.W)*self.size//32, 0, 0, 0)
            tik_instance.data_move(self.scores_gm[i * self.H * self.W],
                                   score_bitmap, 0, 1, (self.H * self.W)*self.size//32, 0, 0, 0)

    def generate_bbox(self, input_list, output_region_proposal, burst, i):
        """
        :param input_list:
        :param batch_id:
...
class OneCoreProcess:
    ...
    def __init__(self, input_data):
        ...
        self.is_rpn_supported = check_soc_version()
        if not self.is_rpn_supported:
            self.x1y1x2y2_gm = input_data[8]
            self.scores_gm = input_data[9]
        ...
        with self.tik_instance.for_range(0, self.num_anchor) as i:
            ...
            if tiling_number == 0:
                ...
                decode_bbox_object.generate_bbox((input_bbox_offset,
                                                  ...
                                                  region_proposal,
                                                  im_info),
                                                 output_region_proposal, burst, i)
