...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['kv', 'gamma', 'cos', 'sin', 'index', 'k_cache', 'ckv_cache', 'k_rope_scale', 'c_kv_scale', 'k_rope_offset', 'c_kv_offset']
    output_names = ['k_cache', 'ckv_cache', 'k_rope', 'c_kv']
...
def _build_args(kv_in__, gamma_in__, cos_in__, sin_in__, index_in__, k_cache_in__, ckv_cache_in__, k_rope_scale_in__, c_kv_scale_in__, k_rope_offset_in__, c_kv_offset_in__, k_cache_out_, ckv_cache_out_, k_rope_out_, c_kv_out_, epsilon, cache_mode, is_output_kv):
    __inputs__ = []
    for arg in [kv_in__, gamma_in__, cos_in__, sin_in__, index_in__, k_cache_in__, ckv_cache_in__, k_rope_scale_in__, c_kv_scale_in__, k_rope_offset_in__, c_kv_offset_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [k_cache_out_, ckv_cache_out_, k_rope_out_, c_kv_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if cache_mode != None:
        ...
        __attrs__.append(attr)
    if is_output_kv != None:
        attr = {}
        attr["name"] = "is_output_kv"
        attr["dtype"] = "bool"
        attr["value"] = is_output_kv
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("KvRmsNormRopeCache", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def kv_rms_norm_rope_cache(kv_in__, gamma_in__, cos_in__, sin_in__, index_in__, k_cache_in__, ckv_cache_in__, k_rope_scale_in__=None, c_kv_scale_in__=None, k_rope_offset_in__=None, c_kv_offset_in__=None, k_cache_out_=None, ckv_cache_out_=None, k_rope_out_=None, c_kv_out_=None, epsilon=1e-05, cache_mode="Norm", is_output_kv=False, kernel_name="kv_rms_norm_rope_cache", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(kv_in__, gamma_in__, cos_in__, sin_in__, index_in__, k_cache_in__, ckv_cache_in__, k_rope_scale_in__, c_kv_scale_in__, k_rope_offset_in__, c_kv_offset_in__, k_cache_out_, ckv_cache_out_, k_rope_out_, c_kv_out_, epsilon, cache_mode, is_output_kv)
    ...
    origin_func_name = "kv_rms_norm_rope_cache"
    ascendc_src_dir_ex = "kv_rms_norm_rope_cache"
    ascendc_src_dir = "kv_rms_norm_rope_cache"
    ascendc_src_file = "kv_rms_norm_rope_cache.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator KvRmsNormRopeCache, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "KvRmsNormRopeCache"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[kv_in__, gamma_in__, cos_in__, sin_in__, index_in__, k_cache_in__, ckv_cache_in__, k_rope_scale_in__, c_kv_scale_in__, k_rope_offset_in__, c_kv_offset_in__], origin_outputs = [k_cache_out_, ckv_cache_out_, k_rope_out_, c_kv_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required', 'required', 'required', 'optional', 'optional', 'optional', 'optional', 'required', 'required', 'required', 'required'], init_value_list = [None, None, None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{"auto_sync": false}')

def op_select_format(kv_in__, gamma_in__, cos_in__, sin_in__, index_in__, k_cache_in__, ckv_cache_in__, k_rope_scale_in__=None, c_kv_scale_in__=None, k_rope_offset_in__=None, c_kv_offset_in__=None, k_cache_out_=None, ckv_cache_out_=None, k_rope_out_=None, c_kv_out_=None, epsilon=1e-05, cache_mode="Norm", is_output_kv=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(kv_in__, gamma_in__, cos_in__, sin_in__, index_in__, k_cache_in__, ckv_cache_in__, k_rope_scale_in__, c_kv_scale_in__, k_rope_offset_in__, c_kv_offset_in__, k_cache_out_, ckv_cache_out_, k_rope_out_, c_kv_out_, epsilon, cache_mode, is_output_kv)
    result = check_op_cap("op_select_format", "KvRmsNormRopeCache", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(kv_in__, gamma_in__, cos_in__, sin_in__, index_in__, k_cache_in__, ckv_cache_in__, k_rope_scale_in__=None, c_kv_scale_in__=None, k_rope_offset_in__=None, c_kv_offset_in__=None, k_cache_out_=None, ckv_cache_out_=None, k_rope_out_=None, c_kv_out_=None, epsilon=1e-05, cache_mode="Norm", is_output_kv=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(kv_in__, gamma_in__, cos_in__, sin_in__, index_in__, k_cache_in__, ckv_cache_in__, k_rope_scale_in__, c_kv_scale_in__, k_rope_offset_in__, c_kv_offset_in__, k_cache_out_, ckv_cache_out_, k_rope_out_, c_kv_out_, epsilon, cache_mode, is_output_kv)
    result = check_op_cap("get_op_specific_info", "KvRmsNormRopeCache", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
