...
def _build_args(x_in__, y_in__, z_out_, axis):
    __inputs__ = []
...
@tbe_register.register_operator("SubMulConcat", trans_bool_to_s8=False)
...
def sub_mul_concat(x_in__, y_in__, z_out_, axis=-1, kernel_name="sub_mul_concat", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_in__, z_out_, axis)
    ...
    origin_func_name = "sub_mul_concat"
    ascendc_src_dir_ex = "sub_mul_concat"
    ascendc_src_dir = "sub_mul_concat"
    ascendc_src_file = "sub_mul_concat.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator SubMulConcat, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "SubMulConcat"
...
def op_select_format(x_in__, y_in__, z_out_, axis=-1, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_in__, z_out_, axis)
    result = check_op_cap("op_select_format", "SubMulConcat", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, y_in__, z_out_, axis=-1, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_in__, z_out_, axis)
    result = check_op_cap("get_op_specific_info", "SubMulConcat", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
