...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['log_probs', 'targets', 'input_lengths', 'target_lengths']
    output_names = ['neg_log_likelihood', 'log_alpha']
...
def _build_args(log_probs_in__, targets_in__, input_lengths_in__, target_lengths_in__, neg_log_likelihood_out_, log_alpha_out_, blank, reduction, zero_infinity):
    __inputs__ = []
    for arg in [log_probs_in__, targets_in__, input_lengths_in__, target_lengths_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [neg_log_likelihood_out_, log_alpha_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if reduction != None:
        ...
        __attrs__.append(attr)
    if zero_infinity != None:
        attr = {}
        attr["name"] = "zero_infinity"
        attr["dtype"] = "bool"
        attr["value"] = zero_infinity
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("CTCLossV3", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def ctc_loss_v3(log_probs_in__, targets_in__, input_lengths_in__, target_lengths_in__, neg_log_likelihood_out_, log_alpha_out_, blank=0, reduction="mean", zero_infinity=False, kernel_name="ctc_loss_v3", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(log_probs_in__, targets_in__, input_lengths_in__, target_lengths_in__, neg_log_likelihood_out_, log_alpha_out_, blank, reduction, zero_infinity)
    ...
    origin_func_name = "ctc_loss_v3"
    ascendc_src_dir_ex = "ctc_loss_v3"
    ascendc_src_dir = "c_t_c_loss_v3"
    ascendc_src_file = "ctc_loss_v3.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator CTCLossV3, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "CTCLossV3"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[log_probs_in__, targets_in__, input_lengths_in__, target_lengths_in__], origin_outputs = [neg_log_likelihood_out_, log_alpha_out_],\
...
def op_select_format(log_probs_in__, targets_in__, input_lengths_in__, target_lengths_in__, neg_log_likelihood_out_, log_alpha_out_, blank=0, reduction="mean", zero_infinity=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(log_probs_in__, targets_in__, input_lengths_in__, target_lengths_in__, neg_log_likelihood_out_, log_alpha_out_, blank, reduction, zero_infinity)
    result = check_op_cap("op_select_format", "CTCLossV3", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(log_probs_in__, targets_in__, input_lengths_in__, target_lengths_in__, neg_log_likelihood_out_, log_alpha_out_, blank=0, reduction="mean", zero_infinity=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(log_probs_in__, targets_in__, input_lengths_in__, target_lengths_in__, neg_log_likelihood_out_, log_alpha_out_, blank, reduction, zero_infinity)
    result = check_op_cap("get_op_specific_info", "CTCLossV3", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
