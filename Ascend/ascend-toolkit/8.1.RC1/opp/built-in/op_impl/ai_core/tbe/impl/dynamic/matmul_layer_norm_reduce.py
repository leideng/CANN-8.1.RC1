...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x1', 'weight', 'bias', 'x2', 'x3']
    output_names = ['x2', 'sum', 'square_sum']
...
def _build_args(x1_in__, weight_in__, bias_in__, x2_in__, x3_in__, x2_out_, sum_out_, square_sum_out_):
    __inputs__ = []
    for arg in [x1_in__, weight_in__, bias_in__, x2_in__, x3_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [x2_out_, sum_out_, square_sum_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("MatmulLayerNormReduce", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.KERNEL_NAME)
def matmul_layer_norm_reduce(x1_in__, weight_in__, bias_in__, x2_in__, x3_in__=None, x2_out_=None, sum_out_=None, square_sum_out_=None, kernel_name="matmul_layer_norm_reduce", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, weight_in__, bias_in__, x2_in__, x3_in__, x2_out_, sum_out_, square_sum_out_)
    ...
    origin_func_name = "matmul_layer_norm_reduce"
    ascendc_src_dir_ex = "matmul_layer_norm_reduce"
    ascendc_src_dir = "matmul_layer_norm_reduce"
    ascendc_src_file = "matmul_layer_norm_reduce.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MatmulLayerNormReduce, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "MatmulLayerNormReduce"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x1_in__, weight_in__, bias_in__, x2_in__, x3_in__], origin_outputs = [x2_out_, sum_out_, square_sum_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'optional', 'required', 'required', 'required'], init_value_list = [None, None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{"auto_sync": false}')

def op_select_format(x1_in__, weight_in__, bias_in__, x2_in__, x3_in__=None, x2_out_=None, sum_out_=None, square_sum_out_=None, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, weight_in__, bias_in__, x2_in__, x3_in__, x2_out_, sum_out_, square_sum_out_)
    result = check_op_cap("op_select_format", "MatmulLayerNormReduce", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x1_in__, weight_in__, bias_in__, x2_in__, x3_in__=None, x2_out_=None, sum_out_=None, square_sum_out_=None, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, weight_in__, bias_in__, x2_in__, x3_in__, x2_out_, sum_out_, square_sum_out_)
    result = check_op_cap("get_op_specific_info", "MatmulLayerNormReduce", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
