...
@tbe_register.register_operator("ForeachZeroInplace", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.DYNAMIC_INPUT, para_check.KERNEL_NAME)
def foreach_zero_inplace(x_in__, kernel_name="foreach_zero_inplace", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__)
    ...
    origin_func_name = "foreach_zero_inplace"
    ascendc_src_dir_ex = "foreach_zero_inplace"
    ascendc_src_dir = "foreach_zero_inplace"
    ascendc_src_file = "foreach_zero_inplace.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator ForeachZeroInplace, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "ForeachZeroInplace"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__], origin_outputs = [],\
                param_type_dynamic = True, mc2_ctx = [], param_type_list = ['dynamic'], init_value_list = [],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{"auto_sync": false}')

def op_select_format(x_in__, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__)
    result = check_op_cap("op_select_format", "ForeachZeroInplace", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__)
    result = check_op_cap("get_op_specific_info", "ForeachZeroInplace", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
