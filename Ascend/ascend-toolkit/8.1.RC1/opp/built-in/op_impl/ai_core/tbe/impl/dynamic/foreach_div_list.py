...
@tbe_register.register_operator("ForeachDivList", trans_bool_to_s8=False)
...
def foreach_div_list(x1_in__, x2_in__, y_out_, kernel_name="foreach_div_list", impl_mode=""):
    ...
    origin_func_name = "foreach_div_list"
    ascendc_src_dir_ex = "foreach_div_list"
    ascendc_src_dir = "foreach_div_list"
    ascendc_src_file = "foreach_div_list.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator ForeachDivList, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "ForeachDivList"
...
def op_select_format(x1_in__, x2_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, y_out_)
    result = check_op_cap("op_select_format", "ForeachDivList", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x1_in__, x2_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, y_out_)
    result = check_op_cap("get_op_specific_info", "ForeachDivList", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
