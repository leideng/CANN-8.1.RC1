...
public:
    __aicore__ inline MoeFinalizeRoutingV2BF16(
...
template <typename T, const bool ISBIASEXIST>
__aicore__ inline int64_t MoeFinalizeRoutingV2BF16<T, ISBIASEXIST>::AlignmentProcess(int64_t param)
{
    return (param + onceAlgnNum_ - 1) / onceAlgnNum_ * onceAlgnNum_;
}

template <typename T, const bool ISBIASEXIST>
__aicore__ inline int64_t MoeFinalizeRoutingV2BF16<T, ISBIASEXIST>::PadProcess(int64_t param)
{
    return  onceAlgnNum_ - param % onceAlgnNum_;
}

template <typename T, const bool ISBIASEXIST>
__aicore__ inline void MoeFinalizeRoutingV2BF16<T, ISBIASEXIST>::CkechColAlignment()
{
...
template <typename T, const bool ISBIASEXIST>
__aicore__ inline void MoeFinalizeRoutingV2BF16<T, ISBIASEXIST>::Init(
    GM_ADDR expandedPermutedRows,
    GM_ADDR expandedSrcToDstRow,
    GM_ADDR skip1,
    GM_ADDR skip2,
...
template <typename T, const bool ISBIASEXIST>
__aicore__ inline void MoeFinalizeRoutingV2BF16<T, ISBIASEXIST>::CopyIn(int64_t nLoopIdx, int64_t curRepeatTimes)
{
...
template <typename T, const bool ISBIASEXIST>
__aicore__ inline void MoeFinalizeRoutingV2BF16<T, ISBIASEXIST>::Compute(int64_t nLoopIdx, int64_t curRepeatTimes)
{
    ...
    for (int64_t i = 0; i < curRepeatTimes; i++) {
        ...
        for (int64_t j = 0; j < tilingData_.K / PARALLEL_NUM; j++) {
            ...
            if (tilingData_.dropPadMode == MODE_VALUE_0 || tilingData_.dropPadMode == MODE_VALUE_1) {
                ...
                expandedSrcToDstRowIndexDb0 = biasInLoop * tilingData_.K + i + PARALLEL_NUM * j * tilingData_.totalRowNum + biasInCore_;
            }
            ...
            if (tilingData_.dropPadMode == MODE_VALUE_0 || tilingData_.dropPadMode == MODE_VALUE_1) {
                expandedSrcToDstRowIndexDb1 = biasInLoop + i + (PARALLEL_NUM * j + 1) * tilingData_.totalRowNum + biasInCore_;
            } else if (tilingData_.dropPadMode == MODE_VALUE_2 || tilingData_.dropPadMode == MODE_VALUE_3) {
        ...
        if (tilingData_.K % PARALLEL_NUM != 0) {
            ...
            int64_t expandedSrcToDstRowIndexDb0 = 0;
            if (tilingData_.dropPadMode == MODE_VALUE_0 || tilingData_.dropPadMode == MODE_VALUE_1) {
                expandedSrcToDstRowIndexDb0 = biasInLoop + i + (tilingData_.K - 1) * tilingData_.totalRowNum + biasInCore_;
            } else if (tilingData_.dropPadMode == MODE_VALUE_2 || tilingData_.dropPadMode == MODE_VALUE_3) {
...
template <typename T, const bool ISBIASEXIST>
__aicore__ inline void MoeFinalizeRoutingV2BF16<T, ISBIASEXIST>::CopyOut(int64_t nLoopIdx, int64_t curRepeatTimes)
{
...
template <typename T, const bool ISBIASEXIST>
__aicore__ inline void MoeFinalizeRoutingV2BF16<T, ISBIASEXIST>::Process()
{
