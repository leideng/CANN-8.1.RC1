...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['gradOutput', 'self', 'target', 'weight', 'totalWeight']
    output_names = ['out']
...
def _build_args(gradOutput_in__, self_in__, target_in__, weight_in__, totalWeight_in__, out_out_, reduction, ignoreIndex):
    __inputs__ = []
    for arg in [gradOutput_in__, self_in__, target_in__, weight_in__, totalWeight_in__]:
        if arg != None:
    ...
    if ignoreIndex != None:
        attr = {}
        attr["name"] = "ignoreIndex"
        attr["dtype"] = "int"
        attr["value"] = ignoreIndex
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("NLLLossGrad310p", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def nll_loss_grad310p(gradOutput_in__, self_in__, target_in__, weight_in__, totalWeight_in__, out_out_, reduction, ignoreIndex=-100, kernel_name="nll_loss_grad310p", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(gradOutput_in__, self_in__, target_in__, weight_in__, totalWeight_in__, out_out_, reduction, ignoreIndex)
    ...
    origin_func_name = "nll_loss_grad310p"
    ascendc_src_dir_ex = "nll_loss_grad310p"
    ascendc_src_dir = "n_l_l_loss_grad310p"
    ascendc_src_file = "nll_loss_grad310p.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator NLLLossGrad310p, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "NLLLossGrad310p"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[gradOutput_in__, self_in__, target_in__, weight_in__, totalWeight_in__], origin_outputs = [out_out_],\
...
def op_select_format(gradOutput_in__, self_in__, target_in__, weight_in__, totalWeight_in__, out_out_, reduction, ignoreIndex=-100, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(gradOutput_in__, self_in__, target_in__, weight_in__, totalWeight_in__, out_out_, reduction, ignoreIndex)
    result = check_op_cap("op_select_format", "NLLLossGrad310p", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(gradOutput_in__, self_in__, target_in__, weight_in__, totalWeight_in__, out_out_, reduction, ignoreIndex=-100, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(gradOutput_in__, self_in__, target_in__, weight_in__, totalWeight_in__, out_out_, reduction, ignoreIndex)
    result = check_op_cap("get_op_specific_info", "NLLLossGrad310p", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
