...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['value', 'value_spatial_shapes', 'value_level_start_index', 'sampling_locations', 'attention_weights', 'grad_output']
    output_names = ['grad_value', 'grad_sampling_locations', 'grad_attention_weights']
...
def _build_args(value_in__, value_spatial_shapes_in__, value_level_start_index_in__, sampling_locations_in__, attention_weights_in__, grad_output_in__, grad_value_out_, grad_sampling_locations_out_, grad_attention_weights_out_):
    __inputs__ = []
    for arg in [value_in__, value_spatial_shapes_in__, value_level_start_index_in__, sampling_locations_in__, attention_weights_in__, grad_output_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [grad_value_out_, grad_sampling_locations_out_, grad_attention_weights_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("MultiScaleDeformableAttentionGrad", trans_bool_to_s8=False)
...
def multi_scale_deformable_attention_grad(value_in__, value_spatial_shapes_in__, value_level_start_index_in__, sampling_locations_in__, attention_weights_in__, grad_output_in__, grad_value_out_, grad_sampling_locations_out_, grad_attention_weights_out_, kernel_name="multi_scale_deformable_attention_grad", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(value_in__, value_spatial_shapes_in__, value_level_start_index_in__, sampling_locations_in__, attention_weights_in__, grad_output_in__, grad_value_out_, grad_sampling_locations_out_, grad_attention_weights_out_)
    ...
    origin_func_name = "multi_scale_deformable_attention_grad"
    ascendc_src_dir_ex = "multi_scale_deformable_attention_grad"
    ascendc_src_dir = "multi_scale_deformable_attention_grad"
    ascendc_src_file = "multi_scale_deformable_attention_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MultiScaleDeformableAttentionGrad, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "MultiScaleDeformableAttentionGrad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[value_in__, value_spatial_shapes_in__, value_level_start_index_in__, sampling_locations_in__, attention_weights_in__, grad_output_in__], origin_outputs = [grad_value_out_, grad_sampling_locations_out_, grad_attention_weights_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required', 'required', 'required', 'required', 'required'], init_value_list = [None, None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(value_in__, value_spatial_shapes_in__, value_level_start_index_in__, sampling_locations_in__, attention_weights_in__, grad_output_in__, grad_value_out_, grad_sampling_locations_out_, grad_attention_weights_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(value_in__, value_spatial_shapes_in__, value_level_start_index_in__, sampling_locations_in__, attention_weights_in__, grad_output_in__, grad_value_out_, grad_sampling_locations_out_, grad_attention_weights_out_)
    result = check_op_cap("op_select_format", "MultiScaleDeformableAttentionGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(value_in__, value_spatial_shapes_in__, value_level_start_index_in__, sampling_locations_in__, attention_weights_in__, grad_output_in__, grad_value_out_, grad_sampling_locations_out_, grad_attention_weights_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(value_in__, value_spatial_shapes_in__, value_level_start_index_in__, sampling_locations_in__, attention_weights_in__, grad_output_in__, grad_value_out_, grad_sampling_locations_out_, grad_attention_weights_out_)
    result = check_op_cap("get_op_specific_info", "MultiScaleDeformableAttentionGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
