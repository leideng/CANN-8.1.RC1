...
def _build_args(a_in__, b_in__, alpha_in__, beta_in__, c_in__, c_out_, transpose_a, transpose_b):
    __inputs__ = []
    for arg in [a_in__, b_in__, alpha_in__, beta_in__, c_in__]:
        if arg != None:
    ...
    if transpose_a != None:
        attr = {}
        attr["name"] = "transpose_a"
        attr["dtype"] = "bool"
        attr["value"] = transpose_a
        __attrs__.append(attr)
    if transpose_b != None:
        attr = {}
        attr["name"] = "transpose_b"
        attr["dtype"] = "bool"
        attr["value"] = transpose_b
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("GemmV2", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def gemm_v2(a_in__, b_in__, alpha_in__, beta_in__, c_in__, c_out_, transpose_a=False, transpose_b=False, kernel_name="gemm_v2", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(a_in__, b_in__, alpha_in__, beta_in__, c_in__, c_out_, transpose_a, transpose_b)
    ...
    origin_func_name = "gemm_v2"
    ascendc_src_dir_ex = "gemm_v2"
    ascendc_src_dir = "gemm_v2"
    ascendc_src_file = "gemm_v2.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator GemmV2, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "GemmV2"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[a_in__, b_in__, alpha_in__, beta_in__, c_in__], origin_outputs = [c_out_],\
...
def op_select_format(a_in__, b_in__, alpha_in__, beta_in__, c_in__, c_out_, transpose_a=False, transpose_b=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(a_in__, b_in__, alpha_in__, beta_in__, c_in__, c_out_, transpose_a, transpose_b)
    result = check_op_cap("op_select_format", "GemmV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(a_in__, b_in__, alpha_in__, beta_in__, c_in__, c_out_, transpose_a=False, transpose_b=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(a_in__, b_in__, alpha_in__, beta_in__, c_in__, c_out_, transpose_a, transpose_b)
    result = check_op_cap("get_op_specific_info", "GemmV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
