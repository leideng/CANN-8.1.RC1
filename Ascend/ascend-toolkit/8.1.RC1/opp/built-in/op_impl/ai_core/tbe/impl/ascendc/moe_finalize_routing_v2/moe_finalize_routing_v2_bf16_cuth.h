...
public:
    __aicore__ inline MoeFinalizeRoutingV2Bf16Cuth(
        const MoeFinalizeRoutingV2TilingData &tilingData,
        TPipe &pipe) : tilingData_(tilingData), pipe_(pipe){};
...
template <typename T, const bool ISBIASEXIST>
__aicore__ inline int64_t MoeFinalizeRoutingV2Bf16Cuth<T, ISBIASEXIST>::AlignmentProcess(int64_t param)
{
    return (param + onceAlgnNum_ - 1) / onceAlgnNum_ * onceAlgnNum_;
}

template <typename T, const bool ISBIASEXIST>
__aicore__ inline int64_t MoeFinalizeRoutingV2Bf16Cuth<T, ISBIASEXIST>::PadProcessT(int64_t param)
{
    return  onceAlgnNum_ - param % onceAlgnNum_;
}

template <typename T, const bool ISBIASEXIST>
__aicore__ inline void MoeFinalizeRoutingV2Bf16Cuth<T, ISBIASEXIST>::CkechColAlignment()
{
...
template <typename T, const bool ISBIASEXIST>
__aicore__ inline void MoeFinalizeRoutingV2Bf16Cuth<T, ISBIASEXIST>::Init(GM_ADDR expandedPermutedRows,
...
template <typename T, const bool ISBIASEXIST>
__aicore__ inline void MoeFinalizeRoutingV2Bf16Cuth<T, ISBIASEXIST>::CopyIn(int64_t nLoopIdx, int64_t bias, int64_t dataLen,
    bool isPadH, int64_t rightPaddingH)
{
...
template <typename T, const bool ISBIASEXIST>
__aicore__ inline void MoeFinalizeRoutingV2Bf16Cuth<T, ISBIASEXIST>::Compute(int64_t nLoopIdx, int64_t bias, int64_t dataLen,
    bool isPadH, int64_t rightPaddingH)
{
    ...
    for (int64_t i = 0; i < tilingData_.K / PARALLEL_NUM; i++) {
        ...
        if (tilingData_.dropPadMode == MODE_VALUE_0 || tilingData_.dropPadMode == MODE_VALUE_1) {
            expandedSrcToDstRowIndexDb0 = nLoopIdx / (tilingData_.hSliceNum + 1) + PARALLEL_NUM * i * tilingData_.totalRowNum + GetBlockIdx() * tilingData_.normalCoreHandleNum;
        ...
        if (tilingData_.dropPadMode == MODE_VALUE_0 || tilingData_.dropPadMode == MODE_VALUE_1) {
            expandedSrcToDstRowIndexDb1 = nLoopIdx / (tilingData_.hSliceNum + 1) + (PARALLEL_NUM * i + 1) * tilingData_.totalRowNum + GetBlockIdx() * tilingData_.normalCoreHandleNum;
        } else if (tilingData_.dropPadMode == MODE_VALUE_2 || tilingData_.dropPadMode == MODE_VALUE_3) {
    ...
    if (tilingData_.K % PARALLEL_NUM != 0) {
        ...
        int64_t expandedSrcToDstRowIndexDb0 = 0; 
        if (tilingData_.dropPadMode == MODE_VALUE_0 || tilingData_.dropPadMode == MODE_VALUE_1) {
            expandedSrcToDstRowIndexDb0 = nLoopIdx / (tilingData_.hSliceNum + 1) + (tilingData_.K - 1) * tilingData_.totalRowNum + GetBlockIdx() * tilingData_.normalCoreHandleNum;
        } else if (tilingData_.dropPadMode == MODE_VALUE_2 || tilingData_.dropPadMode == MODE_VALUE_3) {
...
template <typename T, const bool ISBIASEXIST>
__aicore__ inline void MoeFinalizeRoutingV2Bf16Cuth<T, ISBIASEXIST>::CopyOut(int64_t nLoopIdx, int64_t bias, int64_t dataLen)
{
...
template <typename T, const bool ISBIASEXIST>
__aicore__ inline void MoeFinalizeRoutingV2Bf16Cuth<T, ISBIASEXIST>::Process()
{
