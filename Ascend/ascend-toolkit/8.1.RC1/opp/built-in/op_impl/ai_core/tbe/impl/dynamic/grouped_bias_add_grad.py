...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['grad_y', 'group_idx']
    output_names = ['grad_bias']
...
def _build_args(grad_y_in__, group_idx_in__, grad_bias_out_, group_idx_type):
    __inputs__ = []
    for arg in [grad_y_in__, group_idx_in__]:
        if arg != None:
    ...
    for arg in [grad_bias_out_]:
        if arg != None:
            ...
            __outputs__.append(arg)
    __attrs__ = []
    if group_idx_type != None:
        attr = {}
        attr["name"] = "group_idx_type"
        attr["dtype"] = "int"
        attr["value"] = group_idx_type
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("GroupedBiasAddGrad", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def grouped_bias_add_grad(grad_y_in__, group_idx_in__=None, grad_bias_out_=None, group_idx_type=0, kernel_name="grouped_bias_add_grad", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(grad_y_in__, group_idx_in__, grad_bias_out_, group_idx_type)
    ...
    origin_func_name = "grouped_bias_add_grad"
    ascendc_src_dir_ex = "grouped_bias_add_grad"
    ascendc_src_dir = "grouped_bias_add_grad"
    ascendc_src_file = "grouped_bias_add_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator GroupedBiasAddGrad, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "GroupedBiasAddGrad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[grad_y_in__, group_idx_in__], origin_outputs = [grad_bias_out_],\
