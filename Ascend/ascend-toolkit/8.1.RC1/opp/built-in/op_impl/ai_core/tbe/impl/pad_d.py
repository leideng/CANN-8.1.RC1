...
from impl.dynamic.pad_d import pad_d as pad_d_dynamic
...
class PadParams:
    ...
    def get_align_mask(self, offset):
        ...
        with self.ib_.for_range(0, self.align_offset[0], kind="serial", name="i"):
        ...
        if out_ubuf:
            if in_shape[axis] % self.cp_align_len == 0 and \
                ...
                if self.copy_mode != self.enum_copy_mode['align_dst_fail'] and\
                        self.copy_mode != \
                        self.enum_copy_mode.get('align_dst_src_fail'):
                    self.copy_mode = self.enum_copy_mode['align_ok']
            else:
                if self.copy_mode != self.enum_copy_mode.get('align_dst_src_fail'):
...
def _pad_recursive_fun(axis, bufs, gm_align, params, multi_core_top=False):
    ...
    if fuse_wc_axis:
        ...
        with params.ib_.for_range(0, params.in_shape[axis], kind="serial", name="i") as i:
            if out_buf.buf != 0:
...
def _pad_multi_core_fun(axis, bufs, gm_align, params):
    ...
    if axis + 1 >= len(params.in_shape):
        ...
        with params.ib_.for_range(0, repeat, kind="serial", name="i") as i:
...
def _emit_copy_ubuf_to_gm_safely(tvm_ir, dtype, dst, src, count, tail_ub, dst_offset=0, src_offset=0):
    """
    emit_copy_ubuf_to_gm: count must equal or larger than block_len
    """
    block_len = 16
    if dtype in ('float32', 'int32'):
        block_len = 8
...
def _check_optimization_nhwc(input_x, paddings):
    ...
    if dtype == "float16":
        ub_maxsize = tbe_platform.get_soc_spec(tbe_platform.UB_SIZE) // 32 * 16
    else:
        ub_maxsize = tbe_platform.get_soc_spec(tbe_platform.UB_SIZE) // 32 * 8
    device_core_num = \
        tbe_platform.get_soc_spec(
            tbe_platform.CORE_NUM)
...
def pad_d(input_x, output_x, paddings, kernel_name="pad_d"):
    ...
    check_list_dtype = ("bfloat16", "float16", "float32", "int32", "int8", "uint8")
    dtype = input_x.get("dtype").lower()
    ...
    if input_format == "NC1HWC0" and ori_format == "NHWC" and shape == [125, 4, 16, 16, 16]:
        pad_d_dynamic(input_x, output_x, paddings, kernel_name)
    elif _check_optimization_nchw(input_x, paddings):
