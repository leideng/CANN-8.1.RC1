...
def _build_args(logit_prob_in__, topp_in__, indices_out_, rand_seed):
    __inputs__ = []
    for arg in [logit_prob_in__, topp_in__]:
        if arg != None:
    ...
    if rand_seed != None:
        ...
        attr["name"] = "rand_seed"
        attr["dtype"] = "list_int"
        attr["value"] = rand_seed
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("TopPSample", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_LIST_INT, para_check.KERNEL_NAME)
def top_p_sample(logit_prob_in__, topp_in__, indices_out_, rand_seed=[], kernel_name="top_p_sample", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(logit_prob_in__, topp_in__, indices_out_, rand_seed)
    ...
    origin_func_name = "top_p_sample"
    ascendc_src_dir_ex = "top_p_sample"
    ascendc_src_dir = "top_p_sample"
    ascendc_src_file = "top_p_sample.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator TopPSample, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "TopPSample"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[logit_prob_in__, topp_in__], origin_outputs = [indices_out_],\
...
def op_select_format(logit_prob_in__, topp_in__, indices_out_, rand_seed=[], impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(logit_prob_in__, topp_in__, indices_out_, rand_seed)
    result = check_op_cap("op_select_format", "TopPSample", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(logit_prob_in__, topp_in__, indices_out_, rand_seed=[], impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(logit_prob_in__, topp_in__, indices_out_, rand_seed)
    result = check_op_cap("get_op_specific_info", "TopPSample", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
