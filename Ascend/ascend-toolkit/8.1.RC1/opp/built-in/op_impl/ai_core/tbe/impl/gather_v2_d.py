...
def _get_target_core_num(tensor_params, tensor_indices):
    ...
    dtype_size = get_bit_len(tensor_params.dtype) // 8
...
def _get_ub_available_size(indices_shape, dtype, tensor_params):
    ...
    dtype_size = get_bit_len(dtype) // 8
    params_dtype_size = get_bit_len(tensor_params.dtype) // 8
    ...
    is_params_ub = False
    params_ub_size = 0
    row_size = int(tensor_params.shape[2]) * (get_bit_len(tensor_params.dtype) // 8)
...
def _params_tiling(tensor_params, tensor_indices, row_num_each_core):
    ...
    """
    # Convert byts to Bytes
    src_dtype_size = get_bit_len(tensor_params.dtype) // 8
...
def _gather_v2_operation(tensor_params, tensor_indices, output, tvm_ib,
    ...
    num_in_32b = 32 * 8 // get_bit_len(tensor_params.dtype)
...
def _small_dataset_operation(tensor_params, tensor_indices, output, tvm_ib,
    ...
    bit_size = get_bit_len(tensor_params.dtype)
...
def _indices_tiling(tensor_params, tensor_indices, row_num_each_core):
    ...
    indices_dtype_size = get_bit_len(tensor_indices.dtype) // 8
...
def _kernel_ir(output, tensor_params, tensor_indices, axis):
    ...
    row_size = int(tensor_params.shape[2]) * \
               (get_bit_len(tensor_params.dtype) // 8)
    bit_size = get_bit_len(tensor_params.dtype)
    indices_len = int(functools.reduce(lambda i, j: i * j,
                                       tensor_indices.shape[:]))
    gather_len = int(functools.reduce(lambda i, j: i * j, tensor_params.shape[1:]))
    indices_size = indices_len * (get_bit_len(tensor_indices.dtype) // 8)
    ...
    op_parameters['ub_for_32B'] = _new_alloc(
        tvm_ib,
        tensor_params.dtype,
        32 * 8 // get_bit_len(tensor_params.dtype),
        "ub_for_32B",
        scope=tbe_platform.scope_ubuf)
    ...
    with tvm_ib.for_range(0, pre_axis_num, name='pre_axis_num') as pre_num:
        ...
        row_len = int(tensor_params.shape[2])
        align_offset = 0
        params_bit_size = get_bit_len(tensor_params.dtype) // 8
        with tvm_ib.if_scope(last_loop_row_num > 0):
