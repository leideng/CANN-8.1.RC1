...
template <typename T>
__aicore__ inline void MoeFinalizeRoutingFpDbAllBias<T>::Init(
    GM_ADDR expandedPermutedRows,
    GM_ADDR skip1,
    GM_ADDR skip2,
    GM_ADDR bias,
...
__aicore__ inline void MoeFinalizeRoutingFpDbAllBias<T>::Compute(int64_t nLoopIdx, int64_t curRepeatTimes)
{
    ...
    for (int64_t i = 0; i < curRepeatTimes; i++) {
        ...
        for (int64_t j = 0; j < K_ / PARALLEL_NUM; j++) {
            ...
            int64_t expandedSrcToDstRowIndexDb0 = nLoopIdx * curCoreHandleNumPerLoop_ + i +
            ...
            int64_t biasIndexDb0 = expertForSourceRowLocal.GetValue(i * Int32AlignmentProcess(K_) +
                                                                    PARALLEL_NUM * j);
        ...
        if (K_ % PARALLEL_NUM != 0) {
            wait_flag(PIPE_V, PIPE_S, EVENT_ID0);
            int64_t expandedSrcToDstRowIndexDb0 = nLoopIdx * curCoreHandleNumPerLoop_ + i +
                                                  (K_ - 1) * Int32AlignmentProcess(curCoreHandleNum_);
            int64_t expandedPermutedRowsIndexDb0 = expandedSrcToDstRow_.GetValue(expandedSrcToDstRowIndexDb0);
            set_flag(PIPE_S, PIPE_MTE2, EVENT_ID0);
