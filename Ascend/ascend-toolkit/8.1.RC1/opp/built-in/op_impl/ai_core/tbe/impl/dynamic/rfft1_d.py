...
def _build_args(x_in__, dft_in__, y_out_, n, norm):
    __inputs__ = []
    for arg in [x_in__, dft_in__]:
        if arg != None:
...
@tbe_register.register_operator("Rfft1D", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def rfft1_d(x_in__, dft_in__=None, y_out_=None, n=0, norm=0, kernel_name="rfft1_d", impl_mode=""):
    dft_in__ = {"param_name":"dft_in__","index":1,"dtype":"float32","format":"ND","ori_format":"ND","paramType":"optional","shape":[1],"ori_shape":[1]}
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, dft_in__, y_out_, n, norm)
    ...
    origin_func_name = "rfft1_d"
    ascendc_src_dir_ex = "rfft1_d"
    ascendc_src_dir = "rfft1_d"
    ascendc_src_file = "rfft1_d.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator Rfft1D, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "Rfft1D"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, dft_in__], origin_outputs = [y_out_],\
...
def op_select_format(x_in__, dft_in__=None, y_out_=None, n=0, norm=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, dft_in__, y_out_, n, norm)
    result = check_op_cap("op_select_format", "Rfft1D", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, dft_in__=None, y_out_=None, n=0, norm=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, dft_in__, y_out_, n, norm)
    result = check_op_cap("get_op_specific_info", "Rfft1D", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
