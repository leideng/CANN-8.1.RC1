...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'weight', 'antiquant_scale', 'antiquant_offset', 'quant_scale', 'quant_offset', 'bias']
    output_names = ['y']
...
def _build_args(x_in__, weight_in__, antiquant_scale_in__, antiquant_offset_in__, quant_scale_in__, quant_offset_in__, bias_in__, y_out_, transpose_x, transpose_weight, antiquant_group_size, dtype, inner_precise):
    __inputs__ = []
    for arg in [x_in__, weight_in__, antiquant_scale_in__, antiquant_offset_in__, quant_scale_in__, quant_offset_in__, bias_in__]:
        if arg != None:
...
@tbe_register.register_operator("WeightQuantBatchMatmulV2", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def weight_quant_batch_matmul_v2(x_in__, weight_in__, antiquant_scale_in__, antiquant_offset_in__=None, quant_scale_in__=None, quant_offset_in__=None, bias_in__=None, y_out_=None, transpose_x=False, transpose_weight=False, antiquant_group_size=0, dtype=-1, inner_precise=0, kernel_name="weight_quant_batch_matmul_v2", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, antiquant_scale_in__, antiquant_offset_in__, quant_scale_in__, quant_offset_in__, bias_in__, y_out_, transpose_x, transpose_weight, antiquant_group_size, dtype, inner_precise)
    ...
    origin_func_name = "weight_quant_batch_matmul_v2"
    ascendc_src_dir_ex = "weight_quant_batch_matmul_v2"
    ascendc_src_dir = "weight_quant_batch_matmul_v2"
    ascendc_src_file = "weight_quant_batch_matmul_v2.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator WeightQuantBatchMatmulV2, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, weight_in__, antiquant_scale_in__, antiquant_offset_in__, quant_scale_in__, quant_offset_in__, bias_in__], origin_outputs = [y_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'optional', 'optional', 'optional', 'optional', 'required'], init_value_list = [None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{"auto_sync": false}')

def op_select_format(x_in__, weight_in__, antiquant_scale_in__, antiquant_offset_in__=None, quant_scale_in__=None, quant_offset_in__=None, bias_in__=None, y_out_=None, transpose_x=False, transpose_weight=False, antiquant_group_size=0, dtype=-1, inner_precise=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, antiquant_scale_in__, antiquant_offset_in__, quant_scale_in__, quant_offset_in__, bias_in__, y_out_, transpose_x, transpose_weight, antiquant_group_size, dtype, inner_precise)
    result = check_op_cap("op_select_format", "WeightQuantBatchMatmulV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, weight_in__, antiquant_scale_in__, antiquant_offset_in__=None, quant_scale_in__=None, quant_offset_in__=None, bias_in__=None, y_out_=None, transpose_x=False, transpose_weight=False, antiquant_group_size=0, dtype=-1, inner_precise=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, antiquant_scale_in__, antiquant_offset_in__, quant_scale_in__, quant_offset_in__, bias_in__, y_out_, transpose_x, transpose_weight, antiquant_group_size, dtype, inner_precise)
    result = check_op_cap("get_op_specific_info", "WeightQuantBatchMatmulV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
