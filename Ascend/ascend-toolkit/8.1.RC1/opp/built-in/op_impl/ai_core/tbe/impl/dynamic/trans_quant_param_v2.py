...
def _build_args(scale_in__, offset_in__, y_out_):
    __inputs__ = []
    for arg in [scale_in__, offset_in__]:
        if arg != None:
...
@tbe_register.register_operator("TransQuantParamV2", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.KERNEL_NAME)
def trans_quant_param_v2(scale_in__, offset_in__=None, y_out_=None, kernel_name="trans_quant_param_v2", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(scale_in__, offset_in__, y_out_)
    ...
    origin_func_name = "trans_quant_param_v2"
    ascendc_src_dir_ex = "trans_quant_param_v2"
    ascendc_src_dir = "trans_quant_param_v2"
    ascendc_src_file = "trans_quant_param_v2.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator TransQuantParamV2, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[scale_in__, offset_in__], origin_outputs = [y_out_],\
...
def op_select_format(scale_in__, offset_in__=None, y_out_=None, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(scale_in__, offset_in__, y_out_)
    result = check_op_cap("op_select_format", "TransQuantParamV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(scale_in__, offset_in__=None, y_out_=None, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(scale_in__, offset_in__, y_out_)
    result = check_op_cap("get_op_specific_info", "TransQuantParamV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
