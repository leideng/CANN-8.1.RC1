...
def sparse_apply_adagrad_compute(var, accum, lr, grad, indices, var_out, accum_out, use_locking=False,
                                 update_slots=True, kernel_name="sparse_apply_adagrad"):
...
@register_operator("SparseApplyAdagrad", pattern="SparseApply")
...
def sparse_apply_adagrad(var,
    ...
    lr: dict
        data of input.
        Must be one dimension tensor, support "float32"
    grad: dict
        data of input
    ...
    for (ins_var, ins_accum, ins_grad, ins_indices) in ins:
        with tbe.compute():
            ...
            lr = tvm.placeholder((1,), lr.get("type"), "lr")
            res = sparse_apply_adagrad_compute(data_var, data_accum, lr, data_grad, data_indices, var_out, accum_out,
                                               use_locking, update_slots, kernel_name)
            tensors.append([data_var, data_accum, lr, data_grad, data_indices, res[0], res[1]])
        with tvm.target.cce():
