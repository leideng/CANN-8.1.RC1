...
class KernelKvRmsNormRopeCacheB16PABLKBNSDQUANT {
 public:
  ...
  __aicore__ inline void Process() {
    ...
    for (int64_t loopIdx = 0; loopIdx < ubLoop; ++loopIdx) {
      ...
      ScatterUpdate<KV_DTYPE, isPagedAttention, ROPE_LENGTH>(kCacheGm, kCacheGmNd, outLocal, quantLocal, startIdx, ubFactor);
      ...
      ScatterUpdate<KV_DTYPE, isPagedAttention, RMS_NORM_LENGTH>(vCacheGm, vCacheGmNd, outLocal, quantLocal, startIdx, ubFactor);
      outQueue.FreeTensor(outLocal);
    }
    if (ubTail > 0) {
      ...
      // Scatter Update kCache
      ScatterUpdate<KV_DTYPE, isPagedAttention, ROPE_LENGTH>(kCacheGm, kCacheGmNd, outLocal, quantLocal, startIdx, ubTail);
      outQueue.FreeTensor(outLocal);
      ...
      // Scatter Update vCache
      ScatterUpdate<KV_DTYPE, isPagedAttention, RMS_NORM_LENGTH>(vCacheGm, vCacheGmNd, outLocal, quantLocal, startIdx, ubTail);
      outQueue.FreeTensor(outLocal);
    }
  ...
  __aicore__ inline void ScatterUpdate(const GlobalTensor<int8_t>& dst,
                                       const GlobalTensor<T>& dstNd,
                                       const LocalTensor<T>& outLocal,
                                       const LocalTensor<int8_t>& quantLocal,
                                       int64_t startIdx, int64_t rows) {
    DataCopyExtParams copyParams{1, static_cast<uint32_t>(headSize * sizeof(int8_t)), 0, 0, 0};
    ...
    for (int64_t i = 0; i < rows; i++) {
      ...
      if (pageOffset >= 0) {
        ...
        int64_t gmOffset = pageId * tilingData_->blockSize * headSize + tokenOffsetInCurrentPage * headSize;
        SToMTE3Sync();
