...
private:
    ...
    __aicore__ inline void ComputeGroupNormSwish(const int64_t gmmmaOffset, const float mean,
...
template <typename T1, typename T2>
__aicore__ inline void GroupNormSwishHW1B32<T1, T2>::Init(GM_ADDR x, GM_ADDR gamma, GM_ADDR beta, GM_ADDR y, GM_ADDR mean,
...
__aicore__ inline void GroupNormSwishHW1B32<T1, T2>::ComputeOneLoopInner(const int64_t groupId,
    const LocalTensor<float>& gammaLocal, const LocalTensor<float>& betaLocal, const LocalTensor<float>& xUb)
{
