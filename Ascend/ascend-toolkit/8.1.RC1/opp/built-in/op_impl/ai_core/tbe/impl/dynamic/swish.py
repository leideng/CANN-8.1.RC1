...
def swish_normal(data_input, scale, dtype, exp_support):
    scale_scalar = get_attr_by_cls(scale, SwishAttrInfo.ATTR_SCALE, dtype)
    shape = data_input.shape
    scale_tensor = tbe.broadcast(scale_scalar, shape)
...
def swish_overflow(data_input, scale, dtype):
    data_input = tbe.cast_to(data_input, "float32")
    scale_scalar = get_attr_by_cls(scale, SwishAttrInfo.ATTR_SCALE, "float32")

    if after_v200():
        minus_scale_input = tbe.vmuls(data_input, -1.0 * scale_scalar)
        exp_bottom = tbe.vexp(minus_scale_input)
        one_plus_exp = tbe.vadds(exp_bottom, tvm.const(1.0, dtype="float32"))
        res = tbe.vdiv(data_input, one_plus_exp)
    else:
        scale_input = tbe.vmuls(data_input, scale_scalar)
...
def swish_inference_fp16(data_input, scale):
    """
    swish for float16
    y = x / (1.0 + exp(-scale*x))
    """
...
def swish_none_convert_fp16(data_input, scale):
    """
    swish for float16 without converting to fp32
    y = x / (1.0 + exp(-scale*x))
    """
    scale_scalar = get_attr_by_cls(scale, SwishAttrInfo.ATTR_SCALE, "float16")
    minus_scale_input = tbe.vmuls(data_input, -1.0 * scale_scalar)
    exp_bottom = tbe.vexp(minus_scale_input)
    one_plus_exp = tbe.vadds(exp_bottom, tvm.const(1.0, dtype="float16"))
    res = tbe.vdiv(data_input, one_plus_exp)
...
@register_operator_compute("Swish", op_mode="dynamic", support_fusion=True, support_bfp16=True)
def swish_compute(data_input, y, scale, kernel_name="swish"):
    ...
    if cur_cce_product in ('Ascend310P',) and dtype == "float16":
        return swish_inference_fp16(data_input, scale)

    if check_support_block_size_16():
        return swish_none_convert_fp16(data_input, scale)
