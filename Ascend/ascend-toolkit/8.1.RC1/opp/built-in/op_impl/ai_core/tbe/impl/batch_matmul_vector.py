...
def _schedule_large_km_kn(shape, list_computes, src_type):
    ...
    if src_type == "int32":
        ...
        schedule[tensor_temp_a].emit_insn(tensor_temp_a.op.axis[axis_inner], tbe_platform.CAST)
        schedule[tensor_temp_b].emit_insn(tensor_temp_b.op.axis[axis_inner], tbe_platform.CAST)
        schedule[tensor_result_ub_cast].emit_insn(tensor_result_ub_cast.op.axis[axis_inner],
                                                  tbe_platform.CAST_ROUND)

    schedule[tensor_a_ub].emit_insn(tensor_a_ub.op.axis[axis_inner], tbe_platform.DMA_COPY)
    schedule[tensor_b_ub].emit_insn(tensor_b_ub.op.axis[axis_inner], tbe_platform.DMA_COPY)

    if tensor_bais_ub is not None:
        if src_type == "int32":
            schedule[tensor_temp_bias].emit_insn(tensor_temp_bias.op.axis[axis_inner],
                                                 tbe_platform.CAST)
        schedule[tensor_bais_ub].emit_insn(tensor_bais_ub.op.axis[axis_inner], tbe_platform.DMA_COPY)

    schedule[the_result_mul_ub].emit_insn(the_result_mul_ub.op.axis[axis_inner + 1],
                                          tbe_platform.MULVS)
    schedule[the_result_ub].emit_insn(the_result_ub.op.axis[axis_inner], tbe_platform.ADD)

    if the_result_bais_ub is not None:
        schedule[the_result_bais_ub].emit_insn(the_result_bais_ub.op.axis[axis_inner],
                                               tbe_platform.ADD)

    schedule[result].emit_insn(axis_two[1], tbe_platform.DMA_COPY)
...
def _get_tiling_km_kn(shape):
    ...
    float32_size = 4
    ub_size = tbe_platform.get_soc_spec(tbe_platform.UB_SIZE) / float32_size
...
def _schedule_large_mk_kn(shape, list_computes, src_type):
    ...
    if src_type == "int32":
        ...
        schedule[tensor_temp_a].emit_insn(axis_one_cast[1], tbe_platform.CAST)
        schedule[tensor_temp_b].emit_insn(tensor_temp_b.op.axis[dim_one], tbe_platform.CAST)
        schedule[tensor_result_ub_cast].emit_insn(tensor_result_ub_cast.op.axis[dim_one],
                                                  tbe_platform.CAST_ROUND)

    schedule[tensor_a_ub].emit_insn(axis_one[1], tbe_platform.DMA_COPY)
    schedule[tensor_b_ub].emit_insn(tensor_b_ub.op.axis[dim_one], tbe_platform.DMA_COPY)

    if tensor_bais_ub is not None:
        schedule[tensor_bais_ub].emit_insn(tensor_bais_ub.op.axis[dim_one], tbe_platform.DMA_COPY)
    ...
    schedule[the_result].emit_insn(axis_four[1], tbe_platform.DMA_COPY)
...
def _single_tiling(shape, schedule, list_computes, src_type, shape_length):
    ...
    schedule[tensor_a_ub].emit_insn(axis_one[1], tbe_platform.DMA_COPY)
    schedule[tensor_b_ub].emit_insn(axis_two[1], tbe_platform.DMA_COPY)

    if src_type == "int32":
        schedule[tensor_temp_a].emit_insn(axis_one_cast[1], tbe_platform.CAST)
        schedule[tensor_temp_b].emit_insn(axis_two_cast[1], tbe_platform.CAST)

    schedule[the_result_mul_ub].emit_insn(axis_three[1], tbe_platform.MUL)
    schedule[the_result_ub].emit_insn(axis_four[1], tbe_platform.REDUCE_SUM)
...
def _schedule_large_mk_nk(shape, list_computes, src_type, shape_length):
    ...
    schedule[the_result].emit_insn(axis_two[1], tbe_platform.DMA_COPY)
...
def _tranpose_schedule(schedule, data, data_ub, res, shape_res, dtype):
    ...
    sch[data_ub].emit_insn(data_ub.op.axis[split_axis], tbe_platform.DMA_COPY)
    sch = _do_storage_align(sch, data_ub, shape_res, dtype)
    sch[res].emit_insn(axis_inner, tbe_platform.DMA_COPY)
