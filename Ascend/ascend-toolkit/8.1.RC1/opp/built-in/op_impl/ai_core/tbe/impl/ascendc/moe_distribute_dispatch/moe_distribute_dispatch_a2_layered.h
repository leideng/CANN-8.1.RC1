    ...
    GM_ADDR workspaceGM, TPipe *pipe, GM_ADDR tilingGM)
{
    ...
    uint64_t winSizeMin = moeExpertNum_ * axisBS_ * (axisH_ * sizeof(XType) + EXTRA_TOKEN_INFO_NUM * axisK_ * sizeof(uint32_t)) + 
...
__aicore__ inline void MoeDistributeDispatchA2Layered<TemplateMC2TypeA2layeredFunc>::ReorderTokens()
{
    ...
    // 仅用于量化cast
    LocalTensor<float> tokenCastLt = tBuf.GetWithOffset<float>(((axisH_ * sizeof(float) *
        sendTokenNum) / sizeof(float)), RoundUp(TBUF_TEMP_OFFSET + tokenUbSize * sendTokenNum, B32_PER_BLOCK));
    ...
    DataCopy(expertIdsTensor_, expertIdsGMTensor_, expertIdsCnt_);  // 16*8*4
    SyncFunc<AscendC::HardEvent::MTE2_S>();
    // 专家List读取
    if constexpr (DynamicQuant || StaticQuant) {
    ...
    DataCopyPad(tokenTempTensorU8_[expOffsetInStruct_], expertIdsGMTensorU8_[startTokenId * expLenInStruct_],
    ...
    DataCopyPad(tokenTempTensorU8_[weightOffsetInStruct_], weightGt[startTokenId * weightLenInStruct_],
