...
@tbe_register.register_operator("ForeachAtan", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.DYNAMIC_INPUT, para_check.DYNAMIC_OUTPUT, para_check.KERNEL_NAME)
def foreach_atan(x_in__, y_out_, kernel_name="foreach_atan", impl_mode=""):
    ...
    origin_func_name = "foreach_atan"
    ascendc_src_dir_ex = "foreach_atan"
    ascendc_src_dir = "foreach_atan"
    ascendc_src_file = "foreach_atan.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator ForeachAtan, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "ForeachAtan"
...
def op_select_format(x_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_)
    result = check_op_cap("op_select_format", "ForeachAtan", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_)
    result = check_op_cap("get_op_specific_info", "ForeachAtan", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
