...
class LRNBase4HD(LRNBase):
        ...
        if self.one_column_size < self.alignment_standards:
            ...
            if self._check_c_axis_too_large():
                ...
                if (hw_num_first_core % self.alignment_standards) != 0:
                    ...
                    real_first_core = hw_num_first_core
    ...
    def _get_target_core_num_batch_one(self):
        ...
        if segment_num <= self.device_aicore_num:
            ...
            _total = segment_num * self.alignment_standards
            hw_num_each_core = _align(_total // core_num, self.alignment_standards)
            core_num = _total // hw_num_each_core
            if _total % hw_num_each_core != 0:
                core_num += 1
            hw_num_first_core = self.one_column_size % hw_num_each_core
            if hw_num_first_core == 0:
                hw_num_first_core = hw_num_each_core
            if hw_num_first_core < self.alignment_standards:
                # reback old process
                core_num = self.device_aicore_num
