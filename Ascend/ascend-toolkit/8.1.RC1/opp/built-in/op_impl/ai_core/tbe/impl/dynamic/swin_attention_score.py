...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['query', 'key', 'value', 'padding_mask1', 'padding_mask2', 'scale', 'drop_mask']
    output_names = ['attention_score', 'softmax']
...
def _build_args(query_in__, key_in__, value_in__, padding_mask1_in__, padding_mask2_in__, scale_in__, drop_mask_in__, attention_score_out_, softmax_out_, keep_prob, query_transpose, key_transpose, bmm_score_transpose_a, bmm_score_transpose_b, softmax_axes):
    __inputs__ = []
    for arg in [query_in__, key_in__, value_in__, padding_mask1_in__, padding_mask2_in__, scale_in__, drop_mask_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [attention_score_out_, softmax_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if key_transpose != None:
        ...
        __attrs__.append(attr)
    if bmm_score_transpose_a != None:
        attr = {}
        attr["name"] = "bmm_score_transpose_a"
        attr["dtype"] = "bool"
        attr["value"] = bmm_score_transpose_a
        __attrs__.append(attr)
    if bmm_score_transpose_b != None:
        attr = {}
        attr["name"] = "bmm_score_transpose_b"
        attr["dtype"] = "bool"
        attr["value"] = bmm_score_transpose_b
        __attrs__.append(attr)
...
@tbe_register.register_operator("SwinAttentionScore", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_LIST_INT, para_check.KERNEL_NAME)
def swin_attention_score(query_in__, key_in__, value_in__, padding_mask1_in__=None, padding_mask2_in__=None, scale_in__=None, drop_mask_in__=None, attention_score_out_=None, softmax_out_=None, keep_prob=1, query_transpose=False, key_transpose=False, bmm_score_transpose_a=False, bmm_score_transpose_b=False, softmax_axes=[], kernel_name="swin_attention_score", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(query_in__, key_in__, value_in__, padding_mask1_in__, padding_mask2_in__, scale_in__, drop_mask_in__, attention_score_out_, softmax_out_, keep_prob, query_transpose, key_transpose, bmm_score_transpose_a, bmm_score_transpose_b, softmax_axes)
    ...
    origin_func_name = "swin_attention_score"
    ascendc_src_dir_ex = "swin_attention_score"
    ascendc_src_dir = "swin_attention_score"
    ascendc_src_file = "swin_attention_score.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator SwinAttentionScore, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "SwinAttentionScore"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[query_in__, key_in__, value_in__, padding_mask1_in__, padding_mask2_in__, scale_in__, drop_mask_in__], origin_outputs = [attention_score_out_, softmax_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'optional', 'optional', 'required', 'optional', 'required', 'required'], init_value_list = [None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(query_in__, key_in__, value_in__, padding_mask1_in__=None, padding_mask2_in__=None, scale_in__=None, drop_mask_in__=None, attention_score_out_=None, softmax_out_=None, keep_prob=1, query_transpose=False, key_transpose=False, bmm_score_transpose_a=False, bmm_score_transpose_b=False, softmax_axes=[], impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(query_in__, key_in__, value_in__, padding_mask1_in__, padding_mask2_in__, scale_in__, drop_mask_in__, attention_score_out_, softmax_out_, keep_prob, query_transpose, key_transpose, bmm_score_transpose_a, bmm_score_transpose_b, softmax_axes)
    result = check_op_cap("op_select_format", "SwinAttentionScore", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(query_in__, key_in__, value_in__, padding_mask1_in__=None, padding_mask2_in__=None, scale_in__=None, drop_mask_in__=None, attention_score_out_=None, softmax_out_=None, keep_prob=1, query_transpose=False, key_transpose=False, bmm_score_transpose_a=False, bmm_score_transpose_b=False, softmax_axes=[], impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(query_in__, key_in__, value_in__, padding_mask1_in__, padding_mask2_in__, scale_in__, drop_mask_in__, attention_score_out_, softmax_out_, keep_prob, query_transpose, key_transpose, bmm_score_transpose_a, bmm_score_transpose_b, softmax_axes)
    result = check_op_cap("get_op_specific_info", "SwinAttentionScore", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
