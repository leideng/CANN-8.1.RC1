...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['permuted_tokens', 'unpermuted_tokens_grad', 'sorted_indices', 'probs']
    output_names = ['permuted_tokens_grad', 'probs_grad']
...
def _build_args(permuted_tokens_in__, unpermuted_tokens_grad_in__, sorted_indices_in__, probs_in__, permuted_tokens_grad_out_, probs_grad_out_, padded_mode, restore_shape):
    __inputs__ = []
    for arg in [permuted_tokens_in__, unpermuted_tokens_grad_in__, sorted_indices_in__, probs_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [permuted_tokens_grad_out_, probs_grad_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if padded_mode != None:
        attr = {}
        attr["name"] = "padded_mode"
        attr["dtype"] = "bool"
        attr["value"] = padded_mode
        __attrs__.append(attr)
    if restore_shape != None:
        attr = {}
        attr["name"] = "restore_shape"
        attr["dtype"] = "list_int"
        attr["value"] = restore_shape
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("MoeTokenUnpermuteGrad", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_LIST_INT, para_check.KERNEL_NAME)
def moe_token_unpermute_grad(permuted_tokens_in__, unpermuted_tokens_grad_in__, sorted_indices_in__, probs_in__=None, permuted_tokens_grad_out_=None, probs_grad_out_=None, padded_mode=False, restore_shape=[1,1], kernel_name="moe_token_unpermute_grad", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(permuted_tokens_in__, unpermuted_tokens_grad_in__, sorted_indices_in__, probs_in__, permuted_tokens_grad_out_, probs_grad_out_, padded_mode, restore_shape)
    ...
    origin_func_name = "moe_token_unpermute_grad"
    ascendc_src_dir_ex = "moe_token_unpermute_grad"
    ascendc_src_dir = "moe_token_unpermute_grad"
    ascendc_src_file = "moe_token_unpermute_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MoeTokenUnpermuteGrad, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "MoeTokenUnpermuteGrad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[permuted_tokens_in__, unpermuted_tokens_grad_in__, sorted_indices_in__, probs_in__], origin_outputs = [permuted_tokens_grad_out_, probs_grad_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'optional', 'required', 'required'], init_value_list = [None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(permuted_tokens_in__, unpermuted_tokens_grad_in__, sorted_indices_in__, probs_in__=None, permuted_tokens_grad_out_=None, probs_grad_out_=None, padded_mode=False, restore_shape=[1,1], impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(permuted_tokens_in__, unpermuted_tokens_grad_in__, sorted_indices_in__, probs_in__, permuted_tokens_grad_out_, probs_grad_out_, padded_mode, restore_shape)
    result = check_op_cap("op_select_format", "MoeTokenUnpermuteGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(permuted_tokens_in__, unpermuted_tokens_grad_in__, sorted_indices_in__, probs_in__=None, permuted_tokens_grad_out_=None, probs_grad_out_=None, padded_mode=False, restore_shape=[1,1], impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(permuted_tokens_in__, unpermuted_tokens_grad_in__, sorted_indices_in__, probs_in__, permuted_tokens_grad_out_, probs_grad_out_, padded_mode, restore_shape)
    result = check_op_cap("get_op_specific_info", "MoeTokenUnpermuteGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
