...
@tbe_register.register_operator("Conv3DBackpropInputV2", trans_bool_to_s8=False)
...
def conv3d_backprop_input_v2(input_size_in__, filter_in__, out_backprop_in__, y_out_, strides, pads, dilations=[1,1,1,1,1], groups=1, data_format="NDHWC", kernel_name="conv3d_backprop_input_v2", impl_mode=""):
    ...
    origin_func_name = "conv3d_backprop_input_v2"
    ascendc_src_dir_ex = "conv3_d_backprop_input_v2"
    ascendc_src_dir = "conv3_d_backprop_input_v2"
    ascendc_src_file = "conv3d_backprop_input_v2.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator Conv3DBackpropInputV2, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
...
def op_select_format(input_size_in__, filter_in__, out_backprop_in__, y_out_, strides, pads, dilations=[1,1,1,1,1], groups=1, data_format="NDHWC", impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(input_size_in__, filter_in__, out_backprop_in__, y_out_, strides, pads, dilations, groups, data_format)
    result = check_op_cap("op_select_format", "Conv3DBackpropInputV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(input_size_in__, filter_in__, out_backprop_in__, y_out_, strides, pads, dilations=[1,1,1,1,1], groups=1, data_format="NDHWC", impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(input_size_in__, filter_in__, out_backprop_in__, y_out_, strides, pads, dilations, groups, data_format)
    result = check_op_cap("get_op_specific_info", "Conv3DBackpropInputV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
