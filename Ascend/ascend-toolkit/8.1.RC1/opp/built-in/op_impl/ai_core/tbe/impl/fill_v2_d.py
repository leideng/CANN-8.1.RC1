...
def elewise_binary_phony_ex(stmt_op):
    ...
    """
    ins, outs, _, _ = platform_info.get_dma_buffer(stmt_op)
    ...
    for dim in ins[0].shape:
        if total_element == 0:
            ...
            total_element *= dim
    _block_unit_size = ALIGNMENT_BYTES // platform_info.get_align_factor(dtype)[1]
...
@register_operator_compute("fill_v2_d", op_mode="static", support_fusion=True)
