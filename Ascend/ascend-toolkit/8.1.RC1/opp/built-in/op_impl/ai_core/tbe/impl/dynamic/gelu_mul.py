...
def _build_args(input_in__, output_out_, approximate):
    __inputs__ = []
...
@tbe_register.register_operator("GeluMul", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_STR, para_check.KERNEL_NAME)
def gelu_mul(input_in__, output_out_, approximate="none", kernel_name="gelu_mul", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(input_in__, output_out_, approximate)
    ...
    origin_func_name = "gelu_mul"
    ascendc_src_dir_ex = "gelu_mul"
    ascendc_src_dir = "gelu_mul"
    ascendc_src_file = "gelu_mul.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator GeluMul, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "GeluMul"
...
def op_select_format(input_in__, output_out_, approximate="none", impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(input_in__, output_out_, approximate)
    result = check_op_cap("op_select_format", "GeluMul", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(input_in__, output_out_, approximate="none", impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(input_in__, output_out_, approximate)
    result = check_op_cap("get_op_specific_info", "GeluMul", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
