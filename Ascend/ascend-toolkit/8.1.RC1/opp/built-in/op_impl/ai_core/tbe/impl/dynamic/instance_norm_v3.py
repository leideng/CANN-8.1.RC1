...
def _build_args(x_in__, gamma_in__, beta_in__, y_out_, mean_out_, variance_out_, data_format, epsilon):
    __inputs__ = []
    for arg in [x_in__, gamma_in__, beta_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [y_out_, mean_out_, variance_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("InstanceNormV3", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def instance_norm_v3(x_in__, gamma_in__, beta_in__, y_out_, mean_out_, variance_out_, data_format="NDHWC", epsilon=1e-06, kernel_name="instance_norm_v3", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, gamma_in__, beta_in__, y_out_, mean_out_, variance_out_, data_format, epsilon)
    ...
    origin_func_name = "instance_norm_v3"
    ascendc_src_dir_ex = "instance_norm_v3"
    ascendc_src_dir = "instance_norm_v3"
    ascendc_src_file = "instance_norm_v3.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator InstanceNormV3, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "InstanceNormV3"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, gamma_in__, beta_in__], origin_outputs = [y_out_, mean_out_, variance_out_],\
