...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['input_ids', 'cum_offsets_now', 'token_num', 'seq_len']
    output_names = ['x_remove_padding', 'cum_offsets_out', 'padding_offset']
...
def _build_args(input_ids_in__, cum_offsets_now_in__, token_num_in__, seq_len_in__, x_remove_padding_out_, cum_offsets_out_out_, padding_offset_out_):
    __inputs__ = []
    for arg in [input_ids_in__, cum_offsets_now_in__, token_num_in__, seq_len_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [x_remove_padding_out_, cum_offsets_out_out_, padding_offset_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("GetPaddingOffset", trans_bool_to_s8=False)
...
def get_padding_offset(input_ids_in__, cum_offsets_now_in__, token_num_in__, seq_len_in__, x_remove_padding_out_, cum_offsets_out_out_, padding_offset_out_, kernel_name="get_padding_offset", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(input_ids_in__, cum_offsets_now_in__, token_num_in__, seq_len_in__, x_remove_padding_out_, cum_offsets_out_out_, padding_offset_out_)
    ...
    origin_func_name = "get_padding_offset"
    ascendc_src_dir_ex = "get_padding_offset"
    ascendc_src_dir = "get_padding_offset"
    ascendc_src_file = "get_padding_offset.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator GetPaddingOffset, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "GetPaddingOffset"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[input_ids_in__, cum_offsets_now_in__, token_num_in__, seq_len_in__], origin_outputs = [x_remove_padding_out_, cum_offsets_out_out_, padding_offset_out_],\
...
def op_select_format(input_ids_in__, cum_offsets_now_in__, token_num_in__, seq_len_in__, x_remove_padding_out_, cum_offsets_out_out_, padding_offset_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(input_ids_in__, cum_offsets_now_in__, token_num_in__, seq_len_in__, x_remove_padding_out_, cum_offsets_out_out_, padding_offset_out_)
    result = check_op_cap("op_select_format", "GetPaddingOffset", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(input_ids_in__, cum_offsets_now_in__, token_num_in__, seq_len_in__, x_remove_padding_out_, cum_offsets_out_out_, padding_offset_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(input_ids_in__, cum_offsets_now_in__, token_num_in__, seq_len_in__, x_remove_padding_out_, cum_offsets_out_out_, padding_offset_out_)
    result = check_op_cap("get_op_specific_info", "GetPaddingOffset", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
