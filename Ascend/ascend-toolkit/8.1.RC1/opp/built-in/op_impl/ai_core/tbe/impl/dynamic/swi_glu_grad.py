...
def _build_args(y_grad_in__, x_in__, x_grad_out_, dim):
    __inputs__ = []
    for arg in [y_grad_in__, x_in__]:
        if arg != None:
...
@tbe_register.register_operator("SwiGluGrad", trans_bool_to_s8=False)
...
def swi_glu_grad(y_grad_in__, x_in__, x_grad_out_, dim=-1, kernel_name="swi_glu_grad", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(y_grad_in__, x_in__, x_grad_out_, dim)
    ...
    origin_func_name = "swi_glu_grad"
    ascendc_src_dir_ex = "swi_glu_grad"
    ascendc_src_dir = "swi_glu_grad"
    ascendc_src_file = "swi_glu_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator SwiGluGrad, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "SwiGluGrad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[y_grad_in__, x_in__], origin_outputs = [x_grad_out_],\
...
def op_select_format(y_grad_in__, x_in__, x_grad_out_, dim=-1, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(y_grad_in__, x_in__, x_grad_out_, dim)
    result = check_op_cap("op_select_format", "SwiGluGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(y_grad_in__, x_in__, x_grad_out_, dim=-1, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(y_grad_in__, x_in__, x_grad_out_, dim)
    result = check_op_cap("get_op_specific_info", "SwiGluGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
