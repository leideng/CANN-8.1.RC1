...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['tokens', 'expert_token_num_per_rank', 'per_token_scales']
    output_names = ['permute_tokens', 'permute_per_token_scales', 'permute_token_idx', 'expert_token_num']
...
def _build_args(tokens_in__, expert_token_num_per_rank_in__, per_token_scales_in__, permute_tokens_out_, permute_per_token_scales_out_, permute_token_idx_out_, expert_token_num_out_, expert_token_num_type, idx_type):
    __inputs__ = []
    for arg in [tokens_in__, expert_token_num_per_rank_in__, per_token_scales_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [permute_tokens_out_, permute_per_token_scales_out_, permute_token_idx_out_, expert_token_num_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
            ...
            __outputs__.append(arg)
    __attrs__ = []
    if expert_token_num_type != None:
        attr = {}
        attr["name"] = "expert_token_num_type"
        attr["dtype"] = "int"
        attr["value"] = expert_token_num_type
...
@tbe_register.register_operator("MoeReRouting", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def moe_re_routing(tokens_in__, expert_token_num_per_rank_in__, per_token_scales_in__=None, permute_tokens_out_=None, permute_per_token_scales_out_=None, permute_token_idx_out_=None, expert_token_num_out_=None, expert_token_num_type=1, idx_type=0, kernel_name="moe_re_routing", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(tokens_in__, expert_token_num_per_rank_in__, per_token_scales_in__, permute_tokens_out_, permute_per_token_scales_out_, permute_token_idx_out_, expert_token_num_out_, expert_token_num_type, idx_type)
    ...
    origin_func_name = "moe_re_routing"
    ascendc_src_dir_ex = "moe_re_routing"
    ascendc_src_dir = "moe_re_routing"
    ascendc_src_file = "moe_re_routing.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MoeReRouting, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "MoeReRouting"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[tokens_in__, expert_token_num_per_rank_in__, per_token_scales_in__], origin_outputs = [permute_tokens_out_, permute_per_token_scales_out_, permute_token_idx_out_, expert_token_num_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'optional', 'required', 'required', 'required', 'required'], init_value_list = [None, None, None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(tokens_in__, expert_token_num_per_rank_in__, per_token_scales_in__=None, permute_tokens_out_=None, permute_per_token_scales_out_=None, permute_token_idx_out_=None, expert_token_num_out_=None, expert_token_num_type=1, idx_type=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(tokens_in__, expert_token_num_per_rank_in__, per_token_scales_in__, permute_tokens_out_, permute_per_token_scales_out_, permute_token_idx_out_, expert_token_num_out_, expert_token_num_type, idx_type)
    result = check_op_cap("op_select_format", "MoeReRouting", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(tokens_in__, expert_token_num_per_rank_in__, per_token_scales_in__=None, permute_tokens_out_=None, permute_per_token_scales_out_=None, permute_token_idx_out_=None, expert_token_num_out_=None, expert_token_num_type=1, idx_type=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(tokens_in__, expert_token_num_per_rank_in__, per_token_scales_in__, permute_tokens_out_, permute_per_token_scales_out_, permute_token_idx_out_, expert_token_num_out_, expert_token_num_type, idx_type)
    result = check_op_cap("get_op_specific_info", "MoeReRouting", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
