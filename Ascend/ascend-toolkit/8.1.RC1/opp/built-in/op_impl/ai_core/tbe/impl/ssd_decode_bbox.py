...
class SSDDectionParamInit():
    ...
    def __init__(self, input_dict):
        ...
        self.loc_coord = input_dict.get("mbox_loc").get("shape")[1] // Constant.FOUR_DIRECTION
        ...
        # data for cnms
        self.max_total_size = input_dict.get("mbox_conf").get("shape")[1]  # N*Num_classes
        self.max_size_per_class = self.loc_num // Constant.FOUR_DIRECTION  # N
        self.iou_thresh = input_dict.get("nms_threshold")
        self.score_thresh = input_dict.get("confidence_threshold")
...
class SSDDecodeBBox(SSDDectionParamInit):
    ...
    def __init__(self, input_dict, tik_instance):
        ...
        mbox_conf_len = self.get_shape_total_number(input_dict.get("mbox_conf").get("shape"))
        ...
        # decode bbox out for cnms
        self.decode_bbox_cnms_gm = self.instance.Tensor(
            self.dtype,
            (self.batch, Constant.FOUR_DIRECTION, loc_coord_num_align),
            name="decode_bbox_cnms_gm",
            is_workspace=True,
            scope=tbe_platform.scope_gm)
    ...
    def compute_detection_out(self, batch):
        ...
        with self.instance.new_stmt_scope():
            ...
            if self.handle_tail_actual_num > 0:
                ...
                data_offset.set_as(data_tail_offet)
                self.comput_decoced_bbox(batch, data_offset, True)
    ...
    def comput_decoced_bbox(self, batch, data_offset, is_tail):
        ...
        if tbe_platform.api_check_support("tik.vcopy") or self.ascend_name in (
            tbe_platform.ASCEND_610, tbe_platform.cce_params.BS9SX1A, tbe_platform.ASCEND_310P):
        ...
        if tbe_platform.api_check_support("tik.vcopy"):
            self.gen_decode_bbox_output_without_rpn(batch, decode_bbox_clip, data_offset, is_tail)
        else:
    ...
    def gen_decode_bbox_output_without_rpn(self, batch, decode_bbox_ori, data_offset, is_tail):
        """
        general parse data for cnms without RNP
        :param batch:
        :param decode_bbox_ori:
        :param data_offset:
        :param is_tail:
        :return:
        """
        if is_tail is True:
            actual_num = self.handle_tail_actual_num // 4
            burst_len = math.ceil(actual_num / Constant.BLOCK_ELE)
        else:
            burst_len = math.ceil(self.handle_each_dst / Constant.BLOCK_ELE)
        with self.instance.if_scope(burst_len > 0):
            dst_off = data_offset // Constant.FOUR_DIRECTION
            self.instance.data_move(self.decode_bbox_cnms_gm[0, 0, dst_off], decode_bbox_ori[0, 0],
                                    0, 1, burst_len, 0, 0)
            self.instance.data_move(self.decode_bbox_cnms_gm[0, 1, dst_off], decode_bbox_ori[1, 0],
                                    0, 1, burst_len, 0, 0)
            self.instance.data_move(self.decode_bbox_cnms_gm[0, 2, dst_off], decode_bbox_ori[2, 0],
                                    0, 1, burst_len, 0, 0)
            self.instance.data_move(self.decode_bbox_cnms_gm[0, 3, dst_off], decode_bbox_ori[3, 0],
                                    0, 1, burst_len, 0, 0)
    ...
    def parser_loc_data_v200(self, batch):
        ...
        with self.instance.new_stmt_scope():
            ...
            with self.instance.for_range(0, handle_loops) as loc_handle_index:
                ...
                self.instance.vreduce(4 * handle_num, loc_dst_ub, loc_ub,
    ...
    def parser_prior_bbox_data_v200(self, batch):
        ...
        with self.instance.new_stmt_scope():
            ...
            with self.instance.for_range(0, handle_loops) as loc_handle_index:
                ...
                self.instance.vreduce(4 * handle_num, prior_bbox_dst_ub,
    ...
    def parser_prior_var_data_v200(self, batch):
        ...
        with self.instance.new_stmt_scope():
            ...
            with self.instance.for_range(0, handle_loops) as loc_handle_index:
                ...
                self.instance.vreduce(4 * handle_num, prior_var_dst_ub, prior_var_ub,
