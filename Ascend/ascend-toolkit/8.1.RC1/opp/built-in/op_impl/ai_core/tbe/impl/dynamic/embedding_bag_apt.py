...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['weight', 'indices', 'offsets', 'per_sample_weight']
    output_names = ['y', 'offset2bag', 'bag_size', 'max_indices']
...
def _build_args(weight_in__, indices_in__, offsets_in__, per_sample_weight_in__, y_out_, offset2bag_out_, bag_size_out_, max_indices_out_, mode, scale_grad_by_freq, sparse, include_last_offset, padding_idx):
    __inputs__ = []
    for arg in [weight_in__, indices_in__, offsets_in__, per_sample_weight_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [y_out_, offset2bag_out_, bag_size_out_, max_indices_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if scale_grad_by_freq != None:
        attr = {}
        attr["name"] = "scale_grad_by_freq"
        attr["dtype"] = "bool"
        attr["value"] = scale_grad_by_freq
        __attrs__.append(attr)
    if sparse != None:
        attr = {}
        attr["name"] = "sparse"
        ...
        attr["value"] = sparse
        __attrs__.append(attr)
    if include_last_offset != None:
        attr = {}
        attr["name"] = "include_last_offset"
        attr["dtype"] = "bool"
        attr["value"] = include_last_offset
        __attrs__.append(attr)
    if padding_idx != None:
        attr = {}
        attr["name"] = "padding_idx"
        attr["dtype"] = "int"
        attr["value"] = padding_idx
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("EmbeddingBag", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def embedding_bag(weight_in__, indices_in__, offsets_in__, per_sample_weight_in__=None, y_out_=None, offset2bag_out_=None, bag_size_out_=None, max_indices_out_=None, mode="mean", scale_grad_by_freq=False, sparse=False, include_last_offset=False, padding_idx=-1, kernel_name="embedding_bag", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(weight_in__, indices_in__, offsets_in__, per_sample_weight_in__, y_out_, offset2bag_out_, bag_size_out_, max_indices_out_, mode, scale_grad_by_freq, sparse, include_last_offset, padding_idx)
    ...
    origin_func_name = "embedding_bag"
    ascendc_src_dir_ex = "embedding_bag"
    ascendc_src_dir = "embedding_bag"
    ascendc_src_file = "embedding_bag_apt.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator EmbeddingBag, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "EmbeddingBag"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[weight_in__, indices_in__, offsets_in__, per_sample_weight_in__], origin_outputs = [y_out_, offset2bag_out_, bag_size_out_, max_indices_out_],\
