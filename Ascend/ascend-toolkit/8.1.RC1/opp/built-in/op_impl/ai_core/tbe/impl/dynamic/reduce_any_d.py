...
def reduce_any_d_compute(x, y, axes=None, keepdims=None, kernel_name="reduce_any_d"):
    ...
    reduce_any_support = tbe_platform.api_check_support('tbe.dsl.reduce_any', dtype)
    if zero_tensor_flg and reduce_any_support:
        res = tbe.reduce_any(x, axes, keepdims)
        return res
...
def reduce_any_d(x, y, axes=None, keep_dims=None, kernel_name="reduce_any_d"):
    """
    ...
    for (x, axes) in ins:
        with tbe.compute():
            ...
            res = reduce_any_d_compute(data_input, y, axes.get("value"), keep_dims, kernel_name)
