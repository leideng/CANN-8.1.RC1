...
@tbe_register.register_operator("ForeachDivScalar", trans_bool_to_s8=False)
...
def foreach_div_scalar(x_in__, scalar_in__, y_out_, kernel_name="foreach_div_scalar", impl_mode=""):
    ...
    origin_func_name = "foreach_div_scalar"
    ascendc_src_dir_ex = "foreach_div_scalar"
    ascendc_src_dir = "foreach_div_scalar"
    ascendc_src_file = "foreach_div_scalar.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator ForeachDivScalar, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "ForeachDivScalar"
...
def op_select_format(x_in__, scalar_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, scalar_in__, y_out_)
    result = check_op_cap("op_select_format", "ForeachDivScalar", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, scalar_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, scalar_in__, y_out_)
    result = check_op_cap("get_op_specific_info", "ForeachDivScalar", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
