...
class Scatter:
    ...
    def __init__(self, var, indices, updates, var_out, nd_flag, kernel_name,
        ...
        self.ub_size_bytes = (
                platform_info.get_soc_spec(
                    tbe_platform.UB_SIZE) - 8192)
        self.var_dtype_bytes_size = platform_info.get_bit_len(
            self.var_dtype) // 8
        self.indices_dtype_bytes_size = platform_info.get_bit_len(
    ...
    def init_ub_tensor_para(self):
        ...
        if self.var_dtype in need_vconv_dtype:
            vconv_dtype_bytes_size = platform_info.get_bit_len(
