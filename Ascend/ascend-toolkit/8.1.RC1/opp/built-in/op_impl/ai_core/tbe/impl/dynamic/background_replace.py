...
def _build_args(bkg_in__, src_in__, mask_in__, out_out_):
    __inputs__ = []
    for arg in [bkg_in__, src_in__, mask_in__]:
        if arg != None:
...
@tbe_register.register_operator("BackgroundReplace", trans_bool_to_s8=False)
...
def background_replace(bkg_in__, src_in__, mask_in__, out_out_, kernel_name="background_replace", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(bkg_in__, src_in__, mask_in__, out_out_)
    ...
    origin_func_name = "background_replace"
    ascendc_src_dir_ex = "background_replace"
    ascendc_src_dir = "background_replace"
    ascendc_src_file = "background_replace.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator BackgroundReplace, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "BackgroundReplace"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[bkg_in__, src_in__, mask_in__], origin_outputs = [out_out_],\
...
def op_select_format(bkg_in__, src_in__, mask_in__, out_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(bkg_in__, src_in__, mask_in__, out_out_)
    result = check_op_cap("op_select_format", "BackgroundReplace", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(bkg_in__, src_in__, mask_in__, out_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(bkg_in__, src_in__, mask_in__, out_out_)
    result = check_op_cap("get_op_specific_info", "BackgroundReplace", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
