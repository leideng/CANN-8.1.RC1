...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'weight', 'bias', 'scale', 'offset', 'antiquant_scale', 'antiquant_offset', 'group_list', 'per_token_scale']
    output_names = ['y']
...
def _build_args(x_in__, weight_in__, bias_in__, scale_in__, offset_in__, antiquant_scale_in__, antiquant_offset_in__, group_list_in__, per_token_scale_in__, y_out_, split_item, dtype, transpose_weight, transpose_x, group_type, group_list_type, act_type):
    __inputs__ = []
    for arg in [x_in__, weight_in__, bias_in__, scale_in__, offset_in__, antiquant_scale_in__, antiquant_offset_in__, group_list_in__, per_token_scale_in__]:
        if arg != None:
    ...
    if split_item != None:
        attr = {}
        attr["name"] = "split_item"
        attr["dtype"] = "int"
        attr["value"] = split_item
    ...
    if dtype != None:
        ...
        attr["value"] = dtype
        __attrs__.append(attr)
    if transpose_weight != None:
        attr = {}
        attr["name"] = "transpose_weight"
        attr["dtype"] = "bool"
        attr["value"] = transpose_weight
        __attrs__.append(attr)
    if transpose_x != None:
        attr = {}
...
@tbe_register.register_operator("GroupedMatmul", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.DYNAMIC_INPUT, para_check.DYNAMIC_INPUT, para_check.DYNAMIC_INPUT, para_check.DYNAMIC_INPUT, para_check.DYNAMIC_INPUT, para_check.DYNAMIC_INPUT, para_check.DYNAMIC_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.DYNAMIC_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def grouped_matmul(x_in__, weight_in__, bias_in__, scale_in__, offset_in__, antiquant_scale_in__, antiquant_offset_in__, group_list_in__=None, per_token_scale_in__=None, y_out_=None, split_item=0, dtype=0, transpose_weight=False, transpose_x=False, group_type=-1, group_list_type=0, act_type=0, kernel_name="grouped_matmul", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, bias_in__, scale_in__, offset_in__, antiquant_scale_in__, antiquant_offset_in__, group_list_in__, per_token_scale_in__, y_out_, split_item, dtype, transpose_weight, transpose_x, group_type, group_list_type, act_type)
    ...
    origin_func_name = "grouped_matmul"
    ascendc_src_dir_ex = "grouped_matmul"
    ascendc_src_dir = "grouped_matmul"
    ascendc_src_file = "grouped_matmul.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator GroupedMatmul, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, weight_in__, bias_in__, scale_in__, offset_in__, antiquant_scale_in__, antiquant_offset_in__, group_list_in__, per_token_scale_in__], origin_outputs = [y_out_],\
                param_type_dynamic = True, mc2_ctx = [], param_type_list = ['dynamic', 'dynamic', 'dynamic', 'dynamic', 'dynamic', 'dynamic', 'dynamic', 'optional', 'optional', 'dynamic'], init_value_list = [None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(x_in__, weight_in__, bias_in__, scale_in__, offset_in__, antiquant_scale_in__, antiquant_offset_in__, group_list_in__=None, per_token_scale_in__=None, y_out_=None, split_item=0, dtype=0, transpose_weight=False, transpose_x=False, group_type=-1, group_list_type=0, act_type=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, bias_in__, scale_in__, offset_in__, antiquant_scale_in__, antiquant_offset_in__, group_list_in__, per_token_scale_in__, y_out_, split_item, dtype, transpose_weight, transpose_x, group_type, group_list_type, act_type)
    result = check_op_cap("op_select_format", "GroupedMatmul", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, weight_in__, bias_in__, scale_in__, offset_in__, antiquant_scale_in__, antiquant_offset_in__, group_list_in__=None, per_token_scale_in__=None, y_out_=None, split_item=0, dtype=0, transpose_weight=False, transpose_x=False, group_type=-1, group_list_type=0, act_type=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, bias_in__, scale_in__, offset_in__, antiquant_scale_in__, antiquant_offset_in__, group_list_in__, per_token_scale_in__, y_out_, split_item, dtype, transpose_weight, transpose_x, group_type, group_list_type, act_type)
    result = check_op_cap("get_op_specific_info", "GroupedMatmul", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
