...
constexpr bool InputIsAbleToFreeAux() {
...
*/
template <typename FunList, int posInFuns, typename InArg>
constexpr bool IsConnectOutput() {
  if constexpr (posInFuns < FunList::Size) {
    using func = typename FunList::template At<posInFuns>;
    if constexpr (Vec::IsCopyOutOp<typename func::Fun>::Value &&
                  Placeholder::IsOutHolder<typename func::Args::template At<0>>::Value) {
...
/*
* 检测依赖节点中是否存在CopyInBrc或者VecBrc节点
* 模板参数：
*   1. Depends:   依赖列表
*   2. pos: 递归过程中，当前节点索引
*/
template <typename Depends, int pos = 0>
constexpr bool IsDependOnBrc() {
  if constexpr (pos < Depends::Size){
    using func = typename Depends::template At<pos>;
    if constexpr (Vec::IsCopyInBrcOp<typename func::Fun>::Value ||
                  Vec::IsVecBrcOp<typename func::Fun>::Value) {
      return true;
    }
    return IsDependOnBrc<Depends, pos + 1>();
  }
  return false;
}
...
constexpr int GetDependByVecBrcIdx() {
