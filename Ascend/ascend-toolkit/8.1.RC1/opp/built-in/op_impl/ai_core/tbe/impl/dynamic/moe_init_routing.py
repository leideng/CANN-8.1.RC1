...
def _build_args(x_in__, row_idx_in__, expert_idx_in__, expanded_x_out_, expanded_row_idx_out_, expanded_expert_idx_out_, active_num):
    __inputs__ = []
...
@tbe_register.register_operator("MoeInitRouting", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def moe_init_routing(x_in__, row_idx_in__, expert_idx_in__, expanded_x_out_, expanded_row_idx_out_, expanded_expert_idx_out_, active_num, kernel_name="moe_init_routing", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, row_idx_in__, expert_idx_in__, expanded_x_out_, expanded_row_idx_out_, expanded_expert_idx_out_, active_num)
    ...
    origin_func_name = "moe_init_routing"
    ascendc_src_dir_ex = "moe_init_routing"
    ascendc_src_dir = "moe_init_routing"
    ascendc_src_file = "moe_init_routing.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MoeInitRouting, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
...
def op_select_format(x_in__, row_idx_in__, expert_idx_in__, expanded_x_out_, expanded_row_idx_out_, expanded_expert_idx_out_, active_num, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, row_idx_in__, expert_idx_in__, expanded_x_out_, expanded_row_idx_out_, expanded_expert_idx_out_, active_num)
    result = check_op_cap("op_select_format", "MoeInitRouting", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, row_idx_in__, expert_idx_in__, expanded_x_out_, expanded_row_idx_out_, expanded_expert_idx_out_, active_num, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, row_idx_in__, expert_idx_in__, expanded_x_out_, expanded_row_idx_out_, expanded_expert_idx_out_, active_num)
    result = check_op_cap("get_op_specific_info", "MoeInitRouting", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
