...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['var', 'value', 'sorted_indices', 'pos', 'alpha']
    output_names = ['var']
...
def _build_args(var_in__, value_in__, sorted_indices_in__, pos_in__, alpha_in__, var_out_, axis):
    __inputs__ = []
    for arg in [var_in__, value_in__, sorted_indices_in__, pos_in__, alpha_in__]:
        if arg != None:
...
@tbe_register.register_operator("InplaceIndexAddWithSorted", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def inplace_index_add_with_sorted(var_in__, value_in__, sorted_indices_in__, pos_in__, alpha_in__=None, var_out_=None, axis=0, kernel_name="inplace_index_add_with_sorted", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(var_in__, value_in__, sorted_indices_in__, pos_in__, alpha_in__, var_out_, axis)
    ...
    origin_func_name = "inplace_index_add_with_sorted"
    ascendc_src_dir_ex = "inplace_index_add_with_sorted"
    ascendc_src_dir = "inplace_index_add_with_sorted"
    ascendc_src_file = "inplace_index_add_with_sorted.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator InplaceIndexAddWithSorted, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "InplaceIndexAddWithSorted"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[var_in__, value_in__, sorted_indices_in__, pos_in__, alpha_in__], origin_outputs = [var_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'optional', 'required'], init_value_list = [None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{"auto_sync": false}')

def op_select_format(var_in__, value_in__, sorted_indices_in__, pos_in__, alpha_in__=None, var_out_=None, axis=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(var_in__, value_in__, sorted_indices_in__, pos_in__, alpha_in__, var_out_, axis)
    result = check_op_cap("op_select_format", "InplaceIndexAddWithSorted", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(var_in__, value_in__, sorted_indices_in__, pos_in__, alpha_in__=None, var_out_=None, axis=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(var_in__, value_in__, sorted_indices_in__, pos_in__, alpha_in__, var_out_, axis)
    result = check_op_cap("get_op_specific_info", "InplaceIndexAddWithSorted", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
