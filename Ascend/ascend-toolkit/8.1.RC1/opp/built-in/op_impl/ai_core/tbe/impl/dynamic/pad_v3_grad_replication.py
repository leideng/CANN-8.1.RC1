...
def _build_args(x_in__, paddings_in__, z_out_):
    __inputs__ = []
...
@tbe_register.register_operator("PadV3GradReplication", trans_bool_to_s8=False)
...
def pad_v3_grad_replication(x_in__, paddings_in__, z_out_, kernel_name="pad_v3_grad_replication", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, paddings_in__, z_out_)
    ...
    origin_func_name = "pad_v3_grad_replication"
    ascendc_src_dir_ex = "pad_v3_grad_replication"
    ascendc_src_dir = "pad_v3_grad_replication"
    ascendc_src_file = "pad_v3_grad_replication.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator PadV3GradReplication, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "PadV3GradReplication"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, paddings_in__], origin_outputs = [z_out_],\
...
def op_select_format(x_in__, paddings_in__, z_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, paddings_in__, z_out_)
    result = check_op_cap("op_select_format", "PadV3GradReplication", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, paddings_in__, z_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, paddings_in__, z_out_)
    result = check_op_cap("get_op_specific_info", "PadV3GradReplication", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
