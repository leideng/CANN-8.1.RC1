...
def dynamic_lstm(input_x, weight, bias,
    ...
    bias_bc_ub = tbe.broadcast(bias_ub, shape_c)
    c_ub_bias = tbe.vadd(c_ub, bias_bc_ub)
    ...
    c_t_tmp1 = tbe.vmul(s_state_c, f_t_sigmoid)
    c_t_tmp2 = tbe.vmul(j_t_tanh, i_t_sigmoid)
    update_c = tbe.vadd(c_t_tmp1, c_t_tmp2)
    ...
    update_h = tbe.vmul(c_t_tanh, o_t_sigmoid)
    ...
    # set scope
    s[a_l1].set_scope(tbe_platform.scope_cbuf)
    s[b_l1].set_scope(tbe_platform.scope_cbuf)
    s[a_l0a].set_scope(tbe_platform.scope_ca)
    s[b_l0b].set_scope(tbe_platform.scope_cb)
    s[c_l0c].set_scope(tbe_platform.scope_cc)
    s[c_ub].set_scope(tbe_platform.scope_ubuf)
    s[s_init_h].set_scope(tbe_platform.scope_ubuf)
    s[bias_ub].set_scope(tbe_platform.scope_ubuf)
    s[bias_bc_ub].set_scope(tbe_platform.scope_ubuf)
    s[scan_h].set_scope(tbe_platform.scope_ubuf)
    s[scan_c].set_scope(tbe_platform.scope_ubuf)
    s[update_h_ub].set_scope(tbe_platform.scope_ubuf)
    s[update_c_ub].set_scope(tbe_platform.scope_ubuf)
    s[s_state_h_ub].set_scope(tbe_platform.scope_ubuf)
    s[s_state_c_ub].set_scope(tbe_platform.scope_ubuf)

    s[weight_ub].set_scope(tbe_platform.scope_ubuf)
    s[weight_fp16].set_scope(tbe_platform.scope_ubuf)
    s[a_ub].set_scope(tbe_platform.scope_ubuf)
    s[a_ub_fp16].set_scope(tbe_platform.scope_ubuf)
