...
def maximum_grad_compute(data_dz, data_x, data_x2, y1, y2, grad_x, grad_y, kernel_name="maximum_grad"):
    """
    algorithm:
    ...
    result_dx, result_dy = _calculate_result_ge(data_x, data_x2, data_dz, dtype, shape_max)
...
def maximum_grad(grads, x1, x2, y1, y2, grad_x=True, grad_y=True, kernel_name="maximum_grad"):
    ...
    for (grads_dict, x1_dict, x2_dict) in ins:
        with tbe.compute():
            ...
            res = maximum_grad_compute(data_dz, data_x, data_y, y1, y2, grad_x, grad_y, kernel_name)
