...
def check_supported(images, y, size, align_corners=False, half_pixel_centers=False, kernel_name="resize_bilinear_v2"):
    ...
    need_support_shape_list = [(1, 480, 640, 3), (64, 512, 1, 1), (64, 512, 2, 2), 
                               (64, 512, 3, 3), (64, 512, 6, 6), (64, 21, 63, 63), (64, 21, 500, 500)]
    soc_version = tbe_platform.get_soc_spec(tbe_platform.SHORT_SOC_VERSION)
    special_soc = "Ascend310P"
    reason = ""
    if image_shape not in need_support_shape_list and soc_version == special_soc:
        return False, reason
...
def _resize_bilinear_ir(inputs, outputs, align_corners, half_pixel_centers):
    ...
    """
    # get device version
    core_counts = tbe_platform.get_soc_spec("CORE_NUM")
    ...
    pad_mode = tvm.call_cce_pure_intrin("int32", "tvm_cce_string_print", "PAD_NONE")
    ...
    # `No.2 situation : input H/W = (1,1)`
        ...
        if dtype == "float16":
            ...
            if expand_loop > HW_SIZE_512:
                ...
                if tail_level1 > 0:
                    # Note:
                    #   1. tail_level1 larger than HW_SIZE_512 need move to L1 first
                    #   2. tail_level1 less than HW_SIZE_512 move to ubuf directly
    ...
    # No.4 situation: for mini_dc
        ...
        pipe_all_str = tvm.call_cce_pure_intrin("int32", "tvm_cce_string_print", "PIPE_ALL")
    ...
    # No.5 normal input/output H/W
        ...
        args_str_v = tvm.call_cce_pure_intrin("int32", "tvm_cce_string_print", "PIPE_V")
        args_str_v_all = tvm.call_cce_pure_intrin("int32", "tvm_cce_string_print", "PIPE_ALL")
        # use multi-core resource
        h_per_core = h_out // core_counts + (1 if h_out % core_counts > 0 else 0)
        is_same_percore = 0 if h_out % core_counts == 0 else 1
        core_num = h_out // h_per_core + is_same_percore
