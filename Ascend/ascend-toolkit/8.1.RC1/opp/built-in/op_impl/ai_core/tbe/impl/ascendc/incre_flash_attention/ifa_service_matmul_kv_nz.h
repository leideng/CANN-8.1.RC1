...
/*!
 * \file ifa_service_matmul_kv_nz.h
 * \brief
 */
#ifndef IFA_SERVICE_MATMUL_KV_NZ_H
#define IFA_SERVICE_MATMUL_KV_NZ_H
...
public:
    ...
    __aicore__ inline IfaMatmulKvNz() {};
...
protected:
    ...
    __aicore__ inline void LoadDataMm2A(LocalTensor<KV_T>& aL0Tensor, LocalTensor<KV_T>& aL1Tensor, uint32_t mSize, uint32_t startKL1, uint32_t kL0SizeAlign);
    ...
    __aicore__ inline void LoadDataMm2B(LocalTensor<KV_T> &bL0Tensor, LocalTensor<KV_T> &bL1Tensor, uint32_t bL1Offset,
                                        uint32_t nSize, uint32_t kSize, uint32_t kL1Size);
...
private:
    // L1 buffer size
    static constexpr uint32_t L1_KP_SIZE = (128 * (512 + 64) * sizeof(KV_T));
    static constexpr uint32_t L1_V_SIZE = (512 * 64 * sizeof(KV_T));
    ...
    uint32_t vL1BufIter = 0; // double buffer
    uint32_t qL1BufIter = 0; // single buffer
...
template <typename IFAT>
__aicore__ inline void IfaMatmulKvNz<IFAT>::InitParams(uint64_t qHeadNum, uint64_t kvHeadNum, uint64_t headDim,
...
template <typename IFAT>
__aicore__ inline void IfaMatmulKvNz<IFAT>::InitMm1GlobalTensor(GlobalTensor<Q_T> queryGm, GlobalTensor<Q_T> qRopeGm,
...
template <typename IFAT>
__aicore__ inline void IfaMatmulKvNz<IFAT>::InitMm2GlobalTensor(
...
template <typename IFAT>
__aicore__ inline void IfaMatmulKvNz<IFAT>::InitPageAttentionInfo(GlobalTensor<int32_t> blockTableGm,
...
template <typename IFAT>
__aicore__ inline void IfaMatmulKvNz<IFAT>::InitBuffers(TPipe *pipe)
{
    // L1
    pipe->InitBuffer(kpBufL1, L1_KP_SIZE * 2); // (128K + 16K) * 2
    kpL1Tensor = kpBufL1.Get<KV_T>();

    pipe->InitBuffer(valueBufL1, L1_V_SIZE); // 64K
    vL1Tensor = valueBufL1.Get<KV_T>();

    pipe->InitBuffer(queryBufL1, L1_Q_SIZE); // 128K + 16K
...
template <typename IFAT>
__aicore__ inline void IfaMatmulKvNz<IFAT>::UpdateKey(GlobalTensor<KV_T> keyGm)
{
    this->keyGm = keyGm;
}

template <typename IFAT>
__aicore__ inline void IfaMatmulKvNz<IFAT>::UpdateValue(GlobalTensor<KV_T> valueGm)
{
    this->valueGm = valueGm;
}

template <typename IFAT>
__aicore__ inline void IfaMatmulKvNz<IFAT>::AllocEventID()
{
...
template <typename IFAT>
__aicore__ inline void IfaMatmulKvNz<IFAT>::FreeEventID()
{
...
template <typename IFAT>
__aicore__ inline void IfaMatmulKvNz<IFAT>::CopyGmToL1(LocalTensor<KV_T>& l1Tensor, GlobalTensor<KV_T> &gmSrcTensor,
...
template <typename IFAT>
__aicore__ inline void IfaMatmulKvNz<IFAT>::CopyInMm1AToL1(LocalTensor<KV_T> &l1Tensor,
...
template <typename IFAT>
__aicore__ inline void IfaMatmulKvNz<IFAT>::CopyInMm1ARopeToL1(LocalTensor<KV_T> &l1Tensor,
                                                                                    const ExtraInfoMla &info)
{
...
template <typename IFAT>
__aicore__ inline void IfaMatmulKvNz<IFAT>::CopyInMm1BToL1(LocalTensor<KV_T>& bL1Tensor,
...
template <typename IFAT>
__aicore__ inline void IfaMatmulKvNz<IFAT>::CopyInMm1BRopeToL1(LocalTensor<KV_T>& bL1Tensor,
                                                                                    const ExtraInfoMla &info, uint32_t subNid, uint32_t subNSize)
{
...
template <typename IFAT>
__aicore__ inline void IfaMatmulKvNz<IFAT>::CopyInMm1BToL1ForPA(
...
template <typename IFAT>
__aicore__ inline void IfaMatmulKvNz<IFAT>::CopyInMm1BRopeToL1ForPA(
...
template <typename IFAT>
__aicore__ inline void IfaMatmulKvNz<IFAT>::LoadDataMm1A(LocalTensor<KV_T>& aL0Tensor,
...
template <typename IFAT>
__aicore__ inline void IfaMatmulKvNz<IFAT>::LoadDataMm1B(LocalTensor<KV_T> &l0Tensor,
...
template <typename IFAT>
__aicore__ inline void IfaMatmulKvNz<IFAT>::CopyInMm2AToL1(LocalTensor<KV_T> &aL1Tensor,
                                                                                const ExtraInfoMla &info)
{
    // 全量拷贝
...
template <typename IFAT>
__aicore__ inline void IfaMatmulKvNz<IFAT>::CopyInMm2BToL1(LocalTensor<KV_T> &bL1Tensor, const ExtraInfoMla &info,
...
template <typename IFAT>
__aicore__ inline void IfaMatmulKvNz<IFAT>::CopyInMm2BToL1ForPA(LocalTensor<KV_T>& bL1Tensor,
...
template <typename IFAT>
__aicore__ inline void IfaMatmulKvNz<IFAT>::LoadDataMm2A(LocalTensor<KV_T> &aL0Tensor, LocalTensor<KV_T> &aL1Tensor,
                                                         uint32_t mSize, uint32_t startKL1, uint32_t kL0SizeAlign)
{
    // 加载 64 * 128
    uint32_t mLoops = mSize / 16; // mSize需16对齐

    LocalTensor<KV_T> aL1SrcTensor = aL1Tensor[mSize * startKL1];
    ...
    for (uint32_t i = 0; i < mLoops; i++) {
        ...
        if constexpr (KVINT4) {
            loadData2DParams.repeatTimes = kL0SizeAlign / 64;
        } else {
            loadData2DParams.repeatTimes = kL0SizeAlign / (32 / sizeof(KV_T));
        }
        loadData2DParams.srcStride = mSize / 16;
        loadData2DParams.dstGap = 0;
        loadData2DParams.ifTranspose = false;
        ...
        LoadData(aL0Tensor[i * 16 * kL0SizeAlign], tmpSrcTensor, loadData2DParams);
    }
}

template <typename IFAT>
__aicore__ inline void IfaMatmulKvNz<IFAT>::LoadDataMm2B(LocalTensor<KV_T> &bL0Tensor,
...
template <typename IFAT>
__aicore__ inline void IfaMatmulKvNz<IFAT>::LoadDataMm2B(LocalTensor<KV_T> &bL0Tensor, LocalTensor<KV_T> &bL1Tensor,
                                                         uint32_t bL1Offset, uint32_t nSize, uint32_t kSize,
                                                         uint32_t kL1Size)
{
    // L1 128 * 256; L0 64 * 128
    // kSize为16对齐, nSize为C0对齐
    uint32_t kloops = kSize / 16;

    LocalTensor<KV_T> srcTensor = bL1Tensor[bL1Offset];
    for (uint32_t i = 0; i < kloops; i++) {
        LoadData2DParams loadData2DParams;
        loadData2DParams.startIndex = 0;
        if constexpr (KVINT4) {
        ...
        loadData2DParams.srcStride = kL1Size / 16;
        loadData2DParams.dstGap = 0;
        loadData2DParams.ifTranspose = true;
...
template <typename IFAT>
__aicore__ inline void IfaMatmulKvNz<IFAT>::ComputeMm1(const ExtraInfoMla &info)
{
    uint32_t mSizeAct = info.gSize * info.s1Size;
    uint32_t mSize = Align(mSizeAct, 16U);
    LocalTensor<Q_T> qTensor = qL1Tensor;
    ...
    for (uint32_t n = 0; n < nloops; n++) {
        ...
        LocalTensor<KV_T> kTensor = kpL1Tensor[(kpL1BufIter % 2) * (L1_KP_SIZE / sizeof(KV_T))];
...
template <typename IFAT>
__aicore__ inline void IfaMatmulKvNz<IFAT>::ComputeMm2(const ExtraInfoMla &info)
{
    uint32_t mSizeAct = info.gSize * info.s1Size;
    uint32_t mSize = Align(mSizeAct, 16U);
    uint32_t nSize = BlockAlign<KV_T>(headDim);

    LocalTensor<KV_T> pTensor = kpL1Tensor[(kpL1BufIter % 2) * L1_KP_SIZE / sizeof(KV_T)];
    ...
    for (uint32_t n = 0; n < nLoops; n++) {
        ...
        // 切K
        uint32_t kSplitSize = (mSize <= 64) ? 128 : 256;
        ...
        for (uint32_t k = 0; k < kloops; k++) { // 128 循环
            ...
            LocalTensor vTensor = vL1Tensor[(vL1BufIter % 2) * (L1_V_SIZE / sizeof(KV_T))];
            ...
            uint32_t baseK = 128;
            uint32_t baseN = 128;
            uint32_t kL0Loops = (subKSizeAct + baseK - 1) / baseK;
            uint32_t kL0SizeTail = subKSizeAct - (kL0Loops - 1) * baseK;
            for (uint32_t kL0Loop = 0, kL0SizeAct = baseK, kL0SizeAlign = baseK; kL0Loop < kL0Loops; kL0Loop++) {
                if (kL0Loop + 1 == kL0Loops) {
                    kL0SizeAct = kL0SizeTail;
                    kL0SizeAlign = Align(kL0SizeTail, 16U);
                }
                WaitFlag<HardEvent::M_MTE1>(L0A_EVENT0 + aL0BufIter % 2);
                LocalTensor<KV_T> aL0Tensor = aL0TensorPingPong[(aL0BufIter % 2) * (L0C_PP_SIZE / sizeof(MM_OUT_T))];

                LoadDataMm2A(aL0Tensor, pTensor, mSize, k * kSplitSize + kL0Loop * baseK, kL0SizeAlign);

                SetFlag<HardEvent::MTE1_M>(L0A_EVENT0 + aL0BufIter % 2);
                WaitFlag<HardEvent::MTE1_M>(L0A_EVENT0 + aL0BufIter % 2);

                uint32_t nL0Loops = nSplitSize / baseN; // 要求nSplitSize是baseN整数倍
                for (uint32_t i = 0; i < nL0Loops; i++) { // nSplitSize(256) 按照 128 切分两份，这里需要处理尾块
                    ...
                    LoadDataMm2B(bL0Tensor, vTensor, i * baseN * subKSize + kL0Loop * baseK * (32 / sizeof(KV_T)),
                                 baseN, kL0SizeAlign, subKSize);
                    ...
                    mmadParams.cmatrixInitVal = (k == 0) && (kL0Loop == 0);
                    mmadParams.cmatrixSource = false;

                    LocalTensor<MM_OUT_T> destL0C = cL0Tensor[mSize * baseN * i]; // 64 * 128
