...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['query', 'key', 'value', 'dy', 'pse_shift', 'drop_mask', 'padding_mask', 'atten_mask', 'softmax_max', 'softmax_sum', 'softmax_in', 'attention_in', 'prefix', 'actual_seq_qlen', 'actual_seq_kvlen', 'q_start_idx', 'kv_start_idx']
    output_names = ['dq', 'dk', 'dv', 'dpse']
...
def _build_args(query_in__, key_in__, value_in__, dy_in__, pse_shift_in__, drop_mask_in__, padding_mask_in__, atten_mask_in__, softmax_max_in__, softmax_sum_in__, softmax_in_in__, attention_in_in__, prefix_in__, actual_seq_qlen_in__, actual_seq_kvlen_in__, q_start_idx_in__, kv_start_idx_in__, dq_out_, dk_out_, dv_out_, dpse_out_, scale_value, keep_prob, pre_tockens, next_tockens, head_num, input_layout, inner_precise, sparse_mode, pse_type):
    __inputs__ = []
    for arg in [query_in__, key_in__, value_in__, dy_in__, pse_shift_in__, drop_mask_in__, padding_mask_in__, atten_mask_in__, softmax_max_in__, softmax_sum_in__, softmax_in_in__, attention_in_in__, prefix_in__, actual_seq_qlen_in__, actual_seq_kvlen_in__, q_start_idx_in__, kv_start_idx_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [dq_out_, dk_out_, dv_out_, dpse_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if pre_tockens != None:
        attr = {}
        attr["name"] = "pre_tockens"
        attr["dtype"] = "int"
        attr["value"] = pre_tockens
        __attrs__.append(attr)
    if next_tockens != None:
        attr = {}
        attr["name"] = "next_tockens"
        attr["dtype"] = "int"
        attr["value"] = next_tockens
        __attrs__.append(attr)
    if head_num != None:
        attr = {}
        attr["name"] = "head_num"
...
@tbe_register.register_operator("DetectFlashAttentionScoreGrad", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def detect_flash_attention_score_grad(query_in__, key_in__, value_in__, dy_in__, pse_shift_in__=None, drop_mask_in__=None, padding_mask_in__=None, atten_mask_in__=None, softmax_max_in__=None, softmax_sum_in__=None, softmax_in_in__=None, attention_in_in__=None, prefix_in__=None, actual_seq_qlen_in__=None, actual_seq_kvlen_in__=None, q_start_idx_in__=None, kv_start_idx_in__=None, dq_out_=None, dk_out_=None, dv_out_=None, dpse_out_=None, scale_value=1, keep_prob=1, pre_tockens=2147483647, next_tockens=2147483647, head_num=0, input_layout="", inner_precise=0, sparse_mode=0, pse_type=1, kernel_name="detect_flash_attention_score_grad", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(query_in__, key_in__, value_in__, dy_in__, pse_shift_in__, drop_mask_in__, padding_mask_in__, atten_mask_in__, softmax_max_in__, softmax_sum_in__, softmax_in_in__, attention_in_in__, prefix_in__, actual_seq_qlen_in__, actual_seq_kvlen_in__, q_start_idx_in__, kv_start_idx_in__, dq_out_, dk_out_, dv_out_, dpse_out_, scale_value, keep_prob, pre_tockens, next_tockens, head_num, input_layout, inner_precise, sparse_mode, pse_type)
    ...
    origin_func_name = "detect_flash_attention_score_grad"
    ascendc_src_dir_ex = "detect_flash_attention_score_grad"
    ascendc_src_dir = "detect_flash_attention_score_grad"
    ascendc_src_file = "detect_flash_attention_score_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator DetectFlashAttentionScoreGrad, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "DetectFlashAttentionScoreGrad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[query_in__, key_in__, value_in__, dy_in__, pse_shift_in__, drop_mask_in__, padding_mask_in__, atten_mask_in__, softmax_max_in__, softmax_sum_in__, softmax_in_in__, attention_in_in__, prefix_in__, actual_seq_qlen_in__, actual_seq_kvlen_in__, q_start_idx_in__, kv_start_idx_in__], origin_outputs = [dq_out_, dk_out_, dv_out_, dpse_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'optional', 'optional', 'optional', 'optional', 'optional', 'optional', 'optional', 'optional', 'optional', 'optional', 'optional', 'optional', 'optional', 'required', 'required', 'required', 'required'], init_value_list = [None, None, None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(query_in__, key_in__, value_in__, dy_in__, pse_shift_in__=None, drop_mask_in__=None, padding_mask_in__=None, atten_mask_in__=None, softmax_max_in__=None, softmax_sum_in__=None, softmax_in_in__=None, attention_in_in__=None, prefix_in__=None, actual_seq_qlen_in__=None, actual_seq_kvlen_in__=None, q_start_idx_in__=None, kv_start_idx_in__=None, dq_out_=None, dk_out_=None, dv_out_=None, dpse_out_=None, scale_value=1, keep_prob=1, pre_tockens=2147483647, next_tockens=2147483647, head_num=0, input_layout="", inner_precise=0, sparse_mode=0, pse_type=1, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(query_in__, key_in__, value_in__, dy_in__, pse_shift_in__, drop_mask_in__, padding_mask_in__, atten_mask_in__, softmax_max_in__, softmax_sum_in__, softmax_in_in__, attention_in_in__, prefix_in__, actual_seq_qlen_in__, actual_seq_kvlen_in__, q_start_idx_in__, kv_start_idx_in__, dq_out_, dk_out_, dv_out_, dpse_out_, scale_value, keep_prob, pre_tockens, next_tockens, head_num, input_layout, inner_precise, sparse_mode, pse_type)
    result = check_op_cap("op_select_format", "DetectFlashAttentionScoreGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(query_in__, key_in__, value_in__, dy_in__, pse_shift_in__=None, drop_mask_in__=None, padding_mask_in__=None, atten_mask_in__=None, softmax_max_in__=None, softmax_sum_in__=None, softmax_in_in__=None, attention_in_in__=None, prefix_in__=None, actual_seq_qlen_in__=None, actual_seq_kvlen_in__=None, q_start_idx_in__=None, kv_start_idx_in__=None, dq_out_=None, dk_out_=None, dv_out_=None, dpse_out_=None, scale_value=1, keep_prob=1, pre_tockens=2147483647, next_tockens=2147483647, head_num=0, input_layout="", inner_precise=0, sparse_mode=0, pse_type=1, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(query_in__, key_in__, value_in__, dy_in__, pse_shift_in__, drop_mask_in__, padding_mask_in__, atten_mask_in__, softmax_max_in__, softmax_sum_in__, softmax_in_in__, attention_in_in__, prefix_in__, actual_seq_qlen_in__, actual_seq_kvlen_in__, q_start_idx_in__, kv_start_idx_in__, dq_out_, dk_out_, dv_out_, dpse_out_, scale_value, keep_prob, pre_tockens, next_tockens, head_num, input_layout, inner_precise, sparse_mode, pse_type)
    result = check_op_cap("get_op_specific_info", "DetectFlashAttentionScoreGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
