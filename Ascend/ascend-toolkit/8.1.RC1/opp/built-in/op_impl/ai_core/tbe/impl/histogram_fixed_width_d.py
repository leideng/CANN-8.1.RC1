...
class IrParams:
    ...
    def __init__(self, ib_, dtype_list, shape_list, nbins):
        ...
        self.input_dtype_size = \
            get_bit_len(self.input_dtype) // \
            cce_params.VECTOR_INST_BLOCK_NUM
        self.output_dtype_size = \
            get_bit_len(self.output_dtype) // \
            cce_params.VECTOR_INST_BLOCK_NUM
        self.mid_dtype_size = \
            get_bit_len(self.mid_dtype) // \
            cce_params.VECTOR_INST_BLOCK_NUM
        ...
        self.compile_plat = tbe_platform.get_soc_spec("SHORT_SOC_VERSION")
        ...
        self.args_str = tvm.call_cce_pure_intrin("int32", "tvm_cce_string_print",
                                             "PIPE_ALL")
        self.deqscale = tvm.call_cce_pure_intrin("float16", "tvm_cce_string_print",
                                             "(half)1.000000e+00f")
    ...
    def get_mask_list(self, mask_list_num):
        ...
        None
        """
        with self.ir_builder.for_range(0, mask_list_num, kind="serial",
...
def _fuction_calcu_one_segment(calc_ub_info,
    ...
    def _do_cmp_calcu(repeat, cal_offset, nbins_index):
        if params.compile_plat in ("Ascend910", "Ascend610", "Ascend310P") \
                or tbe_platform.api_check_support("tik.vcopy"):
...
def _histogram_fixed_width_ir(dst, src, nbins, shape_list):
    ...
    """
    data, data_range = src
    ib_create = tvm.tir.ir_builder.create()
    # params init
