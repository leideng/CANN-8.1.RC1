...
def _build_args(x_in__, cos_in__, sin_in__, y_out_, mode):
    __inputs__ = []
    for arg in [x_in__, cos_in__, sin_in__]:
        if arg != None:
...
@tbe_register.register_operator("RotaryPositionEmbedding", trans_bool_to_s8=False)
...
def rotary_position_embedding(x_in__, cos_in__, sin_in__, y_out_, mode=0, kernel_name="rotary_position_embedding", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, cos_in__, sin_in__, y_out_, mode)
    ...
    origin_func_name = "rotary_position_embedding"
    ascendc_src_dir_ex = "rotary_position_embedding"
    ascendc_src_dir = "rotary_position_embedding"
    ascendc_src_file = "rotary_position_embedding.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator RotaryPositionEmbedding, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, cos_in__, sin_in__], origin_outputs = [y_out_],\
...
def op_select_format(x_in__, cos_in__, sin_in__, y_out_, mode=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, cos_in__, sin_in__, y_out_, mode)
    result = check_op_cap("op_select_format", "RotaryPositionEmbedding", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, cos_in__, sin_in__, y_out_, mode=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, cos_in__, sin_in__, y_out_, mode)
    result = check_op_cap("get_op_specific_info", "RotaryPositionEmbedding", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
