...
class PadInit(OpBase):
        ...
        with self.tik_instance.if_scope(self.tiling_input_dim_cut_axis == 1):
            ...
            with self.tik_instance.if_scope(core_outer_all % self.get_running_core_num() != 0):
                with self.tik_instance.if_scope(core_idx >= core_outer_all % self.get_running_core_num()):
                    ...
                    self.core_outer_start.set_as(
                        core_idx * self.core_outer_num + core_outer_all % self.get_running_core_num())
    ...
    def init_src_dst_gm(self,
        ...
        if self.const_values_dtype == "bfloat16":
            self.const_values_dtype = "float16"
        ...
        self.input_bytes_size = get_bit_len(input_dict_list[pad_input_idx]["dtype"]) // Constant.EIGHT_BIT
        x_input_gm = self.tik_instance.Tensor(input_dict_list[0]["dtype"], (2 ** 64 - 1,),
                                              name="x_gm",
                                              scope=tik.scope_gm)
        self.op_init_gm(input_dict_list[1:], output_dict_list, tiling_info=tiling_dict)
        self.input_gm_list.insert(0, x_input_gm)
    ...
    def data_move_with_mask_less_block(self, gm_src_info, gm_dst_info, copy_len, used_ub, ub_one_block):
        ...
        if self.is_v220:
            ...
            self.tik_instance.data_move(output_gm[output_offset],
                                        used_ub[Constant.TRANS_MIN_BLKS * Constant.TRANS_MIN_BLKS],
                                        0, 1, bursn_len, 0, 0)
    ...
    def calc_core_args(self, core_index, output_shape):
        ...
        core_len.set_as((output_shape + self.get_running_core_num() - 1) // self.get_running_core_num())
        ...
        with self.tik_instance.if_scope((core_offset + core_len) > output_shape):
            ...
            with self.tik_instance.if_scope(core_offset < output_shape):
                ...
                core_len.set_as((core_len + self.block_num - 1) // self.block_num)
                core_len.set_as(core_len * self.block_num)
                # use address rollback,will not oom
                core_offset.set_as(output_shape - core_len)
                with self.tik_instance.if_scope(core_offset < 0):
    ...
    def pre_compute(self, core_index):
        ...
        """
        with self.tik_instance.new_stmt_scope():
            const_bytes_size = get_bit_len(self.const_values_dtype) // Constant.EIGHT_BIT
            ...
            if get_bit_len(self.const_values_dtype) in [64]:
                for i in range(4):
        ...
        with self.tik_instance.if_scope(self.get_running_core_num() > 1):
    ...
    def empty_tensor_decorator(self, tiling_func):
        with self.tik_instance.if_scope(self.tiling_input_dim_0 * self.tiling_input_dim_1 * self.tiling_input_dim_2 *
                                        self.tiling_input_dim_3 * self.tiling_input_dim_4 * self.tiling_input_dim_5 *
                                        self.tiling_input_dim_6 * self.tiling_input_dim_7 != 0):
            tiling_func()
...
def pad_v2(x, paddings, constant_values, y, kernel_name="pad_v2"):
    ...
    supported_dtype = ("float32", "float16", "bfloat16", "int32", "int16", "uint16", "int64")
