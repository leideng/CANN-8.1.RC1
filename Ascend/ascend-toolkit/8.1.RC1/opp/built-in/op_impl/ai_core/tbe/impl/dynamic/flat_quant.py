...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'kronecker_p1', 'kronecker_p2']
    output_names = ['out', 'quant_scale']
...
def _build_args(x_in__, kronecker_p1_in__, kronecker_p2_in__, out_out_, quant_scale_out_, clip_ratio):
    __inputs__ = []
    for arg in [x_in__, kronecker_p1_in__, kronecker_p2_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [out_out_, quant_scale_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if clip_ratio != None:
        attr = {}
        attr["name"] = "clip_ratio"
        attr["dtype"] = "float"
        attr["value"] = clip_ratio
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("FlatQuant", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def flat_quant(x_in__, kronecker_p1_in__, kronecker_p2_in__, out_out_, quant_scale_out_, clip_ratio=1, kernel_name="flat_quant", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, kronecker_p1_in__, kronecker_p2_in__, out_out_, quant_scale_out_, clip_ratio)
    ...
    origin_func_name = "flat_quant"
    ascendc_src_dir_ex = "flat_quant"
    ascendc_src_dir = "flat_quant"
    ascendc_src_file = "flat_quant.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator FlatQuant, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "FlatQuant"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, kronecker_p1_in__, kronecker_p2_in__], origin_outputs = [out_out_, quant_scale_out_],\
...
def op_select_format(x_in__, kronecker_p1_in__, kronecker_p2_in__, out_out_, quant_scale_out_, clip_ratio=1, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, kronecker_p1_in__, kronecker_p2_in__, out_out_, quant_scale_out_, clip_ratio)
    result = check_op_cap("op_select_format", "FlatQuant", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, kronecker_p1_in__, kronecker_p2_in__, out_out_, quant_scale_out_, clip_ratio=1, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, kronecker_p1_in__, kronecker_p2_in__, out_out_, quant_scale_out_, clip_ratio)
    result = check_op_cap("get_op_specific_info", "FlatQuant", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
