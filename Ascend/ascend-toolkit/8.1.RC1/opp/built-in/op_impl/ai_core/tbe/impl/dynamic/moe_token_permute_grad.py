...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['permuted_output_grad', 'sorted_indices']
    output_names = ['input_grad']
...
def _build_args(permuted_output_grad_in__, sorted_indices_in__, input_grad_out_, num_topk, padded_mode):
    __inputs__ = []
    for arg in [permuted_output_grad_in__, sorted_indices_in__]:
        if arg != None:
...
@tbe_register.register_operator("MoeTokenPermuteGrad", trans_bool_to_s8=False)
...
def moe_token_permute_grad(permuted_output_grad_in__, sorted_indices_in__, input_grad_out_, num_topk, padded_mode=False, kernel_name="moe_token_permute_grad", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(permuted_output_grad_in__, sorted_indices_in__, input_grad_out_, num_topk, padded_mode)
    ...
    origin_func_name = "moe_token_permute_grad"
    ascendc_src_dir_ex = "moe_token_permute_grad"
    ascendc_src_dir = "moe_token_permute_grad"
    ascendc_src_file = "moe_token_permute_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MoeTokenPermuteGrad, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "MoeTokenPermuteGrad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[permuted_output_grad_in__, sorted_indices_in__], origin_outputs = [input_grad_out_],\
...
def op_select_format(permuted_output_grad_in__, sorted_indices_in__, input_grad_out_, num_topk, padded_mode=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(permuted_output_grad_in__, sorted_indices_in__, input_grad_out_, num_topk, padded_mode)
    result = check_op_cap("op_select_format", "MoeTokenPermuteGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(permuted_output_grad_in__, sorted_indices_in__, input_grad_out_, num_topk, padded_mode=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(permuted_output_grad_in__, sorted_indices_in__, input_grad_out_, num_topk, padded_mode)
    result = check_op_cap("get_op_specific_info", "MoeTokenPermuteGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
