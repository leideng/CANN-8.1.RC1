...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'scale', 'zero_point']
    output_names = ['y', 'mask']
...
def _build_args(x_in__, scale_in__, zero_point_in__, y_out_, mask_out_, axis, quant_min, quant_max):
    __inputs__ = []
    for arg in [x_in__, scale_in__, zero_point_in__]:
        if arg != None:
    ...
    if quant_max != None:
        ...
        attr["name"] = "quant_max"
        attr["dtype"] = "int"
        attr["value"] = quant_max
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("FakeQuantAffineCachemask", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def fake_quant_affine_cachemask(x_in__, scale_in__, zero_point_in__, y_out_, mask_out_, axis, quant_min, quant_max, kernel_name="fake_quant_affine_cachemask", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, scale_in__, zero_point_in__, y_out_, mask_out_, axis, quant_min, quant_max)
    ...
    origin_func_name = "fake_quant_affine_cachemask"
    ascendc_src_dir_ex = "fake_quant_affine_cachemask"
    ascendc_src_dir = "fake_quant_affine_cachemask"
    ascendc_src_file = "fake_quant_affine_cachemask.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator FakeQuantAffineCachemask, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "FakeQuantAffineCachemask"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, scale_in__, zero_point_in__], origin_outputs = [y_out_, mask_out_],\
...
def op_select_format(x_in__, scale_in__, zero_point_in__, y_out_, mask_out_, axis, quant_min, quant_max, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, scale_in__, zero_point_in__, y_out_, mask_out_, axis, quant_min, quant_max)
    result = check_op_cap("op_select_format", "FakeQuantAffineCachemask", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, scale_in__, zero_point_in__, y_out_, mask_out_, axis, quant_min, quant_max, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, scale_in__, zero_point_in__, y_out_, mask_out_, axis, quant_min, quant_max)
    result = check_op_cap("get_op_specific_info", "FakeQuantAffineCachemask", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
