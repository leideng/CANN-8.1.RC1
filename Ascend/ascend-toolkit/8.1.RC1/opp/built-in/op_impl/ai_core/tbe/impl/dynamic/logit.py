...
def _build_args(input_in__, output_out_, eps):
    __inputs__ = []
...
@tbe_register.register_operator("Logit", trans_bool_to_s8=False)
...
def logit(input_in__, output_out_, eps=-1, kernel_name="logit", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(input_in__, output_out_, eps)
    ...
    origin_func_name = "logit"
    ascendc_src_dir_ex = "logit"
    ascendc_src_dir = "logit"
    ascendc_src_file = "logit.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator Logit, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "Logit"
...
def op_select_format(input_in__, output_out_, eps=-1, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(input_in__, output_out_, eps)
    result = check_op_cap("op_select_format", "Logit", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(input_in__, output_out_, eps=-1, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(input_in__, output_out_, eps)
    result = check_op_cap("get_op_specific_info", "Logit", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
