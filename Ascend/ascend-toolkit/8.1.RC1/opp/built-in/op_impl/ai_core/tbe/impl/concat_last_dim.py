...
class ConcatWithVnchw:
    ...
    def __init__(self, input_data, output_data, kernel_name="concat_last_dim"):
        ...
        self.tik_instance = tik.Tik(block_size=int(tbe_platform.get_soc_spec("ubblock_size")))
        self.core_num = tbe_platform.get_soc_spec(tbe_platform.CORE_NUM)
        self.ele_each_block_fp16 = int(tbe_platform.get_soc_spec("ubblock_size")) // 2
        self.transpose_size = 256
    ...
    def check_vnchw_supported(self):
        ...
        if self.data_dtype in ["float32", "float16"] \
                ...
                and output_last_dim * factor <= 16 and self.src_size >= self.transpose_size:
            self.use_last_dim = True

        if self.data_dtype in ["float16"] \
                and input_last_dim == 1 and self.src_size >= self.transpose_size:
            self.use_last_dim = True

        if self.data_dtype == "float16" \
                and input_last_dim == 1 \
                and self.input_num == 3 \
                and self.src_size >= self.transpose_size * 8:
    ...
    def concat_last_dim_one_core(self, src_core_offset, des_core_offset,
        ...
        input_ub_0 = \
            self.tik_instance.Tensor("float16", (self.transpose_size * max_mov_num,),
                                     tik.scope_ubuf, "input_ub_0")
        vnchw_ub_0 = \
            self.tik_instance.Tensor("float16",
                                     (self.transpose_size * self.input_num * max_mov_num,),
                                     tik.scope_ubuf, "vnchw_ub_0")
        out_ub_0 = \
            self.tik_instance.Tensor("float16",
                                     (self.transpose_size * self.input_num * max_mov_num,),
                                     tik.scope_ubuf, "out_ub_0")
        input_ub_1 = self.tik_instance.Tensor("float16", (self.transpose_size * max_mov_num,),
        ...
        def _run_copy_input_and_vnchw(input_idx, ub_list, gm_input_offset,
            ...
            copy_len = run_mov_num * self.transpose_size - copy_tail
            nburst = get_ceil_int(copy_len,
                                  self.ele_each_block_fp16)
            ...
            # for nano version, tik_vnchwconv_nano is created to adapted for the situation which blocksize is 16B.
            if self.ele_each_block_fp16 == 8:
                each_stride = self.last_dim * self.input_num * 16
                transpose_offset = input_idx * 16
                src_offset = 0
                tik_vnchwconv_nano(self.tik_instance, ub_vnchw, ub_copy, transpose_offset,
                                   each_stride, src_offset, run_mov_num)
            else:
                _src_addrs = [
                    ub_copy[self.ele_each_block_fp16 * x]
                    for x in range(self.ele_each_block_fp16)
                ]
                dst_offset_ub = self.last_dim * input_idx
                dst_loop = self.ele_each_block_fp16 // self.last_dim
        ...
        def _run_vnchw_all_to_out(ub_list, gm_output_offset,
            ...
            _dst_rep_stride = (self.transpose_size // 16) * self.input_num
            _src_rep_stride = (self.transpose_size // 16) * self.input_num
            if run_mov_num == 1:
                _dst_rep_stride = 0
                _src_rep_stride = 0
            if self.ele_each_block_fp16 == 8:
                ...
                for i in range(self.input_num):
                    transpose_offset = i * 16
                    src_offset = self.transpose_size * i
                    tik_vnchwconv_nano(self.tik_instance, ub_out, ub_vnchw, transpose_offset,
                                       each_stride, src_offset, run_mov_num)
            else:
                for i_idx in range(self.input_num):
                    _src_addrs = [
                        ub_vnchw[i_idx * self.transpose_size + self.ele_each_block_fp16 * x]
                        for x in range(self.ele_each_block_fp16)
                    ]
                    _dst_addrs = [
                    ...
                    self.tik_instance.vnchwconv(False, False, _dst_addrs,
                                                ...
                                                _dst_rep_stride, _src_rep_stride)
            copy_len = \
                (run_mov_num * self.transpose_size - copy_tail)*self.input_num
            nburst = get_ceil_int(copy_len,
                                  self.ele_each_block_fp16)
            # copy ub to gm
        ...
        with self.tik_instance.for_range(0, core_loop // 2) as loop_idx:
            ...
            _run_one_loop(tiling_ub_list_0,
                          _idx * self.transpose_size * max_mov_num,
                          max_mov_num)
            _idx = loop_idx * 2 + 1
        ...
        if mov_tail != 0:
            _offset = core_loop * self.transpose_size * max_mov_num \
                      + core_tail * self.transpose_size
    ...
    def concat_last_dim_compute(self):
        ...
        """
        ub_half_size = \
            int(tbe_platform.get_soc_spec(tbe_platform.UB_SIZE) // 2 // 2 - 16)
        max_mov_num = \
            ub_half_size // self.transpose_size // (self.input_num * 2 + 1)

        # core scedule
        mov_num = (self.src_size + self.transpose_size - 1) // self.transpose_size
        mov_tail = mov_num * self.transpose_size - self.src_size
        ...
        move_num_core_tail = mov_num - (core_used - 1) * move_num_per_core
        ...
        with self.tik_instance.for_range(0, core_used, block_num=core_used) as core_idx:
            src_offset_core = \
                core_idx * move_num_per_core * self.transpose_size
...
class ConcatWith5HD:
    ...
    def check_shape_support(self):
        ...
        for shape in self.input_shapes + [self.output_shape]:
    ...
    def do_5hd_concat_cut_by_batch(self):
        ...
        # define concat fuction
        concat_fuc = self.do_5hd_concat_schedule
    ...
    def do_5hd_concat_schedule(self, input_offset, output_offset,
                              process_batch):
        """
        do_5hd_concat_cut_by_batch
        """
        ...
        max_transpose_sigment = \
            ub_half_size // self.transpose_size // 3

        copy_size = \
            self.output_shape[2] * self.output_shape[3] * self.shape_5hd_c0
        copy_loop = copy_size // (max_transpose_sigment * self.transpose_size)
        copy_loop_tail = copy_size % (max_transpose_sigment * self.transpose_size)
        tail_transpose_sigment = copy_loop_tail // self.transpose_size
        copy_nchw_tail = \
            get_ceil_int(copy_loop_tail, self.transpose_size) * self.transpose_size \
            - copy_loop_tail

        input_size = self.transpose_size * max_transpose_sigment
        input_ub_0 = \
        ...
        def _run_copy_input_and_vnchw(input_idx, ub_list, gm_input_offset,
            ...
            if self.ele_each_block_fp16 == 8:
                ...
                transpose_offset = 0
                src_offset = 0
                tik_vnchwconv_nano(self.tik_instance, ub_vnchw, ub_copy,
                                   transpose_offset, each_stride, src_offset, run_mov_num)
            else:
                _src_addrs = [
                    ub_copy[self.ele_each_block_fp16 * x]
                    for x in range(self.ele_each_block_fp16)
                ...
                _dst_addrs = [
                    ub_vnchw[self.ele_each_block_fp16 * x]
                    for x in range(self.ele_each_block_fp16)
                ]
                self.tik_instance.vnchwconv(False, False, _dst_addrs,
                                            _src_addrs, run_mov_num,
            ...
            des_c_dim = sum(self.last_ori_dim_list[:input_idx]) % 16
            des_offset = des_c_dim * 16
            src_repeat_block = \
        ...
        def _run_vector_dump(ub_list, run_mov_num):
            ...
            # copy gm to ub
            vector_size = run_mov_num * self.transpose_size
            repeat = get_ceil_int(vector_size, self.ele_each_block_fp16 * 8)
            self.tik_instance.vector_dup(self.ele_each_block_fp16 * 8,
                                         ub_out, 0.0,
                                         repeat, 1, 8)
        ...
        def _run_vnchw_all_to_out(ub_list, gm_output_offset,
            ...
            if self.ele_each_block_fp16 == 8:
                ...
                transpose_offset = 0
                src_offset = 0
                tik_vnchwconv_nano(self.tik_instance, ub_vnchw, ub_out,
                                   transpose_offset, each_stride, src_offset, run_mov_num)
            else:
                _src_addrs = [
                    ub_out[self.ele_each_block_fp16 * x]
                    for x in range(self.ele_each_block_fp16)
                ]
                _dst_addrs = [
                    ub_vnchw[self.ele_each_block_fp16 * x]
            ...
            copy_len = \
                (run_mov_num * self.transpose_size - copy_tail)
            nburst = get_ceil_int(copy_len,
                                  self.ele_each_block_fp16)
            # copy ub to gm
            ...
            if copy_loop > 0:
                with self.tik_instance.for_range(0, copy_loop) as loop_idx:
                    ...
                    _run_one_loop(tiling_ub_list_0, batch_offset_list,
                                  _idx * self.transpose_size * max_transpose_sigment,
                                  max_transpose_sigment)
