...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'y_grad', 'indices', 'locations']
    output_names = ['gates_grad']
...
def _build_args(x_in__, y_grad_in__, indices_in__, locations_in__, gates_grad_out_):
    __inputs__ = []
    for arg in [x_in__, y_grad_in__, indices_in__, locations_in__]:
        if arg != None:
...
@tbe_register.register_operator("MoeTutelCombineGates", trans_bool_to_s8=False)
...
def moe_tutel_combine_gates(x_in__, y_grad_in__, indices_in__, locations_in__, gates_grad_out_, kernel_name="moe_tutel_combine_gates", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_grad_in__, indices_in__, locations_in__, gates_grad_out_)
    ...
    origin_func_name = "moe_tutel_combine_gates"
    ascendc_src_dir_ex = "moe_tutel_combine_gates"
    ascendc_src_dir = "moe_tutel_combine_gates"
    ascendc_src_file = "moe_tutel_combine_gates.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MoeTutelCombineGates, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "MoeTutelCombineGates"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, y_grad_in__, indices_in__, locations_in__], origin_outputs = [gates_grad_out_],\
...
def op_select_format(x_in__, y_grad_in__, indices_in__, locations_in__, gates_grad_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_grad_in__, indices_in__, locations_in__, gates_grad_out_)
    result = check_op_cap("op_select_format", "MoeTutelCombineGates", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, y_grad_in__, indices_in__, locations_in__, gates_grad_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_grad_in__, indices_in__, locations_in__, gates_grad_out_)
    result = check_op_cap("get_op_specific_info", "MoeTutelCombineGates", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
