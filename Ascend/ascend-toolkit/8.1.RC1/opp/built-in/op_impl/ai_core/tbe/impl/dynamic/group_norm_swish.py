...
def _build_args(x_in__, gamma_in__, beta_in__, y_out_, mean_out_, rstd_out_, num_groups, data_format, eps, activate_swish, swish_scale):
    __inputs__ = []
    ...
    if eps != None:
        ...
        attr["value"] = eps
        __attrs__.append(attr)
    if activate_swish != None:
        attr = {}
        attr["name"] = "activate_swish"
        attr["dtype"] = "bool"
        attr["value"] = activate_swish
        __attrs__.append(attr)
    if swish_scale != None:
        attr = {}
...
@tbe_register.register_operator("GroupNormSwish", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def group_norm_swish(x_in__, gamma_in__, beta_in__, y_out_, mean_out_, rstd_out_, num_groups, data_format="NCHW", eps=1e-05, activate_swish=True, swish_scale=1, kernel_name="group_norm_swish", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, gamma_in__, beta_in__, y_out_, mean_out_, rstd_out_, num_groups, data_format, eps, activate_swish, swish_scale)
    ...
    origin_func_name = "group_norm_swish"
    ascendc_src_dir_ex = "group_norm_swish"
    ascendc_src_dir = "group_norm_swish"
    ascendc_src_file = "group_norm_swish.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator GroupNormSwish, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "GroupNormSwish"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, gamma_in__, beta_in__], origin_outputs = [y_out_, mean_out_, rstd_out_],\
