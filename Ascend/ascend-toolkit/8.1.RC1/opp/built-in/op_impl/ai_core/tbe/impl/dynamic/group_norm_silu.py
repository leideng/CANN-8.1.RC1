...
def _build_args(x_in__, gamma_in__, beta_in__, y_out_, mean_out_, rstd_out_, num_groups, eps, activate_silu):
    __inputs__ = []
    ...
    if eps != None:
        ...
        attr["value"] = eps
        __attrs__.append(attr)
    if activate_silu != None:
        attr = {}
        attr["name"] = "activate_silu"
        attr["dtype"] = "bool"
        attr["value"] = activate_silu
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("GroupNormSilu", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def group_norm_silu(x_in__, gamma_in__=None, beta_in__=None, y_out_=None, mean_out_=None, rstd_out_=None, num_groups=0, eps=1e-05, activate_silu=True, kernel_name="group_norm_silu", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, gamma_in__, beta_in__, y_out_, mean_out_, rstd_out_, num_groups, eps, activate_silu)
    ...
    origin_func_name = "group_norm_silu"
    ascendc_src_dir_ex = "group_norm_silu"
    ascendc_src_dir = "group_norm_silu"
    ascendc_src_file = "group_norm_silu.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator GroupNormSilu, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "GroupNormSilu"
