...
def tanh_compute_high_precision(shape, input_x, symbol):
    ...
    scope[key] = tbe_platform.scope_ubuf
...
def tanh_compute_high_performance(shape, input_x, symbol):
    ...
    if dtype_x == "float32":
        ...
        exp_val_fp16 = tvm.compute(
            shape, lambda *i: shape_util.cast(power_val(*i), "float16"), name=key)
        ...
        exp_val_fp32 = tvm.compute(
            shape, lambda *i: shape_util.cast(exp_val(*i), "float32"), name=key)
    ...
    scope["ub_rec_" + symbol] = tbe_platform.scope_ubuf
    ...
    scope["ub_tanh_" + symbol] = tbe_platform.scope_ubuf
...
def tanh_split_input_by_val(shape, input_x, symbol):
    ...
    if dtype_x == "float32":
        ...
        cmp_val_fp16 = tvm.compute(
            shape, lambda *i: shape_util.cast(input_abs(*i), "float16"), name=key)
        ...
        input_val_fp16 = tvm.compute(
            shape, lambda *i: shape_util.cast(input_x(*i), "float16"), name=key)
        ...
        input_gt = tvm.compute(
            shape, lambda *i: shape_util.cast(input_gt_fp16(*i), "float32"), name=key)
        res[key] = input_gt
        operation[key] = "vector_conv"
        scope[key] = tbe_platform.scope_ubuf
        ...
        input_lt = tvm.compute(
            shape, lambda *i: shape_util.cast(input_lt_fp16(*i), "float32"), name=key)
        res[key] = input_lt
        operation[key] = "vector_conv"
        scope[key] = tbe_platform.scope_ubuf
    else:
...
def newton_iteration(shape, tensor_x_rec, tensor_x, symbol, iter_num):
    ...
    for index in range(0, iter_num):
        ...
        scope_list[key] = tbe_platform.scope_ubuf
