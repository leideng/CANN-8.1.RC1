...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['input', 'target', 'weight']
    output_names = ['loss', 'log_prob', 'zloss', 'lse_for_zloss']
...
def _build_args(input_in__, target_in__, weight_in__, loss_out_, log_prob_out_, zloss_out_, lse_for_zloss_out_, reduction, ignore_index, label_smoothing, lse_square_scale_for_zloss, return_zloss):
    __inputs__ = []
    for arg in [input_in__, target_in__, weight_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [loss_out_, log_prob_out_, zloss_out_, lse_for_zloss_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if ignore_index != None:
        attr = {}
        attr["name"] = "ignore_index"
        attr["dtype"] = "int"
        attr["value"] = ignore_index
        __attrs__.append(attr)
    if label_smoothing != None:
        attr = {}
        attr["name"] = "label_smoothing"
        attr["dtype"] = "float"
        attr["value"] = label_smoothing
        __attrs__.append(attr)
    if lse_square_scale_for_zloss != None:
        attr = {}
        attr["name"] = "lse_square_scale_for_zloss"
        attr["dtype"] = "float"
        attr["value"] = lse_square_scale_for_zloss
        __attrs__.append(attr)
    if return_zloss != None:
        attr = {}
        attr["name"] = "return_zloss"
        attr["dtype"] = "bool"
        attr["value"] = return_zloss
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("CrossEntropyLoss", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def cross_entropy_loss(input_in__, target_in__, weight_in__=None, loss_out_=None, log_prob_out_=None, zloss_out_=None, lse_for_zloss_out_=None, reduction="mean", ignore_index=-100, label_smoothing=0, lse_square_scale_for_zloss=0, return_zloss=False, kernel_name="cross_entropy_loss", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(input_in__, target_in__, weight_in__, loss_out_, log_prob_out_, zloss_out_, lse_for_zloss_out_, reduction, ignore_index, label_smoothing, lse_square_scale_for_zloss, return_zloss)
    ...
    origin_func_name = "cross_entropy_loss"
    ascendc_src_dir_ex = "cross_entropy_loss"
    ascendc_src_dir = "cross_entropy_loss"
    ascendc_src_file = "cross_entropy_loss.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator CrossEntropyLoss, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[input_in__, target_in__, weight_in__], origin_outputs = [loss_out_, log_prob_out_, zloss_out_, lse_for_zloss_out_],\
...
def op_select_format(input_in__, target_in__, weight_in__=None, loss_out_=None, log_prob_out_=None, zloss_out_=None, lse_for_zloss_out_=None, reduction="mean", ignore_index=-100, label_smoothing=0, lse_square_scale_for_zloss=0, return_zloss=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(input_in__, target_in__, weight_in__, loss_out_, log_prob_out_, zloss_out_, lse_for_zloss_out_, reduction, ignore_index, label_smoothing, lse_square_scale_for_zloss, return_zloss)
    result = check_op_cap("op_select_format", "CrossEntropyLoss", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(input_in__, target_in__, weight_in__=None, loss_out_=None, log_prob_out_=None, zloss_out_=None, lse_for_zloss_out_=None, reduction="mean", ignore_index=-100, label_smoothing=0, lse_square_scale_for_zloss=0, return_zloss=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(input_in__, target_in__, weight_in__, loss_out_, log_prob_out_, zloss_out_, lse_for_zloss_out_, reduction, ignore_index, label_smoothing, lse_square_scale_for_zloss, return_zloss)
    result = check_op_cap("get_op_specific_info", "CrossEntropyLoss", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
