...
def strided_slice_last_dim_one(input_shape, dtype, output_shape, begin, kernel_name):
    ...
    strided_slice_last_dim_one
    """
    vmul_support = tbe_platform.api_check_support("tik.vmul", "float32")
    if not vmul_support:
        return False
    tik_instance = tik.Tik()
    aicore_num = tbe_platform.get_soc_spec(tbe_platform.CORE_NUM)
    support_dmp = tbe_platform.api_check_support("tik.data_move_pad")

    def _get_ub_block_num():
        """
        get the ub_size for dtype, get the block_size for dtype
        """
        ub_size_bytes = tbe_platform.get_soc_spec(tbe_platform.UB_SIZE) - 8192
    ...
    if max_ub_data <= ub_number and last_max_ub_data <= ub_number:
        input_ub_data = tik_instance.Tensor(dtype, (len_burst_one, ), name="input_ub_data", scope=tik.scope_ubuf)
        input_ub_data_one = tik_instance.Tensor(dtype, (input_ub_len_one, ),
                                                name="input_ub_data_one",
    ...
    if max_ub_data > ub_number:
        ...
        if input_ub_len_one > max_ub_number and len_burst_one > max_ub_number:
            ...
            input_ub_data = tik_instance.Tensor(dtype, (max_ub_size_last, ), name="input_ub_data", scope=tik.scope_ubuf)
    ...
    input_data = tik_instance.Tensor(dtype, (input_size, ), name="input_data", scope=tik.scope_gm)
    output_data = tik_instance.Tensor(dtype, (output_size, ), name="output_data", scope=tik.scope_gm)
    ...
    if max_ub_data <= ub_number and last_max_ub_data <= ub_number:
        with tik_instance.for_range(0, core_num, block_num=core_num) as total_cycle:
            with tik_instance.if_scope(total_cycle < core_num - 1):
                tik_instance.data_move(input_ub_data, input_data[total_cycle * len_burst_one], 0, 1, input_len_burst, 0,
                                       0)
                ...
                tik_instance.data_move(output_data[total_cycle * input_ub_len_one], input_ub_data_one, 0, 1,
                                       input_ub_length, 0, 0)

            with tik_instance.if_scope(total_cycle == core_num - 1):
                dtype_size = common_util.get_data_size(input_ub_data.dtype)
                if support_dmp is False:
                    tik_instance.data_move(input_ub_data, input_data[(core_num - 1) * len_burst_one], 0, 1,
                                           tail_len_burst, 0, 0)
                else:
                    input_ub_data_b8 = input_ub_data.reinterpret_cast_to("int8")
                    input_gm_data_b8 = input_data[(core_num - 1) * len_burst_one].reinterpret_cast_to("int8")
                    tik_instance.data_move_pad(input_ub_data_b8, input_gm_data_b8, 1, tail_num * dtype_size, 0, 0)
                with tik_instance.for_range(0, max_num_one) as group:
                    for cur_num in range(0, consecutive):
                        input_ub_data_one[group * consecutive + cur_num].set_as(input_ub_data[group * len_burst +
                                                                                              start_num + cur_num])
                if support_dmp is False:
                    tik_instance.data_move(output_data[(core_num - 1) * input_ub_len_one], input_ub_data_one, 0, 1,
                                           tail_len_output_burst, 0, 0)
                else:
                    input_ub_data_one_b8 = input_ub_data_one.reinterpret_cast_to("int8")
                    output_gm_data_b8 = output_data[(core_num - 1) * input_ub_len_one].reinterpret_cast_to("int8")
                    tik_instance.data_move_pad(output_gm_data_b8, input_ub_data_one_b8, 1,
                                               output_tail_size * dtype_size, 0, 0)
    if max_ub_data > ub_number:
        if input_ub_len_one > max_ub_number and len_burst_one > max_ub_number:
            with tik_instance.for_range(0, core_num, block_num=core_num) as total_cycle:
                with tik_instance.if_scope(total_cycle < core_num - 1):
                    with tik_instance.for_range(0, loop_index) as loop:
                        tik_instance.data_move(input_ub_data,
                                               input_data[total_cycle * len_burst_one + loop * max_ub_size_last], 0, 1,
                                               both_ub_data_length, 0, 0)
                        with tik_instance.for_range(0, max_both_number) as group:
                            with tik_instance.for_range(0, consecutive) as cur_num:
                                input_ub_data_one[group * consecutive + cur_num]\
                                    .set_as(input_ub_data[group * len_burst + start_num + cur_num])
                        tik_instance.data_move(
                            output_data[total_cycle * input_ub_len_one + loop * max_ub_size_last_one],
                            input_ub_data_one, 0, 1, max_out_ub_size_stride, 0, 0)

                    if tail_len_burst_one > 0:
                        tik_instance.data_move(input_ub_data,
                                               input_data[total_cycle * len_burst_one + loop_index * max_ub_size_last],
                        ...
                        with tik_instance.for_range(0, tail_max_both_number) as group:
                            with tik_instance.for_range(0, consecutive) as cur_num:
                                input_ub_data_one[group * consecutive + cur_num]\
                                    .set_as(input_ub_data[group * len_burst + start_num + cur_num])
                        tik_instance.data_move(
                            output_data[total_cycle * input_ub_len_one + loop_index * max_ub_size_last_one],
                            input_ub_data_one, 0, 1, tail_output_data_length, 0, 0)

                with tik_instance.if_scope(total_cycle == core_num - 1):
                    with tik_instance.for_range(0, last_core_loop_index) as loop:
                        tik_instance.data_move(input_ub_data,
                                               input_data[(core_num - 1) * len_burst_one + loop * max_ub_size_last], 0,
                                               1, both_ub_data_length, 0, 0)
                        with tik_instance.for_range(0, max_both_number) as group:
                            with tik_instance.for_range(0, consecutive) as cur_num:
                                input_ub_data_one[group * consecutive + cur_num]\
                                    .set_as(input_ub_data[group * len_burst + start_num + cur_num])
                        tik_instance.data_move(
                            output_data[(core_num - 1) * input_ub_len_one + loop * max_ub_size_last_one],
                            input_ub_data_one, 0, 1, max_out_ub_size_stride, 0, 0)

                    if last_core_burst_one > 0:
                        tik_instance.data_move(
                            input_ub_data,
                            input_data[(core_num - 1) * len_burst_one + last_core_loop_index * max_ub_size_last], 0, 1,
                            last_core_len_burst_data_length, 0, 0)
                        with tik_instance.for_range(0, last_core_tail_max_both_number) as group:
                            with tik_instance.for_range(0, consecutive) as cur_num:
                                input_ub_data_one[group * consecutive + cur_num]\
                                    .set_as(input_ub_data[group * len_burst + start_num + cur_num])
                        tik_instance.data_move(
                            output_data[(core_num - 1) * input_ub_len_one +
                                        last_core_loop_index * max_ub_size_last_one], input_ub_data_one, 0, 1,
        ...
        if input_ub_len_one <= max_ub_number and len_burst_one > max_ub_number:
            with tik_instance.for_range(0, core_num, block_num=core_num) as total_cycle:
                with tik_instance.if_scope(total_cycle < core_num - 1):
                    with tik_instance.for_range(0, loop_index) as loop:
                        tik_instance.data_move(input_ub_data,
                                               input_data[total_cycle * len_burst_one + loop * use_input_ub_size], 0, 1,
                                               use_input_length, 0, 0)
                        with tik_instance.for_range(0, max_use_number) as group:
                            with tik_instance.for_range(0, consecutive) as cur_num:
                                input_ub_data_one[group * consecutive + cur_num]\
                                    .set_as(input_ub_data[group * len_burst + start_num + cur_num])
                        tik_instance.data_move(output_data[total_cycle * input_ub_len_one + loop * max_use_number_one],
                                               input_ub_data_one, 0, 1, use_output_length, 0, 0)

                    if last_input_ub_data > 0:
                        tik_instance.data_move(input_ub_data,
                                               input_data[total_cycle * len_burst_one + loop_index * use_input_ub_size],
                        ...
                        with tik_instance.for_range(0, max_last_use_number) as group:
                            with tik_instance.for_range(0, consecutive) as cur_num:
                                input_ub_data_one[group * consecutive + cur_num]\
                                    .set_as(input_ub_data[group * len_burst + start_num + cur_num])
                        tik_instance.data_move(
                            output_data[total_cycle * input_ub_len_one + loop_index * max_use_number_one],
                            input_ub_data_one, 0, 1, last_output_ub_data_length, 0, 0)
                ...
                with tik_instance.if_scope(total_cycle == core_num - 1):
                    ...
                    if last_core_input_ub_data > 0:
                        tik_instance.data_move(
                            input_ub_data,
                            input_data[total_cycle * len_burst_one + last_core_loop_index * use_input_ub_size], 0, 1,
                            last_core_input_ub_data_length, 0, 0)
                        with tik_instance.for_range(0, last_core_tail_max_use_number) as group:
                            with tik_instance.for_range(0, consecutive) as cur_num:
                                input_ub_data_one[group * consecutive + cur_num]\
                                    .set_as(input_ub_data[group * len_burst + start_num + cur_num])
                        tik_instance.data_move(
                            output_data[total_cycle * input_ub_len_one + last_core_loop_index * max_use_number_one],
                            input_ub_data_one, 0, 1, last_core_output_ub_data_length, 0, 0)
