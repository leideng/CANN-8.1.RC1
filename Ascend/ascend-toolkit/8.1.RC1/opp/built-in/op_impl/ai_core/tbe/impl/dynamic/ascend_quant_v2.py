...
def _build_args(x_in__, scale_in__, offset_in__, y_out_, sqrt_mode, round_mode, dst_type, axis):
    __inputs__ = []
    for arg in [x_in__, scale_in__, offset_in__]:
        if arg != None:
    ...
    if sqrt_mode != None:
        ...
        attr["name"] = "sqrt_mode"
        attr["dtype"] = "bool"
        attr["value"] = sqrt_mode
        __attrs__.append(attr)
    if round_mode != None:
        attr = {}
        attr["name"] = "round_mode"
        attr["dtype"] = "str"
        attr["value"] = round_mode
        __attrs__.append(attr)
    if dst_type != None:
        attr = {}
        attr["name"] = "dst_type"
...
@tbe_register.register_operator("AscendQuantV2", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def ascend_quant_v2(x_in__, scale_in__, offset_in__=None, y_out_=None, sqrt_mode=False, round_mode="round", dst_type=2, axis=-1, kernel_name="ascend_quant_v2", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, scale_in__, offset_in__, y_out_, sqrt_mode, round_mode, dst_type, axis)
    ...
    origin_func_name = "ascend_quant_v2"
    ascendc_src_dir_ex = "ascend_quant_v2"
    ascendc_src_dir = "ascend_quant_v2"
    ascendc_src_file = "ascend_quant_v2.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator AscendQuantV2, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "AscendQuantV2"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, scale_in__, offset_in__], origin_outputs = [y_out_],\
