...
class Constant:
    ...
    MAX_CORE = tbe_platform.get_soc_spec(
...
class MaxPool3DGradCompute:
    ...
    def __init__(self, shape_list, params):
        ...
        self.ub_maxsize = tbe_platform. \
            get_soc_spec(tbe_platform.UB_SIZE) // self.num_bit
        self.L1_maxsize = tbe_platform. \
            get_soc_spec(tbe_platform.L1_SIZE) // self.num_bit
        self.orig_x_gm = None
        self.orig_y_gm = None
        self.grads_gm = None
        self.ou_y_gm = None

        self.check_load3d_support = tbe_platform.api_check_support(
            "tik.load3dv1")
    ...
    def _vector_op(self, tik_instance, operator,
        ...
        if operator == "vadd":
            ...
            if repeat_max_loop > 0:
                ...
                dst_offset += Constant.MINI_UNIT // (tbe_platform.get_bit_len(
                    dst.dtype.lower()) // 8) * stride_config[3] * 255
                src1_offset += Constant.MINI_UNIT // (tbe_platform.get_bit_len(
                    src1.dtype.lower()) // 8) * stride_config[4] * 255
                src2_offset += Constant.MINI_UNIT // (tbe_platform.get_bit_len(
...
def check_supported(orig_x, orig_y, grads, y,
                    ksize, strides, padding="SAME", pads=(0, 0, 0, 0, 0, 0),
                    data_format="NDHWC",
                    kernel_name="max_pool3d_grad"):
    """
...
# 'pylint: disable = too-many-arguments,huawei-too-many-arguments
def op_select_format(orig_x, orig_y, grads, y,
                    ksize, strides, padding="SAME", pads=(0, 0, 0, 0, 0, 0),
                    data_format="NDHWC",
                    kernel_name="max_pool3d_grad"):
    ...
    x1_dtype, x2_dtype, grad_dtype, y_dtype = \
        ["float16", ], ["float16", ], ["float16", ], ["float", ]

    input0 = gen_param(classify="input0", name="orig_x",
                       datatype=",".join(x1_dtype),
                       format=",".join(x1_format),
                       unknownshape_format=",".join(x1_dy_format))
    input1 = gen_param(classify="input1", name="orig_y",
                       datatype=",".join(x2_dtype),
                       format=",".join(x2_format),
                       unknownshape_format=",".join(x2_dy_format))
    input2 = gen_param(classify="input2", name="grads",
