...
def check_supported(input_x, constant_values, output_x, paddings, kernel_name="pad_v2_d"):
    input_x_shape = input_x.get("shape")
    input_x_shape = list(input_x_shape)
    if input_x_shape in [[256, 64, 112, 112], [128, 64, 112, 112]]:
        return False, "change to dynamic op padv2"
    return True
...
def _check_optimization_nhwc(input_x, paddings):
    ...
    if dtype == "float16":
        ub_maxsize = (tbe_platform.get_soc_spec(tbe_platform.UB_SIZE) // 32 * 16 - 16)
    else:
        ub_maxsize = (tbe_platform.get_soc_spec(tbe_platform.UB_SIZE) // 32 * 8 - 8)
    device_core_num = \
        tbe_platform.get_soc_spec(
            tbe_platform.CORE_NUM)
