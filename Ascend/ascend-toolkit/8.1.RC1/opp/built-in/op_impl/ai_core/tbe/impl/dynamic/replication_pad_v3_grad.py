...
class ReplicationPadV3GradInit:
    ...
    def __init__(self, x, paddings, y, mode, padding_contiguous=True, kernel_name='pad_v3_grad'):
        ...
        # when this opp process fp32, origin work_space may overstep the boundary. 
        # a more proper empirical value replace the original one.
        ...
        self.core_num_var = self.tik_instance.Scalar(self.tiling_dtype, "core_num_var", init_value=self.core_nums)
    ...
    def do_tiling_key_mode_0(self, core_index):
        ...
        align_input_dim_2.set_as(((self.tiling_input_dim_2 - 1) // Constant.ALIGN_16 + 1) * Constant.ALIGN_16)
        align_input_dim_3.set_as(((self.tiling_input_dim_3 - 1) // Constant.ALIGN_16 + 1) * Constant.ALIGN_16)
                                    ...
                                    for k in range(Constant.DST_CYCLES):
                                        dst_list.append(
                                            pang_ub_1[time_2 * Constant.TRANS_MIN_BLKS * Constant.TRANS_MIN_BLKS * j +
                                                      ...
                                                      time_2 * Constant.TRANS_MIN_BLKS * k])
                                        dst_list.append(
                                            pang_ub_1[time_2 * Constant.TRANS_MIN_BLKS * Constant.TRANS_MIN_BLKS * j +
                                                      Constant.TRANS_MIN_BLKS * i + time_2 * Constant.TRANS_MIN_BLKS * k
                                                      + Constant.DST_OFFSET])
                                    self.tik_instance.vnchwconv(True, False, dst_list, src_list, 2, dst_stride, 
                                                                src_stride)
                        ...
                        if self.inner_dtype == "float16":
                            ...
                            for k in range(Constant.DST_CYCLES):
                                ...
                                dst_list.append(ping_ub_1[time_3 * Constant.TRANS_MIN_BLKS * Constant.TRANS_MIN_BLKS * i
                                                + Constant.TRANS_MIN_BLKS * j + time_3 * Constant.TRANS_MIN_BLKS * k
                                                + Constant.DST_OFFSET])
                            self.tik_instance.vnchwconv(True, False, dst_list, src_list, 2, dst_stride, src_stride)

                align_output_dim_3 = self.tik_instance.Scalar('int64', name='align_output_dim_3')
                align_output_dim_3.set_as(((self.tiling_output_dim_3 - 1) // Constant.ALIGN_16 + 1) * Constant.ALIGN_16)
                with self.tik_instance.if_scope(core_index == self.core_uesd_num - 1):
                        ...
                        with self.tik_instance.if_scope(index == flag):
                            ...
                            with self.tik_instance.if_scope(flag == ranges - 1):
                                self.tik_instance.data_move(self.output_gm[gm_offset], self.block_less_16, 0, 
                                                            Constant.TRANS_MIN_BLKS // self.block_num, 1, 0, 0)
    ...
    def do_tiling_key_mode_1(self, core_index):
        ...
        with self.tik_instance.new_stmt_scope():
            ...
            with self.tik_instance.for_range(0, ranges) as index:
                ...
                align_inter.set_as(((self.tiling_output_dim_2 - 1) // Constant.ALIGN_16 + 1) * Constant.ALIGN_16)
                time = self.tik_instance.Scalar('int64', name='time')
                time.set_as(align_inter // Constant.ALIGN_16)
                if self.inner_dtype == "float32":
                    float32_trans_block = Constant.FLOAT32_TRANS_BLK
                    src_stride = 1
                    dst_stride = time * float32_trans_block * Constant.FLOAT32_BYTES // Constant.BLOCK_SIZE
