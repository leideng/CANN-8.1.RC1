...
def _build_args(x_in__, indices_in__, axis_in__, y_out_, batch_dims, negative_index_support):
    __inputs__ = []
    for arg in [x_in__, indices_in__, axis_in__]:
        if arg != None:
    ...
    if batch_dims != None:
        attr = {}
        attr["name"] = "batch_dims"
        attr["dtype"] = "int"
        attr["value"] = batch_dims
        __attrs__.append(attr)
    if negative_index_support != None:
        attr = {}
        attr["name"] = "negative_index_support"
        attr["dtype"] = "bool"
        attr["value"] = negative_index_support
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("GatherV3", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def gather_v3(x_in__, indices_in__, axis_in__, y_out_, batch_dims=0, negative_index_support=False, kernel_name="gather_v3", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, indices_in__, axis_in__, y_out_, batch_dims, negative_index_support)
    ...
    origin_func_name = "gather_v3"
    ascendc_src_dir_ex = "gather_v3"
    ascendc_src_dir = "gather_v3"
    ascendc_src_file = "gather_v3.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator GatherV3, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "GatherV3"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, indices_in__, axis_in__], origin_outputs = [y_out_],\
...
def check_supported(x_in__, indices_in__, axis_in__, y_out_, batch_dims=0, negative_index_support=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, indices_in__, axis_in__, y_out_, batch_dims, negative_index_support)
    ret_str = check_op_cap("check_supported", "GatherV3", __inputs__, __outputs__, __attrs__)
...
def get_op_support_info(x_in__, indices_in__, axis_in__, y_out_, batch_dims=0, negative_index_support=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, indices_in__, axis_in__, y_out_, batch_dims, negative_index_support)
    result = check_op_cap("get_op_support_info", "GatherV3", __inputs__, __outputs__, __attrs__)
