...
def _build_args(x_in__, size_in__, y_out_, align_corners, scales):
    __inputs__ = []
    for arg in [x_in__, size_in__]:
        if arg != None:
...
@tbe_register.register_operator("UpsampleBilinear2d", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_LIST_FLOAT, para_check.KERNEL_NAME)
def upsample_bilinear2d(x_in__, size_in__, y_out_, align_corners=False, scales=[], kernel_name="upsample_bilinear2d", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, size_in__, y_out_, align_corners, scales)
    ...
    origin_func_name = "upsample_bilinear2d"
    ascendc_src_dir_ex = "upsample_bilinear2d"
    ascendc_src_dir = "upsample_bilinear2d"
    ascendc_src_file = "upsample_bilinear2d.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator UpsampleBilinear2d, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "UpsampleBilinear2d"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, size_in__], origin_outputs = [y_out_],\
...
def op_select_format(x_in__, size_in__, y_out_, align_corners=False, scales=[], impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, size_in__, y_out_, align_corners, scales)
    result = check_op_cap("op_select_format", "UpsampleBilinear2d", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, size_in__, y_out_, align_corners=False, scales=[], impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, size_in__, y_out_, align_corners, scales)
    result = check_op_cap("get_op_specific_info", "UpsampleBilinear2d", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
