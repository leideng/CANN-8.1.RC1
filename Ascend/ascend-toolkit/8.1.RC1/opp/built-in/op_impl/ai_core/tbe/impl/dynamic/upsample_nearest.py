...
def _build_args(x_in__, y_out_, output_size, scales_h, scales_w, exact_mode):
    __inputs__ = []
    for arg in [x_in__]:
        if arg != None:
    ...
    if exact_mode != None:
        attr = {}
        attr["name"] = "exact_mode"
        attr["dtype"] = "bool"
        attr["value"] = exact_mode
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("UpsampleNearest", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def upsample_nearest(x_in__, y_out_, output_size, scales_h=0, scales_w=0, exact_mode=False, kernel_name="upsample_nearest", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, output_size, scales_h, scales_w, exact_mode)
    ...
    origin_func_name = "upsample_nearest"
    ascendc_src_dir_ex = "upsample_nearest"
    ascendc_src_dir = "upsample_nearest"
    ascendc_src_file = "upsample_nearest.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator UpsampleNearest, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "UpsampleNearest"
...
def op_select_format(x_in__, y_out_, output_size, scales_h=0, scales_w=0, exact_mode=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, output_size, scales_h, scales_w, exact_mode)
    result = check_op_cap("op_select_format", "UpsampleNearest", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, y_out_, output_size, scales_h=0, scales_w=0, exact_mode=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, output_size, scales_h, scales_w, exact_mode)
    result = check_op_cap("get_op_specific_info", "UpsampleNearest", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
