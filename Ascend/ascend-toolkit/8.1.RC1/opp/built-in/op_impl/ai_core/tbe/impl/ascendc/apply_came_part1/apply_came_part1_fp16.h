...
private:
    ...
    __aicore__ inline void CopyInLast(int64_t nLoopIdx, int64_t mLoopIdx, int64_t curRepeatTimes, LocalTensor<T> gradLocal);
    __aicore__ inline void CopyInNormal(int64_t nLoopIdx, int64_t mLoopIdx, int64_t curRepeatTimes, LocalTensor<T> gradLocal);
...
private:
    ...
    const int64_t ONCE_ALGN_NUM{32 / sizeof(T)};
...
template <typename T>
__aicore__ inline void ApplyCamePart1FP16<T>::Init(GM_ADDR grad, GM_ADDR eps, GM_ADDR sum_grad_r, GM_ADDR sum_grad_c, GM_ADDR sum_grad_rc, GM_ADDR workspace, const ApplyCamePart1TilingData* tilingData)
{
    // 初始化tiling
    ...
    gmGrad_.SetGlobalBuffer((__gm__ T *)grad + inputIdx_);
...
template <typename T>
__aicore__ inline void ApplyCamePart1FP16<T>::UpdateRepeatTimes()
{
...
template <typename T>
__aicore__ inline void ApplyCamePart1FP16<T>::CopyInLast(int64_t nLoopIdx, int64_t mLoopIdx, int64_t curRepeatTimes, LocalTensor<T> gradLocal)
{
    Duplicate<T>(gradLocal, (T)0.0, curRepeatTimes * ONCE_HANDLE_NUM64);
    int64_t srcStrideLast = (mNormalCoreNum_ - mNormalCoreNum_ % ONCE_HANDLE_NUM64) * sizeof(T);
    int64_t dstStrideLast = (ONCE_HANDLE_NUM64 - mNormalCoreNum_ % ONCE_HANDLE_NUM64) * sizeof(T) / 32;
    int64_t blockLenLast = mNormalCoreNum_ % ONCE_HANDLE_NUM64 * sizeof(T);
...
template <typename T>
__aicore__ inline void ApplyCamePart1FP16<T>::CopyInNormal(int64_t nLoopIdx, int64_t mLoopIdx, int64_t curRepeatTimes, LocalTensor<T> gradLocal)
{
    int64_t srcStrideNormal = (mNormalCoreNum_ - ONCE_HANDLE_NUM64) * sizeof(T);
    int64_t blockLenNormal = ONCE_HANDLE_NUM64 * sizeof(T);
...
template <typename T>
__aicore__ inline void ApplyCamePart1FP16<T>::ComputeR(int64_t curRepeatTimes, LocalTensor<float> accuComTmpUb)
{
...
template <typename T>
__aicore__ inline void ApplyCamePart1FP16<T>::ComputeRC(int64_t curRepeatTimes, LocalTensor<float> accuComTmpUb, LocalTensor<float> mComTmpUb)
{
    LocalTensor<float> sumGradRCLocal = sumGradRCQueue_.AllocTensor<float>();
    pipe_barrier(PIPE_V);
