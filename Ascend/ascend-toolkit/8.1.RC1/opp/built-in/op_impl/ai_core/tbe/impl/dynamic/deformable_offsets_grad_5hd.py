...
class DeformableOffsetsGradV2:
    ...
    def __init__(self, grad, x, offsets, grad_x, grad_offsets, strides, pads, ksize, dilations,
        ...
        self.elem_num_offsets_filter = self.dim_group * self.dim_kh * self.dim_kw * 2
    ...
    def _get_tiling_args(self, tiling_ub):
        ...
        self.dim_c1 = self.tik_instance.Scalar(dtype=self.tiling_dtype, name="dim_c1")
        self.dim_c1.set_as(tiling_ub[17])
        self.dim_grad_h = self.tik_instance.Scalar(dtype=self.tiling_dtype, name="dim_grad_h")
        self.dim_grad_h.set_as(tiling_ub[18])
        self.dim_grad_w = self.tik_instance.Scalar(dtype=self.tiling_dtype, name="dim_grad_w")
        self.dim_grad_w.set_as(tiling_ub[19])
    ...
    def deformable_offset_grad_v2_compute(self):
        """
    ...
    def compute_one_filter(self, n_idx, start_list, kh_idx, out_c_num):
        ...
        with self.tik_instance.for_range(0, out_c_num * self.dim_kw * self.dim_group) as inner_index:
            ...
            # 'grad_scale_lb = grad * l_b_w * l_b_h
            scale_lb = sub_floor_x * ceil_sub_y
            ...
            # 'grad_scale_lt = grad * l_t_w * l_t_h
            scale_lt = sub_floor_x * sub_floor_y
            ...
            # 'grad_scale_rt = grad * r_t_w * r_t_h
            scale_rt = ceil_sub_x * sub_floor_y
            ...
            # 'grad_scale_rb = grad * r_b_w * r_b_h
            scale_rb = ceil_sub_x * ceil_sub_y
            ...
            self.get_offset_w(self.ub_out, ub_out_w_index,
                            [ceil_sub_x, ceil_sub_y, sub_floor_x, sub_floor_y])
    ...
    def input_x_out_weight(self, flag, ub_tensor, gm_start):
        ...
        group_c_nburst = self.dim_group_c // self.dim_c1
        group_c_burst = self.dim_c1  // self.data_in_one_block
        group_src_stride = ((self.dim_h_in * self.dim_w_in - 1) * self.dim_c1) // self.data_in_one_block
        gap = (self.dim_h_in * self.dim_w_in - 1) * self.dim_c1 * self.dsize
        group_c_burst_pad = self.dim_c1 * self.dsize
        with self.tik_instance.if_scope(flag):
            if self.dtype in ("float16", "bfloat16"):
                if tbe_platform.api_check_support("tik.data_move_pad", self.dtype):
                    self.tik_instance.data_move_pad(fp16_tensor, self.x_gm[gm_start], group_c_nburst,
                                                    group_c_burst_pad, 0, gap)
                    self.vector_conv(x_ub, fp16_tensor, self.dim_group_c)
                    self.tik_instance.set_atomic_add(1)
                    self.tik_instance.data_move_pad(self.grad_x_gm[gm_start], weight_ub, group_c_nburst,
                                                    group_c_burst_pad * Constant.BURST_DOUBLE,
                                                    gap * Constant.BURST_DOUBLE,
                                                    0)
                    self.tik_instance.set_atomic_add(0)
                else:
                    self.tik_instance.data_move(fp16_tensor, self.x_gm[gm_start], 0,
                                                group_c_nburst, group_c_burst, 0,
                                                group_src_stride)
                    # fp16 -> fp32
                    self.vector_conv(x_ub, fp16_tensor, self.dim_group_c)

                    self.tik_instance.set_atomic_add(1)
                    self.tik_instance.data_move(self.grad_x_gm[gm_start], weight_ub, 0, group_c_nburst,
                                                group_c_burst * Constant.BURST_DOUBLE, group_src_stride * Constant.BURST_DOUBLE, 0)
                    self.tik_instance.set_atomic_add(0)

            else:
                if tbe_platform.api_check_support("tik.data_move_pad", self.dtype): 
                    self.tik_instance.data_move_pad(x_ub, self.x_gm[gm_start], group_c_nburst,
                                                    group_c_burst_pad, 0, gap)
                    self.tik_instance.set_atomic_add(1)
                    self.tik_instance.data_move_pad(self.grad_x_gm[gm_start], weight_ub, group_c_nburst,
                                                    group_c_burst_pad, gap, 0)
                    self.tik_instance.set_atomic_add(0)
                else:
                    self.tik_instance.data_move(x_ub, self.x_gm[gm_start], 0, group_c_nburst, group_c_burst, 0,
                                                group_src_stride)
                    self.tik_instance.set_atomic_add(1)
                    self.tik_instance.data_move(self.grad_x_gm[gm_start], weight_ub, 0, group_c_nburst, group_c_burst,
                                                group_src_stride, 0)
                    self.tik_instance.set_atomic_add(0)
    ...
    def load_grad(self, size, grad_start):
        grad_nburst_len = size // self.dim_c1
        grad_burst_len = self.dim_c1  // self.data_in_one_block
        grad_src_stride = ((self.dim_grad_h * self.dim_grad_w - 1) * self.dim_c1) // self.data_in_one_block
        group_c_burst_pad = self.dim_c1 * self.dsize
        gap = (self.dim_grad_h * self.dim_grad_w - 1) * self.dim_c1 * self.dsize
        if self.dtype in ("float16", "bfloat16"):
            if tbe_platform.api_check_support("tik.data_move_pad", self.dtype):
                ...
                self.tik_instance.data_move(self.ub_grad_fp16, self.grad_gm[grad_start], 0,
                                            grad_nburst_len, grad_burst_len, 0, grad_src_stride)

            self.vector_conv(self.grad_ub, self.  ub_grad_fp16, size)
        else:
            if tbe_platform.api_check_support("tik.data_move_pad", self.dtype):
                self.tik_instance.data_move_pad(self.grad_ub, self.grad_gm[grad_start], grad_nburst_len,
                                                group_c_burst_pad, 0, gap)
            else:
                self.tik_instance.data_move(self.grad_ub, self.grad_gm[grad_start], 0,
                                            grad_nburst_len, grad_burst_len, 0, grad_src_stride)
    ...
    def get_offset_w(self, dst_tensor, dst_index, val_list):
        ceil_sub_x, ceil_sub_y, sub_floor_x, sub_floor_y = val_list
        self.vector_muls(self.weight_ub_lt, self.ub_lt_x, sub_floor_y, self.dim_group_c, "float32")
        self.vector_muls(self.weight_ub_lb, self.ub_lb_x, ceil_sub_y, self.dim_group_c, "float32")
        self.vector_muls(self.weight_ub_rt, self.ub_rt_x, sub_floor_y, self.dim_group_c, "float32")
        self.vector_muls(self.weight_ub_rb, self.ub_rb_x, ceil_sub_y, self.dim_group_c, "float32")
    ...
    def get_offset_h(self, dst_tensor, dst_index, val_list):
        ceil_sub_x, ceil_sub_y, sub_floor_x, sub_floor_y = val_list
        self.vector_muls(self.weight_ub_lt, self.ub_lt_x, sub_floor_x, self.dim_group_c, "float32")
        self.vector_muls(self.weight_ub_lb, self.ub_lb_x, sub_floor_x, self.dim_group_c, "float32")
        self.vector_muls(self.weight_ub_rt, self.ub_rt_x, ceil_sub_x, self.dim_group_c, "float32")
        self.vector_muls(self.weight_ub_rb, self.ub_rb_x, ceil_sub_x, self.dim_group_c, "float32")
        self.vector_binary_op(self.dim_group_c,
