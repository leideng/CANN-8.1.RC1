...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['attention', 'ori_index_a', 'ori_index_b', 'topk_indice', 'arg_max']
    output_names = ['unzip_token']
...
def _build_args(attention_in__, ori_index_a_in__, ori_index_b_in__, topk_indice_in__, arg_max_in__, unzip_token_out_, top_rate):
    __inputs__ = []
    for arg in [attention_in__, ori_index_a_in__, ori_index_b_in__, topk_indice_in__, arg_max_in__]:
        if arg != None:
...
@tbe_register.register_operator("TomeUnmerge", trans_bool_to_s8=False)
...
def tome_unmerge(attention_in__, ori_index_a_in__, ori_index_b_in__, topk_indice_in__, arg_max_in__, unzip_token_out_, top_rate=0.5, kernel_name="tome_unmerge", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(attention_in__, ori_index_a_in__, ori_index_b_in__, topk_indice_in__, arg_max_in__, unzip_token_out_, top_rate)
    ...
    origin_func_name = "tome_unmerge"
    ascendc_src_dir_ex = "tome_unmerge"
    ascendc_src_dir = "tome_unmerge"
    ascendc_src_file = "tome_unmerge.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator TomeUnmerge, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "TomeUnmerge"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[attention_in__, ori_index_a_in__, ori_index_b_in__, topk_indice_in__, arg_max_in__], origin_outputs = [unzip_token_out_],\
...
def op_select_format(attention_in__, ori_index_a_in__, ori_index_b_in__, topk_indice_in__, arg_max_in__, unzip_token_out_, top_rate=0.5, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(attention_in__, ori_index_a_in__, ori_index_b_in__, topk_indice_in__, arg_max_in__, unzip_token_out_, top_rate)
    result = check_op_cap("op_select_format", "TomeUnmerge", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(attention_in__, ori_index_a_in__, ori_index_b_in__, topk_indice_in__, arg_max_in__, unzip_token_out_, top_rate=0.5, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(attention_in__, ori_index_a_in__, ori_index_b_in__, topk_indice_in__, arg_max_in__, unzip_token_out_, top_rate)
    result = check_op_cap("get_op_specific_info", "TomeUnmerge", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
