...
from impl.util.util_tik_comm_func import tik_vnchwconv_nano
from impl.dynamic.pad_v3 import PadV3Init
...
def op_select_format(x, paddings, y, kernel_name="pad"):
    ...
    x_dtype = x.get("ori_dtype")
    paddings_value = paddings.get("const_value")
    is_support_hd = False
    if paddings_value and x_dtype not in ("int8",):
    ...
    base_data_type = \
        ["float", "float16", "int16", "int32", "int64", "uint16", "uint32", "uint64", "int8"]
...
def check_supported(x, paddings, y, kernel_name="pad"):
    ...
    check supported
    """
    black_list_shape = [[1, 3200, 256], [1, 3204, 256], [1, 3208, 256], [1, 3216, 256], [1, 3232, 256], [1, 3264, 256],
                        [1, 3328, 256], [1, 3456, 256]]
...
class PadInit(OpBase):
    ...
    def __init__(self, kernel_name, max_shape_len=8):
        ...
        self.is_v220 = tbe_platform.api_check_support("tik.data_move_pad")
        ...
        self.block_num = tbe_platform.get_block_size() // self.inner_bytes_size
        ...
        if check_support_block_size_16():
            # ascned035's size is less than four times with 128kb
            self.copy_num = 1600
    ...
    def core_scedule_args(self, core_idx):
        ...
        with self.tik_instance.if_scope(self.tiling_input_dim_cut_axis == 0):
            ...
            self.core_outer_num.set_as(
                (core_outer_all + self.get_running_core_num() - 1) // self.get_running_core_num())
            ...
            with self.tik_instance.if_scope(self.core_outer_start + self.core_outer_num > core_outer_all):
                ...
                with self.tik_instance.if_scope(self.core_outer_num < 0):
        ...
        with self.tik_instance.if_scope(self.tiling_input_dim_cut_axis == 1):
            ...
            self.core_outer_num.set_as(
                (core_outer_all + self.get_running_core_num() - 1) // self.get_running_core_num())
            self.core_outer_start.set_as(core_idx * self.core_outer_num)
            with self.tik_instance.if_scope(core_outer_all % self.get_running_core_num() != 0):
                with self.tik_instance.if_scope(core_idx >= core_outer_all % self.get_running_core_num()):
                    self.core_outer_num.set_as(self.core_outer_num - 1)
                    self.core_outer_start.set_as(core_idx * self.core_outer_num +
                                                 core_outer_all % self.get_running_core_num())
        with self.tik_instance.if_scope(self.tiling_input_dim_cut_axis == 2):
            ...
            with self.tik_instance.if_scope(self.tiling_output_shape[-1] *
                                            self.tiling_output_shape[-2] < self.block_num):
    ...
    def tiling_args(self):
        ...
        with self.tik_instance.new_stmt_scope():
            ...
            # tiling data有明确个数，使用data_move无内存越界风险。tiling data个数变化时需要修改此处搬移量
            ...
            self.set_running_core_num(tiling_ub[26])
    ...
    def get_output_outer_idx(self, in_idx, outer_num=5):
        ...
        for i in range(outer_num):
            output_idx = output_idx + \
                output_list[i] * self.output_offset[i + 1]
        return output_idx
    ...
    def data_move(self, gm_src_info, gm_dst_info, copy_len, used_ub):
        ...
        burst_len_pad = copy_len * self.inner_bytes_size
        if self.is_v220:
            self.tik_instance.data_move_pad(used_ub, input_gm[input_offset], 1, burst_len_pad, 0, 0)
            self.tik_instance.data_move_pad(output_gm[output_offset], used_ub, 1, burst_len_pad, 0, 0)
        else:
    ...
    def data_move_with_mask_less_block(self, gm_src_info, gm_dst_info, copy_len, used_ub, ub_one_block):
        ...
        burst_len_pad = copy_len * self.inner_bytes_size
        if self.is_v220:
            self.tik_instance.data_move_pad(ub_one_block, input_gm[input_offset], 1, burst_len_pad, 0, 0)
        else:
            self.tik_instance.data_move(ub_one_block, input_gm[input_offset], 0, 1, bursn_len, 0, 0)
        if check_support_block_size_16():
            vnchw_src_list0 = [ub_one_block] * Constant.TRANS_MIN_BLKS
            vnchw_dst_list0 = [0] * Constant.TRANS_MIN_BLKS
            vnchw_src_list1 = [ub_one_block] * Constant.TRANS_MIN_BLKS
            vnchw_dst_list1 = [0] * Constant.TRANS_MIN_BLKS
            for y in range(16):
                if y % 2 == 0:
                    vnchw_dst_list0[y] = used_ub[16 * (y // 2)]
                    vnchw_dst_list1[y] = used_ub[16 * 8 + 16 * (y // 2)]
                else:
                    vnchw_dst_list0[y] = used_ub[8 + 16 * ((y - 1) // 2)]
                    vnchw_dst_list1[y] = used_ub[(16 + 1) * 8 + 16 * ((y - 1) // 2)]
            self.tik_instance.vnchwconv(False, False, vnchw_dst_list0, vnchw_src_list0, 1, 0, 0)
            self.tik_instance.vnchwconv(False, False, vnchw_dst_list1, vnchw_src_list1, 1, 0, 0)
        else:
        ...
        src_offset = self.block_num
        if check_support_block_size_16():
            src_offset = self.block_num * 2
        self.tik_instance.data_move(used_ub[copy_len * self.block_num:], ub_one_block[src_offset:], 0, 1,
                                    burst_len, 0, 0) # UBtoUB无需适配data_move_pad
        if check_support_block_size_16():
            each_stride = 16
            transpose_offsets = Constant.TRANS_MIN_BLKS * Constant.TRANS_MIN_BLKS
            src_offsets = 0
            tik_vnchwconv_nano(self.tik_instance, used_ub, used_ub, transpose_offsets, each_stride, src_offsets, 1)
        else:
            ...
            self.tik_instance.vnchwconv(False, False, vnchw_dst_list, vnchw_src_list, 1, 0, 0)
        if self.is_v220:
            self.tik_instance.data_move_pad(output_gm[output_offset], used_ub[Constant.TRANS_MIN_BLKS * \
                                            Constant.TRANS_MIN_BLKS], 1, burst_len_pad, 0, 0)
        else:
            self.tik_instance.data_move(output_gm[output_offset], used_ub[Constant.TRANS_MIN_BLKS * \
                                        Constant.TRANS_MIN_BLKS], 0, 1, bursn_len, 0, 0)
    ...
    def do_pad_with_move_cut_inner(self):
        ...
        with self.tik_instance.if_scope(tik.all(scalar_copy_num > self.core_outer_num, self.core_outer_num != 0)):
    ...
    def do_pad_with_move_cut_outer_default(self, is_last_align=False):
        ...
        if check_support_block_size_16():
            self.tik_instance.vector_dup(self.block_num * 8, ping_data_ub_one_block[8:], self.pad_scalar, 1, 1, 8)
            self.tik_instance.vector_dup(self.block_num * 8, pang_data_ub_one_block[8:], self.pad_scalar, 1, 1, 8)
        else:
        ...
        def run_outer_by_outer(second_dim_start, do_inner_num, do_outer_num, align_tail, disable_sync_mte3=False):
            ...
            def _run_one_outer(_outer_num_idx, ub_list):
                ...
                with self.tik_instance.new_stmt_scope(disable_sync=True):
                    with self.tik_instance.for_range(0, do_outer_num) as _copy_idx:
                        ...
                        burst_len_pad = (do_inner_num * third_dim_input_num) * self.inner_bytes_size
                ...
                if not check_support_block_size_16():
                    ...
                    self.tik_instance.data_move(vnchw_output_data_ub[dst_offset], vnchw_data_ub[src_offset], 0,
                                                burst_num, burst_len, src_stride, dst_stride) # UBtoUB无需适配data_move_pad
                    ...
                    # step2. vnchw 16 dims origin_data_ub to vnchw_data_ub
                    origin_data_ub_list0 = [
                        origin_data_ub[i * max_output_size] for i in range(0, Constant.TRANS_MIN_BLKS)]
                    vnchw_data_ub_list0 = [0] * Constant.TRANS_MIN_BLKS
                    for y in range(16):
                        if y % 2 == 0:
                            vnchw_data_ub_list0[y] = vnchw_data_ub[16 * (y // 2)]
                        else:
                            vnchw_data_ub_list0[y] = vnchw_data_ub[8 + 16 * ((y - 1) // 2)]
                    self.tik_instance.vnchwconv(False, False, vnchw_data_ub_list0, origin_data_ub_list0, vnchw_repeat0,
                                                vnchw_dst_stride0, vnchw_src_stride0)

                    origin_data_ub_list1 = [
                        origin_data_ub[max_output_size // 2 + i * max_output_size]
                        for i in range(0, Constant.TRANS_MIN_BLKS)]
                    vnchw_data_ub_list1 = [0] * Constant.TRANS_MIN_BLKS
                    for y in range(16):
                        if y % 2 == 0:
                            vnchw_data_ub_list1[y] = vnchw_data_ub[16 * max_output_size // 2 + 16 * (y // 2)]
                        else:
                            vnchw_data_ub_list1[y] = vnchw_data_ub[(16 + 1) * max_output_size // 2 + 16 *
                                                                   ((y - 1) // 2)]
                    self.tik_instance.vnchwconv(False, False, vnchw_data_ub_list1, origin_data_ub_list1, vnchw_repeat0,
                                                vnchw_dst_stride0, vnchw_src_stride0)
                    ...
                    src_offset = 0
                    src_stride = 0
                    burst_len = third_dim_input_num * 2
                    dst_offset = pad_left * self.block_num * 2
                    dst_stride = (pad_left + pad_right) * 2
                    self.tik_instance.data_move(vnchw_output_data_ub[dst_offset], vnchw_data_ub[src_offset], 0,
                                                burst_num, burst_len, src_stride, dst_stride) # UBtoUB无需适配data_move_pad
                    # step4. vnchw vnchw_output_data_ub to 16 dims origin_output_data_ub
                    vnchw_output_data_ub_list0 = [vnchw_output_data_ub[i * 16] for i in range(8)] + \
                                                 [vnchw_output_data_ub[8 + i * 16] for i in range(8)]
                    origin_output_data_ub_list0 = [0] * Constant.TRANS_MIN_BLKS
                    for y in range(16):
                        if y % 2 == 0:
                            origin_output_data_ub_list0[y] = origin_output_data_ub[max_output_size * (y // 2)]
                        else:
                            origin_output_data_ub_list0[y] = origin_output_data_ub[max_output_size * 8 +
                                                                                   max_output_size * ((y - 1) // 2)]
                    self.tik_instance.vnchwconv(False, False, origin_output_data_ub_list0, vnchw_output_data_ub_list0,
                                                vnchw_repeat1, vnchw_dst_stride1, vnchw_src_stride1)
                ...
                with self.tik_instance.if_scope(do_inner_num * third_dim_output_num % self.block_num != 0):
                    with self.tik_instance.new_stmt_scope(disable_sync=disable_sync_mte3):
                        with self.tik_instance.for_range(0, do_outer_num - 1) as _copy_idx:
                            ...
                            burst_len_pad = (do_inner_num * third_dim_output_num) * self.inner_bytes_size
                            dst_offset = \
                                output_outer_offset + \
                                (self.tiling_pading_value[self.shape_len - 2][0] + \
                                second_dim_start + _copy_idx * do_inner_num) * self.output_offset[self.shape_len - 1]
                            if self.is_v220:
                                self.tik_instance.data_move_pad(self.output_gm[dst_offset],
                                                                origin_output_data_ub[_copy_idx * max_output_size], 1,
                                                                burst_len_pad, 0, 0)
                            else:
                        ...
                        burst_len_pad = (do_inner_num * third_dim_output_num) * self.inner_bytes_size
                        dst_offset = output_outer_offset + (self.tiling_pading_value[self.shape_len - 2][0]
                        ...
                        if self.is_v220:
                            ...
                            self.tik_instance.data_move(self.output_gm[dst_offset],
                                                        origin_output_data_ub[(do_outer_num - 1) * max_output_size], 0,
                                                        1, burst_len, 0, 0)

                    # step6. process tail for the last line
                    with self.tik_instance.if_scope(align_tail != 0):
                        if check_support_block_size_16():
                            src_offset = (do_inner_num * third_dim_output_num - 16) * 16
                            vnchw_output_data_ub_list1 = [vnchw_output_data_ub[
                                                         src_offset + i * 16] for i in range(8)] + \
                                                         [vnchw_output_data_ub[src_offset + 8 + i * 16]
                                                             for i in range(8)]
                            origin_output_data_ub_list1 = [0] * Constant.TRANS_MIN_BLKS
                            for y in range(16):
                                if y % 2 == 0:
                                    origin_output_data_ub_list1[y] = origin_output_tail_data_ub[16 * (y // 2)]
                                else:
                                    origin_output_data_ub_list1[y] = origin_output_tail_data_ub[8 + 16 * ((y - 1) // 2)]
                            self.tik_instance.vnchwconv(False, False, origin_output_data_ub_list1,
                                                        vnchw_output_data_ub_list1, 1, 0, 0)
                        else:
                            origin_output_data_ub_list = \
                        ...
                        burst_len = 1
                        burst_len_pad = align_tail * self.inner_bytes_size
                        dst_offset = output_outer_offset + (self.tiling_pading_value[self.shape_len - 2][0]
                               + second_dim_start + do_outer_num * do_inner_num) \
                            * self.output_offset[self.shape_len - 1] - self.block_num
                        if self.is_v220:
                            self.tik_instance.data_move_pad(self.output_gm[dst_offset],
                                                            origin_output_tail_data_ub[(do_outer_num - 1) * 16], 1,
                                                            burst_len_pad, 0, 0)
                        else:
                            self.tik_instance.data_move(self.output_gm[dst_offset],
            ...
            if check_support_block_size_16():
                self.tik_instance.vector_dup(self.block_num * 8, vnchw_output_data_ub_ping, self.pad_scalar,
                                             max_line_in_ub * max_output_size // self.block_num // 16, 1, 8)
            else:
                self.tik_instance.vector_dup(self.block_num * 8, vnchw_output_data_ub_ping, self.pad_scalar,
        ...
        with self.tik_instance.for_range(0, first_dim_loop_num_ceil) as first_dim_idx:
            ...
            with self.tik_instance.for_range(0, second_dim_loop_num_ceil) as second_dim_idx:
                ...
                def _run_one_outer(_outer_num_idx, ub_list):
                    ...
                    _, _, _, origin_output_data_ub, origin_output_tail_data_ub = ub_list
                    input_outer_idx = _outer_num_idx + self.core_outer_start
                    input_gm_offset = input_outer_idx * \
                        self.input_offset[self.shape_len - 3]
                    output_outer_offset = self.tik_instance.Scalar(dtype="int64", name="output_outer_offset")
                    output_outer_offset.set_as(
                    ...
                    with self.tik_instance.for_range(0, first_dim_process_num) as _copy_idx:
                        burst_len = \
                            ((second_dim_process_num * third_dim_input_num) +
                             self.block_num - 1) // self.block_num
                        burst_len_pad = second_dim_process_num * third_dim_input_num * self.inner_bytes_size
                        src_offset = \
                            (first_dim_start + _copy_idx) * self.input_offset[self.shape_len - 2] \
                            + second_dim_start * \
                            self.input_offset[self.shape_len - 1]
                        if self.is_v220:
                            self.tik_instance.data_move_pad(origin_data_ub[_copy_idx * max_output_size],
                                                            self.input_gm[input_gm_offset + src_offset], 1,
                                                            burst_len_pad, 0, 0)
                        else:
                            self.tik_instance.data_move(origin_data_ub[_copy_idx * max_output_size],
                                                        self.input_gm[input_gm_offset + src_offset],
                    ...
                    if check_support_block_size_16():
                        ...
                        for y in range(16):
                            if y % 2 == 0:
                                ...
                                vnchw_data_ub_list0[y] = vnchw_data_ub[8 + 16 * ((y - 1) // 2)]
                        self.tik_instance.vnchwconv(False, False, vnchw_data_ub_list0, origin_data_ub_list0,
                                                    vnchw_repeat0, vnchw_dst_stride0, vnchw_src_stride0)
                        ...
                        for y in range(16):
                            if y % 2 == 0:
                                ...
                                vnchw_data_ub_list1[y] = vnchw_data_ub[(16 + 1) * max_output_size // 2 + 16 *
                                                                       ((y - 1) // 2)]
                        self.tik_instance.vnchwconv(False, False, vnchw_data_ub_list1, origin_data_ub_list1,
                                                    vnchw_repeat0, vnchw_dst_stride0, vnchw_src_stride0)
                    else:
                        origin_data_ub_list = [
                            origin_data_ub[i * max_output_size] for i in range(0, Constant.TRANS_MIN_BLKS)
                        ...
                        vnchw_data_ub_list = [vnchw_data_ub[i * 16] for i in range(0, Constant.TRANS_MIN_BLKS)]
                        self.tik_instance.vnchwconv(False, False, vnchw_data_ub_list, origin_data_ub_list,
                                                    vnchw_repeat0, vnchw_dst_stride0, vnchw_src_stride0)
                    ...
                    # step4. vnchw vnchw_output_data_ub to 16 dims origin_output_data_ub
                    origin_output_data_ub_list = \
                        [origin_output_data_ub[i * max_output_size]
                            for i in range(0, Constant.TRANS_MIN_BLKS)]
                    vnchw_output_data_ub_list = \
                        [vnchw_output_data_ub[i * 16]
                            for i in range(0, Constant.TRANS_MIN_BLKS)]
                    self.tik_instance.vnchwconv(False, False, origin_output_data_ub_list, vnchw_output_data_ub_list,
                                                vnchw_repeat1, vnchw_dst_stride1, vnchw_src_stride1)
                    ...
                    with self.tik_instance.for_range(0, first_dim_process_num) as _copy_idx:
                        ...
                        burst_len_pad = (second_dim_process_num * third_dim_output_num) * self.inner_bytes_size
                        dst_offset = \
                            output_outer_offset \
                    ...
                    if not is_last_output_algin:
                        ...
                        origin_output_tail_data_ub_list = \
                            [origin_output_tail_data_ub[i * 16]
                                for i in range(0, Constant.TRANS_MIN_BLKS)]
                        vnchw_output_data_ub_list = \
                        ...
                        with self.tik_instance.for_range(0, first_dim_process_num) as _copy_idx:
                            ...
                            burst_len_pad = copy_tail_offset * self.inner_bytes_size
                            if self.is_v220:
                                self.tik_instance.data_move_pad(self.output_gm[dst_offset],
                                                                origin_output_tail_data_ub[_copy_idx * 16], 1,
                                                                burst_len_pad, 0, 0)
                            else:
                                self.tik_instance.data_move(self.output_gm[dst_offset],
    ...
    def empty_tensor_decorator(self, tiling_func):
        with self.tik_instance.if_scope(functools.reduce(lambda x, y: x * y, self.tiling_input_shape) != 0):
            tiling_func()
    ...
    def pad_compute(self, outer_compile_info=None):
        ...
        self.regist_compute(Constant.MODE0, self.empty_tensor_decorator, self.do_pad_with_move_cut_inner)
        self.regist_compute(Constant.MODE1, self.empty_tensor_decorator, self.do_pad_with_move_cut_outer)
        self.regist_compute(Constant.MODE2, self.empty_tensor_decorator, self.do_pad_with_vnchw_for_last_two_dim)
        self.regist_compute(Constant.MODE3, self.empty_tensor_decorator, self.do_pad_with_vnchw_for_last_three_dim)
        ...
        # x_bytes_size default value is 1, the tiling parameter when input is int8
        # max_output_size default value is 1，the tiling parameter when input is int8
...
def pad(x, paddings, y, kernel_name="pad"):
    ...
    supported_dtype = ("float32", "float16", "bfloat16", "int16", "int32",
                       "int64", "uint16", "uint32", "uint64", "int8")
    ...
    if src_dtype in ("int8", ):
        obj = PadV3Init(x, paddings, None, y, "constant", True, kernel_name)
        obj.init_src_dst_gm((x, paddings), pad_input_idx=0, pad_outnput_idx=0)
        return obj.pad_compute()
    else:
        obj = PadInit(kernel_name, max_shape_len=8)
