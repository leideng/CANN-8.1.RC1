...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['self', 'linear_index', 'pos_idx', 'values']
    output_names = ['self']
...
def _build_args(self_in__, linear_index_in__, pos_idx_in__, values_in__, self_out_, slice_size, accumulate):
    __inputs__ = []
    for arg in [self_in__, linear_index_in__, pos_idx_in__, values_in__]:
        if arg != None:
    ...
    if slice_size != None:
        attr = {}
        attr["name"] = "slice_size"
        attr["dtype"] = "int"
        attr["value"] = slice_size
        __attrs__.append(attr)
    if accumulate != None:
        attr = {}
        attr["name"] = "accumulate"
        attr["dtype"] = "bool"
        attr["value"] = accumulate
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("IndexPutWithSort", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def index_put_with_sort(self_in__, linear_index_in__, pos_idx_in__, values_in__, self_out_, slice_size, accumulate=False, kernel_name="index_put_with_sort", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(self_in__, linear_index_in__, pos_idx_in__, values_in__, self_out_, slice_size, accumulate)
    ...
    origin_func_name = "index_put_with_sort"
    ascendc_src_dir_ex = "index_put_with_sort"
    ascendc_src_dir = "index_put_with_sort"
    ascendc_src_file = "index_put_with_sort.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator IndexPutWithSort, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "IndexPutWithSort"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[self_in__, linear_index_in__, pos_idx_in__, values_in__], origin_outputs = [self_out_],\
...
def op_select_format(self_in__, linear_index_in__, pos_idx_in__, values_in__, self_out_, slice_size, accumulate=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(self_in__, linear_index_in__, pos_idx_in__, values_in__, self_out_, slice_size, accumulate)
    result = check_op_cap("op_select_format", "IndexPutWithSort", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(self_in__, linear_index_in__, pos_idx_in__, values_in__, self_out_, slice_size, accumulate=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(self_in__, linear_index_in__, pos_idx_in__, values_in__, self_out_, slice_size, accumulate)
    result = check_op_cap("get_op_specific_info", "IndexPutWithSort", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
