...
def _indice_unit_tiling(dst, data, indices, step_indices, jump_step, shape_data, shape_indices, dict_out):
    ...
    """
    ir_build = tvm.tir.ir_builder.create()
            ...
            with ir_build.for_range(0, indies_part_num // jump_step, name='row1') as row1:
                ...
                if burst_length_tiling.value >= 1:
                    ...
                    buf_param_var = ir_build.allocate(data.dtype,
                                                      ...
                                                      "half_data_ub",
                                                      scope=tbe_platform.scope_ubuf)
                    ir_build.scope_attr(buf_param_var.asobject(), "double_buffer_scope", 1)
...
def _calculate_last_burst_len(data, ele_num, last_ele_num):
    ...
    if data.dtype == "int32" or data.dtype == "float32":
        burst_len = (ele_num + type_dict.get("dtype32_ele") - 1) // type_dict.get("dtype32_ele")
        burst_len_last = (last_ele_num + type_dict.get("dtype32_ele") - 1) // type_dict.get("dtype32_ele")
    elif data.dtype == "float16":
        burst_len = (ele_num + type_dict.get("dtype16_ele") - 1) // type_dict.get("dtype16_ele")
        burst_len_last = (last_ele_num + type_dict.get("dtype16_ele") - 1) // type_dict.get("dtype16_ele")
    elif data.dtype == "int8" or data.dtype == "uint8":
        burst_len = (ele_num + type_dict.get("dtype8_ele") - 1) // type_dict.get("dtype8_ele")
        burst_len_last = (last_ele_num + type_dict.get("dtype8_ele") - 1) // type_dict.get("dtype8_ele")
...
def _calculate_burst_len_by_dtype(data, ele_num):
    ...
    if data.dtype == "int32" or data.dtype == "float32":
        ...
        burst_len = (ele_num + type_dict.get("dtype64_ele") - 1) // type_dict.get("dtype64_ele")
    return burst_len
...
def _param_ele_per_block_by_dtype(data):
    ...
    if data.dtype == "int32" or data.dtype == "float32":
        ele_per_block = type_dict.get("dtype32_ele")
    elif data.dtype == "float16":
        ele_per_block = type_dict.get("dtype16_ele")
    elif data.dtype == "int8" or data.dtype == "uint8":
        ele_per_block = type_dict.get("dtype8_ele")
...
def _calculate_ele_num_by_dtype(data, ele_num):
    ...
    if data.dtype == "int32" or data.dtype == "float32":
        half_param = int(type_dict.get("dtype32_ele") * ele_num)
        half_param_ub_shape = (half_param,)
    elif data.dtype == "float16":
        half_param = int(type_dict.get("dtype16_ele") * ele_num)
        half_param_ub_shape = (half_param,)
    elif data.dtype == "int8" or data.dtype == "uint8":
        half_param = int(type_dict.get("dtype8_ele") * ele_num)
        half_param_ub_shape = (half_param,)
...
def _reassign_aglined_number(data, ele_num):
    ...
    if data.dtype == "int32" or data.dtype == "float32":
        ele_num = (ele_num // type_dict.get("dtype32_ele")) * type_dict.get("dtype32_ele")
    elif data.dtype == "float16":
        ele_num = (ele_num // type_dict.get("dtype16_ele")) * type_dict.get("dtype16_ele")
    elif data.dtype == "int8" or data.dtype == "uint8":
        ele_num = (ele_num // type_dict.get("dtype8_ele")) * type_dict.get("dtype8_ele")
    return ele_num
...
def _indice_core_len_by_dtype(indices, indice_ele_per_core, indice_last_ele_per_core):
    ...
    if indices.dtype == "int32":
        indice_core_len = (indice_ele_per_core + type_dict.get("dtype32_ele") - 1) // type_dict.get("dtype32_ele")
        indice_core_len_last = (indice_ele_per_core + indice_last_ele_per_core + type_dict.get("dtype32_ele") -
                                1) // type_dict.get("dtype32_ele")
    elif indices.dtype == "int64":
        indice_core_len = (indice_ele_per_core + type_dict.get("dtype64_ele") - 1) // type_dict.get("dtype64_ele")
        indice_core_len_last = (indice_ele_per_core + indice_last_ele_per_core + type_dict.get("dtype64_ele") -
                                1) // type_dict.get("dtype64_ele")
