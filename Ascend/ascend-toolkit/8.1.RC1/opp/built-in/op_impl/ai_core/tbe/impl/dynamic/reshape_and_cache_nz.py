...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['keyIn', 'valueIn', 'keyCacheIn', 'valueCacheIn', 'slotMapping']
...
def _build_args(keyIn_in__, valueIn_in__, keyCacheIn_in__, valueCacheIn_in__, slotMapping_in__, keyCacheOut_out_, valueCacheOut_out_):
    __inputs__ = []
    for arg in [keyIn_in__, valueIn_in__, keyCacheIn_in__, valueCacheIn_in__, slotMapping_in__]:
        if arg != None:
...
@tbe_register.register_operator("ReshapeAndCacheNz", trans_bool_to_s8=False)
...
def reshape_and_cache_nz(keyIn_in__, valueIn_in__, keyCacheIn_in__, valueCacheIn_in__, slotMapping_in__, keyCacheOut_out_, valueCacheOut_out_, kernel_name="reshape_and_cache_nz", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(keyIn_in__, valueIn_in__, keyCacheIn_in__, valueCacheIn_in__, slotMapping_in__, keyCacheOut_out_, valueCacheOut_out_)
    ...
    origin_func_name = "reshape_and_cache_nz"
    ascendc_src_dir_ex = "reshape_and_cache_nz"
    ascendc_src_dir = "reshape_and_cache_nz"
    ascendc_src_file = "reshape_and_cache_nz.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator ReshapeAndCacheNz, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "ReshapeAndCacheNz"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[keyIn_in__, valueIn_in__, keyCacheIn_in__, valueCacheIn_in__, slotMapping_in__], origin_outputs = [keyCacheOut_out_, valueCacheOut_out_],\
...
def op_select_format(keyIn_in__, valueIn_in__, keyCacheIn_in__, valueCacheIn_in__, slotMapping_in__, keyCacheOut_out_, valueCacheOut_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(keyIn_in__, valueIn_in__, keyCacheIn_in__, valueCacheIn_in__, slotMapping_in__, keyCacheOut_out_, valueCacheOut_out_)
    result = check_op_cap("op_select_format", "ReshapeAndCacheNz", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(keyIn_in__, valueIn_in__, keyCacheIn_in__, valueCacheIn_in__, slotMapping_in__, keyCacheOut_out_, valueCacheOut_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(keyIn_in__, valueIn_in__, keyCacheIn_in__, valueCacheIn_in__, slotMapping_in__, keyCacheOut_out_, valueCacheOut_out_)
    result = check_op_cap("get_op_specific_info", "ReshapeAndCacheNz", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
