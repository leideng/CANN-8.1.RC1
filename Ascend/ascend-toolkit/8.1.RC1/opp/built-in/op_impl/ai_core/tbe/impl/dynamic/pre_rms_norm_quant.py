...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'res_in', 'gamma', 'beta']
    output_names = ['y', 'res_out']
...
def _build_args(x_in__, res_in_in__, gamma_in__, beta_in__, y_out_, res_out_out_, epsilon, gemma_mode, high_precision_mode, scale, offset):
    __inputs__ = []
    for arg in [x_in__, res_in_in__, gamma_in__, beta_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [y_out_, res_out_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if gemma_mode != None:
        attr = {}
        attr["name"] = "gemma_mode"
        attr["dtype"] = "bool"
        attr["value"] = gemma_mode
        __attrs__.append(attr)
    if high_precision_mode != None:
        attr = {}
        attr["name"] = "high_precision_mode"
        attr["dtype"] = "bool"
        attr["value"] = high_precision_mode
        __attrs__.append(attr)
    if scale != None:
        attr = {}
        attr["name"] = "scale"
...
@tbe_register.register_operator("PreRmsNormQuant", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def pre_rms_norm_quant(x_in__, res_in_in__, gamma_in__, beta_in__=None, y_out_=None, res_out_out_=None, epsilon=0.0, gemma_mode=False, high_precision_mode=False, scale=1, offset=0, kernel_name="pre_rms_norm_quant", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, res_in_in__, gamma_in__, beta_in__, y_out_, res_out_out_, epsilon, gemma_mode, high_precision_mode, scale, offset)
    ...
    origin_func_name = "pre_rms_norm_quant"
    ascendc_src_dir_ex = "pre_rms_norm_quant"
    ascendc_src_dir = "pre_rms_norm_quant"
    ascendc_src_file = "pre_rms_norm_quant.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator PreRmsNormQuant, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "PreRmsNormQuant"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, res_in_in__, gamma_in__, beta_in__], origin_outputs = [y_out_, res_out_out_],\
...
def op_select_format(x_in__, res_in_in__, gamma_in__, beta_in__=None, y_out_=None, res_out_out_=None, epsilon=0.0, gemma_mode=False, high_precision_mode=False, scale=1, offset=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, res_in_in__, gamma_in__, beta_in__, y_out_, res_out_out_, epsilon, gemma_mode, high_precision_mode, scale, offset)
    result = check_op_cap("op_select_format", "PreRmsNormQuant", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, res_in_in__, gamma_in__, beta_in__=None, y_out_=None, res_out_out_=None, epsilon=0.0, gemma_mode=False, high_precision_mode=False, scale=1, offset=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, res_in_in__, gamma_in__, beta_in__, y_out_, res_out_out_, epsilon, gemma_mode, high_precision_mode, scale, offset)
    result = check_op_cap("get_op_specific_info", "PreRmsNormQuant", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
