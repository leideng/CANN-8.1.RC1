...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'weight_scale', 'activate_scale', 'bias']
    output_names = ['y']
...
def _build_args(x_in__, weight_scale_in__, activate_scale_in__, bias_in__, y_out_, output_dtype):
    __inputs__ = []
    for arg in [x_in__, weight_scale_in__, activate_scale_in__, bias_in__]:
        if arg != None:
...
@tbe_register.register_operator("DequantBias", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def dequant_bias(x_in__, weight_scale_in__, activate_scale_in__=None, bias_in__=None, y_out_=None, output_dtype=0, kernel_name="dequant_bias", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_scale_in__, activate_scale_in__, bias_in__, y_out_, output_dtype)
    ...
    origin_func_name = "dequant_bias"
    ascendc_src_dir_ex = "dequant_bias"
    ascendc_src_dir = "dequant_bias"
    ascendc_src_file = "dequant_bias.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator DequantBias, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "DequantBias"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, weight_scale_in__, activate_scale_in__, bias_in__], origin_outputs = [y_out_],\
...
def op_select_format(x_in__, weight_scale_in__, activate_scale_in__=None, bias_in__=None, y_out_=None, output_dtype=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_scale_in__, activate_scale_in__, bias_in__, y_out_, output_dtype)
    result = check_op_cap("op_select_format", "DequantBias", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, weight_scale_in__, activate_scale_in__=None, bias_in__=None, y_out_=None, output_dtype=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_scale_in__, activate_scale_in__, bias_in__, y_out_, output_dtype)
    result = check_op_cap("get_op_specific_info", "DequantBias", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
