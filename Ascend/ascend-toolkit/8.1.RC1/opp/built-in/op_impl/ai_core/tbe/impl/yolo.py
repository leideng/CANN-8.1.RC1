...
class ComputerCommon(ShapeInfo):
    ...
    def newton_iteration(self, tik_inst, rec, ori, params):
        """
        ...
        tmp = tik_inst.Tensor(rec.dtype, rec.shape,
                              scope=tbe_platform.scope_ubuf, name="tmp")
    ...
    def self_div(self, tik_inst, dividend, divisor, params):
        """
        self define divide
        Parameters
        ----------
        ...
        if tbe_platform.get_soc_spec("SHORT_SOC_VERSION") not in ("Ascend310",):
            ...
            rec = tik_inst.Tensor(divisor.dtype, (dlen // self.dtype_size, ),
                                  scope=tbe_platform.scope_ubuf, name="rec")
            tik_inst.vec_rec(mask, rec, divisor, repeats, 8, 8)
            self.newton_iteration(tik_inst, rec, divisor, [mask, 2])
...
class SigMovCommon(ComputerCommon):
    ...
    def sig_mov_tiling(self, available_ub):
        ...
        if available_ub - 256 >= chn_ceil_len:
            ...
            self.tiling['chn_loop'] = chn_ceil_len // self.tiling.get('data')
            self.tiling['tail'] = chn_ceil_len % self.tiling.get('data')
            if self.tiling['tail'] > 0:
        ...
        with self.tik_inst.if_scope(idx['elem'] < 4):
            ...
            with self.tik_inst.if_scope(idx['elem'] == 3):
                ofst_idx.set_as(idx['box'] * self.tiling['box_num'] +
                                2 * self.param['boxes'])
        with self.tik_inst.if_scope(idx['elem'] == 4):
            ofst_idx.set_as(idx['box'] * self.tiling.get('box_num') * self.hw_size)
        with self.tik_inst.if_scope(idx['elem'] > 4):
...
class SigmoidComputer(SigMovCommon):
    ...
    def sigmoid_computer(self, cmpt_ub, data_len):
        ...
        with self.tik_inst.new_stmt_scope():
            self.self_div(self.tik_inst, tmp_ub_2, tmp_ub_1, [mask, dlen])
        ...
        with self.tik_inst.new_stmt_scope():
            self.self_div(self.tik_inst, tmp_ub_1, tmp_ub_3, [mask, dlen])
...
class SoftmaxComputer(ComputerCommon):
    ...
    def softmax_create_ub(self, data_len):
        ...
        sfmx_ub = self.tik_inst.Tensor(self.param['dtype'], (sfmx_size, ),
                                       scope=tbe_platform.scope_ubuf, name="sfmx_ub")
        mid_ub = self.tik_inst.Tensor(self.param['dtype'], (mid_size, ),
                                      scope=tbe_platform.scope_ubuf, name="mid_ub")
    ...
    def softmax_vop(self, sfmx_ub, mid_ub, compute_params):
        """
        ...
        NONE
        """
        stride_len, mask, op_type, ofst = compute_params
        repeats = 255
    ...
    def softmax_compute(self, sfmx_ub, mid_ub, compute_params):
        """
        ...
        NONE
        """
        stride_len, proc_len, ofst = compute_params
        ...
        with self.tik_inst.for_range(0, self.sfmx_num) as sfmx_i:
            self.self_div(self.tik_inst,
                          sfmx_ub[ofst+sfmx_i*stride_len//self.dtype_size],
                          mid_ub[ofst], [mask, proc_len])

    def softmax_vop_2(self, sfmx_ub, mid_ub, compute_params):
        """
        ...
        NONE
        """
        mask, ofst, stride_len, proc_len, op_type = compute_params
    ...
    def softmax_compute_2(self, sfmx_ub, mid_ub, compute_params):
        """
        execute softmax option, usage for stride_len <= 255 * 32
        Parameters
        ----------
        ...
        NONE
        """
        mask, stride_len, proc_len, ofst = compute_params
        fp_mini = FP32_MINI if self.param['dtype'] == "float32" else FP16_MINI
        repeats = (proc_len + 255) // 256
        self.tik_inst.vec_dup(mask, mid_ub[ofst], fp_mini, repeats, 8)
        self.softmax_vop_2(sfmx_ub, mid_ub, [mask, ofst, stride_len, proc_len, 'max'])
        self.softmax_vop_2(sfmx_ub, mid_ub, [mask, ofst, stride_len, proc_len, 'sub'])
        self.softmax_vop_2(sfmx_ub, mid_ub, [mask, ofst, stride_len, proc_len, 'exp'])
        self.tik_inst.vec_dup(mask, mid_ub[ofst], 0, repeats, 8)
        self.softmax_vop_2(sfmx_ub, mid_ub, [mask, ofst, stride_len, proc_len, 'add'])
    ...
    def run_compute(self, idx_dic, sfmx_len):
        ...
        if sfmx_len <= 8160:
            ...
            for i in range(0, loop):
                if i == loop-1 and tail > 0:
                    self.softmax_compute(sfmx_ub, mid_ub,
                                         [sfmx_len, tail, i * 256 // self.dtype_size])
                else:
                    self.softmax_compute(sfmx_ub, mid_ub,
                                         [sfmx_len, 256, i * 256 // self.dtype_size])
        else:
            loop = sfmx_len // 65280
            tail = sfmx_len % 65280
            if tail > 0:
            ...
            for i in range(0, loop):
                if i == loop-1 and tail > 0:
                    ...
                    tail_tail = tail % 256
                    if tail_len > 0:
                        compute_params = [128, sfmx_len, tail_len, i * 65280 // self.dtype_size]
                    if tail_tail > 0:
                        compute_params = [tail_tail // self.dtype_size, sfmx_len, tail_tail,
                                          (i * 65280 + tail_len) // self.dtype_size]
                else:
                    compute_params = [128, sfmx_len, 65280, i * 65280 // self.dtype_size]
                self.softmax_compute_2(sfmx_ub, mid_ub, compute_params)
    ...
    def __init__(self, shape_info, param_info):
        ...
        self.yolo_din = \
            self.tik_inst.Tensor(dtype, (batch+batch_padding,
                                         ...
                                         height, width),
                                 scope=tbe_platform.scope_gm, name="yolo_din")
        ...
        self.crd_dout = \
            self.tik_inst.Tensor(dtype, (batch, boxes*coords,
                                 ...
                                 scope=tbe_platform.scope_gm, name="crd_dout")

        self.obj_dout = \
            self.tik_inst.Tensor(dtype, (batch, ceil_x(boxes*height*width *
                                                       2+32, 32)//2),
                                 scope=tbe_platform.scope_gm, name="obj_dout")

        self.cls_dout = \
            self.tik_inst.Tensor(dtype, (batch, classes,
                                         ceil_x(boxes*height*width *
                                                2+32, 32)//2),
                                 scope=tbe_platform.scope_gm, name="cls_dout")
