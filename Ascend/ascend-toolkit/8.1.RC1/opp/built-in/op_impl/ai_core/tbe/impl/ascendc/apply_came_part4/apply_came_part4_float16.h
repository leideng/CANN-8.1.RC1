...
/*!
 * \file apply_came_part4_float16.h
 * \brief
 */
#ifndef _ASCENDC_APPLY_CAME_PART4_FLOAT16_H_
#define _ASCENDC_APPLY_CAME_PART4_FLOAT16_H_
...
template <typename T>
class ApplyCamePart4Float16 {
public:
    __aicore__ inline ApplyCamePart4Float16() {}
...
private:
    ...
    __aicore__ inline void MulRC(LocalTensor<float> &dst, LocalTensor<float> &r, LocalTensor<float> &c, int64_t numR, int64_t numC);
...
private:
    ...
    TBuf<QuePosition::VECCALC> ub3Buf_;
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::Init(GM_ADDR paramIn, GM_ADDR m, GM_ADDR rIn, GM_ADDR cIn, 
                                                      GM_ADDR weight_decay, GM_ADDR lr, GM_ADDR beta3, GM_ADDR sum_r, 
                                                      GM_ADDR sum_u_r, GM_ADDR sum_u_c, GM_ADDR sum_u_rc, GM_ADDR global_shape,
                                                      GM_ADDR paramOut, GM_ADDR rOut, GM_ADDR cOut,
                                                      GM_ADDR workspace, const ApplyCamePart4TilingData* tilingData)
{
    ...
    sumurGm_.SetGlobalBuffer((__gm__ float*)sum_u_r + GetBlockIdx() * rNumPerCore_);
    sumucGm_.SetGlobalBuffer((__gm__ float*)sum_u_c + GetBlockIdx() * cNumPerCore_);
    ...
    // scalar input
    weightDecayGm_.SetGlobalBuffer((__gm__ float*)weight_decay);
    lrGm_.SetGlobalBuffer((__gm__ float*)lr);
    beta3Gm_.SetGlobalBuffer((__gm__ float*)beta3);
    sumurcGm_.SetGlobalBuffer((__gm__ float*)sum_u_rc);
    ...
    pipe_.InitBuffer(inQueR_, 1, (rBufferLength * sizeof(T) + ONE_BLK_SIZE - 1) / ONE_BLK_SIZE * ONE_BLK_SIZE);
    pipe_.InitBuffer(inQueC_, 1, (cBufferLength * sizeof(T) + ONE_BLK_SIZE - 1) / ONE_BLK_SIZE * ONE_BLK_SIZE);
    // for cast
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::CalcScalarInput()
{
    ...
    // copy sum_r
    if (sum_r_ != nullptr) {
        sumRGm_.SetGlobalBuffer((__gm__ float*)sum_r_);
    ...
    // copy global_shape
    int64_t INT64_PER_BLOCK = ONE_BLK_SIZE / sizeof(int64_t);
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::ParseTilingData(const ApplyCamePart4TilingData* tilingData)
{
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::Process()
{
    if (g_coreType == AIC) {
        return;
    }
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::ProcessR()
{
    ...
    if (GetBlockIdx() != rCoreNumToUse_ - 1) {
        ...
        if (rNumTailLoopLast_ != 0) {
            ...
            sumurGm_.SetGlobalBuffer((__gm__ float*)sum_u_r_ + rLoopCountTailCore_ * rNumTailPerLoop_);
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::CopyInR(int iter, int64_t rNumPerLoop)
{
    // copy r gm -> ub
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::CopyInSumur(int iter, int64_t rNumPerLoop)
{
    // copy sum_u_r/sum_u_c gm -> ub
    LocalTensor<float> sumurInput = sumurcQue_.AllocTensor<float>();
    int64_t ONCE_COPY_NUM = ONE_BLK_SIZE / sizeof(float);
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::ComputeR(int64_t rNumPerLoop)
{
    ...
    Cast(rLocal, rInput, RoundMode::CAST_NONE, rNumPerLoop);
    LocalTensor<float> outCast = castOutRBuf_.Get<float>();
    PipeBarrier<PIPE_V>();
    // calc: beta3 * r/c
    Muls(outCast, rLocal, beta3_, rNumPerLoop);
    PipeBarrier<PIPE_V>();

    // calc: beta3 * r + (1 - beta3) * sum_u_r
    float scalarVal = (1 - beta3_) / M_;
    Axpy(outCast, sumurInput, scalarVal, rNumPerLoop);
    PipeBarrier<PIPE_V>();
    Cast(Output, outCast, RoundMode::CAST_RINT, rNumPerLoop);
    PipeBarrier<PIPE_V>();
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::CopyOutR(int64_t iter, int64_t rNumPerLoop)
{
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::ProcessC()
{
    ...
    // tail core
        ...
        if (cNumTailLoopLast_ != 0) {
            ...
            sumucGm_.SetGlobalBuffer((__gm__ float*)sum_u_c_ + cLoopCountTailCore_ * cNumTailPerLoop_);
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::CopyInC(int iter, int64_t cNumPerLoop)
{
    // copy c gm -> ub
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::CopyInSumuc(int iter, int64_t cNumPerLoop)
{
    // copy sum_u_c gm -> ub
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::ComputeC(int64_t cNumPerLoop)
{
    ...
    Cast(cLocal, cInput, RoundMode::CAST_NONE, cNumPerLoop);
    LocalTensor<float> outCast = castOutCBuf_.Get<float>();
    PipeBarrier<PIPE_V>();
    // calc: beta3 * c
    Muls(outCast, cLocal, beta3_, cNumPerLoop);
    PipeBarrier<PIPE_V>();

    // calc: beta3 * c + (1 - beta3) * sum_u_c
    float scalarVal = (1 - beta3_) / N_;
    PipeBarrier<PIPE_V>();
    Axpy(outCast, sumucInput, scalarVal, cNumPerLoop);
    PipeBarrier<PIPE_V>();
    Cast(Output, outCast, RoundMode::CAST_RINT, cNumPerLoop);
    PipeBarrier<PIPE_V>();
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::CopyOutC(int64_t iter, int64_t cNumPerLoop)
{
    LocalTensor<T> Output = rcOutQue_.DeQue<T>();
    uint16_t blockCount = 1;
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::ProcessParam() {
    ...
    float denominator = beta3_ * sumR_  / N_ + (1 - beta3_) * sum_u_rc_ / (M_ * N_);
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::ProcessTailCoreParam()
{
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::CopyInNotAlignedParamr(int loopIdx, int64_t dataCount)
{
    uint16_t blockCount = 1;
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::ProcessPerCoreParam()
{
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::CalcParam(int rLoopIdx, int cLoopIdx, int64_t curRNumInLoop, int64_t curCNumInLoop)
{
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::CopyInNotAlignedParam(int rLoopIdx, int cLoopIdx,
                                                               int64_t curRNumInLoop, int64_t curCNumInLoop) {
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::CopyInNotAlignedm(int rLoopIdx, int cLoopIdx,
                                                            int64_t curRNumInLoop, int64_t curCNumInLoop) {
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::ComputeParam(int64_t curRNumInLoop, int64_t curCNumInLoop)
{
    ...
    // cast r
    LocalTensor<float> rLocalCast = castInRBuf_.Get<float>();
    Cast(rLocalCast, rRcLocalTensor_, RoundMode::CAST_NONE, curRNumInLoop);
    PipeBarrier<PIPE_V>();

    // cast c
    LocalTensor<float> cLocalCast = castInCBuf_.Get<float>();
    Cast(cLocalCast, cLocal, RoundMode::CAST_NONE, curCNumInLoop);
    PipeBarrier<PIPE_V>();

    // cast m
    LocalTensor<float> mLocalCast = castmBuf_.Get<float>();
    Cast(mLocalCast, mLocal, RoundMode::CAST_NONE, dataCount);
    PipeBarrier<PIPE_V>();

    // cast param
    LocalTensor<float> paramInLocalCast = castParamBuf_.Get<float>();
    Cast(paramInLocalCast, paramInLocal, RoundMode::CAST_NONE, dataCount);
    PipeBarrier<PIPE_V>();
    ...
    if ((curRNumInLoop % HALF_CALC_SIZE != 0)
        || (curCNumInLoop % HALF_CALC_SIZE != 0)) {
            CalcRcCycleMode(ub3, cLocalCast, rLocalCast, curRNumInLoop, curCNumInLoop);
    } else {
        // ub1: r --> broadcast
        BroadcastR(ub1, rLocalCast, curRNumInLoop, curCNumInLoop);
        PipeBarrier<PIPE_V>();
        // ub2: r --> broadcast and transpose
        TransposeR(ub2, ub1, curRNumInLoop, curCNumInLoop);
        PipeBarrier<PIPE_V>();
        // ub3: r*c
        MulRC(ub3, ub2, cLocalCast, curRNumInLoop, curCNumInLoop);
    }
    PipeBarrier<PIPE_V>();
    // ub1: rc*coefficient
    ...
    PipeBarrier<PIPE_V>();
    // lr*m
    Muls(mLocalCast, mLocalCast, lr_, dataCount);
    PipeBarrier<PIPE_V>();
    // (m*lr) / ub2
    Div(ub2, mLocalCast, ub2, dataCount);
    PipeBarrier<PIPE_V>();

    // (1 - lr * weight_decay_)
    float mScalar = 1 - lr_ * weight_decay_;
    // paramInLocal: (1 - lr * weight_decay) * param
    Muls(paramInLocalCast, paramInLocalCast, mScalar, dataCount);
    PipeBarrier<PIPE_V>();

    // paramInLocal - ub2
    Sub(paramInLocalCast, paramInLocalCast, ub2, dataCount);
    // cast paramOut and DataCopy
    PipeBarrier<PIPE_V>();
    Cast(paramOutLocal, paramInLocalCast, RoundMode::CAST_RINT, dataCount);
    PipeBarrier<PIPE_V>();
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::CalcRcCycleMode(LocalTensor<float> &dst, LocalTensor<float> &src,
                                                          LocalTensor<float> &srcScalar,
                                                          int64_t curRNumInLoop, int64_t curCNumInLoop)
{
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::CopyOutParam(int rLoopIdx, int cLoopIdx, int64_t curRNumInLoop, int64_t curCNumInLoop)
{
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::CopyInNotAlignedParamc(int loopIdx, int64_t dataCount)
{
    uint16_t blockCount = 1;
    uint16_t blockLen = dataCount * sizeof(T);
    uint16_t srcStride = 0;
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::CopyInm(int rLoopIdx, int cLoopIdx, int64_t curRNumInLoop, int64_t curCNumInLoop)
{
    LocalTensor<T> mLocal = inQuem_.AllocTensor<T>();
    int64_t startRowNum = rRcBlockOffset_ + rLoopIdx * rRcNumPerLoop_;
    int64_t startColumnNum = cLoopIdx * cRcNumPerLoop_;
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::CopyInParam(int rLoopIdx, int cLoopIdx, int64_t curRNumInLoop, int64_t curCNumInLoop)
{
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::MulRC(LocalTensor<float> &dst,
                                                LocalTensor<float> &r, LocalTensor<float> &c,
                                                int64_t numR, int64_t numC)
{
    // r:(numR, numC)  c:(numC,)
    int64_t rcStride = (numC * sizeof(float)) / ONE_BLK_SIZE;
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::BroadcastR(LocalTensor<float> &dst, LocalTensor<float> &src, int64_t numR, int64_t numC)
{
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::TransposeR(LocalTensor<float> &dst, LocalTensor<float> &src, int64_t numR, int64_t numC)
{
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::CopyInParamc(int loopIdx, int64_t dataCount) {
    LocalTensor<T> cLocal = inQueC_.AllocTensor<T>();
    int64_t offset = loopIdx * cRcNumPerLoop_;
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::CopyInParamr(int loopIdx, int64_t dataCount)
{
    LocalTensor<T> rLocal = inQueR_.AllocTensor<T>();
    int64_t offset = rRcBlockOffset_ + loopIdx * rRcNumPerLoop_;
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::InitForCalcParam()
{
    ...
    pipe_.InitBuffer(castParamBuf_, uPerLoopSize); // for cast
    pipe_.InitBuffer(castmBuf_, uPerLoopSize); // for cast
}

template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::InitProcessParam()
{
...
template <typename T>
__aicore__ inline void ApplyCamePart4Float16<T>::GetConfusionTransposeTiling(int64_t numR, int64_t numC, const uint32_t stackBufferSize,
                                                                             const uint32_t typeSize, ConfusionTransposeTiling &tiling)
{
