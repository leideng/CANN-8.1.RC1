...
@register_operator_compute("tan", op_mode="static", support_fusion=True)
def tan_compute(x, y, kernel_name="tan"):
    ...
    pi_div = tbe.vmuls(x, tvm.const(1.0 / PI, cast_dtype))
    if not tbe_platform.api_check_support("tbe.dsl.round", pi_div.dtype):
        pi_div = tbe.cast_to(pi_div, FLOAT_16)
    round_pi_div = tbe.round(pi_div)
    if dtype != round_pi_div.dtype:
        round_pi_div = tbe.cast_to(round_pi_div, dtype)
    if has_improve_precision:
