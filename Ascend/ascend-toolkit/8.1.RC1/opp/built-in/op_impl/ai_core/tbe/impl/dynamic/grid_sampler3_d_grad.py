...
@tbe_register.register_operator("GridSampler3DGrad", trans_bool_to_s8=False)
...
def grid_sampler3_d_grad(grad_in__, x_in__, grid_in__, dx_out_, dgrid_out_, interpolation_mode, padding_mode, align_corners, kernel_name="grid_sampler3_d_grad", impl_mode=""):
    ...
    origin_func_name = "grid_sampler3_d_grad"
    ascendc_src_dir_ex = "grid_sampler3_d_grad"
    ascendc_src_dir = "grid_sampler3_d_grad"
    ascendc_src_file = "grid_sampler3_d_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator GridSampler3DGrad, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "GridSampler3DGrad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[grad_in__, x_in__, grid_in__], origin_outputs = [dx_out_, dgrid_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required'], init_value_list = ['{ "is_list" : false, "type": "float32", "value": 0.000000}', '{ "is_list" : false, "type": "float32", "value": 0.000000}'],\
...
def op_select_format(grad_in__, x_in__, grid_in__, dx_out_, dgrid_out_, interpolation_mode, padding_mode, align_corners, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_in__, x_in__, grid_in__, dx_out_, dgrid_out_, interpolation_mode, padding_mode, align_corners)
    result = check_op_cap("op_select_format", "GridSampler3DGrad", __inputs__, __outputs__, __attrs__)
...
def get_op_specific_info(grad_in__, x_in__, grid_in__, dx_out_, dgrid_out_, interpolation_mode, padding_mode, align_corners, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_in__, x_in__, grid_in__, dx_out_, dgrid_out_, interpolation_mode, padding_mode, align_corners)
    result = check_op_cap("get_op_specific_info", "GridSampler3DGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
