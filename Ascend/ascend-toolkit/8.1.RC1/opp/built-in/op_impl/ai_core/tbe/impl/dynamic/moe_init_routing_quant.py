...
def _build_args(x_in__, row_idx_in__, expert_idx_in__, expanded_x_out_, expanded_row_idx_out_, expanded_expert_idx_out_, active_num, scale, offset):
    __inputs__ = []
    for arg in [x_in__, row_idx_in__, expert_idx_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [expanded_x_out_, expanded_row_idx_out_, expanded_expert_idx_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("MoeInitRoutingQuant", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def moe_init_routing_quant(x_in__, row_idx_in__, expert_idx_in__, expanded_x_out_, expanded_row_idx_out_, expanded_expert_idx_out_, active_num, scale, offset, kernel_name="moe_init_routing_quant", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, row_idx_in__, expert_idx_in__, expanded_x_out_, expanded_row_idx_out_, expanded_expert_idx_out_, active_num, scale, offset)
    ...
    origin_func_name = "moe_init_routing_quant"
    ascendc_src_dir_ex = "moe_init_routing_quant"
    ascendc_src_dir = "moe_init_routing_quant"
    ascendc_src_file = "moe_init_routing_quant.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MoeInitRoutingQuant, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "MoeInitRoutingQuant"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, row_idx_in__, expert_idx_in__], origin_outputs = [expanded_x_out_, expanded_row_idx_out_, expanded_expert_idx_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required', 'required'], init_value_list = [None, None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(x_in__, row_idx_in__, expert_idx_in__, expanded_x_out_, expanded_row_idx_out_, expanded_expert_idx_out_, active_num, scale, offset, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, row_idx_in__, expert_idx_in__, expanded_x_out_, expanded_row_idx_out_, expanded_expert_idx_out_, active_num, scale, offset)
    result = check_op_cap("op_select_format", "MoeInitRoutingQuant", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, row_idx_in__, expert_idx_in__, expanded_x_out_, expanded_row_idx_out_, expanded_expert_idx_out_, active_num, scale, offset, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, row_idx_in__, expert_idx_in__, expanded_x_out_, expanded_row_idx_out_, expanded_expert_idx_out_, active_num, scale, offset)
    result = check_op_cap("get_op_specific_info", "MoeInitRoutingQuant", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
