 ...
 public:
  ...
  __aicore__ inline void Process() {
    ...
    for (int64_t loopIdx = 0; loopIdx < ubLoop; ++loopIdx) {
      ...
      ScatterUpdate<KV_DTYPE, isPagedAttention, RMS_NORM_LENGTH>(vCacheGm, outLocal, indexLocal, startIdx, ubFactor);
      outQueue.FreeTensor(outLocal);
    ...
    if (ubTail > 0) {
      ...
      // Scatter Update vCache
      ScatterUpdate<KV_DTYPE, isPagedAttention, RMS_NORM_LENGTH>(vCacheGm, outLocal, indexLocal, startIdx, ubTail);
      outQueue.FreeTensor(outLocal);
      inQueueIndex.FreeTensor(indexLocal);
    }
  ...
  __aicore__ inline void ScatterUpdate(const GlobalTensor<T>& dst, const LocalTensor<T>& outLocal,
                                       const LocalTensor<int64_t>& indexLocal, int64_t startIdx, int64_t rows) {
