...
template <typename T>
__aicore__ inline void MoeFinalizeRoutingFP<T>::Init(
    GM_ADDR expandedPermutedRows,
    GM_ADDR skip1,
    GM_ADDR skip2,
    GM_ADDR bias,
...
__aicore__ inline void MoeFinalizeRoutingFP<T>::CopyIn(int64_t nLoopIdx, int64_t curRepeatTimes)
{
    ...
    // ---------------------------- [Skip] -------------------------------
    DataCopyParams copyParamsNormalSkip {
        static_cast<uint16_t>(curRepeatTimes), static_cast<uint16_t>(H_ * sizeof(T)), 0, 0
    };
    DataCopyPadParams padParamsNormalSkip {
        isPadH_, 0, static_cast<uint8_t>(rightPaddingH_), 0
    };

    DataCopyPad(skip1Local, gmSkip1_[nLoopIdx * curCoreHandleNumPerLoop_ * H_], copyParamsNormalSkip, padParamsNormalSkip);
    if (skip2IsNull_ == 0) {
        DataCopyPad(skip2Local, gmSkip2_[nLoopIdx * curCoreHandleNumPerLoop_ * H_], copyParamsNormalSkip, padParamsNormalSkip);
    }

    // ---------------------------- [Scales] -------------------------------
    DataCopyParams copyParamsNormalScales {
        static_cast<uint16_t>(curRepeatTimes), static_cast<uint16_t>(K_ * sizeof(T)), 0, 0
    };
    DataCopyPadParams padParamsNormalScales {
        isPadK_, 0, static_cast<uint8_t>(rightPaddingK_), 0
    };
    DataCopyPad(scalesLocal, gmScales_[nLoopIdx * curCoreHandleNumPerLoop_ * K_], copyParamsNormalScales, padParamsNormalScales);

    // ---------------------------- [Expert] -------------------------------
    DataCopyParams copyParamsNormalExpert {
        static_cast<uint16_t>(curRepeatTimes), static_cast<uint16_t>(K_ * sizeof(int32_t)), 0, 0
    };
    DataCopyPadParams padParamsNormalExpert {
        isPadKInt32_, 0, static_cast<uint8_t>(rightPaddingKInt32_), 0
    };
...
__aicore__ inline void MoeFinalizeRoutingFP<T>::Compute(int64_t nLoopIdx, int64_t curRepeatTimes)
{
    ...
    for (int64_t i = 0; i < curRepeatTimes; i++) {
        ...
        for (int64_t j = 0; j < K_; j++) {
            int64_t expandedSrcToDstRowIndex = nLoopIdx * curCoreHandleNumPerLoop_ + i + j * BS_ +
                                               GetBlockIdx() * normalCoreHandleNum_;
            ...
            DataCopyParams copyParams {
                1, static_cast<uint16_t>(H_ * sizeof(T)), 0, 0
            };
            DataCopyPadParams padParams {
                isPadH_, 0, static_cast<uint8_t>(rightPaddingH_), 0
            };
            DataCopyPad(expandedPermutedTmpUb, gmExpandedPermutedRows_[expandedPermutedRowsIndex * H_],
