...
/*!
 * \file detect_flash_attention_dropmask.h
 * \brief
 */

#ifndef DETECT_FLASH_ATTENTION_DROPMASK_H
#define DETECT_FLASH_ATTENTION_DROPMASK_H
    ...
    if constexpr (hasDrop == true) {
        ...
        if (unlikely(dropMaskInfo.boolMode)) {
            BoolCopyIn(dstTensor, srcBoolTensor, dropMaskOffset,
                       dropMaskInfo.s1CopySize, dropMaskInfo.s2CopySize, dropMaskInfo.s2TotalSize);
        } else {
...
#endif // DETECT_FLASH_ATTENTION_DROPMASK_H
