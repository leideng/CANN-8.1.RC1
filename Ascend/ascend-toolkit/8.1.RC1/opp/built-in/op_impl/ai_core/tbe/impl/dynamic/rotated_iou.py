...
rotated_iou
"""
from impl.dynamic.rotated_overlaps import Constant
from impl.dynamic.rotated_overlaps import RotatedOverlaps
...
class RotatedIou(RotatedOverlaps):
    ...
    def __init__(self, boxes, trans, mode, is_cross, kernel_name):
        """
        class init
        """
        RotatedOverlaps.__init__(self, boxes, trans, kernel_name)
    ...
    def get_tiling_data(self):
        ...
        self.v_threshold.set_as(tiling_ub[11])
        self.e_threshold.set_as(tiling_ub[12])
    ...
    def compute_core(self, task_idx):
        ...
        with self.tik_instance.for_range(0, self.batch) as current_batch:
            ...
            with self.tik_instance.if_scope(self.trans == 1):
            ...
            with self.tik_instance.for_range(0, self.b1_batch) as b1_idx:
                ...
                with self.tik_instance.if_scope(self.is_cross == 1):
                    with self.tik_instance.for_range(0, self.valid_box_num) as b2_idx:
                        ...
                        with self.tik_instance.if_scope(self.corners_num == 3):
                            ...
                            with self.tik_instance.if_scope(b1_area + b2_area - overlap > 0):
                                ...
                                if self.mode == "iof":
                                    self.overlap_ub[b2_idx].set_as(
                                        overlap / (b1_area + Constant.EPSILON))
                ...
                with self.tik_instance.else_scope():
                    self.record_vertex_point(task_idx * self.b1_batch + b1_idx)
                    self.record_intersection_point(task_idx * self.b1_batch + b1_idx)
                    b2_area.set_as(self.area_of_boxes_ub[task_idx * self.b1_batch + b1_idx])
                    with self.tik_instance.if_scope(self.corners_num == 3):
                        self.b1_x1.set_as(self.corners_ub[0])
                        self.b1_y1.set_as(self.corners_ub[Constant.BLOCK])
                        self.get_area_of_triangle(1, 2)
                        ...
                        with self.tik_instance.if_scope(b1_area + b2_area - overlap > 0):
                            if self.mode == "iou":
                                self.overlap_ub[task_idx * self.b1_batch + b1_idx].set_as(
                                    overlap / (b1_area + b2_area - overlap + Constant.EPSILON))
                            if self.mode == "iof":
                                self.overlap_ub[task_idx * self.b1_batch + b1_idx].set_as(
                                    overlap / (b1_area + Constant.EPSILON))
...
# 'pylint:disable=too-many-arguments, disable=too-many-statements
@register_operator_compute("rotated_iou", op_mode="dynamic", support_fusion=True)
...
def rotated_iou(boxes, query_boxes, iou,
    ...
    ----------
    """
    op_obj = RotatedIou(boxes, trans, mode, is_cross, kernel_name)
