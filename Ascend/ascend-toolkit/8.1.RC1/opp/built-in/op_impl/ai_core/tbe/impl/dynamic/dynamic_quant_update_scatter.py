...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['var', 'var_scale', 'indices', 'updates', 'smooth_scales']
    output_names = ['var', 'var_scale']
...
def _build_args(var_in__, var_scale_in__, indices_in__, updates_in__, smooth_scales_in__, var_out_, var_scale_out_, reduce, axis):
    __inputs__ = []
    for arg in [var_in__, var_scale_in__, indices_in__, updates_in__, smooth_scales_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [var_out_, var_scale_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("DynamicQuantUpdateScatter", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def dynamic_quant_update_scatter(var_in__, var_scale_in__, indices_in__, updates_in__, smooth_scales_in__=None, var_out_=None, var_scale_out_=None, reduce="", axis=0, kernel_name="dynamic_quant_update_scatter", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(var_in__, var_scale_in__, indices_in__, updates_in__, smooth_scales_in__, var_out_, var_scale_out_, reduce, axis)
    ...
    origin_func_name = "dynamic_quant_update_scatter"
    ascendc_src_dir_ex = "dynamic_quant_update_scatter"
    ascendc_src_dir = "dynamic_quant_update_scatter"
    ascendc_src_file = "dynamic_quant_update_scatter.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator DynamicQuantUpdateScatter, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "DynamicQuantUpdateScatter"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[var_in__, var_scale_in__, indices_in__, updates_in__, smooth_scales_in__], origin_outputs = [var_out_, var_scale_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'optional', 'required', 'required'], init_value_list = [None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(var_in__, var_scale_in__, indices_in__, updates_in__, smooth_scales_in__=None, var_out_=None, var_scale_out_=None, reduce="", axis=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(var_in__, var_scale_in__, indices_in__, updates_in__, smooth_scales_in__, var_out_, var_scale_out_, reduce, axis)
    result = check_op_cap("op_select_format", "DynamicQuantUpdateScatter", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(var_in__, var_scale_in__, indices_in__, updates_in__, smooth_scales_in__=None, var_out_=None, var_scale_out_=None, reduce="", axis=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(var_in__, var_scale_in__, indices_in__, updates_in__, smooth_scales_in__, var_out_, var_scale_out_, reduce, axis)
    result = check_op_cap("get_op_specific_info", "DynamicQuantUpdateScatter", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
