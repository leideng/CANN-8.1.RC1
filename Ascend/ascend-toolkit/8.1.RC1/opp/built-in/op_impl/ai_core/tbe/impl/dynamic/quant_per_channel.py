...
def _build_args(x_in__, scale_in__, offset_in__, y_out_):
    __inputs__ = []
...
@tbe_register.register_operator("QuantPerChannel", trans_bool_to_s8=False)
...
def quant_per_channel(x_in__, scale_in__, offset_in__=None, y_out_=None, kernel_name="quant_per_channel", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, scale_in__, offset_in__, y_out_)
    ...
    origin_func_name = "quant_per_channel"
    ascendc_src_dir_ex = "quant_per_channel"
    ascendc_src_dir = "quant_per_channel"
    ascendc_src_file = "quant_per_channel.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator QuantPerChannel, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "QuantPerChannel"
...
def op_select_format(x_in__, scale_in__, offset_in__=None, y_out_=None, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, scale_in__, offset_in__, y_out_)
    result = check_op_cap("op_select_format", "QuantPerChannel", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, scale_in__, offset_in__=None, y_out_=None, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, scale_in__, offset_in__, y_out_)
    result = check_op_cap("get_op_specific_info", "QuantPerChannel", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
