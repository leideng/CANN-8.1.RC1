...
def _build_args(x_in__, grad_in__, argmax_in__, y_out_, ksize, strides, pads, dilation, ceil_mode):
    __inputs__ = []
    for arg in [x_in__, grad_in__, argmax_in__]:
        if arg != None:
...
@tbe_register.register_operator("MaxPool3DGradWithArgmax", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def max_pool3d_grad_with_argmax(x_in__, grad_in__, argmax_in__, y_out_, ksize, strides, pads, dilation=[1,1,1], ceil_mode=False, kernel_name="max_pool3d_grad_with_argmax", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, grad_in__, argmax_in__, y_out_, ksize, strides, pads, dilation, ceil_mode)
    ...
    origin_func_name = "max_pool3d_grad_with_argmax"
    ascendc_src_dir_ex = "max_pool3_d_grad_with_argmax"
    ascendc_src_dir = "max_pool3_d_grad_with_argmax"
    ascendc_src_file = "max_pool3d_grad_with_argmax.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MaxPool3DGradWithArgmax, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "MaxPool3DGradWithArgmax"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, grad_in__, argmax_in__], origin_outputs = [y_out_],\
