...
def _build_args(x_in__, y_out_, output_size, scale_d, scale_h, scale_w):
    __inputs__ = []
    for arg in [x_in__]:
        if arg != None:
...
@tbe_register.register_operator("UpsampleNearestExact3d", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def upsample_nearest_exact3d(x_in__, y_out_, output_size, scale_d=0, scale_h=0, scale_w=0, kernel_name="upsample_nearest_exact3d", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, output_size, scale_d, scale_h, scale_w)
    ...
    origin_func_name = "upsample_nearest_exact3d"
    ascendc_src_dir_ex = "upsample_nearest_exact3d"
    ascendc_src_dir = "upsample_nearest_exact3d"
    ascendc_src_file = "upsample_nearest_exact3d.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator UpsampleNearestExact3d, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "UpsampleNearestExact3d"
...
def op_select_format(x_in__, y_out_, output_size, scale_d=0, scale_h=0, scale_w=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, output_size, scale_d, scale_h, scale_w)
    result = check_op_cap("op_select_format", "UpsampleNearestExact3d", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, y_out_, output_size, scale_d=0, scale_h=0, scale_w=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, output_size, scale_d, scale_h, scale_w)
    result = check_op_cap("get_op_specific_info", "UpsampleNearestExact3d", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
