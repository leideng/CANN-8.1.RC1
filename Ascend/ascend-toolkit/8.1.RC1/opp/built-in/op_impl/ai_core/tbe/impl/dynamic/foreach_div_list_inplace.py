...
@tbe_register.register_operator("ForeachDivListInplace", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.DYNAMIC_INPUT, para_check.DYNAMIC_INPUT, para_check.KERNEL_NAME)
def foreach_div_list_inplace(x1_in__, x2_in__, kernel_name="foreach_div_list_inplace", impl_mode=""):
    ...
    origin_func_name = "foreach_div_list_inplace"
    ascendc_src_dir_ex = "foreach_div_list_inplace"
    ascendc_src_dir = "foreach_div_list_inplace"
    ascendc_src_file = "foreach_div_list_inplace.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator ForeachDivListInplace, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "ForeachDivListInplace"
...
def op_select_format(x1_in__, x2_in__, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__)
    result = check_op_cap("op_select_format", "ForeachDivListInplace", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x1_in__, x2_in__, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__)
    result = check_op_cap("get_op_specific_info", "ForeachDivListInplace", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
