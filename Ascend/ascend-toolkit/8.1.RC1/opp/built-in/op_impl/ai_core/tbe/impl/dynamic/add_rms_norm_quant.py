...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x1', 'x2', 'gamma', 'scales1', 'scales2', 'zero_points1', 'zero_points2']
...
def _build_args(x1_in__, x2_in__, gamma_in__, scales1_in__, scales2_in__, zero_points1_in__, zero_points2_in__, y1_out_, y2_out_, x_out_, axis, epsilon, div_mode):
    __inputs__ = []
    for arg in [x1_in__, x2_in__, gamma_in__, scales1_in__, scales2_in__, zero_points1_in__, zero_points2_in__]:
        if arg != None:
...
@tbe_register.register_operator("AddRmsNormQuant", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def add_rms_norm_quant(x1_in__, x2_in__, gamma_in__, scales1_in__, scales2_in__=None, zero_points1_in__=None, zero_points2_in__=None, y1_out_=None, y2_out_=None, x_out_=None, axis=-1, epsilon=1e-06, div_mode=True, kernel_name="add_rms_norm_quant", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, gamma_in__, scales1_in__, scales2_in__, zero_points1_in__, zero_points2_in__, y1_out_, y2_out_, x_out_, axis, epsilon, div_mode)
    ...
    origin_func_name = "add_rms_norm_quant"
    ascendc_src_dir_ex = "add_rms_norm_quant"
    ascendc_src_dir = "add_rms_norm_quant"
    ascendc_src_file = "add_rms_norm_quant.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator AddRmsNormQuant, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "AddRmsNormQuant"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x1_in__, x2_in__, gamma_in__, scales1_in__, scales2_in__, zero_points1_in__, zero_points2_in__], origin_outputs = [y1_out_, y2_out_, x_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'optional', 'optional', 'optional', 'required', 'required', 'required'], init_value_list = [None, None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{"auto_sync": false}')

def op_select_format(x1_in__, x2_in__, gamma_in__, scales1_in__, scales2_in__=None, zero_points1_in__=None, zero_points2_in__=None, y1_out_=None, y2_out_=None, x_out_=None, axis=-1, epsilon=1e-06, div_mode=True, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, gamma_in__, scales1_in__, scales2_in__, zero_points1_in__, zero_points2_in__, y1_out_, y2_out_, x_out_, axis, epsilon, div_mode)
    result = check_op_cap("op_select_format", "AddRmsNormQuant", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x1_in__, x2_in__, gamma_in__, scales1_in__, scales2_in__=None, zero_points1_in__=None, zero_points2_in__=None, y1_out_=None, y2_out_=None, x_out_=None, axis=-1, epsilon=1e-06, div_mode=True, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, gamma_in__, scales1_in__, scales2_in__, zero_points1_in__, zero_points2_in__, y1_out_, y2_out_, x_out_, axis, epsilon, div_mode)
    result = check_op_cap("get_op_specific_info", "AddRmsNormQuant", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
