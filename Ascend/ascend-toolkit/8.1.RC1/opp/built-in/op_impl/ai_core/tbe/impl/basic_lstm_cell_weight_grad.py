...
class LstmCellGradInput():
    ...
    def __init__(self, input_x, hidden_state, dgate, weight_gradient,
        ...
        self.is_910b = short_soc_version in ("Ascend910B", "Ascend910_93")
...
class LstmCellGrad(LstmCellGradInput):
    ...
    def __init__(self, input_x, hidden_state, dgate, weight_gradient,
        ...
        dtype_bytes_size = get_bit_len(self.x_dtype) // 8
        ...
        with self.tik_instance.for_range(0, m_loop) as idx_0:
            ...
            with self.tik_instance.for_range(0, n_loop, thread_num=thread) as idx_1:
                ...
                ub_db = self.tik_instance.Tensor(
                    self.db_dtype, (256,), name="ub_db", scope=tik.scope_ubuf)
    ...
    def matmul_compute_each_k_loop(self, l0a_x, l0b_y, l1_left, l1_right,
        ...
        if self.is_910b:
            self.tik_instance.load2dv2(l0a_x, l1_left[k_offset], 0, load_len, 0, 1, 0,
                                       is_trans_l)

            self.tik_instance.load2dv2(l0b_y, l1_right[k_offset], 0, load_len, 0, 1, 0,
                                       is_trans_r)
        else:
    ...
    def init_b_matrix(self):
        ...
        if self.is_910b:
            self.tik_instance.load2dv2(self.zero_matrix, self.l1_b, 0, self.k_num,
                                       0, 1, 0, False)
        else:
