...
@tbe_register.register_operator("ForeachMulScalarInplace", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.DYNAMIC_INPUT, para_check.REQUIRED_INPUT, para_check.KERNEL_NAME)
def foreach_mul_scalar_inplace(x_in__, scalar_in__, kernel_name="foreach_mul_scalar_inplace", impl_mode=""):
    ...
    origin_func_name = "foreach_mul_scalar_inplace"
    ascendc_src_dir_ex = "foreach_mul_scalar_inplace"
    ascendc_src_dir = "foreach_mul_scalar_inplace"
    ascendc_src_file = "foreach_mul_scalar_inplace.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator ForeachMulScalarInplace, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "ForeachMulScalarInplace"
...
def op_select_format(x_in__, scalar_in__, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, scalar_in__)
    result = check_op_cap("op_select_format", "ForeachMulScalarInplace", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, scalar_in__, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, scalar_in__)
    result = check_op_cap("get_op_specific_info", "ForeachMulScalarInplace", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
