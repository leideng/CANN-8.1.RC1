...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['tmp_out', 'padding_offset', 'seq_len', 'input_ids']
    output_names = ['output']
...
def _build_args(tmp_out_in__, padding_offset_in__, seq_len_in__, input_ids_in__, output_out_):
    __inputs__ = []
    for arg in [tmp_out_in__, padding_offset_in__, seq_len_in__, input_ids_in__]:
        if arg != None:
...
@tbe_register.register_operator("PadInfer", trans_bool_to_s8=False)
...
def pad_infer(tmp_out_in__, padding_offset_in__, seq_len_in__, input_ids_in__, output_out_, kernel_name="pad_infer", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(tmp_out_in__, padding_offset_in__, seq_len_in__, input_ids_in__, output_out_)
    ...
    origin_func_name = "pad_infer"
    ascendc_src_dir_ex = "pad_infer"
    ascendc_src_dir = "pad_infer"
    ascendc_src_file = "pad_infer.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator PadInfer, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "PadInfer"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[tmp_out_in__, padding_offset_in__, seq_len_in__, input_ids_in__], origin_outputs = [output_out_],\
...
def op_select_format(tmp_out_in__, padding_offset_in__, seq_len_in__, input_ids_in__, output_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(tmp_out_in__, padding_offset_in__, seq_len_in__, input_ids_in__, output_out_)
    result = check_op_cap("op_select_format", "PadInfer", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(tmp_out_in__, padding_offset_in__, seq_len_in__, input_ids_in__, output_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(tmp_out_in__, padding_offset_in__, seq_len_in__, input_ids_in__, output_out_)
    result = check_op_cap("get_op_specific_info", "PadInfer", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
