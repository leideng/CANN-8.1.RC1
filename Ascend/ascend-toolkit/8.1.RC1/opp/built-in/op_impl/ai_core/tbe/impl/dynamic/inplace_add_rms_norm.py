...
def _build_args(x1_in__, x2_in__, gamma_in__, x1_out_, rstd_out_, x2_out_, epsilon):
    __inputs__ = []
    for arg in [x1_in__, x2_in__, gamma_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [x1_out_, rstd_out_, x2_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("InplaceAddRmsNorm", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def inplace_add_rms_norm(x1_in__, x2_in__, gamma_in__, x1_out_, rstd_out_, x2_out_, epsilon=1e-06, kernel_name="inplace_add_rms_norm", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, gamma_in__, x1_out_, rstd_out_, x2_out_, epsilon)
    ...
    origin_func_name = "inplace_add_rms_norm"
    ascendc_src_dir_ex = "inplace_add_rms_norm"
    ascendc_src_dir = "inplace_add_rms_norm"
    ascendc_src_file = "inplace_add_rms_norm.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator InplaceAddRmsNorm, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "InplaceAddRmsNorm"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x1_in__, x2_in__, gamma_in__], origin_outputs = [x1_out_, rstd_out_, x2_out_],\
...
def op_select_format(x1_in__, x2_in__, gamma_in__, x1_out_, rstd_out_, x2_out_, epsilon=1e-06, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, gamma_in__, x1_out_, rstd_out_, x2_out_, epsilon)
    result = check_op_cap("op_select_format", "InplaceAddRmsNorm", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x1_in__, x2_in__, gamma_in__, x1_out_, rstd_out_, x2_out_, epsilon=1e-06, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, gamma_in__, x1_out_, rstd_out_, x2_out_, epsilon)
    result = check_op_cap("get_op_specific_info", "InplaceAddRmsNorm", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
