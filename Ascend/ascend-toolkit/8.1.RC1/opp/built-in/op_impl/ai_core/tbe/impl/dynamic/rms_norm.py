...
@tbe_register.register_operator("RmsNorm", trans_bool_to_s8=False)
...
def rms_norm(x_in__, gamma_in__, y_out_, rstd_out_, epsilon=1e-06, kernel_name="rms_norm", impl_mode=""):
    ...
    origin_func_name = "rms_norm"
    ascendc_src_dir_ex = "rms_norm"
    ascendc_src_dir = "rms_norm"
    ascendc_src_file = "rms_norm.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator RmsNorm, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "RmsNorm"
...
def op_select_format(x_in__, gamma_in__, y_out_, rstd_out_, epsilon=1e-06, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, gamma_in__, y_out_, rstd_out_, epsilon)
    result = check_op_cap("op_select_format", "RmsNorm", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, gamma_in__, y_out_, rstd_out_, epsilon=1e-06, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, gamma_in__, y_out_, rstd_out_, epsilon)
    result = check_op_cap("get_op_specific_info", "RmsNorm", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
