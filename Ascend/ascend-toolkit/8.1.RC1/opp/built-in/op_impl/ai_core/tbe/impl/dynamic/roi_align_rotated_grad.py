...
def _build_args(x_grad_in__, rois_in__, y_grad_out_, y_grad_shape, pooled_h, pooled_w, spatial_scale, sampling_ratio, aligned, clockwise):
    __inputs__ = []
    for arg in [x_grad_in__, rois_in__]:
        if arg != None:
    ...
    for arg in [y_grad_out_]:
        if arg != None:
            ...
            __outputs__.append(arg)
    __attrs__ = []
    if y_grad_shape != None:
        attr = {}
        attr["name"] = "y_grad_shape"
        attr["dtype"] = "list_int"
        attr["value"] = y_grad_shape
...
@tbe_register.register_operator("RoiAlignRotatedGrad", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def roi_align_rotated_grad(x_grad_in__, rois_in__, y_grad_out_, y_grad_shape, pooled_h, pooled_w, spatial_scale, sampling_ratio, aligned, clockwise, kernel_name="roi_align_rotated_grad", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_grad_in__, rois_in__, y_grad_out_, y_grad_shape, pooled_h, pooled_w, spatial_scale, sampling_ratio, aligned, clockwise)
    ...
    origin_func_name = "roi_align_rotated_grad"
    ascendc_src_dir_ex = "roi_align_rotated_grad"
    ascendc_src_dir = "roi_align_rotated_grad"
    ascendc_src_file = "roi_align_rotated_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator RoiAlignRotatedGrad, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "RoiAlignRotatedGrad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_grad_in__, rois_in__], origin_outputs = [y_grad_out_],\
...
def op_select_format(x_grad_in__, rois_in__, y_grad_out_, y_grad_shape, pooled_h, pooled_w, spatial_scale, sampling_ratio, aligned, clockwise, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_grad_in__, rois_in__, y_grad_out_, y_grad_shape, pooled_h, pooled_w, spatial_scale, sampling_ratio, aligned, clockwise)
    result = check_op_cap("op_select_format", "RoiAlignRotatedGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_grad_in__, rois_in__, y_grad_out_, y_grad_shape, pooled_h, pooled_w, spatial_scale, sampling_ratio, aligned, clockwise, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_grad_in__, rois_in__, y_grad_out_, y_grad_shape, pooled_h, pooled_w, spatial_scale, sampling_ratio, aligned, clockwise)
    result = check_op_cap("get_op_specific_info", "RoiAlignRotatedGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
