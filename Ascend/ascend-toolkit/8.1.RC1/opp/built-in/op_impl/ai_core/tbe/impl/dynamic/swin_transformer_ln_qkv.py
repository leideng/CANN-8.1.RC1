...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'gamma', 'beta', 'weight', 'bias']
...
def _build_args(x_in__, gamma_in__, beta_in__, weight_in__, bias_in__, query_output_out_, key_output_out_, value_output_out_, head_num, head_dim, seq_length, shifts, epsilon):
    __inputs__ = []
    for arg in [x_in__, gamma_in__, beta_in__, weight_in__, bias_in__]:
        if arg != None:
...
@tbe_register.register_operator("SwinTransformerLnQKV", trans_bool_to_s8=False)
...
def swin_transformer_ln_qkv(x_in__, gamma_in__, beta_in__, weight_in__, bias_in__, query_output_out_, key_output_out_, value_output_out_, head_num, head_dim, seq_length, shifts=[], epsilon=1e-07, kernel_name="swin_transformer_ln_qkv", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, gamma_in__, beta_in__, weight_in__, bias_in__, query_output_out_, key_output_out_, value_output_out_, head_num, head_dim, seq_length, shifts, epsilon)
    ...
    origin_func_name = "swin_transformer_ln_qkv"
    ascendc_src_dir_ex = "swin_transformer_ln_qkv"
    ascendc_src_dir = "swin_transformer_ln_q_k_v"
    ascendc_src_file = "swin_transformer_ln_qkv.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator SwinTransformerLnQKV, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "SwinTransformerLnQKV"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, gamma_in__, beta_in__, weight_in__, bias_in__], origin_outputs = [query_output_out_, key_output_out_, value_output_out_],\
...
def op_select_format(x_in__, gamma_in__, beta_in__, weight_in__, bias_in__, query_output_out_, key_output_out_, value_output_out_, head_num, head_dim, seq_length, shifts=[], epsilon=1e-07, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, gamma_in__, beta_in__, weight_in__, bias_in__, query_output_out_, key_output_out_, value_output_out_, head_num, head_dim, seq_length, shifts, epsilon)
    result = check_op_cap("op_select_format", "SwinTransformerLnQKV", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, gamma_in__, beta_in__, weight_in__, bias_in__, query_output_out_, key_output_out_, value_output_out_, head_num, head_dim, seq_length, shifts=[], epsilon=1e-07, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, gamma_in__, beta_in__, weight_in__, bias_in__, query_output_out_, key_output_out_, value_output_out_, head_num, head_dim, seq_length, shifts, epsilon)
    result = check_op_cap("get_op_specific_info", "SwinTransformerLnQKV", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
