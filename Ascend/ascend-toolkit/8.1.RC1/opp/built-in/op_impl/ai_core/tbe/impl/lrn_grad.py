...
class LrnGrad:
    ...
    def __init__(self, shape, dtype, depth_radius=5, bias=1, alpha=1, beta=0.5, kernel_name="lrn_grad"):
        ...
        if tbe_platform.api_check_support("tik.vln", "float32"):
            self.is_mini = False
        ...
        self.data_input_grads = tik_instance.Tensor(dtype, gm_shape, name="input_grads", scope=tbe_platform.scope_gm)
        self.data_input_image = tik_instance.Tensor(dtype, gm_shape, name="input_image", scope=tbe_platform.scope_gm)
        self.data_output_image = tik_instance.Tensor(dtype, gm_shape, name="output_image", scope=tbe_platform.scope_gm)
        self.data_output = tik_instance.Tensor(dtype, gm_shape, name="output", scope=tbe_platform.scope_gm)
    ...
    def _alloc_all_ub(self):
        ...
        if self.dtype != self.ub_dtype:
            self.data_cov_ub = tik_instance.Tensor(self.dtype, self.ub_shape, name="data_cov_ub",
                                                   scope=tbe_platform.scope_ubuf)

        dtype = self.ub_dtype
        self.input_grads_ub = tik_instance.Tensor(dtype, self.ub_shape, name="input_grads_ub",
                                                  scope=tbe_platform.scope_ubuf)
        self.input_image_ub = tik_instance.Tensor(dtype, self.ub_shape, name="input_image_ub",
                                                  scope=tbe_platform.scope_ubuf)
        self.output_image_ub = tik_instance.Tensor(dtype, self.ub_shape, name="output_image_ub",
                                                   scope=tbe_platform.scope_ubuf)
        self.pre_norm_ub = tik_instance.Tensor(dtype, self.ub_shape, name="pre_norm_ub", scope=tbe_platform.scope_ubuf)
        self.norm_ub = tik_instance.Tensor(dtype, self.ub_shape, name="norm_ub", scope=tbe_platform.scope_ubuf)
        self.norm_beta_ub = tik_instance.Tensor(dtype, self.ub_shape, name="norm_beta_ub",
                                                scope=tbe_platform.scope_ubuf)
    ...
    def _alloc_all_ub_cut_channel(self):
        ...
        tik_instance = self.tik_instance
        if self.dtype != self.ub_dtype:
            self.data_cov_ub = tik_instance.Tensor(self.dtype, ub_input_shape, name="data_cov_ub",
                                                   scope=tbe_platform.scope_ubuf)

        dtype = self.ub_dtype
        self.input_grads_ub = tik_instance.Tensor(dtype, ub_input_shape, name="input_grads_ub",
                                                  scope=tbe_platform.scope_ubuf)
        self.input_image_ub = tik_instance.Tensor(dtype, ub_input_shape, name="input_image_ub",
                                                  scope=tbe_platform.scope_ubuf)
        self.output_image_ub = tik_instance.Tensor(dtype, ub_input_shape, name="output_image_ub",
                                                   scope=tbe_platform.scope_ubuf)
        self.pre_norm_ub = tik_instance.Tensor(dtype, ub_pre_norm_shape, name="pre_norm_ub",
                                               scope=tbe_platform.scope_ubuf)
        self.norm_ub = tik_instance.Tensor(dtype, ub_norm_shape, name="norm_ub", scope=tbe_platform.scope_ubuf)
        self.norm_beta_ub = tik_instance.Tensor(dtype, ub_norm_shape, name="norm_beta_ub",
                                                scope=tbe_platform.scope_ubuf)
        self.pre_norm2_ub = tik_instance.Tensor(dtype, ub_pre_norm2_shape, name="pre_norm2_ub",
                                                scope=tbe_platform.scope_ubuf)
        self.norm2_ub = tik_instance.Tensor(dtype, ub_norm2_shape, name="norm2_ub", scope=tbe_platform.scope_ubuf)
        self.data_output_ub = tik_instance.Tensor(dtype, ub_output_shape, name="output_ub",
                                                  scope=tbe_platform.scope_ubuf)
