...
class GMMSwigluCompute{
 ...
 private: 
    ...
    TQue<QuePosition::VECIN, 1> groupListInQueue;
...
{   
    ...
    if ASCEND_IS_AIV {
        ...
        pipe->InitBuffer(groupListInQueue, 1, AlignUp<int32_t>(groupNum, 4) * sizeof(int64_t));
    }
}
...
__aicore__ inline void GMMSwigluCompute<mmType, sync, CHANNELDTYPE>::Quant(uint32_t loopIdx) {
    LocalTensor<float> _inMMLocal = mmOutQueue.DeQue<float>();
    PipeBarrier<PIPE_ALL>();
    Abs(_inMMLocal[loopIdx * gmmSwiglu->tokenLen + gmmSwiglu->tokenLen / 2],
        _inMMLocal[loopIdx * gmmSwiglu->tokenLen],
        gmmSwiglu->tokenLen / 2);
    LocalTensor<float> workspaceLocal= reduceWorkspace.Get<float>();
    PipeBarrier<PIPE_ALL>();
    if (gmmSwiglu->tokenLen > 128){
        WholeReduceMax(workspaceLocal, 
                       _inMMLocal[loopIdx * gmmSwiglu->tokenLen + gmmSwiglu->tokenLen / 2], 64,
                       (gmmSwiglu->tokenLen / 2) / 64, 1, 1, 8, ReduceOrder::ORDER_ONLY_VALUE);
        PipeBarrier<PIPE_ALL>();
        WholeReduceMax(workspaceLocal, workspaceLocal, 
                       (gmmSwiglu->tokenLen / 2) / 64, 1, 1, 1, 8, ReduceOrder::ORDER_ONLY_VALUE);
    } else {
        WholeReduceMax(workspaceLocal, 
                       _inMMLocal[loopIdx * gmmSwiglu->tokenLen + gmmSwiglu->tokenLen / 2], 
                       gmmSwiglu->tokenLen / 2, 1, 1, 1, 8, ReduceOrder::ORDER_ONLY_VALUE);
    }
    PipeBarrier<PIPE_ALL>();
    float quantScale = workspaceLocal.GetValue(0) / 127.0f;
    PipeBarrier<PIPE_ALL>();
    LocalTensor<float> quantScaleLocal = quantScaleOutQueue.DeQue<float>();
    PipeBarrier<PIPE_ALL>();
    ...
    PipeBarrier<PIPE_ALL>();
    Muls(_inMMLocal[loopIdx * gmmSwiglu->tokenLen], _inMMLocal[loopIdx * gmmSwiglu->tokenLen], quantScale, gmmSwiglu->tokenLen / 2);
    PipeBarrier<PIPE_ALL>();
    LocalTensor<int8_t> quantLocal = quantOutQueue.DeQue<int8_t>();
    Cast(_inMMLocal[loopIdx * gmmSwiglu->tokenLen].ReinterpretCast<half>(), _inMMLocal[loopIdx * gmmSwiglu->tokenLen], RoundMode::CAST_RINT, gmmSwiglu->tokenLen / 2);
    PipeBarrier<PIPE_ALL>();
    Cast(quantLocal[loopIdx * gmmSwiglu->tokenLen / 2], _inMMLocal[loopIdx * gmmSwiglu->tokenLen].ReinterpretCast<half>(), RoundMode::CAST_RINT, gmmSwiglu->tokenLen / 2);
    mmOutQueue.EnQue(_inMMLocal);
    quantOutQueue.EnQue(quantLocal);
}
