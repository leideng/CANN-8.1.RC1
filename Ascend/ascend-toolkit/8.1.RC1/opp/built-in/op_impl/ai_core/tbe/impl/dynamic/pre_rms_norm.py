...
def _build_args(x_in__, res_in_in__, gamma_in__, beta_in__, y_out_, res_out_out_, epsilon, high_precision_mode, gemma_mode):
    __inputs__ = []
...
@tbe_register.register_operator("PreRmsNorm", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def pre_rms_norm(x_in__, res_in_in__, gamma_in__, beta_in__=None, y_out_=None, res_out_out_=None, epsilon=0.0, high_precision_mode=False, gemma_mode=False, kernel_name="pre_rms_norm", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, res_in_in__, gamma_in__, beta_in__, y_out_, res_out_out_, epsilon, high_precision_mode, gemma_mode)
    ...
    origin_func_name = "pre_rms_norm"
    ascendc_src_dir_ex = "pre_rms_norm"
    ascendc_src_dir = "pre_rms_norm"
    ascendc_src_file = "pre_rms_norm.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator PreRmsNorm, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "PreRmsNorm"
...
def op_select_format(x_in__, res_in_in__, gamma_in__, beta_in__=None, y_out_=None, res_out_out_=None, epsilon=0.0, high_precision_mode=False, gemma_mode=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, res_in_in__, gamma_in__, beta_in__, y_out_, res_out_out_, epsilon, high_precision_mode, gemma_mode)
    result = check_op_cap("op_select_format", "PreRmsNorm", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, res_in_in__, gamma_in__, beta_in__=None, y_out_=None, res_out_out_=None, epsilon=0.0, high_precision_mode=False, gemma_mode=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, res_in_in__, gamma_in__, beta_in__, y_out_, res_out_out_, epsilon, high_precision_mode, gemma_mode)
    result = check_op_cap("get_op_specific_info", "PreRmsNorm", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
