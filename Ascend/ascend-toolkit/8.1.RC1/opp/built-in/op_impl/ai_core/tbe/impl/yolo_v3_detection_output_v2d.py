...
from tbe.tik import all as _all
from tbe.tik import any as _any
from impl.util import util_select_op_base
from impl.util.platform_adapter import tbe_platform
from tbe.common.platform.platform_info import api_check_support
from impl.cnms_yolov3_ver import process_nms_mode_v300
...
def check_soc_version():
    ...
    if api_check_support("tik.vcopy"):
...
class DetectionOutput(ClsProbComputer):
    ...
    def __init__(self, input_dict):
        ...
        if tbe_platform.get_soc_spec(
                tbe_platform.UB_SIZE) // 1024 < 200 or \
        ...
        self.bbox = self.instance.Tensor(self.dtype,
                                         ...
                                         name="box_out", scope=tbe_platform.scope_gm)
        ...
        # idx_gm is used for cnms_yolov3_ver to sort scores
        if not self.is_rpn_supported:
            # every loop process 4096 units
            pre_loop_unit = 4096
            idx_size = ceil_div(PRE_NMS_TOPN, pre_loop_unit) * pre_loop_unit
            idx_init = [i for i in range(idx_size)]
            self.idx_gm = self.instance.Tensor("uint32",
                                               [idx_size, ],
                                               name="idx_gm",
                                               scope=tbe_platform.scope_gm, init_value=idx_init)
    ...
    def multi_class(self, batch, image_ub, param):
        ...
        if tbe_platform.get_soc_spec("SHORT_SOC_VERSION") not in (
            ...
            mask = self.instance.Tensor(dtype, (self.max_ub_num,),
                                        name="mask", scope=tbe_platform.scope_ubuf)
        ...
        with self.instance.new_stmt_scope():
            ...
            self.process_each_class(proposals_ub, x1y1x2y2_ub, mask, param)
    ...
    def process_each_class(self, proposals_ub, x1y1x2y2_ub, mask, param):
        """
        process each class
        ...
        x1y1x2y2_ub: a tensor,which store x1, y1, x2, y2
        ...
        # when yolov3 running on 310B or 910B, it does not need proposal format data, only need x1y1x2y2
        if self.is_rpn_supported:
        ...
        if not self.is_rpn_supported:
            # tensors for calculation when classes is larger than or equal to 1
            multi_ret_ub = self.instance.Tensor(self.dtype, (5, PRE_NMS_TOPN),
                                                name="multi_ret_ub",
                                                scope=tbe_platform.scope_ubuf)
            param["multi_ret_ub"] = multi_ret_ub
            
            self.multi_class_boxes(param["x1y1x2y2_ub"], param["image_ub"], PRE_NMS_TOPN)
            self.scale_boxes(param["x1y1x2y2_ub"], param["image_ub"], PRE_NMS_TOPN)
            ...
            if self.is_rpn_supported:
                with self.instance.if_scope(selected_count > 0):
                    ...
                    ret_ub = self.instance.Tensor(self.dtype, (5, PRE_NMS_TOPN),
                                                  name="ret_ub",
                                                  scope=tbe_platform.scope_ubuf)
                    self.instance.vextract(ret_ub[0, 0], proposals_selected[0], repeats, 0)
                    self.instance.vextract(ret_ub[1, 0], proposals_selected[0], repeats, 1)
                    self.instance.vextract(ret_ub[2, 0], proposals_selected[0], repeats, 2)
                    self.instance.vextract(ret_ub[3, 0], proposals_selected[0], repeats, 3)
                ...
                self.recover_boxes(param["multi_ret_ub"], param["image_ub"], selected_count)
                self.move_result_to_gm(param["multi_ret_ub"], param["ret_label_ub"], 
                                       selected_count, param["batch"])
    ...
    def nms_of_each_class(self, param, selected_count, proposals_selected):
        ...
        if self.is_rpn_supported:
            ...
            nms_flag = self.instance.Scalar(dtype="int32", init_value=999)
            num_zero = self.instance.Scalar(dtype="float32")
            class_ub_num = self.instance.Scalar(dtype="float16")
            compare_num = self.instance.Scalar(dtype="float32")
            num_zero.set_as(0.01)
            with self.instance.for_range(0, param["count_offset"]) as num_idx:
                class_ub_num.set_as(param["classes_ub_nms"][num_idx])
                self.instance.scalar_conv("none", compare_num, class_ub_num)
                with self.instance.if_scope(compare_num >= num_zero):
                    nms_flag.set_as(100)
            
            with self.instance.if_scope(nms_flag == 100):
                ret_ub = self.instance.Tensor(self.dtype, (5, PRE_NMS_TOPN),
                                            name="ret_ub",
                                            scope=tbe_platform.scope_ubuf)
                nmsed_count = self.instance.Scalar("int32", name="nmsed_count", init_value=0)

                param["ret_ub"] = ret_ub
                param["nmsed_count"] = nmsed_count

                repeat_times = (PRE_NMS_TOPN - param["count_offset"]) // self.mask
                start_offset = ceil_div(param["count_offset"], self.mask) * self.mask
                
                # clean random data
                self.instance.vec_dup(self.mask, param["classes_ub_nms"][start_offset], 0.0, repeat_times, 8)

                with self.instance.for_range(0, start_offset - param["count_offset"]) as idx:
                    param["classes_ub_nms"][param["count_offset"] + idx].set_as(0.0)

                # call cnms_yolov3_ver for calculating data to get same results as original nms calculation
                self.cnms_yolov3_ver(param)

                # store x1, x2, y1, y2, scores, multi-classes, and total boxes num
                with self.instance.for_range(0, param["nmsed_count"]) as count_idx:
                    param["multi_ret_ub"][0, selected_count + count_idx].set_as(param["ret_ub"][0, count_idx])
                    param["multi_ret_ub"][1, selected_count + count_idx].set_as(param["ret_ub"][1, count_idx])
                    param["multi_ret_ub"][2, selected_count + count_idx].set_as(param["ret_ub"][2, count_idx])
                    param["multi_ret_ub"][3, selected_count + count_idx].set_as(param["ret_ub"][3, count_idx])
                    param["multi_ret_ub"][4, selected_count + count_idx].set_as(param["ret_ub"][4, count_idx])
                    param["ret_label_ub"][0, selected_count + count_idx].set_as(param["class_cycle"])

                selected_count.set_as(selected_count + param["nmsed_count"])

    def scale_boxes(self, ret_ub, image_ub, selected_count):
        """
        scale boxes
        ...
        if self.dtype == "float16":
            ...
            shape = (8,)
        div_res = self.instance.Tensor(self.dtype, shape, name="div_res", scope=tbe_platform.scope_ubuf)
        self.instance.vec_dup(shape[0], div_res, 1.0, 1, 8)
        self.instance.vdiv(4, div_res, div_res, image_ub, 1, 1, 1, 1, 8, 8, 8)
        div_image_w = self.instance.Scalar(self.dtype, name="div_image_w", init_value=div_res[3])
        div_image_h = self.instance.Scalar(self.dtype, name="div_image_h", init_value=div_res[2])
        ...
        self.instance.vec_muls(self.mask, ret_ub[0, 0], ret_ub[0, 0],
                               div_image_w, repeats, constant.REPEAT_STRIDE_EIGHT,
                               constant.REPEAT_STRIDE_EIGHT)
        self.instance.vec_muls(self.mask, ret_ub[1, 0], ret_ub[1, 0],
                               div_image_h, repeats, constant.REPEAT_STRIDE_EIGHT,
                               constant.REPEAT_STRIDE_EIGHT)
    ...
    def recover_boxes(self, ret_ub, image_ub, selected_count):
        """
        recover boxes
        ...
        None
        """
        image_w = self.instance.Scalar(self.dtype, name="image_w", init_value=image_ub[3])
        image_h = self.instance.Scalar(self.dtype, name="image_h", init_value=image_ub[2])
        repeats = get_vector_repeat_times(self.instance, selected_count * self.dsize)
    ...
    def cnms_yolov3_ver(self, param):
        """
        set cnms_yolov3_ver

        Parameters
        ----------
        param: param is a dict, focus on parameters as follow:
                                count / x1y1x2y2_ub / classes_ub_nms
        Returns
        -------
        None
        """
        boxes_num = PRE_NMS_TOPN
        box_type = self.dtype
        score_thresh = self.classes_threshold
        iou_thresh = self.nms_threshold
        max_size_per_class = PRE_NMS_TOPN
        max_total_size = PRE_NMS_TOPN

        bbox_out_list = [param["ret_ub"], param["nmsed_count"]]

        _yolov3_param = [self.instance, [box_type, boxes_num],
                        [score_thresh, iou_thresh], [max_size_per_class, max_total_size],
                        [self.idx_gm, param["x1y1x2y2_ub"], param["classes_ub_nms"], bbox_out_list]]
        
        process_nms_mode_v300(*_yolov3_param)
    ...
    def class_filter_v200(self, selected_tmp, selected_class, param):
        ...
        pattern = self.instance.Tensor(pat_type, (PRE_NMS_TOPN, ),
                                       name="pattern", scope=tbe_platform.scope_ubuf)
        ...
        reduce_xy = self.instance.Tensor(self.dtype, (PRE_NMS_TOPN, ),
                                         name="reduce_xy", scope=tbe_platform.scope_ubuf)
    ...
    def nms_filter(self, proposals_selected, selected_tmp, selected_count,
        ...
        """
        if tbe_platform.get_soc_spec("SHORT_SOC_VERSION") in (
                "Ascend910", "Ascend310P", "Ascend610", "BS9SX1A", "Hi3796CV300ES", "Hi3796CV300CS", "SD3403"):
