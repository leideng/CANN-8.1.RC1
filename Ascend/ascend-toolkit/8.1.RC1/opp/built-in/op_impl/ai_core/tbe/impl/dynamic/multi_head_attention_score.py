...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['query', 'key', 'value', 'real_shift', 'drop_mask', 'padding_mask', 'atten_mask']
    output_names = ['softmax_out', 'attention_out']
...
def _build_args(query_in__, key_in__, value_in__, real_shift_in__, drop_mask_in__, padding_mask_in__, atten_mask_in__, softmax_out_out_, attention_out_out_, scale_value, keep_prob, pre_tockens, next_tockens, head_num, input_layout):
    __inputs__ = []
    for arg in [query_in__, key_in__, value_in__, real_shift_in__, drop_mask_in__, padding_mask_in__, atten_mask_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [softmax_out_out_, attention_out_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("MultiHeadAttentionScore", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_STR, para_check.KERNEL_NAME)
def multi_head_attention_score(query_in__, key_in__, value_in__, real_shift_in__=None, drop_mask_in__=None, padding_mask_in__=None, atten_mask_in__=None, softmax_out_out_=None, attention_out_out_=None, scale_value=1, keep_prob=1, pre_tockens=2147483647, next_tockens=2147483647, head_num=0, input_layout="", kernel_name="multi_head_attention_score", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(query_in__, key_in__, value_in__, real_shift_in__, drop_mask_in__, padding_mask_in__, atten_mask_in__, softmax_out_out_, attention_out_out_, scale_value, keep_prob, pre_tockens, next_tockens, head_num, input_layout)
    ...
    origin_func_name = "multi_head_attention_score"
    ascendc_src_dir_ex = "multi_head_attention_score"
    ascendc_src_dir = "multi_head_attention_score"
    ascendc_src_file = "multi_head_attention_score.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MultiHeadAttentionScore, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "MultiHeadAttentionScore"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[query_in__, key_in__, value_in__, real_shift_in__, drop_mask_in__, padding_mask_in__, atten_mask_in__], origin_outputs = [softmax_out_out_, attention_out_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'optional', 'optional', 'optional', 'optional', 'required', 'required'], init_value_list = [None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{"auto_sync": false}')

def op_select_format(query_in__, key_in__, value_in__, real_shift_in__=None, drop_mask_in__=None, padding_mask_in__=None, atten_mask_in__=None, softmax_out_out_=None, attention_out_out_=None, scale_value=1, keep_prob=1, pre_tockens=2147483647, next_tockens=2147483647, head_num=0, input_layout="", impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(query_in__, key_in__, value_in__, real_shift_in__, drop_mask_in__, padding_mask_in__, atten_mask_in__, softmax_out_out_, attention_out_out_, scale_value, keep_prob, pre_tockens, next_tockens, head_num, input_layout)
    result = check_op_cap("op_select_format", "MultiHeadAttentionScore", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(query_in__, key_in__, value_in__, real_shift_in__=None, drop_mask_in__=None, padding_mask_in__=None, atten_mask_in__=None, softmax_out_out_=None, attention_out_out_=None, scale_value=1, keep_prob=1, pre_tockens=2147483647, next_tockens=2147483647, head_num=0, input_layout="", impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(query_in__, key_in__, value_in__, real_shift_in__, drop_mask_in__, padding_mask_in__, atten_mask_in__, softmax_out_out_, attention_out_out_, scale_value, keep_prob, pre_tockens, next_tockens, head_num, input_layout)
    result = check_op_cap("get_op_specific_info", "MultiHeadAttentionScore", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
