...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'weight_scale', 'activation_scale', 'bias', 'quant_scale', 'quant_offset', 'group_index']
    output_names = ['y', 'scale']
...
def _build_args(x_in__, weight_scale_in__, activation_scale_in__, bias_in__, quant_scale_in__, quant_offset_in__, group_index_in__, y_out_, scale_out_, activate_left, quant_mode):
    __inputs__ = []
    for arg in [x_in__, weight_scale_in__, activation_scale_in__, bias_in__, quant_scale_in__, quant_offset_in__, group_index_in__]:
        if arg != None:
    ...
    for arg in [y_out_, scale_out_]:
        if arg != None:
            ...
            __outputs__.append(arg)
    __attrs__ = []
    if activate_left != None:
        attr = {}
        attr["name"] = "activate_left"
        attr["dtype"] = "bool"
        attr["value"] = activate_left
...
@tbe_register.register_operator("DequantSwigluQuant", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_STR, para_check.KERNEL_NAME)
def dequant_swiglu_quant(x_in__, weight_scale_in__=None, activation_scale_in__=None, bias_in__=None, quant_scale_in__=None, quant_offset_in__=None, group_index_in__=None, y_out_=None, scale_out_=None, activate_left=False, quant_mode="static", kernel_name="dequant_swiglu_quant", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_scale_in__, activation_scale_in__, bias_in__, quant_scale_in__, quant_offset_in__, group_index_in__, y_out_, scale_out_, activate_left, quant_mode)
    ...
    origin_func_name = "dequant_swiglu_quant"
    ascendc_src_dir_ex = "dequant_swiglu_quant"
    ascendc_src_dir = "dequant_swiglu_quant"
    ascendc_src_file = "dequant_swiglu_quant.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator DequantSwigluQuant, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, weight_scale_in__, activation_scale_in__, bias_in__, quant_scale_in__, quant_offset_in__, group_index_in__], origin_outputs = [y_out_, scale_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'optional', 'optional', 'optional', 'optional', 'optional', 'optional', 'required', 'required'], init_value_list = [None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(x_in__, weight_scale_in__=None, activation_scale_in__=None, bias_in__=None, quant_scale_in__=None, quant_offset_in__=None, group_index_in__=None, y_out_=None, scale_out_=None, activate_left=False, quant_mode="static", impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_scale_in__, activation_scale_in__, bias_in__, quant_scale_in__, quant_offset_in__, group_index_in__, y_out_, scale_out_, activate_left, quant_mode)
    result = check_op_cap("op_select_format", "DequantSwigluQuant", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, weight_scale_in__=None, activation_scale_in__=None, bias_in__=None, quant_scale_in__=None, quant_offset_in__=None, group_index_in__=None, y_out_=None, scale_out_=None, activate_left=False, quant_mode="static", impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_scale_in__, activation_scale_in__, bias_in__, quant_scale_in__, quant_offset_in__, group_index_in__, y_out_, scale_out_, activate_left, quant_mode)
    result = check_op_cap("get_op_specific_info", "DequantSwigluQuant", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
