...
def _build_args(x_in__, size_in__, y_out_, align_corners, scale):
    __inputs__ = []
...
@tbe_register.register_operator("UpsampleLinear1d", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def upsample_linear1d(x_in__, size_in__, y_out_, align_corners=False, scale=0, kernel_name="upsample_linear1d", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, size_in__, y_out_, align_corners, scale)
    ...
    origin_func_name = "upsample_linear1d"
    ascendc_src_dir_ex = "upsample_linear1d"
    ascendc_src_dir = "upsample_linear1d"
    ascendc_src_file = "upsample_linear1d.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator UpsampleLinear1d, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "UpsampleLinear1d"
...
def op_select_format(x_in__, size_in__, y_out_, align_corners=False, scale=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, size_in__, y_out_, align_corners, scale)
    result = check_op_cap("op_select_format", "UpsampleLinear1d", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, size_in__, y_out_, align_corners=False, scale=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, size_in__, y_out_, align_corners, scale)
    result = check_op_cap("get_op_specific_info", "UpsampleLinear1d", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
