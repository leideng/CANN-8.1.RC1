...
/*!
 * \file swin_attention_score_quant_int8.h
 * \brief
 */
#ifndef SWIN_ATTENTION_SCORE_QUANT_INT8_H
#define SWIN_ATTENTION_SCORE_QUANT_INT8_H
...
class SwinAttentionScoreQuant<int8_t, DTYPE_OUT, DTYPE_SCALE, DTYPE_BIAS, DTYPE_MASK, true> {
...
public:
    ...
    __aicore__ inline void Init(__gm__ uint8_t *query, __gm__ uint8_t *key, __gm__ uint8_t *value,
        ...
        __gm__ uint8_t *paddingMask1, __gm__ uint8_t *attentionOut, __gm__ uint8_t *workspace,
        const SwinAttentionScoreQuantTilingData *tiling, AscendC::TPipe *tPipe)
    {
        ...
        mask1Gm.SetGlobalBuffer((__gm__ DTYPE_MASK *)paddingMask1);
        ...
        pipe->InitBuffer(mask1TBuf, singleCoreM * round16S * sizeof(DTYPE_MASK));
    }
    __aicore__ inline void Process()
    {
        ...
        eventID0 =  static_cast<event_t>(GetTPipePtr()->AllocEventID<AscendC::HardEvent::V_MTE2>());
        ...
        for (uint32_t sIdx = 0; sIdx < numPerS; sIdx++) {
            ...
            for (uint32_t nIdx = 0; nIdx < n; nIdx++) {
                CopyInMask(nIdx, sIdx);
                for (uint32_t bIdx = coreIdx; bIdx < coreLoops; bIdx += blockDim) {
                    ...
                    uint32_t bOffset = (bIdx * n + nIdx) * s * h;
                    Stage1BmmQK(bIdx, nIdx, sIdx, bOffset, qL1Tensor, kL1Tensor);
                    qL1TQue.FreeTensor(qL1Tensor);
                    kL1TQue.FreeTensor(kL1Tensor);
                    Stage2Vec(bIdx, nIdx, sIdx, bOffset, pL1Tensor, vL1Tensor);
                    Stage3BmmPV(bIdx, nIdx, sIdx, bOffset, pL1Tensor, vL1Tensor);
        ...
        GetTPipePtr()->ReleaseEventID<AscendC::HardEvent::V_MTE2>(eventID0);
    }
...
protected:
    __aicore__ inline void CopyInMask(uint32_t nIdx, uint32_t sIdx)
    {
        AscendC::LocalTensor<DTYPE_MASK> inputMask1 = mask1TBuf.template Get<DTYPE_MASK>();
        AscendC::LocalTensor<DTYPE_MASK> transTensor = pUbTBuf.template Get<DTYPE_MASK>();
        uint32_t maskIdx = nIdx * s * s;

        AscendC::WaitFlag<AscendC::HardEvent::V_MTE2>(eventID0);
        CopyND2NZ(inputMask1, mask1Gm[maskIdx], transTensor, sIdx * singleCoreM, 0, actualM, s, s);
    }

    __aicore__ inline void Stage1BmmQK(uint32_t bIdx, uint32_t nIdx, uint32_t sIdx, uint32_t bOffset,
        ...
        AscendC::LocalTensor<DTYPE_MASK> &biasQuantUb)
    {
        ...
        for (uint32_t i = 0; i < round16S / BLOCK_SIZE_16; i++) {
            Mul(src[i * roundM * BLOCK_SIZE_16], src[i * roundM * BLOCK_SIZE_16], scaleQuantUb[i * BLOCK_SIZE_16],
                mask0, roundM / BLOCK_NUM_PER_VEC, { 1, 1, 0, BLOCK_NUM_PER_VEC, BLOCK_NUM_PER_VEC, 0 });
            Add(src[i * roundM * BLOCK_SIZE_16], src[i * roundM * BLOCK_SIZE_16], biasQuantUb[i * BLOCK_SIZE_16],
                mask0, roundM / BLOCK_NUM_PER_VEC, { 1, 1, 0, BLOCK_NUM_PER_VEC, BLOCK_NUM_PER_VEC, 0 });
        }
        
        for (uint32_t i = 0; i < round16S / BLOCK_SIZE_16; i += 2) {  // 将2个BLOCK的fp16数据转成1个BLOCK的int8数据
            Cast(dst[i / 2 * roundM * BLOCK_SIZE_32], src[i * roundM * BLOCK_SIZE_16], AscendC::RoundMode::CAST_NONE,
                mask1, roundM, { 1, (uint16_t)roundM, 1, 1 });     // 输出每次循环偏移2列
        }
    }

    __aicore__ inline void Stage2Vec(uint32_t bIdx, uint32_t nIdx, uint32_t sIdx, uint32_t bOffset,
        AscendC::LocalTensor<DTYPE_IN> &pL1Tensor, AscendC::LocalTensor<DTYPE_IN> &vL1Tensor)
    {
        ...
        Add(pUbTensor, pUbTensor, inputMask1, roundM * round16S);
        if (bIdx + blockDim >= b) {
            AscendC::SetFlag<AscendC::HardEvent::V_MTE2>(eventID0);
        }
        if (s > CRITICAL_S_DIM) {
            AscendC::SoftMax<DTYPE_MASK, true, false, true>(pUbTensor, softmaxSumUb, softmaxMaxUb, pUbTensor,
                tilingData->softmaxTilingData, AscendC::SoftMaxShapeInfo { roundM, round16S, actualM, s });
        } else {
            AscendC::SoftMax<DTYPE_MASK, true, false, true>(pUbTensor, softmaxSumUb, softmaxMaxUb, pUbTensor,
        ...
        event_t eventIDVToMte3ForP = static_cast<event_t>(GetTPipePtr()->FetchEventID(AscendC::HardEvent::V_MTE3));
        AscendC::SetFlag<AscendC::HardEvent::V_MTE3>(eventIDVToMte3ForP);
        AscendC::WaitFlag<AscendC::HardEvent::V_MTE3>(eventIDVToMte3ForP);
        ...
        event_t eventIDVToMte3ForV = static_cast<event_t>(GetTPipePtr()->FetchEventID(AscendC::HardEvent::V_MTE3));
        AscendC::SetFlag<AscendC::HardEvent::V_MTE3>(eventIDVToMte3ForV);
        AscendC::WaitFlag<AscendC::HardEvent::V_MTE3>(eventIDVToMte3ForV);
    ...
    __aicore__ inline void Stage3BmmPV(uint32_t bIdx, uint32_t nIdx, uint32_t sIdx, uint32_t bOffset,
        AscendC::LocalTensor<DTYPE_IN> &pL1Tensor, AscendC::LocalTensor<DTYPE_IN> &vL1Tensor)
    {
    ...
    AscendC::GlobalTensor<DTYPE_MASK> mask1Gm;
