...
# 'pylint: disable=too-many-instance-attributes,unused-argument,too-many-lines,huawei-too-many-arguments
...
# 'pylint: disable=too-many-instance-attributes,unused-argument,too-many-lines,huawei-too-many-arguments
# 'pylint: disable=too-many-lines,too-many-locals,too-many-statements,unused-variable,too-many-arguments
def op_select_format_for_fp32(x1, x2, grad, y, ksize, strides,
    ...
    if dynamic:
        ...
        x1_dtype, x2_dtype, grad_dtype, y_dtype = \
            ["float", "float16"], ["float", "float16"], ["float", "float16"], ["float", "float"]
    else:
        x1_dy_format, x2_dy_format, grad_dy_format, y_dy_format = \
                ["NC1HWC0", "NC1HWC0"], ["NC1HWC0", "NC1HWC0"], ["NC1HWC0", "NC1HWC0"], ["NC1HWC0", "NC1HWC0"]
...
def op_select_format(x1, x2, grad, y, ksize, strides,
    ...
    if soc_ver in (ASCEND_910B, ASCEND_910_93):
        return op_select_format_for_fp32(x1, x2, grad, y, ksize, strides,
                                         padding, data_format=data_format, kernel_name=kernel_name)
    dynamic = False
    for i, _ in enumerate(x1_shape):
        if x1_shape[i] == -1 or x2_shape[i] == -1 or grad_shape[i] == -1:
            dynamic = True
...
class MaxpoolGrad:
    ...
    def _vector_op(self, operator, src1, src2, dst, dtype, ele_num, stride_cofig=None, offset=None):
        ...
        if operator == "vmuls":
            ...
            if repeat_max_loop > 0:
                ...
                dst_offset += \
                    BLOCK_SIZE // (get_bit_len(dst.dtype.lower()) // 8) * stride_cofig[2] * 255
                src1_offset += \
                    BLOCK_SIZE // (get_bit_len(src1.dtype.lower()) // 8) * stride_cofig[3] * 255
            if remain_max_loop > 0:
                ...
                dst_offset += BLOCK_SIZE // (get_bit_len(dst.dtype.lower()) //
                                             8) * stride_cofig[2] * remain_max_loop
                src1_offset += BLOCK_SIZE // (get_bit_len(src1.dtype.lower()) //
        ...
        if operator == "vadd":
            ...
            if stride_cofig[3] > 255 or stride_cofig[4] > 255 or stride_cofig[5] > 255:
                dst_offset_stride = BLOCK_SIZE // (get_bit_len(dst.dtype.lower()) // 8) * stride_cofig[3]
                src1_offset_stride = BLOCK_SIZE // (get_bit_len(src1.dtype.lower()) // 8) * stride_cofig[4]
                src2_offset_stride = BLOCK_SIZE // (get_bit_len(src2.dtype.lower()) // 8) * stride_cofig[5]
                if repeat_max_loop > 0:
                ...
                if repeat_max_loop > 0:
                    ...
                    src1_offset += \
                        BLOCK_SIZE // (get_bit_len(src1.dtype.lower()) // 8) * stride_cofig[4] * 255
                    src2_offset += \
                        BLOCK_SIZE // (get_bit_len(src2.dtype.lower()) // 8) * stride_cofig[5] * 255
                if remain_max_loop > 0:
                    ...
                    src2_offset += BLOCK_SIZE // (get_bit_len(src2.dtype.lower()) //
    ...
    def _calc_mask(self, index_h, index_w, mask_shape, ori_output_ub, ori_input_col_ub, mask_or, mask_not,
        ...
        mask_ub = self.tik_instance.Tensor('uint16', mask_shape, name='mask_ub', scope=tik.scope_ubuf)
        if not self.check_load3d_support:
            mask_ub_temp = self.tik_instance.Tensor('uint16', mask_shape, name='mask_ub_temp', scope=tik.scope_ubuf)
            mask_ub_not_temp = self.tik_instance.Tensor('uint16', mask_shape, name='mask_ub_not_temp',
    ...
    def _mov_func(self, cut_ho_nums_index, cut_ho_nums, remain_ho_nums, each_process_ho, each_process_hi, each_valid_ho,
                  col2img_fp32_ub, temp_tensor_ub, pad, ori_input_ub=None):
        pad_left, pad_right, pad_top, _ = pad
        ...
        col2img_fp16_ub = self.tik_instance.Tensor("float16",
                                                   ...
                                                   scope=tik.scope_ubuf) if ori_input_ub is None else ori_input_ub
    ...
    def _move_func_block(self, cut_ho_nums_index, cut_ho_nums, start_h, end_h, each_process_ho, valid_hi_block,
                         col2img_fp32_ub, temp_tensor_ub, remained_hi, remain, pad, ori_input_ub=None):
        pad_left, pad_right, pad_top, _ = pad
        wi = self.wi + pad_left + pad_right
    ...
    def _tilling_factor(self, ori_input_shape, pad):
        ...
        if not self.check_load3d_support:
            temp_output_num = 3
            wi_max = wi_temp
            if self.padding == "VALID" and self.stride_w > wi_temp:
                wi_max = self.stride_w
            ori_output_shape = (_ceil_div(self.wo, 16), 16, c0_local)
            total_used_ub = self.stride_h * wi_max * c0_local * (fp16_data_size + fp32_data_size) + \
                            _cal_shape_ele(ori_output_shape) * fp16_data_size * temp_output_num + FIXED_USED_UB
            if total_used_ub > SIZE_UB:
                self.exceed_ub = True
                each_process_hi = self.kh
        ...
        if not self.check_load3d_support and each_process_wo_min < self.wo and each_process_wo_min < C0:
            used_ub_size = (col_size_times + mask_size_times + tensor_size_times + ori_size_times) * C0
            if used_ub_size > const_remain:
                each_process_wo_min = 1
                each_process_wo_max = 1
    ...
    def _not_tilling(self, n_index, c1_index, each_process_ho_block, each_process_hi_block, mov_len_ho, mov_len_hi,
        ...
        if not self.check_load3d_support:
            ...
            if self.padding == "VALID":
                self.tik_instance.data_move(ori_input_ub, self.ori_input_gm[src_input_offset],
                                            0, 1, self.hi * self.wi, 0, 0)
            else:
        ...
        if (self.check_load3d_support and self.stride_h * wi * C0 * 6 > SIZE_L1) or self.exceed_ub:
            ...
            with self.tik_instance.for_range(0, mov_len_ho) as ho_idx:
                ...
                with self.tik_instance.for_range(0, self.kh, thread_num=1) as index_h:
                    with self.tik_instance.for_range(0, self.kw, thread_num=1) as index_w:
                        if self.check_load3d_support:
                            ...
                            self.tik_instance.data_move(ori_input_col_ub[output_nums * ho_idx],
                                                        ori_input_ub[wi * C0 * (ho_idx * self.stride_h + index_h) +
                                                                     index_w * C0], 
                                                        0, shape_wo, 1, self.stride_w - 1, 0, 0)
                ...
                col2img_fp16_ub = \
                    self.tik_instance.Tensor("float16",
                                             ...
                                             name="col2img_fp16_ub",
                                             scope=tik.scope_ubuf) \
                        if self.check_load3d_support else ori_input_ub
                self._vconv(col2img_fp32_ub, 0, col2img_fp16_ub, 0, _cal_shape_ele(col2img_fp32_ub.shape), "float32")
            ...
            with self.tik_instance.for_range(0, self.kh, thread_num=1) as index_h:
                with self.tik_instance.for_range(0, self.kw, thread_num=1) as index_w:
                    if self.check_load3d_support:
                        ...
                        self._vector_dup(ori_input_col_ub, 0, ori_input_col_ub.shape, MIN_VALUE_FP16, "float16")
                        with self.tik_instance.for_range(0, mov_len_ho) as ho_idx:
                            self.tik_instance.data_move(ori_input_col_ub[shape_wo * C0 * ho_idx],
                                                        ori_input_ub[wi * C0 * (ho_idx * self.stride_h + index_h) +
                                                                     index_w * C0], 
                                                        0, shape_wo, 1, self.stride_w - 1, 0, 0)
            ...
            col2img_fp16_ub = \
                self.tik_instance.Tensor("float16", col2img_ub_shape, name="col2img_fp16_ub",
                                         scope=tik.scope_ubuf) if self.check_load3d_support else ori_input_ub
            self._vconv(col2img_fp32_ub, 0, col2img_fp16_ub, 0, _cal_shape_ele(col2img_fp32_ub.shape), "float32")
        ...
        def process_ho(output_data_nums, cut_ho_nums_index, each_valid_ho, remained_hi):
            ...
            with self.tik_instance.for_range(0, self.kh, thread_num=1) as index_h:
                with self.tik_instance.for_range(0, self.kw, thread_num=1) as index_w:
                    if self.check_load3d_support:
                        ...
                        self._vector_dup(ori_input_col_ub, 0, ori_input_col_ub.shape, MIN_VALUE_FP16, "float16")
                        with self.tik_instance.for_range(0, each_process_ho) as ho_idx:
                            output_nums = self.wo * C0
                            self.tik_instance.data_move(ori_input_col_ub[output_nums * ho_idx],
                                                        ori_input_ub[wi * C0 * (ho_idx * self.stride_h + index_h) +
                                                                     index_w * C0],
                                                        0, self.wo, 1, self.stride_w - 1, 0, 0)
                    mask_shape = (_ceil_div(_cal_shape_ele(ori_output_ub.shape[:2]), MASK128_VALUE) * MASK128_VALUE, )
                    mask_ub = self._calc_mask(index_h, index_w, mask_shape, ori_output_ub, ori_input_col_ub, mask_or,
                                              mask_not, is_vadds=True)
                    grad_sel_ub = self._vsel_grad_col(mask_ub, grad_ub)
                    grad_sel_ub_fp32 = self.tik_instance.Tensor("float32",
                                                                grad_sel_ub.shape,
                                                                name='grad_sel_ub_fp32',
            ...
            if self.tile_h_to_block:
                ...
                self._mov_func(cut_ho_nums_index, cut_ho_nums, remain_ho_nums, each_process_ho, each_process_hi,
                               each_valid_ho, col2img_fp32_ub, temp_tensor_ub, pad, ori_input_ub)
            return remained_hi
    ...
    def _tilling_l1_ho_wo(self, each_process_wo, n_index, c1_index, each_process_ho_block, each_process_hi_block,
        ...
        with self.tik_instance.for_range(0, mov_len_ho, thread_num=1) as ho_index:
            ...
            if remain_wo_nums:
                ...
                ori_output_ub_shape = (each_process_remain_div16, 16, C0)
                ori_output_ub = self.tik_instance.Tensor(self.dtype,
                                                         ori_output_ub_shape,
                                                         name='ori_output_ub',
                                                         scope=tik.scope_ubuf)
                ...
                if self.check_load3d_support:
                    ori_input_col_ub_shape = (each_process_remain_div16 * 16, C0)
                    ori_input_col_ub = self.tik_instance.Tensor(self.dtype,
                ...
                if exceeding_l1_memory:
                    ...
                    if self.check_load3d_support:
                        ...
                        if self.kw > self.stride_w:
                            each_process_remain_wi_align = (each_process_remain_div16 * 16 - 1) * self.stride_w + \
                                                           self.kw
                        else:
                            each_process_remain_wi_align = each_process_remain_div16 * 16 * self.stride_w
                        ori_input_ub_shape = (each_process_hi, each_process_remain_wi_align, C0)
                        ori_input_ub = self.tik_instance.Tensor(self.dtype, ori_input_ub_shape,
                                                                name="ori_input_ub", scope=tik.scope_ubuf)
                        self._vector_dup(ori_input_ub, 0, ori_input_ub_shape, MIN_VALUE_FP16, "float16")
                        ...
                        with self.tik_instance.if_scope(load3d_start_h <= pad_top):
                            ub_offset = each_process_remain_wi_align * C0 * (pad_top - load3d_start_h) + \
                                        actual_pad_left * C0
                            self.tik_instance.data_move(ori_input_ub[ub_offset], self.ori_input_gm[gm_offset],
                                                        0, mov_len_h_l1, cut_wo_burst, src_stride,
                                                        each_process_remain_wi_align - cut_wo_burst)
                        with self.tik_instance.else_scope():
                            self.tik_instance.data_move(ori_input_ub[actual_pad_left * C0],
                                                        self.ori_input_gm[gm_offset],
                                                        0, mov_len_h_l1, cut_wo_burst, src_stride,
...
class MaxpoolGradAtomic:
    ...
    def __init__(self, shape_list, params):
        ...
        self.ub_maxsize = tbe_platform. \
                              get_soc_spec(tbe_platform.
                                           UB_SIZE) // self.num_bit
        self.L1_maxsize = tbe_platform. \
                              get_soc_spec(tbe_platform.
                                           L1_SIZE) // self.num_bit
        self.orig_x_gm = None
        self.orig_y_gm = None
        self.grads_gm = None
        self.ou_y_gm = None
    ...
    def _vector_op(self, tik_instance, operator,
        ...
        if operator == "vadd":
            ...
            if repeat_max_loop > 0:
                ...
                dst_offset += BLOCK_SIZE // (get_bit_len(
                    dst.dtype.lower()) // 8) * stride_config[3] * 255
                src1_offset += BLOCK_SIZE // (get_bit_len(
                    src1.dtype.lower()) // 8) * stride_config[4] * 255
                src2_offset += BLOCK_SIZE // (get_bit_len(
    ...
    def _pure_atomic_tiling_do_ho(self, tik_instance, core_loop,
        ...
        '''
        do_batch = model[0]
        ho_batch = model[1] if self.check_load3d_support else 1
        wo_batch = model[2]
        c0 = self.c0
        c1 = self.c1
                            ...
                            with tik_instance.for_range(0, self.kw, thread_num=1) as idx_w:
                                if self.check_load3d_support:
                                    ...
                                    self._set_vector_dup(tik_instance, param.col_in_size, col_in_buf, 0,
                                                         MIN_VALUE_FP16, "float16")
                                    with tik_instance.for_range(0, ho_batch) as ho_idx:
                                        tik_instance.data_move(col_in_buf[wo_batch * c0 * ho_idx],
                                                               ori_in_buf[map_wi * c0 * (ho_idx * self.sh + idx_h) + 
                                                                          idx_w * c0],
                                                               0, self.wo, 1, self.sw - 1, 0, 0)
