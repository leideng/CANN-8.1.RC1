...
@register_operator_compute("not_equal", op_mode="static", support_fusion=True)
def not_equal_compute(input_x, input_y, output_z, kernel_name="not_equal"):
    ...
    if dtype_x == "float32":
        tensor_min = tbe.broadcast(tvm.const(Constant.SCALAR_MIN_FP32,
                                                     dtype="float32"),
                                           shape_broadcast)
        ...
        tensor_min = tbe.broadcast(tvm.const(Constant.SCALAR_MIN_FP16,
                                                     dtype="float16"),
                                           shape_broadcast)
    ...
    if dtype_x in ("int8", "uint8", "int32"):
