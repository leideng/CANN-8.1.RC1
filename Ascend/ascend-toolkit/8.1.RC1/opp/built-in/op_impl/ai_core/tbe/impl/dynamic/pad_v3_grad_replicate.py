...
def _build_args(x_in__, paddings_in__, y_out_, mode, paddings_contiguous):
    __inputs__ = []
    ...
    if paddings_contiguous != None:
        attr = {}
        attr["name"] = "paddings_contiguous"
        attr["dtype"] = "bool"
        attr["value"] = paddings_contiguous
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("PadV3GradReplicate", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def pad_v3_grad_replicate(x_in__, paddings_in__, y_out_, mode, paddings_contiguous, kernel_name="pad_v3_grad_replicate", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, paddings_in__, y_out_, mode, paddings_contiguous)
    ...
    origin_func_name = "pad_v3_grad_replicate"
    ascendc_src_dir_ex = "pad_v3_grad_replicate"
    ascendc_src_dir = "pad_v3_grad_replicate"
    ascendc_src_file = "pad_v3_grad_replicate.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator PadV3GradReplicate, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "PadV3GradReplicate"
...
def op_select_format(x_in__, paddings_in__, y_out_, mode, paddings_contiguous, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, paddings_in__, y_out_, mode, paddings_contiguous)
    result = check_op_cap("op_select_format", "PadV3GradReplicate", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, paddings_in__, y_out_, mode, paddings_contiguous, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, paddings_in__, y_out_, mode, paddings_contiguous)
    result = check_op_cap("get_op_specific_info", "PadV3GradReplicate", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
