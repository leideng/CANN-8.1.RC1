...
@register_operator_compute("asin", op_mode="static", support_fusion=True)
def asin_compute(x, y, kernel_name="asin"):
    ...
    res_1_dtype = res_1.dtype
    if res_1_dtype != choice_1.dtype and tbe_platform.api_check_support("tbe.dsl.vmul", "float32"):
        res_1 = tbe.cast_to(res_1, "float32")
        choice_1 = tbe.cast_to(choice_1, "float32")
    res_1 = tbe.vmul(res_1, choice_1)
    if res_1_dtype != res_1.dtype:
        res_1 = tbe.cast_to(res_1, res_1_dtype)
    ...
    res_2 = tbe.cast_to(res_2, choice_2.dtype)
    res_2_dtype = res_2.dtype
    if res_2_dtype != choice_2.dtype and tbe_platform.api_check_support("tbe.dsl.vmul", "float32"):
        res_2 = tbe.cast_to(res_2, "float32")
        choice_2 = tbe.cast_to(choice_2, "float32")
    res_2 = tbe.vmul(res_2, choice_2)
    if res_2_dtype != res_2.dtype:
        res_2 = tbe.cast_to(res_2, res_2_dtype)

    # Restore sign
    res_1 = tbe.vadd(res_1, res_2)
    res_1 = tbe.cast_to(res_1, sign.dtype)
    res_1_dtype = res_1.dtype
    if res_1_dtype != sign.dtype and tbe_platform.api_check_support("tbe.dsl.vmul", "float32"):
        res_1 = tbe.cast_to(res_1, "float32")
