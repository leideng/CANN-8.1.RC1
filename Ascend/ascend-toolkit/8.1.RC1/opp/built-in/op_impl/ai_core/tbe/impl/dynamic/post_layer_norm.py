...
def _build_args(x_in__, res_in_in__, gamma_in__, beta_in__, result_out_, epsilon, zoomScaleValue):
    __inputs__ = []
    ...
    if zoomScaleValue != None:
        attr = {}
        attr["name"] = "zoomScaleValue"
        attr["dtype"] = "float"
        attr["value"] = zoomScaleValue
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("PostLayerNorm", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def post_layer_norm(x_in__, res_in_in__, gamma_in__, beta_in__, result_out_, epsilon=0, zoomScaleValue=0, kernel_name="post_layer_norm", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, res_in_in__, gamma_in__, beta_in__, result_out_, epsilon, zoomScaleValue)
    ...
    origin_func_name = "post_layer_norm"
    ascendc_src_dir_ex = "post_layer_norm"
    ascendc_src_dir = "post_layer_norm"
    ascendc_src_file = "post_layer_norm.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator PostLayerNorm, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "PostLayerNorm"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, res_in_in__, gamma_in__, beta_in__], origin_outputs = [result_out_],\
...
def op_select_format(x_in__, res_in_in__, gamma_in__, beta_in__, result_out_, epsilon=0, zoomScaleValue=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, res_in_in__, gamma_in__, beta_in__, result_out_, epsilon, zoomScaleValue)
    result = check_op_cap("op_select_format", "PostLayerNorm", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, res_in_in__, gamma_in__, beta_in__, result_out_, epsilon=0, zoomScaleValue=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, res_in_in__, gamma_in__, beta_in__, result_out_, epsilon, zoomScaleValue)
    result = check_op_cap("get_op_specific_info", "PostLayerNorm", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
