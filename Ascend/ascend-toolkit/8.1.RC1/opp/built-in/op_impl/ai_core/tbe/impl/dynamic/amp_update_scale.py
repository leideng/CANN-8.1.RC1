...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['current_scale', 'growth_tracker', 'found_inf']
    output_names = ['updated_scale', 'updated_growth_tracker']
...
def _build_args(current_scale_in__, growth_tracker_in__, found_inf_in__, updated_scale_out_, updated_growth_tracker_out_, growth_factor, backoff_factor, growth_interval):
    __inputs__ = []
    for arg in [current_scale_in__, growth_tracker_in__, found_inf_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [updated_scale_out_, updated_growth_tracker_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
            ...
            __outputs__.append(arg)
    __attrs__ = []
    if growth_factor != None:
        attr = {}
        attr["name"] = "growth_factor"
        attr["dtype"] = "float"
        attr["value"] = growth_factor
        __attrs__.append(attr)
    if backoff_factor != None:
        attr = {}
        attr["name"] = "backoff_factor"
        attr["dtype"] = "float"
        attr["value"] = backoff_factor
        __attrs__.append(attr)
    if growth_interval != None:
        attr = {}
        attr["name"] = "growth_interval"
        attr["dtype"] = "int"
        attr["value"] = growth_interval
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("AmpUpdateScale", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def amp_update_scale(current_scale_in__, growth_tracker_in__, found_inf_in__, updated_scale_out_, updated_growth_tracker_out_, growth_factor, backoff_factor, growth_interval, kernel_name="amp_update_scale", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(current_scale_in__, growth_tracker_in__, found_inf_in__, updated_scale_out_, updated_growth_tracker_out_, growth_factor, backoff_factor, growth_interval)
    ...
    origin_func_name = "amp_update_scale"
    ascendc_src_dir_ex = "amp_update_scale"
    ascendc_src_dir = "amp_update_scale"
    ascendc_src_file = "amp_update_scale.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator AmpUpdateScale, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "AmpUpdateScale"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[current_scale_in__, growth_tracker_in__, found_inf_in__], origin_outputs = [updated_scale_out_, updated_growth_tracker_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required'], init_value_list = [None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(current_scale_in__, growth_tracker_in__, found_inf_in__, updated_scale_out_, updated_growth_tracker_out_, growth_factor, backoff_factor, growth_interval, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(current_scale_in__, growth_tracker_in__, found_inf_in__, updated_scale_out_, updated_growth_tracker_out_, growth_factor, backoff_factor, growth_interval)
    result = check_op_cap("op_select_format", "AmpUpdateScale", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(current_scale_in__, growth_tracker_in__, found_inf_in__, updated_scale_out_, updated_growth_tracker_out_, growth_factor, backoff_factor, growth_interval, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(current_scale_in__, growth_tracker_in__, found_inf_in__, updated_scale_out_, updated_growth_tracker_out_, growth_factor, backoff_factor, growth_interval)
    result = check_op_cap("get_op_specific_info", "AmpUpdateScale", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
