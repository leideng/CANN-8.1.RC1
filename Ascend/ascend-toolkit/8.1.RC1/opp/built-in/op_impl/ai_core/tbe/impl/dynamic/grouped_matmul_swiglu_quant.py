...
@tbe_register.register_operator("GroupedMatmulSwigluQuant", trans_bool_to_s8=False)
...
def grouped_matmul_swiglu_quant(x_in__, weight_in__, weight_scale_in__, x_scale_in__, group_list_in__, y_out_, y_scale_out_, kernel_name="grouped_matmul_swiglu_quant", impl_mode=""):
    ...
    origin_func_name = "grouped_matmul_swiglu_quant"
    ascendc_src_dir_ex = "grouped_matmul_swiglu_quant"
    ascendc_src_dir = "grouped_matmul_swiglu_quant"
    ascendc_src_file = "grouped_matmul_swiglu_quant.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator GroupedMatmulSwigluQuant, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "GroupedMatmulSwigluQuant"
...
def op_select_format(x_in__, weight_in__, weight_scale_in__, x_scale_in__, group_list_in__, y_out_, y_scale_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, weight_scale_in__, x_scale_in__, group_list_in__, y_out_, y_scale_out_)
    result = check_op_cap("op_select_format", "GroupedMatmulSwigluQuant", __inputs__, __outputs__, __attrs__)
...
def get_op_specific_info(x_in__, weight_in__, weight_scale_in__, x_scale_in__, group_list_in__, y_out_, y_scale_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, weight_scale_in__, x_scale_in__, group_list_in__, y_out_, y_scale_out_)
    result = check_op_cap("get_op_specific_info", "GroupedMatmulSwigluQuant", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
