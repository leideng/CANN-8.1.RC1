...
def _conv_remove_pad(input_list, align_shape, remove_padded_column_flag, invalid_data_rm_flag):
    """
    get conv remove pad branch lambda calculation function
    """
    x, res_f16 = input_list
...
def _vector_dequant_v100(x, x_shape, align_shape, deq_scale, flags):
    ...
    elif check_split_w(x):
        res = _vector_dequant_split_w(x, x_shape, align_shape, deq_scale, relu_flag)
    else:
        res_f16 = tvm.compute(align_shape,
...
def _scalar_dequant_v100(x, x_shape, align_shape, deq_scale, flags):
    ...
    if conv_flag:
        ...
        res = _scalar_dequant_split_w(x, x_shape, align_shape, deq_scale)
    else:
        res_f16 = tvm.compute(align_shape,
...
def _vector_dequant_split_w(x, x_shape, align_shape, deq_scale, relu_flag):
    real_w = align_shape[-2]
    if util.is_support_v200():
        res_f16 = tvm.compute(
            align_shape,
            lambda batch, c1, h, w, c0:
                tvm.vdeq_cast(x(batch, c1, h * real_w + w, c0), deq_scale(0, c1, 0, 0, c0),
        ...
        if relu_flag:
            res_f16 = tvm.compute(
                align_shape,
                lambda batch, c1, h, w, c0:
                    tvm.relu(x(batch, c1, h*real_w + w, c0).astype("float16") * deq_scale(0, c1, 0, 0, c0)),
            ...
            res_f16 = tvm.compute(
                align_shape,
                lambda batch, c1, h, w, c0:
                    x(batch, c1, h * real_w + w, c0).astype("float16") * deq_scale(0, c1, 0, 0, c0),
                name="dequant1", tag="dequant1_vector", attrs={"relu_flag": 0}
            )
    res = tvm.compute(
        x_shape,
        lambda n, c1, hw, c0: res_f16(n, c1, hw // real_w, hw % real_w, c0),
        name="dequant_remove_pad",
        tag="dequant_remove_pad"
    )
    return res
...
def _scalar_dequant_split_w(x, x_shape, align_shape, deq_scale):
    real_w = align_shape[-2]
    if util.is_support_v200():
        res_f16 = tvm.compute(
            align_shape,
            lambda batch, c1, h, w, c0:
                tvm.vdeq_cast(x(batch, c1, h * real_w + w, c0), deq_scale(0, 0, 0, 0, 0), dtype="float16"),
            name="dequant", tag="dequant_scale"
        )
    else:
        res_f16 = tvm.compute(
            align_shape,
            lambda batch, c1, h, w, c0:
                x(batch, c1, h * real_w + w, c0).astype("float16") * deq_scale(0, 0, 0, 0, 0),
            name="dequant1", tag="dequant1_scale"
        )
    res = tvm.compute(
        x_shape,
...
def _vector_dequant_v200(x, x_shape, align_shape, deq_scale, flags):
    ...
    if conv_flag:
        ...
        res_f16 = tvm.compute(align_shape,
                              ...
                              name="dequant", tag="dequant_vector",
                              attrs={'no_vector': False})

        res, _ = _conv_remove_pad([x, res_f16], align_shape, remove_padded_column_flag, invalid_data_rm_flag)
    else:
        if check_split_w(x):
            return _vector_dequant_split_w(x, x_shape, align_shape, deq_scale, relu_flag)
        res_f16 = tvm.compute(align_shape,
...
def _scalar_dequant_v200(x, x_shape, align_shape, deq_scale, conv_flag):
    ...
    if conv_flag:
        ...
        if check_split_w(x):
            return _scalar_dequant_split_w(x, x_shape, align_shape, deq_scale)
        res_f16 = tvm.compute(align_shape,
...
@register_operator_compute("ascend_dequant", op_mode="static", support_fusion=True)
def ascend_dequant_compute(x, deq_scale, y, sqrt_mode=False, relu_flag=False, dtype=1, kernel_name="ascend_dequant"):
    ...
    if x.op.tag != "depthwise_conv2d" and not check_split_w(x):
...
def _dequant_v200_v2(x_l0c, deq_ub, align_shape, x_shape, flags):
    """
    dequant for vector in v200
    """
    tensor_flag, relu_flag, _ = flags
    if tensor_flag:
        res_f16 = tvm.compute(align_shape,
...
def _vector_dequant_v100_v2(x_l0c, deq_ub, align_shape, x_shape, flags):
    """
    dequant for vector in v100
    """
    _, relu_flag, sqrt_mode = flags
    if relu_flag:
        res = tvm.compute(align_shape,
...
def _scalar_dequant_v100_v2(x_l0c, deq_ub, align_shape, x_shape, flags):
    """
    dequant for scale in v100
    """
    _, relu_flag, sqrt_mode = flags
...
def ascend_dequant_compute_v2(x, deq_scale, sqrt_mode=False, relu_flag=False):
    """
    int32 -> fp16
    ...
    flags = [tensor_flag, relu_flag, sqrt_mode]
    if tensor_flag:
        if util.is_support_v200():
            res = _dequant_v200_v2(x_l0c, deq_ub, align_shape, x.shape, flags)
        else:
            res = _vector_dequant_v100_v2(x_l0c, deq_ub, align_shape, x.shape, flags)
    else:
        if util.is_support_v200():
            res = _dequant_v200_v2(x_l0c, deq_ub, align_shape, x.shape, flags)
        else:
            res = _scalar_dequant_v100_v2(x_l0c, deq_ub, align_shape, x.shape, flags)
    return res
...
def ascend_dequant(x, deq_scale, y, sqrt_mode=False, relu_mode=False, dtype=1, kernel_name="ascend_dequant"):
    ...
    with tvm.target.cce():
        res = ascend_dequant_compute_v2(input_x, input_deq, sqrt_mode, relu_mode)
