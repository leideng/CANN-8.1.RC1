...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'gamma', 'beta', 'scale', 'offset']
    output_names = ['result']
...
def _build_args(x_in__, gamma_in__, beta_in__, scale_in__, offset_in__, result_out_, epsilon):
    __inputs__ = []
    for arg in [x_in__, gamma_in__, beta_in__, scale_in__, offset_in__]:
        if arg != None:
...
@tbe_register.register_operator("LayerNormQuant", trans_bool_to_s8=False)
...
def layer_norm_quant(x_in__, gamma_in__, beta_in__, scale_in__, offset_in__, result_out_, epsilon=0, kernel_name="layer_norm_quant", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, gamma_in__, beta_in__, scale_in__, offset_in__, result_out_, epsilon)
    ...
    origin_func_name = "layer_norm_quant"
    ascendc_src_dir_ex = "layer_norm_quant"
    ascendc_src_dir = "layer_norm_quant"
    ascendc_src_file = "layer_norm_quant.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator LayerNormQuant, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "LayerNormQuant"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, gamma_in__, beta_in__, scale_in__, offset_in__], origin_outputs = [result_out_],\
...
def op_select_format(x_in__, gamma_in__, beta_in__, scale_in__, offset_in__, result_out_, epsilon=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, gamma_in__, beta_in__, scale_in__, offset_in__, result_out_, epsilon)
    result = check_op_cap("op_select_format", "LayerNormQuant", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, gamma_in__, beta_in__, scale_in__, offset_in__, result_out_, epsilon=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, gamma_in__, beta_in__, scale_in__, offset_in__, result_out_, epsilon)
    result = check_op_cap("get_op_specific_info", "LayerNormQuant", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
