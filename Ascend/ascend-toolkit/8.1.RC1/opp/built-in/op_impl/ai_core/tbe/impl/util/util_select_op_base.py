...
def get_dynamic_param_dict(param_desc_list):
    param_dynamic = {}
    for item in param_desc_list:
        ...
        if item.element.sub_format is not None:
            param_dict["sub_format"] = item.element.sub_format
        param_dynamic[item.classify] = param_dict
    return param_dynamic


def get_dynamic_param_in_json(param_desc_list):
    param_dynamic_in_json = json.dumps(get_dynamic_param_dict(param_desc_list), indent=4)
    return param_dynamic_in_json


# 'pylint: disable=locally-disabled,redefined-builtin
def gen_param(classify, name, datatype, format, unknownshape_format=None, sub_format=None):
...
class Element:
    def __init__(self, name, datatype, format, unknownshape_format, sub_format):
        self.name = name
