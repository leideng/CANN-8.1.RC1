...
def _build_args(x_in__, dy_in__, dx_out_, eps):
    __inputs__ = []
    for arg in [x_in__, dy_in__]:
        if arg != None:
...
@tbe_register.register_operator("LogitGrad", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def logit_grad(x_in__, dy_in__, dx_out_, eps=-1, kernel_name="logit_grad", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, dy_in__, dx_out_, eps)
    ...
    origin_func_name = "logit_grad"
    ascendc_src_dir_ex = "logit_grad"
    ascendc_src_dir = "logit_grad"
    ascendc_src_file = "logit_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator LogitGrad, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "LogitGrad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, dy_in__], origin_outputs = [dx_out_],\
...
def op_select_format(x_in__, dy_in__, dx_out_, eps=-1, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, dy_in__, dx_out_, eps)
    result = check_op_cap("op_select_format", "LogitGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, dy_in__, dx_out_, eps=-1, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, dy_in__, dx_out_, eps)
    result = check_op_cap("get_op_specific_info", "LogitGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
