...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'gates', 'indices', 'locations']
    output_names = ['y']
...
def _build_args(x_in__, gates_in__, indices_in__, locations_in__, y_out_, capacity):
    __inputs__ = []
    for arg in [x_in__, gates_in__, indices_in__, locations_in__]:
        if arg != None:
...
@tbe_register.register_operator("MoeTutelDispatch", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def moe_tutel_dispatch(x_in__, gates_in__, indices_in__, locations_in__, y_out_, capacity, kernel_name="moe_tutel_dispatch", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, gates_in__, indices_in__, locations_in__, y_out_, capacity)
    ...
    origin_func_name = "moe_tutel_dispatch"
    ascendc_src_dir_ex = "moe_tutel_dispatch"
    ascendc_src_dir = "moe_tutel_dispatch"
    ascendc_src_file = "moe_tutel_dispatch.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MoeTutelDispatch, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "MoeTutelDispatch"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, gates_in__, indices_in__, locations_in__], origin_outputs = [y_out_],\
...
def op_select_format(x_in__, gates_in__, indices_in__, locations_in__, y_out_, capacity, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, gates_in__, indices_in__, locations_in__, y_out_, capacity)
    result = check_op_cap("op_select_format", "MoeTutelDispatch", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, gates_in__, indices_in__, locations_in__, y_out_, capacity, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, gates_in__, indices_in__, locations_in__, y_out_, capacity)
    result = check_op_cap("get_op_specific_info", "MoeTutelDispatch", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
