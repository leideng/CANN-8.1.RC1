...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'w', 'scale', 'bias', 'pertoken_scale', 'group_list', 'shared_input', 'logit', 'row_index']
    output_names = ['y']
...
def _build_args(x_in__, w_in__, scale_in__, bias_in__, pertoken_scale_in__, group_list_in__, shared_input_in__, logit_in__, row_index_in__, y_out_, dtype, shared_input_weight, shared_input_offset, transpose_x, transpose_w, output_bs, group_list_type):
    __inputs__ = []
    for arg in [x_in__, w_in__, scale_in__, bias_in__, pertoken_scale_in__, group_list_in__, shared_input_in__, logit_in__, row_index_in__]:
        if arg != None:
    ...
    if shared_input_weight != None:
        attr = {}
        attr["name"] = "shared_input_weight"
        attr["dtype"] = "float"
        attr["value"] = shared_input_weight
        __attrs__.append(attr)
    if shared_input_offset != None:
        attr = {}
        attr["name"] = "shared_input_offset"
        attr["dtype"] = "int"
        attr["value"] = shared_input_offset
        __attrs__.append(attr)
    if transpose_x != None:
        attr = {}
        attr["name"] = "transpose_x"
        attr["dtype"] = "bool"
        attr["value"] = transpose_x
        __attrs__.append(attr)
    if transpose_w != None:
        attr = {}
        attr["name"] = "transpose_w"
        attr["dtype"] = "bool"
        attr["value"] = transpose_w
        __attrs__.append(attr)
    if output_bs != None:
        attr = {}
        attr["name"] = "output_bs"
        attr["dtype"] = "int"
        attr["value"] = output_bs
        __attrs__.append(attr)
...
@tbe_register.register_operator("GroupedMatmulFinalizeRouting", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def grouped_matmul_finalize_routing(x_in__, w_in__, scale_in__=None, bias_in__=None, pertoken_scale_in__=None, group_list_in__=None, shared_input_in__=None, logit_in__=None, row_index_in__=None, y_out_=None, dtype=0, shared_input_weight=1, shared_input_offset=0, transpose_x=False, transpose_w=False, output_bs=0, group_list_type=1, kernel_name="grouped_matmul_finalize_routing", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, w_in__, scale_in__, bias_in__, pertoken_scale_in__, group_list_in__, shared_input_in__, logit_in__, row_index_in__, y_out_, dtype, shared_input_weight, shared_input_offset, transpose_x, transpose_w, output_bs, group_list_type)
    ...
    origin_func_name = "grouped_matmul_finalize_routing"
    ascendc_src_dir_ex = "grouped_matmul_finalize_routing"
    ascendc_src_dir = "grouped_matmul_finalize_routing"
    ascendc_src_file = "grouped_matmul_finalize_routing.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator GroupedMatmulFinalizeRouting, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "GroupedMatmulFinalizeRouting"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, w_in__, scale_in__, bias_in__, pertoken_scale_in__, group_list_in__, shared_input_in__, logit_in__, row_index_in__], origin_outputs = [y_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'optional', 'optional', 'optional', 'optional', 'optional', 'optional', 'optional', 'required'], init_value_list = [None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{"auto_sync": false}')

def op_select_format(x_in__, w_in__, scale_in__=None, bias_in__=None, pertoken_scale_in__=None, group_list_in__=None, shared_input_in__=None, logit_in__=None, row_index_in__=None, y_out_=None, dtype=0, shared_input_weight=1, shared_input_offset=0, transpose_x=False, transpose_w=False, output_bs=0, group_list_type=1, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, w_in__, scale_in__, bias_in__, pertoken_scale_in__, group_list_in__, shared_input_in__, logit_in__, row_index_in__, y_out_, dtype, shared_input_weight, shared_input_offset, transpose_x, transpose_w, output_bs, group_list_type)
    result = check_op_cap("op_select_format", "GroupedMatmulFinalizeRouting", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, w_in__, scale_in__=None, bias_in__=None, pertoken_scale_in__=None, group_list_in__=None, shared_input_in__=None, logit_in__=None, row_index_in__=None, y_out_=None, dtype=0, shared_input_weight=1, shared_input_offset=0, transpose_x=False, transpose_w=False, output_bs=0, group_list_type=1, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, w_in__, scale_in__, bias_in__, pertoken_scale_in__, group_list_in__, shared_input_in__, logit_in__, row_index_in__, y_out_, dtype, shared_input_weight, shared_input_offset, transpose_x, transpose_w, output_bs, group_list_type)
    result = check_op_cap("get_op_specific_info", "GroupedMatmulFinalizeRouting", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
