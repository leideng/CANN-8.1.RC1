...
def _build_args(var_in__, indices_in__, updates_in__, var_out_, axis, reduction):
    __inputs__ = []
...
@tbe_register.register_operator("ScatterElementsV2", trans_bool_to_s8=False)
...
def scatter_elements_v2(var_in__, indices_in__, updates_in__, var_out_, axis=0, reduction="none", kernel_name="scatter_elements_v2", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(var_in__, indices_in__, updates_in__, var_out_, axis, reduction)
    ...
    origin_func_name = "scatter_elements_v2"
    ascendc_src_dir_ex = "scatter_elements_v2"
    ascendc_src_dir = "scatter_elements_v2"
    ascendc_src_file = "scatter_elements_v2.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator ScatterElementsV2, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "ScatterElementsV2"
...
def op_select_format(var_in__, indices_in__, updates_in__, var_out_, axis=0, reduction="none", impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(var_in__, indices_in__, updates_in__, var_out_, axis, reduction)
    result = check_op_cap("op_select_format", "ScatterElementsV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(var_in__, indices_in__, updates_in__, var_out_, axis=0, reduction="none", impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(var_in__, indices_in__, updates_in__, var_out_, axis, reduction)
    result = check_op_cap("get_op_specific_info", "ScatterElementsV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
