...
/*!
 * \file detect_flash_attention_pse.h
 * \brief
 */

#ifndef DETECT_FLASH_ATTENTION_PSE_H
#define DETECT_FLASH_ATTENTION_PSE_H
...
__aicore__ inline void PseSlopeCopyIn(LocalTensor<T> &dstTensor, LocalTensor<half> &helpTensor,
    ...
    if constexpr (hasPse == true) {
        ...
        DataCopyIn<half, hasPse>(helpTensor, alibiGm, 0, pseInfo.vec1S1RealSize,
                                 pseInfo.s2RealSize, pseInfo.pseAlibiBaseS2);
        event_t eventIdMte2ToV = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::MTE2_V));
        SetFlag<HardEvent::MTE2_V>(eventIdMte2ToV);
        WaitFlag<HardEvent::MTE2_V>(eventIdMte2ToV);
...
#endif  // DETECT_FLASH_ATTENTION_PSE_H
