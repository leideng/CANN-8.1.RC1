...
def _build_args(x_in__, y_out_, output_size):
    __inputs__ = []
    for arg in [x_in__]:
        if arg != None:
...
@tbe_register.register_operator("AdaptiveAvgPool3d", trans_bool_to_s8=False)
...
def adaptive_avg_pool3d(x_in__, y_out_, output_size, kernel_name="adaptive_avg_pool3d", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, output_size)
    ...
    origin_func_name = "adaptive_avg_pool3d"
    ascendc_src_dir_ex = "adaptive_avg_pool3d"
    ascendc_src_dir = "adaptive_avg_pool3d"
    ascendc_src_file = "adaptive_avg_pool3d.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator AdaptiveAvgPool3d, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "AdaptiveAvgPool3d"
...
def op_select_format(x_in__, y_out_, output_size, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, output_size)
    result = check_op_cap("op_select_format", "AdaptiveAvgPool3d", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, y_out_, output_size, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, output_size)
    result = check_op_cap("get_op_specific_info", "AdaptiveAvgPool3d", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
