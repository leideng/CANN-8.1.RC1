...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['grad', 'sort_indices', 'pos_idx']
    output_names = ['y']
...
def _build_args(grad_in__, sort_indices_in__, pos_idx_in__, y_out_, num_weights, padding_idx, scale_grad_by_freq):
    __inputs__ = []
    for arg in [grad_in__, sort_indices_in__, pos_idx_in__]:
        if arg != None:
...
@tbe_register.register_operator("EmbeddingDenseGradV2", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def embedding_dense_grad_v2(grad_in__, sort_indices_in__, pos_idx_in__, y_out_, num_weights, padding_idx=-1, scale_grad_by_freq=False, kernel_name="embedding_dense_grad_v2", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(grad_in__, sort_indices_in__, pos_idx_in__, y_out_, num_weights, padding_idx, scale_grad_by_freq)
    ...
    origin_func_name = "embedding_dense_grad_v2"
    ascendc_src_dir_ex = "embedding_dense_grad_v2"
    ascendc_src_dir = "embedding_dense_grad_v2"
    ascendc_src_file = "embedding_dense_grad_v2.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator EmbeddingDenseGradV2, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "EmbeddingDenseGradV2"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[grad_in__, sort_indices_in__, pos_idx_in__], origin_outputs = [y_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required'], init_value_list = ['0'],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(grad_in__, sort_indices_in__, pos_idx_in__, y_out_, num_weights, padding_idx=-1, scale_grad_by_freq=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_in__, sort_indices_in__, pos_idx_in__, y_out_, num_weights, padding_idx, scale_grad_by_freq)
    result = check_op_cap("op_select_format", "EmbeddingDenseGradV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(grad_in__, sort_indices_in__, pos_idx_in__, y_out_, num_weights, padding_idx=-1, scale_grad_by_freq=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_in__, sort_indices_in__, pos_idx_in__, y_out_, num_weights, padding_idx, scale_grad_by_freq)
    result = check_op_cap("get_op_specific_info", "EmbeddingDenseGradV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
