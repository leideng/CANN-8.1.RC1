...
def _build_args(x_in__, y_out_, ksize, strides, pads, ceil_mode, count_include_pad, divisor_override, data_format):
    __inputs__ = []
    for arg in [x_in__]:
        if arg != None:
...
@tbe_register.register_operator("AvgPool3D", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_STR, para_check.KERNEL_NAME)
def avg_pool3_d(x_in__, y_out_, ksize, strides, pads, ceil_mode=False, count_include_pad=True, divisor_override=0, data_format="NDHWC", kernel_name="avg_pool3_d", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, ksize, strides, pads, ceil_mode, count_include_pad, divisor_override, data_format)
    ...
    origin_func_name = "avg_pool3_d"
    ascendc_src_dir_ex = "avg_pool3_d"
    ascendc_src_dir = "avg_pool3_d"
    ascendc_src_file = "avg_pool3_d.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator AvgPool3D, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "AvgPool3D"
