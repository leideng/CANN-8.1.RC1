...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'expert_ids', 'scales', 'x_active_mask', 'expert_scales']
    output_names = ['expand_x', 'dynamic_scales', 'expand_idx', 'expert_token_nums', 'ep_recv_count', 'tp_recv_count', 'expand_scales']
...
def _build_args(x_in__, expert_ids_in__, scales_in__, x_active_mask_in__, expert_scales_in__, expand_x_out_, dynamic_scales_out_, expand_idx_out_, expert_token_nums_out_, ep_recv_count_out_, tp_recv_count_out_, expand_scales_out_, group_ep, ep_world_size, ep_rank_id, moe_expert_num, group_tp, tp_world_size, tp_rank_id, expert_shard_type, shared_expert_num, shared_expert_rank_num, quant_mode, global_bs, expert_token_nums_type):
    __inputs__ = []
    for arg in [x_in__, expert_ids_in__, scales_in__, x_active_mask_in__, expert_scales_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [expand_x_out_, dynamic_scales_out_, expand_idx_out_, expert_token_nums_out_, ep_recv_count_out_, tp_recv_count_out_, expand_scales_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if global_bs != None:
        ...
        __attrs__.append(attr)
    if expert_token_nums_type != None:
        attr = {}
        attr["name"] = "expert_token_nums_type"
        attr["dtype"] = "int"
        attr["value"] = expert_token_nums_type
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("MoeDistributeDispatch", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def moe_distribute_dispatch(x_in__, expert_ids_in__, scales_in__=None, x_active_mask_in__=None, expert_scales_in__=None, expand_x_out_=None, dynamic_scales_out_=None, expand_idx_out_=None, expert_token_nums_out_=None, ep_recv_count_out_=None, tp_recv_count_out_=None, expand_scales_out_=None, group_ep="", ep_world_size=0, ep_rank_id=0, moe_expert_num=0, group_tp="", tp_world_size=0, tp_rank_id=0, expert_shard_type=0, shared_expert_num=1, shared_expert_rank_num=0, quant_mode=0, global_bs=0, expert_token_nums_type=1, kernel_name="moe_distribute_dispatch", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, expert_ids_in__, scales_in__, x_active_mask_in__, expert_scales_in__, expand_x_out_, dynamic_scales_out_, expand_idx_out_, expert_token_nums_out_, ep_recv_count_out_, tp_recv_count_out_, expand_scales_out_, group_ep, ep_world_size, ep_rank_id, moe_expert_num, group_tp, tp_world_size, tp_rank_id, expert_shard_type, shared_expert_num, shared_expert_rank_num, quant_mode, global_bs, expert_token_nums_type)
    ...
    origin_func_name = "moe_distribute_dispatch"
    ascendc_src_dir_ex = "moe_distribute_dispatch"
    ascendc_src_dir = "moe_distribute_dispatch"
    ascendc_src_file = "moe_distribute_dispatch.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MoeDistributeDispatch, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, expert_ids_in__, scales_in__, x_active_mask_in__, expert_scales_in__], origin_outputs = [expand_x_out_, dynamic_scales_out_, expand_idx_out_, expert_token_nums_out_, ep_recv_count_out_, tp_recv_count_out_, expand_scales_out_],\
                param_type_dynamic = False, mc2_ctx = ["mc2_context_0", "mc2_context_1"], param_type_list = ['required', 'required', 'optional', 'optional', 'optional', 'required', 'required', 'required', 'required', 'required', 'required', 'required'], init_value_list = [None, None, None, None, None, None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{"auto_sync": false}')

def op_select_format(x_in__, expert_ids_in__, scales_in__=None, x_active_mask_in__=None, expert_scales_in__=None, expand_x_out_=None, dynamic_scales_out_=None, expand_idx_out_=None, expert_token_nums_out_=None, ep_recv_count_out_=None, tp_recv_count_out_=None, expand_scales_out_=None, group_ep="", ep_world_size=0, ep_rank_id=0, moe_expert_num=0, group_tp="", tp_world_size=0, tp_rank_id=0, expert_shard_type=0, shared_expert_num=1, shared_expert_rank_num=0, quant_mode=0, global_bs=0, expert_token_nums_type=1, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, expert_ids_in__, scales_in__, x_active_mask_in__, expert_scales_in__, expand_x_out_, dynamic_scales_out_, expand_idx_out_, expert_token_nums_out_, ep_recv_count_out_, tp_recv_count_out_, expand_scales_out_, group_ep, ep_world_size, ep_rank_id, moe_expert_num, group_tp, tp_world_size, tp_rank_id, expert_shard_type, shared_expert_num, shared_expert_rank_num, quant_mode, global_bs, expert_token_nums_type)
    result = check_op_cap("op_select_format", "MoeDistributeDispatch", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, expert_ids_in__, scales_in__=None, x_active_mask_in__=None, expert_scales_in__=None, expand_x_out_=None, dynamic_scales_out_=None, expand_idx_out_=None, expert_token_nums_out_=None, ep_recv_count_out_=None, tp_recv_count_out_=None, expand_scales_out_=None, group_ep="", ep_world_size=0, ep_rank_id=0, moe_expert_num=0, group_tp="", tp_world_size=0, tp_rank_id=0, expert_shard_type=0, shared_expert_num=1, shared_expert_rank_num=0, quant_mode=0, global_bs=0, expert_token_nums_type=1, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, expert_ids_in__, scales_in__, x_active_mask_in__, expert_scales_in__, expand_x_out_, dynamic_scales_out_, expand_idx_out_, expert_token_nums_out_, ep_recv_count_out_, tp_recv_count_out_, expand_scales_out_, group_ep, ep_world_size, ep_rank_id, moe_expert_num, group_tp, tp_world_size, tp_rank_id, expert_shard_type, shared_expert_num, shared_expert_rank_num, quant_mode, global_bs, expert_token_nums_type)
    result = check_op_cap("get_op_specific_info", "MoeDistributeDispatch", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
