...
def _build_args(x_in__, y_out_, indices_out_, output_size):
    __inputs__ = []
    for arg in [x_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [y_out_, indices_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("AdaptiveMaxPool3d", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_LIST_INT, para_check.KERNEL_NAME)
def adaptive_max_pool3d(x_in__, y_out_, indices_out_, output_size, kernel_name="adaptive_max_pool3d", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, indices_out_, output_size)
    ...
    origin_func_name = "adaptive_max_pool3d"
    ascendc_src_dir_ex = "adaptive_max_pool3d"
    ascendc_src_dir = "adaptive_max_pool3d"
    ascendc_src_file = "adaptive_max_pool3d.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator AdaptiveMaxPool3d, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "AdaptiveMaxPool3d"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__], origin_outputs = [y_out_, indices_out_],\
...
def op_select_format(x_in__, y_out_, indices_out_, output_size, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, indices_out_, output_size)
    result = check_op_cap("op_select_format", "AdaptiveMaxPool3d", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, y_out_, indices_out_, output_size, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, indices_out_, output_size)
    result = check_op_cap("get_op_specific_info", "AdaptiveMaxPool3d", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
