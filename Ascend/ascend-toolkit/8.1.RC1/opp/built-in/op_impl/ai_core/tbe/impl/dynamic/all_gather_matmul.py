...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x1', 'x2', 'bias']
    output_names = ['y', 'gather_out']
...
def _build_args(x1_in__, x2_in__, bias_in__, y_out_, gather_out_out_, group, is_trans_a, is_trans_b, gather_index, comm_turn, rank_size, is_gather_out):
    __inputs__ = []
    for arg in [x1_in__, x2_in__, bias_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [y_out_, gather_out_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if is_trans_b != None:
        ...
        __attrs__.append(attr)
    if gather_index != None:
        attr = {}
        attr["name"] = "gather_index"
        attr["dtype"] = "int"
        attr["value"] = gather_index
        __attrs__.append(attr)
    if comm_turn != None:
        attr = {}
        attr["name"] = "comm_turn"
    ...
    if rank_size != None:
        ...
        __attrs__.append(attr)
    if is_gather_out != None:
        attr = {}
        attr["name"] = "is_gather_out"
        attr["dtype"] = "bool"
        attr["value"] = is_gather_out
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("AllGatherMatmul", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def all_gather_matmul(x1_in__, x2_in__, bias_in__=None, y_out_=None, gather_out_out_=None, group="", is_trans_a=False, is_trans_b=False, gather_index=0, comm_turn=0, rank_size=0, is_gather_out=True, kernel_name="all_gather_matmul", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, bias_in__, y_out_, gather_out_out_, group, is_trans_a, is_trans_b, gather_index, comm_turn, rank_size, is_gather_out)
    ...
    origin_func_name = "all_gather_matmul"
    ascendc_src_dir_ex = "all_gather_matmul"
    ascendc_src_dir = "all_gather_matmul"
    ascendc_src_file = "all_gather_matmul.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator AllGatherMatmul, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "AllGatherMatmul"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x1_in__, x2_in__, bias_in__], origin_outputs = [y_out_, gather_out_out_],\
                param_type_dynamic = False, mc2_ctx = ["mc2_context_0"], param_type_list = ['required', 'required', 'optional', 'required', 'required'], init_value_list = [None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{"auto_sync": false}')

def op_select_format(x1_in__, x2_in__, bias_in__=None, y_out_=None, gather_out_out_=None, group="", is_trans_a=False, is_trans_b=False, gather_index=0, comm_turn=0, rank_size=0, is_gather_out=True, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, bias_in__, y_out_, gather_out_out_, group, is_trans_a, is_trans_b, gather_index, comm_turn, rank_size, is_gather_out)
    result = check_op_cap("op_select_format", "AllGatherMatmul", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x1_in__, x2_in__, bias_in__=None, y_out_=None, gather_out_out_=None, group="", is_trans_a=False, is_trans_b=False, gather_index=0, comm_turn=0, rank_size=0, is_gather_out=True, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, bias_in__, y_out_, gather_out_out_, group, is_trans_a, is_trans_b, gather_index, comm_turn, rank_size, is_gather_out)
    result = check_op_cap("get_op_specific_info", "AllGatherMatmul", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
