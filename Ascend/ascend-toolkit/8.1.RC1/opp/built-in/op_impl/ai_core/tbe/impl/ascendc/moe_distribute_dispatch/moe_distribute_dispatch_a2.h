...
constexpr static uint64_t MB_SIZE = 1024 * 1024;
...
private:
    ...
    TBuf<> tempBuf_;
    TBuf<> indexOutBuf_;
    TBuf<> expertIdsOutBuf_;
    TBuf<> sortedBuf_;
    TBuf<AscendC::TPosition::VECCALC> receiveDataCastFloatAbsBuf_;
...
__aicore__ inline void MoeDistributeDispatchA2<TemplateMC2TypeA2Func>::Init(
    GM_ADDR x, GM_ADDR expertIds, GM_ADDR scales, GM_ADDR expandXOut,
    GM_ADDR dynamicScalesOut, GM_ADDR expandIdxOut, GM_ADDR expertTokenNumsOut, GM_ADDR epRecvCountsOut,
    GM_ADDR workspaceGM, TPipe *pipe, GM_ADDR tilingGM)
{
    tpipe_ = pipe;
    ...
    sumTarget_ = worldSize_ - (float)0.0;
