...
def _build_args(cost_in__, p_out_, tol):
    __inputs__ = []
    for arg in [cost_in__]:
        if arg != None:
...
@tbe_register.register_operator("Sinkhorn", trans_bool_to_s8=False)
...
def sinkhorn(cost_in__, p_out_, tol=0.0001, kernel_name="sinkhorn", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(cost_in__, p_out_, tol)
    ...
    origin_func_name = "sinkhorn"
    ascendc_src_dir_ex = "sinkhorn"
    ascendc_src_dir = "sinkhorn"
    ascendc_src_file = "sinkhorn.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator Sinkhorn, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "Sinkhorn"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[cost_in__], origin_outputs = [p_out_],\
...
def op_select_format(cost_in__, p_out_, tol=0.0001, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(cost_in__, p_out_, tol)
    result = check_op_cap("op_select_format", "Sinkhorn", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(cost_in__, p_out_, tol=0.0001, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(cost_in__, p_out_, tol)
    result = check_op_cap("get_op_specific_info", "Sinkhorn", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
