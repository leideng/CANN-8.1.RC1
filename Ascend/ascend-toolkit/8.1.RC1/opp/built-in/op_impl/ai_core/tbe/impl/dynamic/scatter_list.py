...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['var', 'indice', 'updates', 'mask']
    output_names = ['var']
...
def _build_args(var_in__, indice_in__, updates_in__, mask_in__, var_out_, reduce, axis):
    __inputs__ = []
    for arg in [var_in__, indice_in__, updates_in__, mask_in__]:
        if arg != None:
...
@tbe_register.register_operator("ScatterList", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.DYNAMIC_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.DYNAMIC_OUTPUT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def scatter_list(var_in__, indice_in__, updates_in__, mask_in__=None, var_out_=None, reduce="update", axis=-2, kernel_name="scatter_list", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(var_in__, indice_in__, updates_in__, mask_in__, var_out_, reduce, axis)
    ...
    origin_func_name = "scatter_list"
    ascendc_src_dir_ex = "scatter_list"
    ascendc_src_dir = "scatter_list"
    ascendc_src_file = "scatter_list.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator ScatterList, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "ScatterList"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[var_in__, indice_in__, updates_in__, mask_in__], origin_outputs = [var_out_],\
                param_type_dynamic = True, mc2_ctx = [], param_type_list = ['dynamic', 'required', 'required', 'optional', 'dynamic'], init_value_list = [None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{"compile_options": {"ascend310p": ["-mllvm -cce-aicore-jump-expand=true"], "ascend910": ["-mllvm -cce-aicore-jump-expand=true"]}}')

def op_select_format(var_in__, indice_in__, updates_in__, mask_in__=None, var_out_=None, reduce="update", axis=-2, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(var_in__, indice_in__, updates_in__, mask_in__, var_out_, reduce, axis)
    result = check_op_cap("op_select_format", "ScatterList", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(var_in__, indice_in__, updates_in__, mask_in__=None, var_out_=None, reduce="update", axis=-2, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(var_in__, indice_in__, updates_in__, mask_in__, var_out_, reduce, axis)
    result = check_op_cap("get_op_specific_info", "ScatterList", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
