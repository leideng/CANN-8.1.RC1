...
def _build_args(x_in__, scalar_in__, y_out_):
    __inputs__ = []
    for arg in [x_in__, scalar_in__]:
        if arg != None:
...
@tbe_register.register_operator("ForeachMaximumScalar", trans_bool_to_s8=False)
...
def foreach_maximum_scalar(x_in__, scalar_in__, y_out_, kernel_name="foreach_maximum_scalar", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, scalar_in__, y_out_)
    ...
    origin_func_name = "foreach_maximum_scalar"
    ascendc_src_dir_ex = "foreach_maximum_scalar"
    ascendc_src_dir = "foreach_maximum_scalar"
    ascendc_src_file = "foreach_maximum_scalar.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator ForeachMaximumScalar, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "ForeachMaximumScalar"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, scalar_in__], origin_outputs = [y_out_],\
...
def op_select_format(x_in__, scalar_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, scalar_in__, y_out_)
    result = check_op_cap("op_select_format", "ForeachMaximumScalar", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, scalar_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, scalar_in__, y_out_)
    result = check_op_cap("get_op_specific_info", "ForeachMaximumScalar", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
