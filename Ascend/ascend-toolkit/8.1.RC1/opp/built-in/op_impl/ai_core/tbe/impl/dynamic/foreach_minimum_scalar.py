...
@tbe_register.register_operator("ForeachMinimumScalar", trans_bool_to_s8=False)
...
def foreach_minimum_scalar(x_in__, scalar_in__, y_out_, kernel_name="foreach_minimum_scalar", impl_mode=""):
    ...
    origin_func_name = "foreach_minimum_scalar"
    ascendc_src_dir_ex = "foreach_minimum_scalar"
    ascendc_src_dir = "foreach_minimum_scalar"
    ascendc_src_file = "foreach_minimum_scalar.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator ForeachMinimumScalar, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "ForeachMinimumScalar"
...
def op_select_format(x_in__, scalar_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, scalar_in__, y_out_)
    result = check_op_cap("op_select_format", "ForeachMinimumScalar", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, scalar_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, scalar_in__, y_out_)
    result = check_op_cap("get_op_specific_info", "ForeachMinimumScalar", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
