...
class MapIndexProcess:
    ...
    def __init__(self, input_data):
        ...
        self.x = self.tik_instance.Tensor("int32", x_shape, name="x", scope=tbe_platform.scope_gm)
        self.data_seq = self.tik_instance.Tensor("int32", data_seq_shape, name="data_seq",
                                                 scope=tbe_platform.scope_gm)
        if self.have_level_index:
            self.level_index = self.tik_instance.Tensor("int32", level_index_shape, name="level_index",
                                                        scope=tbe_platform.scope_gm)
            self.level_index_ub = self.tik_instance.Tensor("int32", (((level_index_shape[0] + 7) // 8) * 8,),
                                                           name="level_index", scope=tbe_platform.scope_ubuf)
            self.tik_instance.data_move(self.level_index_ub, self.level_index, 0, 1, (level_index_shape[0] + 7) // 8,
                                        0, 0, 0)

        self.y = self.tik_instance.Tensor("int32", (8,), name="y", scope=tbe_platform.scope_gm)

        x_dim_align = (x_shape[0] + 7) // 8 * 8
        self.x_ub = self.tik_instance.Tensor("int32", (x_dim_align,), name="x_ub", scope=tbe_platform.scope_ubuf)
        self.data_seq_ub = self.tik_instance.Tensor("int32", data_seq_shape, name="data_seq_ub",
                                                    scope=tbe_platform.scope_ubuf)
        self.y_ub = self.tik_instance.Tensor("int32", (8,), name="y_ub", scope=tbe_platform.scope_ubuf)
    ...
    def cce_map_index(self, kernel_name="map_index"):
        ...
        res = tik_instance.Tensor("uint16", (count,), name="res", scope=tbe_platform.scope_ubuf)
        map_index_vec_dup(tik_instance, 128, res, 0, count)

        with tik_instance.for_range(0, self.x_length) as i:
            # compute ===> tmp : vec_dup(x[i])
            tmp = tik_instance.Tensor("int32", (length,), name="tmp", scope=tbe_platform.scope_ubuf)
            ...
            sub_value = tik_instance.Tensor("int32", (max,), name="sub_value", scope=tbe_platform.scope_ubuf)
            ...
            sub_fp16 = tik_instance.Tensor("float16", (max,), name="sub_fp16", scope=tbe_platform.scope_ubuf)
            map_index_vec_dup(tik_instance, 128, sub_fp16, 1, max)
            if cce_product in ("Hi3796CV300CS", "SD3403"):
                sub_int16 = tik_instance.Tensor("int16", (max,), name="sub_int16", scope=tbe_platform.scope_ubuf)
            ...
            # sub_FP16 compv 0
            zero_ub = tik_instance.Tensor("float16", (max,), name="zero_ub_fp16", scope=tbe_platform.scope_ubuf)
            map_index_vec_dup(tik_instance, 128, zero_ub, 0, max)

            cmp_res = tik_instance.Tensor("uint16", (count,), name="cmp_res", scope=tbe_platform.scope_ubuf)
...
def map_index(x_dic, data_seq_dic, level_index_dic, y_dic, kernel_name="map_index"):
    ...
    if cce_product in ("Hi3796CV300ES", "Hi3796CV300CS", "SD3403"):
        if x_shape[0] > 128:
            raise RuntimeError("the length of x should be less than or equal to 128 on ihisi")
    elif cce_product in ("Ascend031",):
        if x_shape[0] > 48:
            raise RuntimeError("the length of x should be less than or equal to 48 on Ascend031")
    else:
        if x_shape[0] > 400:
            raise RuntimeError("the length of x should be less than or equal to 400")
