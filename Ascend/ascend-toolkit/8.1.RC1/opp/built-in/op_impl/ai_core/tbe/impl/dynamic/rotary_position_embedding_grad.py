...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['dy', 'cos', 'sin', 'x']
    output_names = ['dx', 'dcos', 'dsin']
...
def _build_args(dy_in__, cos_in__, sin_in__, x_in__, dx_out_, dcos_out_, dsin_out_, mode):
    __inputs__ = []
    for arg in [dy_in__, cos_in__, sin_in__, x_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [dx_out_, dcos_out_, dsin_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("RotaryPositionEmbeddingGrad", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def rotary_position_embedding_grad(dy_in__, cos_in__, sin_in__, x_in__=None, dx_out_=None, dcos_out_=None, dsin_out_=None, mode=0, kernel_name="rotary_position_embedding_grad", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(dy_in__, cos_in__, sin_in__, x_in__, dx_out_, dcos_out_, dsin_out_, mode)
    ...
    origin_func_name = "rotary_position_embedding_grad"
    ascendc_src_dir_ex = "rotary_position_embedding_grad"
    ascendc_src_dir = "rotary_position_embedding_grad"
    ascendc_src_file = "rotary_position_embedding_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator RotaryPositionEmbeddingGrad, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[dy_in__, cos_in__, sin_in__, x_in__], origin_outputs = [dx_out_, dcos_out_, dsin_out_],\
...
def op_select_format(dy_in__, cos_in__, sin_in__, x_in__=None, dx_out_=None, dcos_out_=None, dsin_out_=None, mode=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(dy_in__, cos_in__, sin_in__, x_in__, dx_out_, dcos_out_, dsin_out_, mode)
    result = check_op_cap("op_select_format", "RotaryPositionEmbeddingGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(dy_in__, cos_in__, sin_in__, x_in__=None, dx_out_=None, dcos_out_=None, dsin_out_=None, mode=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(dy_in__, cos_in__, sin_in__, x_in__, dx_out_, dcos_out_, dsin_out_, mode)
    result = check_op_cap("get_op_specific_info", "RotaryPositionEmbeddingGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
