...
def _build_args(x1_in__, x2_in__, bias_in__, y_out_, group, reduce_op, is_trans_a, is_trans_b, comm_turn, rank_size):
    __inputs__ = []
...
@tbe_register.register_operator("MatmulReduceScatter", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def matmul_reduce_scatter(x1_in__, x2_in__, bias_in__=None, y_out_=None, group="", reduce_op="sum", is_trans_a=False, is_trans_b=False, comm_turn=0, rank_size=0, kernel_name="matmul_reduce_scatter", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, bias_in__, y_out_, group, reduce_op, is_trans_a, is_trans_b, comm_turn, rank_size)
    ...
    origin_func_name = "matmul_reduce_scatter"
    ascendc_src_dir_ex = "matmul_reduce_scatter"
    ascendc_src_dir = "matmul_reduce_scatter"
    ascendc_src_file = "matmul_reduce_scatter.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MatmulReduceScatter, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "MatmulReduceScatter"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x1_in__, x2_in__, bias_in__], origin_outputs = [y_out_],\
                param_type_dynamic = False, mc2_ctx = ["mc2_context_0"], param_type_list = ['required', 'required', 'optional', 'required'], init_value_list = [None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{"auto_sync": false}')

def op_select_format(x1_in__, x2_in__, bias_in__=None, y_out_=None, group="", reduce_op="sum", is_trans_a=False, is_trans_b=False, comm_turn=0, rank_size=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, bias_in__, y_out_, group, reduce_op, is_trans_a, is_trans_b, comm_turn, rank_size)
    result = check_op_cap("op_select_format", "MatmulReduceScatter", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x1_in__, x2_in__, bias_in__=None, y_out_=None, group="", reduce_op="sum", is_trans_a=False, is_trans_b=False, comm_turn=0, rank_size=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, bias_in__, y_out_, group, reduce_op, is_trans_a, is_trans_b, comm_turn, rank_size)
    result = check_op_cap("get_op_specific_info", "MatmulReduceScatter", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
