...
from .unsorted_segment_min import op_select_format as unsorted_segment_max_op_select_format
from .unsorted_segment_min import check_supported as unsorted_segment_max_check_supported
...
def check_supported(x,
    ...
    return unsorted_segment_max_check_supported(x, segment_ids, num_segments, y, kernel_name, impl_mode)


def op_select_format(x, segment_ids, num_segments, y,
                     kernel_name="unsorted_segment_prod",
                     impl_mode=OpImplMode.HIGH_PRECISION):
    return unsorted_segment_max_op_select_format(x, segment_ids, num_segments, y, kernel_name, impl_mode)


def unsorted_segment_max_compute(x, segment_ids, var_num_segments, y,
                                 kernel_name="segmentensor_max", impl_mode=OpImplMode.HIGH_PRECISION):
    x_dtype = x.dtype
    if x_dtype == "float16":
        init_data = -65504
    ...
    if impl_mode == OpImplMode.SUPPORT_OUT_OF_BOUND_INDEX:
        check_ids = True
    res = tbe.segment(x, segment_ids, var_num_segments, init_data, "segmentensor_max", check_ids)
    return res
...
def unsorted_segment_max(x,
    ...
    for (input1, input2, input3) in ins:
        with tbe.compute():
            ...
            res = unsorted_segment_max_compute(x_tensor, ids_tensor, var_segments, y_dict, kernel_name, impl_mode)
