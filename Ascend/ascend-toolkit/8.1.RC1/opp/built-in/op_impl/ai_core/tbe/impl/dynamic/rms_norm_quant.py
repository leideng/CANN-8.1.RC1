...
def _build_args(x_in__, gamma_in__, beta_in__, y_out_, epsilon, gemma_mode, high_precision_mode, scale, offset):
    __inputs__ = []
...
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def rms_norm_quant(x_in__, gamma_in__, beta_in__=None, y_out_=None, epsilon=0.0, gemma_mode=False, high_precision_mode=False, scale=1, offset=0, kernel_name="rms_norm_quant", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, gamma_in__, beta_in__, y_out_, epsilon, gemma_mode, high_precision_mode, scale, offset)
    ...
    msg = "start compile Acend C Operator RmsNormQuant, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, gamma_in__, beta_in__], origin_outputs = [y_out_],\
...
def op_select_format(x_in__, gamma_in__, beta_in__=None, y_out_=None, epsilon=0.0, gemma_mode=False, high_precision_mode=False, scale=1, offset=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, gamma_in__, beta_in__, y_out_, epsilon, gemma_mode, high_precision_mode, scale, offset)
    result = check_op_cap("op_select_format", "RmsNormQuant", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, gamma_in__, beta_in__=None, y_out_=None, epsilon=0.0, gemma_mode=False, high_precision_mode=False, scale=1, offset=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, gamma_in__, beta_in__, y_out_, epsilon, gemma_mode, high_precision_mode, scale, offset)
