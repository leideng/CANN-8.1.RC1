...
def _get_pd_var(data, params, shape_x, pd_xl, cast_dtype):
    ...
    pd_var_1 = tbe.cast_to(pd_var_1, pdvar_sum.dtype)
...
def _get_pd_mean(params, pd_xl, pd_var, var_elta_2, sub_x_mean, cast_dtype):
    ...
    var_elta_2 = tbe.cast_to(var_elta_2, pdmean1_sum.dtype)
    ...
    pdmean2_sum = tbe.sum(pdmean2_mul1, params.get("reduce_axis"), keepdims=True)
    pdmean2_mul3 = tbe.vmuls(pdmean2_sum, tvm.const((params.get("mean_num") ** (-1)), dtype=cast_dtype))
...
def _get_pd_x_front(data, params, shape_x, cast_dtype):
    ...
    var_elta_2_cast = tbe.cast_to(var_elta_2_cast, pd_xl.dtype)
...
@register_operator_compute("layer_norm_x_backprop", op_mode="static", support_fusion=True)
...
def _get_pd_var_nz(data, param_nz, pd_xl, cast_dtype):
    ...
    pdvar_mul1 = tbe.vmul(pd_xl, sub_x_mean)
    pdvar_sum = tbe.sum(pdvar_mul1, param_nz.get("reduce_nz_axis"), keepdims=True)
    pdvar_mul3 = tbe.vmul(pdvar_sum, pd_var_1)
    pd_var = tbe.vmuls(pdvar_mul3, tvm.const(-0.5, dtype=cast_dtype))
...
def _get_pd_mean_nz(param_nz, pd_xl, pd_var, var_elta_2, sub_x_mean,
    ...
    """
    pdmean1_sum = tbe.sum(pd_xl, param_nz.get("reduce_nz_axis"), keepdims=True)
    pdmean1_mul = tbe.vmul(pdmean1_sum, var_elta_2)
...
def _get_pd_x_front_nz(data, param_nz, cast_dtype):
    ...
    pdx2_mul = tbe.vmul(pdx2_broad, sub_x_mean)
    pd_x_2 = tbe.vmuls(pdx2_mul, tvm.const((2 * (param_nz.get("mean_nz_num") ** (-1))), dtype=cast_dtype))
...
def layer_norm_x_back_nz_compute(data_dy, data_x, data_variance,
    ...
    res_tuple: tuple
        (pd_x, pd_gamma, pd_beta)
    """
    pd_x = _get_pds_nz(data_dy, data_x, data_variance, data_mean, data_gamma, param_nz)
...
def layer_norm_x_backprop(input_dy, input_x, input_variance, input_mean,
    ...
    if format_dy.upper() == "FRACTAL_NZ":
        ...
        pd_x = layer_norm_x_back_nz_compute(data_dy, data_x, data_variance, data_mean, data_gamma, param_nz)

        tensor_list = [data_dy, data_x, data_variance, data_mean, data_gamma, pd_x]

        with tvm.target.cce():
            sch = auto_schedule(pd_x)
