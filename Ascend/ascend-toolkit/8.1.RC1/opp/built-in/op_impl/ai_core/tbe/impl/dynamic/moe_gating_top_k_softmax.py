...
def _build_args(x_in__, finished_in__, y_out_, expert_idx_out_, row_idx_out_, k):
    __inputs__ = []
    for arg in [x_in__, finished_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [y_out_, expert_idx_out_, row_idx_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("MoeGatingTopKSoftmax", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def moe_gating_top_k_softmax(x_in__, finished_in__=None, y_out_=None, expert_idx_out_=None, row_idx_out_=None, k=0, kernel_name="moe_gating_top_k_softmax", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, finished_in__, y_out_, expert_idx_out_, row_idx_out_, k)
    ...
    origin_func_name = "moe_gating_top_k_softmax"
    ascendc_src_dir_ex = "moe_gating_top_k_softmax"
    ascendc_src_dir = "moe_gating_top_k_softmax"
    ascendc_src_file = "moe_gating_top_k_softmax.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MoeGatingTopKSoftmax, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "MoeGatingTopKSoftmax"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, finished_in__], origin_outputs = [y_out_, expert_idx_out_, row_idx_out_],\
...
def op_select_format(x_in__, finished_in__=None, y_out_=None, expert_idx_out_=None, row_idx_out_=None, k=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, finished_in__, y_out_, expert_idx_out_, row_idx_out_, k)
    result = check_op_cap("op_select_format", "MoeGatingTopKSoftmax", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, finished_in__=None, y_out_=None, expert_idx_out_=None, row_idx_out_=None, k=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, finished_in__, y_out_, expert_idx_out_, row_idx_out_, k)
    result = check_op_cap("get_op_specific_info", "MoeGatingTopKSoftmax", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
