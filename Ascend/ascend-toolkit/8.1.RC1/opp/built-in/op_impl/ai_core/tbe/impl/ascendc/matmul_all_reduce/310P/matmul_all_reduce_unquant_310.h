...
public:
    ...
    __aicore__ inline void Init(GM_ADDR aGM, GM_ADDR bGM, GM_ADDR biasGM, GM_ADDR cGM,
                                GM_ADDR workspaceGM, UnQuantMatmulAllReduceTilingData* tilingData,
    ...
    if (tilingData_->param.isTransposeB == 0) {
        ...
        if (TILING_KEY_IS(10000000000000002000UL)) {
            MatmulBaseUnAlignedKernel<
                aMatmulType, bMatmulType, cMatmulType, biasMatmulType, MatmulBaseBlock, MM_CFG_VEC_ND2NZ> mmop;
        ...
        if (TILING_KEY_IS(10000000000000002000UL)) {
            MatmulBaseUnAlignedKernel<aMatmulType, bMatmulType, cMatmulType, biasMatmulType,
                MatmulBaseBlock, MM_CFG_VEC_ND2NZ> mmop;
            MATMUL_ALL_REDUCE_TEMPLATE(mmop, tileCnt, tPipe_, aGM_, bGM_, cGM_, biasGM_, workspaceGM_, mm_tiling,
                hcclServer_, shift, aOffset, cOffset);
        } else if (TILING_KEY_IS(10000000000000002001UL)) {
            MatmulBaseKernel<
                aMatmulType, bMatmulType, cMatmulType, biasMatmulType, MatmulBaseBlock, MM_CFG_VEC_ND2NZ> mmop;
            MATMUL_ALL_REDUCE_TEMPLATE(mmop, tileCnt, tPipe_, aGM_, bGM_, cGM_, biasGM_,
                workspaceGM_, mm_tiling, hcclServer_, shift, aOffset, cOffset);
        }
    }
}

__aicore__ inline void MatMulEmptyTensorKernelUnquantNzBias(GM_ADDR biasGM, GM_ADDR cGM,
    UnQuantMatmulAllReduceTilingData *tilingData, TBuf<TPosition::VECCALC> &tmpBuf)
{
    // 搬运biase对齐部分
...
__aicore__ inline void MatMulEmptyTensorKernelUnquantNz(GM_ADDR biasGM, GM_ADDR cGM, GM_ADDR workspaceGM,
    UnQuantMatmulAllReduceTilingData *tilingData, HcclServer *hcclServer)
{
