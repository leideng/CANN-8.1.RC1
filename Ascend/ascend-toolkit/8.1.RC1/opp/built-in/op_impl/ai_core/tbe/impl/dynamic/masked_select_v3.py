...
def _build_args(x_in__, mask_in__, y_out_):
    __inputs__ = []
...
@tbe_register.register_operator("MaskedSelectV3", trans_bool_to_s8=False)
...
def masked_select_v3(x_in__, mask_in__, y_out_, kernel_name="masked_select_v3", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, mask_in__, y_out_)
    ...
    origin_func_name = "masked_select_v3"
    ascendc_src_dir_ex = "masked_select_v3"
    ascendc_src_dir = "masked_select_v3"
    ascendc_src_file = "masked_select_v3.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MaskedSelectV3, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "MaskedSelectV3"
...
def op_select_format(x_in__, mask_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, mask_in__, y_out_)
    result = check_op_cap("op_select_format", "MaskedSelectV3", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, mask_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, mask_in__, y_out_)
    result = check_op_cap("get_op_specific_info", "MaskedSelectV3", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
