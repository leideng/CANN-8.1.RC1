...
def _build_args(x1_in__, x2_in__, gamma_in__, beta_in__, bias_in__, scales1_in__, scales2_in__, zero_points1_in__, zero_points2_in__, y1_out_, y2_out_, x_out_, dtype, axis, epsilon, additional_output):
    __inputs__ = []
    for arg in [x1_in__, x2_in__, gamma_in__, beta_in__, bias_in__, scales1_in__, scales2_in__, zero_points1_in__, zero_points2_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [y1_out_, y2_out_, x_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("DuaQuantizeAddLayerNorm", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def dua_quantize_add_layer_norm(x1_in__, x2_in__, gamma_in__, beta_in__, bias_in__, scales1_in__, scales2_in__, zero_points1_in__=None, zero_points2_in__=None, y1_out_=None, y2_out_=None, x_out_=None, dtype=0, axis=-1, epsilon=1e-05, additional_output=False, kernel_name="dua_quantize_add_layer_norm", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, gamma_in__, beta_in__, bias_in__, scales1_in__, scales2_in__, zero_points1_in__, zero_points2_in__, y1_out_, y2_out_, x_out_, dtype, axis, epsilon, additional_output)
    ...
    origin_func_name = "dua_quantize_add_layer_norm"
    ascendc_src_dir_ex = "dua_quantize_add_layer_norm"
    ascendc_src_dir = "dua_quantize_add_layer_norm"
    ascendc_src_file = "dua_quantize_add_layer_norm.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator DuaQuantizeAddLayerNorm, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "DuaQuantizeAddLayerNorm"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x1_in__, x2_in__, gamma_in__, beta_in__, bias_in__, scales1_in__, scales2_in__, zero_points1_in__, zero_points2_in__], origin_outputs = [y1_out_, y2_out_, x_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required', 'required', 'required', 'optional', 'optional', 'required', 'required', 'required'], init_value_list = [None, None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{"auto_sync": false}')

def op_select_format(x1_in__, x2_in__, gamma_in__, beta_in__, bias_in__, scales1_in__, scales2_in__, zero_points1_in__=None, zero_points2_in__=None, y1_out_=None, y2_out_=None, x_out_=None, dtype=0, axis=-1, epsilon=1e-05, additional_output=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, gamma_in__, beta_in__, bias_in__, scales1_in__, scales2_in__, zero_points1_in__, zero_points2_in__, y1_out_, y2_out_, x_out_, dtype, axis, epsilon, additional_output)
    result = check_op_cap("op_select_format", "DuaQuantizeAddLayerNorm", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x1_in__, x2_in__, gamma_in__, beta_in__, bias_in__, scales1_in__, scales2_in__, zero_points1_in__=None, zero_points2_in__=None, y1_out_=None, y2_out_=None, x_out_=None, dtype=0, axis=-1, epsilon=1e-05, additional_output=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, gamma_in__, beta_in__, bias_in__, scales1_in__, scales2_in__, zero_points1_in__, zero_points2_in__, y1_out_, y2_out_, x_out_, dtype, axis, epsilon, additional_output)
    result = check_op_cap("get_op_specific_info", "DuaQuantizeAddLayerNorm", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
