...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'scale', 'group_index', 'offset']
    output_names = ['y']
...
def _build_args(x_in__, scale_in__, group_index_in__, offset_in__, y_out_, dst_type):
    __inputs__ = []
    for arg in [x_in__, scale_in__, group_index_in__, offset_in__]:
        if arg != None:
...
@tbe_register.register_operator("GroupQuant", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def group_quant(x_in__, scale_in__, group_index_in__, offset_in__=None, y_out_=None, dst_type=2, kernel_name="group_quant", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, scale_in__, group_index_in__, offset_in__, y_out_, dst_type)
    ...
    origin_func_name = "group_quant"
    ascendc_src_dir_ex = "group_quant"
    ascendc_src_dir = "group_quant"
    ascendc_src_file = "group_quant.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator GroupQuant, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "GroupQuant"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, scale_in__, group_index_in__, offset_in__], origin_outputs = [y_out_],\
