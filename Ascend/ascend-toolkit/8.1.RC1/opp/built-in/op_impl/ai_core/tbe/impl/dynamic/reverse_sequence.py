...
def _build_args(x_in__, seq_lengths_in__, y_out_, seq_dim, batch_dim):
    __inputs__ = []
    for arg in [x_in__, seq_lengths_in__]:
        if arg != None:
    ...
    if batch_dim != None:
        ...
        attr["name"] = "batch_dim"
        attr["dtype"] = "int"
        attr["value"] = batch_dim
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("ReverseSequence", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def reverse_sequence(x_in__, seq_lengths_in__, y_out_, seq_dim, batch_dim=0, kernel_name="reverse_sequence", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, seq_lengths_in__, y_out_, seq_dim, batch_dim)
    ...
    origin_func_name = "reverse_sequence"
    ascendc_src_dir_ex = "reverse_sequence"
    ascendc_src_dir = "reverse_sequence"
    ascendc_src_file = "reverse_sequence.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator ReverseSequence, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, seq_lengths_in__], origin_outputs = [y_out_],\
...
def check_supported(x_in__, seq_lengths_in__, y_out_, seq_dim, batch_dim=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, seq_lengths_in__, y_out_, seq_dim, batch_dim)
    ret_str = check_op_cap("check_supported", "ReverseSequence", __inputs__, __outputs__, __attrs__)
...
def get_op_support_info(x_in__, seq_lengths_in__, y_out_, seq_dim, batch_dim=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, seq_lengths_in__, y_out_, seq_dim, batch_dim)
    result = check_op_cap("get_op_support_info", "ReverseSequence", __inputs__, __outputs__, __attrs__)
