...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['dy', 'x1', 'x2', 'rstd', 'mean', 'gamma', 'dsum']
...
def _build_args(dy_in__, x1_in__, x2_in__, rstd_in__, mean_in__, gamma_in__, dsum_in__, dx_out_, dgamma_out_, dbeta_out_):
    __inputs__ = []
    for arg in [dy_in__, x1_in__, x2_in__, rstd_in__, mean_in__, gamma_in__, dsum_in__]:
        if arg != None:
...
@tbe_register.register_operator("AddLayerNormGrad", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.KERNEL_NAME)
def add_layer_norm_grad(dy_in__, x1_in__, x2_in__, rstd_in__, mean_in__, gamma_in__, dsum_in__=None, dx_out_=None, dgamma_out_=None, dbeta_out_=None, kernel_name="add_layer_norm_grad", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(dy_in__, x1_in__, x2_in__, rstd_in__, mean_in__, gamma_in__, dsum_in__, dx_out_, dgamma_out_, dbeta_out_)
    ...
    origin_func_name = "add_layer_norm_grad"
    ascendc_src_dir_ex = "add_layer_norm_grad"
    ascendc_src_dir = "add_layer_norm_grad"
    ascendc_src_file = "add_layer_norm_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator AddLayerNormGrad, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "AddLayerNormGrad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[dy_in__, x1_in__, x2_in__, rstd_in__, mean_in__, gamma_in__, dsum_in__], origin_outputs = [dx_out_, dgamma_out_, dbeta_out_],\
...
def op_select_format(dy_in__, x1_in__, x2_in__, rstd_in__, mean_in__, gamma_in__, dsum_in__=None, dx_out_=None, dgamma_out_=None, dbeta_out_=None, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(dy_in__, x1_in__, x2_in__, rstd_in__, mean_in__, gamma_in__, dsum_in__, dx_out_, dgamma_out_, dbeta_out_)
    result = check_op_cap("op_select_format", "AddLayerNormGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(dy_in__, x1_in__, x2_in__, rstd_in__, mean_in__, gamma_in__, dsum_in__=None, dx_out_=None, dgamma_out_=None, dbeta_out_=None, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(dy_in__, x1_in__, x2_in__, rstd_in__, mean_in__, gamma_in__, dsum_in__, dx_out_, dgamma_out_, dbeta_out_)
    result = check_op_cap("get_op_specific_info", "AddLayerNormGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
