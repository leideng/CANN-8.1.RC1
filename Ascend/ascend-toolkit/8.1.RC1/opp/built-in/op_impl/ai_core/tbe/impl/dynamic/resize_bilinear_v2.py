...
class ResizeBilinearV2(OpBase):
    ...
    def _function_resize_with_l1_default(self,
        ...
        with self.tik_instance.new_stmt_scope():
            ...
            with self.tik_instance.for_range(0, self.width_idx_sigment_num - 1) as ub_idx:
                self.tik_instance.vadds(8, self.idx_ub_fp32[(ub_idx + 1) * 8],
                                        self.idx_ub_fp32[ub_idx * 8],
                                        1.0, 1, 1,
                                        1, 8, 8)
        ...
        self.tik_instance.vadds(64, self.idx_ub_fp32,
                                self.idx_ub_fp32,
                                scalar_idx_fp32,
        ...
        def _run_w_loop_default(w_loop_idx,
                                w_do_len,
                                h_loop_offset,
                                h_do_len):
            ...
            self.tik_instance.vadds(64,
                                    ...
                                    self.idx_ub_fp32,
                                    scalar_idx_fp32,
                                    (self.width_idx_sigment_num * 8 + 63) // 64,
                                    1, 1, 8, 8)
            ...
            def _do_single_nc(do_nc_num, _nc_loop_idx):
                ...
                def _do_one_height(h_idx, input_l1, input_ub, inner_ub):
                    ...
                    self.tik_instance.vadds(64, height_idx_ub_fp32,
                                            height_idx_ub_fp32,
                                            1.0, 1, 1, 1, 8, 8)
                    self.calcu_out_in_idx(self.resize_scale_h,
                                          height_idx_ub,
                ...
                # to fix precision issue caused by dirty data in ub
                util_tik_comm_func.tik_func_vector(self.tik_instance, image_out_ub_ping, 0, ub_max_num)
                util_tik_comm_func.tik_func_vector(self.tik_instance, image_out_ub_pang, 0, ub_max_num)
                                ...
                                with self.tik_instance.if_scope(self.scalar_is_dst_stride != 0):
                                    ...
                                    with self.tik_instance.if_scope(w_copy_tail != 0):
                                        ...
                                        data_move_dst_stride = \
                                            ...
                                            * self.images_shape_c0 // self.output_block_num
                                        data_move_src_stride = \
                                            (w_algin_num - w_copy_tail) * self.images_shape_c0 // self.output_block_num
                                        self.tik_instance.data_move(self.out_gm[new_output_gm_offset:],
                                                                    copy_output_ub[data_move_ub_offset:], 0,
                                                                    data_move_burst_num, data_move_burst_len,
                                                                    data_move_src_stride, data_move_dst_stride)
                                with self.tik_instance.if_scope(self.scalar_is_dst_stride == 0):
                                    data_move_burst_num = 1
                                    with self.tik_instance.for_range(0, w_copy_num) as w_copy_idx:
    ...
    def _function_default_apply_ub_vbi(self, vbi_dtype="float32"):
        ...
        mem_default["src0_offset"] = self.tik_instance.Tensor("int32", (self.width_idx_sigment_num * 4,),
                                                              name="src0_offset_ub",
                                                              scope=tik.scope_ubuf)
        src1 = self.tik_instance.Tensor("float32", (self.width_idx_sigment_num * 4,),
                                                       name="src1_ub",
                                                       scope=tik.scope_ubuf)
        self.tik_instance.vector_dup(64, src1, 0, 8, 1, 8)
    ...
    def _function_default_vbi_fp32(self,
        ...
        def _run_w_loop_default(w_loop_idx, w_do_len, h_loop_offset, h_do_len):
            ...
            # to avoid over ub bound, the unit is Byte
            max_vbi_offset = (self.width_idx_sigment_num * self.images_shape_c0 * 4 - 8) * 4
            util_tik_comm_func.tik_func_vector(self.tik_instance, tmp_ub, max_vbi_offset, 8)
            with self.tik_instance.if_scope(src0_offset[self.width_idx_sigment_num * 4 - 1] > max_vbi_offset):
                util_tik_comm_func.tik_func_vcomple(self.tik_instance,
                                                    "vmin",
                                                    src0_offset,
                                                    src0_offset,
