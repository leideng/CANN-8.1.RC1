...
def _build_args(x_in__, grid_in__, y_out_, interpolation_mode, padding_mode, align_corners, channel_last, scheduler_mode):
    __inputs__ = []
    for arg in [x_in__, grid_in__]:
        if arg != None:
    ...
    if align_corners != None:
        ...
        __attrs__.append(attr)
    if channel_last != None:
        attr = {}
        attr["name"] = "channel_last"
        attr["dtype"] = "bool"
        attr["value"] = channel_last
        __attrs__.append(attr)
    if scheduler_mode != None:
        attr = {}
        attr["name"] = "scheduler_mode"
        attr["dtype"] = "int"
        attr["value"] = scheduler_mode
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("GridSample", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def grid_sample(x_in__, grid_in__, y_out_, interpolation_mode="bilinear", padding_mode="zeros", align_corners=False, channel_last=False, scheduler_mode=1, kernel_name="grid_sample", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, grid_in__, y_out_, interpolation_mode, padding_mode, align_corners, channel_last, scheduler_mode)
    ...
    origin_func_name = "grid_sample"
    ascendc_src_dir_ex = "grid_sample"
    ascendc_src_dir = "grid_sample"
    ascendc_src_file = "grid_sample.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator GridSample, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "GridSample"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, grid_in__], origin_outputs = [y_out_],\
