...
def strided_slice_last_dim_mte(input_shape, dtype, output_shape,
    ...
    output_dim_size_except_last = funct_reduce(lambda x, y: x * y, output_shape[:-1])
    input_dim_size_except_last = funct_reduce(lambda x, y: x * y, input_shape[:-1])
    ...
    output_last_dim_part_size_1 = output_last_dim_part_block_1 * block_data_cnt
    allowed_repeat_cnt = input_ub_size // (output_last_dim_part_size_1 * block_data_cnt)
    ...
    if input_ub_size > ub_max_size // byte_cnt:
        input_ub_size = ub_max_size // byte_cnt // block_data_cnt * block_data_cnt

    if input_ub_size // (output_last_dim_part_size_1 * block_data_cnt) == 0:
        return False
    ...
    with tik_instance.for_range(0, core_num,
        ...
        def _inner_do_strides(repeat_num, in_offset, out_offset):
            ...
            output_last_dim_part_size_2 = output_last_dim_part_block_2 * block_data_cnt
            max_repeat = 255
