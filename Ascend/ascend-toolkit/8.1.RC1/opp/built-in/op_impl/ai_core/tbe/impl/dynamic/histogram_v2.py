...
def _build_args(x_in__, min_in__, max_in__, y_out_, bins):
    __inputs__ = []
    for arg in [x_in__, min_in__, max_in__]:
        if arg != None:
...
@tbe_register.register_operator("HistogramV2", trans_bool_to_s8=False)
...
def histogram_v2(x_in__, min_in__, max_in__, y_out_, bins=100, kernel_name="histogram_v2", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, min_in__, max_in__, y_out_, bins)
    ...
    origin_func_name = "histogram_v2"
    ascendc_src_dir_ex = "histogram_v2"
    ascendc_src_dir = "histogram_v2"
    ascendc_src_file = "histogram_v2.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator HistogramV2, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "HistogramV2"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, min_in__, max_in__], origin_outputs = [y_out_],\
...
def op_select_format(x_in__, min_in__, max_in__, y_out_, bins=100, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, min_in__, max_in__, y_out_, bins)
    result = check_op_cap("op_select_format", "HistogramV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, min_in__, max_in__, y_out_, bins=100, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, min_in__, max_in__, y_out_, bins)
    result = check_op_cap("get_op_specific_info", "HistogramV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
