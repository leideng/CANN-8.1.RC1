...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x1', 'x2', 'bias', 'residual', 'gamma', 'antiquant_scale', 'antiquant_offset', 'dequant_scale']
    output_names = ['residual', 'norm_out']
...
def _build_args(x1_in__, x2_in__, bias_in__, residual_in__, gamma_in__, antiquant_scale_in__, antiquant_offset_in__, dequant_scale_in__, residual_out_, norm_out_out_, group, reduce_op, is_trans_a, is_trans_b, comm_turn, antiquant_group_size, epsilon):
    __inputs__ = []
    for arg in [x1_in__, x2_in__, bias_in__, residual_in__, gamma_in__, antiquant_scale_in__, antiquant_offset_in__, dequant_scale_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [residual_out_, norm_out_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("InplaceMatmulAllReduceAddRmsNorm", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def inplace_matmul_all_reduce_add_rms_norm(x1_in__, x2_in__, bias_in__=None, residual_in__=None, gamma_in__=None, antiquant_scale_in__=None, antiquant_offset_in__=None, dequant_scale_in__=None, residual_out_=None, norm_out_out_=None, group="", reduce_op="sum", is_trans_a=False, is_trans_b=False, comm_turn=0, antiquant_group_size=0, epsilon=1e-06, kernel_name="inplace_matmul_all_reduce_add_rms_norm", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, bias_in__, residual_in__, gamma_in__, antiquant_scale_in__, antiquant_offset_in__, dequant_scale_in__, residual_out_, norm_out_out_, group, reduce_op, is_trans_a, is_trans_b, comm_turn, antiquant_group_size, epsilon)
    ...
    origin_func_name = "inplace_matmul_all_reduce_add_rms_norm"
    ascendc_src_dir_ex = "inplace_matmul_all_reduce_add_rms_norm"
    ascendc_src_dir = "inplace_matmul_all_reduce_add_rms_norm"
    ascendc_src_file = "inplace_matmul_all_reduce_add_rms_norm.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator InplaceMatmulAllReduceAddRmsNorm, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "InplaceMatmulAllReduceAddRmsNorm"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x1_in__, x2_in__, bias_in__, residual_in__, gamma_in__, antiquant_scale_in__, antiquant_offset_in__, dequant_scale_in__], origin_outputs = [residual_out_, norm_out_out_],\
                param_type_dynamic = False, mc2_ctx = ["mc2_context_0"], param_type_list = ['required', 'required', 'optional', 'required', 'required', 'optional', 'optional', 'optional', 'required', 'required'], init_value_list = [None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{"auto_sync": false}')

def op_select_format(x1_in__, x2_in__, bias_in__=None, residual_in__=None, gamma_in__=None, antiquant_scale_in__=None, antiquant_offset_in__=None, dequant_scale_in__=None, residual_out_=None, norm_out_out_=None, group="", reduce_op="sum", is_trans_a=False, is_trans_b=False, comm_turn=0, antiquant_group_size=0, epsilon=1e-06, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, bias_in__, residual_in__, gamma_in__, antiquant_scale_in__, antiquant_offset_in__, dequant_scale_in__, residual_out_, norm_out_out_, group, reduce_op, is_trans_a, is_trans_b, comm_turn, antiquant_group_size, epsilon)
    result = check_op_cap("op_select_format", "InplaceMatmulAllReduceAddRmsNorm", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x1_in__, x2_in__, bias_in__=None, residual_in__=None, gamma_in__=None, antiquant_scale_in__=None, antiquant_offset_in__=None, dequant_scale_in__=None, residual_out_=None, norm_out_out_=None, group="", reduce_op="sum", is_trans_a=False, is_trans_b=False, comm_turn=0, antiquant_group_size=0, epsilon=1e-06, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, bias_in__, residual_in__, gamma_in__, antiquant_scale_in__, antiquant_offset_in__, dequant_scale_in__, residual_out_, norm_out_out_, group, reduce_op, is_trans_a, is_trans_b, comm_turn, antiquant_group_size, epsilon)
    result = check_op_cap("get_op_specific_info", "InplaceMatmulAllReduceAddRmsNorm", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
