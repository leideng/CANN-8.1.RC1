...
@register_operator_compute("in_infer_v2", op_mode="static", support_fusion=True)
def in_infer_compute(x,
    ...
    mean_board = tbe.broadcast(mean, shape_x)

    x_mean = tbe.vsub(x, mean_board)

    multiplier_add = tbe.vadds(variance, epsilon)

    if impl_mode == 'high_precision':
        multiplier_sqrt = tbe.vrsqrt(multiplier_add, priority_flag=1)
    else:
        multiplier_sqrt = tbe.vsqrt(multiplier_add, priority_flag=0)

    if gamma is not None and beta is not None:
        if impl_mode == 'high_precision':
            mean_wo_scale = tbe.vmul(gamma, multiplier_sqrt)
        else:
            mean_wo_scale = tbe.vdiv(gamma, multiplier_sqrt)
        mean_wo_board = tbe.broadcast(mean_wo_scale, shape_x)
        beta = tbe.broadcast(beta, shape_x)
        gamma_scale = tbe.vmul(x_mean, mean_wo_board)
        result = tbe.vadd(gamma_scale, beta)
    else:
        multiplier_sqrt_board = tbe.broadcast(multiplier_sqrt, shape_x)
        if impl_mode == 'high_precision':
            result = tbe.vmul(x_mean, multiplier_sqrt_board)
        else:
            result = tbe.vdiv(x_mean, multiplier_sqrt_board)

    if is_cast:
        result = tbe.cast_to(result, "float16")
    scalar_zero = 0.0
    res_batch_mean = tbe.vadds(mean, scalar_zero)
    res_batch_var = tbe.vadds(variance, scalar_zero)
