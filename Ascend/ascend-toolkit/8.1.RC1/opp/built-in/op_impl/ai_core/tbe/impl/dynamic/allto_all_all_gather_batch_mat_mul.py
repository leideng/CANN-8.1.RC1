...
def _build_args(x_in__, weight_in__, bias_in__, y1_out_, y2_out_, y3_out_, group_ep, group_tp, ep_world_size, tp_world_size, x_shard_type, act_type, transpose_weight, output_y2_flag, output_y3_flag):
    __inputs__ = []
    for arg in [x_in__, weight_in__, bias_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [y1_out_, y2_out_, y3_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if tp_world_size != None:
        ...
        __attrs__.append(attr)
    if x_shard_type != None:
        attr = {}
        attr["name"] = "x_shard_type"
        attr["dtype"] = "int"
        attr["value"] = x_shard_type
        __attrs__.append(attr)
    if act_type != None:
        attr = {}
        attr["name"] = "act_type"
    ...
    if transpose_weight != None:
        ...
        __attrs__.append(attr)
    if output_y2_flag != None:
        attr = {}
        attr["name"] = "output_y2_flag"
        attr["dtype"] = "bool"
        attr["value"] = output_y2_flag
        __attrs__.append(attr)
    if output_y3_flag != None:
        attr = {}
        attr["name"] = "output_y3_flag"
        attr["dtype"] = "bool"
        attr["value"] = output_y3_flag
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("AlltoAllAllGatherBatchMatMul", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_OUTPUT, para_check.OPTION_OUTPUT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def allto_all_all_gather_batch_mat_mul(x_in__, weight_in__, bias_in__=None, y1_out_=None, y2_out_=None, y3_out_=None, group_ep="", group_tp="", ep_world_size=0, tp_world_size=0, x_shard_type=0, act_type=0, transpose_weight=False, output_y2_flag=False, output_y3_flag=False, kernel_name="allto_all_all_gather_batch_mat_mul", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, bias_in__, y1_out_, y2_out_, y3_out_, group_ep, group_tp, ep_world_size, tp_world_size, x_shard_type, act_type, transpose_weight, output_y2_flag, output_y3_flag)
    ...
    origin_func_name = "allto_all_all_gather_batch_mat_mul"
    ascendc_src_dir_ex = "allto_all_all_gather_batch_mat_mul"
    ascendc_src_dir = "allto_all_all_gather_batch_mat_mul"
    ascendc_src_file = "allto_all_all_gather_batch_mat_mul.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator AlltoAllAllGatherBatchMatMul, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "AlltoAllAllGatherBatchMatMul"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, weight_in__, bias_in__], origin_outputs = [y1_out_, y2_out_, y3_out_],\
                param_type_dynamic = False, mc2_ctx = ["mc2_context_0", "mc2_context_1"], param_type_list = ['required', 'required', 'optional', 'required', 'optional', 'optional'], init_value_list = [None, None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{"auto_sync": false}')

def op_select_format(x_in__, weight_in__, bias_in__=None, y1_out_=None, y2_out_=None, y3_out_=None, group_ep="", group_tp="", ep_world_size=0, tp_world_size=0, x_shard_type=0, act_type=0, transpose_weight=False, output_y2_flag=False, output_y3_flag=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, bias_in__, y1_out_, y2_out_, y3_out_, group_ep, group_tp, ep_world_size, tp_world_size, x_shard_type, act_type, transpose_weight, output_y2_flag, output_y3_flag)
    result = check_op_cap("op_select_format", "AlltoAllAllGatherBatchMatMul", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, weight_in__, bias_in__=None, y1_out_=None, y2_out_=None, y3_out_=None, group_ep="", group_tp="", ep_world_size=0, tp_world_size=0, x_shard_type=0, act_type=0, transpose_weight=False, output_y2_flag=False, output_y3_flag=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, bias_in__, y1_out_, y2_out_, y3_out_, group_ep, group_tp, ep_world_size, tp_world_size, x_shard_type, act_type, transpose_weight, output_y2_flag, output_y3_flag)
    result = check_op_cap("get_op_specific_info", "AlltoAllAllGatherBatchMatMul", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
