 ...
 public:
  ...
  __aicore__ inline void Rope(int64_t idx) {
    ...
    for (int64_t hiddenDimLoop = 0; hiddenDimLoop < hiddenDimLoops_; hiddenDimLoop++) {
      ...
      int64_t sinCosIdxOffset = ((GetBlockIdx() * tilingData_->batchsPerBlock + idx) * seqLength_ + hiddenDimLoop * hiddenDimCountPerLoop_) * hiddenDim;
      LocalTensor<float> cosLocal = inQueueCos.AllocTensor<float>();
      DataCopy(cosLocal[hiddenDimCount * hiddenDim].template ReinterpretCast<T>(), cosGm[sinCosIdxOffset], hiddenDimCount * hiddenDim);
      
      inQueueCos.EnQue(cosLocal);
      cosLocal = inQueueCos.DeQue<float>();
      Cast(cosLocal, cosLocal[hiddenDimCount * hiddenDim].template ReinterpretCast<T>(), RoundMode::CAST_NONE, hiddenDimCount * hiddenDim);

      LocalTensor<float> sinLocal = inQueueSin.AllocTensor<float>();
      DataCopy(sinLocal[hiddenDimCount * hiddenDim].template ReinterpretCast<T>(), sinGm[sinCosIdxOffset], hiddenDimCount * hiddenDim);
      inQueueSin.EnQue(sinLocal);
      sinLocal = inQueueSin.DeQue<float>();
      Cast(sinLocal, sinLocal[hiddenDimCount * hiddenDim].template ReinterpretCast<T>(), RoundMode::CAST_NONE, hiddenDimCount * hiddenDim);

      int64_t batchOffset = (GetBlockIdx() * tilingData_->batchsPerBlock + idx) * numHead_ * seqLength_;
      for (int64_t nIdx = 0; nIdx < numHead_; nIdx++) {
        int64_t xOffset = (batchOffset + nIdx * seqLength_ + hiddenDimLoop * hiddenDimCountPerLoop_) * hiddenDim;
        // load xLocal
        ...
        uint64_t rsvdCnt = 0;
        GatherMask(realLocal[hiddenDimCount * hiddenDimHalf].template ReinterpretCast<T>(), xLocal, 1, true, hiddenDimCount * hiddenDim, {1, 1, 8, 0}, rsvdCnt);
        GatherMask(imagLocal[hiddenDimCount * hiddenDimHalf].template ReinterpretCast<T>(), xLocal, numTwo, true, hiddenDimCount * hiddenDim, {1, 1, 8, 0}, rsvdCnt);
        Cast(realLocal, realLocal[hiddenDimCount * hiddenDimHalf].template ReinterpretCast<T>(), RoundMode::CAST_NONE, hiddenDimCount * hiddenDimHalf);
        Cast(imagLocal, imagLocal[hiddenDimCount * hiddenDimHalf].template ReinterpretCast<T>(), RoundMode::CAST_NONE, hiddenDimCount * hiddenDimHalf);
        ...
        Mul(outLocal[hiddenDimHalf], imagLocal, cosLocal[hiddenDimHalf], mask, hiddenDimCount, {1, 1, 1, 8, 4, 8});
        PipeBarrier<PIPE_V>();
        ...
        PipeBarrier<PIPE_V>();
        Cast(outLocal[hiddenDimCount * hiddenDim].template ReinterpretCast<T>(), outLocal, RoundMode::CAST_RINT, hiddenDimCount * hiddenDim);
        PipeBarrier<PIPE_V>();
        outQueueY.EnQue(outLocal);
        outLocal = outQueueY.DeQue<float>();
        DataCopy(yGm[xOffset], outLocal[hiddenDimCount * hiddenDim].template ReinterpretCast<T>(), hiddenDimCount * hiddenDim);
