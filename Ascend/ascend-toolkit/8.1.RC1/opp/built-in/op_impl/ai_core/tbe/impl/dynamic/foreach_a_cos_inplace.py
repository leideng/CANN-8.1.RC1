...
@tbe_register.register_operator("ForeachACosInplace", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.DYNAMIC_INPUT, para_check.KERNEL_NAME)
def foreach_a_cos_inplace(x_in__, kernel_name="foreach_a_cos_inplace", impl_mode=""):
    ...
    origin_func_name = "foreach_a_cos_inplace"
    ascendc_src_dir_ex = "foreach_a_cos_inplace"
    ascendc_src_dir = "foreach_a_cos_inplace"
    ascendc_src_file = "foreach_a_cos_inplace.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator ForeachACosInplace, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
...
def op_select_format(x_in__, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__)
    result = check_op_cap("op_select_format", "ForeachACosInplace", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__)
    result = check_op_cap("get_op_specific_info", "ForeachACosInplace", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
