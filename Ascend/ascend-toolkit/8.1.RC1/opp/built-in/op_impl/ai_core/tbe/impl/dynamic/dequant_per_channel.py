...
def _build_args(y_in__, scale_in__, offset_in__, x_out_):
    __inputs__ = []
    for arg in [y_in__, scale_in__, offset_in__]:
        if arg != None:
...
@tbe_register.register_operator("DequantPerChannel", trans_bool_to_s8=False)
...
def dequant_per_channel(y_in__, scale_in__, offset_in__=None, x_out_=None, kernel_name="dequant_per_channel", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(y_in__, scale_in__, offset_in__, x_out_)
    ...
    origin_func_name = "dequant_per_channel"
    ascendc_src_dir_ex = "dequant_per_channel"
    ascendc_src_dir = "dequant_per_channel"
    ascendc_src_file = "dequant_per_channel.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator DequantPerChannel, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "DequantPerChannel"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[y_in__, scale_in__, offset_in__], origin_outputs = [x_out_],\
...
def op_select_format(y_in__, scale_in__, offset_in__=None, x_out_=None, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(y_in__, scale_in__, offset_in__, x_out_)
    result = check_op_cap("op_select_format", "DequantPerChannel", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(y_in__, scale_in__, offset_in__=None, x_out_=None, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(y_in__, scale_in__, offset_in__, x_out_)
    result = check_op_cap("get_op_specific_info", "DequantPerChannel", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
