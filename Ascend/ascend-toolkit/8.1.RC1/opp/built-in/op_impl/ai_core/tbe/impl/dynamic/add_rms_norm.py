...
def _build_args(x1_in__, x2_in__, gamma_in__, y_out_, rstd_out_, x_out_, epsilon):
    __inputs__ = []
...
@tbe_register.register_operator("AddRmsNorm", trans_bool_to_s8=False)
...
def add_rms_norm(x1_in__, x2_in__, gamma_in__, y_out_, rstd_out_, x_out_, epsilon=1e-06, kernel_name="add_rms_norm", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, gamma_in__, y_out_, rstd_out_, x_out_, epsilon)
    ...
    origin_func_name = "add_rms_norm"
    ascendc_src_dir_ex = "add_rms_norm"
    ascendc_src_dir = "add_rms_norm"
    ascendc_src_file = "add_rms_norm.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator AddRmsNorm, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "AddRmsNorm"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x1_in__, x2_in__, gamma_in__], origin_outputs = [y_out_, rstd_out_, x_out_],\
...
def op_select_format(x1_in__, x2_in__, gamma_in__, y_out_, rstd_out_, x_out_, epsilon=1e-06, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, gamma_in__, y_out_, rstd_out_, x_out_, epsilon)
    result = check_op_cap("op_select_format", "AddRmsNorm", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x1_in__, x2_in__, gamma_in__, y_out_, rstd_out_, x_out_, epsilon=1e-06, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, gamma_in__, y_out_, rstd_out_, x_out_, epsilon)
    result = check_op_cap("get_op_specific_info", "AddRmsNorm", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
