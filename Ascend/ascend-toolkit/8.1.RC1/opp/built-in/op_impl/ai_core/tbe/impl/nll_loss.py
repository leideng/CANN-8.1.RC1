...
class NllLossCompute:
    ...
    def __init__(self, x, target, weight, reduction, ignore_index, kernel_name, impl_mode):
        ...
        if impl_mode == "high_precision":
            self.invalid_target = ignore_index < 0 or ignore_index >= self.c_dim
        self.ub_size_bytes = tbe_platform.get_soc_spec(tbe_platform.UB_SIZE) - two_kb
        self.init_gm_size()
        self.init_gm()
    ...
    def _db_tiling(self):
        self.ub_size_bytes = tbe_platform.get_soc_spec(tbe_platform.UB_SIZE) // 2 - 2048
        self.init_tiling_size()

        # cancel db when cannot move weight or target to ub one time
        # with open db
        if self.big_weight or self.big_target:
            self.ub_size_bytes = tbe_platform.get_soc_spec(tbe_platform.UB_SIZE) - 2048
            self.thread_num = 1
            self.init_tiling_size()
        self._compute_none_and_sum_size()
