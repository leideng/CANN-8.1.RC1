...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['qkv', 'cos', 'sin', 'quant_scale', 'quant_offset', 'k_cache', 'v_cache', 'indice']
...
def _build_args(qkv_in__, cos_in__, sin_in__, quant_scale_in__, quant_offset_in__, k_cache_in__, v_cache_in__, indice_in__, q_out_, k_out_, v_out_, k_cache_out_, v_cache_out_, size_splits, layout, kv_output):
    __inputs__ = []
    for arg in [qkv_in__, cos_in__, sin_in__, quant_scale_in__, quant_offset_in__, k_cache_in__, v_cache_in__, indice_in__]:
        if arg != None:
...
@tbe_register.register_operator("RopeQuantKvcache", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def rope_quant_kvcache(qkv_in__, cos_in__, sin_in__, quant_scale_in__, quant_offset_in__, k_cache_in__, v_cache_in__, indice_in__, q_out_, k_out_, v_out_, k_cache_out_, v_cache_out_, size_splits=[], layout="BSND", kv_output=False, kernel_name="rope_quant_kvcache", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(qkv_in__, cos_in__, sin_in__, quant_scale_in__, quant_offset_in__, k_cache_in__, v_cache_in__, indice_in__, q_out_, k_out_, v_out_, k_cache_out_, v_cache_out_, size_splits, layout, kv_output)
    ...
    origin_func_name = "rope_quant_kvcache"
    ascendc_src_dir_ex = "rope_quant_kvcache"
    ascendc_src_dir = "rope_quant_kvcache"
    ascendc_src_file = "rope_quant_kvcache.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator RopeQuantKvcache, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "RopeQuantKvcache"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[qkv_in__, cos_in__, sin_in__, quant_scale_in__, quant_offset_in__, k_cache_in__, v_cache_in__, indice_in__], origin_outputs = [q_out_, k_out_, v_out_, k_cache_out_, v_cache_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required', 'required', 'required', 'required', 'required', 'required', 'required', 'required', 'required'], init_value_list = [None, None, None, None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{"auto_sync": false}')

def op_select_format(qkv_in__, cos_in__, sin_in__, quant_scale_in__, quant_offset_in__, k_cache_in__, v_cache_in__, indice_in__, q_out_, k_out_, v_out_, k_cache_out_, v_cache_out_, size_splits=[], layout="BSND", kv_output=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(qkv_in__, cos_in__, sin_in__, quant_scale_in__, quant_offset_in__, k_cache_in__, v_cache_in__, indice_in__, q_out_, k_out_, v_out_, k_cache_out_, v_cache_out_, size_splits, layout, kv_output)
    result = check_op_cap("op_select_format", "RopeQuantKvcache", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(qkv_in__, cos_in__, sin_in__, quant_scale_in__, quant_offset_in__, k_cache_in__, v_cache_in__, indice_in__, q_out_, k_out_, v_out_, k_cache_out_, v_cache_out_, size_splits=[], layout="BSND", kv_output=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(qkv_in__, cos_in__, sin_in__, quant_scale_in__, quant_offset_in__, k_cache_in__, v_cache_in__, indice_in__, q_out_, k_out_, v_out_, k_cache_out_, v_cache_out_, size_splits, layout, kv_output)
    result = check_op_cap("get_op_specific_info", "RopeQuantKvcache", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
