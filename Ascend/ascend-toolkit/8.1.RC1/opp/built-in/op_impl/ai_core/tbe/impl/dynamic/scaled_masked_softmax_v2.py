...
def _build_args(x_in__, mask_in__, y_out_, scale, fixed_triu_mask):
    __inputs__ = []
    for arg in [x_in__, mask_in__]:
        if arg != None:
...
@tbe_register.register_operator("ScaledMaskedSoftmaxV2", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def scaled_masked_softmax_v2(x_in__, mask_in__=None, y_out_=None, scale=1, fixed_triu_mask=False, kernel_name="scaled_masked_softmax_v2", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, mask_in__, y_out_, scale, fixed_triu_mask)
    ...
    origin_func_name = "scaled_masked_softmax_v2"
    ascendc_src_dir_ex = "scaled_masked_softmax_v2"
    ascendc_src_dir = "scaled_masked_softmax_v2"
    ascendc_src_file = "scaled_masked_softmax_v2.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator ScaledMaskedSoftmaxV2, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "ScaledMaskedSoftmaxV2"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, mask_in__], origin_outputs = [y_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'optional', 'required'], init_value_list = [None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(x_in__, mask_in__=None, y_out_=None, scale=1, fixed_triu_mask=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, mask_in__, y_out_, scale, fixed_triu_mask)
    result = check_op_cap("op_select_format", "ScaledMaskedSoftmaxV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, mask_in__=None, y_out_=None, scale=1, fixed_triu_mask=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, mask_in__, y_out_, scale, fixed_triu_mask)
    result = check_op_cap("get_op_specific_info", "ScaledMaskedSoftmaxV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
