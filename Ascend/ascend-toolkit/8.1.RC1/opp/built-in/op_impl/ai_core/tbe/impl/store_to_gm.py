...
@register_operator_compute("store_to_gm", op_mode="static", support_fusion=True)
...
def _tilling_axis(valid_shape, input_dtype):
    ub_size_bytes = tbe_platform.get_soc_spec(tbe_platform.UB_SIZE) - 32
    dtype_bytes_size = tbe_platform.get_bit_len(input_dtype) // 8
...
def store_to_gm(input_x, output_x, kernel_name="store_to_gm"):
    ...
    sch[input_tensor].set_scope(tbe_platform.scope_cbuf_fusion)
