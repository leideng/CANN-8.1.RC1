...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['dy', 'x', 'rstd', 'mean', 'gamma']
    output_names = ['pd_x', 'pd_gamma', 'pd_beta']
...
def _build_args(dy_in__, x_in__, rstd_in__, mean_in__, gamma_in__, pd_x_out_, pd_gamma_out_, pd_beta_out_):
    __inputs__ = []
    for arg in [dy_in__, x_in__, rstd_in__, mean_in__, gamma_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [pd_x_out_, pd_gamma_out_, pd_beta_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("LayerNormGradV3", trans_bool_to_s8=False)
...
def layer_norm_grad_v3(dy_in__, x_in__, rstd_in__, mean_in__, gamma_in__, pd_x_out_, pd_gamma_out_, pd_beta_out_, kernel_name="layer_norm_grad_v3", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(dy_in__, x_in__, rstd_in__, mean_in__, gamma_in__, pd_x_out_, pd_gamma_out_, pd_beta_out_)
    ...
    origin_func_name = "layer_norm_grad_v3"
    ascendc_src_dir_ex = "layer_norm_grad_v3"
    ascendc_src_dir = "layer_norm_grad_v3"
    ascendc_src_file = "layer_norm_grad_v3.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator LayerNormGradV3, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "LayerNormGradV3"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[dy_in__, x_in__, rstd_in__, mean_in__, gamma_in__], origin_outputs = [pd_x_out_, pd_gamma_out_, pd_beta_out_],\
...
def op_select_format(dy_in__, x_in__, rstd_in__, mean_in__, gamma_in__, pd_x_out_, pd_gamma_out_, pd_beta_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(dy_in__, x_in__, rstd_in__, mean_in__, gamma_in__, pd_x_out_, pd_gamma_out_, pd_beta_out_)
    result = check_op_cap("op_select_format", "LayerNormGradV3", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(dy_in__, x_in__, rstd_in__, mean_in__, gamma_in__, pd_x_out_, pd_gamma_out_, pd_beta_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(dy_in__, x_in__, rstd_in__, mean_in__, gamma_in__, pd_x_out_, pd_gamma_out_, pd_beta_out_)
    result = check_op_cap("get_op_specific_info", "LayerNormGradV3", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
