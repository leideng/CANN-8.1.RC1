...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['prev_attn_out', 'prev_softmax_max', 'prev_softmax_sum', 'cur_attn_out', 'cur_softmax_max', 'cur_softmax_sum', 'actual_seq_qlen']
    output_names = ['attn_out', 'softmax_max', 'softmax_sum']
...
def _build_args(prev_attn_out_in__, prev_softmax_max_in__, prev_softmax_sum_in__, cur_attn_out_in__, cur_softmax_max_in__, cur_softmax_sum_in__, actual_seq_qlen_in__, attn_out_out_, softmax_max_out_, softmax_sum_out_, input_layout):
    __inputs__ = []
    for arg in [prev_attn_out_in__, prev_softmax_max_in__, prev_softmax_sum_in__, cur_attn_out_in__, cur_softmax_max_in__, cur_softmax_sum_in__, actual_seq_qlen_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [attn_out_out_, softmax_max_out_, softmax_sum_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("RingAttentionUpdate", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_STR, para_check.KERNEL_NAME)
def ring_attention_update(prev_attn_out_in__, prev_softmax_max_in__, prev_softmax_sum_in__, cur_attn_out_in__, cur_softmax_max_in__, cur_softmax_sum_in__, actual_seq_qlen_in__=None, attn_out_out_=None, softmax_max_out_=None, softmax_sum_out_=None, input_layout="SBH", kernel_name="ring_attention_update", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(prev_attn_out_in__, prev_softmax_max_in__, prev_softmax_sum_in__, cur_attn_out_in__, cur_softmax_max_in__, cur_softmax_sum_in__, actual_seq_qlen_in__, attn_out_out_, softmax_max_out_, softmax_sum_out_, input_layout)
    ...
    origin_func_name = "ring_attention_update"
    ascendc_src_dir_ex = "ring_attention_update"
    ascendc_src_dir = "ring_attention_update"
    ascendc_src_file = "ring_attention_update.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator RingAttentionUpdate, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "RingAttentionUpdate"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[prev_attn_out_in__, prev_softmax_max_in__, prev_softmax_sum_in__, cur_attn_out_in__, cur_softmax_max_in__, cur_softmax_sum_in__, actual_seq_qlen_in__], origin_outputs = [attn_out_out_, softmax_max_out_, softmax_sum_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required', 'required', 'optional', 'required', 'required', 'required'], init_value_list = [None, None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{"auto_sync": false}')

def op_select_format(prev_attn_out_in__, prev_softmax_max_in__, prev_softmax_sum_in__, cur_attn_out_in__, cur_softmax_max_in__, cur_softmax_sum_in__, actual_seq_qlen_in__=None, attn_out_out_=None, softmax_max_out_=None, softmax_sum_out_=None, input_layout="SBH", impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(prev_attn_out_in__, prev_softmax_max_in__, prev_softmax_sum_in__, cur_attn_out_in__, cur_softmax_max_in__, cur_softmax_sum_in__, actual_seq_qlen_in__, attn_out_out_, softmax_max_out_, softmax_sum_out_, input_layout)
    result = check_op_cap("op_select_format", "RingAttentionUpdate", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(prev_attn_out_in__, prev_softmax_max_in__, prev_softmax_sum_in__, cur_attn_out_in__, cur_softmax_max_in__, cur_softmax_sum_in__, actual_seq_qlen_in__=None, attn_out_out_=None, softmax_max_out_=None, softmax_sum_out_=None, input_layout="SBH", impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(prev_attn_out_in__, prev_softmax_max_in__, prev_softmax_sum_in__, cur_attn_out_in__, cur_softmax_max_in__, cur_softmax_sum_in__, actual_seq_qlen_in__, attn_out_out_, softmax_max_out_, softmax_sum_out_, input_layout)
    result = check_op_cap("get_op_specific_info", "RingAttentionUpdate", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
