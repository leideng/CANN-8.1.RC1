...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['token_a', 'token_b', 'topk_indice', 'arg_max']
    output_names = ['unmerge_token_a', 'unreduce_token_b', 'unreduce_count']
...
def _build_args(token_a_in__, token_b_in__, topk_indice_in__, arg_max_in__, unmerge_token_a_out_, unreduce_token_b_out_, unreduce_count_out_, top_rate):
    __inputs__ = []
    for arg in [token_a_in__, token_b_in__, topk_indice_in__, arg_max_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [unmerge_token_a_out_, unreduce_token_b_out_, unreduce_count_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("TomeMerge", trans_bool_to_s8=False)
...
def tome_merge(token_a_in__, token_b_in__, topk_indice_in__, arg_max_in__, unmerge_token_a_out_, unreduce_token_b_out_, unreduce_count_out_, top_rate=0.5, kernel_name="tome_merge", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(token_a_in__, token_b_in__, topk_indice_in__, arg_max_in__, unmerge_token_a_out_, unreduce_token_b_out_, unreduce_count_out_, top_rate)
    ...
    origin_func_name = "tome_merge"
    ascendc_src_dir_ex = "tome_merge"
    ascendc_src_dir = "tome_merge"
    ascendc_src_file = "tome_merge.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator TomeMerge, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "TomeMerge"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[token_a_in__, token_b_in__, topk_indice_in__, arg_max_in__], origin_outputs = [unmerge_token_a_out_, unreduce_token_b_out_, unreduce_count_out_],\
...
def op_select_format(token_a_in__, token_b_in__, topk_indice_in__, arg_max_in__, unmerge_token_a_out_, unreduce_token_b_out_, unreduce_count_out_, top_rate=0.5, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(token_a_in__, token_b_in__, topk_indice_in__, arg_max_in__, unmerge_token_a_out_, unreduce_token_b_out_, unreduce_count_out_, top_rate)
    result = check_op_cap("op_select_format", "TomeMerge", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(token_a_in__, token_b_in__, topk_indice_in__, arg_max_in__, unmerge_token_a_out_, unreduce_token_b_out_, unreduce_count_out_, top_rate=0.5, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(token_a_in__, token_b_in__, topk_indice_in__, arg_max_in__, unmerge_token_a_out_, unreduce_token_b_out_, unreduce_count_out_, top_rate)
    result = check_op_cap("get_op_specific_info", "TomeMerge", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
