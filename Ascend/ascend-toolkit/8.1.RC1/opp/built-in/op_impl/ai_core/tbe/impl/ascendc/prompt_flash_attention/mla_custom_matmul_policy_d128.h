...
namespace Impl {
namespace Detail {
template <typename IMPL, class INPUT_TYPE, const auto &MM_CFG> class MlaCopyCubeInMM2HeadDim128 {
...
public:
                                                        ...
                                                        int32_t baseBlockSize)
    {
        ...
        MATMUL_MODULE(DataCopyUtils)
            ->CopyTileToCube(MATMUL_MODULE(Context)->CurrentTensorA, curRow, curCol, tileHeight, tileWidth);
        if (posL1 == 0) {
