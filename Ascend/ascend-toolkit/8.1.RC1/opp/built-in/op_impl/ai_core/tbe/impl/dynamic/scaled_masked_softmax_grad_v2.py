...
def _build_args(yGrad_in__, y_in__, mask_in__, xGrad_out_, scale, fixed_triu_mask):
    __inputs__ = []
    for arg in [yGrad_in__, y_in__, mask_in__]:
        if arg != None:
    ...
    if fixed_triu_mask != None:
        attr = {}
        attr["name"] = "fixed_triu_mask"
        attr["dtype"] = "bool"
        attr["value"] = fixed_triu_mask
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("ScaledMaskedSoftmaxGradV2", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def scaled_masked_softmax_grad_v2(yGrad_in__, y_in__, mask_in__=None, xGrad_out_=None, scale=1, fixed_triu_mask=False, kernel_name="scaled_masked_softmax_grad_v2", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(yGrad_in__, y_in__, mask_in__, xGrad_out_, scale, fixed_triu_mask)
    ...
    origin_func_name = "scaled_masked_softmax_grad_v2"
    ascendc_src_dir_ex = "scaled_masked_softmax_grad_v2"
    ascendc_src_dir = "scaled_masked_softmax_grad_v2"
    ascendc_src_file = "scaled_masked_softmax_grad_v2.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator ScaledMaskedSoftmaxGradV2, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "ScaledMaskedSoftmaxGradV2"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[yGrad_in__, y_in__, mask_in__], origin_outputs = [xGrad_out_],\
...
def op_select_format(yGrad_in__, y_in__, mask_in__=None, xGrad_out_=None, scale=1, fixed_triu_mask=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(yGrad_in__, y_in__, mask_in__, xGrad_out_, scale, fixed_triu_mask)
    result = check_op_cap("op_select_format", "ScaledMaskedSoftmaxGradV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(yGrad_in__, y_in__, mask_in__=None, xGrad_out_=None, scale=1, fixed_triu_mask=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(yGrad_in__, y_in__, mask_in__, xGrad_out_, scale, fixed_triu_mask)
    result = check_op_cap("get_op_specific_info", "ScaledMaskedSoftmaxGradV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
