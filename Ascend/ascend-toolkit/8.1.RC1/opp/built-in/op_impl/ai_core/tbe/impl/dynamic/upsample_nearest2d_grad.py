...
@tbe_register.register_operator("UpsampleNearest2dGrad", trans_bool_to_s8=False)
...
def upsample_nearest2d_grad(grad_output_in__, grad_input_out_, output_size, input_size, scales_h=0, scales_w=0, kernel_name="upsample_nearest2d_grad", impl_mode=""):
    ...
    origin_func_name = "upsample_nearest2d_grad"
    ascendc_src_dir_ex = "upsample_nearest2d_grad"
    ascendc_src_dir = "upsample_nearest2d_grad"
    ascendc_src_file = "upsample_nearest2d_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator UpsampleNearest2dGrad, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "UpsampleNearest2dGrad"
...
def op_select_format(grad_output_in__, grad_input_out_, output_size, input_size, scales_h=0, scales_w=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_output_in__, grad_input_out_, output_size, input_size, scales_h, scales_w)
    result = check_op_cap("op_select_format", "UpsampleNearest2dGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(grad_output_in__, grad_input_out_, output_size, input_size, scales_h=0, scales_w=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_output_in__, grad_input_out_, output_size, input_size, scales_h, scales_w)
    result = check_op_cap("get_op_specific_info", "UpsampleNearest2dGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
