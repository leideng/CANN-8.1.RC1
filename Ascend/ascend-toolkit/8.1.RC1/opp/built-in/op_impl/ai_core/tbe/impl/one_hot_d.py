...
def check_supported(input_x,
    ...
    shape_list = [(2048,), (240, 21128), (2, 94, 311), (128, 128, 10, 5, 5, 10)]
...
def _is_greater_than_32b(row_num_each_core, depth, indices_dtype,
    ...
    """
    byte_one_num = get_bit_len(output_dtype) // 8
...
def _get_core_num_depth_large(indices_input, depth, output_dtype):
    ...
    byte_size_output_dtype = get_bit_len(output_dtype) // 8
...
def _one_core_depth_large(tvm_ib, indices_input, output, args_dict):
    ...
    args_dict['block_index']: tvm.tir.IterVar.
    ...
    op_args['num_in_32B'] = 32 * 8 // get_bit_len(output.dtype)
...
def _split_large_depth_func(depth, output_dtype):
    ...
    ub_remaining = tbe_platform.get_soc_spec(tbe_platform.UB_SIZE) -\
                   ...
                   ON_OFF_VAL_SPACE
    byte_num_dtype = get_bit_len(output_dtype) // 8
...
def one_hot_ir(ins, output, depth, axis):
    ...
    if not _check_is_depth_large(depth, output.dtype):
        if _check_is_prime_case(indices_input, output.dtype, depth):
            ...
            if cce_product in ("Ascend910",) or tbe_platform.api_check_support("tik.vcopy"):
