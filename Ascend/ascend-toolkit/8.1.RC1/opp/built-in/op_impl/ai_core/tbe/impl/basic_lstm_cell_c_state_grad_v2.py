...
class LstmCellGradInput():
    ...
    def __init__(self, cell_state, dht_out, dht, dct, inpute_gate, forget_gate,
                 update_gate, output_gate, tanh_ct, gate_order, kernel_name):
        """
        ...
        if (self.batch_size * self.hidden_size) % (self.aicore_num * constant.SIZE_SIXTEEN) != 0:
            self.aicore_num = constant.MAX_BLOCK_NUMBER
...
class LstmCellGrad(LstmCellGradInput):
    ...
    def get_loop_params(self):
        ...
        dtype_size = tbe_platform.get_bit_len(self.dht_dtype) // 8
        ...
        if total_num_each_core * dtype_size < ub_size_bytes:
            ...
            self.ub_size = (
                ...
                self.out_loop_num // constant.BLOCK_SIZE *
                constant.BLOCK_SIZE)
    ...
    def move_vector_data_out(self, index, ele_num):
        ...
        self.tik_instance.data_move(self.gm_dgate[index], gate_data_map.get(self.gate_order[0], dit_src), 0, 1,
                                    dgate_burst_len, 0, 0)
        self.tik_instance.data_move(self.gm_dgate[index + offset], gate_data_map.get(self.gate_order[1], djt_src), 0,
                                    1, dgate_burst_len, 0, 0)
        self.tik_instance.data_move(self.gm_dgate[index + offset * 2], gate_data_map.get(self.gate_order[2], dft_src),
                                    0, 1, dgate_burst_len, 0, 0)
        self.tik_instance.data_move(self.gm_dgate[index + offset * 3], gate_data_map.get(self.gate_order[3], dot_src),
                                    0, 1, dgate_burst_len, 0, 0)
