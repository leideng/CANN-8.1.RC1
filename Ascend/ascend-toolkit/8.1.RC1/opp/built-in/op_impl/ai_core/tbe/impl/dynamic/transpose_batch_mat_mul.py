...
def _build_args(x1_in__, x2_in__, bias_in__, scale_in__, y_out_, perm_x1, perm_x2, perm_y, enable_hf32, batch_split_factor):
    __inputs__ = []
    for arg in [x1_in__, x2_in__, bias_in__, scale_in__]:
        if arg != None:
    ...
    if enable_hf32 != None:
        ...
        __attrs__.append(attr)
    if batch_split_factor != None:
        attr = {}
        attr["name"] = "batch_split_factor"
        attr["dtype"] = "int"
        attr["value"] = batch_split_factor
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("TransposeBatchMatMul", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def transpose_batch_mat_mul(x1_in__, x2_in__, bias_in__=None, scale_in__=None, y_out_=None, perm_x1=[], perm_x2=[], perm_y=[], enable_hf32=False, batch_split_factor=1, kernel_name="transpose_batch_mat_mul", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, bias_in__, scale_in__, y_out_, perm_x1, perm_x2, perm_y, enable_hf32, batch_split_factor)
    ...
    origin_func_name = "transpose_batch_mat_mul"
    ascendc_src_dir_ex = "transpose_batch_mat_mul"
    ascendc_src_dir = "transpose_batch_mat_mul"
    ascendc_src_file = "transpose_batch_mat_mul.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator TransposeBatchMatMul, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x1_in__, x2_in__, bias_in__, scale_in__], origin_outputs = [y_out_],\
