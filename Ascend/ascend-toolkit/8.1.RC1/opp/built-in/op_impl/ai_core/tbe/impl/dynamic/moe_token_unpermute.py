...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['permuted_tokens', 'sorted_indices', 'probs']
    output_names = ['unpermuted_tokens']
...
def _build_args(permuted_tokens_in__, sorted_indices_in__, probs_in__, unpermuted_tokens_out_, padded_mode, restore_shape):
    __inputs__ = []
    for arg in [permuted_tokens_in__, sorted_indices_in__, probs_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [unpermuted_tokens_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("MoeTokenUnpermute", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_LIST_INT, para_check.KERNEL_NAME)
def moe_token_unpermute(permuted_tokens_in__, sorted_indices_in__, probs_in__=None, unpermuted_tokens_out_=None, padded_mode=False, restore_shape=[], kernel_name="moe_token_unpermute", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(permuted_tokens_in__, sorted_indices_in__, probs_in__, unpermuted_tokens_out_, padded_mode, restore_shape)
    ...
    origin_func_name = "moe_token_unpermute"
    ascendc_src_dir_ex = "moe_token_unpermute"
    ascendc_src_dir = "moe_token_unpermute"
    ascendc_src_file = "moe_token_unpermute.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MoeTokenUnpermute, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[permuted_tokens_in__, sorted_indices_in__, probs_in__], origin_outputs = [unpermuted_tokens_out_],\
...
def op_select_format(permuted_tokens_in__, sorted_indices_in__, probs_in__=None, unpermuted_tokens_out_=None, padded_mode=False, restore_shape=[], impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(permuted_tokens_in__, sorted_indices_in__, probs_in__, unpermuted_tokens_out_, padded_mode, restore_shape)
    result = check_op_cap("op_select_format", "MoeTokenUnpermute", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(permuted_tokens_in__, sorted_indices_in__, probs_in__=None, unpermuted_tokens_out_=None, padded_mode=False, restore_shape=[], impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(permuted_tokens_in__, sorted_indices_in__, probs_in__, unpermuted_tokens_out_, padded_mode, restore_shape)
    result = check_op_cap("get_op_specific_info", "MoeTokenUnpermute", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
