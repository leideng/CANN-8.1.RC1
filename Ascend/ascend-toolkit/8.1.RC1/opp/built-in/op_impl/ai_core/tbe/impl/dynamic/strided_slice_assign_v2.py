...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['var', 'input_value', 'begin', 'end', 'strides', 'axes']
    output_names = ['var']
...
def _build_args(var_in__, input_value_in__, begin_in__, end_in__, strides_in__, axes_in__, var_out_):
    __inputs__ = []
    for arg in [var_in__, input_value_in__, begin_in__, end_in__, strides_in__, axes_in__]:
        if arg != None:
...
@tbe_register.register_operator("StridedSliceAssignV2", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.KERNEL_NAME)
def strided_slice_assign_v2(var_in__, input_value_in__, begin_in__, end_in__, strides_in__, axes_in__=None, var_out_=None, kernel_name="strided_slice_assign_v2", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(var_in__, input_value_in__, begin_in__, end_in__, strides_in__, axes_in__, var_out_)
    ...
    origin_func_name = "strided_slice_assign_v2"
    ascendc_src_dir_ex = "strided_slice_assign_v2"
    ascendc_src_dir = "strided_slice_assign_v2"
    ascendc_src_file = "strided_slice_assign_v2.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator StridedSliceAssignV2, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "StridedSliceAssignV2"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[var_in__, input_value_in__, begin_in__, end_in__, strides_in__, axes_in__], origin_outputs = [var_out_],\
...
def op_select_format(var_in__, input_value_in__, begin_in__, end_in__, strides_in__, axes_in__=None, var_out_=None, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(var_in__, input_value_in__, begin_in__, end_in__, strides_in__, axes_in__, var_out_)
    result = check_op_cap("op_select_format", "StridedSliceAssignV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(var_in__, input_value_in__, begin_in__, end_in__, strides_in__, axes_in__=None, var_out_=None, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(var_in__, input_value_in__, begin_in__, end_in__, strides_in__, axes_in__, var_out_)
    result = check_op_cap("get_op_specific_info", "StridedSliceAssignV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
