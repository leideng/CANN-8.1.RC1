...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['var', 'm', 'v', 'grad', 'step', 'max_grad_norm']
    output_names = []
...
def _build_args(var_in__, m_in__, v_in__, grad_in__, step_in__, max_grad_norm_in__, lr, beta1, beta2, weight_decay, eps, amsgrad, maximize):
    __inputs__ = []
    for arg in [var_in__, m_in__, v_in__, grad_in__, step_in__, max_grad_norm_in__]:
        if arg != None:
...
@tbe_register.register_operator("ApplyAdamWV2", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def apply_adam_w_v2(var_in__, m_in__, v_in__, grad_in__, step_in__, max_grad_norm_in__=None, lr=0.1, beta1=0.1, beta2=0.1, weight_decay=0.1, eps=0.1, amsgrad=False, maximize=False, kernel_name="apply_adam_w_v2", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(var_in__, m_in__, v_in__, grad_in__, step_in__, max_grad_norm_in__, lr, beta1, beta2, weight_decay, eps, amsgrad, maximize)
    ...
    origin_func_name = "apply_adam_w_v2"
    ascendc_src_dir_ex = "apply_adam_wv2"
    ascendc_src_dir = "apply_adam_w_v2"
    ascendc_src_file = "apply_adam_w_v2.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator ApplyAdamWV2, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "ApplyAdamWV2"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[var_in__, m_in__, v_in__, grad_in__, step_in__, max_grad_norm_in__], origin_outputs = [],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required', 'optional'], init_value_list = [],\
