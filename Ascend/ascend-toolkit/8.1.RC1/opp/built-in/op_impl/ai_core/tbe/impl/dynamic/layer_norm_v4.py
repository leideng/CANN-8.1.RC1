...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'normalized_shape', 'gamma', 'beta']
...
def _build_args(x_in__, normalized_shape_in__, gamma_in__, beta_in__, y_out_, mean_out_, rstd_out_, epsilon):
    __inputs__ = []
    for arg in [x_in__, normalized_shape_in__, gamma_in__, beta_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [y_out_, mean_out_, rstd_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("LayerNormV4", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def layer_norm_v4(x_in__, normalized_shape_in__, gamma_in__=None, beta_in__=None, y_out_=None, mean_out_=None, rstd_out_=None, epsilon=1e-05, kernel_name="layer_norm_v4", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, normalized_shape_in__, gamma_in__, beta_in__, y_out_, mean_out_, rstd_out_, epsilon)
    ...
    origin_func_name = "layer_norm_v4"
    ascendc_src_dir_ex = "layer_norm_v4"
    ascendc_src_dir = "layer_norm_v4"
    ascendc_src_file = "layer_norm_v4.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator LayerNormV4, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "LayerNormV4"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, normalized_shape_in__, gamma_in__, beta_in__], origin_outputs = [y_out_, mean_out_, rstd_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'optional', 'optional', 'required', 'required', 'required'], init_value_list = [None, None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{"auto_sync": false}')

def op_select_format(x_in__, normalized_shape_in__, gamma_in__=None, beta_in__=None, y_out_=None, mean_out_=None, rstd_out_=None, epsilon=1e-05, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, normalized_shape_in__, gamma_in__, beta_in__, y_out_, mean_out_, rstd_out_, epsilon)
    result = check_op_cap("op_select_format", "LayerNormV4", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, normalized_shape_in__, gamma_in__=None, beta_in__=None, y_out_=None, mean_out_=None, rstd_out_=None, epsilon=1e-05, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, normalized_shape_in__, gamma_in__, beta_in__, y_out_, mean_out_, rstd_out_, epsilon)
    result = check_op_cap("get_op_specific_info", "LayerNormV4", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
