...
@tbe_register.register_operator("MultiScaleDeformableAttnFunction", trans_bool_to_s8=False)
...
def multi_scale_deformable_attn_function(value_in__, value_spatial_shapes_in__, value_level_start_index_in__, sampling_locations_in__, attention_weights_in__, output_out_, kernel_name="multi_scale_deformable_attn_function", impl_mode=""):
    ...
    origin_func_name = "multi_scale_deformable_attn_function"
    ascendc_src_dir_ex = "multi_scale_deformable_attn_function"
    ascendc_src_dir = "multi_scale_deformable_attn_function"
    ascendc_src_file = "multi_scale_deformable_attn_function.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MultiScaleDeformableAttnFunction, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "MultiScaleDeformableAttnFunction"
...
def op_select_format(value_in__, value_spatial_shapes_in__, value_level_start_index_in__, sampling_locations_in__, attention_weights_in__, output_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(value_in__, value_spatial_shapes_in__, value_level_start_index_in__, sampling_locations_in__, attention_weights_in__, output_out_)
    result = check_op_cap("op_select_format", "MultiScaleDeformableAttnFunction", __inputs__, __outputs__, __attrs__)
...
def get_op_specific_info(value_in__, value_spatial_shapes_in__, value_level_start_index_in__, sampling_locations_in__, attention_weights_in__, output_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(value_in__, value_spatial_shapes_in__, value_level_start_index_in__, sampling_locations_in__, attention_weights_in__, output_out_)
    result = check_op_cap("get_op_specific_info", "MultiScaleDeformableAttnFunction", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
