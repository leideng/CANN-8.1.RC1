...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['sorted_experts']
    output_names = ['total_rows_before_expert']
...
def _build_args(sorted_experts_in__, total_rows_before_expert_out_, num_experts):
    __inputs__ = []
    for arg in [sorted_experts_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [total_rows_before_expert_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("MoeComputeExpertTokens", trans_bool_to_s8=False)
...
def moe_compute_expert_tokens(sorted_experts_in__, total_rows_before_expert_out_, num_experts, kernel_name="moe_compute_expert_tokens", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(sorted_experts_in__, total_rows_before_expert_out_, num_experts)
    ...
    origin_func_name = "moe_compute_expert_tokens"
    ascendc_src_dir_ex = "moe_compute_expert_tokens"
    ascendc_src_dir = "moe_compute_expert_tokens"
    ascendc_src_file = "moe_compute_expert_tokens.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MoeComputeExpertTokens, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[sorted_experts_in__], origin_outputs = [total_rows_before_expert_out_],\
...
def op_select_format(sorted_experts_in__, total_rows_before_expert_out_, num_experts, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(sorted_experts_in__, total_rows_before_expert_out_, num_experts)
    result = check_op_cap("op_select_format", "MoeComputeExpertTokens", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(sorted_experts_in__, total_rows_before_expert_out_, num_experts, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(sorted_experts_in__, total_rows_before_expert_out_, num_experts)
    result = check_op_cap("get_op_specific_info", "MoeComputeExpertTokens", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
