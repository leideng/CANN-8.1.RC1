...
def _build_args(input_in__, output_out_, output_size, align_corners, scales_h, scales_w):
    __inputs__ = []
...
@tbe_register.register_operator("UpsampleBilinear2dAA", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def upsample_bilinear2d_aa(input_in__, output_out_, output_size, align_corners=False, scales_h=0, scales_w=0, kernel_name="upsample_bilinear2d_aa", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(input_in__, output_out_, output_size, align_corners, scales_h, scales_w)
    ...
    origin_func_name = "upsample_bilinear2d_aa"
    ascendc_src_dir_ex = "upsample_bilinear2d_aa"
    ascendc_src_dir = "upsample_bilinear2d_a_a"
    ascendc_src_file = "upsample_bilinear2d_aa.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator UpsampleBilinear2dAA, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "UpsampleBilinear2dAA"
...
def op_select_format(input_in__, output_out_, output_size, align_corners=False, scales_h=0, scales_w=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(input_in__, output_out_, output_size, align_corners, scales_h, scales_w)
    result = check_op_cap("op_select_format", "UpsampleBilinear2dAA", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(input_in__, output_out_, output_size, align_corners=False, scales_h=0, scales_w=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(input_in__, output_out_, output_size, align_corners, scales_h, scales_w)
    result = check_op_cap("get_op_specific_info", "UpsampleBilinear2dAA", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
