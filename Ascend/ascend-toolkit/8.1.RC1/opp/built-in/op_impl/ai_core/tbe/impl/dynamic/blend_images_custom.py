...
def _build_args(rgb_in__, alpha_in__, frame_in__, out_out_):
    __inputs__ = []
    for arg in [rgb_in__, alpha_in__, frame_in__]:
        if arg != None:
...
@tbe_register.register_operator("BlendImagesCustom", trans_bool_to_s8=False)
...
def blend_images_custom(rgb_in__, alpha_in__, frame_in__, out_out_, kernel_name="blend_images_custom", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(rgb_in__, alpha_in__, frame_in__, out_out_)
    ...
    origin_func_name = "blend_images_custom"
    ascendc_src_dir_ex = "blend_images_custom"
    ascendc_src_dir = "blend_images_custom"
    ascendc_src_file = "blend_images_custom.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator BlendImagesCustom, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "BlendImagesCustom"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[rgb_in__, alpha_in__, frame_in__], origin_outputs = [out_out_],\
...
def op_select_format(rgb_in__, alpha_in__, frame_in__, out_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(rgb_in__, alpha_in__, frame_in__, out_out_)
    result = check_op_cap("op_select_format", "BlendImagesCustom", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(rgb_in__, alpha_in__, frame_in__, out_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(rgb_in__, alpha_in__, frame_in__, out_out_)
    result = check_op_cap("get_op_specific_info", "BlendImagesCustom", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
