...
@register_operator_compute("TruncateDiv", op_mode="dynamic", support_fusion=True, support_bfp16=True)
def truncate_div_compute(input_x, input_y, output_x,
    ...
    if input_dtype in int_list:
        ...
        if input_dtype in ("int8", "uint8") and is_v200():
            res_trunc = tbe.cast_to(res_trunc, "int32")
            return util_common.int_cast_to_b8(res_trunc, input_dtype)
    else:
...
def truncate_div(input_x, input_y, output_x, kernel_name="truncate_div"):
    ...
    input_x: dict with keys(shape and dtype)
        ...
        {bfloat16,float16,float,int32,int8,uint8(on milan)}
