...
@tbe_register.register_operator("PadV4Grad", trans_bool_to_s8=False)
...
def pad_v4_grad(x_in__, paddings_in__, y_out_, mode, paddings_contiguous, kernel_name="pad_v4_grad", impl_mode=""):
    ...
    origin_func_name = "pad_v4_grad"
    ascendc_src_dir_ex = "pad_v4_grad"
    ascendc_src_dir = "pad_v4_grad"
    ascendc_src_file = "pad_v4_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator PadV4Grad, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "PadV4Grad"
...
def op_select_format(x_in__, paddings_in__, y_out_, mode, paddings_contiguous, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, paddings_in__, y_out_, mode, paddings_contiguous)
    result = check_op_cap("op_select_format", "PadV4Grad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, paddings_in__, y_out_, mode, paddings_contiguous, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, paddings_in__, y_out_, mode, paddings_contiguous)
    result = check_op_cap("get_op_specific_info", "PadV4Grad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
