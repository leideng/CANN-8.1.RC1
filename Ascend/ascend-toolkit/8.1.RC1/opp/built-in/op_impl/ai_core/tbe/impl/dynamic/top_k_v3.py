...
def _build_args(x_in__, k_in__, values_out_, indices_out_, sorted, dim, largest):
    __inputs__ = []
    for arg in [x_in__, k_in__]:
        if arg != None:
...
@tbe_register.register_operator("TopKV3", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def top_k_v3(x_in__, k_in__, values_out_, indices_out_, sorted=True, dim=-1, largest=True, kernel_name="top_k_v3", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, k_in__, values_out_, indices_out_, sorted, dim, largest)
    ...
    origin_func_name = "top_k_v3"
    ascendc_src_dir_ex = "top_kv3"
    ascendc_src_dir = "top_k_v3"
    ascendc_src_file = "top_k_v3.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator TopKV3, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "TopKV3"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, k_in__], origin_outputs = [values_out_, indices_out_],\
