...
template <typename T>
__aicore__ inline void MoeFinalizeRoutingFpCuth<T>::CopyIn(int64_t nLoopIdx, int64_t bias, int64_t dataLen,
    bool isPadH, int64_t rightPaddingH)
{
    ...
    DataCopyPad(expertForSourceRowLocal, gmExpertForSourceRow_[nLoopIdx / (cutNumH_ + 1) * K_],
    ...
    bool isPadH, int64_t rightPaddingH)
{
    ...
    for (int64_t i = 0; i < K_ / PARALLEL_NUM; i++) {
        ...
        int64_t expandedSrcToDstRowIndexDb0 = nLoopIdx / (cutNumH_ + 1) + PARALLEL_NUM * i * BS_ +
        ...
        int64_t expandedSrcToDstRowIndexDb1 = nLoopIdx / (cutNumH_ + 1) + (PARALLEL_NUM * i + 1) * BS_ +
                                              GetBlockIdx() * normalCoreHandleNum_;
    ...
    if (K_ % PARALLEL_NUM != 0) {
        wait_flag(PIPE_V, PIPE_S, EVENT_ID0);
        int64_t expandedSrcToDstRowIndexDb0 = nLoopIdx / (cutNumH_ + 1) + (K_ - 1) * BS_ +
                                              GetBlockIdx() * normalCoreHandleNum_;
        int64_t expandedPermutedRowsIndexDb0 = gmExpandedSrcToDstRow_.GetValue(expandedSrcToDstRowIndexDb0);
        set_flag(PIPE_S, PIPE_MTE2, EVENT_ID0);
