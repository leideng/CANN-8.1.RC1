...
def _build_args(x1_in__, x2_in__, y_out_, N):
    __inputs__ = []
...
@tbe_register.register_operator("MulAddn", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.DYNAMIC_INPUT, para_check.DYNAMIC_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def mul_addn(x1_in__, x2_in__, y_out_, N, kernel_name="mul_addn", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, y_out_, N)
    ...
    origin_func_name = "mul_addn"
    ascendc_src_dir_ex = "mul_addn"
    ascendc_src_dir = "mul_addn"
    ascendc_src_file = "mul_addn.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MulAddn, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "MulAddn"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x1_in__, x2_in__], origin_outputs = [y_out_],\
                param_type_dynamic = True, mc2_ctx = [], param_type_list = ['dynamic', 'dynamic', 'required'], init_value_list = [None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(x1_in__, x2_in__, y_out_, N, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, y_out_, N)
    result = check_op_cap("op_select_format", "MulAddn", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x1_in__, x2_in__, y_out_, N, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, y_out_, N)
    result = check_op_cap("get_op_specific_info", "MulAddn", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
