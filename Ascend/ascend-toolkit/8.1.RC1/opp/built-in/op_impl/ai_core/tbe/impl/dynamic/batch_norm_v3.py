...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'weight', 'bias', 'running_mean', 'running_var']
    output_names = ['y', 'running_mean', 'running_var', 'save_mean', 'save_rstd']
...
def _build_args(x_in__, weight_in__, bias_in__, running_mean_in__, running_var_in__, y_out_, running_mean_out_, running_var_out_, save_mean_out_, save_rstd_out_, epsilon, momentum, is_training):
    __inputs__ = []
    for arg in [x_in__, weight_in__, bias_in__, running_mean_in__, running_var_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [y_out_, running_mean_out_, running_var_out_, save_mean_out_, save_rstd_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("BatchNormV3", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def batch_norm_v3(x_in__, weight_in__, bias_in__, running_mean_in__, running_var_in__, y_out_, running_mean_out_, running_var_out_, save_mean_out_, save_rstd_out_, epsilon=1e-05, momentum=0.1, is_training=True, kernel_name="batch_norm_v3", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, bias_in__, running_mean_in__, running_var_in__, y_out_, running_mean_out_, running_var_out_, save_mean_out_, save_rstd_out_, epsilon, momentum, is_training)
    ...
    origin_func_name = "batch_norm_v3"
    ascendc_src_dir_ex = "batch_norm_v3"
    ascendc_src_dir = "batch_norm_v3"
    ascendc_src_file = "batch_norm_v3.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator BatchNormV3, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "BatchNormV3"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, weight_in__, bias_in__, running_mean_in__, running_var_in__], origin_outputs = [y_out_, running_mean_out_, running_var_out_, save_mean_out_, save_rstd_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required', 'required', 'required', 'required', 'required', 'required'], init_value_list = [None, None, None, None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{"auto_sync": false}')

def op_select_format(x_in__, weight_in__, bias_in__, running_mean_in__, running_var_in__, y_out_, running_mean_out_, running_var_out_, save_mean_out_, save_rstd_out_, epsilon=1e-05, momentum=0.1, is_training=True, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, bias_in__, running_mean_in__, running_var_in__, y_out_, running_mean_out_, running_var_out_, save_mean_out_, save_rstd_out_, epsilon, momentum, is_training)
    result = check_op_cap("op_select_format", "BatchNormV3", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, weight_in__, bias_in__, running_mean_in__, running_var_in__, y_out_, running_mean_out_, running_var_out_, save_mean_out_, save_rstd_out_, epsilon=1e-05, momentum=0.1, is_training=True, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, bias_in__, running_mean_in__, running_var_in__, y_out_, running_mean_out_, running_var_out_, save_mean_out_, save_rstd_out_, epsilon, momentum, is_training)
    result = check_op_cap("get_op_specific_info", "BatchNormV3", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
