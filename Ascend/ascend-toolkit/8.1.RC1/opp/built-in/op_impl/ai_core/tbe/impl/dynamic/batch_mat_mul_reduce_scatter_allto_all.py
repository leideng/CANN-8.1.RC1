...
def _build_args(x_in__, weight_in__, bias_in__, y_out_, group_ep, group_tp, ep_world_size, tp_world_size, y_shard_type, transpose_weight):
    __inputs__ = []
    ...
    if tp_world_size != None:
        ...
        __attrs__.append(attr)
    if y_shard_type != None:
        attr = {}
        attr["name"] = "y_shard_type"
        attr["dtype"] = "int"
        attr["value"] = y_shard_type
        __attrs__.append(attr)
...
@tbe_register.register_operator("BatchMatMulReduceScatterAlltoAll", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def batch_mat_mul_reduce_scatter_allto_all(x_in__, weight_in__, bias_in__=None, y_out_=None, group_ep="", group_tp="", ep_world_size=0, tp_world_size=0, y_shard_type=0, transpose_weight=False, kernel_name="batch_mat_mul_reduce_scatter_allto_all", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, bias_in__, y_out_, group_ep, group_tp, ep_world_size, tp_world_size, y_shard_type, transpose_weight)
    ...
    origin_func_name = "batch_mat_mul_reduce_scatter_allto_all"
    ascendc_src_dir_ex = "batch_mat_mul_reduce_scatter_allto_all"
    ascendc_src_dir = "batch_mat_mul_reduce_scatter_allto_all"
    ascendc_src_file = "batch_mat_mul_reduce_scatter_allto_all.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator BatchMatMulReduceScatterAlltoAll, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "BatchMatMulReduceScatterAlltoAll"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, weight_in__, bias_in__], origin_outputs = [y_out_],\
                param_type_dynamic = False, mc2_ctx = ["mc2_context_0", "mc2_context_1"], param_type_list = ['required', 'required', 'optional', 'required'], init_value_list = [None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{"auto_sync": false}')

def op_select_format(x_in__, weight_in__, bias_in__=None, y_out_=None, group_ep="", group_tp="", ep_world_size=0, tp_world_size=0, y_shard_type=0, transpose_weight=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, bias_in__, y_out_, group_ep, group_tp, ep_world_size, tp_world_size, y_shard_type, transpose_weight)
    result = check_op_cap("op_select_format", "BatchMatMulReduceScatterAlltoAll", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, weight_in__, bias_in__=None, y_out_=None, group_ep="", group_tp="", ep_world_size=0, tp_world_size=0, y_shard_type=0, transpose_weight=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, bias_in__, y_out_, group_ep, group_tp, ep_world_size, tp_world_size, y_shard_type, transpose_weight)
    result = check_op_cap("get_op_specific_info", "BatchMatMulReduceScatterAlltoAll", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
