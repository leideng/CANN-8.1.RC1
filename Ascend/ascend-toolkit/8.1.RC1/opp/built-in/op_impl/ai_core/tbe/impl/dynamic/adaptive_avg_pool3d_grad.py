...
def _build_args(y_grad_in__, x_in__, x_grad_out_):
    __inputs__ = []
...
@tbe_register.register_operator("AdaptiveAvgPool3dGrad", trans_bool_to_s8=False)
...
def adaptive_avg_pool3d_grad(y_grad_in__, x_in__, x_grad_out_, kernel_name="adaptive_avg_pool3d_grad", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(y_grad_in__, x_in__, x_grad_out_)
    ...
    origin_func_name = "adaptive_avg_pool3d_grad"
    ascendc_src_dir_ex = "adaptive_avg_pool3d_grad"
    ascendc_src_dir = "adaptive_avg_pool3d_grad"
    ascendc_src_file = "adaptive_avg_pool3d_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator AdaptiveAvgPool3dGrad, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "AdaptiveAvgPool3dGrad"
...
def op_select_format(y_grad_in__, x_in__, x_grad_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(y_grad_in__, x_in__, x_grad_out_)
    result = check_op_cap("op_select_format", "AdaptiveAvgPool3dGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(y_grad_in__, x_in__, x_grad_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(y_grad_in__, x_in__, x_grad_out_)
    result = check_op_cap("get_op_specific_info", "AdaptiveAvgPool3dGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
