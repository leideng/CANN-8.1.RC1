...
#ifndef __ADD_LAYER_NORM_NORMAL_SPECIAL_REDUCE_H_
#define __ADD_LAYER_NORM_NORMAL_SPECIAL_REDUCE_H_
...
private:
  __aicore__ inline void CopyInAndAddBroadCast(int32_t rowCount, uint32_t elementCount, LocalTensor<T>& biasLocal, LocalTensor<T>& x1x2Local) {
    ...
    if constexpr (is_same<float, T>::value) {
      ...
      pipe_barrier(PIPE_V);
      for (int i = 0; i < rowCount; i++) {
        Add(addBufLocal[i * numLastDimAligned], x1x2Local.template ReinterpretCast<float>(), xBufLocal[i * numLastDimAligned], numLastDim);
      }
      pipe_barrier(PIPE_V);
    }
  ...
  __aicore__ inline void CopyIn(int32_t rowCount, uint32_t elementCount,
  ...
  __aicore__ inline void PrecisionCompute(int32_t nums,
    ...
    ReduceSumForSmallReduceDim(xLocalFp32, xLocalFp32, yLocalFp32, numLastDimAligned, numLastDim, tailCount, repeat, repStride);
  ...
  __aicore__ inline void PrecisionComputeBigN(int32_t nums,
                                              LocalTensor<float>& gammaLocal,
                                              LocalTensor<float>& betaLocal) {
#if __CCE_AICORE__ == 220
  ...
  __aicore__ inline void precisionComputeBigNTranspose(int32_t nums,
                                                       LocalTensor<float>& gammaLocal,
                                                       LocalTensor<float>& betaLocal) {
    event_t eventSV = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::S_V));
    event_t eventVS = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::V_S));
    ...
    InitVAForTranspose((__ubuf__ half *)transposeDstLocal.GetPhyAddr(), (__ubuf__ half *)transposeSrcLocal.GetPhyAddr());
    CCEBroadCastShort((__ubuf__ int16_t *)xLocalFp32.GetPhyAddr(), (__ubuf__ float *)xLocalFp32.GetPhyAddr(),
                      (__ubuf__ int16_t *)transposeDstLocal.GetPhyAddr(), (__ubuf__ int16_t *)transposeSrcLocal.GetPhyAddr(),
...
#if __CCE_AICORE__ == 220
  __aicore__ inline void PrecisionComputeBigNBrcb(int32_t nums,
                                                  LocalTensor<float>& gammaLocal,
                                                  LocalTensor<float>& betaLocal) {
    event_t eventSV = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::S_V));
    event_t eventVS = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::V_S));
    ...
    const uint32_t repeat = nums;    // repeat < 255 * 8 = 2040
    const uint8_t repStride = numLastDimAligned / FLOAT_BLOCK_ELEM;
    ReduceSumForSmallReduceDim(meanLocal, xLocalFp32, yLocalFp32, numLastDimAligned, numLastDim, tailCount, repeat, repStride);
    pipe_barrier(PIPE_V);
    ...
    Duplicate(yLocalFp32, ZERO, nums * ELEM_PER_REP_FP32);
    pipe_barrier(PIPE_V);
    ReduceSumForSmallReduceDim(rstdLocal, xLocalFp32, yLocalFp32, numLastDimAligned, numLastDim, tailCount, repeat, repStride);
    pipe_barrier(PIPE_V);
