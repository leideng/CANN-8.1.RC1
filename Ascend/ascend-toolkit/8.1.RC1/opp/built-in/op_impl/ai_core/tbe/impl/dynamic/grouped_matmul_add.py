...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'weight', 'group_list', 'y']
    output_names = ['y']
...
def _build_args(x_in__, weight_in__, group_list_in__, y_in__, y_out_, transpose_x, transpose_weight, group_type):
    __inputs__ = []
    for arg in [x_in__, weight_in__, group_list_in__, y_in__]:
        if arg != None:
...
@tbe_register.register_operator("GroupedMatmulAdd", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def grouped_matmul_add(x_in__, weight_in__, group_list_in__, y_in__, y_out_, transpose_x=True, transpose_weight=False, group_type=2, kernel_name="grouped_matmul_add", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, group_list_in__, y_in__, y_out_, transpose_x, transpose_weight, group_type)
    ...
    origin_func_name = "grouped_matmul_add"
    ascendc_src_dir_ex = "grouped_matmul_add"
    ascendc_src_dir = "grouped_matmul_add"
    ascendc_src_file = "grouped_matmul_add.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator GroupedMatmulAdd, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "GroupedMatmulAdd"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, weight_in__, group_list_in__, y_in__], origin_outputs = [y_out_],\
...
def op_select_format(x_in__, weight_in__, group_list_in__, y_in__, y_out_, transpose_x=True, transpose_weight=False, group_type=2, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, group_list_in__, y_in__, y_out_, transpose_x, transpose_weight, group_type)
    result = check_op_cap("op_select_format", "GroupedMatmulAdd", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, weight_in__, group_list_in__, y_in__, y_out_, transpose_x=True, transpose_weight=False, group_type=2, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, group_list_in__, y_in__, y_out_, transpose_x, transpose_weight, group_type)
    result = check_op_cap("get_op_specific_info", "GroupedMatmulAdd", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
