...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['grad_out', 'log_probs', 'targets', 'input_lengths', 'target_lengths', 'neg_log_likelihood', 'log_alpha']
    output_names = ['grad']
...
def _build_args(grad_out_in__, log_probs_in__, targets_in__, input_lengths_in__, target_lengths_in__, neg_log_likelihood_in__, log_alpha_in__, grad_out_, blank, reduction, zero_infinity):
    __inputs__ = []
    for arg in [grad_out_in__, log_probs_in__, targets_in__, input_lengths_in__, target_lengths_in__, neg_log_likelihood_in__, log_alpha_in__]:
        if arg != None:
...
@tbe_register.register_operator("CTCLossV3Grad", trans_bool_to_s8=False)
...
def ctc_loss_v3_grad(grad_out_in__, log_probs_in__, targets_in__, input_lengths_in__, target_lengths_in__, neg_log_likelihood_in__, log_alpha_in__, grad_out_, blank=0, reduction="mean", zero_infinity=False, kernel_name="ctc_loss_v3_grad", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(grad_out_in__, log_probs_in__, targets_in__, input_lengths_in__, target_lengths_in__, neg_log_likelihood_in__, log_alpha_in__, grad_out_, blank, reduction, zero_infinity)
    ...
    origin_func_name = "ctc_loss_v3_grad"
    ascendc_src_dir_ex = "ctc_loss_v3_grad"
    ascendc_src_dir = "c_t_c_loss_v3_grad"
    ascendc_src_file = "ctc_loss_v3_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator CTCLossV3Grad, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "CTCLossV3Grad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[grad_out_in__, log_probs_in__, targets_in__, input_lengths_in__, target_lengths_in__, neg_log_likelihood_in__, log_alpha_in__], origin_outputs = [grad_out_],\
...
def op_select_format(grad_out_in__, log_probs_in__, targets_in__, input_lengths_in__, target_lengths_in__, neg_log_likelihood_in__, log_alpha_in__, grad_out_, blank=0, reduction="mean", zero_infinity=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_out_in__, log_probs_in__, targets_in__, input_lengths_in__, target_lengths_in__, neg_log_likelihood_in__, log_alpha_in__, grad_out_, blank, reduction, zero_infinity)
    result = check_op_cap("op_select_format", "CTCLossV3Grad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(grad_out_in__, log_probs_in__, targets_in__, input_lengths_in__, target_lengths_in__, neg_log_likelihood_in__, log_alpha_in__, grad_out_, blank=0, reduction="mean", zero_infinity=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_out_in__, log_probs_in__, targets_in__, input_lengths_in__, target_lengths_in__, neg_log_likelihood_in__, log_alpha_in__, grad_out_, blank, reduction, zero_infinity)
    result = check_op_cap("get_op_specific_info", "CTCLossV3Grad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
