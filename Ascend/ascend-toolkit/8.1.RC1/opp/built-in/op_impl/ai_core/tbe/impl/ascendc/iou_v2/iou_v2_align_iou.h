...
/*!
 * \file iou_v2_align_iou.h
 * \brief
 */
#ifndef _IOU_V2_ALIGN_IOU_H
#define _IOU_V2_ALIGN_IOU_H
...
class KernelIouV2Align {
public:
    __aicore__ inline KernelIouV2Align() {}
    ...
    __aicore__ inline void Init(GM_ADDR bboxes, GM_ADDR gtboxes, GM_ADDR overlap,
        ...
        pipe.InitBuffer(tmpTensor, octaTileLen * 4);  // float大小为4
...
private:
    ...
    __aicore__ inline void Compute() {
        ...
        Sub(tmpBuffer, box1Local[doubleTileLen], box1Local, doubleTileLen);
        Sub(tmpBuffer[doubleTileLen], box2Local[doubleTileLen], box2Local, doubleTileLen);
        Adds(tmpBuffer, tmpBuffer, eps, quadTileLen);
        Mul(tmpBuffer, tmpBuffer, tmpBuffer[tileLen], tileLen);
        Mul(tmpBuffer[doubleTileLen], tmpBuffer[doubleTileLen], tmpBuffer[tripTileLen], tileLen);

        LocalTensor<float> outLocal = outQue.AllocTensor<float>();
        Add(outLocal, tmpBuffer, tmpBuffer[doubleTileLen], tileLen);
        ...
        Mul(tmpBuffer, tmpBuffer, tmpBuffer[tileLen], tileLen);
        Sub(outLocal, outLocal, tmpBuffer, tileLen);
