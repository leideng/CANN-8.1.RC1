...
class MoeTokenUnpermuteGradProbNotNone: protected MoeTokenUnpermuteGradBase<OriT, IdxT> {
public:
    __aicore__ inline MoeTokenUnpermuteGradProbNotNone(){};
    __aicore__ inline void Init(GM_ADDR permuted_tokens, GM_ADDR unpermuted_tokens_grad, GM_ADDR sorted_indices, GM_ADDR probs, GM_ADDR permuted_tokens_grad, GM_ADDR probs_grad,
...
protected:
    ...
    LocalTensor<OriT> inQueueProbLocal;
...
template <typename OriT, typename IdxT>
__aicore__ inline void MoeTokenUnpermuteGradProbNotNone<OriT, IdxT>::Init(GM_ADDR permuted_tokens, GM_ADDR unpermuted_tokens_grad, GM_ADDR sorted_indices, GM_ADDR probs, GM_ADDR permuted_tokens_grad, GM_ADDR probs_grad,
                                                                          const MoeTokenUnpermuteGradTilingData& tilingData) {
    MoeTokenUnpermuteGradBase<OriT, IdxT>::Init(permuted_tokens, unpermuted_tokens_grad, sorted_indices, probs, permuted_tokens_grad, probs_grad, tilingData);
    ...
    this->pipe.InitBuffer(inQueueProb, DOUBLE_BUFFER * this->indicesReserveNumAlign * this->inputTypeSize);
    this->pipe.InitBuffer(outQueueProbGrad, DOUBLE_BUFFER * this->indicesReserveNumAlign * this->inputTypeSize); // probGrad每indicesNumPerLoop搬出，预留indicesNumPerLoopAlign个元素的空间
...
template <typename OriT, typename IdxT>
__aicore__ inline void MoeTokenUnpermuteGradProbNotNone<OriT, IdxT>::Process() {
    ...
    for (int64_t indicesLoop = 0; indicesLoop < indicesLoopTimes; indicesLoop++) { // (token * topK) / indicesNumPerLoop 循环
        ...
        inQueueProbLocal = inQueueProb.GetWithOffset<OriT>(this->indicesReserveNumAlign, this->indicesReserveNumAlign * this->inputTypeSize * pingPongFlagProbs);
        copyParams.blockLen = (uint32_t)indicesNumLoop * this->inputTypeSize;
        DataCopyPad(inQueueProbLocal, this->probGm[loopOffset], copyParams, this->inputPadParams);
        ...
        WaitFlag<HardEvent::MTE3_V>(eventIdVMte3);
        probGradLocal = outQueueProbGrad.GetWithOffset<OriT>(this->indicesReserveNumAlign, this->indicesReserveNumAlign * this->inputTypeSize * pingPongFlagProbsGrad);
        if constexpr (IsSameType<OriT, float>::value) {
