...
@tbe_register.register_operator("Conv2DBackpropFilterV3", trans_bool_to_s8=False)
...
def conv2d_backprop_filter_v3(x_in__, filter_size_in__, out_backprop_in__, y_out_, strides, pads, dilations=[1,1,1,1], groups=1, data_format="NHWC", kernel_name="conv2d_backprop_filter_v3", impl_mode=""):
    ...
    origin_func_name = "conv2d_backprop_filter_v3"
    ascendc_src_dir_ex = "conv2_d_backprop_filter_v3"
    ascendc_src_dir = "conv2_d_backprop_filter_v3"
    ascendc_src_file = "conv2d_backprop_filter_v3.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator Conv2DBackpropFilterV3, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "Conv2DBackpropFilterV3"
...
def op_select_format(x_in__, filter_size_in__, out_backprop_in__, y_out_, strides, pads, dilations=[1,1,1,1], groups=1, data_format="NHWC", impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, filter_size_in__, out_backprop_in__, y_out_, strides, pads, dilations, groups, data_format)
    result = check_op_cap("op_select_format", "Conv2DBackpropFilterV3", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, filter_size_in__, out_backprop_in__, y_out_, strides, pads, dilations=[1,1,1,1], groups=1, data_format="NHWC", impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, filter_size_in__, out_backprop_in__, y_out_, strides, pads, dilations, groups, data_format)
    result = check_op_cap("get_op_specific_info", "Conv2DBackpropFilterV3", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
