...
def _build_args(y_grad_in__, repeats_in__, x_grad_out_, axis):
    __inputs__ = []
    for arg in [y_grad_in__, repeats_in__]:
        if arg != None:
...
@tbe_register.register_operator("RepeatInterleaveGrad", trans_bool_to_s8=False)
...
def repeat_interleave_grad(y_grad_in__, repeats_in__, x_grad_out_, axis=-1, kernel_name="repeat_interleave_grad", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(y_grad_in__, repeats_in__, x_grad_out_, axis)
    ...
    origin_func_name = "repeat_interleave_grad"
    ascendc_src_dir_ex = "repeat_interleave_grad"
    ascendc_src_dir = "repeat_interleave_grad"
    ascendc_src_file = "repeat_interleave_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator RepeatInterleaveGrad, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "RepeatInterleaveGrad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[y_grad_in__, repeats_in__], origin_outputs = [x_grad_out_],\
...
def op_select_format(y_grad_in__, repeats_in__, x_grad_out_, axis=-1, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(y_grad_in__, repeats_in__, x_grad_out_, axis)
    result = check_op_cap("op_select_format", "RepeatInterleaveGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(y_grad_in__, repeats_in__, x_grad_out_, axis=-1, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(y_grad_in__, repeats_in__, x_grad_out_, axis)
    result = check_op_cap("get_op_specific_info", "RepeatInterleaveGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
