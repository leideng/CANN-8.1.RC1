...
def _build_args(query_in__, key_in__, cos_in__, sin_in__, query_out_, key_out_, layout):
    __inputs__ = []
    for arg in [query_in__, key_in__, cos_in__, sin_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [query_out_, key_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("ApplyRotaryPosEmb", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def apply_rotary_pos_emb(query_in__, key_in__, cos_in__, sin_in__, query_out_, key_out_, layout=1, kernel_name="apply_rotary_pos_emb", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(query_in__, key_in__, cos_in__, sin_in__, query_out_, key_out_, layout)
    ...
    origin_func_name = "apply_rotary_pos_emb"
    ascendc_src_dir_ex = "apply_rotary_pos_emb"
    ascendc_src_dir = "apply_rotary_pos_emb"
    ascendc_src_file = "apply_rotary_pos_emb.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator ApplyRotaryPosEmb, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[query_in__, key_in__, cos_in__, sin_in__], origin_outputs = [query_out_, key_out_],\
...
def op_select_format(query_in__, key_in__, cos_in__, sin_in__, query_out_, key_out_, layout=1, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(query_in__, key_in__, cos_in__, sin_in__, query_out_, key_out_, layout)
    result = check_op_cap("op_select_format", "ApplyRotaryPosEmb", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(query_in__, key_in__, cos_in__, sin_in__, query_out_, key_out_, layout=1, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(query_in__, key_in__, cos_in__, sin_in__, query_out_, key_out_, layout)
    result = check_op_cap("get_op_specific_info", "ApplyRotaryPosEmb", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
