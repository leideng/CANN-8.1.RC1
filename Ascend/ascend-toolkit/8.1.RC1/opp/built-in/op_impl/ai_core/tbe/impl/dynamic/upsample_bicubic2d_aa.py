...
def _build_args(x_in__, y_out_, output_size, align_corners, scales_h, scales_w):
    __inputs__ = []
    for arg in [x_in__]:
        if arg != None:
...
@tbe_register.register_operator("UpsampleBicubic2dAA", trans_bool_to_s8=False)
...
def upsample_bicubic2d_aa(x_in__, y_out_, output_size, align_corners=False, scales_h=0, scales_w=0, kernel_name="upsample_bicubic2d_aa", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, output_size, align_corners, scales_h, scales_w)
    ...
    origin_func_name = "upsample_bicubic2d_aa"
    ascendc_src_dir_ex = "upsample_bicubic2d_aa"
    ascendc_src_dir = "upsample_bicubic2d_a_a"
    ascendc_src_file = "upsample_bicubic2d_aa.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator UpsampleBicubic2dAA, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "UpsampleBicubic2dAA"
...
def op_select_format(x_in__, y_out_, output_size, align_corners=False, scales_h=0, scales_w=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, output_size, align_corners, scales_h, scales_w)
    result = check_op_cap("op_select_format", "UpsampleBicubic2dAA", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, y_out_, output_size, align_corners=False, scales_h=0, scales_w=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, output_size, align_corners, scales_h, scales_w)
    result = check_op_cap("get_op_specific_info", "UpsampleBicubic2dAA", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
