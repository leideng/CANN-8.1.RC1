...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'atten_mask', 'relative_pos_bias']
    output_names = ['y']
...
def _build_args(x_in__, atten_mask_in__, relative_pos_bias_in__, y_out_, scale_value, inner_precision_mode):
    __inputs__ = []
    for arg in [x_in__, atten_mask_in__, relative_pos_bias_in__]:
        if arg != None:
    ...
    if inner_precision_mode != None:
        attr = {}
        attr["name"] = "inner_precision_mode"
        attr["dtype"] = "int"
        attr["value"] = inner_precision_mode
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("MaskedSoftmaxWithRelPosBias", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def masked_softmax_with_rel_pos_bias(x_in__, atten_mask_in__=None, relative_pos_bias_in__=None, y_out_=None, scale_value=1, inner_precision_mode=0, kernel_name="masked_softmax_with_rel_pos_bias", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, atten_mask_in__, relative_pos_bias_in__, y_out_, scale_value, inner_precision_mode)
    ...
    origin_func_name = "masked_softmax_with_rel_pos_bias"
    ascendc_src_dir_ex = "masked_softmax_with_rel_pos_bias"
    ascendc_src_dir = "masked_softmax_with_rel_pos_bias"
    ascendc_src_file = "masked_softmax_with_rel_pos_bias.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MaskedSoftmaxWithRelPosBias, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "MaskedSoftmaxWithRelPosBias"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, atten_mask_in__, relative_pos_bias_in__], origin_outputs = [y_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'optional', 'required', 'required'], init_value_list = [None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{"auto_sync": false}')

def op_select_format(x_in__, atten_mask_in__=None, relative_pos_bias_in__=None, y_out_=None, scale_value=1, inner_precision_mode=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, atten_mask_in__, relative_pos_bias_in__, y_out_, scale_value, inner_precision_mode)
    result = check_op_cap("op_select_format", "MaskedSoftmaxWithRelPosBias", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, atten_mask_in__=None, relative_pos_bias_in__=None, y_out_=None, scale_value=1, inner_precision_mode=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, atten_mask_in__, relative_pos_bias_in__, y_out_, scale_value, inner_precision_mode)
    result = check_op_cap("get_op_specific_info", "MaskedSoftmaxWithRelPosBias", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
