...
def _multi_core_on_hw(tik_inst, data_in, data_out, shape_in):
    ...
    in_ub = tik_inst.Tensor(data_in.dtype, (need_ub_size,), name="in_ub", scope=tbe_platform.scope_ubuf)
    out_ub = tik_inst.Tensor(data_in.dtype, (need_ub_size,), name="out_ub", scope=tbe_platform.scope_ubuf)
...
def hwc_2_chw(in_shape, in_dtype, kernel_name="hwc_2_chw"):
    ...
    data_in = tik_inst.Tensor(in_dtype, in_shape, tbe_platform.scope_gm, "data_in")
    data_out = tik_inst.Tensor(in_dtype, dst_shape, tbe_platform.scope_gm, "data_out")
