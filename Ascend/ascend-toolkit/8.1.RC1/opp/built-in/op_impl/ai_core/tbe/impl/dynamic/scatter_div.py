...
from impl.dynamic import scatter_common
from . import scatter_common


def scatter_div_tik(var, indices, updates, var_out, kernel_name="ScatterDiv"):
    obj = scatter_common.ScatterCommon(var, indices, updates, var_out, False, kernel_name, "vdiv")
    return obj.scatter_common_operator()


def scatter_div_dsl(var, indices, updates, var_out, kernel_name="ScatterDiv"):
    """
    scatter_div interface for dsl
    """
    check_list_var = ("float16", "float32")
    check_list_indices = ("int32", "int64")
    check_list_updates = ("float16", "float32")
...
def scatter_div(var, indices, updates, var_out, use_locking=False, kernel_name="scatter_div"):
    ...
    if not tbe_platform.api_check_support("tbe.dsl.vexp", "float32"):
        scatter_div_tik(var, indices, updates, var_out, kernel_name)
    else:
        scatter_div_dsl(var, indices, updates, var_out, kernel_name)
