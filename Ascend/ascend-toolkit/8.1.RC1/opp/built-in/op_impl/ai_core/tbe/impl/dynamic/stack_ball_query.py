...
def _build_args(xyz_in__, center_xyz_in__, xyz_batch_cnt_in__, center_xyz_batch_cnt_in__, idx_out_, max_radius, sample_num):
    __inputs__ = []
...
@tbe_register.register_operator("StackBallQuery", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def stack_ball_query(xyz_in__, center_xyz_in__, xyz_batch_cnt_in__, center_xyz_batch_cnt_in__, idx_out_, max_radius, sample_num, kernel_name="stack_ball_query", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(xyz_in__, center_xyz_in__, xyz_batch_cnt_in__, center_xyz_batch_cnt_in__, idx_out_, max_radius, sample_num)
    ...
    origin_func_name = "stack_ball_query"
    ascendc_src_dir_ex = "stack_ball_query"
    ascendc_src_dir = "stack_ball_query"
    ascendc_src_file = "stack_ball_query.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator StackBallQuery, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "StackBallQuery"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[xyz_in__, center_xyz_in__, xyz_batch_cnt_in__, center_xyz_batch_cnt_in__], origin_outputs = [idx_out_],\
...
def op_select_format(xyz_in__, center_xyz_in__, xyz_batch_cnt_in__, center_xyz_batch_cnt_in__, idx_out_, max_radius, sample_num, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(xyz_in__, center_xyz_in__, xyz_batch_cnt_in__, center_xyz_batch_cnt_in__, idx_out_, max_radius, sample_num)
    result = check_op_cap("op_select_format", "StackBallQuery", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(xyz_in__, center_xyz_in__, xyz_batch_cnt_in__, center_xyz_batch_cnt_in__, idx_out_, max_radius, sample_num, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(xyz_in__, center_xyz_in__, xyz_batch_cnt_in__, center_xyz_batch_cnt_in__, idx_out_, max_radius, sample_num)
    result = check_op_cap("get_op_specific_info", "StackBallQuery", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
