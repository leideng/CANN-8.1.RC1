...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['expanded_x', 'x1', 'x2', 'bias', 'scales', 'expanded_row_idx', 'expanded_expert_idx']
    output_names = ['y']
...
def _build_args(expanded_x_in__, x1_in__, x2_in__, bias_in__, scales_in__, expanded_row_idx_in__, expanded_expert_idx_in__, y_out_):
    __inputs__ = []
    for arg in [expanded_x_in__, x1_in__, x2_in__, bias_in__, scales_in__, expanded_row_idx_in__, expanded_expert_idx_in__]:
        if arg != None:
...
@tbe_register.register_operator("MoeFinalizeRouting", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.KERNEL_NAME)
def moe_finalize_routing(expanded_x_in__, x1_in__, x2_in__=None, bias_in__=None, scales_in__=None, expanded_row_idx_in__=None, expanded_expert_idx_in__=None, y_out_=None, kernel_name="moe_finalize_routing", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(expanded_x_in__, x1_in__, x2_in__, bias_in__, scales_in__, expanded_row_idx_in__, expanded_expert_idx_in__, y_out_)
    ...
    origin_func_name = "moe_finalize_routing"
    ascendc_src_dir_ex = "moe_finalize_routing"
    ascendc_src_dir = "moe_finalize_routing"
    ascendc_src_file = "moe_finalize_routing.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MoeFinalizeRouting, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "MoeFinalizeRouting"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[expanded_x_in__, x1_in__, x2_in__, bias_in__, scales_in__, expanded_row_idx_in__, expanded_expert_idx_in__], origin_outputs = [y_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'optional', 'required', 'required', 'required', 'required', 'required'], init_value_list = [None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{"auto_sync": false}')

def op_select_format(expanded_x_in__, x1_in__, x2_in__=None, bias_in__=None, scales_in__=None, expanded_row_idx_in__=None, expanded_expert_idx_in__=None, y_out_=None, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(expanded_x_in__, x1_in__, x2_in__, bias_in__, scales_in__, expanded_row_idx_in__, expanded_expert_idx_in__, y_out_)
    result = check_op_cap("op_select_format", "MoeFinalizeRouting", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(expanded_x_in__, x1_in__, x2_in__=None, bias_in__=None, scales_in__=None, expanded_row_idx_in__=None, expanded_expert_idx_in__=None, y_out_=None, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(expanded_x_in__, x1_in__, x2_in__, bias_in__, scales_in__, expanded_row_idx_in__, expanded_expert_idx_in__, y_out_)
    result = check_op_cap("get_op_specific_info", "MoeFinalizeRouting", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
