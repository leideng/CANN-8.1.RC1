...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['feeds', 'feeds_repeat_times']
    output_names = ['y']
...
def _build_args(feeds_in__, feeds_repeat_times_in__, y_out_, output_feeds_size):
    __inputs__ = []
    for arg in [feeds_in__, feeds_repeat_times_in__]:
        if arg != None:
    ...
    for arg in [y_out_]:
        if arg != None:
            ...
            __outputs__.append(arg)
    __attrs__ = []
    if output_feeds_size != None:
        attr = {}
        attr["name"] = "output_feeds_size"
        attr["dtype"] = "int"
        attr["value"] = output_feeds_size
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("FeedsRepeat", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def feeds_repeat(feeds_in__, feeds_repeat_times_in__, y_out_, output_feeds_size, kernel_name="feeds_repeat", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(feeds_in__, feeds_repeat_times_in__, y_out_, output_feeds_size)
    ...
    origin_func_name = "feeds_repeat"
    ascendc_src_dir_ex = "feeds_repeat"
    ascendc_src_dir = "feeds_repeat"
    ascendc_src_file = "feeds_repeat.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator FeedsRepeat, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "FeedsRepeat"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[feeds_in__, feeds_repeat_times_in__], origin_outputs = [y_out_],\
...
def op_select_format(feeds_in__, feeds_repeat_times_in__, y_out_, output_feeds_size, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(feeds_in__, feeds_repeat_times_in__, y_out_, output_feeds_size)
    result = check_op_cap("op_select_format", "FeedsRepeat", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(feeds_in__, feeds_repeat_times_in__, y_out_, output_feeds_size, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(feeds_in__, feeds_repeat_times_in__, y_out_, output_feeds_size)
    result = check_op_cap("get_op_specific_info", "FeedsRepeat", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
