...
@tbe_register.register_operator("UpsampleBilinear2dGrad", trans_bool_to_s8=False)
...
def upsample_bilinear2d_grad(grad_output_in__, grad_input_out_, output_size, input_size, align_corners=False, scales_h=0, scales_w=0, kernel_name="upsample_bilinear2d_grad", impl_mode=""):
    ...
    origin_func_name = "upsample_bilinear2d_grad"
    ascendc_src_dir_ex = "upsample_bilinear2d_grad"
    ascendc_src_dir = "upsample_bilinear2d_grad"
    ascendc_src_file = "upsample_bilinear2d_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator UpsampleBilinear2dGrad, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "UpsampleBilinear2dGrad"
...
def op_select_format(grad_output_in__, grad_input_out_, output_size, input_size, align_corners=False, scales_h=0, scales_w=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_output_in__, grad_input_out_, output_size, input_size, align_corners, scales_h, scales_w)
    result = check_op_cap("op_select_format", "UpsampleBilinear2dGrad", __inputs__, __outputs__, __attrs__)
...
def get_op_specific_info(grad_output_in__, grad_input_out_, output_size, input_size, align_corners=False, scales_h=0, scales_w=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_output_in__, grad_input_out_, output_size, input_size, align_corners, scales_h, scales_w)
    result = check_op_cap("get_op_specific_info", "UpsampleBilinear2dGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
