...
def sparse_apply_adagrad_v2_compute(var, accum, lr, epsilon, grad, indices, var_out, accum_out, use_locking=False,
                                    update_slots=True, kernel_name="sparse_apply_adagrad_v2"):
    """
    ...
    kernel_name: str
        kernel name, default value is "sparse_apply_adagrad_v2"

    Returns:
    -----------
    res: TVM tensor list
        the result of sparse_apply_adagrad_v2
    """
    ...
    adds = tbe.vadds(sqrt, epsilon[0])
    div = tbe.vdiv(grad, adds)
    muls = tbe.vmuls(div, lr[0])
...
@register_operator("SparseApplyAdagradV2", pattern="SparseApply")
...
def sparse_apply_adagrad_v2(var,
                            ...
                            use_locking=False,
                            update_slots=True,
                            kernel_name="sparse_apply_adagrad_v2"):
    """
    Adds sparse updates to the variable referenced by resource.
    ...
    for (ins_var, ins_accum, ins_grad, ins_indices) in ins:
        with tbe.compute():
            ...
            data_epsilon = tvm.placeholder((1,), lr.get("dtype"), "data_epsilon")
            ...
            res = sparse_apply_adagrad_v2_compute(data_var, data_accum, data_lr, data_epsilon, data_grad, data_indices,
                                                  var_out, accum_out, use_locking, update_slots, kernel_name)
            tensors.append([data_var, data_accum, data_lr, data_epsilon, data_grad, data_indices, res[0], res[1]])
        with tvm.target.cce():
