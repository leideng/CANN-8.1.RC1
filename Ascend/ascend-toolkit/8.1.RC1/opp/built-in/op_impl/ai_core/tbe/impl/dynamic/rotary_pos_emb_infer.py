...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['q', 'k', 'cos', 'sin', 'seqLen']
    output_names = ['outQ', 'outK']
...
def _build_args(q_in__, k_in__, cos_in__, sin_in__, seqLen_in__, outQ_out_, outK_out_, rotaryCoeff, cosFormat):
    __inputs__ = []
    for arg in [q_in__, k_in__, cos_in__, sin_in__, seqLen_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [outQ_out_, outK_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if rotaryCoeff != None:
        attr = {}
        attr["name"] = "rotaryCoeff"
        attr["dtype"] = "int"
        attr["value"] = rotaryCoeff
        __attrs__.append(attr)
    if cosFormat != None:
        attr = {}
        attr["name"] = "cosFormat"
        attr["dtype"] = "int"
        attr["value"] = cosFormat
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("RotaryPosEmbInfer", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def rotary_pos_emb_infer(q_in__, k_in__, cos_in__, sin_in__, seqLen_in__=None, outQ_out_=None, outK_out_=None, rotaryCoeff=4, cosFormat=0, kernel_name="rotary_pos_emb_infer", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(q_in__, k_in__, cos_in__, sin_in__, seqLen_in__, outQ_out_, outK_out_, rotaryCoeff, cosFormat)
    ...
    origin_func_name = "rotary_pos_emb_infer"
    ascendc_src_dir_ex = "rotary_pos_emb_infer"
    ascendc_src_dir = "rotary_pos_emb_infer"
    ascendc_src_file = "rotary_pos_emb_infer.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator RotaryPosEmbInfer, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "RotaryPosEmbInfer"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[q_in__, k_in__, cos_in__, sin_in__, seqLen_in__], origin_outputs = [outQ_out_, outK_out_],\
...
def op_select_format(q_in__, k_in__, cos_in__, sin_in__, seqLen_in__=None, outQ_out_=None, outK_out_=None, rotaryCoeff=4, cosFormat=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(q_in__, k_in__, cos_in__, sin_in__, seqLen_in__, outQ_out_, outK_out_, rotaryCoeff, cosFormat)
    result = check_op_cap("op_select_format", "RotaryPosEmbInfer", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(q_in__, k_in__, cos_in__, sin_in__, seqLen_in__=None, outQ_out_=None, outK_out_=None, rotaryCoeff=4, cosFormat=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(q_in__, k_in__, cos_in__, sin_in__, seqLen_in__, outQ_out_, outK_out_, rotaryCoeff, cosFormat)
    result = check_op_cap("get_op_specific_info", "RotaryPosEmbInfer", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
