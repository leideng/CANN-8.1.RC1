...
def _build_args(bboxes_in__, gtboxes_in__, overlap_out_, mode, eps, aligned):
    __inputs__ = []
    for arg in [bboxes_in__, gtboxes_in__]:
        if arg != None:
...
@tbe_register.register_operator("IouV2", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def iou_v2(bboxes_in__, gtboxes_in__, overlap_out_, mode="iou", eps=1, aligned=False, kernel_name="iou_v2", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(bboxes_in__, gtboxes_in__, overlap_out_, mode, eps, aligned)
    ...
    origin_func_name = "iou_v2"
    ascendc_src_dir_ex = "iou_v2"
    ascendc_src_dir = "iou_v2"
    ascendc_src_file = "iou_v2.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator IouV2, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "IouV2"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[bboxes_in__, gtboxes_in__], origin_outputs = [overlap_out_],\
...
def op_select_format(bboxes_in__, gtboxes_in__, overlap_out_, mode="iou", eps=1, aligned=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(bboxes_in__, gtboxes_in__, overlap_out_, mode, eps, aligned)
    result = check_op_cap("op_select_format", "IouV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(bboxes_in__, gtboxes_in__, overlap_out_, mode="iou", eps=1, aligned=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(bboxes_in__, gtboxes_in__, overlap_out_, mode, eps, aligned)
    result = check_op_cap("get_op_specific_info", "IouV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
