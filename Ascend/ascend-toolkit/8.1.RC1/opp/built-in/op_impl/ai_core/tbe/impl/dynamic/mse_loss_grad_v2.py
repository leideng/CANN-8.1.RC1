...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['predict', 'label', 'dout']
    output_names = ['y']
...
def _build_args(predict_in__, label_in__, dout_in__, y_out_, reduction):
    __inputs__ = []
    for arg in [predict_in__, label_in__, dout_in__]:
        if arg != None:
...
@tbe_register.register_operator("MseLossGradV2", trans_bool_to_s8=False)
...
def mse_loss_grad_v2(predict_in__, label_in__, dout_in__, y_out_, reduction="mean", kernel_name="mse_loss_grad_v2", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(predict_in__, label_in__, dout_in__, y_out_, reduction)
    ...
    origin_func_name = "mse_loss_grad_v2"
    ascendc_src_dir_ex = "mse_loss_grad_v2"
    ascendc_src_dir = "mse_loss_grad_v2"
    ascendc_src_file = "mse_loss_grad_v2.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MseLossGradV2, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "MseLossGradV2"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[predict_in__, label_in__, dout_in__], origin_outputs = [y_out_],\
...
def op_select_format(predict_in__, label_in__, dout_in__, y_out_, reduction="mean", impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(predict_in__, label_in__, dout_in__, y_out_, reduction)
    result = check_op_cap("op_select_format", "MseLossGradV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(predict_in__, label_in__, dout_in__, y_out_, reduction="mean", impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(predict_in__, label_in__, dout_in__, y_out_, reduction)
    result = check_op_cap("get_op_specific_info", "MseLossGradV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
