...
class KernelKvRmsNormRopeCacheQuantB16PABLKNZ {
 public:
  ...
  __aicore__ inline void Init(GM_ADDR kv, GM_ADDR gamma, GM_ADDR cos, GM_ADDR sin, GM_ADDR index,
                              GM_ADDR k_cache, GM_ADDR v_cache, GM_ADDR optional_k_rope, GM_ADDR optional_c_kv,
                              GM_ADDR kpe_scale, GM_ADDR ckv_scale) {
    ...
    indexGmNz.SetGlobalBuffer((__gm__ int64_t*)index);
    kCacheGm.SetGlobalBuffer((__gm__ int8_t*)k_cache);
    vCacheGm.SetGlobalBuffer((__gm__ int8_t*)v_cache);
    ...
    // init pipe
    pipe_->InitBuffer(inQueueGamma, 2, (2 * RMS_NORM_LENGTH + ROPE_LENGTH) * sizeof(float));  // 2*(2*512+64)*4/1024=8.5
    pipe_->InitBuffer(inQueueX, 2, ubFactor * RMS_NORM_LENGTH * sizeof(KV_DTYPE));  // 2*16*512*2/1024=32
    pipe_->InitBuffer(outQueue, 2, ubFactor * RMS_NORM_LENGTH * (sizeof(KV_DTYPE) + sizeof(int8_t)));  // 2*16*512*3/1024=48
  ...
  __aicore__ inline void Process() {
    ...
    for (int64_t loopIdx = 0; loopIdx < ubLoop; ++loopIdx) {
      ...
      LocalTensor<int8_t> quantOutLocal = outLocal.template ReinterpretCast<int8_t>()[ubFactor * ROPE_LENGTH * sizeof(KV_DTYPE)];
      RoPEWithQuant<KV_DTYPE, true, ROPE_LENGTH>(outLocal, quantOutLocal, ropeLocal, cosLocal, sinLocal, workspaceBuffer, localKpeScale, ubFactor);
      inQueueX.FreeTensor(ropeLocal);
      outQueue.EnQue(outLocal);
      outLocal = outQueue.DeQue<KV_DTYPE>();
      quantOutLocal = outLocal.template ReinterpretCast<int8_t>()[ubFactor * ROPE_LENGTH * sizeof(KV_DTYPE)];
      // Scatter Update kCache
      ScatterUpdateNZWithQuant<int8_t, KV_DTYPE, isPagedAttention, ROPE_LENGTH, D1_SIZE2_PA_BLK_NZ_QUANT, D0_SIZE32_PA_BLK_NZ_QUANT>(kCacheGm, kCacheGmNd, outLocal, quantOutLocal, startIdx, ubFactor);
      ...
      quantOutLocal = outLocal.template ReinterpretCast<int8_t>()[ubFactor * RMS_NORM_LENGTH * sizeof(KV_DTYPE)];
      RmsNormWithQuant<KV_DTYPE, RMS_NORM_LENGTH>(outLocal, quantOutLocal, xLocal, gammaLocalFp32, workspaceBuffer, localCkvScale, ubFactor);
      ...
      ScatterUpdateNZWithQuant<int8_t, KV_DTYPE, isPagedAttention, RMS_NORM_LENGTH, D1_SIZE16_PA_BLK_NZ_QUANT, D0_SIZE32_PA_BLK_NZ_QUANT>(vCacheGm, vCacheGmNd, outLocal, quantOutLocal, startIdx, ubFactor);
      outQueue.FreeTensor(outLocal);
    }
    if (ubTail > 0) {
      ...
      // Calc: RoPE
      LocalTensor<KV_DTYPE> outLocal = outQueue.AllocTensor<KV_DTYPE>();
      LocalTensor<int8_t> quantOutLocal = outLocal.template ReinterpretCast<int8_t>()[ubTail * ROPE_LENGTH * sizeof(KV_DTYPE)];
      RoPEWithQuant<KV_DTYPE, true, ROPE_LENGTH>(outLocal, quantOutLocal, ropeLocal, cosLocal, sinLocal, workspaceBuffer, localKpeScale, ubTail);
      inQueueX.FreeTensor(ropeLocal);
      outQueue.EnQue(outLocal);
      outLocal = outQueue.DeQue<KV_DTYPE>();
      // Scatter Update kCache
      ScatterUpdateNZWithQuant<int8_t, KV_DTYPE, isPagedAttention, ROPE_LENGTH, D1_SIZE2_PA_BLK_NZ_QUANT, D0_SIZE32_PA_BLK_NZ_QUANT>(kCacheGm, kCacheGmNd, outLocal, quantOutLocal, startIdx, ubTail);
      outQueue.FreeTensor(outLocal);
      ...
      // Calc: RmsNorm
      outLocal = outQueue.AllocTensor<KV_DTYPE>();
      quantOutLocal = outLocal.template ReinterpretCast<int8_t>()[ubTail * RMS_NORM_LENGTH * sizeof(KV_DTYPE)];
      RmsNormWithQuant<KV_DTYPE, RMS_NORM_LENGTH>(outLocal, quantOutLocal, xLocal, gammaLocalFp32, workspaceBuffer, localCkvScale, ubTail);
      inQueueX.FreeTensor(xLocal);
      outQueue.EnQue(outLocal);
      outLocal = outQueue.DeQue<KV_DTYPE>();
      quantOutLocal = outLocal.template ReinterpretCast<int8_t>()[ubTail * RMS_NORM_LENGTH * sizeof(KV_DTYPE)];
      ...
      // Scatter Update vCache
      ScatterUpdateNZWithQuant<int8_t, KV_DTYPE, isPagedAttention, RMS_NORM_LENGTH, D1_SIZE16_PA_BLK_NZ_QUANT, D0_SIZE32_PA_BLK_NZ_QUANT>(vCacheGm, vCacheGmNd, outLocal, quantOutLocal, startIdx, ubTail);
      outQueue.FreeTensor(outLocal);
    }
  ...
  __aicore__ inline void RoPEWithQuant(const LocalTensor<T>& outLocal, const LocalTensor<int8_t>& quantOutLocal,
                              const LocalTensor<T>& xLocal, const LocalTensor<T>& cosLocal, const LocalTensor<T>& sinLocal,
                              const LocalTensor<float>& wsLocal, const LocalTensor<float>& scaleLocal, int64_t rows) {
  ...
  __aicore__ inline void RoundFloat2Int8(const LocalTensor<int8_t>& dstTensor, const LocalTensor<float>& srcTensor, int32_t size) {
  ...
  template <typename T1, typename T2, bool isPA = false, int64_t headSize, int64_t D1, int64_t D0>
  __aicore__ inline void ScatterUpdateNZWithQuant(const GlobalTensor<T1>& dst, const GlobalTensor<T2>& dstNd,
                                         const LocalTensor<T2>& outLocal, const LocalTensor<T1>& quantOutLocal,
                                         int64_t startIdx, int64_t rows) {
    DataCopyExtParams copyParamsNz{static_cast<uint16_t>(D1), // 2
                                 static_cast<uint32_t>(D0 * sizeof(T1)),// 32
                                 0,
                                 static_cast<uint32_t>(tilingData_->blockSize * D0 * sizeof(T1)) - static_cast<uint32_t>(D0 * sizeof(T1)),//256*32*1-32*1
                                 0};
  ...
  __aicore__ inline void RmsNormWithQuant(const LocalTensor<T>& outLocal, const LocalTensor<int8_t>& quantOutLocal,
                                          const LocalTensor<T>& xLocal, const LocalTensor<float>& gammaLocal,
                                          const LocalTensor<float>& wsLocal, const LocalTensor<float>& scaleLocal,
                                          int64_t rows) {
    constexpr static int64_t NUM_EIGHT = 8;
 ...
 private:
  ...
  GlobalTensor<int8_t> kCacheGm, vCacheGm;
  GlobalTensor<float> kpeScaleGm, ckvScaleGm;
  GlobalTensor<int64_t> indexGmNz;
