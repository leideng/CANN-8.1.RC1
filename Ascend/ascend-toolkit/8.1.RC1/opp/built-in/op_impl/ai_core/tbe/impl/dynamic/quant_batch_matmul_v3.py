...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x1', 'x2', 'scale', 'offset', 'bias', 'pertoken_scale']
    output_names = ['y']
...
def _build_args(x1_in__, x2_in__, scale_in__, offset_in__, bias_in__, pertoken_scale_in__, y_out_, dtype, transpose_x1, transpose_x2):
    __inputs__ = []
    for arg in [x1_in__, x2_in__, scale_in__, offset_in__, bias_in__, pertoken_scale_in__]:
        if arg != None:
    ...
    if transpose_x1 != None:
        attr = {}
        attr["name"] = "transpose_x1"
        attr["dtype"] = "bool"
        attr["value"] = transpose_x1
        __attrs__.append(attr)
    if transpose_x2 != None:
        attr = {}
        attr["name"] = "transpose_x2"
        attr["dtype"] = "bool"
        attr["value"] = transpose_x2
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("QuantBatchMatmulV3", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def quant_batch_matmul_v3(x1_in__, x2_in__, scale_in__, offset_in__=None, bias_in__=None, pertoken_scale_in__=None, y_out_=None, dtype=0, transpose_x1=False, transpose_x2=False, kernel_name="quant_batch_matmul_v3", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, scale_in__, offset_in__, bias_in__, pertoken_scale_in__, y_out_, dtype, transpose_x1, transpose_x2)
    ...
    origin_func_name = "quant_batch_matmul_v3"
    ascendc_src_dir_ex = "quant_batch_matmul_v3"
    ascendc_src_dir = "quant_batch_matmul_v3"
    ascendc_src_file = "quant_batch_matmul_v3.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator QuantBatchMatmulV3, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x1_in__, x2_in__, scale_in__, offset_in__, bias_in__, pertoken_scale_in__], origin_outputs = [y_out_],\
...
def op_select_format(x1_in__, x2_in__, scale_in__, offset_in__=None, bias_in__=None, pertoken_scale_in__=None, y_out_=None, dtype=0, transpose_x1=False, transpose_x2=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, scale_in__, offset_in__, bias_in__, pertoken_scale_in__, y_out_, dtype, transpose_x1, transpose_x2)
    result = check_op_cap("op_select_format", "QuantBatchMatmulV3", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x1_in__, x2_in__, scale_in__, offset_in__=None, bias_in__=None, pertoken_scale_in__=None, y_out_=None, dtype=0, transpose_x1=False, transpose_x2=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, scale_in__, offset_in__, bias_in__, pertoken_scale_in__, y_out_, dtype, transpose_x1, transpose_x2)
    result = check_op_cap("get_op_specific_info", "QuantBatchMatmulV3", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
