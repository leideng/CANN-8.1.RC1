...
# 'pylint: disable=E0401,too-many-return-statements,too-many-arguments,huawei-too-many-arguments,invalid-name
import te.lang.cce
...
def check_supported(x, y, ksize, strides, padding="SAME", pads=(0, 0, 0, 0, 0, 0),
    ...
    if dilation_list not in default_dilations:
        return False, "[static] don't support if dilation is not 1"
...
def _is_covid_19(shape, ksize, strides, data_format):
    if ksize is None or strides is None or data_format is None:
        return False
...
def op_select_format(x, y, ksize, strides, padding="SAME",
    ...
    if tbe_platform.api_check_support("tik.vcopy"):
        input0_r = gen_param(classify="input0",
                             ...
                             format="NDC1HWC0, NDC1HWC0")
        output0_r = gen_param(classify="output0",
                              name="y",
                              datatype="float16, float32",
                              format="NDC1HWC0, NDC1HWC0")
    elif _is_covid_19(x.get("ori_shape"), ksize, strides, data_format):
        input0_r = gen_param(classify="input0",
                             name="x",
                             datatype="float16",
                             format="NDHWC")
