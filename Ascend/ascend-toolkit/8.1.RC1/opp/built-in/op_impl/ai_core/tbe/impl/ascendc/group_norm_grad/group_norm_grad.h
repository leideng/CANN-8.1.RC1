...
class GroupNormGrad {
 ...
 public:
  __aicore__ inline GroupNormGrad(GM_ADDR dy, GM_ADDR mean, GM_ADDR rstd, GM_ADDR x, GM_ADDR gamma, GM_ADDR dx,
                                  ...
                                  GroupNormGradTilingData* tiling_data) {
 ...
 private:
  ...
  __aicore__ inline void Compute_Mode_0(int32_t task_idx) {
    ...
    for (int32_t C_G_idx = 0; C_G_idx < this->C_G; C_G_idx++) {
      ...
      if (is_align == 0) {
        float temp1I = ReduceSum_template(temp_hxw_local, dy_local[C_G_idx * this->ele_num_per_channel], temp_hxw_local,
                                          this->ele_num_per_channel, true);
        temp_1_local.SetValue(C_G_idx, temp1I);
        event_t eventIDSToV = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::S_V));
        SetFlag<HardEvent::S_V>(eventIDSToV);
        WaitFlag<HardEvent::S_V>(eventIDSToV);
        Mul(temp_hxw_local, x_local[C_G_idx * this->ele_num_per_channel], dy_local[C_G_idx * this->ele_num_per_channel],
        ...
        float temp1I = 
            ReduceSum_template(temp_hxw_local, unalign_channel_dy, temp_hxw_local, this->ele_num_per_channel, true);
    ...
    if (dx_is_require) {
      ...
      for (int32_t C_G_idx = 0; C_G_idx < this->C_G; C_G_idx++) {
        ...
        if (is_align == 0) {
          ...
          Adds_Muls_template(x_local[offset], x_local[offset], (this->rstd_scalar * c1), (this->rstd_scalar * c2),
                              this->ele_num_per_channel);
          WaitFlag<HardEvent::S_V>(eventIDSToV);
  ...
  __aicore__ inline void Compute_Mode_1(int32_t task_idx) {
    ...
    if (dx_is_require) {
      ...
      for (int32_t C_G_idx = 0; C_G_idx < this->C_G; C_G_idx++) {
        ...
        Adds_Muls_template(x_local, x_local, (-1.0f * (this->mean_scalar * this->rstd_scalar * c2 - c1)),
  ...
  __aicore__ inline void Compute_Mode_2(int32_t task_idx) {
    ...
    LocalTensor<float> temp_hxw_local[2] = {temp_queue_HXW.AllocTensor<float>(), temp_queue_HXW.AllocTensor<float>()};
    ...
    for (int32_t C_G_idx = 0; C_G_idx < this->C_G / DOUBLE_BUFFER_; C_G_idx++) {
      ...
      for (int i = 0; i < DOUBLE_BUFFER_; i++) {
        temp1I[i] = 
          ReduceSum_template(temp_hxw_local[i], dy_local[i], temp_hxw_local[i], this->ele_num_per_channel, true);
      }
      for (int i = 0; i < DOUBLE_BUFFER_; i++) {
        Adds_Muls_template(x_local[i], x_local[i], (-1.0f * this->mean_scalar * this->rstd_scalar), 
                           this->rstd_scalar, this->ele_num_per_channel);
      }
      for (int i = 0; i < DOUBLE_BUFFER_; i++) {
      ...
      for (int i = 0; i < DOUBLE_BUFFER_; i++) {
        temp2I[i] =
          ReduceSum_template(temp_hxw_local[i], temp_hxw_local[i], temp_hxw_local[i], this->ele_num_per_channel, true);
      }
      for (int i = 0; i < DOUBLE_BUFFER_; i++) {
    ...
    if (C_G != restChannelStartIdx) {
      ...
      Adds_Muls_template(x_local[0], x_local[0], (-1.0f * this->mean_scalar * this->rstd_scalar),
                        this->rstd_scalar, this->ele_num_per_channel);
      Mul(temp_hxw_local[0], x_local[0], dy_local[0], this->ele_num_per_channel);
      temp2I[0] =
          ReduceSum_template(temp_hxw_local[0], temp_hxw_local[0], temp_hxw_local[0], this->ele_num_per_channel, true);
    ...
    if (dx_is_require) {
      ...
      for (int32_t C_G_idx = 0; C_G_idx < this->C_G / DOUBLE_BUFFER_; C_G_idx++) {
        ...
        for (int i = 0; i < DOUBLE_BUFFER_; i++) {
          ...
          Adds_Muls_template(x_local[i], x_local[i],
                             (-1.0f * (this->mean_scalar * this->rstd_scalar * c2 - c1)),
                             (this->rstd_scalar * c2), this->ele_num_per_channel);
      ...
      if (C_G != restChannelStartIdx) {
        ...
        Adds_Muls_template(x_local[0], x_local[0],
                           (-1.0f * (this->mean_scalar * this->rstd_scalar * c2 - c1)),
                           (this->rstd_scalar * c2), this->ele_num_per_channel);
  ...
  __aicore__ inline void Copyout_dgamma(float& corr2, float& temp2I, uint32_t offset, uint32_t process_num) {
    ...
    float adjustedSum = ReduceSum_template(temp_hxw_local, temp_hxw_local, temp_hxw_local, process_num, true) - corr2;
  ...
  __aicore__ inline void Compute_Mode_5(int32_t task_idx) {
    ...
    if (dx_is_require) {
      ...
      out_queue_dgamma.FreeTensor(temp_2_local);
      in_queue_gamma.FreeTensor(gamma_local);
      Adds_Muls_template(x_local, x_local, this->rstd_scalar * sumC1, 
                         this->rstd_scalar * sumC2, this->C_G);
      ...
      if (calCount > MASK) {
        for (int32_t start = MAX_LEN; start >= 1; start >>= 1) {
          ...
          if (calCount > innerOffset && firstAdd){
            Copy(dstLocal, srcLocal, MASK, start, {STRIDE_BLOCK, STRIDE_BLOCK, PROCESS_BLOCK, PROCESS_BLOCK});
            Add<float>(dstLocal, srcLocal[innerOffset], dstLocal, calCount - innerOffset);
            calCount = innerOffset;
            firstAdd = false;
          } else if (calCount > innerOffset){
            Add<float, false>(dstLocal, dstLocal[innerOffset], dstLocal, MASK, start ,
