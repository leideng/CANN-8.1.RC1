...
@tbe_register.register_operator("ForeachNorm", trans_bool_to_s8=False)
...
def foreach_norm(x_in__, scalar_in__, y_out_, kernel_name="foreach_norm", impl_mode=""):
    ...
    origin_func_name = "foreach_norm"
    ascendc_src_dir_ex = "foreach_norm"
    ascendc_src_dir = "foreach_norm"
    ascendc_src_file = "foreach_norm.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator ForeachNorm, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "ForeachNorm"
...
def op_select_format(x_in__, scalar_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, scalar_in__, y_out_)
    result = check_op_cap("op_select_format", "ForeachNorm", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, scalar_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, scalar_in__, y_out_)
    result = check_op_cap("get_op_specific_info", "ForeachNorm", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
