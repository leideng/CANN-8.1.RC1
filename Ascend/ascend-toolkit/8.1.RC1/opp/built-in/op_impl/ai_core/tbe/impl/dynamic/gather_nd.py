...
def check_supported(x_dict, indices_dict, y_dict, negative_index_support=False, kernel_name="GatherNd"):
    """
    Parameters
    ----------
...
def gather_nd_tik(x_dict, indices_dict, y_dict, kernel_name):
    """
    gather_nd interface for tik
    """
    obj = GatherNd(x_dict, indices_dict, y_dict, kernel_name)
    return obj.gather_nd_compute()


@register_operator_compute("GatherNd", op_mode="dynamic", support_fusion=True)
def gather_nd_compute(x, indices, y, batch_dim, negative_index_support, kernel_name, impl_mode):
    """
    algorithm: gather_nd
    Gather slices from into a Tensor with shape specified by .paramsindices
    ...
    """
    if impl_mode == "support_out_of_bound_index":
        support_out_of_bound_index = True
    else:
        support_out_of_bound_index = False

    res = tbe.gather_nd(x, indices, batch_dim, negative_index_support, support_out_of_bound_index)
    return res


def gather_nd_dsl(x, indices, y, negative_index_support, kernel_name, impl_mode):
    """
    gather_nd interface for dsl
    ...
    batch_dims = 0
    ins = classify([x, indices, batch_dims], "gather_nd")
    schedules, tensors = [], []
    for (params_input, indices_input, batch_dims_input) in ins:
        with tbe.compute():
            params_shape, indices_shape, batch_dim = shape_util.variable_shape([params_input, indices_input,
                                                                               batch_dims_input], "gather")
            params_tensor = tvm.placeholder(params_shape, name="params", dtype=x_dtype)
            indices_tensor = tvm.placeholder(indices_shape, name="indices", dtype=indices_dtype)
            res = gather_nd_compute(params_tensor, indices_tensor, y, batch_dim, negative_index_support, kernel_name,
                                    impl_mode)
            tensors.append([params_tensor, indices_tensor, res])
        with tvm.target.cce():
...
def gather_nd(x_dict, indices_dict, y_dict, negative_index_support=False, kernel_name="GatherNd", impl_mode=None):
    """
    gather_nd interface
    ...
    y_dict: output shape, dtype and range
    kernel_name: kernel name of gather_nd op
    impl_mode: implement mode, default as None
    ...
    if tbe_platform_adapter.api_check_support("tbe.dsl.vexp", "float32"):
        gather_nd_dsl(x_dict, indices_dict, y_dict, negative_index_support, kernel_name, impl_mode)
    else:
        gather_nd_tik(x_dict, indices_dict, y_dict, kernel_name)
