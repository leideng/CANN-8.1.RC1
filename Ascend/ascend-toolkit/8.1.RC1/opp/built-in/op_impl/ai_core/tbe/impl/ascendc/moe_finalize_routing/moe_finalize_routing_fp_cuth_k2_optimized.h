...
public:
    ...
    __aicore__ inline void Init(GM_ADDR expandedPermutedRows, GM_ADDR skip1, GM_ADDR skip2, GM_ADDR bias,
        GM_ADDR scales, GM_ADDR expandedSrcToDstRow, GM_ADDR expertForSourceRow, GM_ADDR out, GM_ADDR workspace,
...
template <typename T>
__aicore__ inline void MoeFinalizeRoutingFpCuthK2Optimized<T>::ParseTilingData(const MoeFinalizeRoutingTilingData *tilingData)
{
...
template <typename T>
__aicore__ inline void MoeFinalizeRoutingFpCuthK2Optimized<T>::Init(GM_ADDR expandedPermutedRows,GM_ADDR skip1,GM_ADDR skip2,
    GM_ADDR bias, GM_ADDR scales, GM_ADDR expandedSrcToDstRow, GM_ADDR expertForSourceRow, GM_ADDR out,
    GM_ADDR workspace, const MoeFinalizeRoutingTilingData* tilingData)
{
    ...
    pipe.InitBuffer(outQueue_, BUFFER_NUM_OPTIMIZED, normalCoreHandleNumPerLoop_ * AlignmentProcess(normalH_) * sizeof(T));
    ...
    int64_t bias, int64_t dataLen, bool isPadH, int64_t rightPaddingH)
{
    ...
    DataCopyPad(expertForSourceRowLocal, 
                gmExpertForSourceRow_[nLoopIdx / (cutNumH_ + 1) * curCoreHandleNumPerLoop_ * K_],
                copyParamsExpert, padParamsExpert);
    
    // ---------------------------- [Scales] -------------------------------
    DataCopyParams copyParamsScales{static_cast<uint16_t>(lineNumInCurrentLoop), 
                                    static_cast<uint16_t>(K_ * sizeof(T)), 0, 0};
    ...
    DataCopyPad(skip1Skip2ScalesLocal[normalCoreHandleNumPerLoop_ * AlignmentProcess(normalH_)], 
                gmScales_[nLoopIdx / (cutNumH_ + 1) * curCoreHandleNumPerLoop_ * K_], 
    ...
    DataCopyParams copyParamsSkip{static_cast<uint16_t>(lineNumInCurrentLoop), 
                                  ...
                                  static_cast<uint16_t>((H_ - dataLen) * sizeof(T)), 
                                  0};
    ...
    int64_t bias, int64_t dataLen,bool isPadH, int64_t rightPaddingH)
{
    ...
    for (int64_t i = 0; i < lineNumInCurrentLoop; i++) {
        ...
        for (int64_t j = 0; j < K_ / PARALLEL_NUM; j++) {
            ...
            int64_t expandedSrcToDstRowIndexDb0 = baseRowLine + i + 
            ...
            int64_t expandedSrcToDstRowIndexDb1 = baseRowLine + i + 
            ...
            int64_t biasIndexDb1 = expertForSourceRowLocal.GetValue(i * Int32AlignmentProcess(K_) + 
                                                                    PARALLEL_NUM * j + 1);
