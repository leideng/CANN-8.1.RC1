...
def get_matmul_tensor(x, h, c, w, b, build_list, tensor_list, scope_list, operation_list, is_hisi_es, is_milan):
    """
    get_matmul_tensor
    """
    shape_x = x.get("shape")
    ...
    for t in symbol:
        ...
        if is_milan is False:
            ...
            # copy matmul result from l0c to ub
            matmul_ub_tmp = tvm.compute(shape_h,
                                        lambda *i: tensor_matmul_l0c_tmp(*i),
                                        name="tensor_matmul_ub_" + t)
            tensor_list["tensor_matmul_ub_" + t] = matmul_ub_tmp
            scope_list["tensor_matmul_ub_" + t] = tbe_platform.scope_ubuf
            operation_list["tensor_matmul_ub_" + t] = "dma_copy"

            # Calculate matmul + bias
            gate_ub_tmp = tvm.compute(shape_h,
                                    lambda i0, i1, i2, i3:
                                    tensor_list["tensor_matmul_ub_" + t][i0, i1, i2, i3] +
                                    tensor_b_ub_true_tmp[i0, i3],
                                    name=t+"_ub")
            tensor_list[t+"_ub"] = gate_ub_tmp
            scope_list[t+"_ub"] = tbe_platform.scope_ubuf
            operation_list[t+"_ub"] = "vector_add"
...
def basic_lstm_cell_compute(x, h, c, w, b, forget_bias, product_info):
    ...
    get_matmul_tensor(x, h, c, w, b, build_list, tensor_list, scope_list, operation_list,
                      product_info["hisi_es"], product_info["milan"])
...
def basic_lstm_cell_schedule(tensor_list, scope_list,
                             ...
                             tilling_info, unreused_flag, is_milan, kernel_name):
    """
...
def basic_lstm_cell(x, h, c, w, b, mask, ct, ht, it, jt, ft, ot, tanhct,
    ...
    if tbe_product == "Ascend910":
        ...
        is_cloud = True
    elif tbe_product in ("Ascend910B", "Ascend910_93"):
        is_cloud = True
        is_milan = True
    ...
    basic_lstm_cell_schedule(tensor_list, scope_list, operation_list, build_list,
                             tilling_info, unreused_flag, product_info["milan"], kernel_name)
