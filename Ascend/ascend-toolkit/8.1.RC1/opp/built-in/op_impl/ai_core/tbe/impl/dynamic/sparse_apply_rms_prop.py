...
def sparse_apply_rms_prop_compute(var,
                                  ms,
                                  mom,
                                  lr,
                                  rho,
    ...
    ms_tmp = tbe.vmuls(ms_gather, rho[0])
    grad_tmp = tbe.vmuls(grad, 1.0 - rho[0])
    grad_tmp = tbe.vmul(grad, grad_tmp)
    ms_out = tbe.vadd(ms_tmp, grad_tmp)

    # `mom_out = mom * momentum + (ms_out + epsilon).rsqrt() * lr * grad`
    mom_tmp = tbe.vmuls(mom_gather, momentum[0])
    grad_tmp = tbe.vmuls(grad, lr[0])
    ms_out_tmp = tbe.vadds(ms_out, epsilon[0])
...
@register_operator("SparseApplyRMSProp", pattern="SparseApply")
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT,
                            para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT,
                            para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT,
                            para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT,
                            para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def sparse_apply_rms_prop(var,
                          ms,
                          mom,
                          lr,
                          rho,
    ...
    use_locking: bool
        not used in this compute
    kernel_name: str
        kernel name, default value is "sparse_apply_rms_prop"
    ...
    for (ins_var, ins_ms, ins_mom, ins_grad, ins_indices) in ins:
        with tbe.compute():
            ...
            data_rho = tvm.placeholder((1,), dtype, "rho")
            data_momentum = tvm.placeholder((1,), dtype, "momentum")
            data_epsilon = tvm.placeholder((1,), dtype, "epsilon")
            data_grad = tvm.placeholder(grad_shape, dtype, "data_grad")
            data_indices = tvm.placeholder(indices_shape, dtype_idx, "data_indices")
            res = sparse_apply_rms_prop_compute(data_var, data_ms, data_mom, data_lr,
                                                data_rho, data_momentum, data_epsilon,
                                                data_grad, data_indices, out_var, out_ms,
                                                out_mom, use_locking, kernel_name)
            tensors.append([data_var, data_ms, data_mom, data_lr, data_rho, data_momentum, data_epsilon,
                            data_grad, data_indices, res[0], res[1], res[2]])
        with tvm.target.cce():
