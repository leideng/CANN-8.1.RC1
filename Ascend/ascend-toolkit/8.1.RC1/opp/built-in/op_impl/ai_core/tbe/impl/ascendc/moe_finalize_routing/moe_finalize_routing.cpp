...
extern "C" __global__ __aicore__ void moe_finalize_routing(GM_ADDR expandedPermutedRows, GM_ADDR skip1,
                                                           GM_ADDR skip2, GM_ADDR bias,
                                                           GM_ADDR scales, GM_ADDR expandedSrcToDstRow,
    ...
    if (TILING_KEY_IS(20000)) {
        ...
        op.Init(expandedPermutedRows, skip1, skip2, bias, scales, expandedSrcToDstRow,
                expertForSourceRow, out, userWS, &tilingData);
        op.Process();
