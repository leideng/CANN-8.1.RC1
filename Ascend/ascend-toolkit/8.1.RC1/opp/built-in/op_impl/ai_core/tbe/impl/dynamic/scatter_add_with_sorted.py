...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['var', 'value', 'sorted_index', 'pos']
    output_names = ['var']
...
def _build_args(var_in__, value_in__, sorted_index_in__, pos_in__, var_out_, reduction):
    __inputs__ = []
    for arg in [var_in__, value_in__, sorted_index_in__, pos_in__]:
        if arg != None:
...
@tbe_register.register_operator("ScatterAddWithSorted", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_STR, para_check.KERNEL_NAME)
def scatter_add_with_sorted(var_in__, value_in__, sorted_index_in__, pos_in__=None, var_out_=None, reduction="add", kernel_name="scatter_add_with_sorted", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(var_in__, value_in__, sorted_index_in__, pos_in__, var_out_, reduction)
    ...
    origin_func_name = "scatter_add_with_sorted"
    ascendc_src_dir_ex = "scatter_add_with_sorted"
    ascendc_src_dir = "scatter_add_with_sorted"
    ascendc_src_file = "scatter_add_with_sorted.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator ScatterAddWithSorted, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "ScatterAddWithSorted"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[var_in__, value_in__, sorted_index_in__, pos_in__], origin_outputs = [var_out_],\
...
def op_select_format(var_in__, value_in__, sorted_index_in__, pos_in__=None, var_out_=None, reduction="add", impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(var_in__, value_in__, sorted_index_in__, pos_in__, var_out_, reduction)
    result = check_op_cap("op_select_format", "ScatterAddWithSorted", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(var_in__, value_in__, sorted_index_in__, pos_in__=None, var_out_=None, reduction="add", impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(var_in__, value_in__, sorted_index_in__, pos_in__, var_out_, reduction)
    result = check_op_cap("get_op_specific_info", "ScatterAddWithSorted", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
