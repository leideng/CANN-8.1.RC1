...
def sparse_apply_adadelta_compute(var, accum, accum_update, lr, rho, epsilon, grad, indices, var_out, accum_out,
                                  accum_update_out, use_locking=False,
                                  kernel_name="sparse_apply_adadelta"):
    ...
    lhs = tbe.vadds(accum_update_gathered, epsilon[0])
    lhs = tbe.vsqrt(lhs)
    rhs = tbe.vadds(accum_out, epsilon[0])
    rhs = tbe.vsqrt(rhs)
...
@register_operator("SparseApplyAdadelta", pattern="SparseApply")
...
def sparse_apply_adadelta(var,
    ...
    kernel_name: str
        kernel name, default value is "sparse_apply_adadelta"
    ...
    for (ins_var, ins_accum, ins_accum_update, ins_grad, ins_indices) in ins:
        with tbe.compute():
            ...
            epsilon = tvm.placeholder((1,), epsilon.get("dtype"), "epsilon")
            res = sparse_apply_adadelta_compute(data_var, data_accum, data_accum_update, lr, rho, epsilon, data_grad,
                                                data_indices, var_out, accum_out, accum_update_out,
                                                use_locking, kernel_name)
            tensors.append([data_var, data_accum, data_accum_update, lr, rho, epsilon, data_grad, data_indices,
                            res[0], res[1], res[2]])
        with tvm.target.cce():
