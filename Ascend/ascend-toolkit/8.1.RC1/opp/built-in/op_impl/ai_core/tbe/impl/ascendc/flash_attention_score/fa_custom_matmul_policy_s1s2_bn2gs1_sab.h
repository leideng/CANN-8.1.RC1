...
/*!
 * \file fa_custom_matmul_policy_s1s2_bn2gs1_sab.h
 * \brief
 */

#ifndef FA_CUSTOM_MATMUL_POLICY_S1S2_BN2GS1_SAB_H
#define FA_CUSTOM_MATMUL_POLICY_S1S2_BN2GS1_SAB_H
...
public:
  ...
  inline __aicore__ FaCopyCubeIn() = default;
  inline __aicore__ ~FaCopyCubeIn() = default;
  ...
  __aicore__ inline LocalTensor<TransT> LoadFullDataA(int32_t curRow, int32_t curCol,
    int32_t tileHeight, int32_t tileWidth, uint8_t l1Index, int32_t posL1, int32_t baseBlockSize)
  {
  ...
  __aicore__ inline LocalTensor<TransT> LoadPartDataA(int32_t curRow, int32_t curCol,
    int32_t tileHeight, int32_t tileWidth, uint8_t l1Index, int32_t posL1, int32_t baseBlockSize)
  {
    MATMUL_MODULE(Context)->l1IndexA = l1Index;
    ...
    //一次搬运两份，缓存个数加2
    l1Global[l1Index].cacheSize += CACHE_SIZE;
  ...
  __aicore__ inline LocalTensor<TransT> LoadFullDataB(int32_t curRow, int32_t curCol,
    int32_t tileHeight, int32_t tileWidth, uint8_t l1Index, int32_t posL1, int32_t baseBlockSize)
  {
    ...
    // k方向一次搬运两份
    if (MATMUL_MODULE(Context)->mmIdx == MM1_INDEX) {
      MATMUL_MODULE(DataCopyUtils)->CopyTileToCube(MATMUL_MODULE(Context)->CurrentTensorB, curRow, curCol,
                                                   singleHeight_, tileWidth);
    } else {
      GlobalTensor<TransT> bGlobal;
      bGlobal.SetGlobalBuffer(srcGlobalAddr_ + curRow * tileHeight * orgWidth_);
      Nd2NzParams nd2nzParams;
      nd2nzParams.ndNum = NUM_2;
      nd2nzParams.nValue = tileHeight;
      nd2nzParams.dValue = tileWidth;
      nd2nzParams.srcNdMatrixStride = tileHeight * tileWidth;
      nd2nzParams.srcDValue = orgWidth_;
      nd2nzParams.dstNzC0Stride = (tileHeight + BLOCK_CEILDIV_NUM) >> NUM_4 << NUM_4; // 向上16对齐
      nd2nzParams.dstNzNStride = 1;
      nd2nzParams.dstNzMatrixStride = tileHeight * ((tileWidth + BLOCK_CEILDIV_NUM) >> NUM_4 << NUM_4);
      DataCopy(MATMUL_MODULE(Context)->CurrentTensorB, bGlobal, nd2nzParams);
    }
                                                 ...
                                                 int32_t tileHeight, int32_t tileWidth)
  {
    ...
    if constexpr (INPUT_TYPE::TAG == InputTypeTag::A) {
      ...
      if (MATMUL_MODULE(Context)->mmIdx == MM2_INDEX) {
    ...
    if constexpr (INPUT_TYPE::TAG == InputTypeTag::B) {
      ...
      MATMUL_MODULE(Context)->needFreeA = MATMUL_MODULE(Context)->isLastBaseBlockA &&
                                          MATMUL_MODULE(Context)->isLastBaseBlockB;
      if (MATMUL_MODULE(Context)->mmIdx == MM2_INDEX && MATMUL_MODULE(Context)->isBufferFullA) {
...
private:
  ...
  int64_t singleHeight_;
  int64_t singleWidth_;
  int64_t orgWidth_;
};
...
class FaMatmulPolicyUnsplitK : public MatmulPolicy<MM_CFG, IMPL, A_TYPE, B_TYPE, C_TYPE, BIAS_TYPE>
{
public:
    using Context = FaContext<IMPL, MatmulInputAType<A_TYPE, typename A_TYPE::T>, MM_CFG>;
    using CopyCubeInA = FaCopyCubeIn<IMPL, MatmulInputAType<A_TYPE, typename A_TYPE::T>, MM_CFG>;
    using CopyCubeInB = FaCopyCubeIn<IMPL, MatmulInputBType<B_TYPE, typename A_TYPE::T>, MM_CFG>;
    using CubeInBufferA = FaCubeInBuffer<IMPL, MatmulInputAType<A_TYPE, typename A_TYPE::T>, MM_CFG>;
    using CubeInBufferB = FaCubeInBuffer<IMPL, MatmulInputBType<B_TYPE, typename A_TYPE::T>, MM_CFG>;
};
}
}
}

#endif // FA_CUSTOM_MATMUL_POLICY_S1S2_BN2GS1_SAB_H