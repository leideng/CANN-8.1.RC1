...
def _check_params(x, kernel_name):
    """
...
def _reform_compute_generate(tensor, in_shape, out_shape, scale_val, c1_index):
    ...
    def lambda_func(*indice):
        ...
        for i in range(n_dim):
            if i == c1_index:
                new_indice[i] = (indice[c1_index] * out_shape[c0_index]) // in_shape[c0_index]
            elif i == c0_index:
                new_indice[i] = (indice[c1_index] * out_shape[c0_index]) % in_shape[c0_index] + indice[c0_index]
            else:
...
@register_operator_compute("AscendAntiQuant", op_mode="dynamic", support_fusion=True)
def ascend_anti_quant_compute(x, y, scale, offset, dtype=1, sqrt_mode=False, kernel_name="ascend_anti_quant"):
    ...
    if y_format in ("NC1HWC0", "FRACTAL_NZ"):
        ...
        offset_ub = tbe.vadds(cast_f16_ub, offset)
        if sqrt_mode:
            scale_sqrt_ub = tbe.vmuls(offset_ub, scale)
            res = tbe.vmuls(scale_sqrt_ub, scale)
        else:
            res = tbe.vmuls(offset_ub, scale)
...
# 'pylint: disable=too-many-arguments,invalid-name,unused-argument
# 'pylint: disable=huawei-too-many-arguments
@register_operator("AscendAntiQuant")
...
def ascend_anti_quant(x, y, scale, offset, dtype=1, sqrt_mode=False, kernel_name="ascend_anti_quant"):
    ...
    None
    """
    _check_params(x, kernel_name)
    input_dtype = x.get("dtype").lower()
    x_format = x.get("format")
    if x_format in ("NC1HWC0", "FRACTAL_NZ"):
        ins = classify([x], OpPatternMode.ASCEND_ANTI_QUANT)
    else:
    ...
    for (_x,) in ins:
        with tbe.compute():
            if x_format in ("NC1HWC0", "FRACTAL_NZ"):
                input_shape = shape_util.variable_shape([_x], OpPatternMode.ASCEND_ANTI_QUANT)
            else:
                input_shape = shape_util.variable_shape([_x], OpPatternMode.ELEWISE)
            input_x = tvm.placeholder(input_shape[0], name="anti_input_x", dtype=input_dtype)
