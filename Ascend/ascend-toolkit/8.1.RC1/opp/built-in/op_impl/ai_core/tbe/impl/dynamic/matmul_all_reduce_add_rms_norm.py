...
def _build_args(x1_in__, x2_in__, bias_in__, residual_in__, gamma_in__, antiquant_scale_in__, antiquant_offset_in__, dequant_scale_in__, y_out_, norm_out_out_, group, reduce_op, is_trans_a, is_trans_b, comm_turn, antiquant_group_size, epsilon):
    __inputs__ = []
    for arg in [x1_in__, x2_in__, bias_in__, residual_in__, gamma_in__, antiquant_scale_in__, antiquant_offset_in__, dequant_scale_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [y_out_, norm_out_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
def matmul_all_reduce_add_rms_norm(x1_in__, x2_in__, bias_in__=None, residual_in__=None, gamma_in__=None, antiquant_scale_in__=None, antiquant_offset_in__=None, dequant_scale_in__=None, y_out_=None, norm_out_out_=None, group="", reduce_op="sum", is_trans_a=False, is_trans_b=False, comm_turn=0, antiquant_group_size=0, epsilon=1e-06, kernel_name="matmul_all_reduce_add_rms_norm", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, bias_in__, residual_in__, gamma_in__, antiquant_scale_in__, antiquant_offset_in__, dequant_scale_in__, y_out_, norm_out_out_, group, reduce_op, is_trans_a, is_trans_b, comm_turn, antiquant_group_size, epsilon)
    ...
    origin_func_name = "matmul_all_reduce_add_rms_norm"
    ascendc_src_dir_ex = "matmul_all_reduce_add_rms_norm"
    ascendc_src_dir = "matmul_all_reduce_add_rms_norm"
    ascendc_src_file = "matmul_all_reduce_add_rms_norm.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MatmulAllReduceAddRmsNorm, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x1_in__, x2_in__, bias_in__, residual_in__, gamma_in__, antiquant_scale_in__, antiquant_offset_in__, dequant_scale_in__], origin_outputs = [y_out_, norm_out_out_],\
