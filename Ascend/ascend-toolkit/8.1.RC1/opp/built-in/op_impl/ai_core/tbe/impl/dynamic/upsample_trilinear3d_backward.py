...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['grad_output']
    output_names = ['grad_input']
...
def _build_args(grad_output_in__, grad_input_out_, output_size, input_size, align_corners, scale_d, scale_h, scale_w):
    __inputs__ = []
    for arg in [grad_output_in__]:
        if arg != None:
    ...
    if output_size != None:
        attr = {}
        attr["name"] = "output_size"
        attr["dtype"] = "list_int"
        attr["value"] = output_size
        __attrs__.append(attr)
    if input_size != None:
        attr = {}
        attr["name"] = "input_size"
        attr["dtype"] = "list_int"
        attr["value"] = input_size
        __attrs__.append(attr)
...
@tbe_register.register_operator("UpsampleTrilinear3dBackward", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def upsample_trilinear3d_backward(grad_output_in__, grad_input_out_, output_size, input_size, align_corners=False, scale_d=0, scale_h=0, scale_w=0, kernel_name="upsample_trilinear3d_backward", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(grad_output_in__, grad_input_out_, output_size, input_size, align_corners, scale_d, scale_h, scale_w)
    ...
    origin_func_name = "upsample_trilinear3d_backward"
    ascendc_src_dir_ex = "upsample_trilinear3d_backward"
    ascendc_src_dir = "upsample_trilinear3d_backward"
    ascendc_src_file = "upsample_trilinear3d_backward.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator UpsampleTrilinear3dBackward, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "UpsampleTrilinear3dBackward"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[grad_output_in__], origin_outputs = [grad_input_out_],\
...
def op_select_format(grad_output_in__, grad_input_out_, output_size, input_size, align_corners=False, scale_d=0, scale_h=0, scale_w=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_output_in__, grad_input_out_, output_size, input_size, align_corners, scale_d, scale_h, scale_w)
    result = check_op_cap("op_select_format", "UpsampleTrilinear3dBackward", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(grad_output_in__, grad_input_out_, output_size, input_size, align_corners=False, scale_d=0, scale_h=0, scale_w=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_output_in__, grad_input_out_, output_size, input_size, align_corners, scale_d, scale_h, scale_w)
    result = check_op_cap("get_op_specific_info", "UpsampleTrilinear3dBackward", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
