...
def op_select_format(x, filter, y, block_size, data_format, kernel_name="space_to_depth"):
    ...
    if filter is not None:
        datatype = "float16, float, int8, uint8, int16, uint16, int32," \
                   "uint32, uint64, int64, bfloat16"
        input_format = "NC1HWC0, NHWC, NHWC, NHWC, NHWC, NHWC, NHWC, NHWC, NHWC, NHWC, NHWC"
    else:
        datatype = "float16, float, int8, uint8, int16, uint16, int32," \
                   "uint32, uint64, int64, bfloat16"
        input_format = "NHWC, NHWC, NHWC, NHWC, NHWC, NHWC, NHWC, NHWC, NHWC, NHWC, NHWC"
    input0 = gen_param(
        classify="input0",
        name="x",
        datatype=datatype,
...
class SpaceToDepth(SpaceToDepthBase):
    ...
    def get_buf_info(self):
        ...
        """
        size_for_tail = 32
        all_ub_buffer = tbe_platform.get_soc_spec(tbe_platform.UB_SIZE) // self.dtype_size - size_for_tail
    ...
    def space_to_depth_compute(self):
        ...
        block_dim = tbe_platform.get_soc_spec(
