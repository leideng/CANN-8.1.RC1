...
@tbe_register.register_operator("ForeachReciprocal", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.DYNAMIC_INPUT, para_check.DYNAMIC_OUTPUT, para_check.KERNEL_NAME)
def foreach_reciprocal(x_in__, y_out_, kernel_name="foreach_reciprocal", impl_mode=""):
    ...
    origin_func_name = "foreach_reciprocal"
    ascendc_src_dir_ex = "foreach_reciprocal"
    ascendc_src_dir = "foreach_reciprocal"
    ascendc_src_file = "foreach_reciprocal.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator ForeachReciprocal, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "ForeachReciprocal"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__], origin_outputs = [y_out_],\
                param_type_dynamic = True, mc2_ctx = [], param_type_list = ['dynamic', 'dynamic'], init_value_list = [None],\
...
def op_select_format(x_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_)
    result = check_op_cap("op_select_format", "ForeachReciprocal", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_)
    result = check_op_cap("get_op_specific_info", "ForeachReciprocal", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
