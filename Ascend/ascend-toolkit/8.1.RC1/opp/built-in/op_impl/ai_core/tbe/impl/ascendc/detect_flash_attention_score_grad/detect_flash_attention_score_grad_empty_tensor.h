...
/*!
 * \file detect_flash_attention_score_grad_empty_tensor.h
 * \brief
 */

#ifndef DETECT_FLASH_ATTENTION_SCORE_GRAD_EMPTY_TENSOR_H_
#define DETECT_FLASH_ATTENTION_SCORE_GRAD_EMPTY_TENSOR_H_
...
public:
    ...
    __aicore__ inline void Init(__gm__ uint8_t *dq, __gm__ uint8_t *dk, __gm__ uint8_t *dv, __gm__ uint8_t *dpse,
                                const DetectFlashAttentionScoreGradTilingData *__restrict tilingData);
    __aicore__ inline void Process();

protected:
    uint64_t m_blockIdx;
    AscendC::GlobalTensor<T> m_dqGm, m_dkGm, m_dvGm, m_dpseGm;
    const DetectFlashAttentionScoreGradEmptyTensorTilingData *__restrict m_tilingData;
};
...
#endif // DETECT_FLASH_ATTENTION_SCORE_GRAD_EMPTY_TENSOR_H_
