...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['y', 'x', 'weightB', 'indices', 'weightA']
    output_names = ['y_out']
...
def _build_args(y_in__, x_in__, weightB_in__, indices_in__, weightA_in__, y_out_out_, layer_idx, scale, y_offset, y_slice_size):
    __inputs__ = []
    for arg in [y_in__, x_in__, weightB_in__, indices_in__, weightA_in__]:
        if arg != None:
    ...
    if y_offset != None:
        ...
        attr["value"] = y_offset
        __attrs__.append(attr)
    if y_slice_size != None:
        attr = {}
        attr["name"] = "y_slice_size"
        attr["dtype"] = "int"
        attr["value"] = y_slice_size
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("AddLora", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def add_lora(y_in__, x_in__, weightB_in__, indices_in__, weightA_in__=None, y_out_out_=None, layer_idx=0, scale=0.001, y_offset=0, y_slice_size=-1, kernel_name="add_lora", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(y_in__, x_in__, weightB_in__, indices_in__, weightA_in__, y_out_out_, layer_idx, scale, y_offset, y_slice_size)
    ...
    origin_func_name = "add_lora"
    ascendc_src_dir_ex = "add_lora"
    ascendc_src_dir = "add_lora"
    ascendc_src_file = "add_lora.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator AddLora, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "AddLora"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[y_in__, x_in__, weightB_in__, indices_in__, weightA_in__], origin_outputs = [y_out_out_],\
...
def op_select_format(y_in__, x_in__, weightB_in__, indices_in__, weightA_in__=None, y_out_out_=None, layer_idx=0, scale=0.001, y_offset=0, y_slice_size=-1, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(y_in__, x_in__, weightB_in__, indices_in__, weightA_in__, y_out_out_, layer_idx, scale, y_offset, y_slice_size)
    result = check_op_cap("op_select_format", "AddLora", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(y_in__, x_in__, weightB_in__, indices_in__, weightA_in__=None, y_out_out_=None, layer_idx=0, scale=0.001, y_offset=0, y_slice_size=-1, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(y_in__, x_in__, weightB_in__, indices_in__, weightA_in__, y_out_out_, layer_idx, scale, y_offset, y_slice_size)
    result = check_op_cap("get_op_specific_info", "AddLora", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
