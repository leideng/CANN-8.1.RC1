...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['grad', 'eps']
    output_names = ['sum_grad_r', 'sum_grad_c', 'sum_grad_rc']
...
def _build_args(grad_in__, eps_in__, sum_grad_r_out_, sum_grad_c_out_, sum_grad_rc_out_):
    __inputs__ = []
    for arg in [grad_in__, eps_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [sum_grad_r_out_, sum_grad_c_out_, sum_grad_rc_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("ApplyCamePart1", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.KERNEL_NAME)
def apply_came_part1(grad_in__, eps_in__, sum_grad_r_out_, sum_grad_c_out_, sum_grad_rc_out_, kernel_name="apply_came_part1", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(grad_in__, eps_in__, sum_grad_r_out_, sum_grad_c_out_, sum_grad_rc_out_)
    ...
    origin_func_name = "apply_came_part1"
    ascendc_src_dir_ex = "apply_came_part1"
    ascendc_src_dir = "apply_came_part1"
    ascendc_src_file = "apply_came_part1.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator ApplyCamePart1, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "ApplyCamePart1"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[grad_in__, eps_in__], origin_outputs = [sum_grad_r_out_, sum_grad_c_out_, sum_grad_rc_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required'], init_value_list = [None, None, None],\
