...
class KernelKvRmsNormRopeCacheB16PANZQUANT {
 public:
  ...
  __aicore__ inline void Init(GM_ADDR kv, GM_ADDR gamma, GM_ADDR cos, GM_ADDR sin, GM_ADDR index, GM_ADDR k_cache,
                              GM_ADDR v_cache, GM_ADDR optional_k_rope, GM_ADDR optional_c_kv, GM_ADDR k_rope_scale, GM_ADDR c_kv_scale) {
    ...
    kRopeScaleGm.SetGlobalBuffer((__gm__ float*)k_rope_scale);
    ...
    // init pipe
    pipe_->InitBuffer(inQueueGamma, 2, (RMS_NORM_LENGTH + RMS_NORM_LENGTH + ROPE_LENGTH) * sizeof(float));  // 2*(512+512+64)*4/1024=8.5
    pipe_->InitBuffer(inQueueX, 2, ubFactor * RMS_NORM_LENGTH * sizeof(KV_DTYPE));  // 2*16*512*2/1024=32
    pipe_->InitBuffer(outQueue, 2, ubFactor * RMS_NORM_LENGTH * sizeof(KV_DTYPE) + ubFactor * RMS_NORM_LENGTH * sizeof(int8_t));  // 2*16*512*2/1024=32
  ...
  __aicore__ inline void Process() {
    ...
    LocalTensor<float> kRopeScaleLocal = gammaLocalFp32[RMS_NORM_LENGTH];  // kRopeScaleLocal has ROPE_LENGTH elements
    ...
    DataCopyPad(kRopeScaleLocal, kRopeScaleGm, copyParamsContinguous, padParamsFp32);
    ...
    for (int64_t loopIdx = 0; loopIdx < ubLoop; ++loopIdx) {
      ...
      LocalTensor<int8_t> quantLocal = outLocal.template ReinterpretCast<int8_t>()[ubFactor * ROPE_LENGTH * sizeof(KV_DTYPE)];
      RoPE<KV_DTYPE, true, ROPE_LENGTH>(outLocal, quantLocal, ropeLocal, cosLocal, sinLocal, kRopeScaleLocal, workspaceBuffer, ubFactor);
      ...
      ScatterUpdateNZ<KV_DTYPE, isPagedAttention, ROPE_LENGTH, D1_SIZE2_PA_NZ_QUANT, D0_SIZE32_PA_NZ_QUANT>(kCacheGm, kCacheGmNd, outLocal, quantLocal, startIdx, ubFactor);
      ...
      RmsNorm<KV_DTYPE, RMS_NORM_LENGTH>(outLocal, quantLocal, xLocal, gammaLocalFp32, cKvScaleLocal, workspaceBuffer, ubFactor);
      ...
      ScatterUpdateNZ<KV_DTYPE, isPagedAttention, RMS_NORM_LENGTH, D1_SIZE16_PA_NZ_QUANT, D0_SIZE32_PA_NZ_QUANT>(vCacheGm, vCacheGmNd, outLocal, quantLocal, startIdx, ubFactor);
      outQueue.FreeTensor(outLocal);
    }
    if (ubTail > 0) {
      ...
      // Calc: RoPE
      LocalTensor<KV_DTYPE> outLocal = outQueue.AllocTensor<KV_DTYPE>();
      LocalTensor<int8_t> quantLocal = outLocal.template ReinterpretCast<int8_t>()[ubTail * ROPE_LENGTH * sizeof(KV_DTYPE)];
      RoPE<KV_DTYPE, true, ROPE_LENGTH>(outLocal, quantLocal, ropeLocal, cosLocal, sinLocal, kRopeScaleLocal, workspaceBuffer, ubTail);
      inQueueX.FreeTensor(ropeLocal);
      outQueue.EnQue(outLocal);
      outLocal = outQueue.DeQue<KV_DTYPE>();
      ...
      // Scatter Update kCache
      ScatterUpdateNZ<KV_DTYPE, isPagedAttention, ROPE_LENGTH, D1_SIZE2_PA_NZ_QUANT, D0_SIZE32_PA_NZ_QUANT>(kCacheGm, kCacheGmNd, outLocal, quantLocal, startIdx, ubTail);
      outQueue.FreeTensor(outLocal);
      ...
      RmsNorm<KV_DTYPE, RMS_NORM_LENGTH>(outLocal, quantLocal, xLocal, gammaLocalFp32, cKvScaleLocal, workspaceBuffer, ubTail);
      inQueueX.FreeTensor(xLocal);
      outQueue.EnQue(outLocal);
      outLocal = outQueue.DeQue<KV_DTYPE>();
      quantLocal = outLocal.template ReinterpretCast<int8_t>()[ubTail * RMS_NORM_LENGTH * sizeof(KV_DTYPE)];
      ...
      // Scatter Update vCache
      ScatterUpdateNZ<KV_DTYPE, isPagedAttention, RMS_NORM_LENGTH, D1_SIZE16_PA_NZ_QUANT, D0_SIZE32_PA_NZ_QUANT>(vCacheGm, vCacheGmNd, outLocal, quantLocal, startIdx, ubTail);
      outQueue.FreeTensor(outLocal);
    }
  ...
  __aicore__ inline void RoPE(const LocalTensor<T>& outLocal,
                              const LocalTensor<int8_t>& quantLocal,
                              const LocalTensor<T>& xLocal,
                              const LocalTensor<T>& cosLocal,
                              const LocalTensor<T>& sinLocal,
  ...
  template <typename T, bool isPA = false, int64_t headSize, int64_t D1, int64_t D0>
  __aicore__ inline void ScatterUpdateNZ(const GlobalTensor<int8_t>& dst,
                                         const GlobalTensor<T>& dstNd,
                                         const LocalTensor<T>& outLocal,
                                         const LocalTensor<int8_t>& quantLocal,
                                         int64_t startIdx, int64_t rows) {
    DataCopyExtParams copyParamsNz{/*burstNum*/static_cast<uint16_t>(D1),
                                   /*burstLength*/static_cast<uint32_t>(D0 * sizeof(int8_t)),
                                   /*srcGap*/0,
                                   /*dstGap*/static_cast<uint32_t>(tilingData_->blockSize * D0 * sizeof(int8_t)) - static_cast<uint32_t>(D0 * sizeof(int8_t)),
                                   0};
    ...
    for (int64_t i = 0; i < rows; i++) {
      ...
      int64_t pageOffset = indexGmNz(tokenId);  // pageOffset = pageId * pageSize + tokenOffsetInCurrentPage
  ...
  __aicore__ inline void RmsNorm(const LocalTensor<T>& outLocal,
                                 const LocalTensor<int8_t>& quantLocal,
                                 const LocalTensor<T>& xLocal,
 ...
 private:
  ...
  GlobalTensor<float> kRopeScaleGm, cKvScaleGm;
