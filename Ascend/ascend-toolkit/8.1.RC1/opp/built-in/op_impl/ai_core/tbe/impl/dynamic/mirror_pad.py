...
class MirrorPadInit:
    ...
    def __init__(self, mode, kernel_name):
        ...
        self.output_dim4_max_cnt_align = (self.output_dim4_max_cnt + self.block_num - 1) // self.block_num
        self.output_dim4_max_cnt_block_align = (self.output_dim4_max_cnt_align // self.block_num) * self.block_num
        ...
        self.move_pad = tbe_platform.api_check_support("tik.data_move_pad")
    ...
    def outer_process_per_core_mode_2(self):
        ...
        # 尾轴不能一次放下
        with self.tik_instance.if_scope(segment_max_num == 0):
            self.process_greater_last_dim_mode_2()
        with self.tik_instance.else_scope():
            self.process_small_last_dim_mode_2(segment_max_num)

    def process_greater_last_dim_mode_2(self):
        self.segment_total.set_as(self.outer_process_num)
        ub_loop = self.process_blks // (self.output_dim4_max_cnt_block_align * 32)
        ub_tail = self.process_blks - ub_loop * (self.output_dim4_max_cnt_block_align * 32)
        ...
        with self.tik_instance.for_range(0, self.segment_total) as i:
            self.segment_move_offset.set_as(self.outer_move_out_core_offset + i)
            self.outer_move_out_index.set_as(self.segment_move_offset)
            self.get_outer_move_in_index(Constant.AXIS_DIMS_4)
            with self.tik_instance.for_range(0, ub_loop) as j:
                if self.move_pad:
                    self.tik_instance.data_move_pad(self.ub_mode2,
                                                    self.input_gm[self.outer_move_in_index * self.output_dim_4 + \
                                                                j * self.output_dim4_max_cnt_block_align * 32 * 16],
                                                    1, (self.output_dim4_max_cnt_block_align * \
                                                    32) * 16 * self.inner_bytes_size, 0, 0)
                    self.tik_instance.data_move_pad(self.output_gm[self.segment_move_offset * self.output_dim_4 + \
                                                    j * self.output_dim4_max_cnt_block_align * 32 * 16], self.ub_mode2,
                                                    1, (self.output_dim4_max_cnt_block_align * \
                                                    32) * 16 * self.inner_bytes_size, 0, 0)
                else:
                    self.tik_instance.data_move(self.ub_mode2,
                                                self.input_gm[self.outer_move_in_index * self.output_dim_4 + \
                                                                j * self.output_dim4_max_cnt_block_align * 32 * 16],
                                                0, 1, self.output_dim4_max_cnt_block_align * 32, 0, 0)
                    self.tik_instance.data_move(self.output_gm[self.segment_move_offset * self.output_dim_4 + \
                                                j * self.output_dim4_max_cnt_block_align * 32 * 16], self.ub_mode2, 0,
                                                1, self.output_dim4_max_cnt_block_align * 32, 0, 0)
            with self.tik_instance.if_scope(ub_tail > 0):
                if self.move_pad:
                    self.tik_instance.data_move_pad(self.ub_mode2,
                                                    self.input_gm[self.outer_move_in_index * self.output_dim_4 + \
                                                    ub_loop * self.output_dim4_max_cnt_block_align * 32 * 16], 1,
                                                    ub_tail * 16 * self.inner_bytes_size, 0, 0)
                    self.tik_instance.data_move_pad(self.output_gm[self.segment_move_offset * self.output_dim_4 + \
                                                    ub_loop * self.output_dim4_max_cnt_block_align * 32 * 16],
                                                    self.ub_mode2, 1, ub_tail * 16 * self.inner_bytes_size, 0, 0)
                else:
                    self.tik_instance.data_move(self.ub_mode2,
                                                self.input_gm[self.outer_move_in_index * self.output_dim_4 + \
                                                ub_loop * self.output_dim4_max_cnt_block_align * 32 * 16], 0, 1,
                                                ub_tail, 0, 0)
                    self.tik_instance.data_move(self.output_gm[self.segment_move_offset * self.output_dim_4 + \
                                                ub_loop * self.output_dim4_max_cnt_block_align * 32 * 16],
                                                self.ub_mode2, 0, 1, ub_tail, 0, 0)

    def process_small_last_dim_mode_2(self, segment_max_num):
        ...
        with self.tik_instance.for_range(0, self.segment_total) as i:
            ...
            with self.tik_instance.for_range(0, self.inner_process_num) as j:
                ...
                self.get_outer_move_in_index(Constant.AXIS_DIMS_4)
                if self.move_pad:
                    self.tik_instance.data_move_pad(self.ub_mode2[self.output_dim_4 * j],
                                                    self.input_gm[self.outer_move_in_index * self.output_dim_4], 1,
                                                    self.process_blks * 16 * self.inner_bytes_size, 0, 0)
                else:
            ...
            if self.move_pad:
                self.tik_instance.data_move_pad(self.output_gm[self.segment_move_offset * self.output_dim_4],
                                                self.ub_mode2, 1,
                                                (self.process_blks * \
                                                self.inner_process_num) * 16 * self.inner_bytes_size, 0, 0)
            else:
                self.tik_instance.data_move(self.output_gm[self.segment_move_offset * self.output_dim_4],
                                            self.ub_mode2, 0, 1, self.process_blks * self.inner_process_num, 0, 0)
    ...
    def compute_loop(self, one_loop_process_blocks):
            ...
            if self.move_pad:
                self.tik_instance.data_move_pad(
                    self.ub_first, self.input_gm[self.outer_move_in_index * self.input_dim_4 +
                                                 ...
                                                 (1 - self.two_mode_diff) * self.dtype_rate], 1,
                                                 one_loop_process_blocks * 16 * self.inner_bytes_size, 0, 0)
            else:
    ...
    def compute_vnchwconv(self, one_loop_process_blocks):
        ...
        if self.move_pad:
            self.tik_instance.data_move_pad(self.output_gm[self.outer_move_out_index * self.output_dim_4],
                                            self.ub_second,
                                            1,
                                            self.tail_loop_true_nums * self.inner_bytes_size,
                                            0, 0)
        else:
            self.tik_instance.data_move(self.output_gm[self.outer_move_out_index * self.output_dim_4],
                                        self.ub_second, 0, 1, self.inner_process_nums_tail_loop, 0, 0)
        self.get_vnchw_stride(one_loop_process_blocks, True)
        self.compute_loop(one_loop_process_blocks)

    def compute_pading_40(self):
        with self.tik_instance.if_scope(self.pading_40 != 0):
            ...
            if self.move_pad:
                self.tik_instance.data_move_pad(
                    self.ub_first, self.input_gm[self.outer_move_in_index * self.input_dim_4 +
                                                 self.move_in_start_blocks * Constant.TRANS_MIN_BLKS + \
                                                 (1 - self.two_mode_diff) * self.dtype_rate], 1,
                                                self.tail_loop_true_nums * self.inner_bytes_size, 0, 0)
            else:
                self.tik_instance.data_move(
                    self.ub_first, self.input_gm[self.outer_move_in_index * self.input_dim_4 +
                                                 self.move_in_start_blocks * Constant.TRANS_MIN_BLKS +
            ...
            self.tik_instance.vnchwconv(False, False, dst_list_0, src_list_0, self.inner_process_nums_tail_loop,
                                        self.conv1_stride_0, self.conv1_stride_1)
            self.compute_vnchwconv(one_loop_process_blocks)

    def compute_more_loop(self, one_loop_process_blocks):
        with self.tik_instance.if_scope(self.inner_loop_times > 1):
            if self.move_pad:
                with self.tik_instance.for_range(0, self.inner_loop_times - 1) as inner_loop_index:
                    ...
                    self.tik_instance.data_move_pad(
                        self.ub_first, self.input_gm[self.outer_move_in_index * self.input_dim_4 +
                                                     self.move_in_start_blocks * Constant.TRANS_MIN_BLKS], 1,
                                                     one_loop_process_blocks * 16 * self.inner_bytes_size, 0, 0)
                    self.tik_instance.data_move_pad(
                        self.output_gm[self.outer_move_out_index * self.output_dim_4 + self.pading_40 +
                                       self.move_in_start_blocks * Constant.TRANS_MIN_BLKS], self.ub_first, 1,
                                       one_loop_process_blocks * 16 * self.inner_bytes_size, 0, 0)
                self.tik_instance.data_move_pad(
                    self.ub_first, self.input_gm[self.outer_move_in_index * self.input_dim_4 + 
                                                 (self.inner_loop_times - 1) * one_loop_process_blocks * \
                                                  Constant.TRANS_MIN_BLKS], 1,
                                                 (self.input_dim_4 - (self.inner_loop_times - \
                                                 1) * one_loop_process_blocks * 16) * self.inner_bytes_size,
                                                 0, 0)
                self.tik_instance.data_move_pad(
                    self.output_gm[self.outer_move_out_index * self.output_dim_4 + self.pading_40 +
                                   (self.inner_loop_times - 1) * one_loop_process_blocks * \
                                    Constant.TRANS_MIN_BLKS], self.ub_first, 1,
                                   (self.input_dim_4 - (self.inner_loop_times - \
                                   1) * one_loop_process_blocks * 16) * self.inner_bytes_size,
                                   0, 0)
            else:
    ...
    def compute_loop_one(self):
        with self.tik_instance.if_scope(self.inner_loop_times == 1):
            if self.move_pad:
                self.tik_instance.data_move_pad(self.ub_first, self.input_gm[self.outer_move_in_index * \
                                                self.input_dim_4], 1,
                                                (self.inner_process_nums_tail_loop - \
                                                1) * 16 * self.inner_bytes_size, 0, 0)
                self.tik_instance.data_move_pad(self.output_gm[self.outer_move_out_index * \
                                                self.output_dim_4 + self.pading_40],
                                                self.ub_first, 1,
                                                (self.inner_process_nums_tail_loop - 1) * 16 * self.inner_bytes_size,
                                                0, 0)
                self.tik_instance.data_move_pad(
                    self.ub_first,
                    self.input_gm[self.outer_move_in_index * self.input_dim_4 + \
                    self.input_dim_4 - Constant.TRANS_MIN_BLKS],
                    1, 1 * 16 * self.inner_bytes_size, 0, 0)
                self.tik_instance.data_move_pad(
                    self.output_gm[self.outer_move_out_index * self.output_dim_4 + self.pading_40 + self.input_dim_4 -
                                   Constant.TRANS_MIN_BLKS], self.ub_first,
                                   1, 1 * 16 * self.inner_bytes_size, 0, 0)
            else:
                ...
                self.tik_instance.data_move(self.output_gm[self.outer_move_out_index * \
                                            self.output_dim_4 + self.pading_40],
    ...
    def compute_small(self):
        if self.move_pad:
            self.tik_instance.data_move_pad(
                ...
                1, 1 * (16 - self.pading_41) * self.inner_bytes_size, 0, 0)
            self.tik_instance.data_move_pad(
                self.ub_first, self.input_gm[self.outer_move_in_index * self.input_dim_4 + self.input_dim_4 - \
        ...
        if self.move_pad:
            self.tik_instance.data_move_pad(
                ...
                self.ub_addr_rollback, 1, 1 * 16 * self.inner_bytes_size, 0, 0)
        else:
            self.tik_instance.data_move(
                self.output_gm[(self.outer_move_out_index + 1) * self.output_dim_4 - Constant.TRANS_MIN_BLKS],
                self.ub_addr_rollback, 0, 1, 1, 0, 0)

    def compute_loop_pading_41(self, one_loop_process_blocks):
        with self.tik_instance.if_scope(tik.all(self.address_rollback_sum == 2, self.inner_loop_times == 1)):
            for i in range(16):
                self.ub_addr_rollback[15 - i].set_as(self.ub_second[self.tail_loop_true_nums - i - 1])
            if self.move_pad:
                self.tik_instance.data_move_pad(
                    self.output_gm[self.outer_move_out_index * self.output_dim_4 + self.pading_40 +
                                   self.input_dim_4], self.ub_second, 1,
                                   self.tail_loop_true_nums * self.inner_bytes_size,
                                   0, 0)
                self.tik_instance.data_move_pad(
                    self.output_gm[self.outer_move_out_index * self.output_dim_4 + self.pading_40 +
                                   self.input_dim_4 + self.tail_loop_true_nums - 16],
                    self.ub_addr_rollback, 1,
                    1 * 16 * self.inner_bytes_size, 0, 0)
            else:
                self.tik_instance.data_move(
                    self.output_gm[self.outer_move_out_index * self.output_dim_4 + self.pading_40 +
    ...
    def compute_inner_loop_times(self, one_loop_process_blocks):
        with self.tik_instance.for_range(0, self.inner_loop_times - 1) as inner_loop_index:
            ...
            if self.move_pad:
                ...
                self.tik_instance.data_move(
                    ...
                    self.tail_loop_true_nums + (self.two_mode_diff - \
                    1) * self.dtype_rate - self.move_in_start_blocks * Constant.TRANS_MIN_BLKS], 0, 1,
                    one_loop_process_blocks, 0, 0)
            dst_list_0 = [self.ub_second[Constant.TRANS_MIN_BLKS * i] for i in range(Constant.TRANS_MIN_BLKS)]
            src_list_0 = [self.ub_first] * Constant.TRANS_MIN_BLKS
            self.tik_instance.vnchwconv(False, False, dst_list_0, src_list_0, one_loop_process_blocks,
            ...
            if self.move_pad:
                self.tik_instance.data_move_pad(
                    self.output_gm[self.outer_move_out_index * self.output_dim_4 + self.input_dim_4 +
                                   ...
                                   inner_loop_index * one_loop_process_blocks * Constant.TRANS_MIN_BLKS],
                    self.ub_second, 1, one_loop_process_blocks * 16 * self.inner_bytes_size, 0, 0)
            else:
                self.tik_instance.data_move(
                    self.output_gm[self.outer_move_out_index * self.output_dim_4 + self.input_dim_4 +
                                   self.pading_40 + self.tail_loop_true_nums +
    ...
    def last_dim_inner_process(self):
        ...
        self.inner_process_nums_tail_loop.set_as(self.input_dim_4_align_blocks -
                                                 (self.inner_loop_times - 1) * one_loop_process_blocks)
        self.compute_more_loop(one_loop_process_blocks)
        self.compute_loop_one()
        ...
        with self.tik_instance.if_scope(self.pading_41 != 0):
            ...
            with self.tik_instance.if_scope(tik.all(self.pading_41 < 16, self.address_rollback_sum == 2)):
                self.compute_small()
            with self.tik_instance.else_scope():
                self.inner_loop_times.set_as(
                ...
                self.tik_instance.vnchwconv(False, False, dst_list_1, src_list_1, self.inner_process_nums_tail_loop,
                                            self.conv2_stride_0, self.conv2_stride_1)
                self.compute_loop_pading_41(one_loop_process_blocks)

                self.get_vnchw_stride(one_loop_process_blocks, True)
                self.compute_inner_loop_times(one_loop_process_blocks)
    ...
    def inner_pading30_process(self):
        ...
        if self.move_pad:
            self.tik_instance.data_move_pad(
                self.ub_first,
                self.input_gm[self.outer_move_in_index * self.inner_insize + self.start_line * self.input_dim_4],
                1, (self.now_process_lines * self.input_dim_4) * self.inner_bytes_size, 0, 0)
        else:
            self.tik_instance.data_move(
                self.ub_first,
        ...
        with self.tik_instance.for_range(0, self.now_process_lines) as line_index:
            self.tik_instance.data_move(
                ...
                self.ub_second[(self.now_process_lines - line_index - \
                1) * self.input_dim_4 * Constant.TRANS_MIN_BLKS],
        ...
        if self.move_pad:
            self.tik_instance.data_move_pad(
                self.output_gm[self.outer_move_out_index * self.inner_outsize +
                               self.move_out_start_line * self.output_dim_4], self.ub_first, 1,
                               (self.now_process_lines * self.output_dim_4) * self.inner_bytes_size, 0, 0)
        else:
            self.tik_instance.data_move(
                self.output_gm[self.outer_move_out_index * self.inner_outsize +
                               self.move_out_start_line * self.output_dim_4],
                               self.ub_first, 0, 1, self.process_blks, 0, 0)

    def compute_out(self):
        if self.move_pad:
            with self.tik_instance.if_scope(tik.all(self.address_rollback_sum == 13, self.process_blks > 1)):
                self.tik_instance.data_move_pad(
                    self.output_gm[self.outer_move_out_index * self.inner_outsize +
                                   (self.pading_30 + self.move_out_start_line) * self.output_dim_4], self.ub_second,
                    1, (self.process_blks - 1) * 16 * self.inner_bytes_size, 0, 0)
                for i in range(16):
                    self.ub_addr_rollback[15 - i].set_as(self.ub_second[self.now_process_lines * \
                                                                        self.output_dim_4 - i - 1])
                self.tik_instance.data_move_pad(self.output_gm[(self.outer_move_out_index + \
                                                               1) * self.inner_outsize - 16],
                                                self.ub_addr_rollback, 1, 1 * 16 * self.inner_bytes_size, 0, 0)
            with self.tik_instance.else_scope():
                self.tik_instance.data_move_pad(
                    self.output_gm[self.outer_move_out_index * self.inner_outsize +
                                   (self.pading_30 + self.move_out_start_line) * self.output_dim_4],
                    self.ub_second, 1,
                    (self.now_process_lines * self.output_dim_4) * self.inner_bytes_size, 0, 0)
        else:
            with self.tik_instance.if_scope(tik.all(self.address_rollback_sum == 13, self.process_blks > 1)):
                ...
                for i in range(16):
                    self.ub_addr_rollback[15 - i].set_as(
                    self.ub_second[self.now_process_lines * self.output_dim_4 - i - 1])
            ...
            with self.tik_instance.else_scope():
                self.tik_instance.data_move(
                    self.output_gm[self.outer_move_out_index * self.inner_outsize +
                                   (self.pading_30 + self.move_out_start_line) * self.output_dim_4],
                    self.ub_second, 0, 1, self.process_blks, 0, 0)
    ...
    def inner_input_dim3_process(self):
        ...
        if self.move_pad:
            self.tik_instance.data_move_pad(
                self.ub_first,
                self.input_gm[self.outer_move_in_index * self.inner_insize + \
                              self.move_out_start_line * self.input_dim_4],
                1, (self.now_process_lines * self.input_dim_4) * self.inner_bytes_size,
                0, 0)
        else:
            self.tik_instance.data_move(
                self.ub_first,
    ...
    def compute_out_pading31(self):
        if self.move_pad:
            self.compute_mode_pad()
        else:
            self.compute_not_move_pad()

    def compute_mode_pad(self):
        with self.tik_instance.if_scope(self.address_rollback_sum == 23):
            with self.tik_instance.if_scope(self.process_blks > 1):
                self.tik_instance.data_move_pad(
                    self.output_gm[self.outer_move_out_index * self.inner_outsize +
                                   (self.pading_30 + self.input_dim_3 + \
                                   self.move_out_start_line) * self.output_dim_4],
                    self.ub_first, 1, (self.process_blks - 1) * 16 * self.inner_bytes_size, 0, 0)
                for i in range(16):
                    self.ub_addr_rollback[15 - i].set_as(self.ub_first[self.now_process_lines * \
                                                         self.output_dim_4 - i - 1])
                self.tik_instance.data_move_pad(self.output_gm[(self.outer_move_out_index + \
                                                               1) * self.inner_outsize - 16],
                                            self.ub_addr_rollback, 1, 1 * 16 * self.inner_bytes_size, 0, 0)
            with self.tik_instance.else_scope():
                self.tik_instance.data_move_pad(self.output_gm[(self.outer_move_out_index + \
                                                               1) * self.inner_outsize - 16],
                                            self.ub_first, 1, 1 * 16 * self.inner_bytes_size, 0, 0)
        with self.tik_instance.else_scope():
            self.tik_instance.data_move_pad(
                self.output_gm[self.outer_move_out_index * self.inner_outsize +
                               (self.pading_30 + self.input_dim_3 + \
                               self.move_out_start_line) * self.output_dim_4],
                self.ub_first, 1, (self.now_process_lines * self.output_dim_4) * self.inner_bytes_size, 0, 0)

    def compute_not_move_pad(self):
        with self.tik_instance.if_scope(self.address_rollback_sum == 23):
            with self.tik_instance.if_scope(self.process_blks > 1):
                self.tik_instance.data_move(
                    self.output_gm[self.outer_move_out_index * self.inner_outsize +
                ...
                for i in range(16):
                    self.ub_addr_rollback[15 - i].set_as(self.ub_first[self.now_process_lines * \
                                                                       self.output_dim_4 - i - 1])
                self.tik_instance.data_move(self.output_gm[(self.outer_move_out_index + 1) * \
                                                            self.inner_outsize - 16],
                                            self.ub_addr_rollback, 0, 1, 1, 0, 0)
            with self.tik_instance.else_scope():
                self.tik_instance.data_move(self.output_gm[(self.outer_move_out_index + 1) * \
                                                            self.inner_outsize - 16],
    ...
    def inner_pading31_process(self):
        ...
        if self.move_pad:
            self.tik_instance.data_move_pad(
                self.ub_first,
                self.input_gm[self.outer_move_in_index * self.inner_insize + self.start_line * self.input_dim_4], 1,
                (self.now_process_lines * self.input_dim_4) * self.inner_bytes_size, 0, 0)
        else:
            self.tik_instance.data_move(
                self.ub_first,
                self.input_gm[self.outer_move_in_index * self.inner_insize + self.start_line * self.input_dim_4],
...
def mirror_pad(x, paddings, y, mode, kernel_name="mirror_pad"):
    ...
    supported_dtype = ("bfloat16", "float16", "int16", "uint16", "float32", "int32", "int64", "uint32")
