...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['grad', 'var', 'm', 'v', 's', 'step']
    output_names = ['var', 'm', 'v', 's']
...
def _build_args(grad_in__, var_in__, m_in__, v_in__, s_in__, step_in__, var_out_, m_out_, v_out_, s_out_, lr, ema_decay, beta1, beta2, eps, mode, bias_correction, weight_decay):
    __inputs__ = []
    for arg in [grad_in__, var_in__, m_in__, v_in__, s_in__, step_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [var_out_, m_out_, v_out_, s_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if bias_correction != None:
        attr = {}
        attr["name"] = "bias_correction"
        attr["dtype"] = "bool"
        attr["value"] = bias_correction
        __attrs__.append(attr)
    if weight_decay != None:
        attr = {}
        attr["name"] = "weight_decay"
        attr["dtype"] = "float"
        attr["value"] = weight_decay
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("ApplyFusedEmaAdam", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def apply_fused_ema_adam(grad_in__, var_in__, m_in__, v_in__, s_in__, step_in__, var_out_, m_out_, v_out_, s_out_, lr=0.001, ema_decay=0.9999, beta1=0.9, beta2=0.999, eps=1e-08, mode=1, bias_correction=True, weight_decay=0, kernel_name="apply_fused_ema_adam", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(grad_in__, var_in__, m_in__, v_in__, s_in__, step_in__, var_out_, m_out_, v_out_, s_out_, lr, ema_decay, beta1, beta2, eps, mode, bias_correction, weight_decay)
    ...
    origin_func_name = "apply_fused_ema_adam"
    ascendc_src_dir_ex = "apply_fused_ema_adam"
    ascendc_src_dir = "apply_fused_ema_adam"
    ascendc_src_file = "apply_fused_ema_adam.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator ApplyFusedEmaAdam, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "ApplyFusedEmaAdam"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[grad_in__, var_in__, m_in__, v_in__, s_in__, step_in__], origin_outputs = [var_out_, m_out_, v_out_, s_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required', 'required', 'required', 'required', 'required', 'required'], init_value_list = [None, None, None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(grad_in__, var_in__, m_in__, v_in__, s_in__, step_in__, var_out_, m_out_, v_out_, s_out_, lr=0.001, ema_decay=0.9999, beta1=0.9, beta2=0.999, eps=1e-08, mode=1, bias_correction=True, weight_decay=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_in__, var_in__, m_in__, v_in__, s_in__, step_in__, var_out_, m_out_, v_out_, s_out_, lr, ema_decay, beta1, beta2, eps, mode, bias_correction, weight_decay)
    result = check_op_cap("op_select_format", "ApplyFusedEmaAdam", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(grad_in__, var_in__, m_in__, v_in__, s_in__, step_in__, var_out_, m_out_, v_out_, s_out_, lr=0.001, ema_decay=0.9999, beta1=0.9, beta2=0.999, eps=1e-08, mode=1, bias_correction=True, weight_decay=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_in__, var_in__, m_in__, v_in__, s_in__, step_in__, var_out_, m_out_, v_out_, s_out_, lr, ema_decay, beta1, beta2, eps, mode, bias_correction, weight_decay)
    result = check_op_cap("get_op_specific_info", "ApplyFusedEmaAdam", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
