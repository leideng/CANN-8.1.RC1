...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'input_scale', 'input_offset']
    output_names = ['y', 'out_scale']
...
def _build_args(x_in__, input_scale_in__, input_offset_in__, y_out_, out_scale_out_, approximate, quant_mode):
    __inputs__ = []
    for arg in [x_in__, input_scale_in__, input_offset_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [y_out_, out_scale_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("GeluQuant", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_STR, para_check.KERNEL_NAME)
def gelu_quant(x_in__, input_scale_in__=None, input_offset_in__=None, y_out_=None, out_scale_out_=None, approximate="none", quant_mode="dynamic", kernel_name="gelu_quant", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, input_scale_in__, input_offset_in__, y_out_, out_scale_out_, approximate, quant_mode)
    ...
    origin_func_name = "gelu_quant"
    ascendc_src_dir_ex = "gelu_quant"
    ascendc_src_dir = "gelu_quant"
    ascendc_src_file = "gelu_quant.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator GeluQuant, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "GeluQuant"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, input_scale_in__, input_offset_in__], origin_outputs = [y_out_, out_scale_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'optional', 'optional', 'required', 'required'], init_value_list = [None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(x_in__, input_scale_in__=None, input_offset_in__=None, y_out_=None, out_scale_out_=None, approximate="none", quant_mode="dynamic", impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, input_scale_in__, input_offset_in__, y_out_, out_scale_out_, approximate, quant_mode)
    result = check_op_cap("op_select_format", "GeluQuant", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, input_scale_in__=None, input_offset_in__=None, y_out_=None, out_scale_out_=None, approximate="none", quant_mode="dynamic", impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, input_scale_in__, input_offset_in__, y_out_, out_scale_out_, approximate, quant_mode)
    result = check_op_cap("get_op_specific_info", "GeluQuant", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
