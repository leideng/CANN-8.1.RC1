...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['grad_y', 'expanded_row_idx', 'expanded_x', 'scales', 'expert_idx', 'bias']
    output_names = ['grad_expanded_x', 'grad_scales']
...
def _build_args(grad_y_in__, expanded_row_idx_in__, expanded_x_in__, scales_in__, expert_idx_in__, bias_in__, grad_expanded_x_out_, grad_scales_out_, drop_pad_mode, active_num, expert_num, expert_capacity):
    __inputs__ = []
    for arg in [grad_y_in__, expanded_row_idx_in__, expanded_x_in__, scales_in__, expert_idx_in__, bias_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [grad_expanded_x_out_, grad_scales_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("MoeFinalizeRoutingV2Grad", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def moe_finalize_routing_v2_grad(grad_y_in__, expanded_row_idx_in__, expanded_x_in__=None, scales_in__=None, expert_idx_in__=None, bias_in__=None, grad_expanded_x_out_=None, grad_scales_out_=None, drop_pad_mode=0, active_num=0, expert_num=0, expert_capacity=0, kernel_name="moe_finalize_routing_v2_grad", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(grad_y_in__, expanded_row_idx_in__, expanded_x_in__, scales_in__, expert_idx_in__, bias_in__, grad_expanded_x_out_, grad_scales_out_, drop_pad_mode, active_num, expert_num, expert_capacity)
    ...
    origin_func_name = "moe_finalize_routing_v2_grad"
    ascendc_src_dir_ex = "moe_finalize_routing_v2_grad"
    ascendc_src_dir = "moe_finalize_routing_v2_grad"
    ascendc_src_file = "moe_finalize_routing_v2_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MoeFinalizeRoutingV2Grad, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[grad_y_in__, expanded_row_idx_in__, expanded_x_in__, scales_in__, expert_idx_in__, bias_in__], origin_outputs = [grad_expanded_x_out_, grad_scales_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'optional', 'optional', 'optional', 'optional', 'required', 'required'], init_value_list = [None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(grad_y_in__, expanded_row_idx_in__, expanded_x_in__=None, scales_in__=None, expert_idx_in__=None, bias_in__=None, grad_expanded_x_out_=None, grad_scales_out_=None, drop_pad_mode=0, active_num=0, expert_num=0, expert_capacity=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_y_in__, expanded_row_idx_in__, expanded_x_in__, scales_in__, expert_idx_in__, bias_in__, grad_expanded_x_out_, grad_scales_out_, drop_pad_mode, active_num, expert_num, expert_capacity)
    result = check_op_cap("op_select_format", "MoeFinalizeRoutingV2Grad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(grad_y_in__, expanded_row_idx_in__, expanded_x_in__=None, scales_in__=None, expert_idx_in__=None, bias_in__=None, grad_expanded_x_out_=None, grad_scales_out_=None, drop_pad_mode=0, active_num=0, expert_num=0, expert_capacity=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_y_in__, expanded_row_idx_in__, expanded_x_in__, scales_in__, expert_idx_in__, bias_in__, grad_expanded_x_out_, grad_scales_out_, drop_pad_mode, active_num, expert_num, expert_capacity)
    result = check_op_cap("get_op_specific_info", "MoeFinalizeRoutingV2Grad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
