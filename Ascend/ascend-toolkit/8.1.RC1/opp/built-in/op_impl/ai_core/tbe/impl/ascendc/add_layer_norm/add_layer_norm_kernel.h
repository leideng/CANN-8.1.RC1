...
#ifndef __ADD_LAYER_NORM_KERNEL_H_
#define __ADD_LAYER_NORM_KERNEL_H_
...
#define IS_BETAGAMMA_NEEDCAST (!is_same<T_GAMMA, float>::value)
 ...
 private:
  ...
  __aicore__ inline void CopyIn(int32_t proc_id, int32_t row_count) {
    ...
    if constexpr (is_same<float, T>::value) {
      if constexpr (IS_X1_NEEDCAST) {
        ...
        for (uint32_t i = 0; i < row_count; i++) {
          Cast(add_buf_local[i*num_last_dim_aligned], y_buf[i*numLastDimAlignedMixDtype], RoundMode::CAST_NONE, num_last_dim);
        }
      } else {
    ...
    if constexpr (is_same<float, T>::value) {
      if constexpr (IS_X2_NEEDCAST) {
        auto y_buf = x_local_fp32.template ReinterpretCast<T_X2>();
        for (uint32_t i = 0; i < row_count; i++) {
          Cast(x2_local[i*num_last_dim_aligned], y_buf[i*numLastDimAlignedMixDtype], RoundMode::CAST_NONE, num_last_dim);
        }
        pipe_barrier(PIPE_V);
  ...
  __aicore__ inline void precision_compute(int32_t nums, LocalTensor<T_GAMMA>& gamma_local, LocalTensor<T_GAMMA>& beta_local) {
    LocalTensor<T> y_local = y_que.template AllocTensor<T>();
#if OUTPUT_MEAN_RSTD == 1
    LocalTensor<float> mean_local = mean_que.template AllocTensor<float>();
    LocalTensor<float> rstd_local = rstd_que.template AllocTensor<float>();
    ...
    if constexpr (IS_BETAGAMMA_NEEDCAST) {
      if constexpr (!is_same<T, float>::value) {
        ...
        for (auto i = 0; i < nums; i++) {
          Mul(z_local_fp32[i * num_last_dim_aligned], z_local_fp32[i * num_last_dim_aligned], x_local_fp32, num_last_dim);
        }
        pipe_barrier(PIPE_V);
        for (auto i = 0; i < nums; i++) {
          Add(z_local_fp32[i * num_last_dim_aligned], z_local_fp32[i * num_last_dim_aligned], y_local_fp32, num_last_dim);
        }
        pipe_barrier(PIPE_V);
        if constexpr (is_same<T, half>::value) {
      ...
      if constexpr (!is_same<T, float>::value) {
        for (auto i = 0; i < nums; i++) {
          Mul(z_local_fp32[i * num_last_dim_aligned], z_local_fp32[i * num_last_dim_aligned], gamma_local, num_last_dim);
        }
        pipe_barrier(PIPE_V);
        for (auto i = 0; i < nums; i++) {
  ...
  __aicore__ inline void precision_compute_single_row(LocalTensor<T_GAMMA>& gamma_local, LocalTensor<T_GAMMA>& beta_local) {
#if OUTPUT_MEAN_RSTD == 1
    LocalTensor<float> mean_local = mean_que.template AllocTensor<float>();
    LocalTensor<float> rstd_local = rstd_que.template AllocTensor<float>();
#endif
