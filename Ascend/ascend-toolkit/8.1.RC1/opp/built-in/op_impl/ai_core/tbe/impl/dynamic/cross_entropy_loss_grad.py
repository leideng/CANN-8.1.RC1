...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['grad_loss', 'log_prob', 'target', 'weight', 'grad_zloss', 'lse_for_zloss']
    output_names = ['x_grad']
...
def _build_args(grad_loss_in__, log_prob_in__, target_in__, weight_in__, grad_zloss_in__, lse_for_zloss_in__, x_grad_out_, reduction, ignore_index, label_smoothing, lse_square_scale_for_zloss):
    __inputs__ = []
    for arg in [grad_loss_in__, log_prob_in__, target_in__, weight_in__, grad_zloss_in__, lse_for_zloss_in__]:
        if arg != None:
...
@tbe_register.register_operator("CrossEntropyLossGrad", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def cross_entropy_loss_grad(grad_loss_in__, log_prob_in__, target_in__, weight_in__=None, grad_zloss_in__=None, lse_for_zloss_in__=None, x_grad_out_=None, reduction="mean", ignore_index=-100, label_smoothing=0, lse_square_scale_for_zloss=0, kernel_name="cross_entropy_loss_grad", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(grad_loss_in__, log_prob_in__, target_in__, weight_in__, grad_zloss_in__, lse_for_zloss_in__, x_grad_out_, reduction, ignore_index, label_smoothing, lse_square_scale_for_zloss)
    ...
    origin_func_name = "cross_entropy_loss_grad"
    ascendc_src_dir_ex = "cross_entropy_loss_grad"
    ascendc_src_dir = "cross_entropy_loss_grad"
    ascendc_src_file = "cross_entropy_loss_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator CrossEntropyLossGrad, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "CrossEntropyLossGrad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[grad_loss_in__, log_prob_in__, target_in__, weight_in__, grad_zloss_in__, lse_for_zloss_in__], origin_outputs = [x_grad_out_],\
...
def op_select_format(grad_loss_in__, log_prob_in__, target_in__, weight_in__=None, grad_zloss_in__=None, lse_for_zloss_in__=None, x_grad_out_=None, reduction="mean", ignore_index=-100, label_smoothing=0, lse_square_scale_for_zloss=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_loss_in__, log_prob_in__, target_in__, weight_in__, grad_zloss_in__, lse_for_zloss_in__, x_grad_out_, reduction, ignore_index, label_smoothing, lse_square_scale_for_zloss)
    result = check_op_cap("op_select_format", "CrossEntropyLossGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(grad_loss_in__, log_prob_in__, target_in__, weight_in__=None, grad_zloss_in__=None, lse_for_zloss_in__=None, x_grad_out_=None, reduction="mean", ignore_index=-100, label_smoothing=0, lse_square_scale_for_zloss=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_loss_in__, log_prob_in__, target_in__, weight_in__, grad_zloss_in__, lse_for_zloss_in__, x_grad_out_, reduction, ignore_index, label_smoothing, lse_square_scale_for_zloss)
    result = check_op_cap("get_op_specific_info", "CrossEntropyLossGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
