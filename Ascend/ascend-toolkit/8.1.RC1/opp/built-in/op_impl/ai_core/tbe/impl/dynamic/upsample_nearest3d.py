...
@tbe_register.register_operator("UpsampleNearest3d", trans_bool_to_s8=False)
...
def upsample_nearest3d(x_in__, y_out_, output_size=[], scale_d=0, scale_h=0, scale_w=0, kernel_name="upsample_nearest3d", impl_mode=""):
    ...
    origin_func_name = "upsample_nearest3d"
    ascendc_src_dir_ex = "upsample_nearest3d"
    ascendc_src_dir = "upsample_nearest3d"
    ascendc_src_file = "upsample_nearest3d.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator UpsampleNearest3d, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "UpsampleNearest3d"
...
def op_select_format(x_in__, y_out_, output_size=[], scale_d=0, scale_h=0, scale_w=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, output_size, scale_d, scale_h, scale_w)
    result = check_op_cap("op_select_format", "UpsampleNearest3d", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, y_out_, output_size=[], scale_d=0, scale_h=0, scale_w=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, output_size, scale_d, scale_h, scale_w)
    result = check_op_cap("get_op_specific_info", "UpsampleNearest3d", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
