...
def get_dtype_fmt_options(__inputs__, __outputs__):
    ...
    output_names = ['permute_tokens', 'sorted_indices']
...
def _build_args(tokens_in__, indices_in__, permute_tokens_out_, sorted_indices_out_, num_out_tokens, padded_mode):
    __inputs__ = []
    for arg in [tokens_in__, indices_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [permute_tokens_out_, sorted_indices_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if num_out_tokens != None:
        attr = {}
        attr["name"] = "num_out_tokens"
        attr["dtype"] = "int"
        attr["value"] = num_out_tokens
...
@tbe_register.register_operator("MoeTokenPermute", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def moe_token_permute(tokens_in__, indices_in__, permute_tokens_out_, sorted_indices_out_, num_out_tokens=0, padded_mode=False, kernel_name="moe_token_permute", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(tokens_in__, indices_in__, permute_tokens_out_, sorted_indices_out_, num_out_tokens, padded_mode)
    ...
    origin_func_name = "moe_token_permute"
    ascendc_src_dir_ex = "moe_token_permute"
    ascendc_src_dir = "moe_token_permute"
    ascendc_src_file = "moe_token_permute.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MoeTokenPermute, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "MoeTokenPermute"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[tokens_in__, indices_in__], origin_outputs = [permute_tokens_out_, sorted_indices_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required'], init_value_list = [None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(tokens_in__, indices_in__, permute_tokens_out_, sorted_indices_out_, num_out_tokens=0, padded_mode=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(tokens_in__, indices_in__, permute_tokens_out_, sorted_indices_out_, num_out_tokens, padded_mode)
    result = check_op_cap("op_select_format", "MoeTokenPermute", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(tokens_in__, indices_in__, permute_tokens_out_, sorted_indices_out_, num_out_tokens=0, padded_mode=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(tokens_in__, indices_in__, permute_tokens_out_, sorted_indices_out_, num_out_tokens, padded_mode)
    result = check_op_cap("get_op_specific_info", "MoeTokenPermute", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
