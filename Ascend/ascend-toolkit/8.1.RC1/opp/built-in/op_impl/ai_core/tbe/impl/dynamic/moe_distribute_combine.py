...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['expand_x', 'expert_ids', 'expand_idx', 'ep_send_counts', 'expert_scales', 'tp_send_counts', 'x_active_mask', 'activation_scale', 'weight_scale', 'group_list', 'expand_scales']
    output_names = ['x']
...
def _build_args(expand_x_in__, expert_ids_in__, expand_idx_in__, ep_send_counts_in__, expert_scales_in__, tp_send_counts_in__, x_active_mask_in__, activation_scale_in__, weight_scale_in__, group_list_in__, expand_scales_in__, x_out_, group_ep, ep_world_size, ep_rank_id, moe_expert_num, group_tp, tp_world_size, tp_rank_id, expert_shard_type, shared_expert_num, shared_expert_rank_num, global_bs, out_dtype, comm_quant_mode, group_list_type):
    __inputs__ = []
    for arg in [expand_x_in__, expert_ids_in__, expand_idx_in__, ep_send_counts_in__, expert_scales_in__, tp_send_counts_in__, x_active_mask_in__, activation_scale_in__, weight_scale_in__, group_list_in__, expand_scales_in__]:
        if arg != None:
    ...
    if tp_rank_id != None:
        ...
        __attrs__.append(attr)
    if expert_shard_type != None:
        attr = {}
        attr["name"] = "expert_shard_type"
        attr["dtype"] = "int"
        attr["value"] = expert_shard_type
        __attrs__.append(attr)
    if shared_expert_num != None:
        attr = {}
        attr["name"] = "shared_expert_num"
        attr["dtype"] = "int"
        attr["value"] = shared_expert_num
        __attrs__.append(attr)
    if shared_expert_rank_num != None:
        attr = {}
        attr["name"] = "shared_expert_rank_num"
        attr["dtype"] = "int"
        attr["value"] = shared_expert_rank_num
        __attrs__.append(attr)
    if global_bs != None:
        attr = {}
        attr["name"] = "global_bs"
    ...
    if out_dtype != None:
        ...
        __attrs__.append(attr)
    if comm_quant_mode != None:
        attr = {}
        attr["name"] = "comm_quant_mode"
        attr["dtype"] = "int"
        attr["value"] = comm_quant_mode
        __attrs__.append(attr)
    if group_list_type != None:
        attr = {}
        attr["name"] = "group_list_type"
        attr["dtype"] = "int"
        attr["value"] = group_list_type
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("MoeDistributeCombine", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def moe_distribute_combine(expand_x_in__, expert_ids_in__, expand_idx_in__, ep_send_counts_in__, expert_scales_in__, tp_send_counts_in__=None, x_active_mask_in__=None, activation_scale_in__=None, weight_scale_in__=None, group_list_in__=None, expand_scales_in__=None, x_out_=None, group_ep="", ep_world_size=0, ep_rank_id=0, moe_expert_num=0, group_tp="", tp_world_size=0, tp_rank_id=0, expert_shard_type=0, shared_expert_num=1, shared_expert_rank_num=0, global_bs=0, out_dtype=0, comm_quant_mode=0, group_list_type=0, kernel_name="moe_distribute_combine", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(expand_x_in__, expert_ids_in__, expand_idx_in__, ep_send_counts_in__, expert_scales_in__, tp_send_counts_in__, x_active_mask_in__, activation_scale_in__, weight_scale_in__, group_list_in__, expand_scales_in__, x_out_, group_ep, ep_world_size, ep_rank_id, moe_expert_num, group_tp, tp_world_size, tp_rank_id, expert_shard_type, shared_expert_num, shared_expert_rank_num, global_bs, out_dtype, comm_quant_mode, group_list_type)
    ...
    origin_func_name = "moe_distribute_combine"
    ascendc_src_dir_ex = "moe_distribute_combine"
    ascendc_src_dir = "moe_distribute_combine"
    ascendc_src_file = "moe_distribute_combine.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MoeDistributeCombine, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[expand_x_in__, expert_ids_in__, expand_idx_in__, ep_send_counts_in__, expert_scales_in__, tp_send_counts_in__, x_active_mask_in__, activation_scale_in__, weight_scale_in__, group_list_in__, expand_scales_in__], origin_outputs = [x_out_],\
                param_type_dynamic = False, mc2_ctx = ["mc2_context_0", "mc2_context_1"], param_type_list = ['required', 'required', 'required', 'required', 'required', 'optional', 'optional', 'optional', 'optional', 'optional', 'optional', 'required'], init_value_list = [None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{"auto_sync": false}')

def op_select_format(expand_x_in__, expert_ids_in__, expand_idx_in__, ep_send_counts_in__, expert_scales_in__, tp_send_counts_in__=None, x_active_mask_in__=None, activation_scale_in__=None, weight_scale_in__=None, group_list_in__=None, expand_scales_in__=None, x_out_=None, group_ep="", ep_world_size=0, ep_rank_id=0, moe_expert_num=0, group_tp="", tp_world_size=0, tp_rank_id=0, expert_shard_type=0, shared_expert_num=1, shared_expert_rank_num=0, global_bs=0, out_dtype=0, comm_quant_mode=0, group_list_type=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(expand_x_in__, expert_ids_in__, expand_idx_in__, ep_send_counts_in__, expert_scales_in__, tp_send_counts_in__, x_active_mask_in__, activation_scale_in__, weight_scale_in__, group_list_in__, expand_scales_in__, x_out_, group_ep, ep_world_size, ep_rank_id, moe_expert_num, group_tp, tp_world_size, tp_rank_id, expert_shard_type, shared_expert_num, shared_expert_rank_num, global_bs, out_dtype, comm_quant_mode, group_list_type)
    result = check_op_cap("op_select_format", "MoeDistributeCombine", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(expand_x_in__, expert_ids_in__, expand_idx_in__, ep_send_counts_in__, expert_scales_in__, tp_send_counts_in__=None, x_active_mask_in__=None, activation_scale_in__=None, weight_scale_in__=None, group_list_in__=None, expand_scales_in__=None, x_out_=None, group_ep="", ep_world_size=0, ep_rank_id=0, moe_expert_num=0, group_tp="", tp_world_size=0, tp_rank_id=0, expert_shard_type=0, shared_expert_num=1, shared_expert_rank_num=0, global_bs=0, out_dtype=0, comm_quant_mode=0, group_list_type=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(expand_x_in__, expert_ids_in__, expand_idx_in__, ep_send_counts_in__, expert_scales_in__, tp_send_counts_in__, x_active_mask_in__, activation_scale_in__, weight_scale_in__, group_list_in__, expand_scales_in__, x_out_, group_ep, ep_world_size, ep_rank_id, moe_expert_num, group_tp, tp_world_size, tp_rank_id, expert_shard_type, shared_expert_num, shared_expert_rank_num, global_bs, out_dtype, comm_quant_mode, group_list_type)
    result = check_op_cap("get_op_specific_info", "MoeDistributeCombine", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
