...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['u', 'm_in', 'eps', 'beta1', 'clip_threshold', 'sum_square_u', 'global_shape']
    output_names = ['m_out', 'sum_u_r', 'sum_u_c', 'sum_u_rc']
...
def _build_args(u_in__, m_in_in__, eps_in__, beta1_in__, clip_threshold_in__, sum_square_u_in__, global_shape_in__, m_out_out_, sum_u_r_out_, sum_u_c_out_, sum_u_rc_out_, use_first_moment):
    __inputs__ = []
    for arg in [u_in__, m_in_in__, eps_in__, beta1_in__, clip_threshold_in__, sum_square_u_in__, global_shape_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [m_out_out_, sum_u_r_out_, sum_u_c_out_, sum_u_rc_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
            ...
            __outputs__.append(arg)
    __attrs__ = []
    if use_first_moment != None:
        attr = {}
        attr["name"] = "use_first_moment"
        attr["dtype"] = "bool"
        attr["value"] = use_first_moment
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("ApplyCamePart3", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def apply_came_part3(u_in__, m_in_in__, eps_in__, beta1_in__, clip_threshold_in__, sum_square_u_in__, global_shape_in__=None, m_out_out_=None, sum_u_r_out_=None, sum_u_c_out_=None, sum_u_rc_out_=None, use_first_moment=False, kernel_name="apply_came_part3", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(u_in__, m_in_in__, eps_in__, beta1_in__, clip_threshold_in__, sum_square_u_in__, global_shape_in__, m_out_out_, sum_u_r_out_, sum_u_c_out_, sum_u_rc_out_, use_first_moment)
    ...
    origin_func_name = "apply_came_part3"
    ascendc_src_dir_ex = "apply_came_part3"
    ascendc_src_dir = "apply_came_part3"
    ascendc_src_file = "apply_came_part3.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator ApplyCamePart3, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "ApplyCamePart3"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[u_in__, m_in_in__, eps_in__, beta1_in__, clip_threshold_in__, sum_square_u_in__, global_shape_in__], origin_outputs = [m_out_out_, sum_u_r_out_, sum_u_c_out_, sum_u_rc_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required', 'required', 'optional', 'required', 'required', 'required', 'required'], init_value_list = [None, None, None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(u_in__, m_in_in__, eps_in__, beta1_in__, clip_threshold_in__, sum_square_u_in__, global_shape_in__=None, m_out_out_=None, sum_u_r_out_=None, sum_u_c_out_=None, sum_u_rc_out_=None, use_first_moment=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(u_in__, m_in_in__, eps_in__, beta1_in__, clip_threshold_in__, sum_square_u_in__, global_shape_in__, m_out_out_, sum_u_r_out_, sum_u_c_out_, sum_u_rc_out_, use_first_moment)
    result = check_op_cap("op_select_format", "ApplyCamePart3", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(u_in__, m_in_in__, eps_in__, beta1_in__, clip_threshold_in__, sum_square_u_in__, global_shape_in__=None, m_out_out_=None, sum_u_r_out_=None, sum_u_c_out_=None, sum_u_rc_out_=None, use_first_moment=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(u_in__, m_in_in__, eps_in__, beta1_in__, clip_threshold_in__, sum_square_u_in__, global_shape_in__, m_out_out_, sum_u_r_out_, sum_u_c_out_, sum_u_rc_out_, use_first_moment)
    result = check_op_cap("get_op_specific_info", "ApplyCamePart3", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
