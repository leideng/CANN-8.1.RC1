...
def _build_args(input_in__, target_in__, output_out_, reduction):
    __inputs__ = []
    for arg in [input_in__, target_in__]:
        if arg != None:
...
@tbe_register.register_operator("MSELossV2", trans_bool_to_s8=False)
...
def mse_loss_v2(input_in__, target_in__, output_out_, reduction="mean", kernel_name="mse_loss_v2", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(input_in__, target_in__, output_out_, reduction)
    ...
    origin_func_name = "mse_loss_v2"
    ascendc_src_dir_ex = "mse_loss_v2"
    ascendc_src_dir = "m_s_e_loss_v2"
    ascendc_src_file = "mse_loss_v2.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MSELossV2, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "MSELossV2"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[input_in__, target_in__], origin_outputs = [output_out_],\
...
def op_select_format(input_in__, target_in__, output_out_, reduction="mean", impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(input_in__, target_in__, output_out_, reduction)
    result = check_op_cap("op_select_format", "MSELossV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(input_in__, target_in__, output_out_, reduction="mean", impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(input_in__, target_in__, output_out_, reduction)
    result = check_op_cap("get_op_specific_info", "MSELossV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
