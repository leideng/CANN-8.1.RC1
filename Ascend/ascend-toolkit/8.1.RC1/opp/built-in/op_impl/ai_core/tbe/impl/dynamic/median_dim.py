...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['self']
    output_names = ['valuesOut', 'indicesOut']
...
def _build_args(self_in__, valuesOut_out_, indicesOut_out_, dim, keepdim):
    __inputs__ = []
    for arg in [self_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [valuesOut_out_, indicesOut_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("MedianDim", trans_bool_to_s8=False)
...
def median_dim(self_in__, valuesOut_out_, indicesOut_out_, dim=-1, keepdim=False, kernel_name="median_dim", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(self_in__, valuesOut_out_, indicesOut_out_, dim, keepdim)
    ...
    origin_func_name = "median_dim"
    ascendc_src_dir_ex = "median_dim"
    ascendc_src_dir = "median_dim"
    ascendc_src_file = "median_dim.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MedianDim, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "MedianDim"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[self_in__], origin_outputs = [valuesOut_out_, indicesOut_out_],\
...
def op_select_format(self_in__, valuesOut_out_, indicesOut_out_, dim=-1, keepdim=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(self_in__, valuesOut_out_, indicesOut_out_, dim, keepdim)
    result = check_op_cap("op_select_format", "MedianDim", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(self_in__, valuesOut_out_, indicesOut_out_, dim=-1, keepdim=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(self_in__, valuesOut_out_, indicesOut_out_, dim, keepdim)
    result = check_op_cap("get_op_specific_info", "MedianDim", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
