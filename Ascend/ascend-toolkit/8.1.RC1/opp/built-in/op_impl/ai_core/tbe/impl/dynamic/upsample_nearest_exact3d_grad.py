...
def _build_args(grad_output_in__, y_out_, input_size, output_size, scales):
    __inputs__ = []
...
@tbe_register.register_operator("UpsampleNearestExact3dGrad", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_LIST_FLOAT, para_check.KERNEL_NAME)
def upsample_nearest_exact3d_grad(grad_output_in__, y_out_, input_size, output_size=[0,0,0], scales=[0,0,0], kernel_name="upsample_nearest_exact3d_grad", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(grad_output_in__, y_out_, input_size, output_size, scales)
    ...
    origin_func_name = "upsample_nearest_exact3d_grad"
    ascendc_src_dir_ex = "upsample_nearest_exact3d_grad"
    ascendc_src_dir = "upsample_nearest_exact3d_grad"
    ascendc_src_file = "upsample_nearest_exact3d_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator UpsampleNearestExact3dGrad, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "UpsampleNearestExact3dGrad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[grad_output_in__], origin_outputs = [y_out_],\
...
def op_select_format(grad_output_in__, y_out_, input_size, output_size=[0,0,0], scales=[0,0,0], impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_output_in__, y_out_, input_size, output_size, scales)
    result = check_op_cap("op_select_format", "UpsampleNearestExact3dGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(grad_output_in__, y_out_, input_size, output_size=[0,0,0], scales=[0,0,0], impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_output_in__, y_out_, input_size, output_size, scales)
    result = check_op_cap("get_op_specific_info", "UpsampleNearestExact3dGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
