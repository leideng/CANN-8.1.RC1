...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'gx', 'beta', 'gamma']
    output_names = ['mean', 'rstd', 'y']
...
def _build_args(x_in__, gx_in__, beta_in__, gamma_in__, mean_out_, rstd_out_, y_out_, alpha, epsilon):
    __inputs__ = []
    for arg in [x_in__, gx_in__, beta_in__, gamma_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [mean_out_, rstd_out_, y_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("DeepNorm", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def deep_norm(x_in__, gx_in__, beta_in__, gamma_in__, mean_out_, rstd_out_, y_out_, alpha=0.3, epsilon=1e-06, kernel_name="deep_norm", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, gx_in__, beta_in__, gamma_in__, mean_out_, rstd_out_, y_out_, alpha, epsilon)
    ...
    origin_func_name = "deep_norm"
    ascendc_src_dir_ex = "deep_norm"
    ascendc_src_dir = "deep_norm"
    ascendc_src_file = "deep_norm.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator DeepNorm, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "DeepNorm"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, gx_in__, beta_in__, gamma_in__], origin_outputs = [mean_out_, rstd_out_, y_out_],\
...
def op_select_format(x_in__, gx_in__, beta_in__, gamma_in__, mean_out_, rstd_out_, y_out_, alpha=0.3, epsilon=1e-06, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, gx_in__, beta_in__, gamma_in__, mean_out_, rstd_out_, y_out_, alpha, epsilon)
    result = check_op_cap("op_select_format", "DeepNorm", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, gx_in__, beta_in__, gamma_in__, mean_out_, rstd_out_, y_out_, alpha=0.3, epsilon=1e-06, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, gx_in__, beta_in__, gamma_in__, mean_out_, rstd_out_, y_out_, alpha, epsilon)
    result = check_op_cap("get_op_specific_info", "DeepNorm", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
