...
"""
lp_norm_v2
The difference from lp_norm is that the attribute `p` is float type
"""
...
def tensor_pow(tensor, power):
    """
    calculate tensor power
    """
    tensor = method_check_and_cast("tbe.dsl.vlog", tensor.dtype, tensor)
    if not tbe_platform.api_check_support("tbe.dsl.vlog", "float32"):  # soc only support high performance mode
        ...
        log_value = tbe.vlog(tensor, impl_mode=OpTbeImplMode.TBE_HIGH_PRECISION)
    log_value = tbe.vmuls(log_value, tvm.const(power, log_value.dtype))
    res = tbe.vexp(log_value)
    return res


# 'pylint: disable=invalid-name,unused-argument,too-many-locals,too-many-arguments,huawei-too-many-arguments
def lp_norm_v2_inf_compute(abs_x, x_type, y, p, axes, keepdim, kernel_name):
    """
...
# 'pylint: disable=invalid-name,unused-argument,too-many-locals,too-many-arguments,huawei-too-many-arguments
def lp_norm_v2_p0_compute(abs_x, x_type, y, axes, keepdim, kernel_name):
    """
    Compute norm for p = 0.
    p == 0, lp_norm equals the number of nonzero-elements
    """
...
# 'pylint: disable=invalid-name,unused-argument,too-many-locals,too-many-arguments,huawei-too-many-arguments
def lp_norm_v2_p1_compute(abs_x, x_type, y, axes, keepdim, kernel_name):
    """
...
# 'pylint: disable=invalid-name,unused-argument,too-many-locals,huawei-too-many-arguments
def lp_norm_v2_p2_compute(abs_x, x_type, y, axes, keepdim, kernel_name):
    """
    Compute norm for p = 2.0.
    p == 2.0, lp_norm = sqrt(sum(abs(x) ^ 2))
    ...
    abs_x = method_check_and_cast("tbe.dsl.vsqrt", x_type, abs_x)
    res = tbe.vsqrt(sum_pow, impl_mode=OpTbeImplMode.TBE_HIGH_PRECISION)
    return res


# 'pylint: disable=invalid-name,unused-argument,too-many-locals,too-many-arguments,huawei-too-many-arguments
def _lp_norm_v2_compute(abs_x, x_type, y, p, axes, keepdim, kernel_name):
    """
    Compute norm for other p value.
    When p equals other value, lp_norm = pow(sum(pow(abs(input),p)),1/p).
    """
    if math.isclose(int(p), p) and p >= Constant.CONST_LOOP_MIN and p <= Constant.CONST_LOOP_MAX:
        ...
        pow_x = tensor_pow(abs_x, p)
    sum_pow_x = tbe.reduce_sum(pow_x, axis=axes, keepdims=keepdim)
    res = tensor_pow(sum_pow_x, 1 / p)
    return res


# 'pylint: disable=invalid-name,unused-argument,too-many-locals,too-many-arguments,huawei-too-many-arguments
@register_operator_compute("LpNormV2", op_mode="dynamic", support_fusion=False)
def lp_norm_v2_compute(input_data, x_type, y, p, axes, keepdim, epsilon, kernel_name):
    """
    ...
    if x_type == "bfloat16":
        input_data = tbe.cast_to(input_data, dtype="float32")
    abs_data = tbe.vabs(input_data)

    if math.isclose(p, 2.0):
        res = lp_norm_v2_p2_compute(abs_data, x_type, y, axes, keepdim, kernel_name)
    elif math.isclose(p, 1.0):
        res = lp_norm_v2_p1_compute(abs_data, x_type, y, axes, keepdim, kernel_name)
    elif math.isclose(p, 0.0):
        res = lp_norm_v2_p0_compute(abs_data, x_type, y, axes, keepdim, kernel_name)
    elif p in p_inf_list or math.isclose(p, Constant.CONST_DEC_INF) or math.isclose(p, Constant.CONST_DEC_NEG_INF):
        res = lp_norm_v2_inf_compute(abs_data, x_type, y, p, axes, keepdim, kernel_name)
    else:
        res = _lp_norm_v2_compute(abs_data, x_type, y, p, axes, keepdim, kernel_name)
...
# 'pylint: disable=too-many-arguments,too-many-branches,huawei-too-many-arguments
@register_operator("LpNormV2")
...
def lp_norm_v2(x, y, p=2.0, axes=None, keepdim=False, epsilon=1e-12, kernel_name="lp_norm_v2"):
    """
    Computes norm for p equals 0, 1, 2, -inf, inf, or other float value.
    p == 2.0 (default)      sqrt(sum(abs(x) ^ 2))
    p == inf                max(abs(x))
    p == -inf               min(abs(x))
    p == 0                  sum(x != 0), number of non zero elements
    p == 1.0                sum(abs(x))
    other int or float      sum(abs(x) ^ {p}) ^ {1/p}
    ...
    kernel_name: str
                 Kernel name.
                 Optional. Default: "lp_norm_v2".
    Returns
    -------
    None
    """
    para_check.check_kernel_name(kernel_name)
    type_list = ["float16", "float32", "bfloat16"]
    ...
    if is_unknown_rank_input(x) and check_no_axes(axes):
        input_axis = {"shape": [-1], "rel_pos_to_reduce": "axis"}
    else:
        no_shape = len(x_shape)
        if check_no_axes(axes):
    ...
    for (_x, axes_dict) in ins:
        with tbe.compute():
            ...
            res = lp_norm_v2_compute(input_data, x_type, y,
                                     p, axes_dict.get("value"), keepdim, epsilon, kernel_name)
