...
/*!
 * \file LstmFP32Seq.cpp
 * \brief
 */
#include "LstmFP32Seq.h"

using namespace AscendC;

template <typename T>
__aicore__ inline int64_t LstmMmSplitNDNDFP32Seq<T>::Ceil(int64_t x, int64_t y) {
  return (x + y - 1) / y;
}

template <typename T>
__aicore__ inline void LstmMmSplitNDNDFP32Seq<T>::Init(GM_ADDR inputX, GM_ADDR weight, GM_ADDR bias, GM_ADDR seqLength,
                                                       GM_ADDR initH, GM_ADDR initC, GM_ADDR wCi, GM_ADDR wCf,
                                                       GM_ADDR wCo, GM_ADDR mask, GM_ADDR outputY, GM_ADDR outputH,
...
template <typename T>
__aicore__ inline void LstmMmSplitNDNDFP32Seq<T>::InitQue() {
  pipe.InitBuffer(qidVecIn, 1, baseVector * sizeof(float));
  pipe.InitBuffer(qidVecOut, 1, baseVector * sizeof(T));
  pipe.InitBuffer(calcBuf, 6 * baseVector * sizeof(float));
  // Init Local Tensors
  ubLocal1 = calcBuf.Get<float>(6 * baseVector);
  ubLocal2 = ubLocal1[baseVector];
  ubLocal3 = ubLocal2[baseVector];
  ubLocal4 = ubLocal3[baseVector];
  ubLocal5 = ubLocal4[baseVector];
  ubLocal6 = ubLocal5[baseVector];
}

template <typename T>
__aicore__ inline void LstmMmSplitNDNDFP32Seq<T>::GetCoreIndex(TCubeTiling& param, int32_t& subKIndx,
...
template <typename T>
__aicore__ inline void LstmMmSplitNDNDFP32Seq<T>::CalcGMOffset(TCubeTiling& param, TRnnOffsets& offset,
                                                               tailSize& mmTail, int64_t cNdFormat, int32_t kSize) {
  int32_t subKIndx;
...
template <typename T>
__aicore__ inline void LstmMmSplitNDNDFP32Seq<T>::InitBuffers(GM_ADDR inputX, GM_ADDR weight, GM_ADDR bias,
                                                              ...
                                                              GM_ADDR outputO, GM_ADDR outputTanhC, GM_ADDR workspace) {
  ...
  allCellSize = tiling->batch * tiling->hiddenSize * LSTM_GATE_SIZE;
...
template <typename T>
__aicore__ inline void LstmMmSplitNDNDFP32Seq<T>::InitVars() {
...
template <typename T>
__aicore__ inline void LstmMmSplitNDNDFP32Seq<T>::ProcessInputMM() {
...
template <typename T>
__aicore__ inline void LstmMmSplitNDNDFP32Seq<T>::ProcessHiddenMM(int64_t tIdx) {
  if (GetBlockIdx() < tiling->hiddenMMParam.usedCoreNum) {
    ...
    hiddenMM.SetTensorB(inputGm.weightGm[hiddenOffsets.BOffset]);
...
template <typename T>
__aicore__ inline void LstmMmSplitNDNDFP32Seq<T>::CopyGate(LocalTensor<T>& ub, GlobalTensor<T>& gm, int64_t mIdx,
                                                           int64_t gateOffset) {
...
template <typename T>
__aicore__ inline void LstmMmSplitNDNDFP32Seq<T>::CopyWithSigmoid(LocalTensor<T>& dstUb, GlobalTensor<T>& mixGm,
...
template <typename T>
__aicore__ inline void LstmMmSplitNDNDFP32Seq<T>::CopyWithTanh(LocalTensor<T>& dstUb, GlobalTensor<T>& mixGm,
                                                               int64_t mIdx, int64_t gateOffset) {
  LocalTensor<T> ubLocalIn = qidVecIn.AllocTensor<T>();
  CopyGate(ubLocalIn, mixGm, mIdx, gateOffset);
  ubLocalIn = qidVecIn.DeQue<T>();
...
template <typename T>
__aicore__ inline void LstmMmSplitNDNDFP32Seq<T>::CopyWithMul(LocalTensor<T>& dstUb, LocalTensor<T>& other,
...
template <typename T>
__aicore__ inline void LstmMmSplitNDNDFP32Seq<T>::CopyOutput(GlobalTensor<T>& gm, LocalTensor<T>& ub, int64_t tIdx,
                                                             int64_t mIdx) {
...
template <typename T>
__aicore__ inline void LstmMmSplitNDNDFP32Seq<T>::CalOutYWithSeq(LocalTensor<float>& ubLocal,
                                                                 LocalTensor<float>& seqLocal, int64_t size) {
  pipe_barrier(PIPE_V);
  Mul(ubLocal, ubLocal, seqLocal, size);
}

template <typename T>
__aicore__ inline void LstmMmSplitNDNDFP32Seq<T>::CalOutHWithSeq(LocalTensor<float>& yLocal,
                                                                 LocalTensor<float>& hPrevLocal,
                                                                 LocalTensor<float>& dstLocal,
                                                                 LocalTensor<float>& seqLocal, int64_t size) {
  pipe_barrier(PIPE_V);
  Sub(yLocal, yLocal, hPrevLocal, size);
  pipe_barrier(PIPE_V);
  Mul(yLocal, yLocal, seqLocal, size);
  pipe_barrier(PIPE_V);
  Add(dstLocal, yLocal, hPrevLocal, size);
}

template <typename T>
__aicore__ inline void LstmMmSplitNDNDFP32Seq<T>::CalcNormalOutCWithSeq(LocalTensor<float>& cLocal,
                                                                        LocalTensor<float>& cPrevLocal,
                                                                        LocalTensor<float>& seqLocal, int64_t size) {
  pipe_barrier(PIPE_V);
  Sub(cLocal, cLocal, cPrevLocal, size);
  pipe_barrier(PIPE_V);
  Mul(cLocal, cLocal, seqLocal, size);
  pipe_barrier(PIPE_V);
  Add(cLocal, cLocal, cPrevLocal, size);
  pipe_barrier(PIPE_V);
}

template <typename T>
__aicore__ inline void LstmMmSplitNDNDFP32Seq<T>::CopyInLastHC(LocalTensor<float>& dstUb, GlobalTensor<T>& outGlobal,
                                                               int64_t mIdx) {
  LocalTensor<T> ubLocalIn = qidVecIn.AllocTensor<T>();
  DataCopy(ubLocalIn, outGlobal[blockIdx * vectorCoreM * tiling->hiddenSize + mIdx * vectorBaseM * tiling->hiddenSize],
           calcSizeAlign);
  qidVecIn.EnQue(ubLocalIn);
  ubLocalIn = qidVecIn.DeQue<T>();

  Muls(dstUb, ubLocalIn, (float)1.0, calcSizeAlign);
  pipe_barrier(PIPE_V);
  qidVecIn.FreeTensor(ubLocalIn);
}

template <typename T>
__aicore__ inline void LstmMmSplitNDNDFP32Seq<T>::CopyInSeqLength(LocalTensor<float>& dstUb, int64_t mIdx,
                                                                  int64_t tIdx) {
  int64_t offset;
  if (tiling->direction == 1) {
    offset = (tiling->timeStep - 1 - tIdx) * tiling->batch * tiling->hiddenSize +
  ...
  LocalTensor<T> ubLocalIn = qidVecIn.AllocTensor<T>();
  DataCopy(ubLocalIn, inputGm.seqLengthGm[offset], calcSizeAlign);
  qidVecIn.EnQue(ubLocalIn);
  ubLocalIn = qidVecIn.DeQue<T>();
...
template <typename T>
__aicore__ inline void LstmMmSplitNDNDFP32Seq<T>::ProcessVectorOnce(int64_t tIdx, int64_t mIdx,
  ...
  CopyInSeqLength(ubLocal5, mIdx, tIdx);
  ...
  // [1] 2 3 4 -> 1 [2] 3 4
  auto cTmp1 = ubLocal2;
  CopyInLastHC(ubLocal6, inputGm.initCGm, mIdx);
  Mul(cTmp1, ubLocal6, fSigmoid, calcSizeAlign);
  // i 1 [2] 3 4 -> [1] [2] 3 4
  ...
  CalcNormalOutCWithSeq(updateC, ubLocal6, ubLocal5, calcSizeAlign);
  ...
  auto updateY = ubLocal3;
  Mul(updateY, oSigmoid, cTanh, calcSizeAlign);
  CalOutYWithSeq(updateY, ubLocal5, calcSizeAlign);
  pipe_barrier(PIPE_V);
  CopyOutput(outputGm.outYGm, updateY, tIdx, mIdx);
  auto updateH = ubLocal4;
  CopyInLastHC(ubLocal6, inputGm.initHGm, mIdx);
  CalOutHWithSeq(updateY, ubLocal6, updateH, ubLocal5, calcSizeAlign);
  CopyOutput(outputGm.outHGm, updateH, tIdx, mIdx);
}

template <typename T>
__aicore__ inline void LstmMmSplitNDNDFP32Seq<T>::ProcessVector(int64_t tIdx) {
...
template <typename T>
__aicore__ inline void LstmMmSplitNDNDFP32Seq<T>::Process() {
  ProcessInputMM();
