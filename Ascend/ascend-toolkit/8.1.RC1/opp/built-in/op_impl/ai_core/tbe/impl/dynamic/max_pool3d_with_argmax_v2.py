...
def _build_args(x_in__, y_out_, argmax_out_, ksize, strides, pads, dilation, ceil_mode, data_format):
    __inputs__ = []
    for arg in [x_in__]:
        if arg != None:
...
@tbe_register.register_operator("MaxPool3DWithArgmaxV2", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_STR, para_check.KERNEL_NAME)
def max_pool3d_with_argmax_v2(x_in__, y_out_, argmax_out_, ksize, strides, pads, dilation=[1,1,1], ceil_mode=False, data_format="NCDHW", kernel_name="max_pool3d_with_argmax_v2", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, argmax_out_, ksize, strides, pads, dilation, ceil_mode, data_format)
    ...
    origin_func_name = "max_pool3d_with_argmax_v2"
    ascendc_src_dir_ex = "max_pool3_d_with_argmax_v2"
    ascendc_src_dir = "max_pool3_d_with_argmax_v2"
    ascendc_src_file = "max_pool3d_with_argmax_v2.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator MaxPool3DWithArgmaxV2, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "MaxPool3DWithArgmaxV2"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__], origin_outputs = [y_out_, argmax_out_],\
