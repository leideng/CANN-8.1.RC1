...
def _build_args(x_in__, y_out_, diagonal):
    __inputs__ = []
    for arg in [x_in__]:
        if arg != None:
...
@tbe_register.register_operator("DiagFlat", trans_bool_to_s8=False)
...
def diag_flat(x_in__, y_out_, diagonal=0, kernel_name="diag_flat", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, diagonal)
    ...
    origin_func_name = "diag_flat"
    ascendc_src_dir_ex = "diag_flat"
    ascendc_src_dir = "diag_flat"
    ascendc_src_file = "diag_flat.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator DiagFlat, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "DiagFlat"
...
def op_select_format(x_in__, y_out_, diagonal=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, diagonal)
    result = check_op_cap("op_select_format", "DiagFlat", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, y_out_, diagonal=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, diagonal)
    result = check_op_cap("get_op_specific_info", "DiagFlat", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
