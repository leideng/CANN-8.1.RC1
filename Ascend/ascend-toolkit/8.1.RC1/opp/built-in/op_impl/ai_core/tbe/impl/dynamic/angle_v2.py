...
@tbe_register.register_operator("AngleV2", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.KERNEL_NAME)
def angle_v2(x_in__, y_out_, kernel_name="angle_v2", impl_mode=""):
    ...
    origin_func_name = "angle_v2"
    ascendc_src_dir_ex = "angle_v2"
    ascendc_src_dir = "angle_v2"
    ascendc_src_file = "angle_v2.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator AngleV2, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "AngleV2"
...
def op_select_format(x_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_)
    result = check_op_cap("op_select_format", "AngleV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_)
    result = check_op_cap("get_op_specific_info", "AngleV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
