...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x', 'weight', 'bias', 'group_list']
    output_names = ['y']
...
def _build_args(x_in__, weight_in__, bias_in__, group_list_in__, y_out_, splitItem, group, reduceOp, commTurn):
    __inputs__ = []
    for arg in [x_in__, weight_in__, bias_in__, group_list_in__]:
        if arg != None:
...
@tbe_register.register_operator("GroupedMatMulAllReduce", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.DYNAMIC_INPUT, para_check.DYNAMIC_INPUT, para_check.DYNAMIC_INPUT, para_check.OPTION_INPUT, para_check.DYNAMIC_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def grouped_mat_mul_all_reduce(x_in__, weight_in__, bias_in__, group_list_in__=None, y_out_=None, splitItem=0, group="", reduceOp="sum", commTurn=0, kernel_name="grouped_mat_mul_all_reduce", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, bias_in__, group_list_in__, y_out_, splitItem, group, reduceOp, commTurn)
    ...
    origin_func_name = "grouped_mat_mul_all_reduce"
    ascendc_src_dir_ex = "grouped_mat_mul_all_reduce"
    ascendc_src_dir = "grouped_mat_mul_all_reduce"
    ascendc_src_file = "grouped_mat_mul_all_reduce.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator GroupedMatMulAllReduce, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "GroupedMatMulAllReduce"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, weight_in__, bias_in__, group_list_in__], origin_outputs = [y_out_],\
                param_type_dynamic = True, mc2_ctx = ["mc2_context_0"], param_type_list = ['dynamic', 'dynamic', 'dynamic', 'optional', 'dynamic'], init_value_list = [None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{"auto_sync": false}')

def op_select_format(x_in__, weight_in__, bias_in__, group_list_in__=None, y_out_=None, splitItem=0, group="", reduceOp="sum", commTurn=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, bias_in__, group_list_in__, y_out_, splitItem, group, reduceOp, commTurn)
    result = check_op_cap("op_select_format", "GroupedMatMulAllReduce", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, weight_in__, bias_in__, group_list_in__=None, y_out_=None, splitItem=0, group="", reduceOp="sum", commTurn=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, bias_in__, group_list_in__, y_out_, splitItem, group, reduceOp, commTurn)
    result = check_op_cap("get_op_specific_info", "GroupedMatMulAllReduce", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
