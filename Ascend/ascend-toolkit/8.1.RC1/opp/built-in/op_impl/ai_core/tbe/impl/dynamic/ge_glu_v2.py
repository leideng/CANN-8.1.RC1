...
def _build_args(x_in__, y_out_, gelu_out_, dim, approximate, activate_left):
    __inputs__ = []
    for arg in [x_in__]:
        if arg != None:
...
@tbe_register.register_operator("GeGluV2", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def ge_glu_v2(x_in__, y_out_, gelu_out_, dim=-1, approximate=1, activate_left=False, kernel_name="ge_glu_v2", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, gelu_out_, dim, approximate, activate_left)
    ...
    origin_func_name = "ge_glu_v2"
    ascendc_src_dir_ex = "ge_glu_v2"
    ascendc_src_dir = "ge_glu_v2"
    ascendc_src_file = "ge_glu_v2.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator GeGluV2, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "GeGluV2"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__], origin_outputs = [y_out_, gelu_out_],\
...
def op_select_format(x_in__, y_out_, gelu_out_, dim=-1, approximate=1, activate_left=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, gelu_out_, dim, approximate, activate_left)
    result = check_op_cap("op_select_format", "GeGluV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, y_out_, gelu_out_, dim=-1, approximate=1, activate_left=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, gelu_out_, dim, approximate, activate_left)
    result = check_op_cap("get_op_specific_info", "GeGluV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
