...
class CorrectBoxComputer():
    ...
    def __init__(self, input_dict):
        ...
        self.one_max_size = (tbe_platform.get_soc_spec(tbe_platform.UB_SIZE) - RESERVE_SIZE) // 8
    ...
    def t_small_mov_to_gm(self, batch, param):
        ...
        if (param['h'] * param['w'] * self.dsize) % constant.BLOCK_SIZE != 0:
            with self.instance.if_scope(param['box_id'] == self.boxes - 1):
                ...
                if tbe_platform.get_soc_spec("SHORT_SOC_VERSION") not in ("Ascend310B", ):
                    ...
                    self.instance.data_move(param['last_32b'], param['ub_b'][tail_idx], 0, 1, 1, 0, 0)
        # move y to gm
