...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['unique_len', 'unique_indices', 'indices', 'values']
    output_names = ['new_indices', 'new_values']
...
def _build_args(unique_len_in__, unique_indices_in__, indices_in__, values_in__, new_indices_out_, new_values_out_):
    __inputs__ = []
    for arg in [unique_len_in__, unique_indices_in__, indices_in__, values_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [new_indices_out_, new_values_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("CoalesceSparse", trans_bool_to_s8=False)
...
def coalesce_sparse(unique_len_in__, unique_indices_in__, indices_in__, values_in__, new_indices_out_, new_values_out_, kernel_name="coalesce_sparse", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(unique_len_in__, unique_indices_in__, indices_in__, values_in__, new_indices_out_, new_values_out_)
    ...
    origin_func_name = "coalesce_sparse"
    ascendc_src_dir_ex = "coalesce_sparse"
    ascendc_src_dir = "coalesce_sparse"
    ascendc_src_file = "coalesce_sparse.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator CoalesceSparse, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "CoalesceSparse"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[unique_len_in__, unique_indices_in__, indices_in__, values_in__], origin_outputs = [new_indices_out_, new_values_out_],\
...
def op_select_format(unique_len_in__, unique_indices_in__, indices_in__, values_in__, new_indices_out_, new_values_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(unique_len_in__, unique_indices_in__, indices_in__, values_in__, new_indices_out_, new_values_out_)
    result = check_op_cap("op_select_format", "CoalesceSparse", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(unique_len_in__, unique_indices_in__, indices_in__, values_in__, new_indices_out_, new_values_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(unique_len_in__, unique_indices_in__, indices_in__, values_in__, new_indices_out_, new_values_out_)
    result = check_op_cap("get_op_specific_info", "CoalesceSparse", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
