...
def _build_args(x_in__, smooth_scales_in__, group_index_in__, y_out_, scale_out_, dst_type):
    __inputs__ = []
...
@tbe_register.register_operator("DynamicQuant", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def dynamic_quant(x_in__, smooth_scales_in__=None, group_index_in__=None, y_out_=None, scale_out_=None, dst_type=2, kernel_name="dynamic_quant", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, smooth_scales_in__, group_index_in__, y_out_, scale_out_, dst_type)
    ...
    origin_func_name = "dynamic_quant"
    ascendc_src_dir_ex = "dynamic_quant"
    ascendc_src_dir = "dynamic_quant"
    ascendc_src_file = "dynamic_quant.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator DynamicQuant, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, smooth_scales_in__, group_index_in__], origin_outputs = [y_out_, scale_out_],\
...
def op_select_format(x_in__, smooth_scales_in__=None, group_index_in__=None, y_out_=None, scale_out_=None, dst_type=2, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, smooth_scales_in__, group_index_in__, y_out_, scale_out_, dst_type)
    result = check_op_cap("op_select_format", "DynamicQuant", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, smooth_scales_in__=None, group_index_in__=None, y_out_=None, scale_out_=None, dst_type=2, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, smooth_scales_in__, group_index_in__, y_out_, scale_out_, dst_type)
    result = check_op_cap("get_op_specific_info", "DynamicQuant", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
