...
def get_op_support_info(input_dy, input_x, input_variance, input_mean, output_pd_gamma, output_pd_beta, shape_gamma,
...
def _get_pd_var_front(data, dtype):
    ...
    """
    var_elta = tbe.vadds(data.get("data_variance"), tvm.const(constant.EPSLON, dtype=dtype))
    var_elta_log = tbe.vlog(var_elta)
    var_elta_mul = tbe.vmuls(var_elta_log, tvm.const(-0.5, dtype=dtype))
...
def _get_pd_mean(params, pd_xl, pd_var, var_elta_2, sub_x_mean):
    ...
    pd_mean_1 = tbe.vmuls(pdmean1_mul, tvm.const(-1.0, dtype="float32"))

    pdmean2_mul1 = tbe.vmuls(sub_x_mean, tvm.const(-2.0, dtype="float32"))
    pdmean2_sum = tbe.reduce_sum(pdmean2_mul1, params.get("reduce_axis"), keepdims=True)
    pdmean2_mul3 = tbe.vmuls(pdmean2_sum, tvm.const((params.get("mean_num")**(-1)), dtype="float32"))
...
def _get_pd_beta(data, params, dtype):
    ...
    if params.get("param_axis"):
        ...
        pd_beta = tbe.vadds(data.get("data_dy"), tvm.const(0, dtype=dtype))
...
def _get_res(data, params, shape_x, dtype):
    ...
    var_elta_2_cast = tbe.cast_to(var_elta_2_cast, sub_x_mean.dtype)
    xl_mul = tbe.vmul(var_elta_2_cast, sub_x_mean)
    pdga_mul = tbe.vmul(data.get("data_dy"), xl_mul)
