...
def _build_args(start_in__, stop_in__, num_in__, output_out_):
    __inputs__ = []
    for arg in [start_in__, stop_in__, num_in__]:
        if arg != None:
...
@tbe_register.register_operator("LinSpace", trans_bool_to_s8=False)
...
def lin_space(start_in__, stop_in__, num_in__, output_out_, kernel_name="lin_space", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(start_in__, stop_in__, num_in__, output_out_)
    ...
    origin_func_name = "lin_space"
    ascendc_src_dir_ex = "lin_space"
    ascendc_src_dir = "lin_space"
    ascendc_src_file = "lin_space.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator LinSpace, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "LinSpace"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[start_in__, stop_in__, num_in__], origin_outputs = [output_out_],\
...
def op_select_format(start_in__, stop_in__, num_in__, output_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(start_in__, stop_in__, num_in__, output_out_)
    result = check_op_cap("op_select_format", "LinSpace", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(start_in__, stop_in__, num_in__, output_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(start_in__, stop_in__, num_in__, output_out_)
    result = check_op_cap("get_op_specific_info", "LinSpace", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
