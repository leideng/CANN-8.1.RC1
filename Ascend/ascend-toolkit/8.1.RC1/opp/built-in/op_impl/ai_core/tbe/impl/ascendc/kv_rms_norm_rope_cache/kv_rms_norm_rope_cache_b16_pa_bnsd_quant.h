 ...
 public:
  ...
  __aicore__ inline void Process() {
    ...
    for (int64_t loopIdx = 0; loopIdx < ubLoop; ++loopIdx) {
      ...
      RoPEWithQuant<KV_DTYPE, true, ROPE_LENGTH>(outLocal, quantLocal, ropeLocal, cosLocal, sinLocal, kRopeScaleLocal, workspaceBuffer, ubFactor);
      ...
      ScatterUpdate<int8_t, KV_DTYPE, isPagedAttention, ROPE_LENGTH>(kCacheGm, kCacheGmNd, outLocal, quantLocal, startIdx, ubFactor);
      ...
      RmsNormWithQuant<KV_DTYPE, RMS_NORM_LENGTH>(outLocal, quantLocal, xLocal, gammaLocalFp32, cKvScaleLocal, workspaceBuffer, ubFactor);
      ...
      ScatterUpdate<int8_t, KV_DTYPE, isPagedAttention, RMS_NORM_LENGTH>(vCacheGm, vCacheGmNd, outLocal, quantLocal, startIdx, ubFactor);
      outQueue.FreeTensor(outLocal);
    }
    if (ubTail > 0) {
      ...
      RoPEWithQuant<KV_DTYPE, true, ROPE_LENGTH>(outLocal, quantLocal, ropeLocal, cosLocal, sinLocal, kRopeScaleLocal, workspaceBuffer, ubTail);
      inQueueX.FreeTensor(ropeLocal);
      outQueue.EnQue(outLocal);
      outLocal = outQueue.DeQue<KV_DTYPE>();
      quantLocal = outLocal.template ReinterpretCast<int8_t>()[ubTail * ROPE_LENGTH * sizeof(KV_DTYPE)];
      // Scatter Update kCache
      ScatterUpdate<int8_t, KV_DTYPE, isPagedAttention, ROPE_LENGTH>(kCacheGm, kCacheGmNd, outLocal, quantLocal, startIdx, ubTail);
      outQueue.FreeTensor(outLocal);
      ...
      RmsNormWithQuant<KV_DTYPE, RMS_NORM_LENGTH>(outLocal, quantLocal, xLocal, gammaLocalFp32, cKvScaleLocal, workspaceBuffer, ubTail);
      inQueueX.FreeTensor(xLocal);
      outQueue.EnQue(outLocal);
      outLocal = outQueue.DeQue<KV_DTYPE>();
      quantLocal = outLocal.template ReinterpretCast<int8_t>()[ubTail * RMS_NORM_LENGTH * sizeof(KV_DTYPE)];
      ...
      // Scatter Update vCache
      ScatterUpdate<int8_t, KV_DTYPE, isPagedAttention, RMS_NORM_LENGTH>(vCacheGm, vCacheGmNd, outLocal, quantLocal, startIdx, ubTail);
      outQueue.FreeTensor(outLocal);
    }
  ...
  __aicore__ inline void RoPEWithQuant(const LocalTensor<T>& outLocal,
  ...
  __aicore__ inline void ScatterUpdate(const GlobalTensor<T1>& dst,
                                       const GlobalTensor<T2>& dstNd,
                                       const LocalTensor<T2>& outLocal,
                                       const LocalTensor<T1>& quantLocal,
  ...
  __aicore__ inline void RmsNormWithQuant(const LocalTensor<T>& outLocal,
                                 const LocalTensor<int8_t>& quantLocal,
                                 const LocalTensor<T>& xLocal,
