...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['xyz1', 'xyz2', 'idx1', 'idx2', 'grad_dist1', 'grad_dist2']
    output_names = ['grad_xyz1', 'grad_xyz2']
...
def _build_args(xyz1_in__, xyz2_in__, idx1_in__, idx2_in__, grad_dist1_in__, grad_dist2_in__, grad_xyz1_out_, grad_xyz2_out_):
    __inputs__ = []
    for arg in [xyz1_in__, xyz2_in__, idx1_in__, idx2_in__, grad_dist1_in__, grad_dist2_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [grad_xyz1_out_, grad_xyz2_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("ChamferDistanceGrad", trans_bool_to_s8=False)
...
def chamfer_distance_grad(xyz1_in__, xyz2_in__, idx1_in__, idx2_in__, grad_dist1_in__, grad_dist2_in__, grad_xyz1_out_, grad_xyz2_out_, kernel_name="chamfer_distance_grad", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(xyz1_in__, xyz2_in__, idx1_in__, idx2_in__, grad_dist1_in__, grad_dist2_in__, grad_xyz1_out_, grad_xyz2_out_)
    ...
    origin_func_name = "chamfer_distance_grad"
    ascendc_src_dir_ex = "chamfer_distance_grad"
    ascendc_src_dir = "chamfer_distance_grad"
    ascendc_src_file = "chamfer_distance_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator ChamferDistanceGrad, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "ChamferDistanceGrad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[xyz1_in__, xyz2_in__, idx1_in__, idx2_in__, grad_dist1_in__, grad_dist2_in__], origin_outputs = [grad_xyz1_out_, grad_xyz2_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required', 'required', 'required', 'required'], init_value_list = [None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(xyz1_in__, xyz2_in__, idx1_in__, idx2_in__, grad_dist1_in__, grad_dist2_in__, grad_xyz1_out_, grad_xyz2_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(xyz1_in__, xyz2_in__, idx1_in__, idx2_in__, grad_dist1_in__, grad_dist2_in__, grad_xyz1_out_, grad_xyz2_out_)
    result = check_op_cap("op_select_format", "ChamferDistanceGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(xyz1_in__, xyz2_in__, idx1_in__, idx2_in__, grad_dist1_in__, grad_dist2_in__, grad_xyz1_out_, grad_xyz2_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(xyz1_in__, xyz2_in__, idx1_in__, idx2_in__, grad_dist1_in__, grad_dist2_in__, grad_xyz1_out_, grad_xyz2_out_)
    result = check_op_cap("get_op_specific_info", "ChamferDistanceGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
