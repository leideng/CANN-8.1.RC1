...
* @li query: A 1D Tensor of type float16 or float32, with shape [D,].
...
* @li code_book: A 3D Tensor of type float16 or float32, with shape [M, ksub, dsub].
* Support values are as follows:
* M = D / dsub, and M is a multiple of 8;
* ksub in {256, 512};
* dsub in {2, 4, 8}.
* The format of the tensor is ND.
* @li centroids: A 2D Tensor of type float16 or float32 with shape [nc, D]. The format of the tensor is ND.
* The value of nc is in range(start=1, end=1e7, step=1). The value of D is the same as query.
* @li bucket_list: A 1D Tensor of type int32 or int64, with shape [ns,]. The format of the tensor is ND.
...
* @li topk_distance: A Tensor, values of the "k" largest or least elements for the last dimension. Must be one of the following types: float32, float16, 
* int32. 
...
*@li x:A Tensor of type int64/uint64. \n

*@par Outputs:
*@li addrs_table: list of [rank id, host addr, device addr, read size].
