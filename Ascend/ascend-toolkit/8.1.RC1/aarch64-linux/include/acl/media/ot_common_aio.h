...
typedef hi_ai_chn                      ot_ai_chn;
typedef hi_ao_chn                      ot_ao_chn;
typedef hi_aenc_chn                    ot_aenc_chn;
typedef hi_adec_chn                    ot_adec_chn;
typedef hi_audio_dev                   ot_audio_dev;
typedef hi_payload_type                ot_payload_type;

#define OT_AI_DEV_MAX_NUM              3
#define OT_AO_DEV_MIN_NUM              0
#define OT_AO_DEV_MAX_NUM              5
#define OT_AENC_MAX_CHN_NUM            6
#define OT_ADEC_MAX_CHN_NUM            6
#define OT_AI_MAX_CHN_NUM              2
#define OT_AO_MAX_CHN_NUM              2
#define OT_AO_SYS_CHN_ID               2

#define OT_MAX_AUDIO_FRAME_NUM     300      /* max count of audio frame in Buffer */
#define OT_MAX_ADEC_AENC_FRAME_NUM 100     /* max cout of adec/aenc frame in buffer */
...
#define OT_MAX_AO_POINT_NUM        4096     /* support 4096 frame len */
#define OT_MIN_AUDIO_POINT_NUM     480       /* min sample per frame */
#define OT_MAX_AI_POINT_NUM        2048     /* max sample per frame for all encoder(aacplus:2048) */
#define OT_MIN_AENC_POINT_NUM      80     /* min point_num_per_frame for aenc */
/* channel number in frame */
...
#define OT_AI_EXPAND                   0x01U
#define OT_AI_CUT                      0x02U
...
/* Defines record type */
typedef enum {
    OT_VQE_RECORD_NORMAL      = 0,  /* double microphone recording. */
    OT_VQE_RECORD_BUTT
} ot_vqe_record_type;
...
typedef struct {
    ...
    hi_u32               expand_flag;    /* expand 8bit to 16bit, use OT_AI_EXPAND(only valid for AI 8bit),
                                            use OT_AI_CUT(only valid for extern codec for 24bit) */
    hi_u32               frame_num;      /* frame num in buf[2,OT_MAX_AUDIO_FRAME_NUM] */
    ...
    hi_u32               chn_cnt;        /* channel number on FS, valid value:1/2/4/8 */
...
typedef struct {
    ...
    hi_s16 erl_band[OT_AEC_ERL_BAND_NUM];    /* ERL protect area, [1, 63] for 8k, [1, 127] for 16k,
                                                frequency band calculated by erl_band * 62.5 */
                                             /* besides, erl_band[n+1] should be greater than erl_band[n] */
    hi_s16 erl[OT_AEC_ERL_BAND_VALUE_NUM];   /* ERL protect value of ERL protect area, the smaller its value,
...
typedef struct {
    hi_s8  gain_db[OT_VQE_EQ_BAND_NUM];   /* EQ band, include 100, 200, 250, 350, 500, 800, 1.2k, 2.5k, 4k, 8k in turn,
...
typedef struct {
    ...
    hi_s32 mic_gain_step_db;        /* the step size of gain adjustment, [1, 3], recommend 2 */
...
typedef struct {
    ...
    hi_s16 old_level_db[OT_VQE_DRC_SEC_NUM];  /* old curve level(dB), default[0, -472, -792, -960, -1280],
                                                 range:[-1440, 0]ms,store from big to small,scale:Q4 */
    hi_s16 new_level_db[OT_VQE_DRC_SEC_NUM];  /* new curve level(dB), default[0, -174, -410, -608, -1021],
                                                 range:[-1440, 0]ms,store from big to small,scale:Q4 */
} ot_ai_drc_cfg;

typedef struct {
    hi_bool usr_mode;                           /* mode 0: auto, mode 1: manual. */
    hi_s8  gain_db[OT_TALKVQEV2_EQ_BAND_NUM];   /* EQ band, range:[-50, 20]dB */
...
typedef struct {
    hi_bool cfg;
    hi_char file_path[OT_MAX_AUDIO_FILE_PATH_LEN];
    hi_char file_name[OT_MAX_AUDIO_FILE_NAME_LEN];
