...
/* bad addr, eg, used for copy_from_user & copy_to_user */
...
typedef struct {
    union {
        hi_u64           phys_addr; // R; the physics addr of stream
        hi_u64           input_addr; // R; the addr of input frame
    };
    hi_u8                ATTRIBUTE *addr; // R; the virtual addr of stream
    ...
    hi_u32               offset; // R; the offset between the Valid data and the start addr
...
/*
 * @brief send jpege frame to video encoder channel when the output addr of stream is assigned
 * @param [in] chn: video encoder channel id [0, VENC_MAX_CHN_NUM)
 * @param [in] frame: pointer of frame struct
 * @param [in] jpege_stream: pointer of img stream to assign the output addr of stream
 * @param [in] milli_sec: -1 is block,0 is no block,other positive number is timeout
 * @return success: return 0
 *         fail: return error number
 */
