...
typedef struct {
    ...
    hi_s32 format_err;  // R; Format error. eg: do not support field.
...
typedef struct {
    ...
    // 0: the same with display order , 1: the same with decoder order.
...
typedef struct {
    hi_pixel_format pixel_format;  // RW; Output pixel format.
...
/*
 * @brief  get jpeg decode image information
 * @param [in] stream: stream info pointer
 * @param [in] output_format: HI_PIXEL_FORMAT_UNKNOWN to decode as source format, or the format you want
 * @param [out] img_info: picture info of the stream
 * @return success: return 0
 *         fail: return error code
 */
hi_s32 hi_mpi_vdec_get_jpegd_output_info(const hi_vdec_stream *stream, const hi_pixel_format output_format,
    hi_img_info *img_info);

/*
 * @brief set video decoder channel display mode
 * @param [in] chn: video decoder channel id [0, VDEC_MAX_CHN_NUM)
 * @param [in] display_mode: video decoder channel display mode enum
 * @return success: return 0
 *         fail: return error number
 */
...
/*
 * @brief get video decoder channel display mode
 * @param [in] chn: video decoder channel id [0, VDEC_MAX_CHN_NUM)
 * @param [out] display_mode: pointer of video decoder channel display mode enum
 * @return success: return 0
 *         fail: return error number
 */
