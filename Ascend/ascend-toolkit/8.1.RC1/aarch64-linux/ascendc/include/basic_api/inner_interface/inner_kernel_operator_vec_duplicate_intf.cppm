...
/*!
 * \file inner_kernel_operator_vec_duplicate_intf.cppm
 * \brief
 */
#ifndef ASCENDC_MODULE_INNER_OPERATOR_VEC_DUPLICATE_INTERFACE_H
#define ASCENDC_MODULE_INNER_OPERATOR_VEC_DUPLICATE_INTERFACE_H
    ...
    const uint8_t repeatTimes, const uint16_t dstBlockStride, const uint8_t dstRepeatStride)
{
    ...
    if (!CheckFunDup(dstLocal, mask, repeatTimes, dstBlockStride, dstRepeatStride, "Duplicate")) {
        ASCENDC_REPORT_CHECK_ERROR("Duplicate", KernelFuncType::MASK_COUNT_MODE);
    }
#endif
    DuplicateImpl<T, isSetMask>((__ubuf__ T*)dstLocal.GetPhyAddr(), scalarValue, mask, repeatTimes, dstBlockStride,
        dstRepeatStride);
}
...
__aicore__ inline void Duplicate(const LocalTensor<T>& dstLocal, const T& scalarValue, const int32_t& calCount)
{
    CheckDuplicateSupportedType<T>();
#if ASCENDC_CPU_DEBUG
    if (!CheckFunDup(dstLocal, calCount, "Duplicate")) {
        ASCENDC_REPORT_CHECK_ERROR("Duplicate", KernelFuncType::CALCOUNT_MODE);
    }
#endif
    DuplicateImpl<T>((__ubuf__ T*)dstLocal.GetPhyAddr(), scalarValue, calCount);
}
} // namespace AscendC
#pragma end_pipe
#endif // ASCENDC_MODULE_INNER_OPERATOR_VEC_DUPLICATE_INTERFACE_H
