...
{
    static_assert((__CCE_AICORE__ == 300), "current device don't support BilinearInterpolation");
}

template <typename T>
__aicore__ inline void BilinearInterpolationCalc(LocalTensor<T> &dstLocal, LocalTensor<T> &src0Local,
    LocalTensor<uint32_t> &src0OffsetLocal, LocalTensor<T> &src1Local, uint64_t mask[], uint8_t hRepeat,
    bool repeatMode, uint16_t dstBlkStride, uint16_t vROffset, uint8_t vRepeat,
    const LocalTensor<uint8_t> &sharedTmpBuffer)
{
    static_assert((__CCE_AICORE__ == 300), "current device don't support BilinearInterpolation");
