...
template <typename DstT, typename SrcT>
__aicore__ inline void FixpipeL0C2UBImpl(__ubuf__ DstT* dst, __cc__ SrcT* src, FixpipeInfoParams<SrcT>& fixpipeInfo)
{
    if (fixpipeInfo.nz2ndEn) {
        uint64_t ndPara = static_cast<uint64_t>(fixpipeInfo.dstNdStride) << 32; // ND_PARA[47:32]
        ndPara |= static_cast<uint64_t>(fixpipeInfo.srcNdStride) << 16;         // ND_PARA[31:16]
...
template <typename DstT, typename SrcT>
__aicore__ inline void FixpipeL0C2UBImplN(__ubuf__ DstT* dst, __cc__ SrcT* src,
    const FixpipeInfoParams<SrcT>& fixpipeInfo, uint16_t calNSize, uint16_t nIterIndex)
{
    // mov deq tensor from L1 to FB
    CopyDeqTensorToFbuf(fixpipeInfo, calNSize, nIterIndex);
    ...
    const FixpipeInfoParams<SrcT>& fixpipeInfo, uint16_t calNSize, uint16_t nIterIndex = 0)
{
    ...
    switch (fixpipeInfo.quantPre) {
        case QuantMode_t::NoQuant:
            return copy_matrix_cc_to_ub((__ubuf__ DstT*)(dst + dstOffset), (__cc__ SrcT*)(src + srcOffset),
                fixpipeInfo.sid, calNSize, fixpipeInfo.m, fixpipeInfo.dstStride, fixpipeInfo.srcStride,
                0, fixpipeInfo.unitFlag, QuantMode_t::NoQuant, static_cast<uint8_t>(fixpipeInfo.reluEn),
                fixpipeInfo.channelSplit, fixpipeInfo.nz2ndEn, false);
        case QuantMode_t::F322F16:
    ...
    const FixpipeParams<ParamT>& intriParams)
{
    ...
    if (dstHWPos == Hardware::UB) {
        FixpipeL0C2UBImpl((__ubuf__ PrimT<DstT>*)dstLocal.GetPhyAddr(),
    ...
    const LocalTensor<BufT> &cbufWorkspace, const FixpipeParams<ParamT> &intriParams)
{
    ...
    fixpipeInfo.cbufWorkspace = (__cbuf__ uint64_t *)cbufWorkspace.GetPhyAddr();
    FixpipeL0C2GMImpl(
        (__gm__ PrimT<DstT> *)dstGlobal.GetPhyAddr(), (__cc__ PrimT<SrcT> *)srcLocal.GetPhyAddr(), fixpipeInfo);
}
} // namespace AscendC
