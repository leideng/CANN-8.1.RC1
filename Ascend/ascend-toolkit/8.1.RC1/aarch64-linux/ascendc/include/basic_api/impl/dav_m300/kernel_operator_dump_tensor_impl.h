...
__aicore__ inline FixpipeParamsM300 GetFixpipeParamImpl(uint32_t dumpSize)
{
    uint32_t dumpSizeFloat = dumpSize / sizeof(float);
    if (dumpSize % sizeof(float) != 0) {
        dumpSizeFloat += 1;
    }
    FixpipeParamsM300 fixpipeInfo;
    fixpipeInfo.nSize = dumpSizeFloat;
    fixpipeInfo.mSize = 1;
    return fixpipeInfo;
}
template <typename T>
...
__aicore__ inline void DumpTensorLocal2GMImpl(const LocalTensor<T>& tensor, uint32_t desc, uint32_t dumpSize)
{
    ...
    *((__gm__ uint32_t*)ptr->dumpAddr + DUMP_MESSAGE_HEAD_RSV_POS) = 0;
    ...
    if (srcHWPos == Hardware::UB) {
        ...
        // L0C to GM
        FixpipeParamsM300 fixpipeInfo = GetFixpipeParamImpl(offset);
        FixpipeL0C2GMImpl< float, float, CFG_NZ>(
            (__gm__ float*)(ptr->dumpAddr), (__cc__ float*)(tensor.GetPhyAddr()), fixpipeInfo);
    }
    PipeBarrier<PIPE_ALL>();
