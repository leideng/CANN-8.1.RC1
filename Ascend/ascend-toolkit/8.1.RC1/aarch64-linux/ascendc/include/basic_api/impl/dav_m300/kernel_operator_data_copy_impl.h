...
__aicore__ inline void CopyImpl(__ubuf__ bfloat16_t* dst, __ubuf__ bfloat16_t* src, const uint64_t mask[], uint8_t repeatTimes,
    const CopyRepeatParams& repeatParams) {
    if constexpr (isSetMask) {
        SetVectorMask<T>(mask[1], mask[0]);
    }
...
template <typename T = bfloat16_t, bool isSetMask = true>
__aicore__ inline void CopyImpl(__ubuf__ bfloat16_t *dst, __ubuf__ bfloat16_t *src, const uint64_t mask, const uint8_t repeatTimes,
    const CopyRepeatParams &repeatParams)
{
    if constexpr (isSetMask) {
        uint32_t sreg = mask;
