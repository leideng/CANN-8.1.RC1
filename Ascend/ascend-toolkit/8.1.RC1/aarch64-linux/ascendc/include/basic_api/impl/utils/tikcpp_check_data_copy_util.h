...
/*!
 * \file tikcpp_check_data_copy_util.h
 * \brief
 */
...
inline bool ReportTensorSizeOverflow(Hardware srcPos, Hardware dstPos, uint64_t srcSizeBytes, uint64_t dstSizeBytes,
    ...
    if (srcPos != Hardware::GM) {
        ASCENDC_ASSERT((srcMaxOffsetBytes <= srcSizeBytes),
            { KERNEL_LOG(KERNEL_ERROR, "Failed to check srcLocal size in %s, tensor size "
            "needs to be at least %lu bytes, while current tensor size is only %lu bytes.",
            apiInfo.c_str(), srcMaxOffsetBytes, srcSizeBytes); return false; });
    }
    if (dstPos != Hardware::GM) {
        ASCENDC_ASSERT((dstMaxOffsetBytes <= dstSizeBytes),
            { KERNEL_LOG(KERNEL_ERROR, "Failed to check dstLocal size in %s, tensor size "
            "needs to be at least %lu bytes, while current tensor size is only %lu bytes.",
            apiInfo.c_str(), dstMaxOffsetBytes, dstSizeBytes); return false; });
    }
    ...
    DeqScale deqScale = DeqScale::DEQ_NONE, const uint8_t biasConvFlag = 0, const uint8_t sidStoreMode = 0)
{
    ...
    std::string srcIDString = GetSrcIDString<SrcT>(srcHardware, blockMode);
    std::string dstIDString = GetDstIDString<DstT>(dstHardware, blockMode);
    std::string srcDstId = srcIDString + dstIDString;
    bool isConv = IsConv<SrcT, DstT>(deqScale);
    ...
    if (sizeof(SrcT) == sizeof(int32_t) && sizeof(DstT) == sizeof(int8_t) && 
    ...
    uint64_t& srcMaxOffset, uint64_t& dstMaxOffset)
{
    ...
    if (srcPos == Hardware::L0C && dstPos == Hardware::GM && intriParams.nz2ndEn) {
        ...
        uint16_t fractalSize = BLOCK_CUBE * c0 * sizeof(SrcT);
        // loop3SrcStride in unit of fractal_size, srcStride in unit of C0_Size
        srcMaxOffset = (ndNum - 1) * loop3SrcStride * fractalSize +
            cburstNum * intriParams.srcStride * c0 * sizeof(SrcT);
        // in unit of element Loop2_dst_stride
        dstMaxOffset = ((ndNum - 1) * loop3DstStride +
            (intriParams.mSize - 1) * intriParams.dstStride + intriParams.nSize) * sizeof(DstT);
    } else {
        srcMaxOffset = (intriParams.mSize + (cburstNum - 1) * intriParams.srcStride) * c0 * sizeof(SrcT);
        dstMaxOffset = intriParams.mSize * c0 * sizeof(DstT) + (cburstNum - 1) * intriParams.dstStride * ONE_BLK_SIZE;
    }
}
