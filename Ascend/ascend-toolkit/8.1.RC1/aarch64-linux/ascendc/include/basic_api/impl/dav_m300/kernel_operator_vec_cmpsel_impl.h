...
#define BITS_MODE_B32_VCMPVS_VF(cmpMode, dataType)                                                 \
    ...
    {                                                                                              \
        ...
        vector_bool preg5;                                                                         \
        vector_bool preg6;                                                                         \
        uint32_t offset0 = (repeatTimes / 2) * 2 * repeatParams.srcRepStride * blockElm;           \
        uint32_t offset2 = (repeatTimes / 2) * 4;                                                  \
        for (uint16_t i = 0; i < (uint16_t)(repeatTimes % 2); ++i) {                               \
            vsldb(vreg0, src0 + offset0, strideConfig0, preg7);                                    \
...
{
    static_assert((__CCE_AICORE__ == 300), "Compare written to CMPMASK is not supported on current device");
}
...
{
    static_assert((__CCE_AICORE__ == 300), "GetCmpMask is not supported on current device");
}

template <typename T>
__aicore__ inline void SetCmpMaskImpl(__ubuf__ T* src)
{
    static_assert((__CCE_AICORE__ == 300), "SetCmpMask is not supported on current device");
}
} // namespace AscendC
