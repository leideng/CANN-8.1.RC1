...
{
    static_assert((__CCE_AICORE__ == 300), "unsupported GetArchVersion on current device");
}
...
{
    static_assert((__CCE_AICORE__ == 300), "unsupported GetProgramCounter on current device");
    return 0;
}

__aicore__ inline int64_t GetSystemCycleImpl()
{
    static_assert((__CCE_AICORE__ == 300), "unsupported GetSystemCycle on current device");
    return 0;
}

__aicore__ inline void SetPcieRDCtrlImpl(bool isSetPcie, uint8_t maxBurstLen)
{
    static_assert((__CCE_AICORE__ == 300), "unsupported SetPcieRDCtrl on current device");
}

__aicore__ inline void SetPcieWRCtrlImpl(bool isSetPcie, uint8_t maxBurstLen)
{
    static_assert((__CCE_AICORE__ == 300), "unsupported SetPcieWRCtrl on current device");
}

__aicore__ inline void TrapImpl()
{
    static_assert((__CCE_AICORE__ == 300), "unsupported Trap on current device");
}
} // namespace AscendC
