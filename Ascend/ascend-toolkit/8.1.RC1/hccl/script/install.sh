...
prompt_set_env() {
    ...
    if [ "$hetero_arch" = "y" ]; then
        echo "Please make sure that
            - PATH includes ${install_path}/hccl/bin
            - LD_LIBRARY_PATH includes ${install_path}/hccl/lib64:${install_path}/hccl/lib64/plugin/opskernel"
    else
        echo "Please make sure that
            - PATH includes ${install_path}/hccl/bin
            - LD_LIBRARY_PATH includes ${install_path}/hccl/lib64:${install_path}/hccl/lib64/plugin/opskernel
...
install_run() {
    ...
    if [ "$1" = "install" ]; then
        ...
        if [ $? -eq 0 ]; then
            update_version_info_version
            if [ "$hetero_arch" = "y" ]; then
                create_compiler_atc_fwkacllib_softlink "${hccl_install_path_param}" "${in_install_for_all}"
            else
                create_compiler_atc_fwkacllib_softlink "${hccl_install_path_param}/${pkg_version_dir}" "${in_install_for_all}"
            fi
...
uninstall_run() {
    ...
    if [ "$1" = "uninstall" ]; then
        ...
        if [ "$hetero_arch" = "y" ]; then
            remove_compiler_atc_fwkacllib_softlink "${hccl_install_path_param}"
        else
            local version_dir="$(basename $(dirname $upgrade_default_dir))"
            remove_compiler_atc_fwkacllib_softlink "${hccl_install_path_param}/${version_dir}"
        fi
...
if [ "$pkg_is_multi_version" = "true" ] && [ "$hetero_arch" != "y" ]; then
    default_dir="${pkg_install_path}/$pkg_version_dir/hccl"
else
    default_dir="${pkg_install_path}/hccl"
fi
...
uninstall_none_multi_version "$pkg_install_path/hccl"
check_install_for_all
create_default_install_dir_for_common_user
log_base_version
