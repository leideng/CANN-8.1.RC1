...
SOURCE_INSTALL_COMMON_PARSER_FILE="${common_parse_dir}/hccl/script/install_common_parser.sh"
SOURCE_FILELIST_FILE="${common_parse_dir}/hccl/script/filelist.csv"
...
if [ ! -d "$common_parse_dir/hccl" ]; then
    log "ERROR" "ERR_NO:0x0001;ERR_DES:path $common_parse_dir/hccl is not exist."
    exit 1
fi

new_uninstall() {
    if [ -f "${common_parse_dir}/hccl/data/version.info" ]; then
        log "INFO" "need to uninstall costmodel files."
        bash "${common_parse_dir}/hccl/data/script/install.sh" -- -- --uninstall --install-path="${common_parse_dir}"
    fi
