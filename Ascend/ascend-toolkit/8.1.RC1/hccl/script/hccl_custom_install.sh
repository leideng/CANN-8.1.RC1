...
custom_install() {
    if [ -z "$common_parse_dir/hccl" ]; then
        log "ERROR" "ERR_NO:0x0001;ERR_DES:hccl directory is empty"
        exit 1
    fi

    if [ "$hetero_arch" != "y" ]; then
        local arch_name="$(get_arch_name $common_parse_dir/hccl)"
        create_stub_softlink "$common_parse_dir/hccl/lib64/stub" "linux/$arch_name"
    else
        local arch_name="$(get_arch_name $common_parse_dir/hccl)"
        create_stub_softlink "$common_parse_dir/hccl/lib64/stub" "linux/$arch_name"
    fi
    ...
    if [ "$hetero_arch" != "y" ]; then
        ...
        if [ "${pylocal}" = "y" ]; then
            ...
            log "INFO" "The package te is already installed in python default path. It is recommended to install it using the '--pylocal' paramter, install the package hccl in the ${WHL_INSTALL_DIR_PATH}."
        fi
