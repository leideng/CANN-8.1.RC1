...
create_compiler_atc_fwkacllib_softlink() {
    ...
    local lib64_plugin_file_list="$(ls ${install_path}/hccl/lib64/plugin/opskernel | awk '{print $NF}')"
    ...
    for pkg in "compiler" "atc" "fwkacllib"; do
        ...
        if [ "$pkg" = "compiler" ]; then
            if [ -e "${install_path}/${pkg}/lib64/plugin" ]; then
                chmod "${dir_mod_new}" "${install_path}/${pkg}/lib64/plugin"
            fi

            mkdir -p "${install_path}/${pkg}/lib64/plugin/opskernel"
            chmod "${dir_mod_new}" "${install_path}/${pkg}/lib64/plugin"
            chmod "${dir_mod_new}" "${install_path}/${pkg}/lib64/plugin/opskernel"
            for file in $lib64_plugin_file_list; do
                create_softlink_by_relative_ln "${install_path}/hccl/lib64/plugin/opskernel" "${install_path}/${pkg}/lib64/plugin/opskernel" "${file}"
            done
            chmod "${dir_mod}" "${install_path}/${pkg}/lib64/plugin/opskernel"
            chmod "${dir_mod}" "${install_path}/${pkg}/lib64/plugin"
        fi
...
remove_compiler_atc_fwkacllib_softlink() {
    ...
    for pkg in "compiler" "atc" "fwkacllib"; do
        ...
        if [ "$pkg" = "compiler" ]; then
            if [ -e "${install_path}/${pkg}/lib64/plugin/opskernel" ]; then
                chmod u+w "${install_path}/${pkg}/lib64/plugin/opskernel"
                for file in $lib64_plugin_file_list; do
                    \rm -rf "${install_path}/${pkg}/lib64/plugin/opskernel/${file}"
                done
                chmod u-w "${install_path}/${pkg}/lib64/plugin/opskernel"

                chmod u+w "${install_path}/${pkg}/lib64/plugin"
                remove_dir_if_empty "${install_path}/${pkg}/lib64/plugin/opskernel"
                remove_dir_if_empty "${install_path}/${pkg}/lib64/plugin"
                if [ -e "${install_path}/${pkg}/lib64/plugin" ]; then
                    chmod u-w "${install_path}/${pkg}/lib64/plugin"
                fi
            fi
