...
def condition_check(input_dict):
    ...
    if tvm.ComputeOp != op_type:
        log.dbg("Only ComputeOp Tensor Can do Cache Write, Tensor Op type:%s",
                op_type)
        return False

    if not tensor.op.name.endswith(platform_info.scope_ubuf):
        log.dbg("Only Tensor In UB do Cache Write from L0C To UB:%s",
                tensor_name)
        return False

    if stage_tag not in util.NEED_WRITE_L0_TO_UB_TAGS:
        log.dbg("op_name:%s, stage_tag: %s, no need write L0C->UB!",
