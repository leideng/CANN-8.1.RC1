...
def equal_const_int(expr, value):
    ...
    if isinstance(expr, int):
        return expr == value
    if not isinstance(expr, (_expr.IntImm)):
...
def dma_copy(ib_expr, src, dst):
    ...
    if dst.scope == cce_params.scope_ubuf and \
        ...
        pad_mode_call = call_cce_pure_intrin("int32", "tvm_cce_string_print",
                                         *pad_mode)
        ...
        cr_mode_call = call_cce_pure_intrin("int32", "tvm_cce_string_print",
                                        *cr_mode)
        ib_expr.emit(call_extern(
            dst.dtype, intrin_name,
            dst_ptr,  # dst buffer
...
def get_pad_info(stmt_in):
    ...
    def _post_order(stmt_op):
        ...
        if isinstance(stmt_op, _stmt.AttrStmt) and \
            ...
            if isinstance(stmt_op.node, tir.IterVar):
...
def get_argnlst_outaxis(stmt_in):
    ...
    if var and (isinstance(var[0], tvm.Var) or is_const(var[0])):
        return var[0]
    ...
    def _post_order(stmt_op):
        ...
        if isinstance(stmt_op, _expr.Select):
            if (not isinstance(stmt_op.condition.a, _expr.Load) and
                ...
                raise RuntimeError(
                    "either a or b must be tvm.tir.Load, other must be const")
...
def del_ins_reuse_out(ins, out):
    ...
    for single_ins in ins:
        if not (tvm.tir.analysis.expr_deep_equal(single_ins.data, out.data) and
                tvm.tir.analysis.expr_deep_equal(single_ins.elem_offset, out.elem_offset)):
...
def get_unique_buffer(buf_list):
    ...
    for buffer1 in buf_list:
        ...
        for buffer2 in buf_list_unique:
            if tvm.tir.analysis.expr_deep_equal(buffer1.data, buffer2.data) and\
                    tvm.tir.analysis.expr_deep_equal(buffer1.elem_offset, buffer2.elem_offset):
                is_buf_exit = True
                break
...
def get_logic_args(stmt_in):
    ...
    def _post_order(stmt_op):
        ...
        if isinstance(stmt_op, _expr.Call):
            if stmt_op.op.name == "tir.bitwise_or":
                args[0] = 'or'
            elif stmt_op.op.name == "tir.bitwise_and":
                args[0] = 'and'
            elif stmt_op.op.name == "tir.bitwise_not":
                args[0] = 'not'
...
def get_input_shape_from_stmt(input_shape, stmt_op):
    ...
    def store_ori(_var):
        if not isinstance(_var, (tvm.Var, _expr.IntImm)):
            store_ori(_var.a)
            store_ori(_var.b)
            return
