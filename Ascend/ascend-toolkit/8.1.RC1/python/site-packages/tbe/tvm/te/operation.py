...
from tvm import api_config

from . import _ffi_api
from . import tag as _tag
from . import tensor as _tensor
from .operation_extended import int8, int32, int64, float32, handle


def _get_annotation(var_):
    return tvm.te.get_expr_annotation(var_)


def _set_annotation(var_, annotation):
    for k, v in annotation.items():
        tvm.te.set_expr_attr(var_, k, v)


def _cast_64_bit(x):
    if _get_annotation(x) is not None and (isinstance(x, _expr.IntImm) or isinstance(x, _expr.FloatImm)):
        x_trans = x.astype("int64")
        _set_annotation(x_trans, _get_annotation(x))
        return x_trans
    return x.astype("int64")


def placeholder(shape, dtype=None, name="placeholder", attrs=None):
    ...
    attrs: dict, optional
        The additional auxiliary attributes about the placeholder
    ...
    tensor: Tensor
        The created tensor
    """
    if isinstance(shape, _expr.PrimExprWithOp):
        shape = (shape,)
    else:
        for x in shape:
            if api_config.query_bit_width() != 64 and isinstance(x, _expr.PrimExprWithOp) and x.dtype == "int64":
                api_config.bit_width_64()
                break
        if api_config.query_bit_width() == 64:
            shape = [ _cast_64_bit(x) if isinstance(x, _expr.PrimExprWithOp) else const(x, int64) for x in shape]
        else:
            shape = [int(x) if not isinstance(x, (int, _expr.PrimExprWithOp)) else x for x in shape]
    dtype = "float32" if dtype is None else dtype
    return _ffi_api.Placeholder(shape, dtype, name, attrs)


def compute(shape, fcompute, name="compute", tag="", attrs=None, span=None):
    ...
    from tbe.common.testing.dsl_source_info import get_span_info
    file_name, line_co = get_span_info()

    if (file_name != "Unknown"):
        span = Span(SourceName(file_name), int(line_co), int(line_co), 0, 0)
    ...
    if api_config.query_bit_width() == 64:
        shape = [ x.astype(int64) if isinstance(x, _expr.PrimExprWithOp) else const(x, int64) for x in shape]
    else:
        shape = (shape,) if isinstance(shape, _expr.PrimExprWithOp) else shape
        # for python3
    ...
    if isinstance(body, _tensor.TensorIntrinCall):
        ...
        if not isinstance(body, (list, tuple)):
            body = [body]
        body = convert(body)
        op_node = _ffi_api.ComputeOp(name, tag, attrs, dim_var, body, span)
...
def scan(init, update, state_placeholder, inputs=None, name="scan", tag="",
         attrs=None, scan_update=None, span=None):
    ...
    if _tag.TagScope.get_current() is not None:
        if tag != "":
            raise ValueError("nested tag is not allowed for now")
        tag = _tag.TagScope.get_current().tag
    scan_inplace_update = None
    if scan_update is not None:
        # NEXT: IN scan_update mode, not do the exchange and change the scan implementation
        temp = update
        update = scan_update
        scan_inplace_update = temp

        if isinstance(scan_inplace_update, _tensor.Tensor):
            scan_inplace_update = [scan_inplace_update]
    ...
    if len(init) == 0:
        raise ValueError("init, update, state_placeholder can't be null")
    # support that scan axis is arbitrary axis
    scan_axis_idx = 0
    all_same = True
    first_one = -1
    for i in range(len(init[0].shape)):
        if not tvm.tir.analysis.expr_deep_equal(init[0].shape[i], update[0].shape[i]):
            scan_axis_idx = i
            all_same = False
            break
        if tvm.tir.analysis.expr_deep_equal(init[0].shape[i], 1) and first_one == -1:
            first_one = i
    if all_same and first_one != -1:
        scan_axis_idx = first_one
    if scan_update is not None:
        axis = tvm.tir.IterVar((0, update[0].shape[scan_axis_idx] - 1), "%s.idx" % name, 3)
    else:
        axis = tvm.tir.IterVar((init[0].shape[scan_axis_idx], update[0].shape[scan_axis_idx]), "%s.idx" % name, 3)
    op = _ffi_api.ScanOp(name, tag, attrs, axis, init, update,
                         state_placeholder, inputs, scan_inplace_update, span)
...
def extern(
    ...
    op = _ffi_api.ExternOp(name, tag, attrs, inputs,
                           input_placeholders, output_placeholders, body, span)
    res = [op.output(i) for i in range(len(output_placeholders))]
    return res[0] if len(res) == 1 else res


def var(name="tindex", dtype=None, span=None):
    ...
    """
    if dtype is None:
        dtype = int64 if api_config.query_bit_width()==64 else int32
