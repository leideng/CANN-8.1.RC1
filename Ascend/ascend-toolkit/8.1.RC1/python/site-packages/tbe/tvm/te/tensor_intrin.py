...
class TensorIntrin(Object):
    ...
    def __call__(self, *args, **kwargs):
        ...
        sparse_axis = []
        if "sparse_axis" in kwargs:
            sparse_axis = kwargs["sparse_axis"]
            if not isinstance(sparse_axis, (list, tuple)):
                sparse_axis = [sparse_axis]
            sparse_axis = _api.convert(sparse_axis)
        sort_axis = []
        if "sort_axis" in kwargs:
            sort_axis = kwargs["sort_axis"]
            if not isinstance(sort_axis, (list, tuple)):
                sort_axis = [sort_axis]
            sort_axis = convert(sort_axis)
        if scalar_inputs:
            scalar_inputs = convert(scalar_inputs)
        return _ffi_api.TensorIntrinCall(self, tensors, regions, reduce_axis, sparse_axis, sort_axis, scalar_inputs)
