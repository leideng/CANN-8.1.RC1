...
def _scalar_type_inference(value):
    if hasattr(value, "dtype"):
        ...
        # We convert the python int to int32 only when value is in [INT32_MIN,INT32_MAX].
        if (value >= INT32_MIN and value <= INT32_MAX):
            dtype = 'int32'
        elif (value >= INT64_MIN and value <= INT64_MAX):
            dtype = 'int64'
        elif (value <= UINT64_MAX):
            dtype = 'uint64'
        else:
            raise RuntimeError("value={} is out of range of uint64".format(value))
    else:
        raise NotImplementedError(
