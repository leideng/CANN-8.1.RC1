...
# lx fusion
from tvm.build_util import buffer_remap
from tvm.build_util import has_extern_op
...
def schedule_to_module(
    ...
    simple_mode: bool
        The symbol of simple compile.
    Returns
    -------
    The body formed according to the given schedule
    """
    return ffi.schedule_to_module(sch, args, name, binds, simple_mode)
...
def lower(
    ...
    if isinstance(inp, schedule.Schedule):
        ...
        check_buffer_remap = (not get_current_build_config(dynamic_shape)) and \
                             get_buffer_manager().get_remapped_buffers() and \
                             not has_extern_op(inp)
        if check_buffer_remap:
            buffer_remap(inp, args, get_buffer_manager())

        schedule.CheckSchedule(inp)
        from tvm.tir import expr
        return ffi.lower_schedule(inp, args, name, binds, expr.IntImm("int64", -1), simple_mode)
...
def build(
    ...
    args: Optional[List[Union[Buffer, tensor.Tensor, Var]]] = None,
    target: Optional[Union[str, Target]] = "cce",
    target_host: Optional[Union[str, Target]] = None,
    name: Optional[str] = "default_function",
    rules: Optional[List[int]] = None,
    binds: Optional[Mapping[tensor.Tensor, Buffer]] = None,
    evaluates: Optional[Mapping[Var, int]]=None
):
    ...
    rules : list of integer, optional
        The tiling select rules in dynamic shape

    binds : Optional[Mapping[tensor.Tensor, tvm.tir.Buffer]]
        Dictionary that maps the binding of symbolic buffer to Tensor.
        By default, a new buffer is created for each tensor in the argument.

    evaluates: Optional[Mapping[Var, int]]
        Use integers to replaces vars to check if there is any
        simplify problem in dynamic shape.
    ...
    """
    from tbe.common.buildcfg import get_current_build_config
    if get_current_build_config("tir.apply_tbe_pass") and (target == "kunpeng" or target == "cce"):
        from .cce_build_module import build_cce
        if get_current_build_config("tir.enable_vector_core"):
