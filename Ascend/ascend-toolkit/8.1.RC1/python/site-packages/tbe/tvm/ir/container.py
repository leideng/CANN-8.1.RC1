...
class Map(Object):
    ...
    def __setitem__(self, k, v):
        _ffi_api.MapSetItem(self, k, convert(v))
...
def type_convert(v):
    from tvm.tir.expr import IntImm, FloatImm, StringImm
    if isinstance(v, Array) or isinstance(v, list):
        new_list = []
        for i in v:
            new_list.append(type_convert(i))
        return new_list
    if isinstance(v, String):
        return str(v)
    if isinstance(v, StringImm):
        return type_convert(v.value)
    if isinstance(v, IntImm):
        if v.dtype == "uint1":
            return bool(v.value)
        else:
            return v.value
    if isinstance(v, FloatImm):
        return v.value
    if isinstance(v, Map) or isinstance(v, dict):
        new_dict = {}
        for k in v:
            if isinstance(k, String) or isinstance(k, StringImm):
                new_dict[type_convert(k)] = type_convert(v[k])
            else:
                new_dict[k] = type_convert(v[k])
        return new_dict
    return v

def deepcopy(origin_object):
    """Deepcopy a object with tvm object.

    Parameters
    ----------
    origin_object : dict/list
        The dict/list contains tvm object.

    Returns
    -------
    new_map: dict
        The dict/list of the new object.
    """
    if isinstance(origin_object, dict):
        origin_map = convert(origin_object)
        new_map = copy.deepcopy(origin_map)
        new_dict = type_convert(new_map)
        return new_dict
    else:
        return copy.deepcopy(origin_object)