...
from tvm.runtime import Object, ObjectGeneric, DataType, DataTypeCode, const, convert
...
def _dtype_is_float(value):
    ...
    if isinstance(value, IterVar):
        return DataType(value.var.dtype).type_code == DataTypeCode.FLOAT
    return (
            isinstance(value, ExprOp) and DataType(value.dtype).type_code == DataTypeCode.FLOAT
    )  # type: ignore


def _dtype_is_complex(value):
    if isinstance(value, IterVar):
        return DataType(value.var.dtype).type_code == DataTypeCode.FLOAT
    return (
            isinstance(value, ExprOp) and DataType(value.dtype).type_code == DataTypeCode.COMPLEX
    )  # type: ignore


def _compatible_to_complex(func, self, other):
    """
    Multiply in compute lambda expression cause complex convert to float by BinaryOpMatchTypes.
    The Unexpected convert may lead to wrong data type of 'real' after ScheduleOps.
    This function is work around to issue above when complex type is not supported completely in backend.
    """
    value = func(self, other)
    if (_dtype_is_complex(self) and _dtype_is_float(other)) or (_dtype_is_float(self) and _dtype_is_complex(other)):
        complex_dtype = self.dtype if _dtype_is_complex(self) else other.dtype
        return _generic.cast(value, complex_dtype)
...
class ExprOp(object):
    ...
    def __mul__(self, other):
        return _compatible_to_complex(_generic.multiply, self, other)
    ...
    def __truediv__(self, other):
        return _compatible_to_complex(_generic.divide, self, other)
        ...
        if dom is not None:
            if isinstance(dom, (list, tuple)):
                ...
                if api_config.query_bit_width() == 64:
                    dom = tvm.ir.Range(convert(dom[0]).astype("int64"), convert(dom[1]).astype("int64"))
                else:
...
class Reduce(PrimExprWithOp):
    ...
    out_of_bound_check : bool
        The reduce out of bound check.
    """

    def __init__(self, combiner, src, rdom, condition, value_index, init=None, span=None,
                 out_of_bound_check=False, conditional_reduce=None):
        self.__init_handle_by_constructor__(
            _ffi_api.Reduce, combiner, src, rdom, condition, value_index, init, span,
            out_of_bound_check, conditional_reduce  # type: ignore
        )
...
class IntImm(ConstExpr):
    ...
    def __eq__(self, other):
        res = _ffi_api._OpEQ(self, other, None)  # type: ignore
        if isinstance(res, EQ):  # compatible with tvm0.6
            return EqualOp(self, other)
        return res

    def __ne__(self, other):
        res = _ffi_api._OpNE(self, other, None)  # type: ignore
        if isinstance(res, NE):  # compatible with tvm0.6
            return NotEqualOp(self, other)
        return res
...
class Call(PrimExprWithOp):
    ...
    def __init__(self, dtype, op, args, span=None, annotations={}):
        if isinstance(op, str):
            ...
            op = Op.get(op)
        self.__init_handle_by_constructor__(_ffi_api.Call, dtype, op, args, span, annotations)  # type: ignore
