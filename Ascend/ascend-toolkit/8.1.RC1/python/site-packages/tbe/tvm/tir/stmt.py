...
class For(Stmt):
    ...
    def __init__(
        ...
        if isinstance(min_val, int):
            min_val = tvm.tir.const(min_val, loop_var.dtype)
        if isinstance(extent, int):
            extent = tvm.tir.const(extent, loop_var.dtype)
        self.__init_handle_by_constructor__(
...
class Allocate(Stmt):
    ...
    new_expr : Optional[PrimExpr]
        new_expr value for current allocate.
    """

    def __init__(self, buffer_var, dtype, extents, condition, body, annotations=None, span=None, new_expr=None):
...
class AttrStmt(Stmt):
    ...
    bounds : Optional[Region]
        Saving allocate at bounds
    """

    def __init__(self, node, attr_key, value, body, span=None, bounds=None):
        self.__init_handle_by_constructor__(
            _ffi_api.AttrStmt, node, attr_key, value, body, span, bounds  # type: ignore
        )
