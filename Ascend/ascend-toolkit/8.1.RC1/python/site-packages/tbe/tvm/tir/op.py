...
def reduce_init_min_value(dtype, span=None):
    """reduce_init minimum value of dtype(-inf)
    ...
    -------
    value : tvm.Expr
        The reduce_init minimum value of dtype.
    """
    return _ffi_api.reduce_init_min_value(dtype, span)  # type: ignore


def reduce_init_max_value(dtype: str, span: Optional[Span] = None) -> Any:
    """reduce_init maximum value of dtype(inf)
    ...
    -------
    value : tvm.Expr
        The reduce_init maximum value of dtype.
    """
    return _ffi_api.reduce_init_max_value(dtype, span)  # type: ignore
...
def comm_reducer(fcombine, fidentity, name="reduce"):
    ...
    def _make_reduce(expr, axis, where=None, init=None, out_of_bound_check=False,
                     conditional_reduce=None):
        ...
        if init is None:
            outputs = tuple(
                tvm.tir.Reduce(combiner, expr, axis, where, i, convert([]),
                               out_of_bound_check=out_of_bound_check,
                               conditional_reduce=conditional_reduce) for i in range(size)
            )
        else:
            outputs = tuple(
    ...
    def reducer(expr, axis, where=None, init=None, out_of_bound_check=False,
                conditional_reduce=None, *args):
        if isinstance(axis, (tvm.tir.IterVar, list, tuple)):
            assert not args
            return _make_reduce(expr, axis, where, init, out_of_bound_check, conditional_reduce)
        if where is None:
            assert not args
...
min = comm_reducer(lambda x, y: _ffi_api._OpMin(x, y, None), reduce_init_max_value, name="min")  # type: ignore
max = comm_reducer(lambda x, y: _ffi_api._OpMax(x, y, None), reduce_init_min_value, name="max")  # type: ignore
