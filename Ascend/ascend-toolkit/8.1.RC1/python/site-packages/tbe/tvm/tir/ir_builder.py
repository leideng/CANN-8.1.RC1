...
class IRBuilder(object):
    ...
    def scope_attr(self, node, attr_key, value, bounds=None):
        ...
        if attr_key == "thread_extent":
            if isinstance(value, int) and isinstance(node, _expr.IterVar):
                value = const(value, node.var.dtype)
            else:
                value = op.max(1, value)
        self.emit(lambda x: _stmt.AttrStmt(node, attr_key, value, x, None, bounds))
    ...
    def for_range(self, begin, end, name="i", dtype=None, kind="serial"):
        ...
        predict_dtypes = set()
        if dtype is None:
            if api_config.query_bit_width() == 32:
                dtype = "int32"
                if isinstance(begin, _expr.PrimExpr):
                    predict_dtypes.add(begin.dtype)
                if isinstance(end, _expr.PrimExpr):
                    predict_dtypes.add(end.dtype)
                if len(predict_dtypes) == 1:
                    dtype = predict_dtypes.pop()
                    begin, end = [const(x, dtype) if isinstance(x, (int, float)) else x for x in [begin, end]]
            else:
                dtype = "int64"
                begin, end = [const(x, dtype) if isinstance(x, (int, float)) else x for x in [begin, end]]
                begin, end = [x.astype(dtype) if isinstance(x, _expr.PrimExpr) else x for x in [begin, end]]
        if name == "i":
        ...
        if kind == "serial":
            if isinstance(extent, int):
                ...
                extent = util.Simplify(extent)
                if isinstance(extent, (_expr.IntImm)) and extent.value == 1:
                    kind = "unroll"
        ...
        def _exit_cb():
            if kind == "serial":
                ...
                kind_id = _stmt.ForKind.SINGLEPOINT
            else:
    ...
    def if_scope(self, cond):
        ...
        if isinstance(cond, bool):
            if not cond:
                cond = tir.IntImm('uint1', 0)
            else:
                cond = tir.IntImm('uint1', 1)
    ...
    def allocate(self, dtype, shape, name="buf", scope="", annotations=None):
        ...
        if not isinstance(shape, (list, tuple, _container.Array)):
            shape = [shape]
        self.emit(lambda x: _stmt.Allocate(buffer_var, dtype, shape, const(1, dtype="uint1"), x, annotations=annotations))
