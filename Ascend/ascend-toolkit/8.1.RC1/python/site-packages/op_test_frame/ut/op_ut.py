...
from op_test_frame.utils import soc_util
from op_test_frame.utils import type_util
...
from op_test_frame.ut.op_tiling_ut_gen import OpTilingUtGen
...
class OpUT:  # 'pylint: disable=too-many-instance-attributes
    ...
    DEFAULT_SUPPORT_SOC = ["Ascend910", "Ascend310", "Ascend310P3"]

    def __init__(self, op_type, op_module_name=None, op_func_name=None):

        self.op_type = op_type
        # ```self._tiling_ut = OpTilingWithUtGen(op_type)```
        ...
        self._case_info_map: Dict[str, Union[op_ut_case_info.OpUTCase, op_ut_case_info.OpUTCustomCase]] = {}
        caller = inspect.stack()[1]
        self.case_file = caller.filename
        # mock patches
        self.mock_patches: Dict[Union[str, Tuple[str]], MagicMock] = {}
        self.run_mock_only: bool = False
        self.mock_soc: Union[str, Tuple[str], List[str]] = None
        self.mock_case: Union[Tuple[str], List[str]] = None
        self.mock_ignore_case: Union[Tuple[str], List[str]] = None
        # op info config map. like: {"ascend910": {"Add": {"coreType.value": None}}}
        self.op_info_cfg: dict = None
        # tiling ut template
        self.tiling_ut_gen = OpTilingUtGen(self.op_type)

        # for valueDepend op ut case check
        def _check_op_is_value_depend():
            if self.imply_type != OpImplyType.DYNAMIC:
                return False

            cur_dir = os.path.realpath(__file__).split(os.path.sep)
            ut_root_path = os.path.sep.join(cur_dir[:-6])
            # the main platform
            ini_pattern = "aic-ascend910b-ops-info.ini"
            cmd = ["find " + ut_root_path, "-name", ini_pattern]
            ini_file = os.popen(" ".join(cmd)).read().strip().split("\n")[0]
            get_type_pos_cmd = ["grep -n '^\[' " + ini_file, "|grep -w -A 1 " + self.op_type, "|cut -d: -f1"]
            type_pos = os.popen("".join(get_type_pos_cmd)).read().strip().split("\n")
            add_line = 120
            if len(type_pos) == 2:
                add_line = int(type_pos[1]) - 1 - int(type_pos[0])
            get_value_depend_cmd = ["grep -w -A " + str(add_line), self.op_type, ini_file, "|grep valueDepend|wc -l"]
            value_depend_cnt = int(os.popen(" ".join(get_value_depend_cmd)).read().strip().split("\n")[0])

            if value_depend_cnt > 0:
                return True
            return False

        self.op_is_value_depend = _check_op_is_value_depend()
        self.value_depend_tmp_file = "/tmp/tmp_value_depend_ops_case_info.txt"
    ...
    @staticmethod
    def _start_all_patches(mock_patches: Dict[Union[str, Tuple[str]], MagicMock] = None):
        if not mock_patches:
            return
        if any(not isinstance(k, (str, tuple)) for k in mock_patches.keys()) or \
            any(not isinstance(v, MagicMock) for v in mock_patches.values()):
            raise ValueError("mock_patches must be type of Dict[Union[str, Tuple[str]], MagicMock]")
        for k, v in mock_patches.items():
            if isinstance(k, tuple):
                if any(not isinstance(t, str) for t in v):
                    OpUT._stop_all_patches()
                    raise ValueError("mock_patches must be type of Dict[Union[str, Tuple[str]], MagicMock]")
                for t in k:
                    patch(t, v).start()
            else:
    ...
    @staticmethod
    def _get_ascendc_compile_cfg(cfg):
        keys = ['tikcfw_path', 'ascendc_op_path']
        return {key: cfg.get(key, '') for key in keys}

    def add_case(self, support_soc=None, case=None, support_core: Union[str, list, tuple] = None):
        """
        add a only test op compile case
        :param support_soc: this case can test soc list
        :param case: case info, this is a dict.
        :param support_core: this case can test core type list. if None, will get from aic-*-ops-info.ini/.json
        :return: None
        """
        self._add_case(support_soc, case, support_core=support_core)

    def add_precision_case(self, support_soc=None, case=None, support_core: Union[str, list, tuple] = None):
        """
        ...
        :return: None
        """
        self._add_case(support_soc, case, op_ut_case_info.CaseUsage.PRECISION, support_core)

    def add_cust_test_func(self, support_soc=None, test_func=None, support_core: Union[str, list, tuple] = None):
        """
        ...
        self._add_case(support_soc, test_func, support_core=support_core)
    ...
    def run_case(self, soc: Union[List[str], Tuple[str]] = None,
                 case_name: Union[List[str], Tuple[str]] = None,
                 case_usage: Union[List[str], Tuple[str]] = None,
                 run_cfg: Dict[str, Any] = None, print_summary: bool = False,
                 report_save_file: str = None) -> ut_report.OpUTReport:
        """
        run case

        Parameters
        ----------
        soc: List[str] or Tuple[str]
            the soc need to run
        case_name: List[str] or Tuple[str], default is None
            only run the cases whose case name in this list, default None means run all.
        case_usage: List[str], Tuple[str], default is None
            only run the cases whose case usage in this list, default None means run all.
        run_cfg: Dict[str, Any]
            run configuration, like: simulator_mode, simulator_lib_path, simulator_dump_path, data_dump_path
        print_summary: bool, default is False
            whether to print result summary
        report_save_file: str, default is None
            the name of the file to save summary report
        ...
        for one_soc in soc:
            if self.mock_patches and (not self.mock_soc or soc_util.check_support_soc(self.mock_soc, one_soc)):
                one_rpt = self._run_case(one_soc, case_name, case_usage, run_cfg, mock=True)
                run_rpt.merge_rpt(one_rpt)
            if not (self.mock_patches and self.run_mock_only):
                one_rpt = self._run_case(one_soc, case_name, case_usage, run_cfg)
                run_rpt.merge_rpt(one_rpt)
        end_time = time.time()
        ...
        print(">>>> end run test case, op_type:%s cost time: %d " % (self.op_type, time_taken))
        return run_rpt

    def run(self, soc: Union[str, List[str], Tuple[str]],
            case_name: Union[str, List[str], Tuple[str]] = None,
            simulator_mode: str = None, simulator_lib_path: str = None):
        """
        run ut
        ...
        if case_name and isinstance(case_name, str):
            case_name = type_util.str_to_list(case_name)
        if not soc:
            soc = OpUT.DEFAULT_SUPPORT_SOC
        if isinstance(soc, str):
            soc_list = [x.strip() for x in soc.split(",")]
        if isinstance(soc, (tuple, list)):
            soc_list = soc
        self.run_case(soc_list, case_name, run_cfg=run_cfg,
                      print_summary=True, report_save_file="ut_report.txt")
        self.tiling_ut_gen.gen_tiling_ut()

    def set_mock_patches(self, mock_patches: Dict[Union[str, Tuple[str]], MagicMock] = None,
                         run_mock_only: bool = False,
                         mock_soc: Union[str, Tuple[str], List[str]] = None,
                         mock_case: Union[str, Tuple[str], List[str]] = None,
                         mock_ignore_case: Union[str, Tuple[str], List[str]] = None) -> NoReturn:
        """Set mock_patches and whether to run cases both for mocked and not mocked scenario

        Parameters
        ----------
        mock_patches: dict
            dictionary of mock patches. Example:
            {"A": MagicMock(side_effect=xx), "B": MagicMock(side_effect=xx)} or {("A", "B"): MagicMock(side_effect=xx)}
        run_mock_only: bool, optional
            if True: run cases only for mocked scenario.
            if False: run cases both for mocked and not mocked scenario. Default
        mock_soc: Union[str, Tuple[str], List[str]], optional
            specify the soc versions for which the mock patches will take effect.
            if not specified, will be same as the supported soc versions in case.
        mock_case: Union[str, Tuple[str], List[str]], optional
            cases where the mock patches will take effect. None means all cases
        mock_ignore_case: Union[str, Tuple[str], List[str]], optional
            skip cases where the mock patches should not take effect. None means no case will be skipped
        """
        if not mock_patches:
            mock_patches = {}
        # validate when used
        self.mock_patches = mock_patches
        self.run_mock_only = run_mock_only
        self.mock_soc = mock_soc

        if mock_case and isinstance(mock_case, str):
            mock_case = type_util.str_to_list(mock_case)
        if mock_ignore_case and isinstance(mock_ignore_case, str):
            mock_ignore_case = type_util.str_to_list(mock_ignore_case)

        self.mock_case = mock_case
        self.mock_ignore_case = mock_ignore_case

    def set_op_info_cfg(self, op_info_cfg: dict):
        """Set op info config

        Parameters
        ----------
        op_info_cfg: A dictionary with 3 levels, like: {"ascend910": {"Add": {"coreType.value": None}}}
        """
        self.op_info_cfg = op_info_cfg

    def _set_run_soc(self, run_soc_version, spec_core: Optional[str] = None):
        # there is a bug in te, we can't import te before tensorflow, so can't import te outside
        # 'pylint: disable=import-outside-toplevel
        from tbe.common.platform import set_current_compile_soc_info
        from tbe.common.platform import get_soc_spec
        if not spec_core:
            if self.op_info_cfg is None:
                self.set_op_info_cfg(op_ut_helper.load_op_info_cfg(ops=self.op_type))
            set_current_compile_soc_info(run_soc_version)

            cv_split = get_soc_spec("cube_vector_combine")
            if cv_split == 'unknown':
                cv_split = get_soc_spec("CUBE_VECTOR_SPLIT")
            else:
            ...
            if cv_split:
                ...
                if short_soc_version in self.op_info_cfg and self.op_type in self.op_info_cfg[short_soc_version]:
                    core_type = self.op_info_cfg[short_soc_version][self.op_type].get("coreType.value", None)
                    if not core_type or core_type == "VectorCore":
                        set_current_compile_soc_info(run_soc_version, core_type="VectorCore")
        else:
            set_current_compile_soc_info(run_soc_version, spec_core)

    def _gen_value_depend_op_tmp_file(self, case):
        if self.op_is_value_depend is False:
            return
        case_info = self.op_type + "|" + str(case.get("params"))
        with open(self.value_depend_tmp_file, 'a', encoding='utf-8') as fd:
            fd.write(case_info + "\n")

    def _add_case(self, support_soc=None, case=None,
                  case_usage: op_ut_case_info.CaseUsage = op_ut_case_info.CaseUsage.IMPL,
                  support_core: Union[str, tuple, list] = None):
        if not case:
            return
        ...
        if support_core:
            if isinstance(support_core, str):
                support_core = support_core.split(",") if support_core != "all" else ("AiCore", "VectorCore")
            if not isinstance(support_core, (tuple, list)):
                raise RuntimeError("support_core only accepts a string or a list.")
            if any(c not in ("AiCore", "VectorCore") for c in support_core):
                raise RuntimeError("support_core only accepts: all / AiCore / VectorCore, but got %s" % support_core)
            support_core = tuple(set(support_core))
        ...
        if callable(case):
            case_info = self._build_custom_test_case(support_soc, case,
                                                     case_line_num, support_core=support_core)
        else:
            case_info = self._build_op_ut_case_info(support_soc, case, case_usage,
                                                    case_line_num=case_line_num, support_core=support_core)
            self._gen_value_depend_op_tmp_file(case)
        self._case_info_map[case_info.case_name] = case_info

    def _build_op_ut_case_info(self, support_soc, case,
                               case_usage: op_ut_case_info.CaseUsage = op_ut_case_info.CaseUsage.IMPL,
                               case_line_num=None,
                               support_core: tuple = None) -> op_ut_case_info.OpUTCase:
        ...
        ori_case_name = case.get("case_name")
        if not ori_case_name:
        ...
        gen_tiling_ut = case.get("gen_tiling_ut")
        if not gen_tiling_ut:
            gen_tiling_ut = False
        return op_ut_case_info.OpUTCase(support_soc=support_soc,
                                        ...
                                        ori_case_name=ori_case_name,
                                        gen_tiling_ut=gen_tiling_ut,
    ...
    def _build_custom_test_case(self, support_soc, test_func, case_line_no, support_core: tuple = None):
    ...
    def _run_compile_stage(self, run_soc_version,
                           ...
                           check_exist=False, ascendc_compile_cfg=None) -> op_ut_case_info.OpUTStageResult:
        compile_success, compile_err_msg = self._compile_op_kernel(run_soc_version,
                                                                   case_info=case_info,
                                                                   check_exist=check_exist,
                                                                   ascendc_compile_cfg=ascendc_compile_cfg)
    ...
    def _do_tiling(self, run_soc_version: str, case_info: op_ut_case_info.OpUTCase,
        ...
        self.tiling_ut_gen.collect_tiling_ut_gen_info(case_info,
                                                      {"inputs": input_info_list, "outputs": output_info_list,
                                                       "compile_info": compile_info, "run_info": tiling_info
                                                      })
        return tiling_info
    ...
    def _run_precision_case(self, run_soc_version, case_info: op_ut_case_info.OpUTCase,
        ...
        compile_stage_status = self._run_compile_stage(run_soc_version, case_info, check_exist=True,
                                                       ascendc_compile_cfg=self._get_ascendc_compile_cfg(run_cfg))
    ...
    def _run_one_case(self, run_soc_version, case_info: Union[op_ut_case_info.OpUTCase, op_ut_case_info.OpUTCustomCase],
                      run_cfg: Dict[str, Any] = None) -> ut_report.OpUTCaseReport:
        case_rpt = None
        try:
    ...
    def _run_case(self, one_soc_version: str, case_name_list: List[str] = None,
                 case_usage_list: List = None, run_cfg: Dict[str, Any] = None,
                 mock: bool = False) -> ut_report.OpUTReport:
        """
        run case
        ...
        run_cfg: Dict[str, Any]
            run configuration, like: simulator_mode, simulator_lib_path, simulator_dump_path, data_dump_path
        mock: bool, default is False
            whether run with mock patches
        ...
        run_report: ut_report.OpUTReport
            run report
        """
        print("[%s%s] %s test start running..." % (one_soc_version, " | Mock" if mock else "", self.op_type))
        counts = {"case": 0, "success": 0, "fail": 0, "error": 0}
        total_rpt = ut_report.OpUTReport()
        for case_name, case_info in self._case_info_map.items():
            if self._skip_case_test(one_soc_version, case_info, case_name_list, case_usage_list, mock):
                continue
            support_core = [None] if not case_info.support_core else case_info.support_core
            for spec_core in support_core:
                total_rpt.add_case_report(self._run_one_case_wrapper(one_soc_version, case_name, counts,
                                                                     case_info, spec_core, run_cfg,  mock))
        print("\n\n----------------------------------")
        summary_msg = "run %d tests, success: %d" % (counts.get("case", 0), counts.get("success", 0))
        if counts.get("fail", 0) > 0:
            summary_msg += ", fail: %d" % counts.get("fail", 0)
        if counts.get("error", 0) > 0:
            summary_msg += ", error: %d" % counts.get("error", 0)
        print(summary_msg)
        return total_rpt

    def _skip_case_test(self, soc: str,
                        case_info: Union[op_ut_case_info.OpUTCase, op_ut_case_info.OpUTCustomCase],
                        case_name_list: List = None, case_usage_list: List = None,
                        mock: bool = False) -> bool:
        if not case_info.check_support_soc(soc):
            return True
        if case_name_list and case_info.ori_case_name not in case_name_list \
            and case_info.case_name not in case_name_list:
            return True
        if case_usage_list and case_info.case_usage not in case_usage_list:
            return True
        if mock:
            if self.mock_case and case_info.ori_case_name not in self.mock_case:
                return True
            if self.mock_ignore_case and case_info.ori_case_name in self.mock_ignore_case:
                return True
        return False

    def _run_one_case_wrapper(self, spec_soc: str, case_name: str, counts: dict,
                              case_info: Union[op_ut_case_info.OpUTCase, op_ut_case_info.OpUTCustomCase],
                              spec_core: Optional[str] = None, run_cfg: Dict[str, Any] = None,
                              mock: bool = False):
        print("%s (%s) (%s) %s... " % (case_name, self.op_type, case_info.case_usage.value,
                                       "" if not spec_core else f"({spec_core})"), end="")
        counts["case"] += 1
        self._set_run_soc(spec_soc, spec_core)
        # clear patches before running
        self._stop_all_patches()
        if mock:
            self._start_all_patches()
        # some op (slice) changes input parameters in implements
        case_info_ = copy.deepcopy(case_info)
        case_rpt = self._run_one_case(spec_soc, case_info_, run_cfg=run_cfg)
        self._stop_all_patches()
