...
def get_total_reduce_axis_size(stage):
    """
...
@mask_rule_register([ComputePattern.POOLING], SearchActionType.AT_STAGE)
def mask(progress, action_mask):
    """
    if the total size of reduce axes is smaller than UB_size/2
    do not compute at the reduce stage
    :param progress:
    :param action_mask:
    :return:
    """
    ...
    for i, stg_idx in enumerate(consumer.at_candidates):
        ...
        if not is_reduce_stage(stage):
            continue
        total_reduce_size = get_total_reduce_axis_size(stage)
        reduce_size_limit = get_limit_ub_size(progress.op_schedule_info, stg_idx)

        if total_reduce_size < reduce_size_limit / 2:
            action_mask[i] = 0

    if sum(action_mask) == 0:
        logger.warn('No need to mask, because no available at target.')
    logger.debug("pooling at_stage done with action_mask:%s", action_mask)
    return action_mask
