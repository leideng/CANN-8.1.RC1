...
class AicErrorInfo:
    ...
    def analyse(self: any) -> str:
        ...
        if utils.GLOBAL_RESULT:
            analysis_result = "Analysis result: success. The reason of aicore error please see 'Root cause conclusion'!"
        else:
...
"""
    ...
    def _get_conclusion(self: any) -> str:
        ...
        if not self.atomic_clean_check:
        ...
        elif self.rts_block_dim > self.driver_aicore_num * 2:
            ...
            conclusion = "The maintenance and debugging information is insufficient or the format is incorrect.\n"
    ...
    def _get_aicerror_info(self: any) -> str:
        aicerror_info_list = []
        handled_err_type = []
        ret = utils.hexstr_to_list_bin(self.error_code)
        if not ret:
            ret = [0]
        for i in ret:
