...
graphStatus WithRankAtLeast(const GeTensorDescPtr& tensorDesc, int64_t rank,
                            GeShape& out_shape, const ge::Operator& op) {
  if (rank > INT32_MAX) {
    VECTOR_INFER_SHAPE_INNER_ERR_REPORT(op,
        ConcatString("rank[", rank, "] cannot exceed kint32max"));
    return GRAPH_FAILED;
...
graphStatus WithRank(const GeTensorDescPtr& tensorDesc, int64_t rank,
                     Shape& out_shape, const ge::Operator& op) {
  if (rank > INT32_MAX) {
    VECTOR_INFER_SHAPE_INNER_ERR_REPORT(op,
        ConcatString("rank[", rank, "] cannot exceed kint32max"));
    return GRAPH_FAILED;
...
graphStatus MergePrefix(const Shape& s, const Shape& prefix, Shape &s_out,
                        Shape &prefix_out, const ge::Operator& op) {
...
graphStatus SubShape(const Shape& s, int64_t start, int64_t end, int64_t stride, Shape& out, const ge::Operator& op) {
  ...
  if (start < 0) {
    start = rank + start;
    if (start < 0) {
      VECTOR_INFER_SHAPE_INNER_ERR_REPORT(op, ConcatString("invalid start[",
  ...
  if (end < 0) {
    ...
    if (end < 0) {
      VECTOR_INFER_SHAPE_INNER_ERR_REPORT(op, ConcatString("invalid end[",
  ...
  if (!((stride <= 0 || start <= end))) {
    VECTOR_INFER_SHAPE_INNER_ERR_REPORT(op, ConcatString("start[", start,
...
static graphStatus GetShapeDataFromShapeTensor(Operator& op,
  ...
  data.clear();
  if (dims[0] < 0) {
    OP_LOGI(op, "Shape rank is %zu, dims[0] value is [%ld]", dims.size(),
            dims[0]);
  ...
  if (data_type == DT_INT32) {
    if (op.GetInputConstData(dst_name.c_str(), shape_tensor) == GRAPH_SUCCESS) {
      ...
      OP_LOGI(op, "Input [%s] is not a const tensor.",
              dst_name.c_str());
      for (int64_t i = 0; i < dim_value; i++) {
...
bool ShapeFirstDimDefined(const Shape& shape) {
  if (!RankKnown(shape)) {
    return false;
  }
  std::vector<int64_t> dims = shape.GetDims();
  if (dims.size() > 0 && dims.at(0) == ge::UNKNOWN_DIM) {
    return false;
  }
  return true;
}
...
std::pair<int64_t, int64_t> GetOutputRange(std::pair<int64_t, int64_t> input_range, int64_t rank_size) {
  int64_t first_value = input_range.first * rank_size;
  int64_t second_value;
  if (input_range.second == -1) {  // -1表示此时为动态图，则继续保持-1
    second_value = input_range.second;
  } else {  // 非-1表示此时为静态图，保持原range倍乘逻辑不变
    second_value = input_range.second * rank_size;
  }
...
graphStatus SetShapeAndRange(Operator& op, const ShapeAndRange& feed_shape_and_range) {
  ...
  if (!marks.empty()) {
    ...
    if (aicpu_resource_context == nullptr) {
      ...
      if (aicpu_resource_context == nullptr) {
        AICPU_INFER_SHAPE_INNER_ERR_REPORT(op,
      ...
      if (context->SetResourceContext(marks[0], aicpu_resource_context) != GRAPH_SUCCESS) {
        ...
        AICPU_INFER_SHAPE_CALL_ERR_REPORT(op,
