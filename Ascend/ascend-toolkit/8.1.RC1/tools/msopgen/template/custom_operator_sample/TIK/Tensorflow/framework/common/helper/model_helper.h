...
class GE_FUNC_VISIBILITY ModelHelper : public ModelSaveHelper {
 public:
  ...
  void UpdateCoreCount(const std::string &key, const std::string &context_key, int64_t platform_count,
                       int64_t &core_num_rts, uint32_t &platform_info_count, const std::map<std::string, std::string> &options) const;
 ...
 private:
  ...
  Status LoadAndStoreOppSo(const std::vector<std::string> &op_so_list, const SoBinType so_bin_type);
