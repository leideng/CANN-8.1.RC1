...
public:
  ...
  explicit ParamsContextBase(const T_IN &inOutData, GetAttrAdditional func = nullptr) : inData_(&inOutData), outData_(const_cast<T_IN *>(&inOutData)), getAttr_(func) {}
  explicit ParamsContextBase(const T_IN &inData, T_OUT &outData, GetAttrAdditional func = nullptr) : inData_(&inData), outData_(&outData), getAttr_(func) {}

  inline int32_t GetInputFormat(size_t inputIdx) const { return inData_->GetInputFormat(inputIdx); }
  inline int32_t GetInputDType(size_t inputIdx) const { return inData_->GetInputDType(inputIdx); }
  inline size_t GetInputDimNum(size_t inputIdx) const { return inData_->GetInputDimNum(inputIdx); }
  inline size_t GetInputDim(size_t inputIdx, size_t dimIdx) const { return inData_->GetInputDim(inputIdx, dimIdx); }
  inline size_t GetInputShapeSize(size_t inputIdx) const { return inData_->GetInputShapeSize(inputIdx); }
  inline size_t GetOutputDimNum(size_t outputIdx) const { return outData_->GetOutputDimNum(outputIdx); }
  inline const uint8_t *GetInputTensorData(size_t inputIdx) const { return inData_->GetInputTensorData(inputIdx); }
  template<typename T>
  inline const T *GetAttr(size_t index) const { return reinterpret_cast<const T *>(inData_->GetAttr(index, getAttr_)); }
  template<typename T>
  inline const T *GetAttrList(size_t index) const { return reinterpret_cast<const T *>(inData_->GetAttrList(index, getAttr_)); }
  inline void SetOutputFormat(size_t outputIdx, uint32_t format) const { outData_->SetOutputFormat(outputIdx, format); }
  inline void SetOutputDType(size_t outputIdx, uint32_t dtype) const { outData_->SetOutputDType(outputIdx, dtype); }
  inline void SetOutputDimNum(size_t outputIdx, size_t dimNum) const { outData_->SetOutputDimNum(outputIdx, dimNum); }
  inline void SetOutputDim(size_t outputIdx, size_t dimIdx, int64_t value) const { outData_->SetOutputDim(outputIdx, dimIdx, value); }
