...
graphStatus ValidateTableResourceHandle(const Operator& op,
  ...
  if (op.GetInferenceContext() == nullptr) {
    OP_LOGI(op, "Op inference context is null, return unknwon shape");
  ...
  if (shapes_and_types.empty()) {
    OP_LOGI(op, "Context GetInputHandleShapesAndTypes result is empty, return unknwon shape");
    output_shape_and_type.SetShape(Shape(UNKNOWN_RANK));
    output_shape_and_type.SetType(DT_UNDEFINED);
    return GRAPH_SUCCESS;
  }
  ...
  if (value_shape_and_type.GetDataType() != value_dtype) {
    OP_LOGW(op, "trying to read value with wrong dtype, expected %d, got %d",
