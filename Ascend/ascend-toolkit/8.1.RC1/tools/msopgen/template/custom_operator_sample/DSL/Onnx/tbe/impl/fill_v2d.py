...
@tvm._ffi.register_func("tvm.intrin.cce.elewise_binary_phony_ex")
def elewise_binary_phony_ex(stmt_op):
    """
    elewise_binary_phony_ex which will eliminate its second input tensor completely
    """
    ins, outs, _, _ = tbe.instrinsic.cce_util.get_dma_buffer(stmt_op)
    ...
    for dim in ins[0].shape:
        if total_element == 0:
            ...
            total_element *= dim
    _block_unit_size = ALIGNMENT_BYTES // tbe.instrinsic.cce_util.get_align_factor(dtype)[1]
...
@register_op_compute("FillV2D", op_mode="static", support_fusion=True)
...
def fill_v2d(y, value, shape, kernel_name="fill_v2d"):
    ...
    elewise_sch = tbe.static_schedule.elewise_schedule_new.ElewiseSchedule()
