...
Format TypeUtils::SerialStringToFormat(const std::string &str) {
  ...
  if (TypeUtilsInner::SplitFormatFromStr(str, primary_format_str, sub_format) != GRAPH_SUCCESS) {
...
bool TypeUtils::GetDataTypeLength(const ge::DataType data_type, uint32_t &length) {
  ...
  if (size > 0) {
    ...
    REPORT_INNER_ERROR("E18888", "data_type not support [%s]", DataTypeToSerialString(data_type).c_str());
    GELOGE(GRAPH_FAILED, "[Check][Param] data_type not support [%s]", DataTypeToSerialString(data_type).c_str());
    return false;
  }
}
...
bool TypeUtilsInner::IsDataTypeValid(const DataType dt) {
  ...
  GE_CHK_BOOL_EXEC((num < DT_MAX),
                   REPORT_INNER_ERROR("E18888", "param dt:%u >= DT_MAX:%d, check invalid", num, DT_MAX);
...
graphStatus TypeUtilsInner::SplitFormatFromStr(const std::string &str,
  ...
  if (split_pos != std::string::npos) {
    ...
    if (sub_format > 0xffff) {
      REPORT_INNER_ERROR("E18888", "sub_format: %d is out of range [0, 0xffff].", sub_format);
