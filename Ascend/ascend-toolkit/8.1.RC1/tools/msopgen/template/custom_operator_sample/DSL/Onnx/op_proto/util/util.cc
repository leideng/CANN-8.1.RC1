...
using namespace std;
namespace {
const int64_t DEFAULT_RANGE_LOWER_BOUND = 0;
const int64_t DEFAULT_RANGE_UPPER_BOUND = -1;
} // namespace
  ...
  for (; iter != inputTensorMap.end(); ++iter) {
    ...
    if (dims != first_shape_dims) {
      VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op),
                                          OtherErrMsg(ConcatString("the op shape of param ", iter->first,
                                                                   " must equal with param ", first_name)));
      return false;
    }
  }
  return true;
  ...
  if (!valid) {
    VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op),
                                        OtherErrMsg(ConcatString("The op do not support the dtype: ",
                                                                 ge::TypeUtils::DataTypeToAscendString(input_type).GetString())));
    return false;
  }
  ...
  if (input_type_x1 != input_type_x2) {
    VECTOR_INFER_SHAPE_INNER_ERR_REPORT(
        TbeGetName(op), OtherErrMsg(ConcatString("The ", TbeGetName(op), " op dtype is not same, type1:",
                                                 ge::TypeUtils::DataTypeToAscendString(input_type_x1).GetString(),
                                                 ", type2:", ge::TypeUtils::DataTypeToAscendString(input_type_x2).GetString())));
    return false;
  }
  ...
  for (const string& input_name : input_names) {
    ...
    if (input_dtype != first_input_dtype) {
      auto error_ms = ConcatString("dtype of inputs must be same, ", input_name, ":",
                                   ge::TypeUtils::DataTypeToAscendString(input_dtype).GetString(), ", ", (*first_name), ":",
                                   ge::TypeUtils::DataTypeToAscendString(first_input_dtype).GetString(), ".");
      ...
      if (i < shape_range_x.size() && i < shape_range_y.size()) {
        if (shape_range_x[i].second == -1 && shape_range_y[i].second == 1) {
          ...
          second_range = (shape_range_x[i].second == -1 || shape_range_y[i].second == -1)
                         ? std::max(shape_range_x[i].second, shape_range_y[i].second) : second_range;
          out_range.push_back(std::pair<int64_t, int64_t>(first_range, second_range));
        }
      }
    }
...
static bool InferBroadcastshapeForStatic(const GeShape& shape_x, const GeShape& shape_y, GeShape& shape_output) {
...
bool InferDtypeWithPromoteType(Operator& op, const int64_t& input_idx_1, const int64_t& input_idx_2,
  ...
  // set output
  DataType input1_dtype = tensordesc_input_1->GetDataType();
  DataType input2_dtype = tensordesc_input_2->GetDataType();
  tensordesc_output->SetDataType(opcommon::PromoteType(input1_dtype, input2_dtype));
...
bool InferShapeAndTypeTwoInOneOutBroadcast(Operator& op, const int64_t& input_idx_1, const int64_t& input_idx_2,
  ...
  if ((!shape_x.IsUnknownShape()) && (!shape_y.IsUnknownShape())) {
    ...
    OP_LOGI(TbeGetName(op).c_str(), "output shape is: %s, output dtype is:%s.", to_string(shape_output).c_str(),
            ge::TypeUtils::DataTypeToAscendString(input_dtype).GetString());
    is_dynamic = false;
...
bool InferShapeAndTypeTwoInOneOutBroadcast(Operator& op, const string& input_name1, const string& input_name2,
  ...
  CHECK(op_desc == nullptr || op_desc->MutableInputDesc(input_name1) == nullptr ||
    ...
    REPORT_GET_INPUT_ERR(TbeGetName(op), ConcatString("op_desc")),
    return false);
...
std::string ToFormatString(ge::Format format) {
  return ge::TypeUtils::FormatToAscendString(format).GetString();
}
...
static std::vector<int64_t> GetConstIntData(const uint8_t* const_data, size_t data_size) {
  ...
  auto data = reinterpret_cast<const T*>(const_data);
...
bool GetConstIntData(const Tensor& data, DataType data_type, std::vector<int64_t>& const_values) {
  ...
  if (found == type_call_map.end()) {
    USER_GE_LOGE("[ERROR]GetConstIntData is not support data_type[%s]!",
                 ge::TypeUtils::DataTypeToAscendString(data_type).GetString());
    return false;
  }
...
bool GetConstValue(const Operator& op, const Tensor& const_tensor, const DataType& dtype,
  ...
  if (dtype == ge::DT_INT32) {
    auto const_data_ptr = reinterpret_cast<const int32_t*>(const_tensor.GetData());
    ...
    auto const_data_ptr = reinterpret_cast<const int64_t*>(const_tensor.GetData());
...
bool GetConstValue(const Operator& op, const Tensor& const_tensor, const DataType& dtype,
  ...
  CHECK(dtype != ge::DT_UINT64,
        VECTOR_INFER_SHAPE_INNER_ERR_REPORT(TbeGetName(op), OtherErrMsg("not support this type")),
        return false);
  auto const_data_ptr = reinterpret_cast<const uint64_t*>(const_tensor.GetData());
...
bool DynamicShapeInfer::CatchFormatAndShape() {
  ...
  for (map<std::string, uint32_t>::iterator it = inputs.begin(); it != inputs.end(); ++it) {
    ...
    Format curr_format = static_cast<ge::Format>(ge::GetPrimaryFormat(tensor_desc_input->GetFormat()));
  ...
  for (map<std::string, uint32_t>::iterator it = outputs.begin(); it != outputs.end(); ++it) {
    ...
    Format curr_format = static_cast<ge::Format>(ge::GetPrimaryFormat(tensor_desc_output->GetFormat()));
...
bool DynamicShapeInfer::UpdateFormatAndShape() {
  ...
  for (map<std::string, uint32_t>::iterator it = inputs.begin(); it != inputs.end(); ++it) {
    ...
    ori_input_format = static_cast<ge::Format>(ge::GetPrimaryFormat(tensor_desc_input->GetFormat()));
  ...
  for (map<std::string, uint32_t>::iterator it = outputs.begin(); it != outputs.end(); ++it) {
    ...
    ori_output_format = static_cast<ge::Format>(ge::GetPrimaryFormat(tensor_desc_output->GetFormat()));
...
ge::graphStatus GetAllValidInputsWithIndices(const Operator& op, map<uint32_t, ConstGeTensorDescPtr>& inputs) {
  ...
  for (uint32_t i = 0; i < inputs_size; ++i) {
    ...
    if (input_desc_ptr) {
      ...
      OP_LOGD(TbeGetName(op), "Can not get optional input desc i(%u). Maybe it is not set up", i);
    } else {
...
ge::graphStatus GetDimNumForOp(const Operator& op,
  ...
  for (size_t idx = 0; idx < input_ids.size(); ++idx) {
    ...
    if (shape.IsUnknownDimNum()) {
      dim_num = static_cast<size_t>(ge::UNKNOWN_DIM_NUM);
...
ge::graphStatus InferAxisType4BroadcastOp(const Operator& op, vector<AxisTypeInfo>& axis_type) {
  ...
  // verify broadcast rules for different format
  int last_dim = input_shapes.begin()->second.back();
  for (auto iter: input_shapes) {
    if (iter.second.back() != 1 && last_dim != 1 && iter.second.back() != -1 && last_dim != -1) {
      CHECK(iter.second.back() != last_dim,
            OP_LOGD(TbeGetName(op), "Check shape for broadcast not pass, input shape last dim %ld not equals %d",
                iter.second.back(), last_dim),
            return GRAPH_FAILED);
    }
  }
  ...
  for (size_t axis = 0; axis < dim_num; ++axis) {
    ...
    for (auto iter: output_shapes) {
      if (axis < iter.second.size() && iter.second[axis] != 1) {
        (void)builder.AddOutputCutInfo({static_cast<int64_t>(iter.first), {static_cast<int64_t>(axis)}});
      }
    }
...
ge::graphStatus InferElementwiseAxisTypeHelper(const Operator& op, vector<AxisTypeInfo>& axis_type,
  ...
  CHECK(GetDimNumForOp(op, allowed_split_inputs, allowed_split_outputs, dim_num) != GRAPH_SUCCESS,
        REPORT_GET_INPUT_ERR(TbeGetName(op), ConcatString("input/output shapes")),
        return GRAPH_FAILED);
...
ge::graphStatus InferAxisType4ReduceOpHelper(const Operator& op, const AxisType& reduce_type,
  ...
  for (int64_t dim = 0; dim < rank; dim++) {
    if (find(axis.begin(), axis.end(), dim) != axis.end()) {
      if (x_dims[dim] != 1 && reduce_type != AxisType::UNSPLIT) {
...
void CalcRangeMinMax(const Operator& op, const GeShape& x_shape,
                     const std::vector<std::pair<int64_t, int64_t>>& x_shape_range, int64_t& range_min,
                     int64_t& range_max) {
  range_max = 1L;
  // reshape should follow x input range, x_input_range is[1,-1] or [0,-1], output should follow not expand
  range_min = 1L;
  auto x_shape_size = x_shape.GetShapeSize();
  if (x_shape_size > 0L) {
    // when known dim, x_shape_size is max_elements_count
    range_max = x_shape_size;
    range_min = DEFAULT_RANGE_LOWER_BOUND;
  } else {
    // unknown dim
    if (x_shape_range.empty() || (x_shape_range.size() != x_shape.GetDimNum())) {
      range_max = DEFAULT_RANGE_UPPER_BOUND;
      range_min = DEFAULT_RANGE_LOWER_BOUND;
      return;
    }
    for (size_t i = 0U; i < x_shape_range.size(); ++i) {
      const int64_t lower_bound = x_shape_range[i].first;
      const int64_t upper_bound = x_shape_range[i].second;
      range_min =
          (lower_bound < DEFAULT_RANGE_LOWER_BOUND) ? DEFAULT_RANGE_LOWER_BOUND : std::min(range_min, lower_bound);

      if (upper_bound <= 0) {
        range_max = DEFAULT_RANGE_UPPER_BOUND;
        break;
      }
      if (array_ops::CheckInt64MulOverflow(range_max, upper_bound)) {
...
void GenerateWorstYShapeAndYShapeRange(int64_t y_rank, int64_t range_min, int64_t range_max,
  ...
  for (int64_t i = 0; i < y_rank; ++i) {
    y_shape_range.emplace_back(std::pair<int64_t, int64_t>(range_min, range_max));
  }
}
...
void ReshapeRangeInferAllDims(const Operator &op, const std::vector<std::pair<int64_t, int64_t>> &x_shape_range,
  ...
  int64_t range_max = -1L;
  int64_t range_min = 0L;
  CalcRangeMinMax(op, x_shape, x_shape_range, range_min, range_max);
  GenerateWorstYShapeAndYShapeRange(y_rank, range_min, range_max, y_shape_range, y_shape);
  ...
  if (IsEmptyTensor(x_shape) || has_zero_in_range) {
    InferShapeRangeForEmptyTensor(y_rank, range_max, value_range, y_shape_range, y_shape);
    return;
  }

  // step 3, calculate accurate dims and shape_range
  std::vector<int64_t> y_dims = y_shape.GetDims();
  UpdateDimsAndShapeRange(op, range_max, value_range, y_dims, y_shape_range);
  y_shape = GeShape(y_dims);
}
...
static int64_t CalculateMaxInputDims(const std::vector<std::pair<int64_t, int64_t>>& x_range, const Operator &op) {
...
void ReshapeRangeInfer(const Operator &op, const std::vector<std::pair<int64_t, int64_t>>& x_range,
  ...
  if (max_input_dims < 0) {
    ...
    for (const auto dim : output_shape.GetDims()) {
      if (dim < 0) {
        y_range.emplace_back(std::pair<int64_t, int64_t>(has_zero_in_range ? 0 : 1, left));
    ...
    if (unknown_dim_indexes.size() == 1U) {
      y_range[unknown_dim_indexes[0U]] = std::pair<int64_t, int64_t>(has_zero_in_range ? 0 : 1, left);
    }
  }
}
