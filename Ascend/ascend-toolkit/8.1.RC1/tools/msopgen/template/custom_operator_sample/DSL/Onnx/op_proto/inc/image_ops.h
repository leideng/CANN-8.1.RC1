...
*@par Attributes:
*@li centered: An optional bool, default is true, indicates if the offset coordinates are centered relative to
...
of the input images.
*@li normalized: An optional bool, default is true, indicates if the offset coordinates are normalized.
*@li uniform_noise: An optional bool, default is true, indicates if the noise should be generated using a
uniform distribution or a Gaussian distribution.
*@li noise: An optional string, default is "uniform", indicates if the noise should uniform, gaussian, or zero.
...
*@li half_pixel_centers: An optional bool. Defaults to False. indicates if the offset coordinates are normalized . \n

*@par Outputs:
*@li resized_images: 4-D with shape [batch, new_height, new_width, channels], Must be one of the following types:
...
*@par Inputs:
*Input grads must be a 4-D tensor. Inputs include:
*@li grads: A Tensor. Must set the format, supported format list ["NCHW, NHWC"].
Must be the type float32.
*@li original_image: A 4-D Tensor. Must set the format, supported format list ["NCHW, NHWC"].
* Must be one of the following types: float16, float32, double. The image tensor that was resized.
* Has the same format as grads. The dimensions of the N-axis or C-axis of grads and original_image are same. \n
...
*@li half_pixel_centers: An optional bool. Indicates if the offset coordinates are normalized. Defaults to false. \n

*@par Outputs:
*y: A Tensor. Has the same type, format and shape as original_image. \n
...
*@li grads: A 5D tensor of type float32. Must set the format, supported format NC1HWC0.
*@li original_image: A 5D tensor of type float32. Must set the format, supported format NC1HWC0.
...
false .
*@li half_pixel_centers: Bool type, indicates if the offset coordinates are normalized. Defaults
to false . \n
...
*@par Inputs:
* Inputs include:
*@li x: A tensor. Input data for 2D bilinear upsampling. Must set the format, supported format list ["NC1HWC0, NHWC"].
* Support 4D and 5D. When the shape is 5D, the data format must be NC1HWC0.
* When the shape is 4D, the data format must be NHWC.
...
* @li half_pixel_centers: An optional bool. Indicates if the offset coordinates are normalized. Defaults to false.
* @li dtype: An optional type attr, support type list [DT_FLOAT, DT_UINT8].
* It is applicable only to the data type of the specified output y in the Dvpp scenario.
* Defaults to DT_FLOAT. \n

*@par Outputs:
* y: A 4-D tensor with shape [batch, new_height, new_width, channels]. Output data for 2D bilinear upsampling.
* Must be one of the following types: uint8, float32, float16. Has the same format as "x".
* The H and W dimensions of the output y are consistent with the H and W dimensions of the input size,
* and the other dimensions are consistent with the input x. \n
...
*@par Inputs:
*Input images must be a 4-D tensor. Inputs include:
*@li x: 4-D float32 tensor. Must set the format, supported format NC1HWC0.
*@li size: A 1-D int32 Tensor of 2 elements: new_height, new_width. The new
size for the images. \n

*@par Attributes:
*@li align_corners: Bool type. If true, the centers of the 4 corner pixels of the input and
output tensors are aligned, preserving the values at the corner pixels.
Defaults to false.
*@li half_pixel_centers: An optional bool. Defaults to False . \n
*@li ori_image_size: An optional listint. Defaults to [].
...
*@par Attributes:
*@li seed: An optional int, default is 0, if either seed or seed2 are set to non-zero, the random number
generator is seeded by the given seed. Otherwise, it is seeded by a random seed.
*@li seed2: An optional int, default is 0, A second seed to avoid seed collision.
*@li min_object_covered: An optional float, default is `[0.1f]`, The cropped area of the image must contain at least
...
*@li aspect_ratio_range: An optional list of `floats`. Defaults to `[0.75, 1.33]`, the cropped area of the image must have an aspect
ratio = width / height within this range.
*@li area_range: An optional list of `floats`. Defaults to `[0.05, 1]`. The
cropped area of the image must contain a fraction of the supplied image
within this range.
*@li max_attempts: An optional int, default is 100, Number of attempts at generating a cropped region of the
image of the specified constraints. After max_attempts failures, return the
entire image.
*@li use_image_if_no_bounding_boxes: An optional bool, default is false, controls behavior if no bounding boxes
...
generator is seeded by the given seed. Otherwise, it is seeded by a random seed.
*@li seed2: An optional int, default is 0, a second seed to avoid seed collision.
*@li aspect_ratio_range: An optional list of `floats`. Defaults to `[0.75f, 1.33f]`,
...
*@par Attributes:
*@li format: An optional string, default is "". Per pixel image format.
*@li quality: An optional int, default is 95. Quality of the compression from 0 to 100 (higher is better
and slower).
*@li progressive: An optional bool, default is false. If true, create a JPEG that loads progressively (coarse
to fine).
*@li optimize_size: An optional bool, default is false. If true, spend CPU/RAM to reduce size with no quality
change.
*@li chroma_downsampling: An optional bool, default is true.
*@li density_unit: An optional string, default is "in". Unit used to specify x_density and y_density: pixels per
inch ('in') or centimeter ('cm').
*@li x_density: An optional int, default is 300. Horizontal pixels per density unit.
*@li y_density: An optional int, default is 300. Vertical pixels per density unit.
*@li xmp_metadata: An optional string, default is "". If not empty, embed this XMP metadata in the image header . \n
...
*@par Attributes:
*@li channels: graph channels. An optional int, default is 0.
*@li dtype: type of image. An optional attribute, default is uint8.
...
*@par Attributes:
*@li pad_to_max_output_size: An optional bool. If true, the output `selected_indices` is padded to be of length
`max_output_size`. Defaults to false. If not specified, defaults to false.
*@li T: The data type of the output tensors. Can be float16, float32. \n
...
* @par Inputs:
* Input grads must be a 4-D tensor. Inputs include:
* @li grads: A Tensor. Must be one of the following types: uint8, uint16, uint32, uint64, int16,
int32, int64, float16, float, double. 4-D with shape [batch, height, width, channels]
or shape [batch, channels, height, width]. The format support ND.
* @li roi: A 1-D Tensor. Must be one of the following types: float16, float, double.
Only takes effect when attr coordinate_transformation_modeis "tf_crop_and_resize". The format support ND.
* @li scales: A 1-D float Tensor, the scale array along each dimension. The format support ND.
* @li original_size: A 1-D int64 or int32 Tensor, The size of the output tensor. The format support ND. \n
...
* Shape depends on grads and sizes. \n
*/
REG_OP(ResizeGradD)
...
* Must be one of the following types: `[float16, float32, bfloat16, double]`.
* The format must be NHWC. 
*@li grid: Flow field grid, 4-D tensor with shape `[batch, out_height, out_width, 2]`. Must be one of the following types:
...
* Defaults to "zeros".
*@li align_corners: An optional bool. Indicates the mapping mode between the coordinates of the feature map and the feature value.
* If "true", the centers of the corner pixels of the input and output tensors are aligned. Defaults to false.

*@par Outputs:
*@li dx: Indicates the output gradient of backpropagation. Returns 4-D tensor with the same dtype, format and shape as `x`.
...
* @li data_format: An optional string specifying the data formats, 
...
The format must be NHWC.
* @li size: A ND(Support 1D) int32 tensor of 2 elements containing the size of the
glimpses to extract. The glimpse height must be specified first, following
by the glimpse width.
* @li offsets: A ND(Support 2D) float32 tensor of shape [batch_size, 2] containing the y,
x locations of the center of each window . \n

* @par Attributes:
* @li centered: bool indicates if the offset coordinates are centered
...
* @li normalized: bool indicates if the offset coordinates are normalized.
* @li uniform_noise: bool indicates if the noise should be generated using a
uniform distribution or a Gaussian distribution.
* @li noise: string indicates if the noise should uniform, gaussian, or zero.
The default is uniform which means the the noise type will be decided by
uniform_noise . \n
