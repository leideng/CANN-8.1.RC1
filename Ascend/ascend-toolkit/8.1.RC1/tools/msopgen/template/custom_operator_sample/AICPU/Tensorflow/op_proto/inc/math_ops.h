...
* @li min: A tensor of type float16, float32, int64, int32, int16, int8, uint8.
* @li max: A tensor of type float16, float32, int64, int32, int16, int8, uint8.
*@par Attributes:
...
*@par Attributes:
*@li boundaries:A sorted list of floats gives the boundary of the buckets.
*@li dtype: An optional int32 or int64. The output data type. Defaults to int32.
*@li right: An optional true or false. If true, return unpperbound index. If false return lowerbound index.
...
*@li x:A Tensor. Must be one of the following types: float16, float, double, int32,
...
*@par Inputs:
*The input indices and segment_ids must have same rank. Inputs include:
*@li x: A Tensor. Must be one of the following types: float16, float, double.
*@li indices: A Tensor. Must be one of the following types: int32, int64.
A 1-D tensor. Has same rank as segment_ids.
...
*@par Attributes:
*@li output_types: An optional list. A nested structure of DType objects corresponding to each
component of an element of this dataset.
*@li output_shapes: An optional list list of int. A nested structure of TensorShape objects corresponding
to each component of an element of this dataset.
*@li output_num: An optional int that indicates the number of output. Defaults to 1.
*@li channel_name: An optional string. Defaults to "". \n
...
* @par Inputs:
* @li x: A 2D Tensor dtype of float32 or bfloat16 with shape (N, C).
* @li y_grad: A 1D Tensor dtype of float32 or bfloat16 with shape (N) if reduction is "none".Otherwise, shape is (1).
* @li target: A 1D Tensor dtype of int32, int64 with shape (N).
* @li weight: A 1D Tensor dtype of float32 or bfloat16 with shape (C).
* @li total_weight: A 1D Tensor dtype of float32 or bfloat16 with shape (1).

* @par Attributes:
* @li reduction: An optional string. Defaults to "mean" .
* @li ignore_index: An optional int. Defaults to -100 .

* @par Outputs:
* x_grad: A Tensor has the same shape as "x". Must be the following type: float32, bfloat16.
...
* @li When the tensor "x" is empty and "p" is infinity, we cannot reduce the whole tensor or reduce over an empty dimension.
...
* @par Inputs:
* @li grad: A ND(Support 1D~8D) Tensor. Must be one of the following types: bfloat16,
  float16, float32, double. gradient propagated to the SparseSegmentSum op.
* @li indices: A ND(Support 1D) Tensor. Must be one of the following types: int32, int64.
  indices passed to the corresponding SparseSegmentSum op.
* @li segment_ids: A ND(Support 1D) Tensor of type int32, int64. segment_ids passed to the
  corresponding SparseSegmentSum op, has save rank as indices.
* @li output_dim0: A Scalar of type int32. dimension 0 of "x" passed to
  SparseSegmentSum op . \n
...
* @par Attributes:
* @li dtype: Data type of output samples. Defaults to float32. \n

* @par Outputs:
* @li y: A Tensor with the float32 or double type generated samples. \n
