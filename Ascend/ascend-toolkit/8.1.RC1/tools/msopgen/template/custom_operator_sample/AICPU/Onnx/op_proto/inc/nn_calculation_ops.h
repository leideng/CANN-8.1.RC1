...
* @li filter_size: A 4D tensor of with shape [filter_height, filter_width,
* in_channels, out_channels] or [out_channels, filter-height, filter_width,
* in_channels]. The dimensions of filter-height and filter_width.
...
* @par Inputs:
* Two inputs include: \n
* @li input: A 4D Tensor of input image. With the format "NHWC" which shape is
...
* @li out_backprop: 4D tensor with shape  [batch, out_height, out_width,
* out_channels] or [batch, out_channels, out_height, out_width].
* Must have the same type as x. Support type float16, bfloat16 and float32.

* @par Attributes:
* @li filter_size: A required list or tuple. Same with filter shape.
...
* Must be of type float16 or float32. \n
...
/**
*@brief Computes a 2D depth-wise convolution given a 4D input tensor and
* a 4D filter tensor. \n

*@par Inputs:
*Two required inputs and two optional inputs, including: \n
* @li x: A 4D tensor of type float16 or int8 or float32, with shape
* [N, C, H, W] or [N, H, W, C].
* @li filter: A 4D tensor of type float16 or int8 or float32, with shape
* [H, W, C, K].
...
*\n
* @par Outputs:
* y: 4D tensor of type float16 or int32 or float32, with shape [N, C, H, W]
* or [N, H, W, C].
...
* @par Inputs:
 * Three inputs:
 * @li input_size: A Tensor of type int32. Currently does not support
 ...
 * on feature map
 * @li dilations: A optional tuple/list of 4 integers, The dilation factor for each
 ...
 * @li groups: a optional integer of blocked connections 
...
* @par Inputs:
 * Two inputs:
 * @li filter: A Tensor. Types is int8, float16, bfloat16.
 * 4-D with shape [filter_height, filter_width, in_channels, out_channels]
 * or [out_channels, filter_height, filter_width, in_channels]
 * or [out_channels, in_channel, filter_height, filter_width].
 * @li out_backprop: A Tensor. Must have the same type as filter.
 * Support type int8, float16, bfloat16.
 * 4-D with shape [batch, out_height, out_width, out_channels]
 * or [batch, out_channels, out_height, out_width].
 * Gradients with respect to the output of the convolution.
*@par Attributes:
 * Six attributes:
 * @li input_size A tuple/list of 4 integers. An integer vector representing the
 ...
 * "NHWC". Specify the data format of the input and output data.
*@par Outputs:
 * y: A Tensor. with the type of: float16, bfloat16, float32, int32, 4-D tensor
...
*\n
* If any dimension of Input/Filter/Bias/Output shape exceeds max
* int32(2147483647), the product of each dimension of Input/Filter/Bias/Output
* shape exceeds max int32(2147483647) or the value of
* stride/padding/dilation/offset_x exceeds the range in the above table,
...
* For example: 'The conv2d has entered the DMA processing process. A timeout
* AiCore error may be reported. If a timeout AiCore error is reported, reduce
* the conv2d specifications and try again'.
*\n
*
*@par Outputs:
...
/**
* @brief Computes a quant and 2D convolution given 4D "x", "filter" and
* "scale" tensors.
* @par Inputs:
* @li x: A 4D tensor of input image. With the format "NHWC" or "NCHW", the data
* is stored in the order of:[batch, in_height, in_width, in_channels]
* or [batch, in_channels, in_height, in_width].
* @li filter: A 4D tensor of learnable filters. With the format "NCHW" or
* "HWCN", the data is stored in the order of:
...
*\n
*\n
* The following are the supported data types and data formats (for Atlas Inference Series Product):
*\n
*\n
...
* If any dimension of Input/Filter/Bias/Output shape exceeds max
* int32(2147483647), the product of each dimension of Input/Filter/Bias/Output
* shape exceeds max int32(2147483647)
* the correctness of the function cannot be guaranteed.
*
*\n
*
...
*@par Inputs:
 * @li x: A 5D tensor of input image. Must be one of the following types: float16, int8, bfloat16.
          The format of x is NCDHW or NDHWC.
          The data is stored in the order of:[batch, in_channels, in_depth, in_height, in_width] or
          [batch, in_depth, in_height, in_width, in_channels].
          Any dimension of x shape must be in [1, 2147483646].
 * @li filter: A 5D tensor of learnable filters. Must be one of the following types: float16, int8, bfloat16.
               Must have the same type as "x".
               The format support NCDHW or DHWCN.
               The data is stored in the order of:[out_channels, in_channels, in_depth, in_height, in_width] or
               [in_depth, in_height, in_width, in_channels, out_channels].
               The height (H) and width (W) dimensions must be in [1, 511].
               The other values of filter_size must be in [1, 2147483646].
 * @li bias: An optional 1D tensor of additives biases to the filter outputs. The format support ND.
       Must be one of the following types: float16, float32, int32. The data is stored in the order of:[out_channels].
       "out_channels" must equals to the "out_channels" of output y.
 * @li offset_w: An optional 1D tensor of for quantized deconvolution. Reserved. \n
 ...
 * @li strides: Required. A list of 5 integers. Specifies the stride of the
                ...
                The H and W dimensions must be in [1, 63].
                The D dimensions must be in [1, 2147483646].
 * @li pads: Required. A list of 6 integers. Supports only padding along the D, H and W dimensions in sequence of head,
             tail, top, bottom, left and right.
             The top, bottom, left and right must be in [0, 255].
             The head and tail must be in [0, 2147483646].
 * @li dilations: Optional. A list of 5 integers. Specifies the dilation
                  ...
                  The N and C dimensions must be 1. Default value is [1, 1, 1, 1, 1].
                  The H and W dimensions must be in [1, 255].
                  The D dimensions must be in [0, 2147483646].
 * @li groups: Optional. An integer of type int32. Number of blocked connections from input channels
               to output channels. in_channels and out_channels must both be divisible by value of "groups".
               Default value is 1.
               The value of groups must be in [1, 65535].
 * @li data_format: Optional. An string from: "NDHWC", "NCDHW".
                    Defaults to "NDHWC". Specify the data format of the input x and output data.
...
* y: A 5D Tensor of output feature map. Has the same type as "x". With the format "NCDHW" or "NDHWC",
     the data is stored in the order of: [batch, out_channels, out_depth, out_height, out_width] or
     [batch, out_depth, out_height, out_width, out_channels].
     Any dimension of y shape must be in [1, 2147483646].
*\n
*     out_height = (in_height + pad_top + pad_bottom -
*                   (dilation_h * (filter_height - 1) + 1))
*                  / stride_h + 1
*\n
*     out_depth = (in_depth + pad_head + pad_tail -
...
*@attention Constraints:
 * @li The image size after padding is greater than the filter size.

 * @li The W dimension of the input x supports cases exceeding 4096, but it may
 * cause compilation errors.

 * @li If any dimension of x/filter/bias/y shape exceeds max int32(2147483647),
 * the product of each dimension of x/filter/bias/y shape exceeds max int32(2147483647) or
 * the value of stride/pads/dilation/offset_x exceeds the range in the above table,
 * the correctness of the function cannot be guaranteed.
 ...
 * If a timeout AI Core error is reported, reduce the Conv2D specifications and try again' \n
 ...
 * The filter_height and filter_width dimensions must be in [1, 511].
 ...
 * The top, bottom, left and right must be in [0, 255].
 * The front and tail must be in [0, 2147483647].
 ...
 * @li groups: Optional. Defaults to 1 and the value must be in [1, 65535].
...
*@brief Computes the gradients of convolution 3d with respect to the input.
*@par Inputs:
 * @li filter: A 5-D Tensor. Supported types includes float16, bfloat16.
 * The format of the filter tensor must be one of the followings:
 * [out_channels, in_channels/groups, filter_depth, filter_height, filter_width] or
 * [filter_depth, filter_height, filter_width, in_channels/groups, out_channels].
 ...
 * [batch, out_depth, out_height, out_width, out_channels] or
 * [batch, out_channels, out_depth, out_height, out_width].
 * The out_height and out_width dimensions must be in [1, 4096].
 * Gradients with respect to the "output" of the convolution.
*@par Attributes:
 * @li input_size: Required. An tuple/list of 5 integers representing the shape of
 ...
 * Modes "SAME" and "VAILD" padding can be achieved with appropriate values of each direction in pads.
 * The top, bottom, left and right must be in [0, 255].
 * The front and tail must be in [1, 2147483647].
 * @li dilations: Optional. Defaults to [1, 1, 1, 1, 1].
 * A tuple/list of 5 integers, The dilation factor for each dimension of filter.
 * The dilations has the same axes sequence as filter:
 * [out_channels, in_channels/groups, depth, dilation_height, dilation_width] or
 ...
 * depth (D), height (H) and width (W) dimensions must be in [1, 255].
 * In graph mode, only configuration to [1, 1, 1, 1, 1] is supported.
 * @li groups: Optional. Defaults to 1 and the value of groups must be in [1, 65535].
 * Number of blocked connections from in_channels to out_channels.
 * @li data_format: Optional. Defaults to "NDHWC". An string from: "NDHWC", "NCDHW".
 * The correspondence is as follows: batch(N), depth(D), height(H), width(W), channels(C).
 * Specify the data format of the feature map, out_backprop and output.
*@par Outputs:
 * y: A Tensor. Supported types includes float16, bfloat16, float32.
 * The tensor has the same format as out_backprop and feature map.
 * The gradients of feature map.
 * The H and W dimensions must be in [1, 4096].
*@attention Constraints:\n
 * Due to hardware resource restrictions,
 * the operator fails to be executed in scenarios of some parameter value combinations.
 * Analyze and rectify the fault based on the log information.
...
*@par Inputs:
 * @li x: A Tensor. Must be one of the following types: float16, bfloat16 or float32.
 ...
 * The height (H) and width (W) dimensions must be in [1, 255].
 * Additionally, Atlas Training Series Product required Depth (D) dimension must be in [1, 255]
 * The other values of filter_size must be in [1, 2147483646].
 * @li out_backprop: Gradients with respect to the output of the convolution. 
 ...
 * The batch(N) and channels(C) must be 1.
 * The stride_height(H) and stride_width(W) must be in [1, 63].
 * The stride_depth(D) must be in [1, 2147483646].
 ...
 * depth(D) must be in [1, 2147483646].
 * In graph mode, only configuration to [1, 1, 1, 1, 1] is supported.
...
/**
*@brief Computes the gradients of convolution 3d with respect to the filter.

*@par Inputs:
 * @li x: A Tensor of type float16 or bfloat16.
 * 5-D with shape [batch, in_depth, in_height, in_width, in_channels]
 * or [batch, in_channels, in_depth, in_height, in_width].
 * The in_height and in_width dimensions must be in [1, 4096].
 * The other dimensions of x shape must be in [1, 2147483646].
 * @li out_backprop: A Tensor. Must have the same type as x.
 * 5-D with shape [batch, out_depth, out_height, out_width, out_channels]
 * or [batch, out_channels, out_depth, out_height, out_width].
 * The out_height and out_width dimensions must be in [1, 4096].
 * The other dimensions of out_backprop shape must be in [1, 2147483646].
 * Gradients with respect to the output of the convolution. \n
 ...
 * The D, H and W dimensions must be in [1, 4096].
 * The other dimensions of y shape must be in [1, 2147483646]. \n
 ...
 * Currently bias is not supported on Atlas 200/500 A2 Inference Product and Atlas A2
 * Training Series Product/Atlas 800I A2 Inference Product/A200I A2 Box Heterogeneous Component. Reserved.
 * @li offset_w: Optional. An optional 1D tensor for quantized deconvolution.
 * Currently offset_w is not supported on Atlas 200/500 A2 Inference Product and Atlas A2
 * Training Series Product/Atlas 800I A2 Inference Product/A200I A2 Box Heterogeneous Component. Reserved. 
 ...
 * Currently offset_x is not supported on Atlas 200/500 A2 Inference Product and Atlas A2 Training
 * Series Product/Atlas 800I A2 Inference Product/A200I A2 Box Heterogeneous Component. Reserved. 
 ...
 * is float16, bias is float16. When x is bfloat16, bias is float32.
 * Currently bias is not supported on Atlas 200/500 A2 Inference Product and Atlas A2 Training
 * Series Product/Atlas 800I A2 Inference Product/A200I A2 Box Heterogeneous Component.
 * @li offset_w: Optional. An 1D tensor for quantized deconvolution.
 * Currently offset_w is not supported on Atlas 200/500 A2 Inference Product and Atlas A2 Training
 * Series Product/Atlas 800I A2 Inference Product/A200I A2 Box Heterogeneous Component. Reserved. 

*@par Attributes:
 * @li input_size: Required. A tuple/list of type int32.
 * An integer vector representing the shape of input.
 * The H and W must be in [1, 4096].
 ...
 * @li pads: Required. A tuple/list of 6 integers.
 * [front, back, top, bottom, left, right] pads on feature map.
 * The top, bottom, left, right must be in [0, 255].
 * The front and back must be in [0, 2147483647].
 * @li dilations: Optional. A tuple/list of 5 integers, The dilation factor for each
 * dimension of input.
 * The batch(N) and channel(C) must be 1. Has the same format as "x".
 * The depth (D), height (H) and width (W) must be in [1, 255].
 * In graph mode, only configuration to [1, 1, 1, 1, 1] is supported.
 ...
 * @li output_padding: Optional. The size will be added in the output shape.
 * Defaults to [0, 0, 0, 0, 0].
 * Currently output_padding is not supported on Atlas 200/500 A2 Inference Product and Atlas A2
 * Training Series Product/Atlas 800I A2 Inference Product/A200I A2 Box Heterogeneous Component.
 * In graph mode, only configuration to [0, 0, 0, 0, 0] is supported.
 * @li offset_x: Optional. Input offset_x value. Defaults to 0.
 * Currently offset_x is not supported on Atlas 200/500 A2 Inference Product and Atlas A2
 * Training Series Product/Atlas 800I A2 Inference Product/A200I A2 Box Heterogeneous Component. Reserved.
 ...
 * @li input_size: A required list or tuple of type int32 or int64. An integer vector representing the
 * shape of input.
 * @li strides: A required list or tuple of int32 or int64. The stride of the sliding window for
 ...
 * Defaults to "0".
*@par Outputs:
 * y: A Tensor. Support type: float16, int32, float32.
*@par Restrictions:
...
* @li modulated: An optional bool specify version of DeformableConv2D, true means v2, false means v1. Defaults to "true".
 ...
 * The format support NHWC. Shape support 4D.
 * @li x: A Tensor of type float16, float32, bfloat16. The format support NHWC.
 * Shape support 4D. Has the same format and dtype as grad.
 * @li offsets: A Tensor of type float16, float32, bfloat16. Deformation offset parameter.
 * The format support NHWC. Shape support 4D. Has the same format and dtype as grad.
*@par Attributes:
 * @li strides: A tuple/list of 4 integers.The stride of the sliding window for
 * height and width for H/W dimension.
 * @li pads: A tuple/list of 4 integers.Padding added to H/W dimension
 ...
 * The format support NHWC. Shape support 4D. Has the same format, shape  as input "x".
 * @li grad_offsets: A Tensor of type float16, float32. Gradients with respect to input_offsets.
 * The format support NHWC. Shape support 4D. Has the same format, shape as input "offsets".
*/
...
*@par Attributes:
 * @li dilations: Controls the spacing between the input x. A required tuple/list of integers with format NCHW.
 * @li pads: Controls the amout of padding applied to the input. A optional tuple/list of integers. Defaults to [].
 * @li padding_value: A optional float32 value. default value filling in blank. Defaults to 0.
*@par Outputs:
...
* @par Attributes:
* @li output_dtype: The data type of output. Defaults to float32.  \n
