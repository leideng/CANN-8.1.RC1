...
/*
 * infer dtype of two input and on output with PromoteType in op_common/data_type_utils.h
 * param[in] op: op desc supply by ge
 * param[in] input_idx_1: input1 idx
 * param[in] input_idx_2: input2 idx
 * param[in] output_idx: output idx
 * @return true:success, false:failed.
 */
...
bool CheckSimilarInputDtypeAndFormat(const Operator& op, std::size_t inputNumBeg, std::size_t inputNumEnd,
  ...
  for (std::size_t i = inputNumBeg; i < inputNumEnd; i++) {
    if (std::is_same<typename std::decay<T>::type, ge::DataType>::value) {
      ...
      ge::Format inType = static_cast<ge::Format>(ge::GetPrimaryFormat(op.GetInputDesc(i).GetFormat()));
      const auto& findDtype = std::find(supportList.begin(), supportList.end(), inType);
      if (findDtype == supportList.end()) {
        return false;
      }
...
public:
  ...
  AxisTypeInfoBuilder &SetAdditionInfo(const std::map<std::string, std::string> &addition_info) {
    axis_type_info_.SetAdditionInfo(addition_info);
    return *this;
  }
