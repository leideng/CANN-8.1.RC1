...
* @par Inputs:
* Two inputs, including:
* @li anchor_box: Anchor boxes. A 2D Tensor of float32 or float16 with shape (N, 4).
* "N" indicates the number of bounding boxes, and the value "4" refers to
* "x0", "x1", "y0", "y1".
* @li ground_truth_box: Ground truth boxes. A 2D Tensor of float32 or float16 with
...
* @li roi_end_mode: An optional attribute of type int, specifying the align mode . Defaults to "1". \n
...
/**
*@brief Performs SSD prior box detection, with four additional matrices and the
*"aspect_ratio" attribute deleted compared to PriorBox .

*@par Inputs:
* Six inputs, including:
*@li x: An NCHW feature map of type is float32 or float16.
*@li img: source image. Has the same type and format as "x".
*@li data_h: An NCHW tensor of type float32 or float16, specifying the matrix
*for indexing the feature map height.
*@li data_w: An NCHW tensor of type float32 or float16, specifying the matrix
*for indexing the feature map width.
*@li box_height: An NCHW tensor of type float32 or float16, specifying the
*height of each prior box.
*@li box_width: An NCHW tensor of type float32 or float16, specifying the width
*of each prior box . \n

*@par Attributes:
*@li min_size: A required float32, specifying the minimum edge length of a
*square prior box.
*@li max_size: A required float32, specifying the maximum edge length of a
*square prior box: sqrt(min_size * max_size)
...
*@li step_h: An optional float32, specifying the height step for mapping the
*center point from the feature map to the source image.
*@li step_w: An optional float32, specifying the width step for mapping the
*center point from the feature map to the source image.
*@li flip: An optional bool. If "True", "aspect_ratio" will be flipped.
*Defaults to "True".
*@li clip: An optional bool. If "True", a prior box is clipped to within [0, 1].
* Defaults to "False".
*@li offset: An optional float32, specifying the offset. Defaults to "0.5".
*@li variance: An optional float32, specifying the variance of a prior box,
*either one or four variances. Defaults to "0.1" (one value) . \n

*@par Outputs:
*y: An ND tensor of type float32 or float16, specifying the prior box
*information, including its coordinates and variance . \n
...
/**
*@brief Performs SSD prior box detection, with four additional matrices and
*the "aspect_ratio" attribute deleted compared to PriorBox .

*@par Inputs:
* Six inputs, including:
*@li x: An NCHW feature map of type is float32 or float16.
*@li img: source image. Has the same type and format as "x".
*@li boxes: An ND tensor of type float32 or float16, specifying the prior box
*information. Same as output y
...
*@li group_size: A required int, specifying the number of groups to encode
* position-sensitive score maps, must be within the range (0, 128).
*@li spatial_scale: A required float, scaling factor for mapping the input
...
*@li rois: An NCHW tensor of type floa16 or float32, output from operator
*proposal_d at the preceding layer, used as the input of operator
*FSRDetectionOutput.
*@li bbox_delta: An NCHWC0 tensor of type floa16 or float32, specifying the
*prediction offset, used to update the coordinates [x1, y1, x2, y2] of each ROI.
*@li score: An NCHWC0 tensor of type floa16 or float32, specifying the
...
*information.
*@li actual_rois_num: An optional NCHW tensor of type int32, specifying the
*number of valid boxes per batch. \n
*@par Attributes:
*@li batch_rois: An optional int, specifying the number of images to be
*predicted. Defaults to "1".
*@li num_classes: An required int, specifying the number of classes to be
*predicted. The value must be greater than 0.
*@li score_threshold: An required float, specifying the threshold for box
*filtering. The value range is [0.0, 1.0].
*@li iou_threshold: An required float, specifying the confidence threshold for
*box filtering, which is the output "obj" of operator Region. The value range
*is (0.0, 1.0). \n
*@par Outputs:
*@li actual_bbox_num: A tensor of type int32 With shape [bacth, num_classes],
*specifying the number of output boxes .
*@li box: A tensor of type float16 or float32 for proposal of actual output,
...
*@li "score" must be with shape (total_num, (num_classes+15)//16, 1, 1, 16),
*where "total_num" indicates the number of valid input boxes of all images.
*@li "bbox_delta" must be with shape (total_num, (num_classes*4+15)//16, 1, 1,
...
*@par Attributes:
*@li boxes: An optional int, specifying the number of anchor boxes. Defaults to
* "3". when "yolo_version = V2", Defaults to "5".
*@li coords: An optional int, specifying the number of parameters required for locating
* an object. The value is fixed at "4", corresponding to (x,y,w,h). Defaults to
* "4".
*@li classes: An optional int, specifying the number of prediction classes. Defaults to
* "80". The value range is [1, 1024].
*@li yolo_version: An optional string, specifying the YOLO version, either "V2" or "V3".
* Defaults to "V3".
*@li softmax: An optional bool, specifying whether to perform softmax, valid only when
* "yolo_version = V2". Defaults to "false".
*@li background: An optional bool, specifying the operation types of the obj and classes,
* used in conjunction with "softmax" and valid only when "yolo_version = V2".
* Defaults to "false".
*@li softmaxtree: An optional bool, Fixed to False, defined in Lite, but not used.
* Defaults to "false" . \n

*@par Outputs:
*@li coord_data: A float16 or float32 with shape [N, boxes*coords,
...
* detected box.
*@li obj_prob: A float16 or float32 with
...
* confidence.
*@li classes_prob: A float16 or float32 with
...
*@li The succeeding layer of the Yolo operator must be operator
...
*@par Inputs:
*Six inputs, including:
*@li coord_data: An NCHW tensor of type float16 or float32. The outputs of
* operator Yolo at the preceding layer (that is, one Yolo operator on YOLO v2)
* are used as the inputs of operator Yolov2DetectionOutput.For details, see the
* description of operator Yolo.
*@li obj_prob: An NCHW tensor of type float16 or float32. The outputs of
 operator Yolo at the preceding layer (that is, one Yolo operator on YOLO v2)
* are used as the inputs of operator Yolov2DetectionOutput.For details, see the
* description of operator Yolo.
*@li classes_prob: An NCHW tensor of type float16 or float32. The outputs of
* operator Yolo at the preceding layer (that is, one Yolo operator on YOLO v2)
* are used as the inputs of operator Yolov2DetectionOutput.For details, see the
* description of operator Yolo.
*@li img_info: A float16 or float32, describing the image information including
* the required image height and width and the actual image height and width.
*@li windex: A windex tensor with shape [height, weight]. Has the same type as
* the inputs. [[0,1,2...(weight-1)],[0,1,2...(w-1)]...[0,1,2...(weight-1)]]
* consisting of h groups of [0, 1, 2...(weight-1)] is formed.
*@li hindex: A hindex tensor with shape [height, weight]. Has the same type as
* the inputs. [[0,0...0],[1,1...1],[2,2...2]...[height-1,height-1...,height-1]]. \n
*
*@par Attributes:
*@li biases: A required float. "biases = Number of Yolo operators at the
* preceding layer x 2 x boxes".
*@li boxes: An optional int, specifying the number of anchor boxes predicted for
* each Yolo layer. Defaults to "5".
*@li coords: An optional int, Specifies the number of coordinate parameters.
* Must be 4.
*@li classes: An optional int, specifying the number of classes to be predicted.
* The value range is [1, 20]. Defaults to "20".
*@li relative: An optional bool. Defaults to and must be "true".
*@li obj_threshold: An optional float, specifying the confidence threshold for
* box filtering, which is the output "obj" of operator Yolo). The value range is
* [0.0, 1.0]. Defaults to "0.5".
*@li post_nms_topn: An optional int. This attribute is reserved. Defaults to
* "512".
*@li score_threshold: An optional float, specifying the class score threshold for
* box filtering, which is the output "class" of operator Yolo). The value range
* is [0.0, 1.0] . Defaults to "0.5".
*@li iou_threshold: An optional float, specifying the intersection-over-union
* (IOU) threshold for box filtering. The value range is [0.0, 1.0]. Defaults to
* "0.45".
*@li pre_nms_topn: An optional int, specifying the number of boxes for
* non-maximum suppression (NMS). Defaults to "512". \n
*
*@par Outputs:
*@li boxout: A tensor of type float16 or float32 with
* shape [batch,6,post_nms_topn]. describing the information of each output box,
* In output shape, 6 means x1, y1, x2, y2, score, label(class). Output by the
* number of box_out_num.
*@li boxoutnum: A tensor of type int with shape [batch,8,1,1], specifying the
* number of output boxes. It means only the first one of the 8 numbers is valid,
* the number of valid boxes in each batch, the maximum number of valid boxes in
* each batch is 1024
*
*@attention Constraints:
*@li This operator applies only to the YOLO v2 network.
*@li The preceding layer of operator Yolov2DetectionOutput must be one Yolo
* operator . \n
...
*@par Inputs:
*The outputs of operator Yolo at the preceding layer (that is, three Yolo
*operators on YOLO v3) are used as the inputs of operator Yolov3DetectionOutput.
*A Yolo operator has three outputs: "coords", "obj", and "class". For details,
*see the description of operator Yolo.
*@li x: A tensor of type float16 or float32.
*@li windex: A windex tensor with shape [height,weight]. Has the same type as
*the input x.
* [[0,1,2...(weight-1)],[0,1,2...(w-1)]...[0,1,2...(weight-1)]] consisting of h
*groups of [0, 1, 2...(weight-1)]
* is formed for the three Yolo outputs, respectively .It's a dynamic input.
*@li hindex: A hindex tensor with shape [height,weight]. Has the same type as
*the input x. [[0,0...0],[1,1...1],[2,2...2]...[height-1,height-1...,height-1]]
*is formed for the three Yolo outputs, respectively . \n

*@par Attributes:
*@li biases: A float list. "biases = Number of Yolo operators at the preceding
*layer x 2 x boxes"
*@li boxes: A required int, specifying the number of anchor boxes predicted
*for each Yolo layer.
*@li coords: Specifies the number of coordinate parameters. Must be 4.
*@li classes: A required int, specifying the number of classes to be
*predicted. The value range is [1, 80].
*@li relative: An optional bool. Defaults to and must be "true".
*@li obj_threshold: A required float, specifying the confidence threshold for
*box filtering, which is the output "obj" of operator Yolo). The value range
*is [0.0, 1.0].
*@li post_nms_topn: An optional int. This attribute is reserved.
*@li score_threshold: A required float, specifying the class score threshold
*for box filtering, which is the output "class" of operator Yolo). The value
*range is [0.0, 1.0].
*@li iou_threshold: A required float, specifying the intersection-over-union
*(IOU) threshold for box filtering. The value range is [0.0, 1.0].
*@li pre_nms_topn: An optional int, specifying the number of boxes for
*non-maximum suppression (NMS). Defaults to "512". \n
*
*@par Outputs:
*@li box_out: A tensor of type float16 or float32 with shape
*[batch,6,post_nms_topn](out_box_dim == 3) or
*[batch, 6*post_nms_topn](out_box_dim == 2), describing the information of
*each output box.
* In output shape, 6 means x1, y1, x2, y2, score, label(class). Output by the
*number of box_out_num.
*@li box_out_num: A tensor of type int32 with shape [batch,8], specifying the
*number of output boxes.
* The output shape means only the first one of the 8 numbers is valid, the
*number of valid boxes in each batch, the maximum number of valid boxes in
*each batch is 1024. \n
*
*@attention Constraints:
*@li This operator applies only to the YOLO v3 network.
*@li The preceding layer of operator Yolov3DetectionOutput must be three Yolo
*operators.
*@see Yolo() \n
...
* @par Restrictions:
* Warning: THIS FUNCTION IS DEPRECATED. Please use YoloV3DetectionOutputV2
*instead.
*/
...
*@par Inputs:
*Inputs include:
* x: A Tensor. Supported type: float16, float32, int16, int8, uint8, int32, int64, bfloat16.
* Supported format: ND . \n
...
* @li y2: A Tensor. Indices of y1 in x. Dtype must be int32. Supported format: ND . \n
...
*@li pool_mode: An optional attribute of type string to indicate pooling mode. Defaults to "avg", only supports "avg". \n
...
*@par Inputs:
* Two inputs, including:
*@li x: A tensor of type float32, describing the result
* feature map . \n
*@li rois: A tensor of type float32, with shape
* [batch, 5, rois_num], describing the ROIs, each ROI consists of five
* elements: "batch_id", "x1", "y1", "x2", and "y2", which "batch_id" indicates
* the index of the input feature map, "x1", "y1", "x2", or "y2" must be
* greater than or equal to "0.0" . \n
...
* @par Inputs:
* @li dy : data of grad increment, a 1D Tensor of type float32 with
* shape (N,).
* @li bboxes: Bounding boxes, a 2D Tensor of type float32 with
* shape (4, N). "N" indicates the number of bounding boxes, and the value
* "4" refers to [x1, y1, x2, y2] or [x, y, w, h].
* @li gtboxes: Ground-truth boxes, a 2D Tensor of type float32
* with shape (4, M). "M" indicates the number of ground truth boxes, and
* the value "4" refers to [x1, y1, x2, y2] or [x, y, w, h] .
* @li atan_sub: Intermediate result of forward calculation,
* a 1D Tensor of type float32 with shape (N,). \n
...
* @par Outputs:
* @li dbboxes: A 2D Tensor of type float32 with shape [4, N].
* @li dgtboxes: A 2D Tensor of type float32 with shape [4, M].
*/
REG_OP(CIoUGrad)
    .INPUT(dy, TensorType({DT_FLOAT}))
    .INPUT(bboxes, TensorType({DT_FLOAT}))
    .INPUT(gtboxes, TensorType({DT_FLOAT}))
    .INPUT(atan_sub, TensorType({DT_FLOAT}))
    .OUTPUT(dbboxes, TensorType({DT_FLOAT}))
    .OUTPUT(dgtboxes, TensorType({DT_FLOAT}))
    .ATTR(trans, Bool, false)
    .ATTR(is_cross, Bool, true)
    .ATTR(mode, String, "iou")
...
* @li pooled_w: A required int32, specifying the pooled W. Must be greater
* than 0.
* @li spatial_scale_h: A required scaling factor for mapping the input
* coordinates of height to the ROI coordinates.
* @li spatial_scale_w: A required scaling factor for mapping the input
* coordinates of width to the ROI coordinates .
