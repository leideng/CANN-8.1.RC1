...
class KernelCache {
 public:
  ...
  void SetCache(uint64_t key, std::shared_ptr<T> value) {
    KERNEL_LOG_INFO("SetCache begin, key[%llu].", key);
    std::unique_lock<std::mutex> lock(kernel_mutex_);
    auto iter = kernel_cache_iter_.find(key);
    if (iter != kernel_cache_iter_.end()) {
      KERNEL_LOG_INFO("SetCache update cache, key[%llu].", key);
      ...
      if (capacity_ < kernel_cache_.size()) {
        ...
        KERNEL_LOG_INFO(
            "SetCache is full, pop last element, capacity[%u], k[%llu], delete "
            "k[%llu], kernel cache size[%zu], cache iter size[%zu].",
      ...
      KERNEL_LOG_INFO("SetCache success, key[%llu].", key);
      kernel_cache_.push_front(pair);
      kernel_cache_iter_[key] = kernel_cache_.begin();
    }
  }
