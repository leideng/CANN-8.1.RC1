 ...
 * \brief
 */
#ifndef OPS_COMMON_INC_SMALL_VECTOR_H
#define OPS_COMMON_INC_SMALL_VECTOR_H
#include <iterator>
#include <memory>
#include <algorithm>
#include "def_types.h"
...
bool operator==(const ops::SmallVector<T, N1, Alloc> &sv1, const ops::SmallVector<T, N2, Alloc> &sv2) {
  if (N1 != N2) {
    // There may be controversy here, because even if N is not the same, size, content can be exactly the same.
    return false;
  }
...
namespace std {
template<typename T, size_t N, typename Alloc = std::allocator<T>>
void swap(ops::SmallVector<T, N, Alloc> &sv1, ops::SmallVector<T, N, Alloc> &sv2) {
  sv1.swap(sv2);
}
}  // namespace std

#endif  // OPS_COMMON_INC_SMALL_VECTOR_H
