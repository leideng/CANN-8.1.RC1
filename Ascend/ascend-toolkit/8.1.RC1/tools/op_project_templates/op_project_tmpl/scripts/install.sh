...
handle()
{
    ...
    if [ ! -d ${sourcedir}/$vendordir/$1 ]; then
        log "[INFO] no need to handle ops $1 files"
        return 0
    fi
    ...
    if [ ! -d ${targetdir}/$vendor_name/$1 ];then
        ...
        for file_a in ${sourcedir}/$vendordir/$1/*; do
            file_b=${file_a##*/};
	          if [ "ls ${targetdir}/$vendor_name/$1" = "" ]; then
                ...
                do
                    ...
                    if [ "$orn" = n ]; then
                        ...
                        break;
	                      ...
	                      [ -n "${targetdir}/$vendor_name/$1/" ] && rm -rf "${targetdir}/$vendor_name/$1"/*
			                  break
                    else
...
handle_proto()
{
    ...
    if [ ! -f ${sourcedir}/$vendordir/custom.proto ]; then
        log "[INFO] no need to handle custom.proto files"
        return 0
    fi
...
if [ -n "${INSTALL_PATH}" ]; then
    ...
    check_path_exist_permission ${ASCEND_CUSTOM_OPP_PATH}
    if [ $? -ne 0 ];then
        exit 1
    fi
    Check_Install_Path_valid ${ASCEND_CUSTOM_OPP_PATH}
    if [ $? -ne 0 ];then
        exit 1
    fi
    targetdir="${ASCEND_CUSTOM_OPP_PATH}/vendors"
    mkdir -p ${targetdir}
    if [ $? -ne 0 ];then
        log "[ERROR] create ${targetdir} failed"
        return 1
    fi
...
if [ -n "${INSTALL_PATH}" ] && [ -d ${INSTALL_PATH} ]; then
    ...
    cp -rf ${scriptdir}/set_env.bash ${bin_path}
    if [ $? -ne 0 ]; then
...
if [ -d ${targetdir}/$vendor_name/op_impl/cpu/aicpu_kernel/impl/ ]; then
    chmod -R 440 ${targetdir}/$vendor_name/op_impl/cpu/aicpu_kernel/impl/* >/dev/null 2>&1
fi
