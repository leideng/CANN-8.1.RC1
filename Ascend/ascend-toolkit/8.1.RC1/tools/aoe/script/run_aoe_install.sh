#!/bin/bash
# Install the run package of aoe.
...
install_info="${common_parse_dir}/${PACKAGE_PATH}/${PACKAGE_NAME}/ascend_install.info"
...
##########################################################################
log "INFO" "step into run_aoe_install.sh ......"
...
new_install() {
    if [ ! -d "${sourcedir}" ]; then
        log "INFO" "no need to install ${PACKAGE_NAME} files."
        return 0
    fi
    output_progress 10
    ...
    # 执行安装
    custom_options="--custom-options=--common-parse-dir=${common_parse_dir},--common-parse-type=${common_parse_type},--stage=install"
    bash "$curpath/install_common_parser.sh" --package="aoe" --install --username="$username" --usergroup="$usergroup" --set-cann-uninstall \
