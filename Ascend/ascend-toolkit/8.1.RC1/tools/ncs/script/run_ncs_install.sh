#!/bin/bash
# Perform install for ncs package
...
ncs_user_type=$(get_install_param "ncs_user_type" "${install_info}")
username=$ncs_user_name
usergroup=$ncs_user_group
feature_type=$ncs_user_type
if [ "$username" == "" ]; then
...
output_progress() {
    new_echo "INFO" "${package_name} install upgradePercentage: $1%"
    log "INFO" "${package_name} install upgradePercentage: $1%"
}

##########################################################################
log "INFO" "step into run_ncs_install.sh ......"
...
new_install() {
    ...
    custom_options="--custom-options=--stage=install,--quiet=$is_quiet"
    bash "$curpath/install_common_parser.sh" --package=${package_name} --install --username="${username}" \
        --usergroup="${usergroup}" --set-cann-uninstall --version=$pkg_version --version-dir=$pkg_version_dir \
        ${setenv_option} ${in_install_for_all} --docker-root="${docker_root}" "${custom_options}" \
        "${common_parse_type}" "${input_install_dir}" "${curpath}/filelist.csv"
