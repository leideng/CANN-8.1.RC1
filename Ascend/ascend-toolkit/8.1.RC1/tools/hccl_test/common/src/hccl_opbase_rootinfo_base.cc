...
{
    if((dtype == HCCL_DATA_TYPE_INT8 || dtype == HCCL_DATA_TYPE_UINT8) && rank_size >= 128) {
        check = 0; //不进行校验
        if (rank_id == root_rank && print_dump) {
...
void HcclOpBaseTest::is_data_overflow()
{
    if (op_type == HCCL_REDUCE_PROD) {
        ...
        if ((dtype == HCCL_DATA_TYPE_INT8 || dtype == HCCL_DATA_TYPE_UINT8) && rank_size >= 7) {
            no_verification();
        }
        if ((dtype == HCCL_DATA_TYPE_INT32 || dtype == HCCL_DATA_TYPE_UINT32)  && rank_size >= 31) {
            no_verification();
        }
        if ((dtype == HCCL_DATA_TYPE_INT64 || dtype == HCCL_DATA_TYPE_UINT64) && rank_size >= 63) {
            no_verification();
        }
    } else if (op_type == HCCL_REDUCE_SUM) {
        if((dtype == HCCL_DATA_TYPE_INT8 || dtype == HCCL_DATA_TYPE_UINT8) && rank_size >= 63) {
            no_verification();
        }
    }
...
void HcclOpBaseTest::print_execution_time(double average_time_us, double algorithm_bandwith_GBytes_s)
{
    ...
    if (check_err != 0)
    {
        ...
        printf("rank id %d, check result failed, %-17llu | %-14.2f | %-20.5f | failed\n", rank_id, data->data_size, 
